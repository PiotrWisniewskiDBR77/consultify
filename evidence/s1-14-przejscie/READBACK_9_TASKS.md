...
Zrzuty `screens/tasks-nine-literal-readback.png` i `screens/tasks-ideas-literal-readback.png` pokazują dziewięć tytułów w tabeli oraz właściciela. Test komponentu dowodzi trzech elementów kontraktu Document → task: różne `sourceId`, stabilny klucz przy retry oraz pojedynczy zapis przy podwójnym kliknięciu. UI pełnej karty nie pokazuje dziś `sourceType/sourceId`; ten fragment przewodu pozostaje `NOT_PROVEN` w zachowaniu i nie jest przedstawiany jako domknięty.
