...
export const useArtifactsStore = create<ArtifactsState>()(
  persist(
    (set, get) => ({
      ...
      // Actions
      addArtifact: (artifact, conversationId) => set((state) => {
      ...
      updateArtifact: (id, content) => set((state) => {
        const updateInArray = (artifacts: Artifact[]) =>
          artifacts.map(a => 
            a.id === id 
              ? { ...a, content, version: a.version + 1, updatedAt: new Date() }
              : a
          );
      ...
      toggleFullscreen: (fullscreen) => set((state) => ({
      ...
      loadConversationArtifacts: (conversationId) => set((state) => {
