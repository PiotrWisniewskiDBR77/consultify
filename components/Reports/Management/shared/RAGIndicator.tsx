...
export const RAGIndicator: React.FC<RAGIndicatorProps> = ({
    ...
    if (showLabel) {
        return (
            <span
                className={`inline-flex items-center gap-2 ${sizes.pill} rounded-full ${config.bg} bg-opacity-10 ${config.text} ${className}`}
            >
...
export const RAGStatusGrid: React.FC<RAGStatusGridProps> = ({ schedule, budget, scope, risk, className = '' }) => {
    const items = [
        ...
        <div className={`grid grid-cols-4 gap-4 ${className}`}>
            {items.map((item) => (
                ...
                >
                    ...
                    <span
                        className={`mt-2 text-sm font-semibold ${statusConfig[item.status]?.text || 'text-slate-400'}`}
                    >
