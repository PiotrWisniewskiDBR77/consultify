...
export const DecisionBoard: React.FC<DecisionBoardProps> = ({
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'ESCALATED': return 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400';
            case 'DEFERRED': return 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400';
    ...
    const renderDecisionCard = (decision: Decision) => {
        const isOverdue = decision.status === 'PENDING' && decision.dueDate && getDaysOverdue(decision.dueDate) > 0;
                ...
                <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                            <span className={`text-xs font-medium px-2 py-0.5 rounded ${getPriorityColor(decision.priority)}`}>
                                {decision.priority}
                            </span>
                            <span className={`text-xs font-medium px-2 py-0.5 rounded ${getStatusColor(decision.status)}`}>
                                {decision.status}
                            </span>
                            {decision.isBlocking && (
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-4 p-4 bg-white dark:bg-navy-900 rounded-xl border border-slate-200 dark:border-white/10">
                ...
                <div className="ml-auto flex items-center gap-2">
                    <select
                        ...
                        className="px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-navy-950 text-navy-900 dark:text-white"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {isLoading ? (
                    ...
                    <div className="col-span-full flex flex-col items-center justify-center h-32 text-slate-400">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white dark:bg-navy-900 rounded-xl w-full max-w-lg p-6 m-4">
                        ...
                        <textarea
                            ...
                            className="w-full p-3 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-navy-950 text-navy-900 dark:text-white mb-4"
                            rows={3}
                        />
