...
export const useStudioDocument = (options: UseStudioDocumentOptions = {}) => {
    ...
    // Save document
    const saveDocument = useCallback(async (createSnapshot = false) => {
    ...
    // Create new document
    const createDocument = useCallback(async (data: Partial<StudioDocument>) => {
        setLoading(true);
        try {
    ...
    const updateMetadata = useCallback(async (metadata: Partial<StudioDocument>) => {
        if (!document?.id) return;
