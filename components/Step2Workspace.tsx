...
export const Step2Workspace: React.FC<Step2WorkspaceProps> = ({ profile, sessionData, onNextStep }) => {
    return (
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
                    ...
                    <div className="space-y-4">
                        {sessionData.strategicGoals && sessionData.strategicGoals.length > 0 ? (
                            sessionData.strategicGoals.map((goal) => (
                                <div
                                    key={goal.id}
                                    className="p-4 bg-white/5 rounded-lg border border-white/5 flex flex-col gap-2"
                                >
                ...
                <div className="bg-navy-950/50 border border-white/10 rounded-xl p-6 relative">
                    ...
                    <div className="p-4 bg-white/5 rounded-lg border border-white/5 min-h-[80px]">
                        <p className="text-slate-300 italic">{sessionData.successCriteria || 'Not defined yet.'}</p>
                    </div>
                </div>
            </div>
