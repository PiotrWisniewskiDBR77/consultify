...
const getDefaultCommands = (onNavigate: (view: AppView) => void): CommandItem[] => [
  ...
  },
  {
    ...
    onSelect: () => onNavigate(AppView.FULL_STEP6_REPORTS),
  },
  {
    id: 'nav-studio',
    label: 'Studio',
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          {/* Palette */}
          <motion.div
            className="relative w-full max-w-xl mx-4 bg-white dark:bg-navy-900 rounded-2xl shadow-2xl overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-200 dark:border-white/10">
              ...
              />
              <kbd className="hidden sm:flex items-center gap-1 px-2 py-1 text-xs font-medium text-slate-400 bg-slate-100 dark:bg-white/5 rounded-lg">
                <Command size={12} />K
              </kbd>
            </div>
                      ...
                      {items.map((cmd) => {
                        ...
                        return (
                          <button
                            ...
                            `}
                          >
                            <span className={`flex-shrink-0 ${isSelected ? '' : 'text-slate-400 dark:text-slate-500'}`}>
                            ...
                            {cmd.shortcut && (
                              <kbd className="flex-shrink-0 px-2 py-0.5 text-xs font-medium text-slate-400 bg-slate-100 dark:bg-white/5 rounded">
                                {cmd.shortcut}
                              </kbd>
                            )}
                            <ArrowRight size={14} className={`flex-shrink-0 ${isSelected ? 'opacity-100' : 'opacity-0'}`} />
                          </button>
                        );
                      })}
                    </div>
            ...
            {/* Footer */}
            <div className="flex items-center justify-between px-4 py-2 border-t border-slate-200 dark:border-white/10 text-xs text-slate-400">
