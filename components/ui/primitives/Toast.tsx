  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-sm ${toast.title ? 'text-slate-500 dark:text-slate-400' : 'text-navy-900 dark:text-white'}`}>
          {toast.message}
        </p>
      </div>
      ...
      {toast.dismissible && (
        <button
          onClick={onDismiss}
          className="flex-shrink-0 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-white/5"
        >
          <X size={16} />
        </button>
      )}
