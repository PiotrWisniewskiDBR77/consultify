    ...
    (
        ...
        return (
            <div className={`${fullWidth ? 'w-full' : ''} ${containerClassName}`}>
                {label && (
                    <label className="block mb-2 text-sm font-medium text-navy-900 dark:text-white">{label}</label>
                )}
            ...
            `
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                        {/* Status icons */}
                        {hasError && !isPassword && <AlertCircle size={iconSize} className="text-danger-500" />}
                        {hasSuccess && !isPassword && <CheckCircle size={iconSize} className="text-success-500" />}
