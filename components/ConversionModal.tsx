...
const ConversionModal: React.FC<ConversionModalProps> = ({ isOpen, onClose, triggerReason, title, message }) => {
    ...
    const getContent = () => {
        if (triggerReason === 'time_limit') {
            ...
            return {
                ...
                subtext:
                    message ||
                    'This action attempts to modify data. In Demo Mode, the environment is read-only to ensure a consistent experience for everyone.',
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in duration-300">
                ...
                <div className="p-8 flex flex-col items-center text-center">
                    ...
                    <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{content.headline}</h2>

                    <p className="text-slate-600 dark:text-slate-300 mb-8 leading-relaxed">{content.subtext}</p>
                    ...
                    <button onClick={onClose} className="mt-6 text-sm text-slate-400 hover:text-slate-500 underline">
