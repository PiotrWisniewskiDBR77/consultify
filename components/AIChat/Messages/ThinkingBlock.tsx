...
import { Brain, CheckCircle2, ChevronDown, ChevronUp, FileText, Loader2, Search, Sparkles } from 'lucide-react';
...
export const ThinkingBlock: React.FC<ThinkingBlockProps> = ({
    ...
    // Total duration
    const totalDuration = useMemo(() => steps.reduce((acc, step) => acc + (step.durationMs || 0), 0), [steps]);
            ...
            >
                ...
                <div className="flex-1 text-left">
                    ...
                    {currentStep && isStreaming && (
                        <span className="ml-2 text-xs text-slate-500 dark:text-slate-400">{currentStep.label}</span>
                    )}
                </div>
            ...
            {isExpanded && (
                <div className="mt-2 px-3 py-2 bg-slate-50 dark:bg-navy-800/50 rounded-lg border border-slate-100 dark:border-navy-700 space-y-2">
                    {steps.map((step, index) => (
                        <ThinkingStepItem key={step.id} step={step} index={index} isLast={index === steps.length - 1} />
                    ))}
        ...
        `}
            ...
            <div className="flex-1 min-w-0 pb-2">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{step.label}</span>
                ...
                {step.content && (
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{step.content}</p>
                )}
            </div>
        </div>
    );
