...
const FRAMEWORK_BADGES: Record<string, { color: string; label: string }> = {
  'ISO': { color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300', label: 'ISO 21500' },
  'PMBOK': { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300', label: 'PMBOK 7' },
  'PRINCE2': { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300', label: 'PRINCE2' }
};
...
const RACIMatrixRenderer: React.FC<{ data: any }> = ({ data }) => {
  ...
  const getRACIColor = (value: string) => {
    switch (value?.toUpperCase()) {
      ...
      case 'A': return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300';
      case 'C': return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300';
      case 'I': return 'bg-slate-100 text-slate-700 dark:bg-slate-700/30 dark:text-slate-300';
      default: return '';
    }
  };
    ...
    <div className="overflow-auto">
        ...
        <thead>
          <tr className="bg-slate-50 dark:bg-navy-800">
            ...
            {roles.map((role: string, idx: number) => (
              <th key={idx} className="px-3 py-3 text-center text-sm font-semibold text-slate-700 dark:text-slate-300 border-b border-slate-200 dark:border-navy-700 min-w-[100px]">
                {role}
              </th>
            ))}
          </tr>
          ...
          {tasks.map((task: string, taskIdx: number) => (
            <tr key={taskIdx} className="hover:bg-slate-50 dark:hover:bg-navy-800/50">
              ...
              {roles.map((role: string, roleIdx: number) => {
                ...
                return (
                  <td key={roleIdx} className="px-3 py-3 text-center border-b border-slate-200 dark:border-navy-700">
                    {value && (
                      <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${getRACIColor(value)}`}>
                        {value.toUpperCase()}
                      </span>
                    )}
                  </td>
      ...
      <div className="flex flex-wrap gap-4 mt-4 px-4 py-3 bg-slate-50 dark:bg-navy-800/50 rounded-b-lg">
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${getRACIColor('R')}`}>R</span>
          <span className="text-sm text-slate-600 dark:text-slate-400">{t('pmo.responsible', 'Responsible')}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${getRACIColor('A')}`}>A</span>
          <span className="text-sm text-slate-600 dark:text-slate-400">{t('pmo.accountable', 'Accountable')}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${getRACIColor('C')}`}>C</span>
          <span className="text-sm text-slate-600 dark:text-slate-400">{t('pmo.consulted', 'Consulted')}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${getRACIColor('I')}`}>I</span>
          <span className="text-sm text-slate-600 dark:text-slate-400">{t('pmo.informed', 'Informed')}</span>
        </div>
      </div>
    </div>
  );
...
const RiskRegisterRenderer: React.FC<{ data: any }> = ({ data }) => {
  ...
  const getRiskColor = (level: string) => {
    switch (level?.toLowerCase()) {
      case 'critical': case 'very high': return 'bg-red-500 text-white';
  ...
  const getStatusIcon = (status: string) => {
    switch (status?.toLowerCase()) {
      ...
      case 'mitigated': return <Shield size={14} className="text-blue-500" />;
  ...
  return (
    <div className="space-y-4 p-4">
      {risks.map((risk: any, idx: number) => (
        <div key={idx} className="border border-slate-200 dark:border-navy-700 rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 bg-slate-50 dark:bg-navy-800/50">
            <div className="flex items-center gap-3">
              <span className={`px-2 py-1 rounded text-xs font-semibold ${getRiskColor(risk.level)}`}>
                {risk.level || 'Unknown'}
              </span>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-600 dark:text-slate-400">{risk.status || 'Open'}</span>
            </div>
          </div>
          <div className="px-4 py-3 space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <p className="text-sm text-slate-700 dark:text-slate-300 mt-1">{risk.probability || '—'}</p>
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">
                  {t('pmo.impact', 'Impact')}
                </span>
                <p className="text-sm text-slate-700 dark:text-slate-300 mt-1">{risk.impact || '—'}</p>
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">
                  {t('pmo.owner', 'Owner')}
                </span>
                <p className="text-sm text-slate-700 dark:text-slate-300 mt-1">{risk.owner || '—'}</p>
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">
                  {t('pmo.dueDate', 'Due Date')}
                </span>
                <p className="text-sm text-slate-700 dark:text-slate-300 mt-1">{risk.dueDate || '—'}</p>
              </div>
            </div>
            {risk.mitigation && (
              <div>
...
const StatusReportRenderer: React.FC<{ data: any }> = ({ data }) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status?.toLowerCase()) {
      case 'green': case 'on track': return 'bg-green-500';
      case 'amber': case 'yellow': case 'at risk': return 'bg-amber-500';
      case 'red': case 'off track': return 'bg-red-500';
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white">{data.projectName || 'Project Status Report'}</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {data.reportDate ? new Date(data.reportDate).toLocaleDateString() : 'Latest Report'}
          </p>
        </div>
        <div className={`w-12 h-12 rounded-full ${getStatusColor(data.overallStatus)}`} title={data.overallStatus} />
      </div>
        ...
        {[
          ...
          <div key={idx} className="bg-slate-50 dark:bg-navy-800/50 rounded-lg p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-700 dark:text-slate-300">{metric.value?.status || '—'}</span>
            </div>
          </div>
        ))}
      </div>
...
export const PMODocumentRenderer: React.FC<PMODocumentRendererProps> = ({
  ...
  const renderDocument = () => {
    switch (docType) {
      ...
      default:
        return <GenericDocumentRenderer content={typeof content === 'string' ? content : JSON.stringify(content, null, 2)} />;
    }
  };
    ...
    <div className={`flex flex-col h-full bg-white dark:bg-navy-900 ${className}`}>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-navy-700 bg-slate-50 dark:bg-navy-800/50">
        <div className="flex items-center gap-3">
          <span className="text-brand">
            {TEMPLATE_ICONS[docType] || TEMPLATE_ICONS.generic}
          </span>
          <h3 className="font-semibold text-slate-900 dark:text-white">
            {document.title || t(`pmo.templates.${docType}`, docType.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()))}
          </h3>
        </div>
        ...
        <div className="flex items-center gap-2">
          {framework && FRAMEWORK_BADGES[framework] && (
            <span className={`px-2 py-1 rounded text-xs font-semibold ${FRAMEWORK_BADGES[framework].color}`}>
      ...
      <div className="px-4 py-2 border-t border-slate-200 dark:border-navy-700 bg-slate-50 dark:bg-navy-800/50">
        <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <span>
            {t('pmo.generatedBy', 'Generated by Consultify AI')}
          </span>
          {document.version && (
            <span>
