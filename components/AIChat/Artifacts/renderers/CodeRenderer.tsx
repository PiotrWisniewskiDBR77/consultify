...
const tokenize = (code: string, language: string): { type: string; value: string }[][] => {
  ...
  const patterns: Record<string, Array<{ pattern: RegExp; type: string }>> = {
    javascript: [
      ...
      { pattern: /\b(const|let|var|function|return|if|else|for|while|class|extends|import|export|from|async|await|try|catch|throw|new|this)\b/g, type: 'keyword' },
    ...
    typescript: [
      ...
      { pattern: /\/\*[\s\S]*?\*\//g, type: 'comment' },
      { pattern: /"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`/g, type: 'string' },
      { pattern: /\b(const|let|var|function|return|if|else|for|while|class|extends|import|export|from|async|await|try|catch|throw|new|this|type|interface|enum|implements|public|private|protected|readonly|abstract)\b/g, type: 'keyword' },
      { pattern: /\b(true|false|null|undefined|NaN|Infinity|string|number|boolean|any|void|never)\b/g, type: 'literal' },
      { pattern: /\b\d+\.?\d*\b/g, type: 'number' },
      { pattern: /[{}[\]();,.:]/g, type: 'punctuation' },
    ],
    python: [
      ...
      { pattern: /"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'/g, type: 'string' },
      { pattern: /\b(def|class|return|if|elif|else|for|while|import|from|as|try|except|raise|with|lambda|yield|async|await|pass|break|continue|in|is|not|and|or)\b/g, type: 'keyword' },
    ...
    sql: [
      ...
      { pattern: /\/\*[\s\S]*?\*\//g, type: 'comment' },
      { pattern: /'(?:[^'\\]|\\.)*'/g, type: 'string' },
      { pattern: /\b(SELECT|FROM|WHERE|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|TABLE|INDEX|JOIN|LEFT|RIGHT|INNER|OUTER|ON|AND|OR|NOT|NULL|AS|ORDER|BY|GROUP|HAVING|LIMIT|OFFSET|UNION|DISTINCT|PRIMARY|KEY|FOREIGN|REFERENCES|DEFAULT|VALUES|INTO|SET)\b/gi, type: 'keyword' },
      { pattern: /\b\d+\.?\d*\b/g, type: 'number' },
    ],
    json: [
    ...
    <div className={`bg-slate-900 dark:bg-navy-950 rounded-lg overflow-hidden ${className}`}>
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-slate-800 dark:bg-navy-900 border-b border-slate-700 dark:border-navy-800">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-2 py-1 text-xs text-slate-400 hover:text-slate-300 hover:bg-slate-700 rounded transition-colors"
          >
            {copied ? (
              <>
              ...
              <div key={lineIndex} className="flex">
                ...
                <span className="flex-1">
                  {lineTokens.map((token, tokenIndex) => (
                    <span key={tokenIndex} className={TOKEN_COLORS[token.type] || TOKEN_COLORS.plain}>
                      {token.value}
                    </span>
                  ))}
                </span>
      ...
      {/* Footer with stats */}
      <div className="px-4 py-1.5 bg-slate-800 dark:bg-navy-900 border-t border-slate-700 dark:border-navy-800 text-xs text-slate-500">
        {tokens.length} {t('code.lines', 'lines')} • {content.length} {t('code.characters', 'chars')}
      </div>
    </div>
  );
};
