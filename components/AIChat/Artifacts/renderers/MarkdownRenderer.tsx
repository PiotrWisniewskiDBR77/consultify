...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ 
  ...
  const components = useMemo(() => ({
    ...
    code: ({ inline, className: codeClassName, children }: any) => {
      ...
      return (
        <code className={`block bg-slate-900 dark:bg-navy-950 text-slate-100 p-4 rounded-lg overflow-x-auto font-mono text-sm ${codeClassName}`}>
          {children}
        </code>
      );
    },
    ...
    // Horizontal rule
    hr: () => (
      <hr className="my-6 border-slate-200 dark:border-navy-700" />
    ),
