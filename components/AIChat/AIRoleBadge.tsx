import { FileEdit, Shield, Zap } from 'lucide-react';
...
export const AIRoleBadge: React.FC<AIRoleBadgeProps> = ({ role, size = 'sm', showDescription = false }) => {
    const config = roleConfig[role] || roleConfig.ADVISOR;
    const Icon = config.icon;

    return (
        <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border ${config.color} ${
