...
export const ProfileSocialSettings: React.FC<ProfileSocialSettingsProps> = ({ currentUser, onUpdateUser }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setSaveStatus('error');
            toast.error(error.message || t('settings.profile.social.error', 'Failed to update social links'));
        } finally {
            setIsSaving(false);
        }
    };
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-slate-400">twitter.com/</span>
                        <input
                            type="text"
                            value={twitter.replace('https://twitter.com/', '').replace('@', '')}
                            onChange={(e) => setTwitter(e.target.value.replace('@', ''))}
                            placeholder={t('settings.profile.social.twitterPlaceholder', 'username')}
                            className="flex-1 px-3 py-2 bg-slate-50 dark:bg-navy-950/50 border border-slate-200 dark:border-white/10 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500/50 outline-none"
                        />
                    </div>
                </div>
                ...
                <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-slate-400">github.com/</span>
                        <input
                            type="text"
