...
const AccountManagementSettings: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <div className={`space-y-8 ${className}`}>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              {t('settings.account.exportDesc', 'Download a copy of all your data including projects, assessments, and settings.')}
            </p>
            <button
            ...
            >
              <Download size={16} />
              {exporting ? t('common.exporting', 'Exporting...') : t('settings.account.export', 'Export Data')}
            </button>
          </div>
        </div>
      </div>
      ...
      {/* Delete Account */}
      <div className="p-6 border border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-900/10 rounded-xl">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            {!showDeleteConfirm ? (
              ...
              <div className="mt-4 p-4 bg-white dark:bg-navy-800 rounded-lg border border-red-200 dark:border-red-800">
                <div className="flex items-center gap-2 text-red-600 mb-3">
                  <AlertTriangle size={18} />
                  <span className="font-medium">{t('settings.account.confirmTitle', 'Are you sure?')}</span>
                </div>
