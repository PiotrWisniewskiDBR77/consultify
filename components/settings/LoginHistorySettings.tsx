...
export const LoginHistorySettings: React.FC<LoginHistorySettingsProps> = ({ className = '' }) => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      setHistory([
        { id: '1', timestamp: new Date().toISOString(), status: 'success', location: 'Warsaw, Poland', ip: '192.168.1.1', device: 'Chrome on Windows' },
        { id: '2', timestamp: new Date(Date.now() - 86400000).toISOString(), status: 'success', location: 'Warsaw, Poland', ip: '192.168.1.1', device: 'Safari on iOS' },
        { id: '3', timestamp: new Date(Date.now() - 172800000).toISOString(), status: 'failed', location: 'Unknown', ip: '10.0.0.1', device: 'Unknown' },
      ]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="space-y-2">
          {(history || []).map((event) => (
            ...
            >
              ...
              <span className="text-sm text-slate-400">{formatDate(event.timestamp || new Date().toISOString())}</span>
            </div>
          ))}
        </div>
      )}
