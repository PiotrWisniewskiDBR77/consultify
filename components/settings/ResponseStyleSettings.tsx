...
export const ResponseStyleSettings: React.FC<ResponseStyleSettingsProps> = ({ className = '' }) => {
  ...
  const [responseLength, setResponseLength] = useState<typeof RESPONSE_LENGTHS[number]>('balanced');
          ...
          {RESPONSE_LENGTHS.map((length) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="font-medium text-slate-900 dark:text-white capitalize">{t(`settings.ai.length.${length}`, length)}</span>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {TONES.map((t_) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="font-medium text-slate-900 dark:text-white capitalize">{t(`settings.ai.tone.${t_}`, t_)}</span>
