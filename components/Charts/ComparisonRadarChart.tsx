...
const CustomTooltip = ({
    ...
    return (
        <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-xl p-3 min-w-[200px]">
            <p className="font-medium text-white text-sm mb-2 pb-2 border-b border-slate-700">{data.axis}</p>
                    ...
                    return (
                        <div key={analysis.id} className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-2">
                                <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: analysis.color }} />
                                <span className="text-slate-400 text-xs truncate max-w-[120px]">{analysis.name}</span>
                            </div>
    ...
    if (analyses.length === 0) {
        return (
            <div className={`flex items-center justify-center ${className}`} style={{ height }}>
                <p className="text-slate-500">Wybierz analizy do porównania</p>
            </div>
        );
    }
        ...
        <div className={`relative ${className}`}>
            ...
            <div className="flex flex-wrap gap-4 mb-4 px-4">
                {summaryData.map((item) => (
                    <div key={item.id} className="flex items-center gap-2 bg-slate-800/50 rounded-lg px-3 py-1.5">
                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: item.color }} />
                        <span className="text-slate-400 text-sm truncate max-w-[100px]">{item.name}</span>
