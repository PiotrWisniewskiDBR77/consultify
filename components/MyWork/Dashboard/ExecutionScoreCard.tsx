...
export const ExecutionScoreCard: React.FC<Partial<ExecutionScoreCardProps> & { className?: string }> = ({
    ...
    if (compact) {
        return (
            <div
                className={`flex items-center gap-4 p-4 bg-white dark:bg-navy-900 rounded-xl border border-slate-200 dark:border-white/10 ${className}`}
            >
                {/* Score */}
                ...
                {showStreak && (
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400">{t('myWork.dashboard.streak', 'streak')}</p>
                        </div>
                    </div>
                )}
