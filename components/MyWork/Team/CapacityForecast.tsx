...
const ForecastDay: React.FC<{
    ...
    const getCapacityBgColor = (capacity: number) => {
        if (capacity > 100) return 'bg-rose-50 dark:bg-rose-900/20 border-rose-200 dark:border-rose-500/30';
        if (capacity > 85) return 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-500/30';
        return 'bg-white dark:bg-navy-900 border-slate-200 dark:border-white/10';
    };
...
export const CapacityForecast: React.FC<CapacityForecastProps> = ({ forecasts = [], loading = false, onDayClick }) => {
    ...
    if (loading) {
        return (
            <div className="bg-white dark:bg-navy-900 rounded-2xl border border-slate-200 dark:border-white/10 p-5 animate-pulse">
