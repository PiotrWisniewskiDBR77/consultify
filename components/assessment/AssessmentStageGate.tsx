...
export const AssessmentStageGate: React.FC<Props> = ({ projectId, currentPhase, onPhaseChange }) => {
            ...
            <div className="px-6 py-4 border-b border-slate-200 dark:border-white/10 bg-gradient-to-r from-slate-50 to-white dark:from-navy-800 dark:to-navy-900">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-semibold text-navy-900 dark:text-white">Stage Gate Control</h3>
                    ...
                    <div className="text-sm text-slate-600 dark:text-slate-400">
                        Current:{' '}
                        <span className="font-medium text-purple-600 dark:text-purple-400">{currentPhase}</span>
                    </div>
                </div>
            </div>
        ...
        <div className="mt-6 p-4 bg-slate-50 dark:bg-navy-800 rounded-lg border border-slate-200 dark:border-white/10">
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                    ...
                    >
                        {isReady ? <Unlock className="w-5 h-5 text-white" /> : <Lock className="w-5 h-5 text-white" />}
                    </div>
                    <div>
                        <h4 className="font-semibold text-navy-900 dark:text-white">{gateInfo.title}</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{gateInfo.description}</p>
                    </div>
                </div>
            ...
            <div className="flex justify-end gap-2">
                {isReady && (
                    ...
                    >
                        {isPassing ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                        Pass Gate
                    </button>
                )}
            </div>
