                ...
                {DBR77_WASTES.map(waste => {
                    ...
                    return (
                        <div key={waste.id} className="relative group">
                            ...
                            >
                                <Icon size={20} className={`mx-auto mb-1 ${isSelected ? `text-${waste.color}-500` : 'text-slate-400'}`} />
                                <div className={`text-xs font-medium ${isSelected ? `text-${waste.color}-700 dark:text-${waste.color}-400` : 'text-slate-500'}`}>
                                    {waste.name}
                                </div>
                            </button>
        ...
        <div className={`bg-white dark:bg-navy-950/50 rounded-xl border-2 transition-all ${
            ...
            <div className="p-4 flex items-center justify-between">
                <button onClick={onToggleExpand} className="flex items-center gap-3 flex-1">
                    <div className={`w-10 h-10 rounded-lg bg-${phaseConfig.color}-100 dark:bg-${phaseConfig.color}-900/30 flex items-center justify-center`}>
                    ...
                    <div className="text-left">
                        ...
                        <p className="text-xs text-slate-500">{process.department} • {process.category}</p>
                    </div>
                </button>
                <div className="flex items-center gap-2">
                    ...
                    {activePhase === 'MEASURE' && (
                        <div className="text-right text-xs">
                            <div className="text-slate-500">OEE: <span className="font-bold">{process.currentState.oee}%</span></div>
                            <div className="text-slate-500">Lead: <span className="font-bold">{process.currentState.leadTime}d</span></div>
                        </div>
                    )}
                    {activePhase === 'OPTIMIZE' && (
                        <div className="text-right text-xs">
                            <div className="text-slate-500">5S: <span className="font-bold">{process.leanAssessment.fiveSLevel}/5</span></div>
                            <div className="text-slate-500">Waste: <span className="font-bold">{process.leanAssessment.wasteIdentified.length}</span></div>
                        </div>
                    )}
                    {activePhase === 'AUTOMATE' && (
                        <div className="text-right text-xs">
                            <div className="text-slate-500">Feasibility: <span className="font-bold">{process.automationPotential.feasibility}/5</span></div>
                            <div className="text-green-600">Savings: <span className="font-bold">{(process.automationPotential.estimatedSavings / 1000).toFixed(0)}k PLN</span></div>
                        </div>
                    )}
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => onChange({
                                        ...process,
                                        currentState: { ...process.currentState, cycleTime: Number(e.target.value) }
                                    })}
                                    disabled={readOnly}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => onChange({
                                        ...process,
                                        currentState: { ...process.currentState, defectRate: Number(e.target.value) }
                                    })}
                                    disabled={readOnly}
                                    className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                />
                    ...
                    {activePhase === 'OPTIMIZE' && (
                        <>
                            <WasteSelector
                                ...
                                onChange={(wastes, impact) => onChange({
                                    ...process,
                                    leanAssessment: {
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...process,
                                            leanAssessment: { ...process.leanAssessment, fiveSLevel: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...process,
                                            leanAssessment: { ...process.leanAssessment, visualManagement: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...process,
                                            leanAssessment: { ...process.leanAssessment, continuousFlow: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div className="flex items-end">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={(e) => onChange({
                                                ...process,
                                                leanAssessment: { ...process.leanAssessment, standardWorkDefined: e.target.checked }
                                            })}
                                            disabled={readOnly}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...process,
                                            automationPotential: { ...process.automationPotential, feasibility: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...process,
                                            automationPotential: { ...process.automationPotential, estimatedSavings: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...process,
                                            automationPotential: { ...process.automationPotential, implementationTime: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => onChange({
                                            ...process,
                                            automationPotential: { ...process.automationPotential, complexity: e.target.value as 'LOW' | 'MEDIUM' | 'HIGH' }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    >
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {DBR77_AUTOMATION_TECHNOLOGIES.map(tech => {
                                        const isSelected = process.automationPotential.recommendedTechnologies.includes(tech.id);
                                        return (
                                            <button
                                                key={tech.id}
                                                onClick={() => {
                                                    ...
                                                    const newTechs = isSelected
                                                        ? process.automationPotential.recommendedTechnologies.filter(t => t !== tech.id)
                                                        : [...process.automationPotential.recommendedTechnologies, tech.id];
                                                    onChange({
                                                        ...process,
                                                        automationPotential: { ...process.automationPotential, recommendedTechnologies: newTechs }
                                                    });
                                                }}
                                                disabled={readOnly}
            ...
            <div className="p-4 flex items-center justify-between">
                <button onClick={onToggleExpand} className="flex items-center gap-3 flex-1">
                    ...
                    <div className="text-left">
                        ...
                        <p className="text-xs text-slate-500">{workstation.department} • {workstation.headcount} FTE</p>
                    </div>
                </button>
                <div className="flex items-center gap-2">
                    {activePhase === 'AUTOMATE' && (
                        <span className={`px-2 py-1 rounded text-xs font-medium bg-${roleEvolutionConfig.color}-100 text-${roleEvolutionConfig.color}-700`}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => onChange({
                                        ...workstation,
                                        currentState: { ...workstation.currentState, tasksPerDay: Number(e.target.value) }
                                    })}
                                    disabled={readOnly}
                                    className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                />
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => onChange({
                                        ...workstation,
                                        currentState: { ...workstation.currentState, avgTaskTime: Number(e.target.value) }
                                    })}
                                    disabled={readOnly}
                                    className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                />
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => onChange({
                                        ...workstation,
                                        currentState: { ...workstation.currentState, errorRate: Number(e.target.value) }
                                    })}
                                    disabled={readOnly}
                                    className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                />
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => onChange({
                                        ...workstation,
                                        currentState: { ...workstation.currentState, overtimeHours: Number(e.target.value) }
                                    })}
                                    disabled={readOnly}
                                    className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                />
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...workstation,
                                            leanAssessment: { ...workstation.leanAssessment, workplaceOrganization: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...workstation,
                                            leanAssessment: { ...workstation.leanAssessment, crossTraining: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...workstation,
                                            leanAssessment: { ...workstation.leanAssessment, kaizen: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div className="flex items-end">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={(e) => onChange({
                                                ...workstation,
                                                leanAssessment: { ...workstation.leanAssessment, standardizedWork: e.target.checked }
                                            })}
                                            disabled={readOnly}
                                            className="w-4 h-4 rounded"
                                        />
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...workstation,
                                            automationPotential: { ...workstation.automationPotential, taskAutomationPercent: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...workstation,
                                            automationPotential: { ...workstation.automationPotential, augmentationPercent: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onChange({
                                            ...workstation,
                                            automationPotential: { ...workstation.automationPotential, estimatedSavings: Number(e.target.value) }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    />
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => onChange({
                                            ...workstation,
                                            automationPotential: { ...workstation.automationPotential, roleEvolution: e.target.value as RoleEvolution }
                                        })}
                                        disabled={readOnly}
                                        className="w-full mt-1 px-3 py-2 border rounded-lg bg-white dark:bg-navy-900 text-sm"
                                    >
...
const DBR77LegalNotice: React.FC = () => (
    <div className="bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-500/30 rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-cyan-800 dark:text-cyan-200">
            ...
            potencjału automatyzacji i AI. Metoda pozwala na kompleksową analizę procesów i stanowisk pracy.
        </div>
    </div>
);
...
export const DBR77LeanMap: React.FC<DBR77LeanMapProps> = ({
    ...
    // Handlers
    const handleProcessChange = useCallback((updatedProcess: ProcessAssessment) => {
        setData(prev => {
            const newData = {
                ...prev,
    ...
    const handleWorkstationChange = useCallback((updatedWs: WorkstationAssessment) => {
        setData(prev => {
            const newData = {
                ...prev,
