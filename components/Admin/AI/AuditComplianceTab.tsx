...
export const AuditComplianceTab: React.FC = () => {
    ...
    const [activeSubTab, setActiveSubTab] = useState<'settings' | 'usage' | 'security' | 'compliance' | 'templates'>('settings');
    ...
    const getEventTypeIcon = (type: string) => {
        switch (type) {
            case 'pii_detected': return <Eye size={16} />;
            case 'injection_attempt': return <AlertTriangle size={16} />;
            case 'rate_limit': return <Activity size={16} />;
            case 'access_denied': return <Lock size={16} />;
            case 'budget_exceeded': return <AlertTriangle size={16} />;
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'compliant':
                return <span className="px-2 py-0.5 rounded-full text-xs bg-green-500/20 text-green-400">Compliant</span>;
            case 'partial':
                return <span className="px-2 py-0.5 rounded-full text-xs bg-amber-500/20 text-amber-400">Partial</span>;
            case 'non_compliant':
                return <span className="px-2 py-0.5 rounded-full text-xs bg-red-500/20 text-red-400">Non-Compliant</span>;
            default:
                return null;
        }
    };
        ...
        <div className="space-y-6">
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                                    {securityEvents.map(event => (
                                        <tr key={event.id} className="hover:bg-slate-50 dark:hover:bg-white/5">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px-2 py-0.5 rounded-full text-xs border ${getSeverityColor(event.severity)}`}>
                                                    {event.severity}
                                                </span>
                                            </td>
            ...
            {activeSubTab === 'templates' && showTemplateEditor && currentOrganization && (
                <CustomComplianceTemplateEditor
                    ...
                    onSave={(template) => {
                        if (editingTemplate) {
                            setCustomTemplates(prev => prev.map(t => t.id === template.id ? {
                                ...t,
                                name: template.name,
                                description: template.description,
                                basedOn: template.basedOn,
                                sectionsCount: template.sections.length,
                                checkpointsCount: template.sections.reduce((sum, s) => sum + s.checkpoints.length, 0),
