...
export const AuditComplianceTab: React.FC = () => {
    ...
    const [activeSubTab, setActiveSubTab] = useState<'settings' | 'usage' | 'security' | 'compliance' | 'templates'>(
        'settings',
    );
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'compliant':
                return (
                    <span className="px-2 py-0.5 rounded-full text-xs bg-green-500/20 text-green-400">Compliant</span>
                );
            case 'partial':
                return <span className="px-2 py-0.5 rounded-full text-xs bg-amber-500/20 text-amber-400">Partial</span>;
            case 'non_compliant':
                return (
                    <span className="px-2 py-0.5 rounded-full text-xs bg-red-500/20 text-red-400">Non-Compliant</span>
                );
            default:
                return null;
        }
