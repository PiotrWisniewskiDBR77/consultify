...
export const HealthMonitoringTab: React.FC = () => {
    ...
    const capabilities = [
        ...
        { id: 'chat_ready', name: 'Chat Ready', icon: '💬', description: 'Test if AI model is ready for conversation' },
            ...
            {activeSubTab === 'mission' && (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white dark:bg-navy-900/50 border border-slate-200 dark:border-white/10 p-6 rounded-xl shadow-sm dark:shadow-none">
                            <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase mb-2">Success Rate (Last 50)</h3>
                            <div className="flex items-baseline">
                                <span className="text-3xl font-bold text-navy-900 dark:text-white">
                                    {status?.metrics.uptime50?.toFixed(1) || '0.0'}%
                                </span>
                                <span className={`ml-2 text-sm font-medium ${
                                    status && status.metrics.uptime50 > 95 ? 'text-success-600 dark:text-green-400' : 'text-warning-600 dark:text-amber-400'
                                }`}>
                        ...
                        <div className="bg-white dark:bg-navy-900/50 border border-slate-200 dark:border-white/10 p-6 rounded-xl shadow-sm dark:shadow-none">
                            <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase mb-2">Avg Latency</h3>
                            <div className="flex items-baseline">
                                <span className="text-3xl font-bold text-navy-900 dark:text-white">
                        ...
                        <div className="bg-white dark:bg-navy-900/50 border border-slate-200 dark:border-white/10 p-6 rounded-xl shadow-sm dark:shadow-none">
                            <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase mb-2">Active Providers</h3>
                            <div className="flex flex-wrap gap-2 mt-2">
                                {status?.providers.filter(p => p.status === 'ACTIVE').map(p => (
                                    <span key={p.name} className="px-2 py-1 bg-success-500/20 text-success-700 dark:text-green-300 text-xs font-medium rounded-full border border-success-500/30 dark:border-transparent">
                                        {p.name}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div className="bg-white dark:bg-navy-900/50 border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden shadow-sm dark:shadow-none">
                        <div className="px-6 py-4 border-b border-slate-200 dark:border-white/10 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                            <div>
                                <h2 className="font-semibold text-navy-900 dark:text-white">AI Capability Diagnostics</h2>
                                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Test individual AI capabilities</p>
                            </div>
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    title={alertsEnabled ? 'Alerts enabled - SuperAdmins will be notified on failure' : 'Alerts disabled'}
                                >
                                ...
                                <div key={cap.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500 dark:text-slate-400">{cap.description}</p>
                                        </div>
                                    </div>
                                        ...
                                        {results[cap.id] && (
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase border ${
                                                    ...
                                                    {results[cap.id].details?.skipped ? 'SKIPPED' : results[cap.id].status} ({results[cap.id].latency}ms)
                                                </div>
                            ...
                            {Object.entries(results).reverse().map(([id, res]) => (
                                <div key={id} className="mb-3">
                                    <div className="flex flex-wrap gap-2 items-center">
                                        ...
                                        <span className={
                                            res.status === 'SUCCESS' 
                                                ? res.details?.skipped ? 'text-amber-400' : 'text-green-400' 
                                                : 'text-red-400'
                                        }>
                                    ...
                                    {res.details?.model && (
                                        <div className="text-gray-500 ml-4 mt-1">Model: {res.details.model}</div>
                                    )}
                                </div>
                            ))}
                        </div>
            ...
            {activeSubTab === 'analytics' && (
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-navy-900/50 border border-slate-200 dark:border-white/10 rounded-xl p-8 text-center shadow-sm dark:shadow-none">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 mt-1">Coming soon: Interactive usage charts</p>
                    </div>
                </div>
            )}
                                    ...
                                    <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors text-sm">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex flex-col">
                                                <span className="text-navy-900 dark:text-white font-medium">{log.provider}</span>
