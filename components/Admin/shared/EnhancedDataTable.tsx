...
import { Dropdown, DropdownContent, DropdownItem, DropdownTrigger } from '../../ui/primitives/Dropdown';
...
export const EnhancedDataTable = forwardRef<HTMLDivElement, EnhancedDataTableProps<Record<string, unknown>>>(
    <T extends Record<string, unknown>>(
        ...
        const isAllSelected =
            paginatedData.length > 0 && paginatedData.every((row, index) => selectedIds.has(getRowKey(row, index)));
        ...
        return (
            <div ref={ref} className={cn('w-full', className)}>
                {/* Toolbar */}
                {(selectable && selectedCount > 0) || title || toolbarContent || editableColumns || exportable ? (
                                ...
                                {displayColumns.map((column) => (
                                    <th
                                        ...
                                        className={cn(
                                            ...
                                            column.sortable &&
                                                'cursor-pointer select-none hover:bg-slate-100 dark:hover:bg-navy-800',
                                        )}
                                            ...
                                            >
                                                ...
                                                {rowActions && rowActions.length > 0 && (
                                                    ...
                                                    >
                                                        <Dropdown>
                                                            <DropdownTrigger asChild>
                                                                <button className="p-1 rounded hover:bg-slate-100 dark:hover:bg-navy-700 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
                ...
                >
                    ...
                    <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-slate-200 dark:border-navy-700">
                        <Button
                            ...
                            size="sm"
                            onClick={() => {
                                setVisibleColumns(new Set(columns.filter((c) => !c.hidden).map((c) => String(c.key))));
                            }}
                        >
