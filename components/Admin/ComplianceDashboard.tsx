...
export function ComplianceDashboard() {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'metrics' | 'activities' | 'incidents'>('overview');
    ...
    const fetchComplianceData = useCallback(async () => {
        ...
        try {
            const [overviewRes, metricsRes, activitiesRes, consentsRes, incidentsRes] = await Promise.all([
                api.get('/compliance/overview').catch(() => ({ data: { success: true, overview: null } })),
                api.get('/compliance/metrics').catch(() => ({ data: { success: true, metrics: [] } })),
                api.get('/compliance/activities').catch(() => ({ data: { success: true, activities: [] } })),
                api.get('/compliance/consents').catch(() => ({ data: { success: true, consents: [] } })),
                api.get('/compliance/incidents').catch(() => ({ data: { success: true, incidents: [] } }))
            ]);
            ...
            setActivities(activitiesRes.data.activities.length ? activitiesRes.data.activities : getDefaultActivities());
    ...
    const getDefaultMetrics = (): ComplianceMetric[] => [
        {
            ...
            requirements: ['Right to Access', 'Right to Erasure', 'Right to Portability', 'Right to Rectification']
        },
        {
            id: '2',
            ...
            requirements: ['Automated Decision Disclosure', 'AI Impact Assessment', 'Human Review Option']
        },
        {
            id: '3',
    ...
    const getStatusBadge = (status: string) => {
        const styles: Record<string, { bg: string; text: string; icon: React.ReactNode }> = {
            COMPLIANT: { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-700 dark:text-green-400', icon: <CheckCircle2 size={14} /> },
            NON_COMPLIANT: { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-400', icon: <XCircle size={14} /> },
            PARTIAL: { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-700 dark:text-yellow-400', icon: <AlertTriangle size={14} /> },
            NOT_APPLICABLE: { bg: 'bg-slate-100 dark:bg-slate-700', text: 'text-slate-600 dark:text-slate-400', icon: null }
        };

        const style = styles[status] || styles.PARTIAL;

        return (
            <span className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${style.bg} ${style.text}`}>
                {style.icon}
                {status.replace('_', ' ')}
            </span>
        );
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            <div className="bg-white dark:bg-navy-800 rounded-xl p-6 border border-slate-200 dark:border-white/10">
                                ...
                                <div className="mt-2 h-2 bg-slate-100 dark:bg-navy-900 rounded-full overflow-hidden">
                                    <div 
                            ...
                            {COMPLIANCE_CATEGORIES.slice(0, 3).map(cat => {
                                const score = cat.id === 'GDPR' ? overview.gdprScore : 
                                             cat.id === 'SOC2' ? overview.soc2Score : overview.isoScore;
                                return (
                                    <div key={cat.id} className="bg-white dark:bg-navy-800 rounded-xl p-6 border border-slate-200 dark:border-white/10">
                                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="bg-white dark:bg-navy-800 rounded-xl p-6 border border-slate-200 dark:border-white/10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500">Data Subjects</p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {filteredMetrics.map(metric => (
                            ...
                            >
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-xs text-slate-400 px-2 py-0.5 bg-slate-100 dark:bg-navy-900 rounded">
                                                    {metric.category}
                                                </span>
                                            </div>
                                ...
                                {expandedMetrics.has(metric.id) && (
                                    <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-navy-900">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-wrap gap-1">
                                                {activity.dataCategories.map((cat, idx) => (
                                                    <span key={idx} className="px-2 py-0.5 text-xs bg-slate-100 dark:bg-navy-900 text-slate-600 dark:text-slate-400 rounded">
                                                        {cat}
                                                    </span>
                                                ))}
                                            </div>
