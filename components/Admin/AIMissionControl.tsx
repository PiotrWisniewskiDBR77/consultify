    ...
    return (
        <div className="p-6 space-y-8 bg-gray-50 min-h-screen">
            <div className="flex justify-between items-center">
                ...
                <Button onClick={fetchStatus} variant="outline" size="sm">Refresh Status</Button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div className="mt-2 flex items-baseline">
                        ...
                        <span className={`ml-2 text-sm font-medium ${status && status.metrics.uptime50 > 95 ? 'text-green-600' : 'text-amber-600'}`}>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                        {status?.providers.filter(p => p.status === 'ACTIVE').map(p => (
                            <span key={p.name} className="px-2 py-1 bg-green-100 text-green-700 text-xs font-medium rounded-full">
                                {p.name}
                            </span>
                        ))}
                    </div>
                            ...
                            <div className="flex items-center gap-4">
                                {results[cap.id] && (
                                    <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${
                                        results[cap.id].status === 'SUCCESS' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                    }`}>
            ...
            {Object.keys(results).length > 0 && (
                <div className="bg-gray-900 rounded-xl shadow-lg p-6 font-mono text-xs text-green-400 overflow-auto max-h-96">
                    <h3 className="text-gray-400 mb-4 border-b border-gray-800 pb-2 flex justify-between">
                        ...
                        <button onClick={() => setResults({})} className="text-gray-500 hover:text-white">Clear</button>
                    </h3>
                    {Object.entries(results).reverse().map(([id, res]) => (
                        <div key={id} className="mb-4">
                            ...
                            {res.details && (
                                <pre className="ml-4 mt-1 opacity-80">
