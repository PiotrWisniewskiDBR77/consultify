...
export const TabLayout: React.FC<TabLayoutProps> = ({
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden">
            ...
            <div className="shrink-0 bg-navy-900/50 border-b border-white/5">
                {/* Title row */}
                {(title || actions) && (
                ...
                {/* Tabs row */}
                <div className="px-6 flex gap-1 overflow-x-auto scrollbar-thin scrollbar-thumb-white/10">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                        ...
                        >
                            {tab.icon && (
                                <span className={activeTab === tab.id ? 'text-red-400' : ''}>
                                    {tab.icon}
                                </span>
                            )}
                            {tab.label}
