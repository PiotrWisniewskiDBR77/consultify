...
export const RolloutStrategyTab: React.FC<RolloutStrategyTabProps> = ({ data, onUpdate, isAdmin }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
                ...
                {isAdmin && (
                    ...
                    >
                        {isLocked ? (
                            <>
                                <Lock size={16} /> Strategy Locked
                            </>
                        ) : (
                            <>
                                <Unlock size={16} /> Strategy Unlocked
                            </>
                        )}
                    </button>
                )}
            ...
            <div
                className={`grid grid-cols-1 lg:grid-cols-2 gap-8 ${isLocked ? 'pointer-events-none opacity-80' : ''}`}
            >
                ...
                <div className="bg-white dark:bg-navy-900 p-6 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-bold text-green-500 mb-2 flex items-center gap-2">
                                <Check size={16} /> In Scope
                            </label>
                            <div className="space-y-2">
                                ...
                                <button
                                    ...
                                    className="text-green-500 text-xs font-bold flex items-center gap-1"
                                >
                                    + Add Scope
                                </button>
                            </div>
            ...
            <div className="bg-white dark:bg-navy-900 p-6 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {['Digital Foundation', 'Customer Experience', 'Operational Excellence'].map((pillar, i) => (
                        <div
                            key={i}
                            className="p-4 bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-white/10 rounded-lg text-center font-bold"
                        >
                            {pillar}
                        </div>
                    ))}
