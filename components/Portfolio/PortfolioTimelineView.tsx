...
import { getStatusColors, getPriorityColors, TIMELINE_COLORS, getAxisColor } from '../../config/portfolioColors';
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const depsRes = await Api.get('/initiatives/portfolio/dependencies' + (projectId ? `?projectId=${projectId}` : ''));
                    ...
                    <div className="relative">
                        ...
                        {initiativeRows.length === 0 ? (
                            ...
                            initiativeRows.map((row, rowIdx) => (
                                <div key={rowIdx} className="relative h-14 border-b border-slate-50 dark:border-white/5">
                                    {row.map(initiative => {
                                        const startIdx = getQuarterIndex(initiative.targetQuarter);
