                    ...
                    <div className="space-y-1.5 text-sm">
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <span className="font-medium text-navy-900 dark:text-white">
                                {new Intl.NumberFormat('pl-PL', { style: 'currency', currency }).format(Math.abs(data.costs))}
                            </span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="border-t border-slate-200 dark:border-white/10 pt-1.5 mt-1.5">
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <span className={`font-bold ${data.netCashFlow >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                                    {new Intl.NumberFormat('pl-PL', { style: 'currency', currency }).format(data.netCashFlow)}
                                </span>
                            </div>
                        </div>
                        {showCumulative && (
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <span className={`font-bold ${data.cumulativeCashFlow >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                                    {new Intl.NumberFormat('pl-PL', { style: 'currency', currency }).format(data.cumulativeCashFlow)}
                                </span>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-6">
                    <div className="text-right">
                        ...
                        <p className="text-sm font-bold text-red-600 dark:text-red-400">
                            {new Intl.NumberFormat('pl-PL', { style: 'currency', currency, maximumFractionDigits: 0 }).format(stats.totalCosts)}
                        </p>
                    </div>
                    <div className="text-right">
                        <p className="text-xs text-slate-500 dark:text-slate-400">Suma korzyści</p>
                        <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400">
                            {new Intl.NumberFormat('pl-PL', { style: 'currency', currency, maximumFractionDigits: 0 }).format(stats.totalBenefits)}
                        </p>
                    </div>
