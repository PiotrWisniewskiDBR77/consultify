...
import { 
    X, Plus, Paperclip, Link as LinkIcon, FileText, 
    Image, Trash2, Check, Loader2, ExternalLink,
    Upload, StickyNote, CheckCircle, AlertCircle
} from 'lucide-react';
...
export const EvidencePanel: React.FC<EvidencePanelProps> = ({
    ...
    return (
        <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-navy-900 shadow-2xl z-50 flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-white/10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500 truncate max-w-[200px]">{axisName} › {areaName}</p>
                    </div>
                </div>
                <button 
                    onClick={onClose}
                    ...
                    <div className="space-y-4">
                        ...
                        {evidence.length === 0 ? (
                            <div className="text-center py-8">
                                <div className="w-16 h-16 mx-auto bg-slate-100 dark:bg-navy-800 rounded-2xl flex items-center justify-center mb-4">
                                    <Paperclip className="text-slate-400" size={28} />
                                </div>
                                <p className="text-sm text-slate-500 mb-2">Brak załączonych dowodów</p>
                                <p className="text-xs text-slate-400">Dodaj dowody potwierdzające ocenę</p>
                            </div>
                        ) : (
                            evidence.map(item => {
                                ...
                                return (
                                    <div 
                                        key={item.id}
                                        className="bg-slate-50 dark:bg-navy-800 rounded-xl p-4 group"
                                    >
                            ...
                            <div className="bg-emerald-50 dark:bg-emerald-500/10 rounded-xl p-4 border border-emerald-200 dark:border-emerald-500/20">
                                            ...
                                            <button
                                                ...
                                                className={`flex-1 p-2 rounded-lg text-xs font-medium transition-all flex items-center justify-center gap-1.5 ${
                                                    addType === type
                                                        ...
                                                        : 'bg-white dark:bg-navy-800 text-slate-600 dark:text-slate-400 hover:bg-slate-100'
                                                }`}
                                            >
                                                <Icon size={14} />
                                                {config?.label}
                                ...
                                <div className="space-y-3">
                                    <input
                                        ...
                                        className="w-full px-3 py-2 bg-white dark:bg-navy-800 border border-slate-200 dark:border-white/10 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                                    />
                                    ...
                                    {addType === 'note' && (
                                        <textarea
                                            ...
                                            onChange={(e) => setContent(e.target.value)}
                                            rows={3}
                                            className="w-full px-3 py-2 bg-white dark:bg-navy-800 border border-slate-200 dark:border-white/10 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none"
                                        />
                                    )}
                                        ...
                                        <div className="border-2 border-dashed border-slate-300 dark:border-white/20 rounded-lg p-4 text-center">
                                            {file ? (
                                                ...
                                                <label className="cursor-pointer">
                                                    ...
                                                    <p className="text-sm text-slate-500">Kliknij, aby wybrać plik</p>
                                                    <p className="text-xs text-slate-400 mt-1">PDF, DOC, XLS, PNG, JPG (max 25MB)</p>
                                                </label>
                                            )}
                                        </div>
                                    )}
