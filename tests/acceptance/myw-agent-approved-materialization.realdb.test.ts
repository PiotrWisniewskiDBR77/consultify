...
describe.skipIf(!enabled)('MYW-AGT mounted signed-JWT acceptance', () => {
  const tag=randomUUID(), org=`org-${tag}`, requester=`requester-${tag}`, approver=`approver-${tag}`, foreign=`foreign-${tag}`, plan=`plan-${tag}`;
  let pool:Pool, app:any, requesterToken:string, approverToken:string, foreignToken:string;
  beforeAll(async()=>{
    ...
    for(const [user,tenant] of [[requester,org],[approver,org],[foreign,foreign]]){
      ...
      await pool.query(`INSERT INTO organization_members(id,organization_id,user_id,role,status) VALUES($1,$2,$3,'OWNER','ACTIVE')`,[randomUUID(),tenant,user]);
    }
    ...
    requesterToken=sign(requester,org); approverToken=sign(approver,org); foreignToken=sign(foreign,foreign);
  ...
  it('mounts request→separate approval→canonical task and blocks body tenant spoof/foreign access',async()=>{
    ...
    const clean=await request(app).post('/api/my-work/agent-materialization/proposals').set(auth(requesterToken)).send({
      sourcePlanId:plan,sourceVersion:source.body.sourceVersion,sourceHash:source.body.sourceHash,targetKind:'task',
      content:{title:'Human approved mounted task'},idempotencyKey:`mounted-${tag}`,expiresAt:new Date(Date.now()+60_000).toISOString()
    });
    expect(clean.status).toBe(201);
    const foreignDecision=await request(app).post(`/api/my-work/agent-materialization/proposals/${clean.body.proposal.proposal_id}/decision`)
      .set(auth(foreignToken)).send({decision:'APPROVE',expectedStateVersion:1,sourceHash:source.body.sourceHash});
    expect(foreignDecision.status).toBe(404);
    const approved=await request(app).post(`/api/my-work/agent-materialization/proposals/${clean.body.proposal.proposal_id}/decision`)
      .set(auth(approverToken)).send({decision:'APPROVE',expectedStateVersion:1,sourceHash:source.body.sourceHash});
    expect(approved.status).toBe(200);
    const materialized=await request(app).post(`/api/my-work/agent-materialization/proposals/${clean.body.proposal.proposal_id}/materialize`)
      .set(auth(approverToken)).send({expectedStateVersion:2});
    expect(materialized.status).toBe(200);
    expect((await pool.query(`SELECT title FROM tasks WHERE id=$1`,[materialized.body.receipt.target_id])).rows[0].title).toBe('Human approved mounted task');
  });
});
