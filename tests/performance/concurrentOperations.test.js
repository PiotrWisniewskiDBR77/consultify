...
import UsageService from '../../server/src/services/usageService';
import PermissionService from '../../server/src/services/permissionService';
import TokenBillingService from '../../server/src/services/tokenBillingService';

// Mock database at module level before any imports
const mockDb = createMockDb();
vi.mock('../../server/src/database/index', () => ({
  default: mockDb,
  getDatabase: () => mockDb,
}));

// Mock module dependencies to prevent real DB/Redis connections during import/executions
vi.mock('../../server/src/services/usageService', () => {
  return {
    default: {
...
vi.mock('../../server/src/services/permissionService', () => {
  return {
    default: {
...
describe('Concurrent Operations Performance', () => {
  ...
  describe('Concurrent Database Operations', () => {
    ...
    it('should handle concurrent INSERT operations', async () => {
      ...
      const start = Date.now();
      const promises = Array(concurrentInserts)
        .fill(null)
        .map(
          () =>
    ...
    it('should handle concurrent UPDATE operations', async () => {
      ...
      const start = Date.now();
      const promises = Array(concurrentUpdates)
        .fill(null)
        .map(
          (_, i) =>
            new Promise((resolve, reject) => {
  ...
  describe('Concurrent Service Operations', () => {
    it('should handle concurrent usage tracking', async () => {
      ...
      const start = Date.now();
      const promises = Array(concurrentRecords)
        .fill(null)
        .map((_, i) =>
          UsageService.recordTokenUsage('org-123', 'user-123', 100 + i, 'test_action')
        );
    ...
    it('should handle concurrent permission checks', async () => {
      ...
      const start = Date.now();
      const promises = Array(concurrentChecks)
        .fill(null)
        .map(() => PermissionService.hasPermission('user-123', 'PERMISSION_TEST', 'org-123'));
  ...
  describe('Race Condition Tests', () => {
    it('should handle race conditions in token billing', async () => {
      ...
      const start = Date.now();
      const promises = Array(concurrentDeductions)
        .fill(null)
        .map(() => TokenBillingService.deductTokens('org-123', 100));
