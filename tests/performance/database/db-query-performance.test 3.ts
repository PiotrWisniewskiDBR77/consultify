...
describe('Database Query Performance', () => {
    ...
    beforeAll(async () => {
        ...
        await new Promise<void>((resolve, reject) => {
            db.serialize(() => {
                ...
                db.run(`CREATE TABLE orders (id TEXT PRIMARY KEY, user_id TEXT, total REAL, created_at DATETIME)`);
                db.run(`CREATE INDEX idx_orders_user ON orders(user_id)`);

                // Insert test data
                for (let i = 0; i < 1000; i++) {
                    db.run('INSERT INTO users (id, name, email) VALUES (?, ?, ?)', [`u${i}`, `User ${i}`, `user${i}@example.com`]);
                    db.run('INSERT INTO orders (id, user_id, total, created_at) VALUES (?, ?, ?, datetime("now"))', [`o${i}`, `u${i % 100}`, Math.random() * 1000]);
                }
                resolve();
            });
        });
    ...
    describe('Write Performance', () => {
        it('should insert single row under 10ms', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO users (id, name, email) VALUES (?, ?, ?)', [`perf-${Date.now()}`, 'Perf User', 'perf@example.com'], (err) => {
                    if (err) reject(err);
                    else resolve();
                });
            });
