...
const createCompositeStrategy = (strategies, combiner = 'all') => {
  return {
    execute: async (...args) => {
      if (combiner === 'all') {
        const results = await Promise.all(strategies.map((s) => s.execute(...args)));
        return results;
      }

      if (combiner === 'race') {
        return Promise.race(strategies.map((s) => s.execute(...args)));
      }
...
describe('Strategy Pattern Tests', () => {
  ...
  describe('Composite Strategy', () => {
    it('should execute all strategies', async () => {
      const composite = createCompositeStrategy(
        [{ execute: (x) => x * 2 }, { execute: (x) => x + 10 }, { execute: (x) => x - 5 }],
        'all'
      );
    ...
    it('should chain strategies', async () => {
      const composite = createCompositeStrategy(
        [{ execute: (x) => x * 2 }, { execute: (x) => x + 10 }, { execute: (x) => x * 3 }],
        'chain'
      );
  ...
  describe('Conditional Strategy', () => {
    ...
    it('should add conditions dynamically', () => {
      ...
      conditional.addCondition((x) => x > 0, { execute: () => 'positive' });
      conditional.addCondition((x) => x < 0, { execute: () => 'negative' });
  ...
  describe('Real-World Examples', () => {
    ...
    it('should compose validation strategies', async () => {
      const composite = createCompositeStrategy(
        [validationStrategies.required, validationStrategies.email],
        'all'
      );
