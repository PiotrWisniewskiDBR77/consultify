...
export const createUnifiedTestSetup = (options = {}) => {
  const { useRealDb = false, useRealRedis = false, useRealLLM = false, customMocks = {} } = options;
...
export const createMockApi = () => ({
  ...
  getReportStatus: vi
    .fn()
    .mockResolvedValue({ status: 'completed', url: 'https://reports.example.com/report.pdf' }),
  exportData: vi.fn().mockResolvedValue({ exportId: 'export-123' }),
  getExportStatus: vi
    .fn()
    .mockResolvedValue({ status: 'ready', downloadUrl: 'https://exports.example.com/data.zip' }),
