...
test.describe('AI Actions Flow', () => {
  ...
  test('should display AI action proposals', async ({ page }) => {
    ...
    await page
      .waitForSelector('[data-testid="action-proposal"]', { timeout: 10000 })
      .catch(() => {});

    // Check if proposals list is visible
    const proposalsList = page
      .locator('[data-testid="action-proposal-list"]')
      .or(page.locator('text=No pending action proposals'));
  ...
  test('should open action proposal details', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      await expect(
        page
          .locator('[data-testid="action-proposal-detail"]')
          .or(page.locator('text=Action Details'))
      )
  ...
  test('should approve AI action', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const reasonInput = page
        .locator('textarea[placeholder*="reason" i]')
  ...
  test('should reject AI action with reason', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      if ((await reasonInput.count()) > 0) {
        ...
        const confirmButton = page
          .locator('button:has-text("Confirm Rejection")')
          .or(page.locator('button:has-text("Confirm")'))
          .first();
        if ((await confirmButton.count()) > 0) {
          await confirmButton.click();
