 ...
 * - Status filters (only PLANNING, REVIEW, APPROVED)
 * - Drawer (50%) + Open wider functionality
 * - Gate decisions (Go/No-Go, Resources Commit, Schedule Lock)
 * - Timeline with dependencies
 * 
 * Flow: PLANNING -> REVIEW -> APPROVED -> (Execution)
 */
...
test.describe('Initiatives + Roadmap Module', () => {
  ...
  test.describe('View Modes', () => {
    ...
    test('should display initiatives in Kanban view', async ({ page }) => {
      ...
      await page.click('[data-view-mode="kanban"], [aria-label*="kanban"], button:has-text("Kanban")');
      await page.waitForTimeout(500);
    ...
    test('should display initiatives in Timeline view', async ({ page }) => {
      ...
      await page.click('[data-view-mode="timeline"], [aria-label*="timeline"], button:has-text("Timeline")');
      await page.waitForTimeout(500);
      
      // Verify timeline is visible
      const timeline = page.locator('[data-testid="initiatives-timeline"], [class*="gantt"], [class*="timeline"]');
  ...
  test.describe('Status Filters', () => {
    test('should filter by REVIEW status (first in workflow)', async ({ page }) => {
      ...
      await page.click('button:has-text("Review"), button:has-text("In Review"), [data-status="REVIEW"]');
      await page.waitForTimeout(500);
    ...
    test('should NOT show DRAFT or EXECUTING statuses in filters', async ({ page }) => {
      ...
      // EXECUTING should not be visible (belongs to Execution module)
      const executingFilter = page.locator('button:has-text("Executing"):not([class*="disabled"])');
      await expect(executingFilter).not.toBeVisible();
    });
  });
  ...
  test.describe('Initiative Drawer', () => {
    test('should open drawer when clicking on initiative', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid*="initiative"], [class*="initiative-card"]', { timeout: 10000 });
      
      // Click on first initiative
      await page.click('[data-testid*="initiative"]:first-child, [class*="initiative-card"]:first-child');
      await page.waitForTimeout(500);
    ...
    test('should have "Open Wider" button', async ({ page }) => {
      ...
      // Check for Open Wider button (Maximize icon or text)
      const openWiderButton = page.locator('button:has-text("Open Wider"), button[title*="wider"], [aria-label*="wider"], [aria-label*="maximize"]');
    ...
    test('should expand to full view when clicking "Open Wider"', async ({ page }) => {
      ...
      // Click Open Wider
      await page.click('button:has-text("Open Wider"), button[title*="wider"], [aria-label*="wider"]');
      await page.waitForTimeout(500);
      
      // Verify full view is shown (drawer should be replaced with full card)
      const fullView = page.locator('[data-testid="initiative-detail"], [class*="detail-card"]');
      // The drawer should close
      const drawer = page.locator('[class*="w-1/2"]:visible');
      await expect(drawer).not.toBeVisible();
    });
  });

  test.describe('Status Transitions', () => {
    test('should show workflow progress indicator (REVIEW -> APPROVED -> PLANNING)', async ({ page }) => {
      // Open drawer
      await page.waitForSelector('[data-testid*="initiative"], [class*="initiative-card"]', { timeout: 10000 });
      await page.click('[data-testid*="initiative"]:first-child, [class*="initiative-card"]:first-child');
      await page.waitForTimeout(500);
      
      // Check for workflow progress (Review -> Approved -> Planning)
      const workflowProgress = page.locator('[class*="workflow"], [class*="progress"]');
      await expect(workflowProgress).toBeVisible();
      
      // Verify the correct order of statuses in workflow
      const reviewLabel = page.locator(':has-text("Review")');
      const approvedLabel = page.locator(':has-text("Approved")');
      const planningLabel = page.locator(':has-text("Planning")');
      
      // All three should be visible in the workflow
    });
    ...
    test('should display status action buttons based on current status', async ({ page }) => {
      ...
      // APPROVED: Start Planning button
      // PLANNING: Start Execution button
      const actionButton = page.locator('button:has-text("Approve"), button:has-text("Start Planning"), button:has-text("Start Execution")');
  ...
  test.describe('Gate Decisions', () => {
    test('should display required gate decisions', async ({ page }) => {
      ...
      const gateSection = page.locator('[class*="gate"], :has-text("Gate Decisions"), :has-text("Go/No-Go")');
    ...
    test('should show gate requirements info', async ({ page }) => {
      ...
      // At least one gate type should be mentioned
    });
  });
  ...
  test.describe('Quick Actions', () => {
    test('should have "New Initiative" button', async ({ page }) => {
      const newButton = page.locator('button:has-text("New Initiative"), button:has-text("Create")');
