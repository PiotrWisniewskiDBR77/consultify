...
test.describe('Assessment Flow', () => {
  ...
  test('should display assessment matrix', async ({ page }) => {
    ...
    await page
      .waitForSelector('[data-testid="assessment-matrix"]', { timeout: 10000 })
      .catch(() => {});

    // Check if matrix cards are visible
    const matrixCards = page
      .locator('[data-testid="assessment-card"]')
      .or(page.locator('text=Axis'));
    await expect(matrixCards.first())
  ...
  test('should open assessment wizard', async ({ page }) => {
    ...
    const startButton = page
      .locator('button:has-text("Start Assessment")')
      .or(page.locator('button:has-text("Assess")'))
      .first();
    ...
    await expect(
      page.locator('[data-testid="assessment-wizard"]').or(page.locator('text=Assessment Wizard'))
    )
      .toBeVisible({ timeout: 5000 })
      .catch(() => {});
  });
    ...
    if ((await startButton.count()) > 0) {
      ...
      if ((await nextButton.count()) > 0) {
        ...
        const levelButtons = page
          .locator('button[data-level]')
          .or(page.locator('button:has-text("Level")'));
