...
test.describe('Governance Flow', () => {
  ...
  test('should view audit log', async ({ page }) => {
    ...
    const auditEntries = page
      ...
      .or(page.locator('text=CREATE').or(page.locator('text=UPDATE')));
    await expect(auditEntries.first())
  ...
  test('should filter audit log', async ({ page }) => {
    ...
    if ((await filterButton.count()) > 0) {
      ...
      const createOption = page
        .locator('option:has-text("CREATE")')
  ...
  test('should manage permissions', async ({ page }) => {
    ...
    await page
      .waitForSelector('[data-testid="permission-manager"]', { timeout: 10000 })
      .catch(() => {});

    // Check if permissions list is visible
    const permissionsList = page
      .locator('[data-testid="permission-list"]')
      .or(page.locator('text=Permission Manager'));
    await expect(permissionsList)
      .toBeVisible({ timeout: 5000 })
      .catch(() => {});
  });
