...
import { addAdmin, authHeaders, bootstrap, cleanup, seedExecution, signedContext, type ExecutionPersona } from './_helpers/executionUiTechnicalFixture';
...
test.describe.serial('EXE-UI-CANON canonical delivery closure', () => {
  ...
  test.afterEach(async ({ request }) => { if (creator) { await cleanup(request, creator); creator = null; } });

  test('signed distinct actors complete spine, four-eyes approval, close, receipt, reload and cold reopen', async ({ browser, request }) => {
    ...
    const creatorContext = await signedContext(browser, creator); const creatorPage = await creatorContext.newPage();
    ...
    for (const [label, value] of [['workRef','work:1'],['resourceRef','resource:1'],['controlRef','control:1'],['reportRef','report:1']] as const) {
    ...
    const approverContext = await signedContext(browser, approver); const page = await approverContext.newPage();
    ...
    const closeButton = adminPanel.getByRole('button', { name: 'Close execution and emit Results signal' });
    ...
    await page.reload(); await expect(adminPanel.getByText('Results receipt persisted')).toBeVisible();
    expect(await adminPanel.getByText(/^Signal:/).textContent()).toBe(signalText);
    const axe = await new AxeBuilder({ page }).include('[data-testid="execution-delivery-closure"]').analyze();
    ...
    const cold = await signedContext(browser, approver); const coldPage = await cold.newPage();
  ...
  test('stale concurrent spine has one winner; foreign and revoked reads fail closed', async ({ request }) => {
    ...
    const seed = await seedExecution(creator);
    const linkResponse = await request.post(`${API}/api/v8/case-workspace/execution-bvp/links`, { headers: { ...authHeaders(creator), 'Idempotency-Key': `link-${Date.now()}` }, data: { initiativeId: seed.initiativeId, caseId: seed.caseId } });
    expect(linkResponse.status()).toBe(201); const linkId = (await linkResponse.json()).data.link_id;
    const writes = await Promise.all(['a','b'].map((x) => request.post(`${API}/api/v8/case-workspace/execution-bvp/links/${linkId}/spine`, { headers: authHeaders(creator!), data: { workRef:`work:${x}`,resourceRef:`resource:${x}`,controlRef:`control:${x}`,reportRef:`report:${x}`,expectedVersion:1 } })));
    expect(writes.map((r) => r.status()).sort()).toEqual([200,404]);
    const foreignRead = await request.get(`${API}/api/v8/case-workspace/execution-bvp/links/${linkId}`, { headers: authHeaders(foreign) });
    expect([403,404]).toContain(foreignRead.status());
    const db = new pg.Client({ connectionString: process.env.DATABASE_URL }); await db.connect();
    await db.query(`UPDATE organization_members SET status='REVOKED' WHERE organization_id=$1 AND user_id=$2`, [creator.organizationId, creator.userId]); await db.end();
    const revokedRead = await request.get(`${API}/api/v8/case-workspace/execution-bvp/links/${linkId}`, { headers: authHeaders(creator) });
