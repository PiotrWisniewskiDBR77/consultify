...
test.describe('Assessment Module - CRUD Operations', () => {
  ...
  test('should create new DRD assessment', async ({ page }) => {
    ...
    await page.click(
      '[data-testid="new-assessment-btn"], button:has-text("New Assessment"), button:has-text("Nowa ocena")'
    );

    // Fill assessment name
    const nameInput = page.locator(
      '[data-testid="assessment-name-input"], input[placeholder*="name"], input[placeholder*="nazwa"]'
    );
    ...
    await expect(page.locator('[data-testid="status-badge"], .status-badge').first()).toContainText(
      /DRAFT|Szkic/i
    );
  });
  ...
  test('should create new SIRI assessment', async ({ page }) => {
    ...
    const nameInput = page.locator(
      '[data-testid="assessment-name-input"], input[placeholder*="name"]'
    );
    if (await nameInput.isVisible()) {
  ...
  test('should filter assessments by status', async ({ page }) => {
    ...
    await page.click(
      '[data-testid="status-filter"], button:has-text("Filter"), button:has-text("Filtr")'
    );
...
test.describe('Assessment Module - Workflow Transitions', () => {
  ...
  test('should show request review button only for DRAFT assessments', async ({ page }) => {
    ...
    const draftRow = page
      .locator('tr:has-text("DRAFT"), .assessment-item:has-text("DRAFT")')
      .first();
    ...
    if (await draftRow.isVisible()) {
      ...
      await expect(
        page.locator('[data-testid="request-review-btn"], button:has-text("Request Review")')
      ).toBeVisible();
    }
  });
  ...
  test('should transition from DRAFT to REVIEW', async ({ page }) => {
    ...
    if (await draftRow.isVisible()) {
      ...
      const confirmBtn = page.locator(
        '[data-testid="confirm-btn"], button:has-text("Confirm"), button:has-text("Potwierdź")'
      );
      ...
      await expect(page.locator('[data-testid="status-badge"], .status-badge')).toContainText(
        /REVIEW|W recenzji/i
      );
    }
  });
  ...
  test('should show approve buttons only for REVIEW status', async ({ page }) => {
    ...
    const reviewRow = page
      .locator('tr:has-text("REVIEW"), .assessment-item:has-text("W recenzji")')
      .first();
    ...
    if (await reviewRow.isVisible()) {
      ...
      await expect(
        page.locator('[data-testid="approve-report-btn"], button:has-text("Approve Report")')
      ).toBeVisible();
    }
  });
  ...
  test('should require report approval before assessment approval', async ({ page }) => {
    ...
    const reviewRow = page
      .locator('tr:has-text("IN_REVIEW"), .assessment-item:has-text("W recenzji")')
      .first();
    ...
    if (await reviewRow.isVisible()) {
      ...
      const approveBtn = page.locator(
        '[data-testid="approve-assessment-btn"], button:has-text("Approve Assessment")'
      );
  ...
  test('should send assessment back to DRAFT', async ({ page }) => {
    ...
    if (await reviewRow.isVisible()) {
      ...
      await page.click(
        '[data-testid="send-back-btn"], button:has-text("Send Back"), button:has-text("Cofnij")'
      );
      ...
      // Confirm
      await page.click(
        '[data-testid="confirm-send-back"], button:has-text("Send Back"), button:has-text("Potwierdź")'
      );
...
test.describe('Assessment Module - Initiative Generation', () => {
  ...
  test('should show generate initiatives only for APPROVED assessments', async ({ page }) => {
    ...
    const approvedRow = page
      .locator('tr:has-text("APPROVED"), .assessment-item:has-text("Zatwierdzony")')
      .first();
    ...
    if (await approvedRow.isVisible()) {
      ...
      await expect(
        page.locator(
          '[data-testid="generate-initiatives-btn"], button:has-text("Generate Initiatives")'
        )
      ).toBeVisible();
    }
  });
  ...
  test('should not allow generate initiatives for non-APPROVED assessments', async ({ page }) => {
    ...
    const nonApprovedRow = page
      .locator('tr:has-text("DRAFT"), .assessment-item:has-text("Szkic")')
      .first();
  ...
  test('should open generate initiatives modal', async ({ page }) => {
    ...
    if (await approvedRow.isVisible()) {
      ...
      await expect(
        page.locator('[data-testid="generate-modal"], .modal, [role="dialog"]')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="count-select"], input[type="number"]')
      ).toBeVisible();
    }
  });
  ...
  test('should limit initiatives count to max 7', async ({ page }) => {
    ...
    if (await approvedRow.isVisible()) {
      ...
      if (await countInput.isVisible()) {
        ...
        expect(
          numValue <= 7 || page.locator('.error, [data-testid="error"]').isVisible()
        ).toBeTruthy();
      }
    }
  });
  ...
  test('should generate initiatives and show in drawer', async ({ page }) => {
    ...
    if (await approvedRow.isVisible()) {
      ...
      await expect(
        page.locator('[data-testid="initiatives-drawer"], .initiatives-drawer')
      ).toBeVisible();

      // Verify initiatives have DRAFT status
      const initiativeStatus = page
        .locator('[data-testid="initiative-status"], .initiative-status')
        .first();
...
test.describe('Assessment Module - Dynamic Submenu', () => {
  ...
  test('should show open assessments in submenu', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="open-assessments-submenu"], .open-assessments')
    ).toBeVisible();
  });
...
test.describe('Assessment Module - Initiatives Drawer', () => {
  ...
  test('should show initiative details in drawer', async ({ page }) => {
    ...
    if (await assessmentRow.isVisible()) {
      ...
      const initiativeItem = page
        .locator('[data-testid="initiative-item"], .initiative-item')
        .first();
      if (await initiativeItem.isVisible()) {
        ...
        await expect(
          page.locator('[data-testid="initiative-details"], .initiative-details')
        ).toBeVisible();
      }
    }
  });
  ...
  test('should enable Go to Initiatives only after PLANNING status', async ({ page }) => {
    ...
    if (await assessmentRow.isVisible()) {
      ...
      const goToBtn = page.locator(
        '[data-testid="go-to-initiatives"], button:has-text("Go to Initiatives")'
      );

      // Button should be disabled if initiative status is DRAFT
      const initiativeItem = page
        .locator('[data-testid="initiative-item"]:has-text("DRAFT")')
        .first();
      if (await initiativeItem.isVisible()) {
        await initiativeItem.click();
