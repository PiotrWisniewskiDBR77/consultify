...
// ★ FORKED for the 2026-08-13 "true E2E" run (task: docs/qa/e2e-true-2026-08-13/)
// — this agent's OWN disposable Postgres container (`mac-pg-e2etrue`, port
// 55540), never `mac-pg-s8`/55520 which belongs to a different concurrent
// session/worktree. See CLAUDE.md orchestration rule: one worktree = one
// agent, never share another session's container.
export const DISPOSABLE_DB = {
  container: 'mac-pg-e2etrue',
  port: 55540,
  user: 't',
  password: 't',
  database: 't_test',
...
export const ORG_ID = 'e2etrue-drd-org';
export const OTHER_ORG_ID = 'e2etrue-drd-other-org';
...
export async function startServer(port: number, logPath: string): Promise<RunningServer> {
  ...
  // ★ Bumped from 130s (upstream default) — this shared sandbox measured
  // load averages >400 during this run (dozens of concurrent sessions'
  // docker/node processes), so a cold boot can genuinely take longer than
  // 130s without being stuck; 130s produced one real hang/timeout in this
  // run (see docs/qa/e2e-true-2026-08-13/extra-scenarios/logs/server-1.log).
  const readyAt = await waitForReady(baseUrl, 280_000);
