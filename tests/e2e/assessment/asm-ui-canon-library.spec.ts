...
test.describe('ASM-UI-CANON mounted Library and cold reopen', () => {
  ...
  test.afterEach(async ({ request }) => {
    await cleanup(
      request,
      ...[owner?.runId, foreign?.runId].filter((id): id is string => Boolean(id))
    );
    owner = undefined;
    foreign = undefined;
  });

  test('Library lists exact identity/version, opens read-only, cold reopens and remains responsive/accessible', async ({
    browser,
    request,
  }) => {
    ...
    await expect(sessionRow).toContainText('2.0.0-methodpack.2');
    ...
    await expect(readerPage.getByText('method: drd@2.0.0-methodpack.2')).toBeVisible();
    ...
    for (const viewport of [
      ...
      await readerPage.screenshot({
        path: test.info().outputPath(`asm-${viewport.name}-light.png`),
        fullPage: true,
      });
    }
    await readerPage.evaluate(() => document.documentElement.classList.add('dark'));
    await readerPage.screenshot({
      path: test.info().outputPath('asm-mobile-dark.png'),
      fullPage: true,
    });
  ...
  test('foreign tenant is denied or concealed and transport loss is mounted fail-closed with retry', async ({
    browser,
    request,
  }) => {
    ...
    await offlinePage.route(`**/api/method/sessions/${sessionId}`, (route) =>
      route.abort('internetdisconnected')
    );
    await offlinePage.goto(`/assessment/drd/${sessionId}`);
    await expect(offlinePage.getByTestId('drd-http-error-view')).toContainText(
      /Brak połączenia|Offline/i
    );
