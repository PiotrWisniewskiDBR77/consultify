...
test('signed valuation wizard reaches canonical assumptions and persists WACC across cold reload', async ({
  ...
  try {
    ...
    await page.getByRole('button', { name: /Założenia|Assumptions/i }).click();
    ...
    if (valuationId) {
      ...
      try {
        ...
        for (const { artifact_id: artifactId } of aliases.rows) {
          ...
          await client.query(
            `DELETE FROM finance_artifact_edges WHERE organization_id=$1 AND (source_artifact_id=$2 OR target_artifact_id=$2)`,
            [state.organizationId, artifactId]
          );
          await client.query(
    ...
    const residue = valuationId
      ...
      : { rows: [{ valuations: 0, aliases: 0 }] };
    expect(residue.rows[0]).toEqual({ valuations: 0, aliases: 0 });
    await pool.end();
  }
});
