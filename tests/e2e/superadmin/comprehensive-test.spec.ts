...
const SUPERADMIN_MODULES = {
    ...
    revenue: {
        ...
        tabs: ['billing', 'invoices', 'usage', 'pricing', 'subscriptions', 'recognition', 'forecasts', 'payments'],
    },
    security: {
        name: 'Security',
...
async function checkForTablesAndData(page: Page): Promise<{ hasTable: boolean; hasData: boolean }> {
    try {
        ...
        const tableSelectors = ['table', '[role="table"]', '.table', '[class*="Table"]', '[data-testid*="table"]'];
