...
test.describe('Task Management Flow', () => {
  ...
  test('complete task lifecycle workflow', async ({ page }) => {
    ...
    await test.step('Switch to assignee view', async () => {
      ...
      await page.click('[data-testid="nav-my-work"]');
      await expect(page.locator('[data-testid="task-inbox"]')).toContainText(
    ...
    await test.step('Update task progress', async () => {
      ...
      await page.fill(
        ...
        'Started implementing basic auth framework with bcrypt'
      );
    ...
    await test.step('Add task dependencies', async () => {
      ...
      await expect(page.locator('[data-testid="task-dependencies"]')).toContainText(
        'OAuth2 Provider'
      );
    });
    ...
    await test.step('Approve task completion', async () => {
      ...
      await page.click(
        '[data-testid="task-review-item"]:has-text("Implement User Authentication")'
      );
      await page.fill(
        '[data-testid="review-feedback"]',
        'Excellent implementation. Security measures look solid.'
      );
    ...
    await test.step('Verify task completion', async () => {
      ...
      await expect(page.locator('[data-testid="completed-tasks"]')).toContainText(
        'Implement User Authentication'
      );
    ...
    await test.step('Check dependent task status', async () => {
      ...
      await expect(page.locator('[data-testid="available-tasks"]')).toContainText(
        'OAuth2 Provider'
      );

      // Verify dependency link
      await page.click('[data-testid="task-item"]:has-text("OAuth2 Provider")');
      await expect(page.locator('[data-testid="dependency-info"]')).toContainText(
        'Implement User Authentication'
      );
    });
  ...
  test('task collaboration features', async ({ page, context }) => {
    ...
    await test.step('Collaborative progress tracking', async () => {
      ...
      await page.fill(
        '[data-testid="progress-note"]',
        'Implemented frontend notification components'
      );
      ...
      await newPage.fill(
        '[data-testid="progress-note"]',
        'Implemented backend notification service'
      );
    ...
    await test.step('Add task comments and discussions', async () => {
      ...
      await page.fill(
        ...
        'WebSockets would be better for bidirectional communication'
      );
    ...
    await test.step('Add task attachments', async () => {
      ...
      await page.fill(
        ...
        'UI mockup for notification center'
      );
