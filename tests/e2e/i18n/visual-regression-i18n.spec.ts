...
async function setLanguage(page: any, langCode: string): Promise<void> {
  ...
  test.describe('Settings - Language Selector', () => {
    ...
    LANGUAGES.forEach(({ code, name }) => {
      test(`${name} (${code}): settings page with language selector`, async ({ page }) => {
        ...
        const langSection = page.locator('[data-testid="language-settings"], text=/language/i, text=/język/i, text=/sprache/i, text=/idioma/i, text=/لغة/i, text=/言語/i').first();
  ...
  test.describe('Help Panel', () => {
    ...
    LANGUAGES.forEach(({ code, name }) => {
      test(`${name} (${code}): help panel should render correctly`, async ({ page }) => {
        ...
        const helpButton = page.locator('[data-testid="help-button"], button:has-text("Help"), button:has-text("Pomoc"), button:has-text("Hilfe"), button:has-text("Ayuda"), button:has-text("مساعدة"), button:has-text("ヘルプ")').first();
        
        if (await helpButton.isVisible()) {
          await helpButton.click();
          await page.waitForTimeout(500);
          
          const helpPanel = page.locator('[data-testid="help-panel"], [role="dialog"]:has-text("Help"), [role="dialog"]:has-text("Pomoc")').first();
  ...
  test.describe('Form with Validation', () => {
    ...
    LANGUAGES.forEach(({ code, name }) => {
      test(`${name} (${code}): form validation should display correctly`, async ({ page }) => {
        ...
        const submitButton = page.locator('button[type="submit"], button:has-text("Save"), button:has-text("Zapisz"), button:has-text("Speichern"), button:has-text("Guardar"), button:has-text("حفظ"), button:has-text("保存")').first();
