...
test.describe('Navigation Smoke Test', () => {
    ...
    test('should have navigation elements after login', async ({ page }) => {
        ...
        const hasNav = await page.locator('nav, [role="navigation"], aside, .sidebar').first().isVisible().catch(() => false);
        const hasContent = await page.locator('main, [role="main"], .content, .dashboard').first().isVisible().catch(() => false);
    ...
    test('should navigate to settings if available', async ({ page }) => {
        ...
        const url = page.url();
        const hasSettingsContent = await page.locator('[class*="settings"], [class*="profile"]').first().isVisible().catch(() => false);
