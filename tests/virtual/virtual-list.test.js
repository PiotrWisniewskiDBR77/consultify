...
const createVirtualGrid = (options = {}) => {
    ...
    return {
        ...
        getVisibleCells: () => {
            ...
            const endRow = Math.min(rows - 1, Math.ceil((scrollTop + containerHeight) / (cellHeight + gap)));

            const startCol = Math.floor(scrollLeft / (cellWidth + gap));
            const endCol = Math.min(columns - 1, Math.ceil((scrollLeft + containerWidth) / (cellWidth + gap)));
