...
const createListFormatter = (locale = 'en-US') => {
  const format = (items, type = 'conjunction') => {
  ...
  return {
    ...
    formatAnd: (items) => {
      return format(items, 'conjunction');
    },

    formatOr: (items) => {
      return format(items, 'disjunction');
    },
