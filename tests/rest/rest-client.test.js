...
describe('Request Builder Tests', () => {
  ...
  it('should build GET request', () => {
    const config = builder.get('/users').param('page', 1).build();
  ...
  it('should build POST request with body', () => {
    const config = builder.post('/users').json({ name: 'John' }).build();
  ...
  it('should add auth header', () => {
    const config = builder.get('/profile').auth('my-token').build();
