...
const createSubject = () => {
    ...
    return {
        ...
        asObservable: () => createObservable((observer) => {
...
// Operators
const operators = {
    map: (fn) => (source) => createObservable((observer) => {
    ...
    filter: (predicate) => (source) => createObservable((observer) => {
        return source.subscribe({
    ...
    take: (count) => (source) => createObservable((observer) => {
        let taken = 0;
    ...
    skip: (count) => (source) => createObservable((observer) => {
        let skipped = 0;
    ...
    debounce: (ms) => (source) => createObservable((observer) => {
        let timeoutId;
    ...
    distinctUntilChanged: () => (source) => createObservable((observer) => {
        let lastValue;
        let hasValue = false;
    ...
    scan: (accumulator, seed) => (source) => createObservable((observer) => {
        let acc = seed;
...
// Creation functions
const from = (iterable) => createObservable((observer) => {
...
const interval = (ms) => createObservable((observer) => {
    let count = 0;
    const id = setInterval(() => observer.next(count++), ms);
    return { unsubscribe: () => clearInterval(id) };
});

const fromEvent = (target, eventName) => createObservable((observer) => {
