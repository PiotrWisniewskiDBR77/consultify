...
const createErrorHandler = () => {
  ...
  return {
    ...
    isValidationError: (errors) => {
      return errors.some(
        (e) => e.extensions?.code === 'BAD_USER_INPUT' || e.extensions?.code === 'VALIDATION_ERROR'
      );
    },

    isNetworkError: (errors) => {
      return errors.some(
        (e) => e.extensions?.code === 'NETWORK_ERROR' || e.message?.includes('fetch')
      );
    },
  };
};
...
describe('Query Builder Tests', () => {
  ...
  it('should build simple query', () => {
    const query = builder.query('GetUsers').select('id', 'name', 'email').build();
  ...
  it('should build query with variables', () => {
    const query = builder.query('GetUser').variable('id', 'ID!').select('id', 'name').build();
