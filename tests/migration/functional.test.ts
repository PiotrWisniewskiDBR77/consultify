...
describe('Functional Migration Tests', () => {
  ...
  beforeAll(() => {
    ...
    middleware = findFiles(path.join(srcDir, 'middleware'), '.ts', []).map(
      (f) => `middleware/${f}`
    );
    controllers = findFiles(path.join(srcDir, 'controllers'), '.ts', []).map(
      (f) => `controllers/${f}`
    );
  });
  ...
  it('should have all route files export router', () => {
    ...
    for (const routeFile of routes.slice(0, 20)) {
      // Sample check
      ...
      if (
        ...
        content.includes('export const router') ||
        content.includes('export { router }')
      ) {
        routesWithRouter++;
      }
    }
