...
describe('Duplicates Tests', () => {
    ...
    beforeAll(() => {
        ...
        for (const jsFile of jsFiles.slice(0, 50)) { // Sample
            ...
            if (tsMatch) {
                ...
                if (fs.existsSync(jsPath) && fs.existsSync(tsPath)) {
                    ...
                    const jsFunctions = (jsContent.match(/(?:function|const|export\s+(?:async\s+)?function)\s+(\w+)/g) || []).map(f => f.split(/\s+/).pop()!).filter(Boolean);
                    const tsFunctions = (tsContent.match(/(?:function|const|export\s+(?:async\s+)?function)\s+(\w+)/g) || []).map(f => f.split(/\s+/).pop()!).filter(Boolean);
    ...
    it('should generate duplicates report', () => {
        ...
        const reportPath = path.join(projectRoot, 'tests', 'migration', 'reports', 'duplicates-report.json');
