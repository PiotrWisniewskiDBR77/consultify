...
const createShortcutManager = () => {
  ...
  return {
    ...
    getShortcuts: (context) => {
      if (context) {
        return (contexts.get(context) || []).map((id) => shortcuts.get(id)).filter(Boolean);
      }
  ...
  return {
    ...
    hasConflict: (shortcuts, newShortcut) => {
      return shortcuts.some(
        (s) =>
          s.shortcut.toLowerCase() === newShortcut.shortcut.toLowerCase() &&
          (s.context === newShortcut.context ||
            s.context === 'global' ||
            newShortcut.context === 'global')
      );
    },
  };
};
...
const createShortcutDisplay = () => {
  ...
  return {
    ...
    parse: (displayString) => {
      // Reverse lookup
      const reverseSymbols = Object.fromEntries(Object.entries(symbols).map(([k, v]) => [v, k]));
...
describe('Conflict Detector Tests', () => {
  ...
  it('should check for conflict', () => {
    const shortcuts = [{ shortcut: 'Ctrl+S', context: 'global' }];
