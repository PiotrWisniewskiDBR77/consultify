...
export interface CustomMatchers<R = unknown> {
  ...
  // Phase 2 Advanced Matchers
  toBeAccessible(): R;
  toHandlePartialFailure(): R;
  toRespectDeadlines(timeoutMs: number): R;
}
...
expect.extend({
  ...
  toBePositive(received: number) {
    ...
    return {
      pass,
      message: () =>
        pass ? `Expected ${received} not to be positive` : `Expected ${received} to be positive`,
    };
  },
  ...
  toBeNegative(received: number) {
    ...
    return {
      pass,
      message: () =>
        pass ? `Expected ${received} not to be negative` : `Expected ${received} to be negative`,
    };
  },
  ...
  toBeNonEmpty(received: unknown[]) {
    ...
    return {
      pass,
      message: () => (pass ? `Expected array to be empty` : `Expected array to be non-empty`),
    };
  },
  ...
  toBeValidUser(received: Record<string, unknown>) {
    ...
    const hasValidEmail =
      typeof received.email === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(received.email);
  ...
  toBeAccessible(received: HTMLElement) {
    ...
    const pass = hasAria || hasRole;
    return {
      pass,
      message: () =>
        pass
          ? `Expected element not to be accessible`
          : `Expected element to be accessible (missing aria labels or roles)`,
    };
  },

  toHandlePartialFailure(received: () => any) {
    let pass = false;
    try {
      received();
      pass = true; // If it doesn't crash on partial failure
    } catch (e) {
      pass = false;
    }
    return {
      ...
      message: () =>
        pass
          ? `Expected not to handle partial failure`
          : `Expected to handle partial failure gracefully`,
    };
  },

  async toRespectDeadlines(received: () => Promise<any>, timeoutMs: number) {
    ...
    try {
      ...
      pass = duration <= timeoutMs + 50; // 50ms buffer
    } catch (e) {
      pass = e.message === 'Timeout';
    }
    ...
    return {
      ...
      message: () =>
        pass
          ? `Expected not to respect deadline of ${timeoutMs}ms`
          : `Expected to respect deadline of ${timeoutMs}ms`,
    };
  },
});
