...
const createFactoryBuilder = () => {
  ...
  return {
    ...
    build: (name, overrides = {}, options = {}) => {
      ...
      const defaults =
        typeof factory.defaults === 'function' ? factory.defaults(seq) : { ...factory.defaults };
...
describe('Test Data Builder Tests', () => {
  ...
  it('should remove properties', () => {
    const data = createTestDataBuilder({ a: 1, b: 2, c: 3 }).without('b').build();
  ...
  it('should merge objects', () => {
    const data = createTestDataBuilder({ a: 1 }).merge({ b: 2, c: 3 }).build();
