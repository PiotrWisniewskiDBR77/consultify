...
const PromptSanitizer = {
  ...
  detectJailbreakAttempt: (input) => {
    ...
    return {
      ...
      indicators: jailbreakIndicators.filter((i) => lowerInput.includes(i.toLowerCase())),
    };
  },
};
...
describe('Rate Limiting for Suspicious Activity', () => {
  test('should detect repeated injection attempts', () => {
    ...
    const shouldRateLimit = (userId) => {
      const now = Date.now();
      const recentMalicious = userAttempts.filter(
        (a) => a.userId === userId && a.isMalicious && now - a.timestamp < TIME_WINDOW
      );
