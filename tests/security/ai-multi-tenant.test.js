...
describe('AI Multi-Tenant Security Tests', () => {
  ...
  describe('Context Builder Isolation', () => {
    test('should not include cross-org data in AI context', async () => {
      ...
      expect(contextA.projects).not.toContainEqual(expect.objectContaining({ id: PROJECT_B.id }));
      expect(contextA.crossOrgData).toBeNull();
    });

    test('should filter relevant memory by organization', async () => {
      AIMemoryManager.getRelevantMemory.mockImplementation(
  ...
  describe('Data Query Isolation', () => {
    test('should include organization filter in all AI data queries', () => {
      ...
      const expectedFilterPatterns = ['organization_id = ?', 'organizationId', 'org_id'];
  ...
  describe('Audit Trail Integrity', () => {
    test('should log organization context for all AI operations', () => {
      ...
      const operations = [
        ...
        {
          event: 'AI_ACTION_CREATED',
          context: { userId: USER_A.id, organizationId: ORG_A.id, actionId: 'a1' },
        },
        {
          event: 'AI_RESPONSE_GENERATED',
          context: { userId: USER_B.id, organizationId: ORG_B.id },
        },
      ];
