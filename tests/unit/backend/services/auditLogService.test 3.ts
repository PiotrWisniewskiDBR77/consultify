...
describe('AuditLogService', () => {
    ...
    describe('Audit Log Querying', () => {
        it('should filter by organization', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO audit_logs (id, organization_id, action, resource_type) VALUES (?, ?, ?, ?)',
                        ['log-1', 'org-A', 'CREATE', 'task']);
                    db.run('INSERT INTO audit_logs (id, organization_id, action, resource_type) VALUES (?, ?, ?, ?)',
                        ['log-2', 'org-B', 'DELETE', 'task']);
                    db.run('INSERT INTO audit_logs (id, organization_id, action, resource_type) VALUES (?, ?, ?, ?)',
                        ['log-3', 'org-A', 'UPDATE', 'task'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
        ...
        it('should order by timestamp descending', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO audit_logs (id, action, resource_type) VALUES (?, ?, ?)',
                        ['log-first', 'CREATE', 'task']);
                    db.run('INSERT INTO audit_logs (id, action, resource_type) VALUES (?, ?, ?)',
                        ['log-second', 'UPDATE', 'task']);
                    db.run('INSERT INTO audit_logs (id, action, resource_type) VALUES (?, ?, ?)',
                        ['log-third', 'DELETE', 'task'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
    ...
    describe('Audit Log Security Features', () => {
        ...
        it('should support correlation IDs for request tracing', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO audit_logs (id, action, resource_type, correlation_id) VALUES (?, ?, ?, ?)',
                        ['log-1', 'READ', 'user', correlationId]);
                    db.run('INSERT INTO audit_logs (id, action, resource_type, correlation_id) VALUES (?, ?, ?, ?)',
                        ['log-2', 'UPDATE', 'user', correlationId], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
            ...
            const correlatedLogs = await new Promise<any[]>((resolve, reject) => {
                db.all('SELECT * FROM audit_logs WHERE correlation_id = ?', [correlationId], (err, rows) => {
                    if (err) reject(err);
                    else resolve(rows);
                });
            });
