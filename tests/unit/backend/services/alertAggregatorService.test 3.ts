...
describe('AlertAggregatorService', () => {
        ...
        it('should create system alert', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [alertId, 'error', 'high', 'Database Connection Failed', 'Unable to connect to primary database', 'db-monitor'],
    ...
    describe('Alert Aggregation', () => {
        it('should count alerts by severity', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO system_alerts (id, alert_type, severity, title) VALUES (?, ?, ?, ?)',
                    ...
                    db.run('INSERT INTO system_alerts (id, alert_type, severity, title) VALUES (?, ?, ?, ?)',
                        ['a-4', 'error', 'critical', 'Critical 2'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
        ...
        it('should get unresolved alerts ordered by severity', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO system_alerts (id, alert_type, severity, title, is_resolved) VALUES (?, ?, ?, ?, ?)',
                        ['a-1', 'info', 'low', 'Low Priority', 0]);
                    db.run('INSERT INTO system_alerts (id, alert_type, severity, title, is_resolved) VALUES (?, ?, ?, ?, ?)',
                        ['a-2', 'error', 'critical', 'Critical Issue', 0]);
                    db.run('INSERT INTO system_alerts (id, alert_type, severity, title, is_resolved) VALUES (?, ?, ?, ?, ?)',
                        ['a-3', 'error', 'critical', 'Already Fixed', 1], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
