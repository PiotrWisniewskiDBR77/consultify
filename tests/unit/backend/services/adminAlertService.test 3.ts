...
describe('AdminAlertService', () => {
    ...
    describe('Alert CRUD', () => {
        it('should create admin alert', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [alertId, 'security', 'high', 'Suspicious Activity', 'Multiple failed login attempts detected'],
        ...
        it('should mark alert as read', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE admin_alerts SET is_read = 1 WHERE id = ?', [alertId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Alert Queries', () => {
        it('should get unread alerts', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO admin_alerts (id, alert_type, severity, title, is_read) VALUES (?, ?, ?, ?, ?)',
                        ['a1', 'system', 'low', 'Alert 1', 0]);
                    db.run('INSERT INTO admin_alerts (id, alert_type, severity, title, is_read) VALUES (?, ?, ?, ?, ?)',
                        ['a2', 'security', 'high', 'Alert 2', 0]);
                    db.run('INSERT INTO admin_alerts (id, alert_type, severity, title, is_read) VALUES (?, ?, ?, ?, ?)',
                        ['a3', 'billing', 'medium', 'Alert 3', 1], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
