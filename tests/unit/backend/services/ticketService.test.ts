...
describe('TicketService', () => {
  ...
  describe('Ticket CRUD', () => {
    ...
    it('should resolve ticket', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO tickets (id, organization_id, user_id, subject) VALUES (?, ?, ?, ?)',
          [ticketId, 'org-1', 'u-1', 'Help'],
          (err) => (err ? reject(err) : resolve())
        );
      });
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'UPDATE tickets SET status = ?, resolved_at = datetime("now") WHERE id = ?',
          ['resolved', ticketId],
          (err) => (err ? reject(err) : resolve())
        );
      });
  ...
  describe('Ticket Queries', () => {
    it('should get open tickets', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO tickets (id, organization_id, user_id, subject, status) VALUES (?, ?, ?, ?, ?)',
            ['t1', 'o1', 'u1', 'T1', 'open']
          );
          db.run(
            'INSERT INTO tickets (id, organization_id, user_id, subject, status) VALUES (?, ?, ?, ?, ?)',
            ['t2', 'o1', 'u2', 'T2', 'resolved']
          );
          db.run(
            'INSERT INTO tickets (id, organization_id, user_id, subject, status) VALUES (?, ?, ?, ?, ?)',
            ['t3', 'o1', 'u3', 'T3', 'open'],
