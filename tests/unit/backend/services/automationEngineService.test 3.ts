...
describe('AutomationEngineService', () => {
    ...
    describe('Automation Rules', () => {
        it('should create automation rule', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [ruleId, 'org-123', 'High Priority Notification', 'event', JSON.stringify(triggerConfig), 'notification', JSON.stringify(actionConfig)],
        ...
        it('should toggle rule active status', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE automation_rules SET is_active = 0 WHERE id = ?', [ruleId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Automation Execution', () => {
        it('should log automation execution', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO automation_logs (id, rule_id, status, input_data) VALUES (?, ?, ?, ?)',
                        [`log-${Date.now()}`, ruleId, 'success', JSON.stringify({ taskId: 'task-123' })]);
                    db.run('UPDATE automation_rules SET trigger_count = trigger_count + 1, last_triggered_at = datetime("now") WHERE id = ?',
                        [ruleId], (err) => err ? reject(err) : resolve());
                });
            });
