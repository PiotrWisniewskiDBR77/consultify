...
describe('CompetitiveIntelligenceService', () => {
    ...
    describe('Market Updates', () => {
        it('should track competitor updates', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO competitor_updates (id, competitor_id, update_type, content, relevance_score) VALUES (?, ?, ?, ?, ?)',
                        ['u1', competitorId, 'product_launch', 'New AI feature released', 0.9]);
                    db.run('INSERT INTO competitor_updates (id, competitor_id, update_type, content, relevance_score) VALUES (?, ?, ?, ?, ?)',
                        ['u2', competitorId, 'pricing_change', 'Reduced enterprise tier by 20%', 0.85], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
            ...
            const updates = await new Promise<any[]>((resolve, reject) => {
                db.all('SELECT * FROM competitor_updates WHERE competitor_id = ? ORDER BY relevance_score DESC', [competitorId], (err, rows) => {
                    if (err) reject(err);
                    else resolve(rows);
                });
            });
