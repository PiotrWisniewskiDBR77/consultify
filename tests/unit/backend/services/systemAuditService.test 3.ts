...
describe('SystemAuditService', () => {
        ...
        it('should store change details', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [logId, 'org-1', 'update', 'account', JSON.stringify(oldValues), JSON.stringify(newValues)],
                    (err) => err ? reject(err) : resolve()
                );
            });
    ...
    describe('Severity Filtering', () => {
        it('should filter by severity level', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO system_audit_logs (id, organization_id, action, resource_type, severity) VALUES (?, ?, ?, ?, ?)', ['l1', 'o1', 'create', 'user', 'info']);
                    db.run('INSERT INTO system_audit_logs (id, organization_id, action, resource_type, severity) VALUES (?, ?, ?, ?, ?)', ['l2', 'o1', 'delete', 'user', 'critical']);
                    db.run('INSERT INTO system_audit_logs (id, organization_id, action, resource_type, severity) VALUES (?, ?, ?, ?, ?)', ['l3', 'o1', 'update', 'user', 'warning'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
