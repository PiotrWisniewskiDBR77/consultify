...
describe('RoleService', () => {
    ...
    describe('Role CRUD', () => {
        ...
        it('should update role permissions', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO roles (id, organization_id, name, permissions) VALUES (?, ?, ?, ?)', [roleId, 'org-1', 'Editor', '["read"]'], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE roles SET permissions = ? WHERE id = ?', ['["read","write"]', roleId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('User Role Assignment', () => {
        it('should assign role to user', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO roles (id, organization_id, name) VALUES (?, ?, ?)', [roleId, 'org-1', 'Member']);
                    db.run('INSERT INTO user_roles (id, user_id, role_id, assigned_by) VALUES (?, ?, ?, ?)', ['ur-1', 'user-123', roleId, 'admin-1'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
