...
describe('ContractService', () => {
    ...
    describe('Contract CRUD', () => {
        ...
        it('should sign contract', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO contracts (id, organization_id, client_id, title) VALUES (?, ?, ?, ?)', [contractId, 'org-1', 'c-1', 'Test'], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE contracts SET status = ?, signed_at = datetime("now") WHERE id = ?', ['signed', contractId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Contract Queries', () => {
        it('should get active contracts', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO contracts (id, organization_id, client_id, title, status) VALUES (?, ?, ?, ?, ?)', ['c1', 'o1', 'cl1', 'Active 1', 'active']);
                    db.run('INSERT INTO contracts (id, organization_id, client_id, title, status) VALUES (?, ?, ?, ?, ?)', ['c2', 'o1', 'cl2', 'Expired', 'expired']);
                    db.run('INSERT INTO contracts (id, organization_id, client_id, title, status) VALUES (?, ?, ?, ?, ?)', ['c3', 'o1', 'cl3', 'Active 2', 'active'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
