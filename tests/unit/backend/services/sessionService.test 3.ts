...
describe('SessionService', () => {
    ...
    describe('Session Management', () => {
        ...
        it('should delete expired sessions', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO sessions (id, user_id, token, expires_at) VALUES (?, ?, ?, ?)', ['s1', 'u1', 't1', '2025-01-01']);
                    db.run('INSERT INTO sessions (id, user_id, token, expires_at) VALUES (?, ?, ?, ?)', ['s2', 'u2', 't2', '2027-01-01'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('DELETE FROM sessions WHERE expires_at < datetime("now")', (err) => err ? reject(err) : resolve());
            });
