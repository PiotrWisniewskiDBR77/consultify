...
describe('FeatureFlagService', () => {
    ...
    describe('Flag CRUD', () => {
        ...
        it('should toggle flag', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO feature_flags (id, name, is_enabled) VALUES (?, ?, ?)', [flagId, 'test-flag', 0], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE feature_flags SET is_enabled = 1 WHERE id = ?', [flagId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Rollout', () => {
        it('should set rollout percentage', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO feature_flags (id, name, rollout_percentage) VALUES (?, ?, ?)', [flagId, 'gradual-rollout', 25], (err) => err ? reject(err) : resolve());
            });
        ...
        it('should target specific users', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO feature_flags (id, name, target_users) VALUES (?, ?, ?)', [flagId, 'beta-feature', JSON.stringify(targetUsers)], (err) => err ? reject(err) : resolve());
            });
