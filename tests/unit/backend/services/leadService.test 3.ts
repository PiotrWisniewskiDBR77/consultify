...
describe('LeadService', () => {
    ...
    describe('Lead CRUD', () => {
        ...
        it('should update lead status', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO leads (id, organization_id, email) VALUES (?, ?, ?)', [leadId, 'org-1', 'test@example.com'], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE leads SET status = ?, score = ? WHERE id = ?', ['qualified', 75, leadId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Lead Queries', () => {
        it('should get high score leads', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO leads (id, organization_id, email, score) VALUES (?, ?, ?, ?)', ['l1', 'o1', 'hot@ex.com', 90]);
                    db.run('INSERT INTO leads (id, organization_id, email, score) VALUES (?, ?, ?, ?)', ['l2', 'o1', 'cold@ex.com', 20]);
                    db.run('INSERT INTO leads (id, organization_id, email, score) VALUES (?, ?, ?, ?)', ['l3', 'o1', 'warm@ex.com', 80], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
