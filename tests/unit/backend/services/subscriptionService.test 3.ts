...
describe('SubscriptionService', () => {
    ...
    describe('Subscription CRUD', () => {
        ...
        it('should cancel subscription', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO subscriptions (id, organization_id, plan_id) VALUES (?, ?, ?)', [subId, 'org-1', 'plan-1'], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE subscriptions SET status = ?, canceled_at = datetime("now") WHERE id = ?', ['canceled', subId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Subscription Queries', () => {
        it('should get active subscriptions', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO subscriptions (id, organization_id, plan_id, status) VALUES (?, ?, ?, ?)', ['s1', 'o1', 'p1', 'active']);
                    db.run('INSERT INTO subscriptions (id, organization_id, plan_id, status) VALUES (?, ?, ?, ?)', ['s2', 'o2', 'p2', 'active']);
                    db.run('INSERT INTO subscriptions (id, organization_id, plan_id, status) VALUES (?, ?, ?, ?)', ['s3', 'o3', 'p3', 'canceled'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
