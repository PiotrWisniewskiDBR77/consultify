...
describe('AttachmentService', () => {
    ...
    describe('Attachment CRUD', () => {
        it('should create attachment', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [attachmentId, 'task', 'task-123', 'file-abc.pdf', 'document.pdf', 'application/pdf', 1024, '/uploads/file-abc.pdf', 'user-456'],
        ...
        it('should delete attachment', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('DELETE FROM attachments WHERE id = ?', [attachmentId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Attachment Queries', () => {
        it('should get attachments by entity', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO attachments (id, entity_type, entity_id, filename, original_filename, storage_path) VALUES (?, ?, ?, ?, ?, ?)',
                        ['a1', 'task', 'task-A', 'f1.pdf', 'doc1.pdf', '/u/f1.pdf']);
                    db.run('INSERT INTO attachments (id, entity_type, entity_id, filename, original_filename, storage_path) VALUES (?, ?, ?, ?, ?, ?)',
                        ['a2', 'task', 'task-A', 'f2.pdf', 'doc2.pdf', '/u/f2.pdf']);
                    db.run('INSERT INTO attachments (id, entity_type, entity_id, filename, original_filename, storage_path) VALUES (?, ?, ?, ?, ?, ?)',
                        ['a3', 'task', 'task-B', 'f3.pdf', 'doc3.pdf', '/u/f3.pdf'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
