...
describe('NotificationPreferenceService', () => {
    ...
    describe('Preference Queries', () => {
        it('should get user preferences by channel', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO notification_preferences (id, user_id, channel, event_type) VALUES (?, ?, ?, ?)', ['p1', 'user-1', 'email', 'event1']);
                    db.run('INSERT INTO notification_preferences (id, user_id, channel, event_type) VALUES (?, ?, ?, ?)', ['p2', 'user-1', 'push', 'event2']);
                    db.run('INSERT INTO notification_preferences (id, user_id, channel, event_type) VALUES (?, ?, ?, ?)', ['p3', 'user-1', 'email', 'event3'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
            ...
            const emailPrefs = await new Promise<any[]>((resolve, reject) => {
                db.all('SELECT * FROM notification_preferences WHERE user_id = ? AND channel = ?', ['user-1', 'email'], (err, rows) => {
                    if (err) reject(err);
                    else resolve(rows);
                });
            });
