...
describe('CollaborationAIService', () => {
    ...
    describe('AI Suggestions', () => {
        it('should store AI suggestion', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [suggestionId, 'org-123', 'document', 'improvement', 'Consider adding more context to this section', 0.85],
        ...
        it('should track suggestion acceptance rate', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO ai_suggestions (id, organization_id, context_type, suggestion_type, content, status, accepted_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now"))',
                        ['s1', orgId, 'doc', 'improve', 'Suggestion 1', 'accepted']);
                    db.run('INSERT INTO ai_suggestions (id, organization_id, context_type, suggestion_type, content, status, accepted_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now"))',
                        ['s2', orgId, 'doc', 'improve', 'Suggestion 2', 'accepted']);
                    db.run('INSERT INTO ai_suggestions (id, organization_id, context_type, suggestion_type, content, status, rejected_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now"))',
                        ['s3', orgId, 'doc', 'improve', 'Suggestion 3', 'rejected']);
                    db.run('INSERT INTO ai_suggestions (id, organization_id, context_type, suggestion_type, content, status) VALUES (?, ?, ?, ?, ?, ?)',
                        ['s4', orgId, 'doc', 'improve', 'Suggestion 4', 'pending'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
