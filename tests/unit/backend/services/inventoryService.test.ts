...
describe('InventoryService', () => {
  ...
  describe('Inventory CRUD', () => {
    ...
    it('should adjust quantity', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO inventory (id, organization_id, sku, name, quantity) VALUES (?, ?, ?, ?, ?)',
          [itemId, 'org-1', 'SKU-002', 'Product', 50],
          (err) => (err ? reject(err) : resolve())
        );
      });

      await new Promise<void>((resolve, reject) => {
        db.run('UPDATE inventory SET quantity = quantity - 10 WHERE id = ?', [itemId], (err) =>
  ...
  describe('Inventory Queries', () => {
    it('should get low stock items', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO inventory (id, organization_id, sku, name, quantity, min_quantity) VALUES (?, ?, ?, ?, ?, ?)',
            ['i1', 'o1', 'S1', 'Low Item', 5, 10]
          );
          db.run(
            'INSERT INTO inventory (id, organization_id, sku, name, quantity, min_quantity) VALUES (?, ?, ?, ?, ?, ?)',
            ['i2', 'o1', 'S2', 'Good Item', 50, 10]
          );
          db.run(
            'INSERT INTO inventory (id, organization_id, sku, name, quantity, min_quantity) VALUES (?, ?, ?, ?, ?, ?)',
            ['i3', 'o1', 'S3', 'Critical', 2, 20],
            (err) => {
              if (err) reject(err);
              else resolve();
            }
