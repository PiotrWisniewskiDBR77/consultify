...
describe('StageGateService', () => {
    ...
    describe('getByProject()', () => {
        it('should return gates sorted by stage', async () => {
            await stageGateService.create({ projectId: 'proj-1', name: 'Gate 3', stage: 3, criteria: [], approvers: [] });
            await stageGateService.create({ projectId: 'proj-1', name: 'Gate 1', stage: 1, criteria: [], approvers: [] });
            await stageGateService.create({ projectId: 'proj-1', name: 'Gate 2', stage: 2, criteria: [], approvers: [] });
            await stageGateService.create({ projectId: 'proj-2', name: 'Other', stage: 1, criteria: [], approvers: [] });
    ...
    describe('submitForReview()', () => {
        it('should reject submission if required criteria not met', async () => {
            ...
            await expect(
                stageGateService.submitForReview(gate.id, 'submitter-1')
            ).rejects.toThrow('Required criteria not met');
        });
    ...
    describe('approve()', () => {
        ...
        it('should reject approval from unauthorized user', async () => {
            ...
            await expect(
                stageGateService.approve(gate.id, 'unauthorized-user')
            ).rejects.toThrow('User not authorized to approve');
        });
    });
    ...
    describe('reject()', () => {
        it('should reject gate with reason', async () => {
            ...
            const rejected = await stageGateService.reject(gate.id, 'approver-1', 'Missing documentation');
