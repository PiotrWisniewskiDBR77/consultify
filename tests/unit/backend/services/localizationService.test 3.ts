...
describe('LocalizationService', () => {
    ...
    describe('Translation CRUD', () => {
        ...
        it('should update translation', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO translations (id, key, locale, value) VALUES (?, ?, ?, ?)', [translationId, 'greeting', 'en', 'Hello'], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE translations SET value = ? WHERE id = ?', ['Hi there', translationId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Translation Queries', () => {
        it('should get translations by locale', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO translations (id, key, locale, value) VALUES (?, ?, ?, ?)', ['t1', 'hello', 'pl', 'Cześć']);
                    db.run('INSERT INTO translations (id, key, locale, value) VALUES (?, ?, ?, ?)', ['t2', 'goodbye', 'pl', 'Do widzenia']);
                    db.run('INSERT INTO translations (id, key, locale, value) VALUES (?, ?, ?, ?)', ['t3', 'hello', 'de', 'Hallo'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
