...
describe('MFAService', () => {
    ...
    describe('MFA Factor Management', () => {
        ...
        it('should set primary factor', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO mfa_factors (id, user_id, factor_type, is_primary) VALUES (?, ?, ?, ?)',
                        ['mfa-1', userId, 'totp', 1]);
                    db.run('INSERT INTO mfa_factors (id, user_id, factor_type, is_primary) VALUES (?, ?, ?, ?)',
                        ['mfa-2', userId, 'sms', 0], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
            ...
            const primaryFactor = await new Promise<any>((resolve, reject) => {
                db.get('SELECT * FROM mfa_factors WHERE user_id = ? AND is_primary = 1', [userId], (err, row) => {
                    if (err) reject(err);
                    else resolve(row);
                });
            });
