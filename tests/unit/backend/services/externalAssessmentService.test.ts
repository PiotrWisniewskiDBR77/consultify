  ...
  describe('Assessment Queries', () => {
    it('should list assessments by type', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO external_assessments (id, organization_id, assessment_type) VALUES (?, ?, ?)',
      ...
      const securityAudits = await new Promise<any[]>((resolve, reject) => {
        db.all(
          'SELECT * FROM external_assessments WHERE assessment_type = ?',
          ['security_audit'],
          (err, rows) => {
            if (err) reject(err);
            else resolve(rows);
