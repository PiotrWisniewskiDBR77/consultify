  ...
  describe('Benchmark Recording', () => {
    ...
    it('should calculate performance vs industry average', async () => {
      ...
      const result = await new Promise<any>((resolve, reject) => {
        db.get(
          'SELECT score, industry_avg, (score - industry_avg) as delta FROM benchmarks WHERE id = ?',
          ['b1'],
          (err, row) => {
            if (err) reject(err);
            else resolve(row);
  ...
  describe('Benchmark Analytics', () => {
    it('should get organization benchmark summary', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO benchmarks (id, organization_id, name, category, score) VALUES (?, ?, ?, ?, ?)',
