...
const createDLPService = () => {
    const policies = new Map([
        ['pii-detection', { id: 'pii-detection', name: 'PII Detection', enabled: true, patterns: ['ssn', 'credit_card', 'email'] }],
        ['file-exfiltration', { id: 'file-exfiltration', name: 'File Exfiltration', enabled: true, maxFileSize: 10485760 }]
    ]);
    const violations = [];
    ...
    return {
        ...
        scanContent: async (content, policyId) => {
            ...
            return { success: true, violations: detected, hasViolation: detected.length > 0, status: 200 };
        },

        // Report violation
        reportViolation: async (userId, policyId, details) => {
            if (!userId || !policyId) return { success: false, error: 'Missing required fields', status: 400 };
            const violation = { id: `viol-${Date.now()}`, userId, policyId, details, timestamp: new Date() };
...
describe('DLPService', () => {
    ...
    describe('Violation Reporting', () => {
        it('should report violation', async () => {
            const result = await dlpService.reportViolation('user-1', 'pii-detection', { data: 'SSN detected' });
            expect(result.success).toBe(true);
