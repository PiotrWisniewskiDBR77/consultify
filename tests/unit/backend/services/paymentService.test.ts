...
describe('PaymentService', () => {
  ...
  describe('Payment CRUD', () => {
    ...
    it('should process payment', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO payments (id, organization_id, amount) VALUES (?, ?, ?)',
          [paymentId, 'org-1', 50.0],
          (err) => (err ? reject(err) : resolve())
        );
      });
  ...
  describe('Payment Queries', () => {
    it('should get successful payments', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run('INSERT INTO payments (id, organization_id, amount, status) VALUES (?, ?, ?, ?)', [
            'p1',
            'o1',
            100,
            'succeeded',
          ...
          db.run(
            'INSERT INTO payments (id, organization_id, amount, status) VALUES (?, ?, ?, ?)',
            ['p3', 'o1', 300, 'succeeded'],
            (err) => {
              if (err) reject(err);
              else resolve();
