...
describe('ReportGeneratorService', () => {
  ...
  describe('Report Generation', () => {
    ...
    it('should update report status on completion', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO generated_reports (id, organization_id, user_id, title, type) VALUES (?, ?, ?, ?, ?)',
          [reportId, 'org-1', 'user-1', 'Test Report', 'pdf'],
          (err) => (err ? reject(err) : resolve())
        );
      });
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'UPDATE generated_reports SET status = ?, file_path = ?, file_size = ?, generated_at = datetime("now") WHERE id = ?',
          ['completed', '/reports/test.pdf', 102400, reportId],
          (err) => (err ? reject(err) : resolve())
        );
      });
    ...
    it('should get user reports', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO generated_reports (id, organization_id, user_id, title, type) VALUES (?, ?, ?, ?, ?)',
            ['r1', 'o1', 'user-A', 'Report 1', 'pdf']
          );
          db.run(
            'INSERT INTO generated_reports (id, organization_id, user_id, title, type) VALUES (?, ?, ?, ?, ?)',
            ['r2', 'o1', 'user-A', 'Report 2', 'csv']
          );
          db.run(
            'INSERT INTO generated_reports (id, organization_id, user_id, title, type) VALUES (?, ?, ?, ?, ?)',
            ['r3', 'o1', 'user-B', 'Report 3', 'pdf'],
            (err) => {
              if (err) reject(err);
              else resolve();
            }
