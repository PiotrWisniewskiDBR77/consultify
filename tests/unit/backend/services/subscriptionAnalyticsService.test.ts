  ...
  describe('MRR Calculations', () => {
    it('should calculate Monthly Recurring Revenue', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO subscriptions (id, organization_id, plan_id, amount_cents, status) VALUES (?, ?, ?, ?, ?)',
      ...
      const result = await new Promise<any>((resolve, reject) => {
        db.get(
          'SELECT SUM(amount_cents) as mrr FROM subscriptions WHERE status = ?',
          ['active'],
          (err, row) => {
            if (err) reject(err);
            else resolve(row);
    ...
    it('should calculate ARR from monthly subscriptions', async () => {
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO subscriptions (id, organization_id, plan_id, amount_cents, status, billing_cycle) VALUES (?, ?, ?, ?, ?, ?)',
      ...
      const result = await new Promise<any>((resolve, reject) => {
        db.get(
          'SELECT SUM(amount_cents * 12) as arr FROM subscriptions WHERE status = ? AND billing_cycle = ?',
          ['active', 'monthly'],
          (err, row) => {
            if (err) reject(err);
            else resolve(row);
  ...
  describe('Churn Analysis', () => {
    it('should identify churned subscriptions', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO subscriptions (id, organization_id, plan_id, status, canceled_at) VALUES (?, ?, ?, ?, ?)',
          ...
          db.run(
            'INSERT INTO subscriptions (id, organization_id, plan_id, status, canceled_at) VALUES (?, ?, ?, ?, ?)',
            ['sub-3', 'org-3', 'pro', 'canceled', '2026-01-08'],
            (err) => {
              if (err) reject(err);
              else resolve();
            }
    ...
    it('should calculate churn rate', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          ...
          db.run(
            'INSERT INTO subscriptions (id, organization_id, plan_id, status) VALUES (?, ?, ?, ?)',
            ['sub-churn-2', 'org-churn-2', 'pro', 'canceled'],
            (err) => {
              if (err) reject(err);
              else resolve();
            }
