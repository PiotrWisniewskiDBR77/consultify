...
describe('KnowledgeService', () => {
        ...
        it('should create knowledge item', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [itemId, 'org-123', 'API Documentation', 'This is how to use the API...', 'documentation'],
    ...
    describe('Knowledge Search', () => {
        it('should search by category', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO knowledge_items (id, organization_id, title, category) VALUES (?, ?, ?, ?)',
                        ['kb-1', 'org-1', 'API Guide', 'api']);
                    db.run('INSERT INTO knowledge_items (id, organization_id, title, category) VALUES (?, ?, ?, ?)',
                        ['kb-2', 'org-1', 'User Manual', 'manual']);
                    db.run('INSERT INTO knowledge_items (id, organization_id, title, category) VALUES (?, ?, ?, ?)',
                        ['kb-3', 'org-1', 'API Reference', 'api'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
        ...
        it('should full-text search in title', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO knowledge_items (id, organization_id, title) VALUES (?, ?, ?)',
                        ['kb-1', 'org-1', 'Getting Started with Authentication']);
                    db.run('INSERT INTO knowledge_items (id, organization_id, title) VALUES (?, ?, ?)',
                        ['kb-2', 'org-1', 'Database Migration Guide']);
                    db.run('INSERT INTO knowledge_items (id, organization_id, title) VALUES (?, ?, ?)',
                        ['kb-3', 'org-1', 'Authentication Best Practices'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
            ...
            const authItems = await new Promise<any[]>((resolve, reject) => {
                db.all("SELECT * FROM knowledge_items WHERE title LIKE ?", ['%Authentication%'], (err, rows) => {
                    if (err) reject(err);
                    else resolve(rows);
                });
            });
        ...
        it('should filter public items', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO knowledge_items (id, organization_id, title, is_public) VALUES (?, ?, ?, ?)',
                        ['kb-1', 'org-1', 'Public FAQ', 1]);
                    db.run('INSERT INTO knowledge_items (id, organization_id, title, is_public) VALUES (?, ?, ?, ?)',
                        ['kb-2', 'org-1', 'Internal Docs', 0]);
                    db.run('INSERT INTO knowledge_items (id, organization_id, title, is_public) VALUES (?, ?, ?, ?)',
                        ['kb-3', 'org-1', 'Public Guide', 1], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
