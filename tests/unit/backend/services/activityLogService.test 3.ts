...
describe('ActivityLogService', () => {
    ...
    describe('Activity Queries', () => {
        it('should get activities by user', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO activity_logs (id, organization_id, user_id, activity_type) VALUES (?, ?, ?, ?)', ['a1', 'org-1', 'user-A', 'login']);
                    db.run('INSERT INTO activity_logs (id, organization_id, user_id, activity_type) VALUES (?, ?, ?, ?)', ['a2', 'org-1', 'user-A', 'task.created']);
                    db.run('INSERT INTO activity_logs (id, organization_id, user_id, activity_type) VALUES (?, ?, ?, ?)', ['a3', 'org-1', 'user-B', 'login'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
