...
describe('FeedbackAIService', () => {
    ...
    describe('Feedback Analytics', () => {
        it('should calculate average rating', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO ai_feedback (id, user_id, feedback_type, rating) VALUES (?, ?, ?, ?)', ['f1', 'u1', 'rating', 5]);
                    db.run('INSERT INTO ai_feedback (id, user_id, feedback_type, rating) VALUES (?, ?, ?, ?)', ['f2', 'u2', 'rating', 4]);
                    db.run('INSERT INTO ai_feedback (id, user_id, feedback_type, rating) VALUES (?, ?, ?, ?)', ['f3', 'u3', 'rating', 4]);
                    db.run('INSERT INTO ai_feedback (id, user_id, feedback_type, rating) VALUES (?, ?, ?, ?)', ['f4', 'u4', 'rating', 3], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
            ...
            const result = await new Promise<any>((resolve, reject) => {
                db.get('SELECT AVG(rating) as avg_rating FROM ai_feedback WHERE rating IS NOT NULL', (err, row) => {
                    if (err) reject(err);
                    else resolve(row);
                });
            });
