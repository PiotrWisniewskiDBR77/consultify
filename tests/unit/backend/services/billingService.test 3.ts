            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO subscriptions (id, organization_id, plan_id) VALUES (?, ?, ?)',
                        [subId, 'org-123', 'plan-pro']);
                    db.run('INSERT INTO invoices (id, subscription_id, organization_id, amount, due_date) VALUES (?, ?, ?, ?, ?)',
                        [invId, subId, 'org-123', 99.99, '2026-02-01'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
