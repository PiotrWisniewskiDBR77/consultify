...
describe('AnalyticsEventService', () => {
        ...
        it('should track event', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [eventId, 'org-123', 'user-456', 'click', 'button_click', JSON.stringify({ button: 'submit' })],
    ...
    describe('Event Queries', () => {
        it('should get events by type', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO analytics_events (id, organization_id, event_type, event_name) VALUES (?, ?, ?, ?)', ['e1', 'o1', 'click', 'btn_click']);
                    db.run('INSERT INTO analytics_events (id, organization_id, event_type, event_name) VALUES (?, ?, ?, ?)', ['e2', 'o1', 'pageview', 'pageview']);
                    db.run('INSERT INTO analytics_events (id, organization_id, event_type, event_name) VALUES (?, ?, ?, ?)', ['e3', 'o1', 'click', 'link_click'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
