    ...
    it('should configure auto-approval threshold', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          ...
          [patternId, 'org-123', 'Small Expenses', 'auto', 500.0],
          (err) => (err ? reject(err) : resolve())
        );
      });
      ...
      expect(pattern.auto_approve_threshold).toBe(500.0);
    });
  });
    ...
    it('should approve request', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          ...
          db.run(
            'INSERT INTO approval_requests (id, pattern_id, requestor_id) VALUES (?, ?, ?)',
            ['req-approve', patternId, 'user-456'],
            (err) => {
              if (err) reject(err);
              else resolve();
            }
