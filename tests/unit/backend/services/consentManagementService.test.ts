  ...
  describe('Consent History', () => {
    it('should track consent changes', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO consent_history (id, user_id, consent_type, action, ip_address) VALUES (?, ?, ?, ?, ?)',
          ...
          db.run(
            'INSERT INTO consent_history (id, user_id, consent_type, action, ip_address) VALUES (?, ?, ?, ?, ?)',
            ['h3', userId, 'marketing', 'granted', '192.168.1.3'],
            (err) => {
              if (err) reject(err);
              else resolve();
            }
      ...
      const history = await new Promise<any[]>((resolve, reject) => {
        db.all(
          'SELECT * FROM consent_history WHERE user_id = ? ORDER BY created_at',
          [userId],
          (err, rows) => {
            if (err) reject(err);
            else resolve(rows);
  ...
  describe('Consent Verification', () => {
    it('should check if user has granted consent', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO user_consents (id, user_id, consent_type, is_granted) VALUES (?, ?, ?, ?)',
            ['c1', userId, 'analytics', 1]
          );
          db.run(
            'INSERT INTO user_consents (id, user_id, consent_type, is_granted) VALUES (?, ?, ?, ?)',
            ['c2', userId, 'marketing', 0],
            (err) => {
              if (err) reject(err);
              else resolve();
            }
      ...
      const analyticsConsent = await new Promise<any>((resolve, reject) => {
        db.get(
          'SELECT is_granted FROM user_consents WHERE user_id = ? AND consent_type = ?',
          [userId, 'analytics'],
          (err, row) => {
            if (err) reject(err);
            else resolve(row);
