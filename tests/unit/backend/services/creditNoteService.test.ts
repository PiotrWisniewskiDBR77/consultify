    ...
    it('should create credit note', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          ...
          [noteId, 'org-123', 'inv-456', 150.0, 'Service credit', 'admin'],
  ...
  describe('Credit Note Analytics', () => {
    it('should calculate total credits by organization', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO credit_notes (id, organization_id, amount, status) VALUES (?, ?, ?, ?)',
            ['c1', 'org-A', 100, 'applied']
          );
          db.run(
            'INSERT INTO credit_notes (id, organization_id, amount, status) VALUES (?, ?, ?, ?)',
            ['c2', 'org-A', 50, 'applied']
      ...
      const result = await new Promise<any>((resolve, reject) => {
        db.get(
          'SELECT SUM(amount) as total FROM credit_notes WHERE organization_id = ? AND status = ?',
          ['org-A', 'applied'],
          (err, row) => {
            if (err) reject(err);
            else resolve(row);
