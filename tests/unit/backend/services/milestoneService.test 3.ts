...
describe('MilestoneService', () => {
    ...
    describe('Milestone CRUD', () => {
        ...
        it('should update progress', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO milestones (id, project_id, title) VALUES (?, ?, ?)', [milestoneId, 'proj-1', 'Launch'], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE milestones SET progress = ? WHERE id = ?', [75, milestoneId], (err) => err ? reject(err) : resolve());
            });
        ...
        it('should complete milestone', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO milestones (id, project_id, title) VALUES (?, ?, ?)', [milestoneId, 'proj-1', 'Testing'], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE milestones SET status = ?, progress = 100 WHERE id = ?', ['completed', milestoneId], (err) => err ? reject(err) : resolve());
            });
