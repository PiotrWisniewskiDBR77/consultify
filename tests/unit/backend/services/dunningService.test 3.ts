...
describe('DunningService', () => {
    ...
    describe('Dunning Email Tracking', () => {
        ...
        it('should track email engagement', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO dunning_entries (id, organization_id, invoice_id, recovery_amount_cents) VALUES (?, ?, ?, ?)',
                        [dunningId, 'org-engage', 'inv-engage', 999]);
                    db.run('INSERT INTO dunning_emails (id, dunning_id, email_type, sent_at) VALUES (?, ?, ?, datetime("now"))',
                        [emailId, dunningId, 'final_warning'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
    ...
    describe('Dunning Analytics', () => {
        it('should calculate recovery rate', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO dunning_entries (id, organization_id, invoice_id, recovery_amount_cents, status) VALUES (?, ?, ?, ?, ?)',
                    ...
                    db.run('INSERT INTO dunning_entries (id, organization_id, invoice_id, recovery_amount_cents, status) VALUES (?, ?, ?, ?, ?)',
                        ['d-4', 'org-4', 'inv-4', 1500, 'pending'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
