...
describe('SurveyService', () => {
    ...
    describe('Survey CRUD', () => {
        ...
        it('should publish survey', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('INSERT INTO surveys (id, organization_id, title) VALUES (?, ?, ?)', [surveyId, 'org-1', 'Test Survey'], (err) => err ? reject(err) : resolve());
            });

            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE surveys SET status = ? WHERE id = ?', ['published', surveyId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Survey Responses', () => {
        it('should submit response', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO surveys (id, organization_id, title) VALUES (?, ?, ?)', [surveyId, 'org-1', 'Feedback']);
                    db.run('INSERT INTO survey_responses (id, survey_id, answers) VALUES (?, ?, ?)', [responseId, surveyId, JSON.stringify({ q1: 5 })], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
