...
describe('QueueService', () => {
  ...
  describe('Job CRUD', () => {
    ...
    it('should update job status', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO job_queue (id, type, payload) VALUES (?, ?, ?)',
          [jobId, 'report.generate', '{}'],
          (err) => (err ? reject(err) : resolve())
        );
      });
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'UPDATE job_queue SET status = ?, started_at = datetime("now") WHERE id = ?',
          ['processing', jobId],
          (err) => (err ? reject(err) : resolve())
        );
      });
  ...
  describe('Job Processing', () => {
    it('should get next pending job by priority', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run(
            'INSERT INTO job_queue (id, type, payload, priority, status) VALUES (?, ?, ?, ?, ?)',
            ['j1', 'low', '{}', 1, 'pending']
          );
          db.run(
            'INSERT INTO job_queue (id, type, payload, priority, status) VALUES (?, ?, ?, ?, ?)',
      ...
      const nextJob = await new Promise<any>((resolve, reject) => {
        db.get(
          'SELECT * FROM job_queue WHERE status = ? ORDER BY priority DESC LIMIT 1',
          ['pending'],
          (err, row) => {
            if (err) reject(err);
            else resolve(row);
    ...
    it('should mark job as failed', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'UPDATE job_queue SET status = ?, error = ?, attempts = attempts + 1 WHERE id = ?',
          ['failed', 'Connection timeout', jobId],
          (err) => (err ? reject(err) : resolve())
        );
      });
