...
describe('CommentService', () => {
    ...
    describe('Threaded Comments', () => {
        it('should create reply', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO comments (id, entity_type, entity_id, user_id, content) VALUES (?, ?, ?, ?, ?)',
                        ['parent-1', 'task', 'task-1', 'user-1', 'Parent comment']);
                    db.run('INSERT INTO comments (id, entity_type, entity_id, user_id, content, parent_id) VALUES (?, ?, ?, ?, ?, ?)',
                        ['reply-1', 'task', 'task-1', 'user-2', 'Reply to parent', 'parent-1'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
