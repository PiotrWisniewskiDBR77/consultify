...
describe('IntegrationService', () => {
  ...
  describe('Integration CRUD', () => {
    ...
    it('should update integration tokens', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO integrations (id, organization_id, provider) VALUES (?, ?, ?)',
          [integrationId, 'org-1', 'google'],
          (err) => (err ? reject(err) : resolve())
        );
      });
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'UPDATE integrations SET access_token = ?, status = ?, connected_at = datetime("now") WHERE id = ?',
          ['new-token', 'connected', integrationId],
          (err) => (err ? reject(err) : resolve())
        );
      });
  ...
  describe('Integration Queries', () => {
    it('should get organization integrations', async () => {
      await new Promise<void>((resolve, reject) => {
        db.serialize(() => {
          db.run('INSERT INTO integrations (id, organization_id, provider) VALUES (?, ?, ?)', [
            'i1',
            'org-A',
            'slack',
          ]);
    ...
    it('should disconnect integration', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO integrations (id, organization_id, provider, status, access_token) VALUES (?, ?, ?, ?, ?)',
          [integrationId, 'org-1', 'notion', 'connected', 'token'],
          (err) => (err ? reject(err) : resolve())
        );
      });
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'UPDATE integrations SET status = ?, access_token = NULL WHERE id = ?',
