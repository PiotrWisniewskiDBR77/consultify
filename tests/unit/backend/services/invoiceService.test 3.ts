...
describe('InvoiceService', () => {
    ...
    describe('Invoice CRUD', () => {
        ...
        it('should add invoice line items', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO invoice_items (id, invoice_id, description, quantity, unit_price_cents) VALUES (?, ?, ?, ?, ?)',
                        ['item-1', invoiceId, 'Pro Subscription', 1, 4999]);
                    db.run('INSERT INTO invoice_items (id, invoice_id, description, quantity, unit_price_cents) VALUES (?, ?, ?, ?, ?)',
                        ['item-2', invoiceId, 'Additional Seats', 5, 1000], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
    ...
    describe('Invoice Queries', () => {
        it('should filter invoices by organization', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO invoices (id, organization_id, invoice_number, amount_cents) VALUES (?, ?, ?, ?)',
                        ['inv-1', 'org-A', 'INV-A-001', 1000]);
                    db.run('INSERT INTO invoices (id, organization_id, invoice_number, amount_cents) VALUES (?, ?, ?, ?)',
                        ['inv-2', 'org-B', 'INV-B-001', 2000]);
                    db.run('INSERT INTO invoices (id, organization_id, invoice_number, amount_cents) VALUES (?, ?, ?, ?)',
                        ['inv-3', 'org-A', 'INV-A-002', 3000], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
        ...
        it('should calculate total revenue from paid invoices', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO invoices (id, organization_id, invoice_number, amount_cents, status) VALUES (?, ?, ?, ?, ?)',
                        ['inv-1', 'org-1', 'INV-1', 1000, 'paid']);
                    db.run('INSERT INTO invoices (id, organization_id, invoice_number, amount_cents, status) VALUES (?, ?, ?, ?, ?)',
                        ['inv-2', 'org-1', 'INV-2', 2500, 'paid']);
                    db.run('INSERT INTO invoices (id, organization_id, invoice_number, amount_cents, status) VALUES (?, ?, ?, ?, ?)',
                        ['inv-3', 'org-1', 'INV-3', 500, 'draft'], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
            ...
            const result = await new Promise<any>((resolve, reject) => {
                db.get('SELECT SUM(amount_cents) as total FROM invoices WHERE status = ?', ['paid'], (err, row) => {
                    if (err) reject(err);
                    else resolve(row);
                });
            });
