...
describe('TemplateService', () => {
        ...
        it('should create template', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run(
                    ...
                    [templateId, 'org-123', 'Invoice Template', 'billing', 'Dear {{name}}, Amount: {{amount}}', JSON.stringify(variables)],
    ...
    describe('Template Queries', () => {
        it('should find templates by category', async () => {
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO templates (id, name, category, content) VALUES (?, ?, ?, ?)', ['t1', 'Template 1', 'email', 'Content 1']);
                    db.run('INSERT INTO templates (id, name, category, content) VALUES (?, ?, ?, ?)', ['t2', 'Template 2', 'report', 'Content 2']);
                    db.run('INSERT INTO templates (id, name, category, content) VALUES (?, ?, ?, ?)', ['t3', 'Template 3', 'email', 'Content 3'], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
