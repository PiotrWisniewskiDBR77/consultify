...
describe('TagService', () => {
    ...
    describe('Tag CRUD', () => {
        ...
        it('should update tag', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.run('UPDATE tags SET name = ? WHERE id = ?', ['New Name', tagId], (err) => err ? reject(err) : resolve());
            });
    ...
    describe('Tag Assignment', () => {
        it('should assign tag to entity', async () => {
            ...
            await new Promise<void>((resolve, reject) => {
                db.serialize(() => {
                    db.run('INSERT INTO tags (id, organization_id, name) VALUES (?, ?, ?)', [tagId, 'org-1', 'Important']);
                    db.run('INSERT INTO entity_tags (id, entity_type, entity_id, tag_id) VALUES (?, ?, ?, ?)',
                        ['et-1', 'task', 'task-123', tagId], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
