...
describe('NotificationService', () => {
    ...
    afterAll(async () => {
        await new Promise(r => db.run(`DELETE FROM notifications WHERE user_id = ?`, [testUserId], () => r()));
    ...
    describe('Notification CRUD', () => {
        ...
        it('should list user notifications', async () => {
            ...
            const notifications = await new Promise((resolve) => {
                db.all(`SELECT * FROM notifications WHERE user_id = ?`, [testUserId], (_, rows) => resolve(rows));
            });
    ...
    describe('Unread Count', () => {
        it('should count unread notifications', async () => {
            ...
            await new Promise(r => db.run(
                ...
                [uuidv4(), testUserId, testOrgId, 'info', 'Unread 1', 'msg', 0], () => r()));
            await new Promise(r => db.run(
                `INSERT INTO notifications (id, user_id, organization_id, type, title, message, read) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testUserId, testOrgId, 'info', 'Unread 2', 'msg', 0], () => r()));
            await new Promise(r => db.run(
                `INSERT INTO notifications (id, user_id, organization_id, type, title, message, read) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testUserId, testOrgId, 'info', 'Read 1', 'msg', 1], () => r()));

            const result = await new Promise((resolve) => {
                db.get(`SELECT COUNT(*) as count FROM notifications WHERE user_id = ? AND read = 0`, [testUserId], (_, row) => resolve(row));
            });
    ...
    describe('Mark All Read', () => {
        it('should mark all notifications as read', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                await new Promise(r => db.run(
                    `INSERT INTO notifications (id, user_id, organization_id, type, title, message, read) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [uuidv4(), testUserId, testOrgId, 'info', `Notif ${i}`, 'msg', 0], () => r()));
            }

            await new Promise((resolve) => {
                db.run(`UPDATE notifications SET read = 1 WHERE user_id = ?`, [testUserId], () => resolve());
            });
