...
vi.mock('../../../../server/src/controllers/ProjectController.js', () => ({
  default: new Proxy(
    {},
    {
      get:
...
vi.mock('../../../../server/src/middleware/planLimits.middleware.js', () => ({
  checkPlanLimit: () => (_req: any, _res: any, next: () => void) => next(),
}));

vi.mock('../../../../server/src/validators/project.validators.js', () => ({
  CreateProjectSchema: {},
  UpdateProjectSchema: {},
  UpdateAIRoleSchema: {},
...
describe('pmo projects routes org guard', () => {
  it('returns 403 RBAC code when authenticated user has no organization', async () => {
    ...
    const { default: router } = await import('../../../../server/src/routes/pmo/projects.routes.js');
    const app = express();
