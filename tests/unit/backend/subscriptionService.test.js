...
describe('SubscriptionService', () => {
    ...
    describe('Subscription Status', () => {
        ...
        it('should identify subscription by plan', async () => {
            const org = await new Promise((resolve) => {
                db.get(`SELECT plan, billing_status FROM organizations WHERE id = ?`, [testOrgId], (_, row) => resolve(row));
            });
    ...
    describe('Plan Changes', () => {
        it('should upgrade plan', async () => {
            ...
            await new Promise((resolve) => {
                db.run(`UPDATE organizations SET plan = ? WHERE id = ?`, ['professional', upgradeOrgId], () => resolve());
            });

            const org = await new Promise((resolve) => {
                db.get(`SELECT plan FROM organizations WHERE id = ?`, [upgradeOrgId], (_, row) => resolve(row));
            });

            expect(org.plan).toBe('professional');

            // Cleanup
            await new Promise(r => db.run(`DELETE FROM organizations WHERE id = ?`, [upgradeOrgId], () => r()));
        });
        ...
        it('should downgrade plan', async () => {
            ...
            await new Promise((resolve) => {
                db.run(`UPDATE organizations SET plan = ? WHERE id = ?`, ['professional', downgradeOrgId], () => resolve());
            });

            const org = await new Promise((resolve) => {
                db.get(`SELECT plan FROM organizations WHERE id = ?`, [downgradeOrgId], (_, row) => resolve(row));
            });

            expect(org.plan).toBe('professional');

            // Cleanup
            await new Promise(r => db.run(`DELETE FROM organizations WHERE id = ?`, [downgradeOrgId], () => r()));
        });
    });
    ...
    describe('Subscription Cancellation', () => {
        it('should cancel subscription', async () => {
            ...
            const org = await new Promise((resolve) => {
                db.get(`SELECT billing_status, status FROM organizations WHERE id = ?`, [cancelOrgId], (_, row) => resolve(row));
            });
            ...
            // Cleanup
            await new Promise(r => db.run(`DELETE FROM organizations WHERE id = ?`, [cancelOrgId], () => r()));
        });
    });
    ...
    describe('Feature Access', () => {
        it('should check plan features', () => {
            const planFeatures = {
                ...
                enterprise: ['basic_ai', 'advanced_ai', 'projects_unlimited', 'team_unlimited', 'sso', 'audit']
            };
