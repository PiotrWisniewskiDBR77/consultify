...
const createInvitationRateLimiter = (options = {}) => {
    ...
    return {
        middleware: (req, res, next) => {
            ...
            const dailyData = invitationCounts.get(dailyKey) || { count: 0, resetAt: now + 24 * 60 * 60 * 1000 };
        ...
        setCount: (userId, hourly, daily) => {
            invitationCounts.set(`${userId}:hourly`, { count: hourly, resetAt: Date.now() + 60 * 60 * 1000 });
            invitationCounts.set(`${userId}:daily`, { count: daily, resetAt: Date.now() + 24 * 60 * 60 * 1000 });
        },
