...
const createWebhookService = () => {
    ...
    return {
        ...
        generateSignature: (payload, secret) => {
            ...
            return `sha256=${Buffer.from(JSON.stringify(payload) + secret).toString('base64').slice(0, 20)}`;
        },

        verifySignature: (payload, signature, secret) => {
            const expected = `sha256=${Buffer.from(JSON.stringify(payload) + secret).toString('base64').slice(0, 20)}`;
...
describe('WebhookService', () => {
    ...
    describe('Webhook Registration', () => {
        it('should register webhook', () => {
            const webhook = webhookService.register('https://example.com/webhook', ['task.created', 'task.updated']);
    ...
    describe('Webhook Delivery', () => {
        ...
        it('should reject inactive webhook', async () => {
            ...
            await expect(webhookService.send(webhook.id, 'event', {}))
                .rejects.toThrow('Webhook is inactive');
        });
    });
