...
describe('UserService', () => {
    ...
    describe('CRUD Operations', () => {
        ...
        it('should update user', async () => {
            ...
            await new Promise((resolve) => {
                db.run(
                    `INSERT INTO users (id, organization_id, email, password, role, status, first_name)
                     VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [userId, testOrgId, `update-${Date.now()}@test.com`, 'hash', 'USER', 'active', 'Original'],
                    () => resolve()
                );
            });
            createdUserIds.push(userId);
            ...
            await new Promise((resolve) => {
                db.run(`UPDATE users SET first_name = ? WHERE id = ?`, ['Updated', userId], () => resolve());
            });
    ...
    describe('User Lookup', () => {
        ...
        it('should return null for non-existent email', async () => {
            const user = await new Promise((resolve) => {
                db.get(`SELECT * FROM users WHERE email = ?`, ['nonexistent@test.com'], (_, row) => resolve(row));
            });
