...
const createADKARService = () => {
  ...
  return {
    ...
    generateReport: (assessmentId) => {
      ...
      const progress = (
        Object.values(assessment.scores).reduce((a, b) => a + b, 0) / phases.length
      ).toFixed(1);
      const weakest = Object.entries(assessment.scores).sort((a, b) => a[1] - b[1])[0];
...
describe('ADKARService', () => {
  ...
  describe('Phase Scoring', () => {
    ...
    it('should reject invalid phase', () => {
      ...
      expect(() => adkarService.updatePhaseScore(assessment.id, 'invalid', 50)).toThrow(
        'Invalid phase'
      );
    });
    ...
    it('should validate score range', () => {
      ...
      expect(() => adkarService.updatePhaseScore(assessment.id, 'awareness', 150)).toThrow(
        'Score must be 0-100'
      );
    });
  });
