...
const createWorkspaceService = () => {
  ...
  return {
    ...
    listByOrganization: async (organizationId) => {
      return Array.from(workspaces.values()).filter((w) => w.organizationId === organizationId);
    },
...
describe('WorkspaceService', () => {
  ...
  describe('Workspace CRUD', () => {
    ...
    it('should require name and organization', async () => {
      await expect(workspaceService.create({})).rejects.toThrow(
        'Name and organization ID required'
      );
    });
  ...
  describe('Member Management', () => {
    ...
    it('should prevent duplicate membership', async () => {
      ...
      await expect(workspaceService.addMember(workspace.id, 'user-2')).rejects.toThrow(
    ...
    it('should not allow removing owner', async () => {
      await expect(workspaceService.removeMember(workspace.id, 'owner-1')).rejects.toThrow(
        'Cannot remove owner'
      );
    });
