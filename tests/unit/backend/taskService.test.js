...
describe('TaskService', () => {
    ...
    beforeEach(async () => {
        ...
        await new Promise(r => db.run(`DELETE FROM tasks WHERE project_id = ?`, [testProjectId], () => r()));
        createdTaskIds = [];
    });
    ...
    describe('Task CRUD Operations', () => {
            ...
            await new Promise((resolve, reject) => {
                db.run(
                    `INSERT INTO tasks (id, project_id, organization_id, title, description, status, priority, assignee_id, created_at)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
                    [taskId, testProjectId, testOrgId, 'Test Task', 'Test Description', 'todo', 'medium', testUserId],
                    (err) => err ? reject(err) : resolve()
                );
            });
        ...
        it('should list tasks for project', async () => {
            ...
            const tasks = await new Promise((resolve) => {
                db.all(`SELECT * FROM tasks WHERE project_id = ?`, [testProjectId], (_, rows) => resolve(rows));
            });
        ...
        it('should update task', async () => {
            ...
            await new Promise((resolve) => {
                db.run(`UPDATE tasks SET title = ?, status = ? WHERE id = ?`, ['Updated Title', 'in_progress', taskId], () => resolve());
            });
    ...
    describe('Task Filtering', () => {
        it('should filter tasks by status', async () => {
            ...
            const todoTasks = await new Promise((resolve) => {
                db.all(`SELECT * FROM tasks WHERE project_id = ? AND status = ?`, [testProjectId, 'todo'], (_, rows) => resolve(rows));
            });
        ...
        it('should filter tasks by priority', async () => {
            ...
            const urgentTasks = await new Promise((resolve) => {
                db.all(`SELECT * FROM tasks WHERE project_id = ? AND priority = ?`, [testProjectId, 'urgent'], (_, rows) => resolve(rows));
            });
        ...
        it('should filter tasks by assignee', async () => {
            ...
            const assignedTasks = await new Promise((resolve) => {
                db.all(`SELECT * FROM tasks WHERE assignee_id = ?`, [testUserId], (_, rows) => resolve(rows));
            });
    ...
    describe('Task Statistics', () => {
        it('should count tasks by status', async () => {
            // Create tasks with different statuses
            await new Promise(r => db.run(`INSERT INTO tasks (id, project_id, organization_id, title, status, priority) VALUES (?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testProjectId, testOrgId, 'T1', 'todo', 'medium'], () => r()));
            await new Promise(r => db.run(`INSERT INTO tasks (id, project_id, organization_id, title, status, priority) VALUES (?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testProjectId, testOrgId, 'T2', 'todo', 'medium'], () => r()));
            await new Promise(r => db.run(`INSERT INTO tasks (id, project_id, organization_id, title, status, priority) VALUES (?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testProjectId, testOrgId, 'T3', 'done', 'medium'], () => r()));
        ...
        it('should calculate completion rate', async () => {
            await new Promise(r => db.run(`INSERT INTO tasks (id, project_id, organization_id, title, status, priority) VALUES (?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testProjectId, testOrgId, 'Done1', 'done', 'medium'], () => r()));
            await new Promise(r => db.run(`INSERT INTO tasks (id, project_id, organization_id, title, status, priority) VALUES (?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testProjectId, testOrgId, 'Done2', 'done', 'medium'], () => r()));
            await new Promise(r => db.run(`INSERT INTO tasks (id, project_id, organization_id, title, status, priority) VALUES (?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testProjectId, testOrgId, 'Todo1', 'todo', 'medium'], () => r()));
            await new Promise(r => db.run(`INSERT INTO tasks (id, project_id, organization_id, title, status, priority) VALUES (?, ?, ?, ?, ?, ?)`,
                [uuidv4(), testProjectId, testOrgId, 'Todo2', 'todo', 'medium'], () => r()));

            const total = await new Promise((resolve) => {
                db.get(`SELECT COUNT(*) as count FROM tasks WHERE project_id = ?`, [testProjectId], (_, row) => resolve(row));
            });

            const done = await new Promise((resolve) => {
                db.get(`SELECT COUNT(*) as count FROM tasks WHERE project_id = ? AND status = 'done'`, [testProjectId], (_, row) => resolve(row));
            });
