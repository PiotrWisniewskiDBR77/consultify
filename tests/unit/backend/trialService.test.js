...
describe('TrialService', () => {
    ...
    describe('Trial Status', () => {
        ...
        it('should identify active trial', async () => {
            const org = await new Promise((resolve) => {
                db.get(`SELECT * FROM organizations WHERE id = ? AND billing_status = 'TRIAL'`, [testOrgId], (_, row) => resolve(row));
            });
    ...
    describe('Trial Expiration', () => {
        it('should identify expired trial', async () => {
            ...
            const org = await new Promise((resolve) => {
                db.get(`SELECT * FROM organizations WHERE id = ?`, [expiredOrgId], (_, row) => resolve(row));
            });
            ...
            // Cleanup
            await new Promise(r => db.run(`DELETE FROM organizations WHERE id = ?`, [expiredOrgId], () => r()));
        });
    });
    ...
    describe('Trial Conversion', () => {
        it('should convert trial to paid', async () => {
            ...
            const org = await new Promise((resolve) => {
                db.get(`SELECT * FROM organizations WHERE id = ?`, [conversionOrgId], (_, row) => resolve(row));
            });
            ...
            // Cleanup
            await new Promise(r => db.run(`DELETE FROM organizations WHERE id = ?`, [conversionOrgId], () => r()));
        });
    });
    ...
    describe('Trial Limits', () => {
        it('should track trial usage', async () => {
            const org = await new Promise((resolve) => {
                db.get(`SELECT token_balance FROM organizations WHERE id = ?`, [testOrgId], (_, row) => resolve(row));
            });
