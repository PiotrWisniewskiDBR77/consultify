...
const createRiskService = () => {
  ...
  return {
    ...
    getRiskMatrix: async (projectId) => {
      const projectRisks = Array.from(risks.values()).filter((r) => r.projectId === projectId);
...
describe('RiskService', () => {
  ...
  describe('Risk Creation', () => {
    ...
    it('should require project and title', async () => {
      await expect(riskService.create({})).rejects.toThrow('Project ID and title required');
    });
  ...
  describe('Risk Status', () => {
    ...
    it('should reject invalid status', async () => {
      ...
      await expect(riskService.updateStatus(risk.id, 'invalid')).rejects.toThrow('Invalid status');
    });
  });
