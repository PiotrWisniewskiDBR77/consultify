...
describe('SubscriptionAnalyticsService', () => {
    ...
    describe('getLTV()', () => {
        it('should calculate LTV metrics', async () => {
            ...
            mockDb.get.mockImplementation((query, params, callback) => {
                if (query.includes('subscription_plans') || query.includes('organization_billing')) {
                    callback(null, { avg_mrr: 100, arpa: 100, active_subscriptions: 50 });
                } else {
                    callback(null, { churn_rate: 0.05, monthly_churn: 0.05, churned_customers: 5, starting_customers: 100 });
                }
            });
        ...
        it('should handle zero churn rate', async () => {
            mockDb.get.mockImplementation((query, params, callback) => {
                if (query.includes('subscription_plans') || query.includes('organization_billing')) {
                    callback(null, { avg_mrr: 100, arpa: 100 });
                } else {
                    callback(null, { churn_rate: 0, monthly_churn: 0 });
                }
            });
