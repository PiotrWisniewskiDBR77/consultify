...
const createRoleService = () => {
  ...
  const defaultRoles = [
    { id: 'admin', name: 'Administrator', permissions: ['*'], isSystem: true },
    {
      id: 'manager',
      name: 'Manager',
      permissions: ['read', 'write', 'delete:own', 'manage:team'],
      isSystem: true,
    },
  ...
  return {
    ...
    list: async (organizationId) => {
      return Array.from(roles.values()).filter(
        (r) => r.isSystem || r.organizationId === organizationId
      );
    },
    ...
    assignRole: async (userId, roleId, context = {}) => {
      ...
      const key = context.projectId ? `${userId}:${context.projectId}` : userId;
...
describe('RoleService', () => {
  ...
  describe('Custom Roles', () => {
    ...
    it('should prevent duplicate role IDs', async () => {
      ...
      await expect(
        roleService.create({ id: 'custom', name: 'Another', organizationId: 'org-1' })
    ...
    it('should not modify system roles', async () => {
      await expect(roleService.update('admin', { name: 'Modified Admin' })).rejects.toThrow(
    ...
    it('should not delete system roles', async () => {
      await expect(roleService.delete('admin')).rejects.toThrow('Cannot delete system role');
    });
  });
