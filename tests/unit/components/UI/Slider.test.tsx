  ...
  return (
    <div data-testid="slider" data-disabled={disabled}>
      <input
        ...
        aria-valuenow={value}
      />
      <div data-testid="slider-track" style={{ width: `${percentage}%` }} />
      {showValue && <span data-testid="slider-value">{value}</span>}
      {marks.length > 0 && (
        <div data-testid="slider-marks">
          {marks.map((mark, index) => (
            <span key={index} data-testid={`slider-mark-${mark.value}`}>
              {mark.label}
            </span>
          ))}
        </div>
