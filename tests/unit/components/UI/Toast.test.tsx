...
const MockToast: React.FC<{
  ...
  return (
    ...
    >
      ...
      {action && (
        <button data-testid="toast-action" onClick={action.onClick}>
          {action.label}
        </button>
      )}
      <button data-testid="toast-close" onClick={onClose} aria-label="Close notification">
        ×
      </button>
    </div>
  );
