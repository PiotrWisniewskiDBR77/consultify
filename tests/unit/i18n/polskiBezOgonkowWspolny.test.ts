...
import { czyPolskiBezOgonkow, znajdzPolskiJsx } from './polskiBezOgonkowWspolny';
...
describe('W77 / wspólny detektor polskiego bez ogonków', () => {
  ...
  it('nie czyta fragmentu operatora ternary JSX jako tekstu dla użytkownika', () => {
    expect(
      znajdzPolskiJsx([{ nazwa: 'DeckBuilder.tsx', tekst: '> ) : autosaveError ? ( <' }])
    ).toEqual([]);
  });
});
