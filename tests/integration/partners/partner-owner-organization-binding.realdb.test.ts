...
async function assertOwnedDisposableDatabase(): Promise<void> {
  ...
  if (!prefix.startsWith("consultify_prt_owner_bind_test_")) {
    throw new Error(
      "PRT_OWNER_BINDING_DB_PREFIX must use consultify_prt_owner_bind_test_",
    );
  }
  const live = (
    await pool.query<{ database: string }>("SELECT current_database() database")
  ).rows[0].database;
  const urlDatabase = new URL(DATABASE_URL!).pathname.replace(/^\/+/, "");
  if (live !== urlDatabase || !live.startsWith(prefix)) {
    throw new Error(
      `Refusing cleanup outside owned disposable database prefix ${prefix}`,
    );
  }
}
...
function signedManifest(params: {
  ...
  const manifest: OwnerBindingManifest = {
    ...
    expiresAt: new Date(
      params.expired ? now - 1 : now + 3_600_000,
    ).toISOString(),
...
describeReal.sequential("Partner historical owner binding", () => {
  ...
  afterAll(async () => {
    ...
    try {
      ...
      if (
        (
          await pool.query(
            `SELECT to_regclass('public.partner_owner_binding_receipts') present`,
          )
        ).rows[0].present
      ) {
        await pool.query(
          `DELETE FROM partner_owner_binding_receipts WHERE actor_user_id=$1`,
          [actorId],
        );
      }
      await pool.query(
        `DELETE FROM partner_organizations WHERE id=ANY($1::uuid[])`,
  ...
  it("migration repeats and refuses a hostile pre-existing receipt table", async () => {
    ...
    try {
      await pool.query(
        `CREATE TABLE partner_owner_binding_receipts(run_id integer)`,
      );
    ...
    try {
      ...
      await pool.query(
        `DROP FUNCTION IF EXISTS protect_partner_owner_binding_receipts()`,
      );
      await pool.query(
  ...
  it("rejects tampering, expiry, duplicate targets and an unauthorized actor before writes", async () => {
    ...
    const duplicate = signedManifest({
      ...
      mappings: [
        ...mapping,
        { partnerOrganizationId: partnerB, ownerOrganizationId: orgA },
      ],
    });
    await expect(
  ...
  it("rolls back every row and the receipt on an injected mid-batch failure", async () => {
    ...
    expect([await owner(partnerA), await owner(partnerB)]).toEqual([
      null,
      null,
    ]);
    expect(
  ...
  it("rejects collision and leaves an unrelated foreign tenant binding unchanged", async () => {
    const occupied = signedManifest({
      ...
      mappings: [
        { partnerOrganizationId: partnerC, ownerOrganizationId: foreignOrg },
      ],
    });
    await expect(
    ...
    const colliding = signedManifest({
      runId: `apply-${suffix}`,
      mappings: [
        { partnerOrganizationId: partnerC, ownerOrganizationId: orgA },
      ],
    });
    await expect(
    ...
    await pool.query(
      `UPDATE partner_organizations SET owner_organization_id=NULL WHERE id=$1`,
      [partnerB],
    );
    try {
      await expect(
      ...
      await pool.query(
        `UPDATE partner_organizations SET owner_organization_id=$2 WHERE id=$1`,
        [partnerB, orgB],
      );
    }
  });
  ...
  it("serializes concurrent identical apply attempts into one effect and one receipt", async () => {
    ...
    await pool.query(
      `UPDATE partner_organizations SET status='inactive' WHERE id=$1`,
      [partnerA],
    );
  ...
  it("rolls back only the exact signed apply receipt to NULL and records an immutable receipt", async () => {
    ...
    await pool.query(
      `UPDATE organizations SET status='inactive',is_active=0 WHERE id=$1`,
      [orgB],
    );
    const result = await executeManifest({
      pool,
    ...
    await expect(
      pool.query(
        `UPDATE partner_owner_binding_receipts SET mapping_count=99 WHERE run_id=$1`,
        [rollback.runId],
      ),
    ).rejects.toThrow(/append-only/);
    await expect(
      pool.query(`DELETE FROM partner_owner_binding_receipts WHERE run_id=$1`, [
        rollback.runId,
      ]),
    ).rejects.toThrow(/append-only/);

    const out = fs.mkdtempSync(
      path.join(os.tmpdir(), "prt-owner-binding-report-"),
    );
    ...
    const markdown = fs.readFileSync(
      path.join(out, "ZERO_WRITER_PARITY_REPORT.md"),
      "utf8",
    );
    expect(markdown).toContain(
      `| ${rollback.runId} | ROLLBACK | ${applyRunId} |`,
    );
