...
describe('Audit Endpoints Integration', () => {
    ...
    beforeAll(() => {
        ...
        // Seed some logs
        auditLogs.push(
            { id: 'log-1', action: 'user.login', userId: 'u1', ip: '192.168.1.1', timestamp: new Date().toISOString() },
            { id: 'log-2', action: 'project.create', userId: 'u1', resource: 'proj-1', timestamp: new Date().toISOString() },
            { id: 'log-3', action: 'user.logout', userId: 'u2', ip: '192.168.1.2', timestamp: new Date().toISOString() }
        );
