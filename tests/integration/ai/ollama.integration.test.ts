...
describe.skipIf(!OLLAMA_ENABLED)('Ollama Local LLM Integration', () => {
  describe('Server Health', () => {
    it(
      'should have Ollama server running',
      async () => {
    ...
    it(
      'should have gemma model available',
      async () => {
        const response = await fetch(`${OLLAMA_BASE_URL}/api/tags`);
        const data = await response.json();

        const hasGemma = data.models?.some(
          (m: any) => m.name.includes('gemma3') || m.name.includes('gemma')
        );
  ...
  describe('API Generate Endpoint', () => {
    it(
      'should generate a response (non-streaming)',
      async () => {
  ...
  describe('OpenAI Compatible API', () => {
    it(
      'should respond to chat completions (OpenAI format)',
      async () => {
