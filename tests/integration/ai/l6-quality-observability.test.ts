...
describe('L6.17: AI Quality & Observability', () => {
    describe('AICostMonitoringService', () => {
        it('should import and instantiate cost monitoring service', async () => {
            const { AICostMonitoringService, aiCostMonitoring } = await import(
                '../../../server/src/services/ai/cost-monitoring.service'
            );
        ...
        it('should record token usage', async () => {
            const { AICostMonitoringService } = await import(
                '../../../server/src/services/ai/cost-monitoring.service'
            );
            const svc = new AICostMonitoringService();

            const record = svc.recordUsage(
                'user-001',
                'org-001',
                'BUDGET',
                'openai',
    ...
    describe('Alerting Specification', () => {
        it('should define 5 alert types', () => {
            const alertTypes = [
                'error_spike', 'latency_degradation', 'quota_warning',
                'provider_down', 'quality_drop',
            ];
