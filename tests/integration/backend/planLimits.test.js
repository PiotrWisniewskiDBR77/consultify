...
describe('Plan Limits Integration', () => {
    ...
    beforeAll(async () => {
        ...
        // Generate Token
        authToken = jwt.sign({ id: userId, organizationId: orgId, role: 'ADMIN' }, process.env.JWT_SECRET);
    ...
    it('should prevent creating more projects than allowed by Free plan', async () => {
        ...
        if (res1.status !== 201) {
            console.error('[Test Error] Project 1 creation failed. Status:', res1.status, 'Body:', res1.body);
        }
        expect(res1.status).toBe(201); // Controller returns 201 for creation

        // DEBUG: Check if project exists in DB
        const checkCount = await DbPromise.get(db, 'SELECT COUNT(*) as count FROM projects WHERE organization_id = ?', [orgId]);
        ...
        if (res2.status !== 403) {
            console.error('[Test Error] Project 2 should have failed with 403. Status:', res2.status, 'Body:', res2.body);
        }
    ...
    it('should allow creating more projects after upgrade', async () => {
        ...
        if (res3.status !== 201) {
            console.error('[Test Error] Project 2 Pro creation failed. Status:', res3.status, 'Body:', res3.body);
        }
