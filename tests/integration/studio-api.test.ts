...
describe('Studio API Integration', () => {
  ...
  beforeAll(async () => {
    ...
    const flows = new Map<string, any>([
      [
        'flow-1',
        { id: 'flow-1', name: 'Test Flow', nodes: [], connections: [], organizationId: 'org-1' },
      ],
    ]);
    ...
    app.get('/api/studio/flows', requireAuth, (req: any, res: any) => {
      const orgFlows = Array.from(flows.values()).filter(
        (f) => f.organizationId === req.user.organizationId
      );
      res.json(orgFlows);
    });
