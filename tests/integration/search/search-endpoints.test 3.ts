...
describe('Search Endpoints Integration', () => {
    ...
    beforeAll(() => {
        ...
        const mockData = {
            ...
            users: [
                { id: 'u1', name: 'John Doe', email: 'john@example.com', type: 'user' }
            ]
        };
        ...
        app.get('/api/search', authMiddleware, (req, res) => {
            ...
            if (!type || type === 'user') {
                results.push(...mockData.users.filter(u => u.name.toLowerCase().includes(query) || u.email.toLowerCase().includes(query)));
            }
