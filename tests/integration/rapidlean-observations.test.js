...
describe('RapidLean Observations Integration', () => {
    ...
    beforeAll(async () => {
        ...
        const assessments = new Map([
            ['assessment-1', { id: 'assessment-1', overall_score: 85, observation_count: 2, organizationId: 'org-1' }]
        ]);
        const observations = new Map([
            ['obs-1', { id: 'obs-1', assessmentId: 'assessment-1', templateId: 'value_stream_template', score: 80 }],
            ['obs-2', { id: 'obs-2', assessmentId: 'assessment-1', templateId: 'waste_template', score: 70 }]
        ]);
        const reports = new Map();

        const templates = [
            { id: 'value_stream_template', dimension: 'Processes', drdAxis: 'processes', checklist: ['Step 1', 'Step 2'] },
            { id: 'waste_template', dimension: 'Waste', drdAxis: 'processes', checklist: ['Check A', 'Check B'] },
        ...
        app.get('/api/rapidlean/observations/:assessmentId', requireAuth, (req, res) => {
            ...
            const assessmentObs = Array.from(observations.values())
                .filter(o => o.assessmentId === req.params.assessmentId);
        ...
        app.get('/api/rapidlean/:id/drd-mapping', requireAuth, (req, res) => {
            ...
            const assessmentObs = Array.from(observations.values())
                .filter(o => o.assessmentId === req.params.id);

            // Calculate DRD mapping
            const drdMapping = { processes: 0, culture: 0 };
            let processCount = 0, cultureCount = 0;
