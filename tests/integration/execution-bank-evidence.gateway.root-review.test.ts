...
describe('EXE-005-006 — change + progress spine golden flow against a real Postgres database (no mocks)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!harness) throw new Error('ROOT_GATEWAY_REAL_DB_PREREQUISITES_MISSING');
  }, 30_000);
  ...
  itDB(
    ...
    async (h) => {
      ...
      expect(baselineGet.status, JSON.stringify(baselineGet.body)).toBe(200);
      ...
      const progressRes = await request(app)
        ...
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({ progress: 42 });
      expect(progressRes.status, JSON.stringify(progressRes.body)).toBe(200);
      ...
      expect(replanRes.status, JSON.stringify(replanRes.body)).toBe(200);
      ...
      expect(changeRes.status, JSON.stringify(changeRes.body)).toBe(201);
      ...
      const reopenGet = await request(reopenApp)
        .get(`/api/initiatives/${h.initiativeAId}`)
        .set('Authorization', `Bearer ${ownerToken}`);
      expect(reopenGet.status, JSON.stringify(reopenGet.body)).toBe(200);
      ...
      const historyGet = await request(reopenApp)
        ...
        .set('Authorization', `Bearer ${ownerToken}`);
      expect(historyGet.status, JSON.stringify(historyGet.body)).toBe(200);
      ...
      const evidenceGet = await request(buildApp())
        ...
        .set('Authorization', `Bearer ${ownerToken}`);
      expect(evidenceGet.status, JSON.stringify(evidenceGet.body)).toBe(200);
  ...
  itDB(
    ...
    async (h) => {
      ...
      expect(legitRes.status, JSON.stringify(legitRes.body)).toBe(200);
      ...
      expect(attackRes.status, JSON.stringify(attackRes.body)).toBe(404);
  ...
  itDB(
    ...
    async (h) => {
      ...
      const finalGet = await request(reopenApp)
        .get(`/api/initiatives/${h.initiativeAId}`)
        .set('Authorization', `Bearer ${ownerToken}`);
      expect(finalGet.status, JSON.stringify(finalGet.body)).toBe(200);
