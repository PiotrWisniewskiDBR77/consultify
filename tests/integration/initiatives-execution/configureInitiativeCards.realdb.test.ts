...
describeRealDb('Configure Initiative Cards PostgreSQL vertical', () => {
  ...
  beforeEach(async () => {
    await pool.query(
      'TRUNCATE ie_initiative_card_versions, ie_initiative_card_selection, ie_command_receipts, ie_audit_events, ie_outbox_delivery_receipts, ie_outbox_events, ie_aggregate_state RESTART IDENTITY'
    );
  ...
  afterAll(async () => {
    await cleanupInitiativesExecutionOrg(pool, 'org-config');
    await pool.end();
  });
});
