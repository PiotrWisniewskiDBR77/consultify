...
describe('Transaction Integrity', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      await db.run(
        `INSERT INTO users (id, email, organization_id, role) VALUES (?, 'trans@test.com', ?, 'USER')`,
        [userId, orgId]
      );
    } catch (e) {
  ...
  describe('Atomic Operations', () => {
    ...
    it('should rollback on foreign key violation', async () => {
      ...
      try {
        await db.run(
          `INSERT INTO tasks (id, title, project_id, organization_id) VALUES (?, 'Task', ?, ?)`,
          ['task-rollback', invalidProjectId, orgId]
        );
  ...
  describe('Rollback on Failure', () => {
    it('should rollback when second operation fails', async () => {
      ...
      try {
        ...
        await db.run(
          `INSERT INTO tasks (id, title, project_id, organization_id) VALUES (?, ?, ?, ?)`,
          [null, 'Task', projectId, orgId]
        );
        expect(true).toBe(false);
      } catch (error) {
        expect(error).toBeDefined();
    ...
    it('should maintain consistency across related tables', async () => {
      ...
      try {
        ...
        await db.run(
          `INSERT INTO tasks (id, title, project_id, organization_id) VALUES (?, 'Task 1', ?, ?)`,
          [taskId1, projectId, orgId]
        );
        await db.run(
          `INSERT INTO tasks (id, title, project_id, organization_id) VALUES (?, 'Task 2', ?, ?)`,
          [taskId2, projectId, orgId]
        );
  ...
  describe('Concurrent Transactions', () => {
    it('should handle concurrent project creation', async () => {
      ...
      const promises = projectIds.map((id) =>
        db.run(`INSERT INTO projects (id, name, organization_id) VALUES (?, 'Project', ?)`, [
          id,
          orgId,
        ])
      );
