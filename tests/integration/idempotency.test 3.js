...
describe('Idempotency', () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            await db.run(`INSERT INTO users (id, email, organization_id, role) VALUES (?, 'idemp@test.com', ?, 'USER')`, [userId, orgId]);
        } catch (e) {
    ...
    describe('Idempotent Updates', () => {
        it('should handle duplicate update requests', async () => {
            ...
            try {
                await db.run(`INSERT INTO projects (id, name, organization_id) VALUES (?, 'Original', ?)`, [projectId, orgId]);
