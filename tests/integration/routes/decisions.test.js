...
import { getDatabase, getDatabaseInstance, resetConnection } from '../../../server/src/database/Database.js';
...
describe('Decisions Routes', () => {
    ...
    describe('GET /api/decisions', () => {
        it('returns list of decisions', async () => {
            ...
            const response = await request(testApp).get('/api/decisions').expect('Content-Type', /json/).expect(200);
        ...
        it('filters by projectId', async () => {
            const response = await request(testApp).get(`/api/decisions?projectId=${projectId}`).expect(200);
