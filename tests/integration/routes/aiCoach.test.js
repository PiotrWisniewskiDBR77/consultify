...
describe('AI Coach API', () => {
    ...
    const testOrgId = `ai-coach-org-${testId}`;
    const testUserId = `ai-coach-user-${testId}`;
    const testEmail = `ai-coach-${testId}@test.com`;
        ...
        await new Promise((resolve) => {
            db.serialize(() => {
                db.run('INSERT INTO organizations (id, name, plan, status) VALUES (?, ?, ?, ?)',
                    [testOrgId, 'AI Coach Org', 'enterprise', 'active']);
                db.run('INSERT INTO users (id, organization_id, email, password, first_name, role) VALUES (?, ?, ?, ?, ?, ?)',
                    [testUserId, testOrgId, testEmail, hash, 'TestUser', 'ADMIN'], resolve);
            });
        });
