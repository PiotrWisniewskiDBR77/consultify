...
describe('SuperAdmin Customers API', () => {
        ...
        await new Promise((resolve) => {
            db.serialize(() => {
                db.run('INSERT INTO organizations (id, name, plan, status) VALUES (?, ?, ?, ?)',
                    [superadminOrgId, 'SuperAdmin Org', 'enterprise', 'active']);
                db.run('INSERT INTO organizations (id, name, plan, status) VALUES (?, ?, ?, ?)',
                    [regularOrgId, 'Regular Org', 'professional', 'active']);
                ...
                db.run('INSERT INTO users (id, organization_id, email, password, first_name, role) VALUES (?, ?, ?, ?, ?, ?)',
                    [regularUserId, regularOrgId, regularEmail, hash, 'Regular', 'ADMIN'], resolve);
            });
        });

        // Login both users
        const saLogin = await request(app).post('/api/auth/login')
            .send({ email: superadminEmail, password: 'test123' });
        if (saLogin.body.token) superadminToken = saLogin.body.token;

        const regLogin = await request(app).post('/api/auth/login')
    ...
    afterAll(async () => {
        await new Promise((resolve) => {
            db.serialize(() => {
                ...
                db.run('DELETE FROM organizations WHERE id IN (?, ?)', [superadminOrgId, regularOrgId], resolve);
            });
        });
    });
    ...
    describe('GET /api/superadmin/customers', () => {
        ...
        it('should return customers list for superadmin', async () => {
            ...
            if (res.status === 200) {
                expect(Array.isArray(res.body) || res.body.customers || res.body.organizations).toBeTruthy();
            }
        });
    });
