...
describe('feedback item-by-id fail-closed contract', () => {
  const app = express();
  ...
  it('returns coded 400 for invalid feedback id', async () => {
    const res = await request(app)
      .get('/api/feedback/not-a-uuid')
      .set('X-Correlation-ID', 'pack09s2-feedback-invalid-id-1');

    expect(res.status).toBe(400);
    expect(res.body.status).toBe('fail');
    expect(res.body.error.code).toBe('FEEDBACK_ITEM_ID_INVALID');
    expect(res.body.error.message).toBe('Feedback id must be a valid UUID.');
    expect(res.body.correlationId).toBe('pack09s2-feedback-invalid-id-1');
  });

  it('returns coded 404 for missing feedback item', async () => {
    dbGetMock.mockResolvedValueOnce(null);

    const res = await request(app)
      .get(`/api/feedback/${VALID_ID}`)
      .set('X-Correlation-ID', 'pack09s2-feedback-not-found-1');

    expect(res.status).toBe(404);
    expect(res.body.status).toBe('fail');
    expect(res.body.error.code).toBe('FEEDBACK_ITEM_NOT_FOUND');
    expect(res.body.error.message).toBe('Feedback item was not found.');
    expect(res.body.correlationId).toBe('pack09s2-feedback-not-found-1');
  });

  it('returns non-leaking coded 500 when item read fails', async () => {
    ...
    try {
      ...
      const res = await request(app)
        .get(`/api/feedback/${VALID_ID}`)
        .set('X-Correlation-ID', 'pack09s2-feedback-read-fail-1');

      expect(res.status).toBe(500);
      expect(res.body.status).toBe('error');
      expect(res.body.error.code).toBe('FEEDBACK_ITEM_READ_FAILED');
      expect(res.body.error.message).toBe('Failed to read feedback item.');
      expect(Number.isNaN(Date.parse(res.body.error.timestamp))).toBe(false);
      expect(res.body.correlationId).toBe('pack09s2-feedback-read-fail-1');
