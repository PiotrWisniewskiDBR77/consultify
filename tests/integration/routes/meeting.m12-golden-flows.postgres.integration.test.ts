...
describe('M12 Meeting — golden flows (real Postgres)', () => {
  ...
  beforeAll(async () => {
    ...
    const { default: meetingRoutes } = await import(
      '../../../server/src/routes/meeting.routes.js'
    );
    app = express();
  ...
  describe('A. open', () => {
    ...
    it('GF-05 projectId query filter narrows the list', async () => {
      ...
      const res = await request(app)
        .get(`/api/meeting?projectId=${PROJECT_A}`)
        .set(member());
      expect(res.status).toBe(200);
  ...
  describe('B. create', () => {
    ...
    it('GF-09 create without title is rejected 400 and writes nothing', async () => {
      const before = (await pool.query(`SELECT count(*) c FROM meetings WHERE organization_id=$1`, [ORG_A])).rows[0].c;
      const res = await createMeeting({ startAt: '2026-09-12T09:00:00.000Z' });
      expect(res.status).toBe(400);
      const after = (await pool.query(`SELECT count(*) c FROM meetings WHERE organization_id=$1`, [ORG_A])).rows[0].c;
  ...
  describe('G. status & roles', () => {
    let id: string;
    beforeAll(async () => {
      const res = await createMeeting({ title: 'Status flow', startAt: '2026-10-10T09:00:00.000Z' });
      id = res.body.meeting.id;
    });
  ...
  describe('H. tenant isolation', () => {
    ...
    beforeAll(async () => {
      const res = await createMeeting({ title: 'Org A private', startAt: '2026-10-15T09:00:00.000Z' });
    ...
    it('GF-43 an org B admin cannot delete an org A meeting', async () => {
      const res = await request(app).delete(`/api/meeting/${orgAMeetingId}`).set(admin(ORG_B, USER_B));
      expect(res.status).toBe(404);
  ...
  describe('I. error handling', () => {
    it('GF-46 every mutating route rejects an unauthenticated caller', async () => {
      ...
      const calls = [
        ...
        request(app).patch('/api/meeting/whatever/status').set(unauth).send({ status: 'completed' }),
        ...
        request(app).post('/api/meeting/whatever/generate-notes').set(unauth).send({ transcript: 'x' }),
      ];
