...
describe('Notification Endpoints Integration', () => {
    ...
    beforeAll(() => {
        ...
        app.post('/api/notifications/read-all', authMiddleware, (req, res) => {
            notifications.filter(n => n.userId === req.user.id).forEach(n => {
                n.read = true;
        ...
        notifications.push(
            { id: 'n1', userId: '1', title: 'New message', read: false, createdAt: new Date().toISOString() },
            { id: 'n2', userId: '1', title: 'Task assigned', read: true, createdAt: new Date().toISOString() }
        );
    });
