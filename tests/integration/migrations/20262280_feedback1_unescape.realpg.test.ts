...
const describePg = rootUrl ? describe : describe.skip;
...
function transactionalMigration(expectedColumns?: number): string {
  const override = expectedColumns === undefined
    ? ''
    : `SET LOCAL consultify.feedback_20262280_expected_columns = '${expectedColumns}';\n`;
...
describePg('20262280 FEEDBACK-1 entity repair on real PostgreSQL', () => {
  let root: Client;
  ...
  it('repairs six tokens in plain/composite-PK and JSON values, replays with zero delta, and rolls back exactly', async () => {
    await withDatabase(databaseNames.fixture, async (client) => {
      ...
      await client.query(transactionalMigration(5));
  ...
  it('rollback conflict preflight preserves a later user edit and restores nothing', async () => {
    await withDatabase(databaseNames.conflict, async (client) => {
      ...
      await client.query(transactionalMigration(2));
  ...
  it('fails closed when one neutral-name text column mixes JSON and plain affected values', async () => {
    await withDatabase(databaseNames.mixed, async (client) => {
      await client.query(`
        CREATE TABLE mixed_content (id text PRIMARY KEY, content text NOT NULL);
        INSERT INTO mixed_content VALUES
          ('json', '{"message":"R&amp;D"}'),
          ('plain', 'M&amp;A');
      `);
      await expect(client.query(transactionalMigration())).rejects.toThrow(/mixes parseable JSON and plain text/i);
      await client.query('ROLLBACK');
      const proof = await client.query(`SELECT id, content FROM mixed_content ORDER BY id`);
      expect(proof.rows).toEqual([
        { id: 'json', content: '{"message":"R&amp;D"}' },
        { id: 'plain', content: 'M&amp;A' },
      ]);
    });
  });

  it('defaults to the measured 87-column gate for any nonzero inventory', async () => {
    await withDatabase(databaseNames.gate, async (client) => {
      await client.query(`
        CREATE TABLE one_affected_column (id text PRIMARY KEY, content text NOT NULL);
        INSERT INTO one_affected_column VALUES ('1', 'R&amp;D');
      `);
      await expect(client.query(transactionalMigration())).rejects.toThrow(
        /expected 87 affected columns, observed 1/i
      );
      await client.query('ROLLBACK');
      const proof = await client.query(`SELECT content FROM one_affected_column`);
      expect(proof.rows[0].content).toBe('R&amp;D');
    });
  });
});
