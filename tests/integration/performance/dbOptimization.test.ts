...
describe('Database Performance Optimization', () => {
    ...
    beforeAll(async () => {
        ...
        await DbPromise.run(`CREATE TABLE IF NOT EXISTS organizations (id TEXT PRIMARY KEY, name TEXT)`);
        ...
        await DbPromise.run(`CREATE INDEX IF NOT EXISTS idx_tasks_org_assignee_status ON tasks(organization_id, assignee_id, status)`);
        await DbPromise.run(`CREATE INDEX IF NOT EXISTS idx_tasks_org_initiative ON tasks(organization_id, initiative_id)`);

        // Insert tasks with various combinations
        const tasks = [];
        for (let i = 0; i < 100; i++) {
            tasks.push(DbPromise.run(
