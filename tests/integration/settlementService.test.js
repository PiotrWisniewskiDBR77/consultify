...
const mockDb = {
  ...
  get: function (sql, params, callback) {
    ...
    if (
      sql.includes('settlement_periods') &&
      sql.includes('status = ?') &&
      params.includes('OPEN')
    ) {
    ...
    it('should select agreement valid at attribution date', () => {
      ...
      const selectAgreement = (date) => {
        return agreements.find(
          (a) => a.validFrom <= date && (a.validUntil === null || a.validUntil >= date)
        );
      };
...
describe('HTTP Status Code Compliance', () => {
  it('should return 409 for PERIOD_OVERLAP', () => {
    ...
    const statusCode = ['PERIOD_OVERLAP', 'OPEN_PERIOD_EXISTS'].includes(error.errorCode)
      ? 409
      : 400;
