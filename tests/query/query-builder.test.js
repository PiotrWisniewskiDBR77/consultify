...
const createInsertBuilder = () => {
    ...
    return {
        ...
        toSQL: () => {
            ...
            const placeholders = state.values.map(() =>
                `(${columns.map(() => '?').join(', ')})`
            ).join(', ');
...
const createUpdateBuilder = () => {
    ...
    return {
        ...
        toSQL: () => {
            const sets = Object.entries(state.sets).map(([k]) => `${k} = ?`).join(', ');
...
describe('Query Builder Tests', () => {
    ...
    it('should build with join', () => {
        const sql = qb.select('users.*', 'orders.total')
            .from('users')
    ...
    it('should build with group by and having', () => {
        const sql = qb.select('category', 'COUNT(*)')
            .from('products')
            .groupBy('category')
            .having('COUNT(*)', '>', 5)
            .toSQL();
