...
const createHedgedRequest = (timeout = 100) => {
    return {
        execute: async (requests) => {
            ...
            const racers = requests.map((fn, i) =>
...
describe('Retry with Backoff Tests', () => {
    ...
    it('should throw after max retries', async () => {
        ...
        await expect(retry.execute(async () => { throw new Error('always fail'); }))
...
describe('Timeout Wrapper Tests', () => {
    ...
    it('should throw on timeout', async () => {
        await expect(wrapper.execute(async () => {
...
describe('Fallback Pattern Tests', () => {
    ...
    it('should use fallback on failure', async () => {
        const primary = vi.fn(async () => { throw new Error(); });
        const fallback = vi.fn(async () => 'fallback');
