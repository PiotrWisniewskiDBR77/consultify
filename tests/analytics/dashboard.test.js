...
const createDashboardBuilder = () => {
    ...
    return {
        ...
        listDashboards: () => [...dashboards.values()].map(d => ({
            id: d.id,
            title: d.title,
            ...
            return data.filter(item => {
                return filters.every(f => {
                    ...
                    switch (f.operator) {
                        ...
                        case 'gt': return itemValue > f.value;
                        case 'gte': return itemValue >= f.value;
                        case 'lt': return itemValue < f.value;
                        case 'lte': return itemValue <= f.value;
                        case 'contains': return String(itemValue).includes(f.value);
                        case 'between': return itemValue >= f.value[0] && itemValue <= f.value[1];
                        case 'in': return f.value.includes(itemValue);
                        default: return true;
                    }
                });
            });
