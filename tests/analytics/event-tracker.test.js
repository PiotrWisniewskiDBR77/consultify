    ...
    return {
        ...
        getConversionRate: (funnelName, fromStep, toStep) => {
            ...
            const usersAtFrom = new Set(
                funnel.entries
                    .filter(e => e.stepIndex === fromIndex)
                    .map(e => e.userId)
            );

            const usersAtTo = new Set(
                funnel.entries
                    .filter(e => e.stepIndex === toIndex)
                    .map(e => e.userId)
            );

            const converted = [...usersAtFrom].filter(u => usersAtTo.has(u));

            return usersAtFrom.size > 0
                ? converted.length / usersAtFrom.size
                : 0;
        },
...
const createABTracker = () => {
    ...
    return {
        ...
        assignVariant: (experimentName, userId) => {
            ...
            const hash = Array.from(key).reduce((h, c) =>
        ...
        calculateSignificance: (experimentName) => {
            ...
            return {
                ...
                isSignificant: Math.abs(treatmentRate - controlRate) > 0.05 &&
                    control.total >= 100 && treatment.total >= 100,
            };
        },
    };
};
