...
const { linkBudgetInitiative }=vi.hoisted(()=>({linkBudgetInitiative:vi.fn()}));
vi.mock('@/services/api/v8/finance',()=>({V8FinanceApi:{linkBudgetInitiative}}));
vi.mock('react-router-dom',()=>({useNavigate:()=>vi.fn()}));
vi.mock('react-i18next',()=>({useTranslation:()=>({t:(key:string,fallback:any)=>typeof fallback==='string'?fallback:fallback?.defaultValue||key,i18n:{language:'en'}})}));
vi.mock('react-hot-toast',()=>({default:{success:vi.fn(),error:vi.fn()}}));
vi.mock('../../../src/services/api',()=>({API_URL:'/api',getHeaders:()=>({Authorization:'test'})}));
...
describe('BudgetWorkspace canonical initiative link handler',()=>{
 beforeEach(()=>{
  vi.clearAllMocks(); let id=0;
  vi.stubGlobal('crypto',{randomUUID:()=>`intent-${++id}`});
  vi.stubGlobal('fetch',vi.fn((url:string)=>{
   if(url.endsWith('/economics/budgets')) return json({budgets:[{id:'budget-1',title:'FY27',status:'DRAFT',periodStart:'2027-01',periodEnd:'2027-12',granularity:'monthly',currency:'PLN',version:1,createdAt:''}]});
   if(url.endsWith('/economics/budgets/budget-1')) return json({version:2,status:'DRAFT',lines:[],scenarios:[]});
   if(url.endsWith('/economics/budgets/budget-1/initiatives')) return json({initiatives:[]});
   if(url.endsWith('/initiatives')) return json({initiatives:[{id:'initiative-1',title:'Growth A',status:'DRAFT',priority:'HIGH'},{id:'initiative-2',title:'Growth B',status:'DRAFT',priority:'HIGH'}]});
   return json({});
  }));
 });
 ...
 it('applies returned version after a rendered successful link',async()=>{
  ...
  await waitFor(()=>expect(linkBudgetInitiative).toHaveBeenCalledWith('budget-1','initiative-1',2,expect.stringMatching(/^intent-/)));
  await screen.findByText(/PLN · v3/);
 });
 it('retries response loss with the exact original key and expectedVersion',async()=>{
  linkBudgetInitiative.mockRejectedValueOnce(new Error('response lost')).mockResolvedValueOnce({budgetVersion:3,replay:true});
  fireEvent.click(await openLink());
 ...
 it('mints a different key for B after ambiguous A while a later A retry keeps A tuple',async()=>{
  linkBudgetInitiative.mockRejectedValueOnce(new Error('A response lost')).mockRejectedValueOnce(new Error('B response lost')).mockResolvedValueOnce({budgetVersion:3,replay:true});
  fireEvent.click(await openLink());
  await waitFor(()=>expect(linkBudgetInitiative).toHaveBeenCalledTimes(1));
