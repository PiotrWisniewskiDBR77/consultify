...
describe('T22-TABLE-PREVIEW-COMPONENT AssessmentOutputsTab', () => {
  ...
  it('honest empty state — no rows, no fabricated data, distinct from the error state', async () => {
    mockArtifactsResponse([]);
    render(<AssessmentOutputsTab />);
    // Current StandardTable (unlike the T22 branch's snapshot this package
    // was drafted against) renders ONLY the EmptyState — no column headers —
    // when data.length === 0; see StandardTable.tsx's `data.length === 0 &&
    // filters.length === 0 && empty` branch. Assert the real, current
    // contract instead of a stale one.
    await waitFor(() => expect(screen.getByText('No outputs yet')).toBeInTheDocument());
    expect(
      screen.getByText('Assessment outputs promoted to the Outputs Library will appear here.')
    ).toBeInTheDocument();
    expect(screen.queryByText('Failed to load outputs. Please try again.')).not.toBeInTheDocument();
  });
