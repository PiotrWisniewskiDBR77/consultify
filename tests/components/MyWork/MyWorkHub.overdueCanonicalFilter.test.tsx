...
import { parseTaskHubDate } from '@/components/MyWork/taskHubFilter';
...
const pgDate = (offset: number): string => {
  const value = new Date();
  value.setHours(12, 0, 0, 0);
  value.setDate(value.getDate() + offset);
...
const buildHubTasks = () =>
  Array.from({ length: 57 }, (_, index) => ({
    ...
    status: 'todo',
    priority: 'medium',
    dueDate: index < 2 ? pgDate(-(index + 1)) : index === 2 ? pgDate(0) : pgDate(30 + index),
    createdAt: day(-60),
    updatedAt: day(-1),
    taskType: 'personal',
...
describe('MyWorkHub — canonical Overdue filter', () => {
  it('parses a PostgreSQL DATE at local midnight without moving the calendar day', () => {
    const parsed = parseTaskHubDate('2026-09-17');
