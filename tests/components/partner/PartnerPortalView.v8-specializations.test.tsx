...
vi.mock('../../../src/services/api/v8', () => ({
  V8PartnerApi: {
    getConnection: vi.fn(),
    updateOrganizationSpecializations: vi.fn(),
  },
...
function renderView() {
  return render(
    <MemoryRouter initialEntries={['/partner?tab=specializations']}>
...
describe('PartnerPortalView specializations V8 seam', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('uses the V8 route first for specialization updates', async () => {
    vi.mocked(V8PartnerApi.updateOrganizationSpecializations).mockResolvedValue({
      success: true,
  ...
  it('fails closed without a legacy mutation when the governed writer fails', async () => {
    ...
    await waitFor(() => expect(toastError).toHaveBeenCalledWith('Failed to save specializations'), {
      timeout: 10000,
    });
    expect(Api.put).not.toHaveBeenCalledWith(
      '/api/partners/organization/specializations',
