...
describe('PartnerCanonicalRuntimePanel', () => {
  ...
  it('loads all five governed surfaces and derives persisted states', async () => {
    ...
    vi.mocked(V8PartnerApi.getAttributions).mockResolvedValue({
      attributions: [{ id: 'attr-1', status: 'ACTIVE' }, { id: 'attr-2', status: 'EXPIRED' }],
    } as any);
  ...
  it('fails closed per surface and exposes no forbidden payout control', async () => {
    ...
    expect(within(panel).getByRole('note')).toHaveTextContent('Automatic payout and self-approval are unavailable');
    expect(within(panel).getByRole('list')).toHaveClass('grid-cols-1', 'sm:grid-cols-2', 'xl:grid-cols-5');
  });
