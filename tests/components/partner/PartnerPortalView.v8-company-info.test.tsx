...
function renderView() {
  return render(
    <MemoryRouter initialEntries={['/partner?tab=company-info']}>
...
describe('PartnerPortalView company info V8 seam', () => {
  ...
  it('uses the V8 route first for company-info updates', async () => {
    vi.mocked(V8PartnerApi.updateOrganization).mockResolvedValue({
      success: true,
    ...
    const nameInput = await screen.findByDisplayValue('Test Partner Co');
    fireEvent.change(nameInput, { target: { value: 'Updated Partner Co' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save Changes' }));

    await waitFor(() => {
      expect(V8PartnerApi.updateOrganization).toHaveBeenCalledWith({
