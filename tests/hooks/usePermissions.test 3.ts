...
describe('usePermissions Hook', () => {
    ...
    describe('User Info', () => {
        ...
        it('returns userId', () => {
            const { result } = renderHook(() => usePermissions());
            expect(result.current.userId === null || typeof result.current.userId === 'string').toBe(true);
        });

        it('returns userRole', () => {
            const { result } = renderHook(() => usePermissions());
            expect(result.current.userRole === null || typeof result.current.userRole === 'string').toBe(true);
        });

        it('returns organizationId', () => {
            const { result } = renderHook(() => usePermissions());
            expect(result.current.organizationId === null || typeof result.current.organizationId === 'string').toBe(true);
        });
    });
