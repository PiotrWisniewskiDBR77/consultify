...
const createFieldValidator = () => {
  return {
    ...
    minLength: (min, message) => (value) =>
      !value || value.length >= min || message || `Minimum ${min} characters`,

    maxLength: (max, message) => (value) =>
      !value || value.length <= max || message || `Maximum ${max} characters`,

    min: (minVal, message) => (value) =>
      value === undefined || value >= minVal || message || `Minimum value is ${minVal}`,

    max: (maxVal, message) => (value) =>
      value === undefined || value <= maxVal || message || `Maximum value is ${maxVal}`,
    ...
    matches: (field, message) => (value, values) =>
      value === values[field] || message || `Must match ${field}`,
