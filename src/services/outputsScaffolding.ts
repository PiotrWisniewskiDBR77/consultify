...
const TOOL_SCAFFOLDS: Record<string, OutputScaffold> = {
  'dynamic-swot': {
    ...
    reportOutline: [
      ...
      sec('cross-impact', 'Cross-Impact Matrix', 'Macierz wzajemnego wpływu', false, 'tool.crossImpact'),
      sec('takeaways', 'Key Takeaways', 'Kluczowe wnioski', true, 'tool.takeaways'),
      sec('initiatives', 'Recommended Initiatives', 'Rekomendowane inicjatywy', true, 'tool.initiatives'),
    ],
    deckOutline: [
      slide('title', 'Dynamic SWOT Analysis', 'Dynamiczna analiza SWOT', 'title', 'tool.meta'),
      slide('sw', 'Strengths & Weaknesses', 'Mocne i słabe strony', 'comparison', 'tool.strengths,tool.weaknesses'),
      slide('ot', 'Opportunities & Threats', 'Szanse i zagrożenia', 'comparison', 'tool.opportunities,tool.threats'),
      slide('matrix', 'Cross-Impact Matrix', 'Macierz wzajemnego wpływu', 'chart', 'tool.crossImpact'),
      slide('takeaways', 'Key Takeaways', 'Kluczowe wnioski', 'content', 'tool.takeaways'),
      slide('initiatives', 'Recommended Initiatives', 'Rekomendowane inicjatywy', 'content', 'tool.initiatives'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('strategy', 'Leverage ${strength} to capture ${opportunity}', 'Wykorzystaj ${strength} do uchwycenia ${opportunity}', 'tool.crossImpact'),
      hint('risk', 'Mitigate ${threat} by addressing ${weakness}', 'Zmitiguj ${threat} adresując ${weakness}', 'tool.crossImpact'),
    ],
  },
  ...
  'market-forces': {
    ...
    reportOutline: [
      ...
      sec('new-entrants', 'Threat of New Entrants', 'Zagrożenie nowych wejść', true, 'tool.newEntrants'),
      sec('substitutes', 'Threat of Substitutes', 'Zagrożenie substytutów', true, 'tool.substitutes'),
    ...
    deckOutline: [
      ...
      slide('rivalry', 'Competitive Rivalry', 'Rywalizacja konkurencyjna', 'content', 'tool.rivalry'),
      slide('forces', 'Entrants, Substitutes, Bargaining', 'Wejścia, substytuty, siła przetargowa', 'comparison', 'tool.newEntrants,tool.substitutes'),
      slide('levers', 'Strategic Levers', 'Dźwignie strategiczne', 'content', 'tool.levers'),
      slide('initiatives', 'Recommended Initiatives', 'Rekomendowane inicjatywy', 'content', 'tool.initiatives'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('strategy', 'Strengthen position against ${force}', 'Wzmocnij pozycję wobec ${force}', 'tool.levers'),
      hint('risk', 'Monitor ${force} intensity changes', 'Monitoruj zmiany intensywności ${force}', 'tool.scores'),
    ],
  },
  ...
  'growth-paths': {
    ...
    reportOutline: [
      ...
      sec('recommended-paths', 'Recommended Growth Paths', 'Rekomendowane ścieżki wzrostu', true, 'tool.recommended'),
    ],
    deckOutline: [
      ...
      slide('recommended', 'Recommended Paths', 'Rekomendowane ścieżki', 'content', 'tool.recommended'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('growth', 'Pursue ${path} growth opportunity', 'Realizuj szansę wzrostu ${path}', 'tool.recommended'),
      hint('risk', 'Mitigate risk for ${path} path', 'Zmitiguj ryzyko ścieżki ${path}', 'tool.riskFraming'),
    ],
  },
  ...
  'value-chain': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('primary-activities', 'Primary Activities', 'Aktywności podstawowe', true, 'tool.primary'),
      sec('support-activities', 'Support Activities', 'Aktywności wspierające', true, 'tool.support'),
    ...
    deckOutline: [
      ...
      slide('drivers', 'Cost & Value Drivers', 'Dźwignie kosztów i wartości', 'comparison', 'tool.costDrivers,tool.valueDrivers'),
      slide('hotspots', 'Improvement Hotspots', 'Hotspoty usprawnień', 'content', 'tool.hotspots'),
      slide('initiatives', 'Recommended Initiatives', 'Rekomendowane inicjatywy', 'content', 'tool.initiatives'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('efficiency', 'Optimize ${activity} cost structure', 'Zoptymalizuj strukturę kosztów ${activity}', 'tool.costDrivers'),
      hint('value', 'Enhance ${activity} value delivery', 'Wzmocnij dostarczanie wartości ${activity}', 'tool.valueDrivers'),
    ],
  },
  ...
  'portfolio-priority': {
    ...
    reportOutline: [
      ...
      sec('constraints', 'Constraints & Dependencies', 'Ograniczenia i zależności', false, 'tool.constraints'),
    ...
    deckOutline: [
      ...
      slide('criteria', 'Prioritization Criteria', 'Kryteria priorytetyzacji', 'content', 'tool.criteria'),
      ...
      slide('sequence', 'Recommended Sequence', 'Rekomendowana sekwencja', 'content', 'tool.roadmap'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('portfolio', 'Invest in ${initiative} (high impact)', 'Zainwestuj w ${initiative} (wysoki impact)', 'tool.topPicks'),
      hint('portfolio', 'Defer ${initiative} (low priority)', 'Odłóż ${initiative} (niski priorytet)', 'tool.scoring'),
    ],
  },
  ...
  'risk-uncertainty': {
    ...
    reportOutline: [
      ...
      sec('impact-probability', 'Impact × Probability Matrix', 'Macierz wpływ × prawdopodobieństwo', true, 'tool.matrix'),
    ...
    deckOutline: [
      slide('title', 'Risk & Uncertainty Analysis', 'Analiza ryzyk i niepewności', 'title', 'tool.meta'),
    ...
    initiativeHints: [
      hint('risk', 'Mitigate ${risk} through ${action}', 'Zmitiguj ${risk} przez ${action}', 'tool.mitigations'),
      hint('risk', 'Investigate unknown: ${unknown}', 'Zbadaj niewiadomą: ${unknown}', 'tool.unknowns'),
    ],
  },
  ...
  'capability-mapper': {
    ...
    reportOutline: [
      ...
      sec('priority-capabilities', 'Priority Capabilities', 'Priorytetowe kompetencje', true, 'tool.priorities'),
    ...
    deckOutline: [
      ...
      slide('priorities', 'Priority Capabilities', 'Priorytetowe kompetencje', 'content', 'tool.priorities'),
      slide('roadmap', 'Development Roadmap', 'Roadmapa rozwoju', 'content', 'tool.roadmap'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('capabilities', 'Build ${capability} to level ${target}', 'Rozwiń ${capability} do poziomu ${target}', 'tool.gaps'),
      hint('capabilities', 'Acquire ${capability} externally', 'Pozyskaj ${capability} z zewnątrz', 'tool.priorities'),
    ],
  },
  ...
  'ambition-decomposer': {
    ...
    initiativeHints: [
      hint('strategy', 'Deliver on ${dimension} ambition', 'Zrealizuj ambicję ${dimension}', 'tool.dimensions'),
      hint('measurement', 'Track ${metric} toward ${target}', 'Śledź ${metric} w kierunku ${target}', 'tool.targets'),
    ],
  },
  ...
  'focus-tradeoff': {
    ...
    initiativeHints: [
      hint('strategy', 'Double down on ${focusArea}', 'Podwój wysiłek w ${focusArea}', 'tool.focusAreas'),
  ...
  'narrative-engine': {
    ...
    initiativeHints: [
      hint('communication', 'Close messaging gap for ${audience}', 'Zamknij lukę komunikacyjną dla ${audience}', 'tool.messagingGaps'),
      hint('alignment', 'Align ${stakeholder} on strategy narrative', 'Wyrównaj ${stakeholder} wokół narracji strategii', 'tool.alignment'),
    ],
  },
  ...
  'sop-builder': {
    ...
    deckOutline: [
      slide('title', 'Standard Operating Procedure', 'Standardowa procedura operacyjna', 'title', 'tool.meta'),
    ...
    initiativeHints: [
      hint('standardization', 'Implement SOP for ${process}', 'Wdróż SOP dla ${process}', 'tool.scope'),
      hint('training', 'Train team on ${process} SOP', 'Przeszkol zespół z SOP ${process}', 'tool.training'),
    ],
  },
  ...
  'a3-problem-solving': {
    ...
    reportOutline: [
      ...
      sec('root-causes', 'Root Cause Analysis', 'Analiza przyczyn źródłowych', true, 'tool.rootCauses'),
      ...
      sec('follow-up', 'Follow-up & Verification', 'Follow-up i weryfikacja', true, 'tool.followUp'),
    ],
    deckOutline: [
      ...
      slide('current', 'Current vs Target', 'Stan obecny vs docelowy', 'comparison', 'tool.currentCondition,tool.targetCondition'),
      slide('root-causes', 'Root Causes', 'Przyczyny źródłowe', 'chart', 'tool.rootCauses'),
      slide('countermeasures', 'Countermeasures', 'Środki zaradcze', 'content', 'tool.countermeasures'),
    ...
    initiativeHints: [
      hint('improvement', 'Implement countermeasure: ${countermeasure}', 'Wdróż środek zaradczy: ${countermeasure}', 'tool.countermeasures'),
      hint('quality', 'Address root cause: ${rootCause}', 'Zaadresuj przyczynę źródłową: ${rootCause}', 'tool.rootCauses'),
    ],
  },
  ...
  'vsm-builder': {
    ...
    reportOutline: [
      ...
      sec('waste-analysis', 'Waste Identification', 'Identyfikacja marnotrawstw', true, 'tool.wasteAnalysis'),
      sec('metrics', 'Flow Metrics', 'Metryki przepływu', true, 'tool.metrics'),
      sec('future-state', 'Future State Design', 'Projekt stanu przyszłego', true, 'tool.futureState'),
    ...
    initiativeHints: [
      hint('lean', 'Eliminate waste at ${step}', 'Wyeliminuj marnotrawstwo w ${step}', 'tool.wasteAnalysis'),
      hint('flow', 'Implement future-state flow at ${step}', 'Wdróż przepływ docelowy w ${step}', 'tool.futureState'),
    ],
  },
  ...
  'constraint-control': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('constraint-id', 'Constraint Identification', 'Identyfikacja constraintu', true, 'tool.constraintId'),
      sec('exploitation', 'Exploitation Strategy', 'Strategia eksploatacji', true, 'tool.exploitation'),
      sec('subordination', 'Subordination Rules', 'Reguły podporządkowania', true, 'tool.subordination'),
    ...
    deckOutline: [
      slide('title', 'Constraint Control (TOC)', 'Kontrola constraintu (TOC)', 'title', 'tool.meta'),
      slide('constraint', 'System Constraint', 'Constraint systemowy', 'content', 'tool.constraintId'),
      slide('exploit', 'Exploitation Strategy', 'Strategia eksploatacji', 'content', 'tool.exploitation'),
    ...
    initiativeHints: [
      hint('throughput', 'Exploit constraint at ${resource}', 'Eksploatuj constraint w ${resource}', 'tool.constraintId'),
      hint('throughput', 'Elevate constraint capacity', 'Podnieś przepustowość constraintu', 'tool.elevation'),
    ],
  },
  ...
  'decision-engine': {
    ...
    initiativeHints: [
      hint('decision', 'Implement decision: ${option}', 'Wdróż decyzję: ${option}', 'tool.rationale'),
    ],
  },
  ...
  'control-tower': {
    ...
    initiativeHints: [
      hint('governance', 'Implement control tower for ${area}', 'Wdróż control tower dla ${area}', 'tool.kpiSet'),
      hint('measurement', 'Define thresholds for ${kpi}', 'Zdefiniuj progi dla ${kpi}', 'tool.thresholds'),
    ],
  },
  ...
  'automation-pipeline': {
    ...
    deckOutline: [
      ...
      slide('sizing', 'Sizing & Prioritization', 'Sizing i priorytetyzacja', 'chart', 'tool.sizing'),
    ...
    initiativeHints: [
      hint('automation', 'Automate ${process} (ROI: ${roi})', 'Zautomatyzuj ${process} (ROI: ${roi})', 'tool.backlog'),
      hint('automation', 'Build automation capability for ${area}', 'Zbuduj zdolność automatyzacji dla ${area}', 'tool.delivery'),
    ],
  },
  ...
  'smed-planner': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('current-changeover', 'Current Changeover Analysis', 'Analiza obecnego przezbrojenia', true, 'tool.currentChangeover'),
      sec('step-classification', 'Step Classification (Internal/External)', 'Klasyfikacja kroków (internal/external)', true, 'tool.stepClassification'),
      sec('conversion-plan', 'Conversion Plan', 'Plan konwersji', true, 'tool.conversionPlan'),
      sec('time-reduction', 'Time Reduction Estimate', 'Szacunek redukcji czasu', true, 'tool.timeReduction'),
      sec('improvement-backlog', 'Improvement Backlog', 'Backlog usprawnień', true, 'tool.improvementBacklog'),
    ],
    deckOutline: [
      slide('title', 'SMED Planner', 'Planer SMED', 'title', 'tool.meta'),
      slide('current', 'Current Changeover', 'Obecne przezbrojenie', 'chart', 'tool.currentChangeover'),
      slide('classification', 'Step Classification', 'Klasyfikacja kroków', 'comparison', 'tool.stepClassification'),
    ...
    initiativeHints: [
      hint('lean', 'Convert internal step to external: ${step}', 'Konwertuj krok wewnętrzny na zewnętrzny: ${step}', 'tool.conversionPlan'),
      hint('efficiency', 'Reduce changeover time by ${percent}%', 'Zredukuj czas przezbrojenia o ${percent}%', 'tool.timeReduction'),
    ],
  },
  ...
  'dms-builder': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('tier-cadence', 'Tier Cadence Design', 'Projekt cadence tierów', true, 'tool.tierCadence'),
      ...
      sec('visual-management', 'Visual Management', 'Zarządzanie wizualne', false, 'tool.visualManagement'),
    ],
    deckOutline: [
      slide('title', 'Daily Management System', 'System zarządzania dziennego', 'title', 'tool.meta'),
      ...
      slide('roles', 'Roles & Responsibilities', 'Role i odpowiedzialności', 'content', 'tool.roles'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('governance', 'Implement DMS for ${area}', 'Wdróż DMS dla ${area}', 'tool.tierCadence'),
      hint('measurement', 'Deploy visual management at ${location}', 'Wdróż zarządzanie wizualne w ${location}', 'tool.visualManagement'),
    ],
  },
  ...
  'inventory-autopilot': {
    ...
    reportOutline: [
      ...
      sec('policy-table', 'Replenishment Policy Table', 'Tabela polityk uzupełnień', true, 'tool.policyTable'),
      sec('simulation', 'Simulation Assumptions', 'Założenia symulacji', true, 'tool.simulation'),
      sec('tradeoffs', 'Cash/Stockout Trade-offs', 'Trade-offy cash/stockout', true, 'tool.tradeoffs'),
    ...
    deckOutline: [
      ...
      slide('policies', 'Replenishment Policies', 'Polityki uzupełnień', 'content', 'tool.policyTable'),
      slide('tradeoffs', 'Cash vs Stockout', 'Cash vs stockout', 'chart', 'tool.tradeoffs'),
      slide('recommendations', 'Recommendations', 'Rekomendacje', 'content', 'tool.recommendations'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('supply-chain', 'Implement autopilot for ${segment} SKUs', 'Wdróż autopilot dla SKU segmentu ${segment}', 'tool.skuSegmentation'),
      hint('cost-reduction', 'Optimize safety stock for ${segment}', 'Zoptymalizuj zapas bezpieczeństwa dla ${segment}', 'tool.policyTable'),
    ],
  },
  ...
  'robotics-feasibility': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('feasibility-score', 'Feasibility Score', 'Wynik wykonalności', true, 'tool.feasibilityScore'),
      sec('prerequisites', 'Prerequisites Assessment', 'Ocena prerekwizytów', true, 'tool.prerequisites'),
    ...
    initiativeHints: [
      hint('automation', 'Deploy robotics pilot for ${process}', 'Wdróż pilota robotyki dla ${process}', 'tool.pilotPlan'),
      hint('digital', 'Address prerequisite: ${prerequisite}', 'Zaadresuj prerekwizyt: ${prerequisite}', 'tool.prerequisites'),
    ],
  },
  ...
  'logistics-automation': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('opportunity-map', 'Automation Opportunity Map', 'Mapa okazji automatyzacji', true, 'tool.opportunityMap'),
      sec('prerequisites', 'Prerequisites', 'Prerekwizyty', true, 'tool.prerequisites'),
      sec('technology-options', 'Technology Options', 'Opcje technologiczne', true, 'tool.technologyOptions'),
    ...
    deckOutline: [
      ...
      slide('tech', 'Technology Options', 'Opcje technologiczne', 'comparison', 'tool.technologyOptions'),
    ...
    initiativeHints: [
      hint('automation', 'Automate ${area} logistics flow', 'Zautomatyzuj przepływ logistyczny ${area}', 'tool.opportunityMap'),
      hint('digital', 'Deploy ${technology} in warehouse', 'Wdróż ${technology} w magazynie', 'tool.technologyOptions'),
    ],
  },
  ...
  'rpa-scanner': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('candidate-list', 'RPA Candidate List', 'Lista kandydatów RPA', true, 'tool.candidateList'),
      sec('suitability-scoring', 'Suitability Scoring', 'Scoring odpowiedniości', true, 'tool.suitabilityScoring'),
      sec('complexity-assessment', 'Complexity Assessment', 'Ocena złożoności', true, 'tool.complexityAssessment'),
      sec('prioritized-backlog', 'Prioritized Backlog', 'Priorytetyzowany backlog', true, 'tool.prioritizedBacklog'),
      sec('economics', 'Economics & FTE Savings', 'Ekonomia i oszczędności FTE', true, 'tool.economics'),
    ],
    deckOutline: [
      ...
      slide('scoring', 'Suitability Scoring', 'Scoring odpowiedniości', 'chart', 'tool.suitabilityScoring'),
      slide('backlog', 'Prioritized Backlog', 'Priorytetyzowany backlog', 'content', 'tool.prioritizedBacklog'),
      slide('economics', 'Economics & FTE Savings', 'Ekonomia i oszczędności FTE', 'chart', 'tool.economics'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('automation', 'Implement RPA for ${process}', 'Wdróż RPA dla ${process}', 'tool.prioritizedBacklog'),
      hint('efficiency', 'Save ${fteCount} FTE via ${process} automation', 'Zaoszczędź ${fteCount} FTE przez automatyzację ${process}', 'tool.economics'),
    ],
  },
  ...
  'ai-discovery': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('use-case-inventory', 'AI Use-Case Inventory', 'Inwentarz use-case\'ów AI', true, 'tool.useCaseInventory'),
      sec('feasibility', 'Feasibility Assessment', 'Ocena wykonalności', true, 'tool.feasibility'),
      sec('prerequisites', 'Data & Infrastructure Prerequisites', 'Prerekwizyty danych i infrastruktury', true, 'tool.prerequisites'),
    ...
    deckOutline: [
      ...
      slide('use-cases', 'AI Use-Case Shortlist', 'Shortlista use-case\'ów AI', 'content', 'tool.useCaseInventory'),
      slide('feasibility', 'Feasibility Matrix', 'Macierz wykonalności', 'chart', 'tool.feasibility'),
      slide('prerequisites', 'Prerequisites', 'Prerekwizyty', 'content', 'tool.prerequisites'),
    ...
    initiativeHints: [
      hint('ai', 'Pilot AI use-case: ${useCase}', 'Pilotuj use-case AI: ${useCase}', 'tool.useCaseInventory'),
      hint('data', 'Build data foundation for ${useCase}', 'Zbuduj fundament danych dla ${useCase}', 'tool.prerequisites'),
    ],
  },
  ...
  'integration-diagnostic': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('integration-map', 'Integration Landscape Map', 'Mapa krajobrazu integracji', true, 'tool.integrationMap'),
      sec('debt-assessment', 'Integration Debt Assessment', 'Ocena długu integracyjnego', true, 'tool.debtAssessment'),
      sec('risk-hotspots', 'Risk Hotspots', 'Hotspoty ryzyk', true, 'tool.riskHotspots'),
      sec('architecture-roadmap', 'Architecture Remediation Roadmap', 'Roadmapa naprawy architektury', true, 'tool.architectureRoadmap'),
      sec('priority-actions', 'Priority Actions', 'Priorytetowe akcje', true, 'tool.priorityActions'),
    ],
    deckOutline: [
      ...
      slide('roadmap', 'Remediation Roadmap', 'Roadmapa naprawy', 'content', 'tool.architectureRoadmap'),
      slide('actions', 'Priority Actions', 'Priorytetowe akcje', 'content', 'tool.priorityActions'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('architecture', 'Remediate integration at ${system}', 'Napraw integrację w ${system}', 'tool.architectureRoadmap'),
      hint('risk', 'Address integration risk: ${risk}', 'Zaadresuj ryzyko integracji: ${risk}', 'tool.riskHotspots'),
    ],
  },
  ...
  'digital-value-pool': {
    ...
    reportOutline: [
      ...
      sec('economic-levers', 'Economic Levers', 'Dźwignie ekonomiczne', true, 'tool.economicLevers'),
      sec('digital-enablers', 'Digital Enablers', 'Enablery cyfrowe', true, 'tool.digitalEnablers'),
      sec('initiative-shortlist', 'Initiative Shortlist', 'Shortlista inicjatyw', true, 'tool.initiativeShortlist'),
    ...
    deckOutline: [
      ...
      slide('initiatives', 'Initiative Shortlist', 'Shortlista inicjatyw', 'content', 'tool.initiativeShortlist'),
      slide('economics', 'Value Sizing', 'Sizing wartości', 'chart', 'tool.economics'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('digital', 'Capture digital value in ${pool}', 'Uchwyć wartość cyfrową w ${pool}', 'tool.valuePoolMap'),
      hint('economics', 'Activate economic lever: ${lever}', 'Aktywuj dźwignię ekonomiczną: ${lever}', 'tool.economicLevers'),
    ],
  },
  ...
  'legacy-analyzer': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('legacy-scorecard', 'Legacy Drag Scorecard', 'Scorecard obciążenia legacy', true, 'tool.legacyScorecard'),
      ...
      sec('modernization-options', 'Modernization Options', 'Opcje modernizacji', true, 'tool.modernizationOptions'),
      sec('modernization-backlog', 'Modernization Backlog', 'Backlog modernizacji', true, 'tool.modernizationBacklog'),
    ],
    deckOutline: [
      slide('title', 'Legacy Analyzer', 'Analizator legacy', 'title', 'tool.meta'),
      slide('scorecard', 'Legacy Drag Scorecard', 'Scorecard obciążenia legacy', 'chart', 'tool.legacyScorecard'),
      ...
      slide('options', 'Modernization Options', 'Opcje modernizacji', 'comparison', 'tool.modernizationOptions'),
      slide('backlog', 'Modernization Backlog', 'Backlog modernizacji', 'content', 'tool.modernizationBacklog'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('modernization', 'Modernize ${system} (strategy: ${strategy})', 'Zmodernizuj ${system} (strategia: ${strategy})', 'tool.modernizationBacklog'),
      hint('risk', 'Mitigate legacy risk at ${system}', 'Zmitiguj ryzyko legacy w ${system}', 'tool.riskHeatmap'),
    ],
  },
  ...
  'data-inventory': {
    ...
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'tool.summary'),
      sec('decision-data-map', 'Decision→Data Map', 'Mapa decyzja→dane', true, 'tool.decisionDataMap'),
      sec('gap-register', 'Data Gap Register', 'Rejestr luk danych', true, 'tool.gapRegister'),
      sec('quality-assessment', 'Data Quality Assessment', 'Ocena jakości danych', true, 'tool.qualityAssessment'),
      sec('data-initiative-backlog', 'Data Initiative Backlog', 'Backlog inicjatyw danych', true, 'tool.dataInitiativeBacklog'),
      sec('governance-needs', 'Governance Needs', 'Potrzeby governance', false, 'tool.governanceNeeds'),
    ],
    deckOutline: [
      ...
      slide('backlog', 'Data Initiative Backlog', 'Backlog inicjatyw danych', 'content', 'tool.dataInitiativeBacklog'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('data', 'Close data gap for ${decision}', 'Zamknij lukę danych dla ${decision}', 'tool.gapRegister'),
      hint('data', 'Improve data quality in ${domain}', 'Popraw jakość danych w ${domain}', 'tool.qualityAssessment'),
    ],
  },
  ...
  'pain-to-solution': {
    ...
    reportOutline: [
      ...
      sec('archetype-mapping', 'Pain→Archetype Mapping', 'Mapowanie ból→archetyp', true, 'tool.archetypeMapping'),
      sec('solution-backlog', 'Solution Backlog', 'Backlog rozwiązań', true, 'tool.solutionBacklog'),
    ...
    deckOutline: [
      ...
      slide('solutions', 'Solution Backlog', 'Backlog rozwiązań', 'content', 'tool.solutionBacklog'),
      slide('feasibility', 'Feasibility Assessment', 'Ocena wykonalności', 'chart', 'tool.feasibility'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('solutions', 'Implement ${solution} for ${pain}', 'Wdróż ${solution} dla ${pain}', 'tool.solutionBacklog'),
      hint('discovery', 'Validate feasibility of ${solution}', 'Zwaliduj wykonalność ${solution}', 'tool.feasibility'),
    ],
  },
  ...
  'pain-explorer': {
    ...
    reportOutline: [
      ...
      sec('structured-problems', 'Structured Problem List', 'Lista ustrukturyzowanych problemów', true, 'tool.structuredProblems'),
      ...
      sec('priority-problems', 'Priority Problems', 'Priorytetowe problemy', true, 'tool.priorityProblems'),
    ],
    deckOutline: [
      ...
      slide('problems', 'Structured Problems', 'Ustrukturyzowane problemy', 'content', 'tool.structuredProblems'),
      ...
      slide('priorities', 'Priority Problems', 'Priorytetowe problemy', 'content', 'tool.priorityProblems'),
      slide('next', 'Next Steps', 'Następne kroki', 'summary', 'tool.nextSteps'),
    ],
    initiativeHints: [
      hint('discovery', 'Investigate: ${problem}', 'Zbadaj: ${problem}', 'tool.structuredProblems'),
      hint('validation', 'Validate hypothesis: ${hypothesis}', 'Zwaliduj hipotezę: ${hypothesis}', 'tool.hypotheses'),
    ],
  },
  ...
  'process-automation': {
    ...
    reportOutline: [
      ...
      sec('target-design', 'Target Process Design', 'Projekt procesu docelowego', true, 'tool.targetDesign'),
      sec('automation-spec', 'Automation Specification', 'Specyfikacja automatyzacji', true, 'tool.automationSpec'),
      sec('economics', 'Economics & Payback', 'Ekonomia i payback', true, 'tool.economics'),
    ...
    deckOutline: [
      ...
      slide('baseline-target', 'Baseline vs Target', 'Baseline vs target', 'comparison', 'tool.baseline,tool.targetDesign'),
      slide('automation', 'Automation Specification', 'Specyfikacja automatyzacji', 'content', 'tool.automationSpec'),
      slide('economics', 'Economics & Payback', 'Ekonomia i payback', 'chart', 'tool.economics'),
    ...
    initiativeHints: [
      hint('automation', 'Automate ${processStep} (payback: ${months}m)', 'Zautomatyzuj ${processStep} (payback: ${months}m)', 'tool.economics'),
      hint('process-improvement', 'Redesign ${processStep} before automation', 'Przeprojektuj ${processStep} przed automatyzacją', 'tool.targetDesign'),
    ],
  },
};
...
const ASSESSMENT_SCAFFOLDS: Record<string, OutputScaffold> = {
  DRD: {
    ...
    sourceSlug: 'DRD',
    reportOutline: [
      sec('exec-summary', 'Executive Summary', 'Podsumowanie wykonawcze', true, 'assessment.summary'),
      sec('methodology', 'Assessment Methodology', 'Metodologia oceny', true, 'assessment.methodology'),
      sec('dimension-scores', 'Dimension Scores', 'Wyniki wymiarów', true, 'assessment.dimensionScores'),
      sec('maturity-profile', 'Maturity Profile', 'Profil dojrzałości', true, 'assessment.maturityProfile'),
      sec('gap-analysis', 'Gap Analysis', 'Analiza luk', true, 'assessment.gapAnalysis'),
      sec('benchmark', 'Benchmark Comparison', 'Porównanie z benchmarkiem', false, 'assessment.benchmark'),
      sec('recommendations', 'Recommendations', 'Rekomendacje', true, 'assessment.recommendations'),
      sec('roadmap', 'Transformation Roadmap', 'Roadmapa transformacji', true, 'assessment.roadmap'),
    ],
    deckOutline: [
      slide('title', 'Digital Readiness Diagnosis', 'Diagnoza gotowości cyfrowej', 'title', 'assessment.meta'),
      ...
      slide('profile', 'Maturity Profile', 'Profil dojrzałości', 'chart', 'assessment.maturityProfile'),
      slide('gaps', 'Gap Analysis', 'Analiza luk', 'chart', 'assessment.gapAnalysis'),
      slide('benchmark', 'Benchmark Comparison', 'Porównanie z benchmarkiem', 'comparison', 'assessment.benchmark'),
      slide('recommendations', 'Recommendations', 'Rekomendacje', 'content', 'assessment.recommendations'),
      slide('roadmap', 'Transformation Roadmap', 'Roadmapa transformacji', 'content', 'assessment.roadmap'),
    ],
    initiativeHints: [
      hint('digital', 'Improve ${dimension} from ${current} to ${target}', 'Popraw ${dimension} z ${current} do ${target}', 'assessment.gapAnalysis'),
      hint('transformation', 'Address gap in ${dimension}', 'Zaadresuj lukę w ${dimension}', 'assessment.gapAnalysis'),
    ],
  },
  ...
  SIRI: {
    ...
    reportOutline: [
      ...
      sec('building-blocks', 'Building Blocks Assessment', 'Ocena bloków budulcowych', true, 'assessment.buildingBlocks'),
      sec('dimension-scores', 'Dimension Scores', 'Wyniki wymiarów', true, 'assessment.dimensionScores'),
      sec('band-classification', 'Band Classification', 'Klasyfikacja pasma', true, 'assessment.bandClassification'),
      sec('gap-analysis', 'Gap Analysis', 'Analiza luk', true, 'assessment.gapAnalysis'),
      sec('prioritization', 'Prioritization Matrix', 'Macierz priorytetyzacji', true, 'assessment.prioritization'),
      sec('roadmap', 'Industry 4.0 Roadmap', 'Roadmapa Industry 4.0', true, 'assessment.roadmap'),
    ],
    deckOutline: [
      slide('title', 'Smart Industry Readiness Index', 'Indeks gotowości Smart Industry', 'title', 'assessment.meta'),
      slide('methodology', 'SIRI Methodology', 'Metodologia SIRI', 'content', 'assessment.methodology'),
      ...
      slide('band', 'Band Classification', 'Klasyfikacja pasma', 'chart', 'assessment.bandClassification'),
      slide('gaps', 'Gap Analysis', 'Analiza luk', 'chart', 'assessment.gapAnalysis'),
      slide('priorities', 'Prioritization Matrix', 'Macierz priorytetyzacji', 'chart', 'assessment.prioritization'),
      slide('roadmap', 'Industry 4.0 Roadmap', 'Roadmapa Industry 4.0', 'content', 'assessment.roadmap'),
    ],
    initiativeHints: [
      hint('industry4.0', 'Advance ${dimension} to band ${targetBand}', 'Podnieś ${dimension} do pasma ${targetBand}', 'assessment.gapAnalysis'),
      hint('technology', 'Deploy ${technology} for ${dimension}', 'Wdróż ${technology} dla ${dimension}', 'assessment.prioritization'),
    ],
  },
  ...
  ADMA: {
    ...
    reportOutline: [
      ...
      sec('dimension-detail', 'Dimension Detail', 'Szczegóły wymiarów', true, 'assessment.dimensionDetail'),
      sec('maturity-level', 'Overall Maturity Level', 'Ogólny poziom dojrzałości', true, 'assessment.maturityLevel'),
      sec('gap-analysis', 'Gap Analysis', 'Analiza luk', true, 'assessment.gapAnalysis'),
      sec('recommendations', 'Recommendations', 'Rekomendacje', true, 'assessment.recommendations'),
      sec('action-plan', 'Action Plan', 'Plan działań', true, 'assessment.actionPlan'),
    ],
    deckOutline: [
      slide('title', 'Advanced Digital Maturity Assessment', 'Zaawansowana ocena dojrzałości cyfrowej', 'title', 'assessment.meta'),
      slide('methodology', 'ADMA Methodology', 'Metodologia ADMA', 'content', 'assessment.methodology'),
      slide('pillars', 'Pillar Scores', 'Wyniki filarów', 'chart', 'assessment.pillarScores'),
      slide('dimensions', 'Dimension Detail', 'Szczegóły wymiarów', 'chart', 'assessment.dimensionDetail'),
      slide('maturity', 'Overall Maturity', 'Ogólna dojrzałość', 'chart', 'assessment.maturityLevel'),
      slide('gaps', 'Gap Analysis', 'Analiza luk', 'chart', 'assessment.gapAnalysis'),
      slide('recommendations', 'Recommendations', 'Rekomendacje', 'content', 'assessment.recommendations'),
      slide('action-plan', 'Action Plan', 'Plan działań', 'content', 'assessment.actionPlan'),
    ],
    initiativeHints: [
      hint('digital', 'Improve ${pillar} maturity from ${current} to ${target}', 'Popraw dojrzałość ${pillar} z ${current} do ${target}', 'assessment.gapAnalysis'),
      hint('transformation', 'Implement recommendation for ${dimension}', 'Wdróż rekomendację dla ${dimension}', 'assessment.recommendations'),
    ],
  },
};
...
function buildGenericTemplateScaffold(slug: string): OutputScaffold {
  return {
    ...
    deckOutline: [
      ...
      slide('recommendations', 'Recommendations', 'Rekomendacje', 'content', 'template.recommendations'),
      slide('next-steps', 'Next Steps', 'Następne kroki', 'summary', 'template.nextSteps'),
    ],
    initiativeHints: [
      hint('general', 'Implement recommendation: ${recommendation}', 'Wdróż rekomendację: ${recommendation}', 'template.recommendations'),
    ],
  };
}
...
export function getOutputScaffold(
  sourceType: string,
  sourceSlug: string
): OutputScaffold | null {
  switch (sourceType) {
...
export { TOOL_SCAFFOLDS, ASSESSMENT_SCAFFOLDS };
