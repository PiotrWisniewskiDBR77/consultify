...
describe('Results recovery canonical client', () => {
  ...
  it('unwraps Axios-like action create and update responses', async () => {
    ...
    vi.mocked(Api.patch).mockResolvedValueOnce({
      data: { action: { ...action, status: 'DONE', rowVersion: 2 } },
    } as any);
    await expect(
      V8ResultsApi.createRecoveryAction('card 1', {
        title: 'Recover',
        actionType: 'IMMEDIATE',
    ...
    await expect(
      V8ResultsApi.updateRecoveryAction('card 1', 'action-1', {
        expectedVersion: 1,
        status: 'DONE',
    ...
    expect(Api.post).toHaveBeenCalledWith(
      '/vnext/results/kpi/recovery-cards/card%201/actions',
      expect.anything()
    );
    expect(Api.patch).toHaveBeenCalledWith(
      '/vnext/results/kpi/recovery-cards/card%201/actions/action-1',
      expect.anything()
    );
  });
  ...
  it('unwraps and validates the atomic task-link identity', async () => {
    ...
    await expect(
      V8ResultsApi.linkRecoveryActionTask('card-1', 'action-1', {
        expectedVersion: 1,
  ...
  it('unwraps Axios-like checkpoint create and resolve responses', async () => {
    ...
    await expect(
      V8ResultsApi.createRecoveryCheckpoint('card-1', {
        checkpointDate: '2026-08-20',
        idempotencyKey: 'idem-checkpoint-1',
      })
    ).resolves.toEqual(checkpoint);
    await expect(
      V8ResultsApi.resolveRecoveryCheckpoint('card-1', 'checkpoint-1', {
        expectedVersion: 1,
        status: 'MISSED',
