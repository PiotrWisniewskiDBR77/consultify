    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={`
                    ...
                    ${
                      hasSelection
                        ? 'bg-slate-100 border-slate-300 text-slate-800 dark:bg-white/[0.08] dark:border-white/[0.15] dark:text-white'
                        : 'bg-white dark:bg-navy-800 border-slate-200 dark:border-navy-700 text-slate-600 dark:text-slate-300 hover:border-slate-300 dark:hover:border-white/[0.15]'
                    }
                `}
      >
        {icon}
        <span>{label}</span>
        {hasSelection && (
          <span className="ml-1 px-1.5 py-0.5 bg-slate-600 dark:bg-white/20 text-white text-[10px] rounded-full">
            {selected.length}
          </span>
        )}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-navy-800 rounded-xl shadow-xl border border-slate-200 dark:border-navy-700 z-30 overflow-hidden"
          >
      ...
      <button
        ...
        className={`
                    ...
                    ${
                      hasSelection
                        ? 'bg-slate-100 border-slate-300 text-slate-800 dark:bg-white/[0.08] dark:border-white/[0.15] dark:text-white'
                        : 'bg-white dark:bg-navy-800 border-slate-200 dark:border-navy-700 text-slate-600 dark:text-slate-300 hover:border-slate-300'
                    }
                `}
      >
        <Layers size={14} />
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-full left-0 mt-1 w-64 bg-white dark:bg-navy-800 rounded-xl shadow-xl border border-slate-200 dark:border-navy-700 z-30 overflow-hidden"
          >
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-white dark:bg-navy-800 border border-slate-200 dark:border-navy-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand/50"
          />
          {filters.search && (
            <button
          ...
          >
            ...
            >
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-slate-200 dark:border-navy-700 rounded-lg mb-4"
                autoFocus
              />
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm bg-navy-900 text-white rounded-lg disabled:opacity-50"
                >
                  {t('common.save', 'Save')}
                </button>
              </div>
