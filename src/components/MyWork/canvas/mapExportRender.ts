...
export function buildVectorSvg(container: HTMLElement, options: VectorSvgOptions = {}): string {
  ...
  for (const path of readEdgePaths(viewport)) {
    ...
    const gradientId = /^url\(["']?#([^"')]+)["']?\)/.exec(strokeRaw)?.[1];
    const solid = gradientId ? null : parseColor(strokeRaw);
  ...
  // Gradienty krawędzi są zdefiniowane w `<defs>` warstwy krawędzi — bierzemy
  // je żywcem, żeby `url(#gradient-…)` wskazywało na coś realnego.
  const defsSource = viewport.querySelector('.react-flow__edges defs');
  const defs = defsSource ? defsSource.innerHTML : '';

  // ── Węzły ────────────────────────────────────────────────────────────────
  const nodeMarkup: string[] = [];
  for (const box of readNodeBoxes(viewport)) {
    const card = (box.element.firstElementChild as HTMLElement | null) ?? box.element;
    ...
    if (label) {
      const fontPx = Math.max(8, firstNumber(computed.fontSize, 12));
      const fontWeight = computed.fontWeight || '500';
      const fontFamily = computed.fontFamily || 'Inter, Arial, Helvetica, sans-serif';
      const textColor = parseColor(computed.color) ?? { hex: '#0f172a', alpha: 1 };
      ...
      const centered = (computed.textAlign || '').includes('center');
