...
const slaPill = (
  ...
  if (!sla)
    return { label: '-', className: 'text-c-text-secondary', dot: 'bg-slate-400' };
      ...
      >
        ...
        <td className="px-3 py-3" style={{ width: columnWidths.title }}>
          <div className="flex items-center gap-2">
            <span
              className="text-sm font-semibold text-c-text truncate block"
              title={item.title}
            >
              {item.title}
            </span>
            ...
            >
              ...
              >
                {isSortable ? (
                  ...
                  <span
                    className={(tableFilters[colId] as string[])?.length ? 'text-c-text-secondary' : ''}
                  >
                    {getColumnLabel(colId)}
                  </span>
                )}
        ...
        >
          <div className="p-3 flex flex-col gap-2.5">
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-1.5">
                  <h3
                    className="text-[13px] font-semibold text-c-text leading-snug line-clamp-2"
                    title={item.title}
                  >
                    {item.title}
                  </h3>
            ...
            {showRowDescription && cardBriefText ? (
              <p className="-mt-0.5 line-clamp-2 text-[11px] leading-4 text-c-text-muted">
                {cardBriefText}
              </p>
            ) : null}
            ...
            <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity -mt-0.5">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 h-6 px-2 rounded-full text-[10px] font-medium border border-c-border-subtle bg-transparent text-c-text-muted hover:bg-black/[0.04] dark:hover:bg-white/[0.04] transition-colors"
              >
                <Archive size={10} />
