...
export const AIWhatIfScenarios: React.FC<AIWhatIfScenariosProps> = ({
      ...
      if (res?.suggestions && Array.isArray(res.suggestions)) {
        setScenarios(
          res.suggestions.slice(0, 6).map((s: any, idx: number) => ({
            ...
            type: s.category === 'risk_flags' ? 'risk' : s.category === 'findings' ? 'alternative' : 'opportunity',
            impact: s.confidence && s.confidence > 0.7 ? 'high' : s.confidence > 0.4 ? 'medium' : 'low',
          }))
        );
      }
    } catch (err: any) {
  ...
  const typeConfig = {
    opportunity: { icon: Sparkles, color: 'text-emerald-500', bg: 'bg-emerald-500/10', label: isPl ? 'Szansa' : 'Opportunity' },
    risk: { icon: AlertTriangle, color: 'text-red-500', bg: 'bg-red-500/10', label: isPl ? 'Ryzyko' : 'Risk' },
    alternative: { icon: Route, color: 'text-blue-500', bg: 'bg-blue-500/10', label: isPl ? 'Alternatywa' : 'Alternative' },
  };
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-slate-200/60 dark:border-navy-700/60">
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
              {isPl ? `Scenariusze dla: "${selectedNodeLabel}"` : `Scenarios for: "${selectedNodeLabel}"`}
            </p>
          </div>
          ...
          {scenarios.length === 0 && !loading && (
            <div className="text-center py-8">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mb-4">
                {isPl ? 'Wygeneruj alternatywne ścieżki, ryzyka i szanse.' : 'Generate alternative paths, risks and opportunities.'}
              </p>
              <button
          ...
          {scenarios.length > 0 && (
            <div className="space-y-2">
              {scenarios.map((scenario) => {
                ...
                const Icon = cfg.icon;
                return (
                  <div key={scenario.id} className={`p-3 rounded-xl border border-slate-200/30 dark:border-navy-700/30 ${cfg.bg}`}>
                    <div className="flex items-start gap-2">
                      <Icon size={14} className={`mt-0.5 shrink-0 ${cfg.color}`} />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] font-bold text-slate-800 dark:text-white">{scenario.title}</span>
                          <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-full ${impactBadge[scenario.impact]}`}>
                            {scenario.impact.toUpperCase()}
                          </span>
                        </div>
                        {scenario.description && (
                          <p className="text-[10px] text-slate-600 dark:text-slate-300 mt-1 leading-relaxed">{scenario.description}</p>
                        )}
