...
const CLUSTER_COLORS = ['#fb7185', '#34d399', '#fbbf24', '#38bdf8', '#a78bfa', '#22d3ee', '#f472b6', '#4ade80'];
        ...
        const detected: Cluster[] = res.suggestions.slice(0, 6).map((s: any, idx: number) => {
          const matchedIds = ideaNodes
            .filter((n) => {
              ...
              return clusterText.includes(label.slice(0, 10)) || label.includes(clusterText.slice(0, 10));
            })
            .map((n) => n.id);
          ...
          return {
            ...
            nodeIds: matchedIds.length > 0 ? matchedIds : ideaNodes.slice(idx * 2, idx * 2 + 2).map((n) => n.id),
        ...
        <div className="px-5 py-4 max-h-[60vh] overflow-y-auto">
          {clusters.length === 0 && !loading && (
            <div className="text-center py-8">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mb-4">
                {isPl ? 'AI pogrupuje pomysły w klastry tematyczne.' : 'AI will group ideas into thematic clusters.'}
              </p>
              <button
          ...
          {loading && (
            <div className="flex items-center justify-center gap-2 py-8">
              ...
              <span className="text-[11px] text-slate-500">{isPl ? 'Grupuję...' : 'Clustering...'}</span>
            </div>
          )}
          ...
          {clusters.length > 0 && (
            <div className="space-y-2">
              {clusters.map((cluster) => (
                <div key={cluster.id} className="p-3 rounded-xl border border-slate-200/30 dark:border-navy-700/30" style={{ borderLeftColor: cluster.color, borderLeftWidth: 3 }}>
                  <div className="flex items-center gap-2 mb-1.5">
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: cluster.color }} />
                    <span className="text-[11px] font-bold text-slate-700 dark:text-slate-200">{cluster.name}</span>
                    <span className="text-[9px] text-slate-400">({cluster.nodeIds.length} nodes)</span>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {cluster.nodeIds.map((nid) => {
                      const node = nodes.find((n) => n.id === nid);
                      return (
                        <span key={nid} className="text-[9px] px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-navy-800 text-slate-600 dark:text-slate-400">
                          {node?.data?.label || nid}
                        </span>
                      );
                    })}
              ...
              <div className="flex items-center gap-2 pt-2">
                <button onClick={detectClusters} disabled={loading} className="inline-flex items-center gap-1 text-[10px] text-slate-500 hover:text-slate-700 transition-colors">
                  <RefreshCw size={10} /> {isPl ? 'Ponownie' : 'Re-run'}
                </button>
                <div className="flex-1" />
                <button onClick={handleApply} disabled={locked} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold bg-cyan-500/10 text-cyan-700 dark:text-cyan-300 hover:bg-cyan-500/20 transition-colors disabled:opacity-40">
