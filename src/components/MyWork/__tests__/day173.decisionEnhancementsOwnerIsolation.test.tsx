...
vi.mock('@/hooks/useCloudIntegrations', () => ({
  useCloudIntegrations: () => ({
    connectedProviderIds: [], openFilePicker: vi.fn(), isPickerOpen: false,
    activeProvider: null, closeFilePicker: vi.fn(), selectFile: vi.fn(), isImplemented: false,
  }),
}));
vi.mock('@/hooks/useOpenChatWithContext', () => ({ useOpenChatWithContext: () => vi.fn() }));
vi.mock('@/hooks/usePresentationMode', () => ({ usePresentationMode: () => ({ mode: 'n', setMode: vi.fn() }) }));
...
vi.mock('@/store/useAppStore', () => ({
  useAppStore: () => ({
    isChatCollapsed: false, toggleChatCollapse: vi.fn(), setChatKickoffMessage: vi.fn(),
    currentProjectId: 'project-1', emitMyWorkEvent: vi.fn(), currentUser: session.user,
  }),
}));
...
vi.mock('@/components/shared/NModeLayout/NModeCardManager', () => ({ SectionsManagerMenu: () => null }));
vi.mock('@/components/shared/RequiredProjectPicker', () => ({ RequiredProjectPicker: () => null }));
vi.mock('@/components/shared/CapabilityGate', () => ({
  CapabilityGate: ({ children }: { children: React.ReactNode }) => React.createElement(React.Fragment, null, children),
}));
vi.mock('../../../services/api', () => ({
  Api: new Proxy({}, {
    get: (_target, property) => {
      if (property === 'get') return vi.fn(async (path: string) =>
        path.includes('/detail')
          ? { data: { id: 'decision-1', title: 'Owner isolation', status: 'pending', projectId: 'project-1' } }
...
describe('day173 decision enhancement owner isolation', () => {
  ...
  it('does not claim or delete an owner-unknown legacy entry when another user opens the decision', async () => {
    ...
    const legacyValue = JSON.stringify({ schemaVersion: 1, savedAt: '2026-08-30T10:00:00.000Z', description: 'user A note' });
    ...
    const userBValue = window.localStorage.getItem('consultify-decision-enhancements:org-1:user-b:decision-1');
