...
export const BottleneckAlerts: React.FC<
  Partial<BottleneckAlertsProps> & { className?: string }
> = ({ bottlenecks: externalBottlenecks, onBottleneckClick, maxVisible = 5, className = '' }) => {
  const { t } = useTranslation();
  const { bottlenecks: hookBottlenecks, loading } = useExecutionScore({
    autoLoad: !externalBottlenecks,
  });
