/**
 * ideaMapToMarkdown — pure serializer that turns an idea-map graph
 * ({nodes, edges}) into a readable, hierarchical markdown outline.
 *
 * Used by the "Omów z Teresą / Discuss with Teresa" action so the chat
 * (Teresa) receives a human-readable structure of the map that it can then
 * turn into a document / initiative via the deliverables intercepts.
 *
 * Design contract:
 * - Node shape (loose): { id, data?: { label?, text? }, label?, text? }
 * - Edge shape (loose): { source|sourceId, target|targetId }
 * - Roots = nodes with no incoming edge. Children nested by outgoing edges.
 * - BFS from roots; cycles and already-visited nodes are not re-expanded.
 * - Orphan nodes (unreachable / part of cycles never rooted) are appended
 *   under a flat "Pozostałe / Other" section so nothing is silently dropped.
 * - Output is capped (~8KB) to keep the chat kickoff message reasonable.
 */
