  ...
  return (
    <div className="fixed inset-0 z-[200]" onClick={onClose}>
      <div
        ...
        className="w-[340px] rounded-2xl border border-slate-200 dark:border-navy-700 bg-white dark:bg-navy-950 shadow-2xl overflow-hidden"
        style={style}
      >
        {/* Header */}
        <div className="px-4 py-2.5 border-b border-slate-200/60 dark:border-navy-700/60 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: typeColor }} />
          <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex-1">{column.header}</span>
          <span className="text-[8px] font-bold uppercase tracking-wider text-slate-400 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-navy-800">{column.type}</span>
          <button onClick={onClose} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-navy-800 transition-colors">
        ...
        <div className="p-4 max-h-[300px] overflow-auto">
          ...
          {(column.type === 'text') && (
            <div>
              ...
              <p className="text-[9px] text-slate-400 mt-1.5">
                ...
                {isPl ? 'Obsługuje **pogrubienie**, *kursywę*, - listy' : 'Supports **bold**, *italic*, - lists'}
              </p>
            </div>
          )}
            ...
            <div>
              <div className="space-y-1 mb-3">
                {(column.options || []).map((opt, idx) => {
                  const optColor = column.optionColors?.[opt] || ROW_ACCENT_COLORS[idx % ROW_ACCENT_COLORS.length];
                  const isSelected = column.type === 'multiselect'
                    ? (Array.isArray(value) ? value.includes(opt) : false)
                    : value === opt;
                  return (
                    <div key={opt} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-navy-800">
                      <button
                        onClick={() => {
                          if (locked) return;
                        ...
                        className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 border transition-colors ${
                          isSelected ? 'border-transparent' : 'border-slate-300 dark:border-navy-600'
                        }`}
                        style={isSelected ? { backgroundColor: optColor } : {}}
                      >
                        {isSelected && <Check size={10} className="text-white" />}
                      </button>
                      <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: optColor }} />
                      <span className="text-[11px] text-slate-700 dark:text-slate-300 flex-1">{opt}</span>
                      {!locked && (
                        <button onClick={() => handleRemoveOption(opt)} className="p-0.5 rounded text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all">
                          <Trash2 size={10} />
                        </button>
                      )}
                    </div>
              ...
              {!locked && (
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <button onClick={handleAddOption} disabled={!newOption.trim()} className="p-1.5 rounded-lg bg-violet-500/10 text-violet-600 dark:text-violet-400 hover:bg-violet-500/20 disabled:opacity-40 transition-colors">
                    <Plus size={12} />
                  </button>
                </div>
              )}
            ...
            <div>
              <div className="rounded-xl border border-violet-500/20 bg-violet-500/5 p-3 mb-3">
                ...
                <textarea
                  ...
                  disabled={locked}
                  rows={3}
                  className="w-full bg-transparent border-0 outline-none text-[11px] text-slate-700 dark:text-slate-300 resize-none leading-relaxed"
                  placeholder={isPl ? 'Opisz co AI ma wygenerować...' : 'Describe what AI should generate...'}
                />
              </div>
              <div className="rounded-xl bg-slate-50 dark:bg-navy-900 p-3 mb-3">
                ...
                <p className="text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed">{String(value || (isPl ? 'Brak wyniku' : 'No result'))}</p>
              </div>
              {!locked && onAIRegenerate && (
                ...
                >
                  ...
                  {aiLoading ? (isPl ? 'Generuję...' : 'Generating...') : (isPl ? 'Regeneruj' : 'Regenerate')}
                </button>
              )}
            </div>
          )}
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={`text-2xl transition-transform hover:scale-110 ${
                    star <= (Number(value) || 0) ? 'text-amber-400' : 'text-slate-300 dark:text-navy-600'
                  }`}
                >
                  ★
                </button>
              ))}
              <span className="text-xs text-slate-500 ml-2">{value || 0}/5</span>
            </div>
          )}
          ...
          {column.type === 'progress' && (
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                />
                <span className="text-sm font-bold text-slate-700 dark:text-slate-300 w-10 text-right">{value || 0}%</span>
              </div>
              <div className="h-2 rounded-full bg-slate-200 dark:bg-navy-700 overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-indigo-500 transition-all" style={{ width: `${Number(value) || 0}%` }} />
              </div>
            </div>
          )}
          ...
          {column.type === 'url' && (
            <div>
              ...
              />
              {value && (
                <a href={String(value)} target="_blank" rel="noopener noreferrer" className="text-[9px] text-blue-500 hover:underline mt-1 block">
          ...
          {/* Fallback */}
          {!['text', 'select', 'multiselect', 'ai_generated', 'number', 'currency', 'rating', 'progress', 'date', 'url'].includes(column.type) && (
            <input
              value={String(value || '')}
              onChange={(e) => onChange(e.target.value)}
              disabled={locked}
        ...
        <div className="px-4 py-2.5 border-t border-slate-200/60 dark:border-navy-700/60">
          ...
          >
            ...
            <ChevronDown size={9} className={`transition-transform ${showNote ? 'rotate-180' : ''}`} />
          </button>
          {showNote && (
            <div className="mt-1.5">
              <textarea
                ...
                className="w-full bg-amber-50/50 dark:bg-amber-900/10 border border-amber-200/40 dark:border-amber-700/20 rounded-lg px-2.5 py-1.5 text-[10px] text-slate-700 dark:text-slate-300 outline-none resize-none"
              />
            </div>
          )}
        </div>
