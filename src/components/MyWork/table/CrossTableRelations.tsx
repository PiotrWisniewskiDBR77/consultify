...
const RELATION_TYPES: { id: CrossRelation['relationType']; label: string; labelPl: string; color: string }[] = [
...
export const CrossTableRelations: React.FC<CrossTableRelationsProps> = ({
    ...
    (async () => {
      ...
      try {
        ...
        if (Array.isArray(result?.maps)) {
          ...
          setExternalMaps([
            { id: 'demo-1', title: isPl ? 'Strategia cyfrowa' : 'Digital Strategy', nodeCount: 12, updatedAt: new Date().toISOString() },
            { id: 'demo-2', title: isPl ? 'Optymalizacja procesów' : 'Process Optimization', nodeCount: 8, updatedAt: new Date().toISOString() },
            { id: 'demo-3', title: isPl ? 'Innowacje produktowe' : 'Product Innovation', nodeCount: 15, updatedAt: new Date().toISOString() },
          ]);
        }
      } catch {
        setExternalMaps([]);
  ...
  const handleSelectMap = useCallback(async (map: ExternalIdeaMap) => {
    setSelectedMap(map);
    setLoading(true);
    try {
      const { Api } = await import('@/services/api');
  ...
  const handleCreateLink = useCallback((targetNodeId: string) => {
  ...
  const existingRelations = useMemo(() => {
    return currentEdges.filter((e) => e.data?.crossTable).map((e) => ({
      id: e.id,
      sourceIdeaId: ideaId,
  ...
  const filteredExternalNodes = searchQuery
    ? externalNodes.filter((n) => (n.data?.label || '').toLowerCase().includes(searchQuery.toLowerCase()))
    : externalNodes;
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/20 backdrop-blur-[2px]" onClick={onClose}>
      <div className="w-[600px] max-w-[95vw] max-h-[85vh] rounded-2xl border border-slate-200 dark:border-navy-700 bg-white dark:bg-navy-950 shadow-2xl overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        ...
        <div className="flex items-center gap-1 px-5 py-2 border-b border-slate-200/30 dark:border-white/[0.04]">
          {(['relations', 'network', 'rollups'] as const).map((tab) => (
            ...
            >
              {tab === 'relations' ? (isPl ? 'Relacje' : 'Relations') : tab === 'network' ? (isPl ? 'Sieć' : 'Network') : 'Rollups'}
                  ...
                  <div className="space-y-1">
                    {externalMaps.map((map) => (
                      ...
                      >
                        ...
                        <div className="text-[8px] text-slate-400">{map.nodeCount} {isPl ? 'elementów' : 'items'}</div>
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="flex-1 p-3 overflow-auto">
                {!selectedMap ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-xs text-slate-400">{isPl ? 'Wybierz mapę aby zobaczyć elementy' : 'Select a map to see items'}</p>
                  </div>
                ) : (
                  <>
                      ...
                      {linkSource && (
                        ...
                        >
                          {RELATION_TYPES.map((rt) => (
                            <option key={rt.id} value={rt.id}>{isPl ? rt.labelPl : rt.label}</option>
                          ))}
                        </select>
                      )}
                    </div>
                    ...
                    {!linkSource && (
                      <div className="mb-3 p-2 rounded-xl bg-amber-500/5 border border-amber-500/20">
                        <p className="text-[9px] font-bold text-amber-600 mb-1.5">{isPl ? 'Wybierz źródło z bieżącej tabeli:' : 'Select source from current table:'}</p>
                    ...
                    {linkSource && (
                      <div className="mb-2 flex items-center gap-1 text-[9px] text-violet-600">
                        <span className="font-bold">{currentNodes.find((n) => n.id === linkSource)?.data?.label || linkSource}</span>
                        <ArrowRight size={9} />
                        <span className="text-slate-400">{isPl ? 'wybierz cel…' : 'select target…'}</span>
                        <button onClick={() => setLinkSource(null)} className="ml-auto text-slate-400 hover:text-slate-600"><X size={9} /></button>
                      </div>
                    )}

                    <div className="space-y-1">
                      {filteredExternalNodes.map((node, idx) => {
                        const color = node.data?.color || ROW_ACCENT_COLORS[idx % ROW_ACCENT_COLORS.length];
                        return (
                          <div key={node.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-navy-800/50 transition-colors">
                            <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />
                            <span className="text-[10px] font-medium text-slate-700 dark:text-slate-300 flex-1 truncate">{node.data?.label || node.id}</span>
                            {linkSource && (
                              <button
          ...
          {activeTab === 'network' && (
            <div className="p-5">
              <div className="text-center mb-4">
                ...
                <p className="text-xs font-bold text-slate-700 dark:text-slate-300">{isPl ? 'Sieć pomysłów' : 'Idea Network'}</p>
                <p className="text-[10px] text-slate-400">{allRelations.length} {isPl ? 'połączeń' : 'connections'}</p>
              </div>
              {allRelations.length === 0 ? (
                <p className="text-xs text-slate-400 text-center">{isPl ? 'Brak relacji — utwórz pierwszą w zakładce Relacje' : 'No relations — create one in the Relations tab'}</p>
              ) : (
                <div className="space-y-1.5">
                  {allRelations.map((rel) => {
                    const relType = RELATION_TYPES.find((r) => r.id === rel.relationType);
                    return (
                      <div key={rel.id} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-50/80 dark:bg-navy-900/50">
                        <span className="text-[10px] font-medium text-slate-700 dark:text-slate-300 truncate flex-1">{rel.sourceLabel}</span>
                        <span className="text-[8px] font-bold px-1.5 py-0.5 rounded-md" style={{ backgroundColor: `${relType?.color || '#6366f1'}15`, color: relType?.color }}>
                          {isPl ? relType?.labelPl : relType?.label}
                        </span>
                        <span className="text-[10px] font-medium text-slate-700 dark:text-slate-300 truncate flex-1 text-right">{rel.targetLabel}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          {activeTab === 'rollups' && (
            <div className="p-5">
              <div className="text-center py-6">
                ...
                <p className="text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">{isPl ? 'Rollup agregacje' : 'Rollup Aggregations'}</p>
                <p className="text-[10px] text-slate-400 mb-4">{isPl ? 'Agreguj dane z powiązanych tabel' : 'Aggregate data from related tables'}</p>
                {onAddRollupColumn && (
                  <button
                    onClick={() => onAddRollupColumn({ sourceIdeaId: '', sourceField: '', aggregation: 'count', label: 'Cross-table count' })}
