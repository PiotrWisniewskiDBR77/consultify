...
function TypeBadge({ type }: { type: string }) {
  ...
  return (
    <span className={`shrink-0 px-2 py-0.5 rounded text-[10px] font-medium whitespace-nowrap ${cls}`}>
      {label}
    </span>
  );
}
        ...
        <div className="absolute right-0 top-8 z-50 min-w-[180px] py-1 rounded-lg border border-slate-200 dark:border-navy-700 bg-white dark:bg-navy-900 shadow-lg">
          {actions.map((a) => (
            <button
              key={a.id}
              type="button"
              onClick={(e) => { e.stopPropagation(); onAction(a); setOpen(false); }}
              className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-navy-800 transition-colors ${
                a.variant === 'danger' ? 'text-rose-600 dark:text-rose-400' :
                a.variant === 'primary' ? 'text-cyan-600 dark:text-cyan-400 font-medium' :
  ...
  return (
    <div className="flex flex-col h-full" onKeyDown={handleKeyDown} tabIndex={0}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!loading && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center h-32 text-slate-400 dark:text-slate-500">
            ...
            <p className="text-xs">{emptyMessage || t('manager.noProblems', 'No problems detected — on track.')}</p>
          </div>
        )}
          ...
          return (
            ...
            >
              ...
              <div className="px-2 py-2 min-w-0">
                <p className="text-xs font-medium text-slate-900 dark:text-white truncate">{row.title}</p>
                <p className="text-[10px] text-slate-400 dark:text-slate-500 truncate mt-0.5">{row.rootCause}</p>
              </div>

              {/* Type */}
              <div className="px-2"><TypeBadge type={row.problemType} /></div>
      ...
      <div className="flex items-center justify-between px-3 py-1.5 text-[10px] text-slate-400 dark:text-slate-500 border-t border-slate-200 dark:border-navy-700 bg-slate-50 dark:bg-navy-800/50">
        <span>{filtered.length} / {rows.length} {t('manager.problems', 'problems')}</span>
