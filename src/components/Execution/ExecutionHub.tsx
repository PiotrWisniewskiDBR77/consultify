...
export const ExecutionHub: React.FC<ExecutionHubProps> = ({ initialTab = 'list' }) => {
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      await Api.patch(`/initiatives/${draggableId}`, { status: newStatus });
      ...
      setInitiatives((prev) =>
        prev.map((i) => (i.id === draggableId ? { ...i, status: initiative.status } : i))
      );
    }
  };
                ...
                <Droppable droppableId={column.id}>
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`flex-1 space-y-2 overflow-y-auto p-1 rounded-lg transition-colors ${
                          ...
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`p-3 bg-navy-800 border border-navy-700 rounded-lg cursor-pointer transition-all ${
                                snapshot.isDragging
                                  ? 'shadow-xl border-cyan-500 rotate-2'
