...
describe('FEEDBACK-1/1e — UI language and persisted-data boundary', () => {
  it.each([
    ...
    async ({ language, statusKey }) => {
      ...
      // W151: the title must stay on FilterableTable's native primary-cell path.
      // That path owns the one-line ellipsis and overflow tooltip; using column.render
      // moved it into CELL_ELEMENT_CLAMP_CLASS and increased the row from 56.5 to 72.5 px.
      const titleCell = screen.getByText(task.title).closest('td');
      expect(titleCell).not.toBeNull();
      expect(titleCell).toHaveAttribute('data-language-source', 'user-content');
      expect(titleCell).toHaveAttribute('translate', 'no');
      expect(titleCell).toHaveClass('h-14', 'py-3');
      expect(titleCell?.querySelector('[data-overflow-tooltip-trigger]')).not.toBeNull();
      expect(titleCell?.querySelector('[data-overflow-tooltip-text]')).toHaveClass(
        'text-sm',
        'block',
        'truncate'
      );
      expect(titleCell?.querySelector('.line-clamp-2')).toBeNull();
