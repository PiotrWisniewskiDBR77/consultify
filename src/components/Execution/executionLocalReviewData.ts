...
export const executionLocalReviewEnabled = import.meta.env.DEV && import.meta.env.MODE !== 'test';
...
export const executionReviewRoleLabel = (
  value: string,
  t: (key: string, fallback: string) => string
): string | null => {
  if (value === 'execution-manager')
    return t('execution.review.roles.executionManager', 'Execution Manager');
  if (value === 'controls-engineer')
    return t('execution.review.roles.controlsEngineer', 'Controls Engineer');
  return null;
};
...
const milestonesByCase: Record<string, any> = {
  'review-exec-supply-chain': {
    items: [
      {
        milestoneId: 'milestone-data-ready',
        version: 2,
        executionCaseId: 'review-exec-supply-chain',
        initiativeId: 'demo-init-supply-chain',
        baselineRef: { ref: 'handoff-supply-chain-v2', version: 2 },
        title: 'Dane dostawców gotowe',
        ownerId: 'anna-kowalska',
        targetAt: '2026-08-28T16:00:00.000Z',
        forecastAt: '2026-08-30T16:00:00.000Z',
        status: 'AT_RISK',
      ...
      {
        milestoneId: 'milestone-pilot-ready',
        version: 1,
        executionCaseId: 'review-exec-supply-chain',
        initiativeId: 'demo-init-supply-chain',
        baselineRef: { ref: 'handoff-supply-chain-v2', version: 2 },
        title: 'Pilotaż logistyczny gotowy',
        ownerId: 'omar-haddad',
        targetAt: '2026-09-08T16:00:00.000Z',
        forecastAt: '2026-09-12T16:00:00.000Z',
        status: 'AT_RISK',
        readiness: 'BLOCKED',
        forecastVarianceDays: 4,
        evidenceRefs: [],
        sourceVersions: { executionCaseVersion: 4, baselineVersion: 2 },
      },
    ],
  ...
  'review-exec-ai-copilot': {
    items: [
      {
        milestoneId: 'milestone-copilot-go-live',
        version: 1,
        executionCaseId: 'review-exec-ai-copilot',
        initiativeId: 'demo-init-ai-copilot',
        baselineRef: { ref: 'handoff-ai-copilot-v1', version: 1 },
        title: 'Pilotaż AI Copilot uruchomiony',
        ownerId: 'lena-meyer',
        targetAt: '2026-09-10T16:00:00.000Z',
        forecastAt: '2026-09-10T16:00:00.000Z',
        status: 'READY',
        readiness: 'READY',
...
const allocationsByCase: Record<string, any> = {
  'review-exec-supply-chain': {
    items: [
      ...
      {
        version: 1,
        allocationId: 'alloc-marek-demand',
        status: 'REQUESTED',
        assigneeId: 'marek-nowak',
        assigneeName: 'Marek Nowak',
  ...
  'review-exec-ai-copilot': {
    items: [
      {
        version: 1,
        allocationId: 'alloc-katarzyna-guardrails',
        status: 'ASSIGNEE_ACCEPTED',
        assigneeId: 'katarzyna-wojcik',
        assigneeName: 'Katarzyna Wójcik',
        teamName: 'AI Governance',
        taskId: 'task-copilot-guardrails',
        initiativeId: 'demo-init-ai-copilot',
        timeBasis: { window: '24–30 sie 2026', windowUnit: 'WEEK', timezone: 'Europe/Warsaw' },
        availability: { knowledgeState: 'KNOWN', value: 24 },
        demand: { knowledgeState: 'KNOWN', value: 20 },
        remainingDemand: { knowledgeState: 'KNOWN', value: 4 },
        load: { low: 0.8, high: 0.9 },
        skillMatch: { state: 'MATCH', label: 'Eksperckie' },
        cost: { knowledgeState: 'KNOWN', value: '7 200 PLN' },
        conflict: { state: 'NONE' },
        freshness: 'CURRENT',
        nextAction: 'Potwierdź przydział',
      },
      {
        version: 1,
        allocationId: 'alloc-unassigned-pilot',
        status: 'PROPOSED',
        roleName: 'Analityk procesu zakupowego',
        teamName: 'Procurement',
        taskId: 'task-user-pilot',
        initiativeId: 'demo-init-ai-copilot',
        timeBasis: { window: '31 sie–6 wrz 2026', windowUnit: 'WEEK', timezone: 'Europe/Warsaw' },
        availability: { knowledgeState: 'UNKNOWN' },
        demand: { knowledgeState: 'KNOWN', value: 16 },
        remainingDemand: { knowledgeState: 'UNKNOWN' },
        load: { knowledgeState: 'UNKNOWN' },
        skillMatch: { state: 'UNKNOWN', label: 'Do potwierdzenia' },
        cost: { knowledgeState: 'UNKNOWN' },
        conflict: { state: 'NOT_ASSESSED' },
        freshness: 'UNKNOWN',
        nextAction: 'Wskaż osobę i potwierdź dostępność',
      },
    ],
  },
};
...
export const executionReviewSignals = [
  {
    signalId: 'signal-demand-model-delay',
    version: 2,
    ruleId: 'STALE_MILESTONE',
    projectId: 'demo-init-ai-copilot',
    sourceType: 'MILESTONE',
    sourceId: 'milestone-pilot-ready',
    sourceVersionKey: 'milestoneVersion',
    sourceVersion: 1,
    sourceVersions: { milestoneVersion: 1 },
    snapshotRef: 'milestone-pilot-ready@1',
    fingerprint: 'stale-milestone:pilot-ready',
    severity: 'CRITICAL',
    occurrences: [
      { evidenceRef: 'evidence-demand-model-blocker', occurredAt: '2026-08-23T08:00:00.000Z' },
      { evidenceRef: 'evidence-demand-model-reminder', occurredAt: '2026-08-23T09:00:00.000Z' },
      { evidenceRef: 'evidence-demand-model-escalation', occurredAt: '2026-08-23T10:30:00.000Z' },
    ],
    updatedAt: '2026-08-23T10:30:00.000Z',
  },
  {
    signalId: 'signal-marek-overload',
    version: 1,
    ruleId: 'CAPACITY_CONFLICT',
    projectId: 'demo-init-supply-chain',
    sourceType: 'OPERATIONAL_ALLOCATION',
    sourceId: 'alloc-marek-demand',
    sourceVersionKey: 'allocationVersion',
    sourceVersion: 1,
    sourceVersions: { allocationVersion: 1 },
    snapshotRef: 'alloc-marek-demand@1',
    fingerprint: 'capacity-conflict:marek-nowak',
    severity: 'WARNING',
    occurrences: [
      { evidenceRef: 'evidence-capacity-baseline', occurredAt: '2026-08-23T09:15:00.000Z' },
    ],
...
export const executionReviewInterventions = [
  {
    interventionId: 'intervention-demand-recovery',
    version: 3,
    title: 'Plan naprawczy modelu prognozowania',
    status: 'PENDING_DECISION',
    ownerId: 'omar-haddad',
    authorityId: 'piotr-wisniewski',
    slaAt: '2026-08-25T12:00:00.000Z',
    verifyBy: '2026-08-30T12:00:00.000Z',
    ...
    counterEvidenceRefs: [],
    unknowns: ['Czy dostawca danych usunie rozbieżność do 25 sierpnia?'],
    ...
    options: [
      {
        optionId: 'do-nothing',
        kind: 'DO_NOTHING',
        label: 'Nie zmieniaj planu',
        impacts: [
          { targetRef: 'milestone-pilot-ready', effect: 'Ryzyko opóźnienia pilotażu o 4–7 dni' },
        ],
        confidence: 'HIGH',
        reversibility: 'REVERSIBLE',
      },
      {
        optionId: 'parallel-validation',
        kind: 'ACTION',
        label: 'Równoległa walidacja obu źródeł',
        impacts: [
          { targetRef: 'task-demand-model', effect: 'Dodatkowe 10 h pracy' },
          { targetRef: 'milestone-pilot-ready', effect: 'Skrócenie opóźnienia o 3 dni' },
        ],
        confidence: 'MEDIUM',
  ...
  {
    interventionId: 'intervention-capacity-rebalance',
    version: 2,
    title: 'Odciążenie Marka w tygodniu 35',
    status: 'DRAFT',
    ownerId: 'lena-meyer',
    authorityId: 'omar-haddad',
    slaAt: '2026-08-26T12:00:00.000Z',
    verifyBy: '2026-09-02T12:00:00.000Z',
    hypotheses: ['Przeniesienie walidacji danych uwolni 10 h krytycznej pojemności'],
    evidenceRefs: ['signal-marek-overload@1'],
    counterEvidenceRefs: [],
    unknowns: ['Dostępność zastępstwa'],
    blastRadiusRefs: ['alloc-marek-demand'],
    options: [],
...
export const executionReviewReportDefinitions = [
  {
    definitionId: 'weekly-execution-pack',
    version: 4,
    currentVersion: 2,
    ...
    versions: [
      {
        ...
        scope: {
          type: 'EXECUTION',
          refs: executionReviewCases.map((item) => item.executionCaseId),
          projectIds: ['atelier-transformation'],
          generalBacklogAllowed: false,
        },
        ownerId: 'omar-haddad',
        approverId: 'piotr-wisniewski',
        access: { classification: 'INTERNAL', audienceRoles: ['SPONSOR', 'PMO'] },
        redaction: { defaultState: 'REDACTED', rules: ['personal-cost'] },
  ...
  {
    definitionId: 'capacity-utilization',
    version: 2,
    currentVersion: 1,
...
export const executionReviewReportRuns = [
  {
    version: 3,
    reportRunId: 'run-weekly-2026-w34',
    status: 'APPROVED',
    definitionRef: { definitionId: 'weekly-execution-pack', version: 2 },
    parentRunRef: null,
    audience: ['PMO', 'Sponsorzy'],
    scopeRefs: executionReviewCases.map((item) => item.executionCaseId),
    period: { start: '2026-08-17', end: '2026-08-23' },
    asOf: '2026-08-23T11:00:00.000Z',
    contentHash: 'review-weekly-w34-v3',
    sources: executionReviewCases.map((item) => ({
      sourceType: 'execution_case',
      sourceId: item.executionCaseId,
      version: item.version,
      capturedAt: '2026-08-23T11:00:00.000Z',
      freshness: 'CURRENT',
      accessState: 'REDACTED',
      confidence: 'HIGH',
      redactions: ['personal-cost'],
    })),
  },
  {
    version: 1,
    reportRunId: 'run-capacity-2026-08',
    status: 'FROZEN',
    definitionRef: { definitionId: 'capacity-utilization', version: 1 },
    parentRunRef: null,
    audience: ['PMO', 'Liderzy zespołów'],
    scopeRefs: executionReviewCases.map((item) => item.executionCaseId),
    period: { start: '2026-08-01', end: '2026-08-31' },
    asOf: '2026-08-23T10:00:00.000Z',
    contentHash: 'review-capacity-aug-v1',
    sources: [
      {
        sourceType: 'operational_allocation',
        sourceId: 'alloc-marek-demand',
...
export const getExecutionReviewCase = (caseId: string) => {
  ...
  return {
    ...
    detail: {
      ...
      state: item.state,
      handoffPackageId: item.handoffPackageId,
      handoffPackageVersion: item.handoffPackageVersion,
    },
  };
};
export const getExecutionReviewWork = (caseId: string) =>
  workByCase[caseId] ?? { tasks: [], decisions: [] };
export const getExecutionReviewMilestones = (caseId: string) =>
  milestonesByCase[caseId] ?? { items: [] };
export const getExecutionReviewAllocations = (caseId: string) =>
  allocationsByCase[caseId] ?? { items: [] };
