...
function toDisplay(value: number | null) {
  const status: FinanceValueStatus =
    value === null ? 'MISSING' : value === 0 ? 'PRESENT_ZERO' : 'PRESENT_NONZERO';
  return formatFinanceValueForDisplay({
    status,
    valueDecimal: value === null ? null : String(value),
  });
}
...
const STATUS_LABEL: Record<MaterialStatus, string> = {
  ...
  provisional: 'Tymczasowy (Provisional)',
};
...
export function ScenarioResultsView({
  ...
  baselineValues,
  exceptionLedger,
}: ScenarioResultsViewProps): React.ReactElement {
  ...
  if (ebitda !== null && netDebt !== null) {
    try {
      ...
      covenantError =
        err instanceof MathUndefinedError ? err.message : 'Błąd obliczenia covenant headroom.';
    }
  }
  ...
  return (
    <div
      ...
      data-testid="prediction-results-view"
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          {freshness === 'CURRENT'
            ? 'Aktualne'
            : freshness === 'STALE'
              ? 'Nieaktualne — zmieniono założenia od ostatniego przeliczenia'
              : 'Nie przeliczono'}
        </span>
        <span
          className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium ${STATUS_TONE[gate.materialStatus]}`}
        ...
        {!gate.allowed && (
          <span
            className="inline-flex items-center rounded-full border border-c-danger/30 bg-c-danger/10 px-2.5 py-1 text-xs font-medium text-c-danger"
            role="alert"
            data-testid="compute-blocked-badge"
          >
      ...
      <section className="rounded-xl border border-c-border-subtle bg-c-surface p-4">
        <h3 className="mb-2 text-sm font-semibold text-c-text">
          Porównanie ze scenariuszem bazowym — absolutne / Δ / %
        </h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            {/* §27-exempt — scenariusz-vs-baseline porównanie linii P&L/BS/CF (Excel/Platforma-tabel archetyp, DOKTRYNA_TABELA_NIE_EXCEL.md Decyzja 07-13), nie lista rekordów: kolumny to Δ/%, nie akcje na wierszu. */}
            ...
            <tbody>
              {comparison.map((cell) => (
                <tr
                  key={`${cell.lineCode}::${cell.periodId}`}
                ...
                >
                  ...
                  <td className="px-3 py-2 text-right tabular-nums">
                    {toDisplay(cell.scenarioValue).text}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums">
                    {toDisplay(cell.baselineValue).text}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums">
                    {toDisplay(cell.absoluteDelta).text}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums">
                    {cell.percentDelta === null ? '—' : `${(cell.percentDelta * 100).toFixed(1)}%`}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="rounded-xl border border-c-border-subtle bg-c-surface p-4">
          ...
          {liquidity === null || liquidity.liquidityHeadroom === null ? (
            <p className="text-sm text-c-text-muted">
              Brak danych o gotówce lub polityce min. cash — wyjątek, nie blokada.
            </p>
          ) : (
            <p className="text-lg tabular-nums text-c-text">
              {liquidity.liquidityHeadroom.toLocaleString('pl-PL')} PLN
            </p>
          )}
        </div>
        <div className="rounded-xl border border-c-border-subtle bg-c-surface p-4">
          <h3 className="mb-1 text-sm font-semibold text-c-text">
            Covenant headroom (Net Debt / EBITDA ≤ 3.5x)
          </h3>
          {covenantError ? (
            <p
              className="text-sm font-medium text-c-danger"
              role="alert"
              data-testid="covenant-hard-block"
            >
            ...
            <p className="text-lg tabular-nums text-c-text">
              {covenant.netDebtToEbitda.toFixed(2)}x{' '}
              <span className="text-sm text-c-text-secondary">
                (zapas {covenant.headroomRatio.toFixed(2)}x)
              </span>
            </p>
          )}
        </div>
          ...
          <ul className="space-y-1 text-sm text-c-text-secondary">
            {exceptionLedger.map((e) => (
              <li key={e.id} data-testid={`exception-${e.id}`}>
                <span className="font-medium text-c-text">[{EXCEPTION_LEVEL_LABEL[e.level]}]</span>{' '}
                {e.message}
              </li>
            ))}
          </ul>
      ...
      <p className="text-xs text-c-text-muted">
        Autor: system@finance-v3 · Jakość: {STATUS_LABEL[gate.materialStatus]}
      </p>
    </div>
  );
}
