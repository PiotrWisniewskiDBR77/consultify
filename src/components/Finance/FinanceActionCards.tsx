...
export function FinanceActionCards() {
  ...
  return <section aria-label={ft('finance.actionCards.title', 'Action cards — Finance')} className="space-y-3 p-4">{cards.map((card) => <ActionCard key={card.id} card={card} />)}</section>;
}
