...
export interface FinanceLineageNavigatorProps {
  ...
  onNavigate?: (versionId: string, artifactType: FinanceLineageNavigatorDto['relatedPanel']['focus']['artifactType']) => void;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1">
        <StatusChip label={businessVersionStatusLabel(metadata.status)} tone="neutral" size="sm" hideDot />
        {metadata.periodLabel ? <span className="text-[11px] text-c-text-secondary">{metadata.periodLabel}</span> : null}
    ...
    <div className="flex flex-col gap-1.5" data-testid={`lineage-related-section-${title}`}>
      ...
      {groups.length === 0 ? (
        ...
        <ul className="flex flex-col gap-1">
          {groups.map((group) => (
            <li key={group.artifactType} className="flex items-center justify-between rounded-md border border-c-border-subtle px-2 py-1">
              <span className="text-xs text-c-text-primary">{financeArtifactTypeLabel(group.artifactType)}</span>
    ...
    <div className={`flex flex-col gap-4 rounded-lg border border-c-border-subtle bg-c-surface p-3 ${className ?? ''}`} data-testid="finance-lineage-navigator">
      ...
      <div className="flex flex-col gap-3" data-testid="lineage-related-panel">
        ...
        <div className="grid grid-cols-2 gap-3">
          <RelatedGroupSection title="Rodzice" groups={relatedPanel.parents} emptyLabel="Brak bezpośrednich rodziców." />
          <RelatedGroupSection title="Dzieci" groups={relatedPanel.children} emptyLabel="Brak bezpośrednich dzieci." />
          <RelatedGroupSection title="Przodkowie pośredni" groups={relatedPanel.indirectAncestors} emptyLabel="Brak." />
          <RelatedGroupSection title="Potomkowie pośredni" groups={relatedPanel.indirectDescendants} emptyLabel="Brak." />
        </div>
        {relatedPanel.siblings.length > 0 ? (
          <div>
            <p className="mb-1 text-xs font-semibold text-c-text-secondary">Inne wersje/warianty</p>
            <ul className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1.5" data-testid="lineage-create-new">
          {relatedPanel.createNew.length > 0 ? (
            ...
            <p className="text-xs text-c-text-secondary">{relatedPanel.createNewBlockedLabel.pl}</p>
          ) : null}
        </div>
