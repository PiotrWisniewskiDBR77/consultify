...
describe('OrganizationIdentityOperatingScreen', () => {
  ...
  it('scala dawne dwa ekrany Profilu w cztery sekcje jednego ekranu', async () => {
    ...
    expect(screen.getByText('Markets & core systems')).toBeInTheDocument();
  });
  ...
  it('wypełnia pola REALNYMI danymi profilu i liczy chipy z tych samych pól', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText('Sub-Industry')).toHaveValue(
        'Doradztwo strategiczne i operacyjne'
      )
    );
    expect(screen.getByLabelText('Industry')).toHaveValue('Professional Services');
    expect(screen.getByLabelText('Employee Count')).toHaveValue(120);
    expect(screen.getByLabelText('Primary Markets')).toHaveValue('Polska, DACH');

    // Konflikty pochodzą z /organization-context i są mapowane po claimPath.
    await waitFor(() => expect(screen.getByTestId('chip-conflicts')).toHaveTextContent('Conflicts:2'));
    expect(screen.getByTestId('chip-all')).toHaveTextContent('All:16');
  });
  ...
  it('jeden „Zapisz zmiany" zapisuje komplet pól i weryfikuje odczyt zwrotny', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Industry Code'), { target: { value: '70.22.Z' } });
