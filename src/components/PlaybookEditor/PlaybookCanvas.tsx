...
import { PlaybookEdge, PlaybookNode as PlaybookNodeType, PlaybookNodeType as NodeType } from '../../types';
...
export const PlaybookCanvas: React.FC<PlaybookCanvasProps> = ({
    ...
    const renderEdges = () => {
        return edges.map((edge) => {
            ...
            return (
                <g key={edge.id}>
                    <path d={path} fill="none" stroke="#9CA3AF" strokeWidth="2" markerEnd="url(#arrowhead)" />
                    {edge.label && edge.label !== 'default' && (
                        <text x={(fromX + toX) / 2} y={midY - 5} textAnchor="middle" className="text-xs fill-gray-500">
                            {edge.label}
                        </text>
                    )}
                </g>
