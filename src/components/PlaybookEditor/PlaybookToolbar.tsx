...
export const PlaybookToolbar: React.FC<PlaybookToolbarProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-navy-900 border-b border-gray-200 px-4 py-2 flex items-center justify-between">
      {/* Left: Add Nodes */}
      <div className="flex items-center gap-2">
        <span className="text-sm font-medium text-gray-700 dark:text-gray-300 mr-2">Add:</span>
        {nodeTypes.map(({ type, icon: Icon, label, color }) => (
          <button
            key={type}
            onClick={() => onAddNode(type)}
            className={`flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-md border border-gray-200 hover:bg-gray-50 dark:bg-navy-800 transition ${color}`}
