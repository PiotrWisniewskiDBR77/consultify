    ...
    <div className="absolute top-0 right-0 w-80 h-full bg-white dark:bg-navy-900 border-l border-slate-200 dark:border-navy-700 z-30 flex flex-col shadow-xl">
        ...
        {!loading && data && (
          <>
            ...
            {data.dailyViews.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide mb-2 flex items-center gap-1.5">
                ...
                <div className="space-y-1">
                  {data.dailyViews.slice(0, 14).map((dv) => (
                    <div key={dv.date} className="flex items-center justify-between text-[11px]">
                      <span className="text-slate-500">{dv.date}</span>
      ...
      <div className="px-4 py-2 border-t border-slate-100 dark:border-navy-800">
        <button
          ...
          disabled={loading}
          className="w-full py-2 rounded-lg border border-slate-200 dark:border-navy-700 text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-navy-800 disabled:opacity-50"
        >
