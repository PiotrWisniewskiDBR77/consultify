...
export const PresentationReviewPanel: React.FC<Props> = ({ deckId, version, organizationId, currentUserId, qualityPanel }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t('presentations.review.loadFailed', 'The review could not be loaded.'));
    } finally { setLoading(false); }
  }, [deckId, organizationId, t]);
  ...
  const execute = async (operation: () => Promise<unknown>) => {
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t('presentations.review.actionFailed', 'The action did not go through.')); }
  ...
  return <div className="flex h-full min-h-0 flex-col" data-testid="presentation-review-panel">
    <div className="flex shrink-0 gap-1 border-b border-c-border-subtle p-2" role="tablist">
      ...
      {(['qa', 'approval'] as const).map((item) => <button key={item} type="button" role="tab" aria-selected={tab === item} onClick={() => setTab(item)} className={`min-h-9 rounded-md px-3 text-xs font-medium ${tab === item ? 'bg-c-focus/10 text-c-focus-solid-on-tint' : 'text-c-text-secondary hover:bg-c-surface-hover'}`}>{item === 'qa' ? t('presentations.review.title', 'Review') : t('presentations.review.approval', 'Approval')}</button>)}
    </div>
    <div className="min-h-0 flex-1 overflow-auto">{tab === 'qa' ? qualityPanel : <div className="space-y-4 p-4 text-sm text-c-text">
      <div><h3 className="font-semibold">{t('presentations.builder.presentationReviewPanel.presentationApproval', 'Presentation approval')}</h3><p className="mt-1 text-xs text-c-text-secondary">{t('presentations.review.approvalHint', 'Version {{version}}. The author cannot approve their own version.', { version })}</p></div>
