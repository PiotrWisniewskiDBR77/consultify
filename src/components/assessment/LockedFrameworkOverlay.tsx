...
const LockedFrameworkOverlay: React.FC<LockedFrameworkOverlayProps> = ({ frameworkId, reason, upgradeCTA, onRequestAccess, onBack }) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-[60vh] p-8">
      <div className="max-w-md w-full text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{config?.fullName || frameworkId}</h2>
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-6">
          {reason || t('licensedTools.lockedReason', 'Your organization does not have access to this framework. Upgrade your plan or contact your administrator to unlock it.')}
        </p>
        <div className="space-y-3">
          <button
            onClick={() => { trackFunnelEvent('licensed_tool_unlock_cta_clicked', { framework: frameworkId }); onRequestAccess?.(); }}
          ...
          {onBack && (
            <button onClick={onBack} className="w-full px-6 py-2.5 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-medium transition-colors">
        ...
        <div className="mt-8 pt-6 border-t border-slate-200 dark:border-white/10">
          <p className="text-xs text-slate-500">
            {config?.legalNoticeType === 'proprietary'
              ? t('licensedTools.proprietaryNote', 'This is a proprietary framework requiring a paid license.')
              : t('licensedTools.educationalNote', 'This framework is available for educational purposes.')}
          </p>
        </div>
      </div>
    </div>
