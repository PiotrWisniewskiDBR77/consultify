...
import {
  LEAN_SCALE,
  LeanDimension,
  RAPID_LEAN_QUESTIONNAIRE,
} from '../../data/rapidLeanQuestionnaire';
...
export const RapidLeanWizard: React.FC<RapidLeanWizardProps> = ({
  ...
  const totalQuestions = RAPID_LEAN_QUESTIONNAIRE.reduce(
    (sum: number, dim: any) => sum + dim.questions.length,
    0
  );
  ...
  const handleBack = () => {
    if (currentQuestionIndex > 0) {
      ...
      setCurrentQuestionIndex(
        RAPID_LEAN_QUESTIONNAIRE[currentDimensionIndex - 1].questions.length - 1
      );
    }
  };
