...
export const VersionHistoryPanel: React.FC<VersionHistoryPanelProps> = ({
  ...
  const handleRestore = useCallback(
    async (version: number) => {
      if (
        !confirm(`Czy na pewno chcesz przywrócić wersję ${version}? Obecne dane zostaną nadpisane.`)
      ) {
        return;
      }
      ...
      try {
        ...
        const response = await fetch(
          `/api/assessment-workflow/${assessmentId}/restore/${version}`,
          {
            method: 'POST',
            headers: { Authorization: `Bearer ${token}` },
          }
      ...
      >
        <div className="flex flex-col h-full">
          ...
          {!success && !loading && versions.length > 0 && (
            <div className="shrink-0 px-6 py-3 border-t border-slate-200 dark:border-navy-700 bg-slate-50 dark:bg-navy-950">
