...
export const SIRIAssessmentEditor: React.FC<Props> = ({
  ...
  // "Why we ask this" — keyed by building block (matches SIRI_BLOCK_WHY_HINTS).
  const whyThisMattersHint = useMemo(
    () => getWhyThisMattersHint('siri', activeDimension?.buildingBlock || activeBlock),
    [activeDimension?.buildingBlock, activeBlock]
  );
  ...
  const requestGuidance = useCallback(
    (dimension: SIRIDimension, level: SIRILevel) => {
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {viewMode === 'dimensions' && activeDimension && currentDimensionState && (
            <div className="max-w-2xl mx-auto space-y-6">
              ...
              {(() => {
                const lvl = SIRI_MATURITY_LEVELS.find(
