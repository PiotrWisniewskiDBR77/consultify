...
export const ADKARWorkspace: React.FC<ADKARWorkspaceProps> = ({ projectId, organizationId }) => {
              ...
              return (
                <div key={dim.id}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-gray-600 dark:text-gray-400">{score.toFixed(1)}/5</span>
                  </div>
                  <div className="h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div
                      className={`h-full ${score >= 3.5 ? 'bg-green-500' : score >= 2.5 ? 'bg-yellow-500' : 'bg-danger-500'}`}
  ...
  const currentDimension = ADKAR_QUESTIONNAIRE.find(
    (d: ADKARDimension) => d.id === currentQ.dimension
  );
