...
export const LevelNavigator: React.FC<LevelNavigatorProps> = ({
  ...
  if (isTablet || isMobile) {
    return (
      <div className="w-full bg-white dark:bg-navy-950 border-b border-slate-200 dark:border-navy-700 shrink-0">
        <div className="px-4 py-2 flex items-center gap-2 overflow-x-auto scrollbar-hide scroll-snap-x momentum-scroll">
          {levelKeys.map((key) => {
            ...
            if (isActual && isTarget) {
              statusDot = (
                <div className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-gradient-to-r from-blue-500 to-primary-500" />
              );
            } else if (isActual) {
              statusDot = (
        ...
        <div className="px-4 py-2 border-t border-slate-200 dark:border-navy-700">
    ...
    <div className="w-80 bg-white dark:bg-navy-900 shadow-sm flex flex-col shrink-0 hidden lg:flex">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {levelKeys.map((key) => {
          ...
          if (isActual && isTarget)
            statusLabel = (
              <span className="text-[10px] font-bold text-white bg-gradient-to-r from-blue-500 to-primary-500 px-2 py-0.5 rounded-full">
