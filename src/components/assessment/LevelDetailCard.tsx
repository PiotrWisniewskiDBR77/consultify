...
export const LevelDetailCard: React.FC<LevelDetailCardProps> = ({
    ...
    return (
        <div
            className={`bg-white dark:bg-navy-950/50 border border-slate-200 dark:border-white/5 rounded-2xl relative overflow-hidden flex flex-col items-center text-center shadow-lg dark:shadow-none ${isCompact ? 'p-4 md:p-6' : 'p-8'}`}
        >
            ...
            <div className="relative z-10 w-full max-w-2xl">
                ...
                <div
                    className={`${isCompact ? 'mb-4' : 'mb-8'} ${isTouchDevice ? 'sticky top-0 bg-white/95 dark:bg-navy-950/95 backdrop-blur-sm -mx-4 px-4 py-3 md:-mx-6 md:px-6 z-20 border-b border-slate-100 dark:border-white/5' : ''}`}
                >
                ...
                {Array.isArray(helperQuestions) && helperQuestions.length > 0 && (
                    <div
                        className={`bg-slate-50 dark:bg-white/5 rounded-xl text-left border border-slate-200 dark:border-white/5 ${isCompact ? 'p-4 mb-4' : 'p-6 mb-8'}`}
                    >
