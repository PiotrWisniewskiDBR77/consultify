...
describe('P-P03 — zapis nie wygasza warsztatu („Loading session…")', () => {
  it('w trakcie autozapisu zachowuje pytanie 3 i wpisany tekst', async () => {
    const { events } = await renderAtInterviewFocus();
    void events;
    ...
    const pole = () =>
      screen.getByTestId('interview-focus-panel').querySelector('textarea')! as HTMLTextAreaElement;
    const getSessionCallsBeforeAutosave = hoisted.getSession.mock.calls.length;
    fireEvent.change(pole(), { target: { value: 'Step 3 draft' } });
    fireEvent.change(pole(), { target: { value: 'Step 3 draft remains visible' } });
    await waitFor(() => expect(hoisted.appendEvent).toHaveBeenCalled());
    await waitFor(() =>
      expect(hoisted.getSession.mock.calls.length).toBeGreaterThan(getSessionCallsBeforeAutosave)
    );
    ...
    expect(screen.getByTestId('question-progress')).toHaveTextContent('Question 3 of 7');
    expect(pole()).toHaveValue('Step 3 draft remains visible');
