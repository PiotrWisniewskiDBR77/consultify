...
function triggerSkip(code: string): void {
  fireEvent.click(screen.getByRole('button', { name: /Pomiń z uzasadnieniem|Skip with a reason/i }));
  fireEvent.change(screen.getByTestId('skip-reason-select'), { target: { value: code } });
  fireEvent.click(screen.getByRole('button', { name: /Potwierdź|Confirm/i }));
}

function breadcrumbText(): string {
  return screen.getByRole('navigation', { name: /Ścieżka pytania|Question path/i }).textContent ?? '';
}
