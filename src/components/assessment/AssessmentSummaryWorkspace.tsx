...
export const AssessmentSummaryWorkspace: React.FC<AssessmentSummaryWorkspaceProps> = ({
  ...
  const overallProgress = axes.reduce(
    (acc, axis) => {
      ...
      return {
        ...
        axesRated:
          acc.axesRated + (progress.rated === progress.total && progress.total > 0 ? 1 : 0),
  ...
  return (
    ...
    >
      ...
      {/* Main Chart / Table */}
      <div className="bg-white dark:bg-navy-950/50 border border-slate-200 dark:border-navy-700 rounded-xl p-6 mb-8">
        <div className="mb-6 border-b border-slate-200 dark:border-navy-700 pb-4">
          <h3 className="text-xl font-bold text-navy-900 dark:text-white mb-3">
            Wizualizacja Luk (Gap Map)
          </h3>
          {/* Legend - above chart */}
        ...
        <div className="grid grid-cols-7 gap-4 mb-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center border-b border-slate-200 dark:border-navy-700 pb-2">
          {axes.map((axis) => (
        ...
        <div className="grid grid-cols-7 gap-4 h-64 items-end pb-4">
          {axes.map((axis) => {
            ...
            return (
              ...
              >
                {/* Tooltip */}
                <div className="absolute -top-12 opacity-0 group-hover:opacity-100 bg-white dark:bg-navy-900 text-navy-900 text-xs p-2 rounded shadow-lg transition-opacity z-10 pointer-events-none whitespace-nowrap">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 gap-6">
          {axes.map((axis) => {
            ...
            const axisContent = translate(`assessment.axisContent.${axis}`, {
              returnObjects: true,
            }) as {
              title: string;
              intro?: string;
            ...
            const calculatedActual =
              areaCount > 0 ? Number((totalActual / areaCount).toFixed(1)) : 0;
            const calculatedTarget =
              areaCount > 0 ? Number((totalTarget / areaCount).toFixed(1)) : 0;
