...
import { DecisionImpact, FullInitiative, RiskRating, Task, TaskStatus, TaskType, User } from '../types';
...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = React.memo(
    ({ task: initialTask, isOpen, onClose, onSave, currentUser, users = [], language = 'EN', initiative }) => {
        const [task, setTask] = useState<Task>({ ...initialTask });
        const [activeTab, setActiveTab] = useState<'strategy' | 'execution' | 'evidence' | 'dependencies' | 'history'>(
            'strategy',
        );
                ...
                <div className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-white/10 rounded-xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col h-[85vh]">
                    ...
                    {initiative && (
                        <div className="px-4 py-2.5 border-b border-slate-200 dark:border-white/5 bg-gradient-to-r from-blue-50 via-purple-50/50 to-transparent dark:from-blue-900/20 dark:via-purple-900/10 dark:to-transparent flex items-center gap-3 shrink-0">
                            ...
                            <span
                                className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${initiative.status === 'DRAFT'
                    ...
                    <div className="p-4 border-b border-slate-200 dark:border-white/10 flex justify-between items-start bg-slate-50 dark:bg-navy-950 shrink-0">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                onClick={onClose}
                                className="text-slate-500 hover:text-navy-900 dark:hover:text-white p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-full transition-colors"
                            >
                                <X size={20} />
                            </button>
                        </div>
                                ...
                                <div className="space-y-6 max-w-3xl animate-in slide-in-from-right-4 duration-300">
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                                        ...
                                                        onChange={(e) =>
                                                            setTask({
                                                                ...
                                                                decisionImpact: {
                                                                    ...
                                                                    decisionStatement:
                                                                        task.decisionImpact?.decisionStatement || '',
                                                                },
                                                            })
                                                        }
                                                        ...
                                                        onChange={(e) =>
                                                            setTask({
                                                                ...
                                                                decisionImpact: {
                                                                    ...task.decisionImpact,
                                                                    decisionType:
                                                                        task.decisionImpact?.decisionType || 'CONTINUE',
                                        ...
                                        <div className="p-4 rounded-lg bg-slate-50 dark:bg-navy-950/30 border border-slate-200 dark:border-white/5">
                                            ...
                                            <div className="space-y-2">
                                                {['PROCESS_CHANGE', 'BEHAVIOR_CHANGE', 'CAPABILITY_CHANGE'].map(
                                                    (type) => (
                                                        <label
                                                            key={type}
                                                        ...
                                                        >
                                                            <input
                                                                type="checkbox"
                                                                checked={
                                                                    task.strategicContribution?.includes(type as any) ||
                                                                    false
                                                                }
                                                                onChange={(e) => {
                                                                    ...
                                                                    if (e.target.checked) {
                                                                        ...
                                                                        setTask({
                                                                            ...task,
                                                                            strategicContribution: (Array.isArray(current) ? current : []).filter(
                                                                                (t: any) => t !== type,
                                                                            ),
                                                                        });
                                                                    }
                            ...
                            {activeTab === 'evidence' && (
                                <div className="space-y-6 max-w-3xl animate-in slide-in-from-right-4 duration-300">
                                        ...
                                        <div className="flex gap-4">
                                            {['DOCUMENT', 'DATA', 'DEMO', 'APPROVAL'].map((type) => (
                                                <button
                                                    key={type}
                                                    onClick={() => {
                                                        const current = Array.isArray(task.evidenceRequired) ? task.evidenceRequired : [];
                                                        if (current.includes(type as any)) {
                                                            setTask({
                                                                ...task,
                                                                evidenceRequired: (Array.isArray(current) ? current : []).filter((t: any) => t !== type),
                                                            });
                                                        } else {
                                                            setTask({
                                                                ...task,
                                                    ...
                                                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${Array.isArray(task.evidenceRequired) && task.evidenceRequired.includes(type as any)
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            value={Array.isArray(task.acceptanceCriteria) ? task.acceptanceCriteria.join('\n') : String(task.acceptanceCriteria || '')}
                                            onChange={(e) => setTask({ ...task, acceptanceCriteria: e.target.value.split('\n') })}
                                    ...
                                    {/* Evidence Sign-off */}
                                    <div
                                        className={`p-5 rounded-xl border-2 transition-all ${task.signedOff
                                            ...
                                            <div>
                                                ...
                                                {task.signedOff && task.signedOffAt && (
                                                    <p className="text-xs text-slate-500 mt-1">
                                                        Signed by{' '}
                                                        {users.find((u) => u.id === task.signedOffBy)?.firstName ||
                                                            'Unknown'}{' '}
                                                        {users.find((u) => u.id === task.signedOffBy)?.lastName || ''}{' '}
                                                        on {new Date(task.signedOffAt).toLocaleDateString()}
                                                    </p>
                                                )}
                                                {!task.signedOff && (
                                                    <p className="text-xs text-slate-500 mt-1">
                                                        I confirm all required evidence has been collected and criteria
                                                        met.
                                                    </p>
                                                )}
                                            </div>
                                    ...
                                    <div className="space-y-3">
                                        {task.changeLog && task.changeLog.length > 0 ? (
                                            task.changeLog.map((log, i: number) => (
                                                <div
                                                    key={i}
                                                    className="flex gap-3 text-xs p-3 rounded bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5"
                                                >
