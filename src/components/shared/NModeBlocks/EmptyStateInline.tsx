...
export const EmptyStateInline: React.FC<EmptyStateInlineProps> = ({
  ...
  return (
    ...
    >
      <Icon size={28} className="mx-auto mb-3 text-slate-300 dark:text-slate-600" />
      <p className="text-sm text-slate-400 dark:text-slate-500 mb-1">{message}</p>
      {hint && <p className="text-xs text-slate-300 dark:text-slate-600 mb-3">{hint}</p>}
      {action && (
        <button
