...
export const TransformationScorecard: React.FC<Props> = ({ projectId, fetcher }) => {
  ...
  if (failed)
    return wrap(<p className="text-sm text-gray-400">Value narrative temporarily unavailable.</p>);
  if (loading && !data) return wrap(<p className="text-sm text-gray-400">Loading value…</p>);
