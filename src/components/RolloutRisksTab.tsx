...
export const RolloutRisksTab: React.FC<RolloutRisksTabProps> = ({
  data,
  initiatives,
  onUpdate,
}) => {
  const [activeType, setActiveType] = useState<'Risk' | 'Issue' | 'Assumption' | 'Dependency'>(
    'Risk'
  );
  ...
  return (
    <div className="space-y-6 flex flex-col h-full">
      ...
      <div className="flex gap-2 border-b border-slate-200 dark:border-navy-700 pb-1">
        {[
          ...
          >
            <type.icon
              size={16}
              className={activeType === type.id ? type.color : 'text-slate-400 dark:text-slate-500'}
            />
            {type.id}s
      ...
      {/* Content List */}
      <div className="flex-1 overflow-y-auto bg-white dark:bg-navy-900 rounded-b-xl rounded-tr-xl border border-slate-200 dark:border-navy-700 shadow-sm">
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-50 dark:bg-navy-950 text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold sticky top-0 z-10">
            <tr>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-white/5">
            {filteredItems.length === 0 ? (
              <tr>
                <td
                  colSpan={6}
                  className="p-12 text-center text-slate-400 dark:text-slate-500 italic"
                >
                        ...
                        <div className="grid grid-cols-2 gap-6">
                          <div>
                            ...
                            <textarea
                              ...
                              className="w-full bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 rounded p-2 text-sm"
                              rows={3}
                            />
                          </div>
                          <div>
