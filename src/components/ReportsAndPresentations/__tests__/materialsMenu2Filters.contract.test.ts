 ...
 * zastąpiony kanonicznymi dropdownami Menu 2 (`Menu2PresetDropdown` —
...
describe('ReportsAndPresentationsHub — Menu 2 filtry (DEC-423)', () => {
  ...
  it('renderuje kanoniczne dropdowny Menu 2 (Status, Widoczność, Source)', () => {
  ...
  it('używa istniejącego Menu2PresetDropdown (Inicjatywy DEC-420) — nie pisze nowego komponentu', () => {
    ...
    // DEC-631: trzeci preset = Source (obok Status + Widoczność).
    const usageHits = source.match(/<Menu2PresetDropdown/g) || [];
    expect(usageHits).toHaveLength(3);
  });
