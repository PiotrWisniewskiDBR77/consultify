...
export const RolloutPlanTab: React.FC<RolloutPlanTabProps> = ({ data, initiatives, onUpdate }) => {
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex justify-between items-center mb-2">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Timeline view of all strategic initiatives.
          </p>
        </div>
        <div className="flex gap-2">
          <button className="px-3 py-1.5 text-xs font-bold bg-slate-100 dark:bg-white/5 rounded border border-slate-200 dark:border-navy-700 hover:bg-slate-200 dark:hover:bg-navy-700 text-slate-600 dark:text-slate-300">
            Optimize Layout
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-auto bg-white dark:bg-navy-900 rounded-xl border border-slate-200 dark:border-navy-700 relative">
        <div className="min-w-[1200px] h-full flex flex-col">
          {/* Header */}
          <div className="grid grid-cols-8 border-b border-slate-200 dark:border-navy-700 bg-slate-50 dark:bg-navy-950 sticky top-0 z-10">
            {quarters.map((q) => (
              <div
                key={q}
                className="p-3 text-center font-bold text-slate-600 dark:text-slate-400 border-r last:border-r-0 border-slate-200 dark:border-navy-700"
              >
                {q}
              </div>
            ))}
                ...
                >
                  ...
                  <div className="space-y-2 mt-4">
                    {inits.map((init) => (
                      <div
                        key={init.id}
                        className="p-2 rounded bg-white dark:bg-navy-800 border border-slate-200 dark:border-navy-700 shadow-sm text-xs hover:shadow-md transition-shadow cursor-pointer dark:text-slate-200"
                      >
                        <div
