...
export const RolloutClosureTab: React.FC<RolloutClosureTabProps> = ({
  data,
  onUpdate,
  onCloseProgram,
}) => {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <Flag className="text-slate-500 dark:text-slate-400" />
            Program Closure
          </h2>
          <p className="text-slate-500 dark:text-slate-400">
            Final review, lessons learned, and handover.
          </p>
        </div>
        {/* Close Button */}
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {checklist.map((item, i) => (
              ...
              >
                <input
                  ...
                  className="w-5 h-5 rounded border-slate-300 dark:border-navy-700 text-green-600 focus:ring-green-500"
                />
                <span
        ...
        <div className="bg-white dark:bg-navy-900 p-6 rounded-xl border border-slate-200 dark:border-navy-700 shadow-sm">
          ...
          <button className="w-full py-2 border border-dashed border-slate-300 dark:border-white/20 rounded text-slate-500 dark:text-slate-400 hover:text-blue-500 font-bold transition-colors">
            + Add Lesson
          </button>
        </div>
      </div>
