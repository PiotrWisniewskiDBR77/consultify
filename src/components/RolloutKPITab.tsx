import { Activity, ArrowDown, ArrowUp, Plus, TrendingUp } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {kpis.length === 0 && (
          <div className="col-span-full py-12 text-center text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-navy-900 border border-dashed border-slate-200 dark:border-navy-700 rounded-xl">
            No KPIs defined yet.
          </div>
        )}
        ...
        {kpis.map((kpi, i) => {
          ...
          return (
            <div
              key={kpi.id}
              className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <div className="grid grid-cols-3 gap-2 mb-4 text-center">
                ...
                <div className="border-x border-slate-100 dark:border-navy-700">
                  ...
                  <div className="font-mono font-bold text-2xl text-slate-800 dark:text-white">
                    {kpi.current}
                    {kpi.unit}
                  </div>
                </div>
              ...
              <div className="h-16 bg-slate-50 dark:bg-navy-950 rounded border border-slate-100 dark:border-navy-700 flex items-end justify-between px-2 pb-2 overflow-hidden">
                {[30, 45, 40, 60, 55, 75, kpi.current].map((val, idx) => (
                  <div
                    key={idx}
                    className="w-1/12 bg-blue-500/20 hover:bg-blue-500 transition-colors rounded-t-sm"
                    style={{ height: `${(val / kpi.target) * 100}%` }}
                  ></div>
                ))}
              </div>
            </div>
