        ...
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            ...
            {initiative.strategicRole && (
              <span
                className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border flex items-center gap-1 ${getRoleColor(initiative.strategicRole)}`}
              >
            ...
            {initiative.strategicIntent ? (
              <span
                className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border ${getIntentColor(initiative.strategicIntent)}`}
              >
                {initiative.strategicIntent}
              </span>
            ) : (
              <span
                className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border ${getStatusColor(initiative.status)}`}
              >
            ...
            {initiative.axis && (
              <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded border bg-navy-900 text-slate-400 dark:text-slate-500 border-white/10">
                Axis{' '}
        ...
        <div className="mt-3 pt-3 border-t border-slate-100 dark:border-navy-700 flex justify-between items-center relative z-20">
          ...
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              ...
              title="Suggest better placement"
            >
              <RefreshCw size={10} />
            </button>
          </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1">
                  <div className="flex justify-between text-[9px] text-slate-500 dark:text-slate-400">
                    <span>Analytic</span>
                    <span>{initiative.effortProfile.analytical}/5</span>
                  </div>
                  <div className="h-1 bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-blue-500"
                      style={{ width: `${(initiative.effortProfile.analytical / 5) * 100}%` }}
                    ></div>
                  </div>
                </div>
                <div className="space-y-1">
                  <div className="flex justify-between text-[9px] text-slate-500 dark:text-slate-400">
                    <span>Ops</span>
                    <span>{initiative.effortProfile.operational}/5</span>
                  </div>
                  <div className="h-1 bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-emerald-500"
                      style={{ width: `${(initiative.effortProfile.operational / 5) * 100}%` }}
                    ></div>
                  </div>
                </div>
                <div className="space-y-1">
                  <div className="flex justify-between text-[9px] text-slate-500 dark:text-slate-400">
                    <span>Change</span>
                    <span>{initiative.effortProfile.change}/5</span>
                  </div>
                  <div className="h-1 bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden">
                    <div
        ...
        <div className="flex items-center gap-2">
          ...
          {onEnrich && !initiative.aiConfidence && (
            <button
              ...
              className={`text-[10px] px-2 py-1 rounded border transition-colors flex items-center gap-1 ${isEnriching ? 'bg-blue-500/20 text-blue-300 border-blue-500/30' : 'bg-white/5 text-slate-400 dark:text-slate-500 border-white/10 hover:bg-slate-100 dark:hover:bg-navy-800/40 hover:text-white'}`}
            >
