...
export const CommissionIntelligence: React.FC<CommissionIntelligenceProps> = ({
    ...
    const aiInsights = useMemo<AIInsight[]>(() => {
        ...
        // Pending payments
        const pendingStatements = statements.filter((s) => s.status === 'PENDING' || s.status === 'APPROVED');
    ...
    return (
        <div className="space-y-6">
            {/* AI Insights Header */}
            <div className="rounded-3xl border border-brand/20 bg-gradient-to-br from-brand/5 to-purple-500/5 p-6 dark:from-brand/10 dark:to-purple-500/10">
                ...
                <div className="space-y-3">
                    {aiInsights.length === 0 ? (
                        <div className="rounded-2xl bg-white/50 p-4 text-center text-sm text-slate-500 dark:bg-navy-900/50">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white/90 p-6 dark:border-white/5 dark:bg-navy-900/60">
                <div className="mb-4 flex items-center justify-between">
                    <h3 className="text-lg font-semibold text-navy-900 dark:text-white">Commission Projections</h3>
            ...
            <div className="grid gap-4 md:grid-cols-2">
                <button
                    ...
                    className="flex items-center justify-between rounded-2xl border border-brand/20 bg-brand/5 p-4 text-left transition hover:bg-brand/10"
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-slate-500">Commission questions or updates</div>
                        </div>
                    </div>
                    <ArrowUpRight size={18} className="text-brand" />
                </button>

                <button className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white/80 p-4 text-left transition hover:border-brand/30 dark:border-white/5 dark:bg-navy-900/40">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-slate-500">Historical payouts & reports</div>
                        </div>
                    </div>
...
const InsightCard: React.FC<{ insight: AIInsight }> = ({ insight }) => {
    ...
    const bgMap = {
        opportunity: 'bg-emerald-50 border-emerald-100 dark:bg-emerald-500/10 dark:border-emerald-500/20',
    ...
    return (
        <div className={`rounded-2xl border p-4 ${bgMap[insight.type]}`}>
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                    ...
                    <div className="mt-1 text-sm text-slate-600 dark:text-slate-300">{insight.description}</div>
                    {insight.actionLabel && (
...
const ProjectionCard: React.FC<ProjectionCardProps> = ({ label, value, status, subtitle, highlight }) => {
    const statusColors = {
        ...
        <div
            className={`rounded-2xl border p-4 ${
                highlight
                    ...
                    : 'border-slate-100 bg-slate-50/50 dark:border-white/5 dark:bg-navy-950/40'
            }`}
        >
...
const DealRow: React.FC<{ deal: PartnerDeal; onView: () => void }> = ({ deal, onView }) => {
    const statusColors: Record<string, string> = {
        ...
        NEGOTIATION: 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400',
    };

    return (
        <div
            onClick={onView}
            className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-100 p-4 transition hover:border-brand/30 dark:border-white/5"
        >
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div className="text-xs text-slate-500">${deal.dealValue.toLocaleString()} deal value</div>
                </div>
            </div>
            <div className="flex items-center gap-3">
                <div className="text-right">
                    ...
                    <div className="text-xs text-slate-400">{deal.commissionRate}% rate</div>
                </div>
