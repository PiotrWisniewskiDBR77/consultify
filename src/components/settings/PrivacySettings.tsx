...
export const PrivacySettings: React.FC<PrivacySettingsProps> = ({ currentUser, onUpdateUser }) => {
  ...
  const activityVisibilityOptions = [
    {
      value: 'all',
      label: t('settings.privacy.visibility.all', 'Everyone'),
      icon: Globe,
      description: 'All users can see your activity',
    },
    {
      value: 'team',
      label: t('settings.privacy.visibility.team', 'Team Only'),
      icon: Users,
  ...
  const profileVisibilityOptions = [
    {
      value: 'public',
      label: t('settings.privacy.profile.public', 'Public'),
      icon: Globe,
      description: 'Anyone can view your profile',
    },
    {
      value: 'organization',
      label: t('settings.privacy.profile.organization', 'Organization'),
      icon: Building2,
      description: 'Only organization members',
    },
    {
      value: 'private',
      label: t('settings.privacy.profile.private', 'Private'),
      icon: Lock,
      description: 'Profile hidden from others',
    },
  ];
      ...
      <div className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 rounded-xl p-6">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-navy-700">
            ...
            >
              <span
                className={`${preferences.showInDirectory ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white dark:bg-navy-900 transition-transform`}
              />
            </button>
          </div>
        </div>
            ...
            return (
              <button
                ...
                onClick={() =>
                  updatePreference(
                    'activityVisibility',
                    option.value as PrivacyPreferences['activityVisibility']
                  )
                }
              ...
              >
                <Icon
                  size={24}
                  className={isSelected ? 'text-blue-600' : 'text-slate-400 dark:text-slate-500'}
                />
                <div
                  className={`mt-2 font-medium ${isSelected ? 'text-blue-700 dark:text-blue-400' : 'text-slate-700 dark:text-slate-300'}`}
                >
                  {option.label}
                </div>
      ...
      <div className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 rounded-xl p-6">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            >
              <span
                className={`${preferences.allowMentions ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white dark:bg-navy-900 transition-transform`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-navy-700">
            ...
            <button
              onClick={() =>
                updatePreference('shareActivityWithAI', !preferences.shareActivityWithAI)
              }
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
            ...
            >
              <span
                className={`${preferences.shareActivityWithAI ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white dark:bg-navy-900 transition-transform`}
              />
            </button>
          </div>
        </div>
