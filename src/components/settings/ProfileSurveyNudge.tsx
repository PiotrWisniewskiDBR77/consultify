    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-navy-900 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
        ...
        <button
          ...
          className="absolute top-4 right-4 p-1.5 rounded-full text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-navy-800 transition-colors z-10"
        >
          <X size={18} />
        </button>
            ...
            <div className={`transition-opacity duration-200 ${animating ? 'opacity-0' : 'opacity-100'}`}>
              ...
              <div className="space-y-2 mb-8">
                    ...
                    {DEPARTMENT_OPTIONS.map((dept) => (
                      <button
                        ...
                        className={`flex items-center gap-2.5 px-4 py-3 rounded-xl text-left text-sm font-medium transition-all border ${
                          department === dept
                            ? 'bg-purple-50 dark:bg-purple-500/20 border-purple-400 dark:border-purple-500 text-purple-700 dark:text-purple-300 shadow-sm shadow-purple-500/10'
                ...
                {step === 'expertise' && (
                  <div>
                    {expertise.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-3 pb-3 border-b border-slate-100 dark:border-navy-700">
                        {expertise.map((tag) => (
                          <span
                            key={tag}
                            className="inline-flex items-center gap-1 px-2.5 py-1 bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 rounded-full text-xs font-medium"
                          >
                            <Tag size={10} />
                            {tag}
                            <button
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                    canProceed || step === 'expertise'
                      ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 hover:scale-[1.02]'
