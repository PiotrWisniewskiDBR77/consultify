...
export const EmailDigestSettings: React.FC<EmailDigestSettingsProps> = ({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const [emailRes, digestRes] = await Promise.all([
        Api.get('/settings/notifications/email').catch(() => emailSettings),
        Api.get('/settings/notifications/digest').catch(() => ({ frequency, content, format })),
      ]);
      ...
      setFrequency(persistedDigest.frequency || frequency);
      setContent(persistedDigest.content || content);
      setFormat(persistedDigest.format || format);
      setOriginalDigest({
