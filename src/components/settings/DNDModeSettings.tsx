...
export const DNDModeSettings: React.FC<DNDModeSettingsProps> = ({ currentUser, onUpdateUser }) => {
    ...
    useEffect(() => {
        const loadPreferences = async () => {
            try {
                const prefs = await Api.get('/settings/notifications/dnd');
                if (prefs) {
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {DND_PRESETS.map((preset) => (
                            <button
                                ...
                                className="px-4 py-3 bg-slate-50 dark:bg-navy-950/50 border border-slate-200 dark:border-white/10 rounded-lg hover:border-purple-300 transition-colors text-sm font-medium text-slate-700 dark:text-slate-300"
                            >
                                {preset.label}
                            </button>
                        ))}
