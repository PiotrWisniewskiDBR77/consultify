...
export const ProposalDiffCard: React.FC<ProposalDiffCardProps> = ({
  ...
  return (
    ...
    >
      <header className="flex items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 px-3 py-2">
      ...
      <section className="border-t border-slate-100 dark:border-slate-800 px-3 py-2">
        <p className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">
          Proposed operations
        </p>
        {!operations || operations.length === 0 ? (
          ...
          <ul
            className="mt-1 max-h-44 overflow-auto rounded border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 p-2 text-[11px] font-mono"
          ...
          >
            {operations.slice(0, 25).map((op, idx) => {
              ...
              return (
                ...
                >
                  {idx + 1}. {opObj?.type ?? 'op'} — {shorten(JSON.stringify(op))}
                </li>
              );
            })}
      ...
      <footer className="flex items-center justify-end gap-2 border-t border-slate-100 dark:border-slate-800 px-3 py-2">
        <button
          type="button"
          onClick={onReject}
