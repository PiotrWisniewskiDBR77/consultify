...
export const EditableSpreadsheetGrid = React.forwardRef<EditableSpreadsheetGridHandle, Props>(({
  ...
  const describeSelection = useCallback(
    (candidate: Selection | null = selected): SpreadsheetCellSelection | null => {
      if (!candidate || !activeRaw?.columns?.length) return null;
      const end = selectionEnd ?? candidate;
      const rowStart = Math.min(candidate.rowIndex, end.rowIndex);
      const rowEnd = Math.max(candidate.rowIndex, end.rowIndex);
      const colStart = Math.min(candidate.colIndex, end.colIndex);
      const colEnd = Math.max(candidate.colIndex, end.colIndex);
      const kind =
        candidate.kind === 'row' || candidate.kind === 'column'
          ? candidate.kind
          : rowStart !== rowEnd || colStart !== colEnd
            ? 'range'
            : 'cell';
      const column = activeRaw.columns[colStart];
      const rawValue =
        kind === 'cell' && column
          ? rawCellToEditText(activeRaw.rows?.[rowStart]?.cells?.[column.key])
          : '';
      const address =
        kind === 'row'
          ? `${activeRaw.name}!${excelRowForDataRowIndex(rowStart)}:${excelRowForDataRowIndex(rowEnd)}`
          : kind === 'column'
            ? `${activeRaw.name}!${colIndexToLetter(colStart)}:${colIndexToLetter(colEnd)}`
            : `${activeRaw.name}!${colIndexToLetter(colStart)}${excelRowForDataRowIndex(rowStart)}${
                rowStart !== rowEnd || colStart !== colEnd
                  ? `:${colIndexToLetter(colEnd)}${excelRowForDataRowIndex(rowEnd)}`
                  : ''
              }`;
      return {
        rowIndex: rowStart,
        colIndex: colStart,
        ...(rowEnd !== rowStart ? { endRowIndex: rowEnd } : {}),
        ...(colEnd !== colStart ? { endColIndex: colEnd } : {}),
        kind,
        address,
        rawValue,
      };
    },
    [activeRaw, selected, selectionEnd]
  );

  useEffect(() => {
    onSelectionChange?.(describeSelection());
  }, [describeSelection, onSelectionChange]);
  ...
  const persistChanges = useCallback(
    async (changes: CellChange[], direction: 'after' | 'before') => {
      ...
      try {
        ...
        for (const payload of payloads) {
          try {
            if (persistCell) await persistCell(payload);
            else await Api.updateWorkbookCell(workbookId, payload);
          } catch (cellError) {
    ...
    [localSheets, offlineQueueKey, persistCell, persistCells, t, workbookId, workingSheetIndex]
  );
  ...
  const applyChanges = useCallback(
    ...
    [onSheetsChange, persistChanges, publishHistoryState, workingSheetIndex]
  );
  ...
  const commit = useCallback(
    ...
    [selected, activeRaw, persistChanges, publishHistoryState, workingSheetIndex]
  );
  ...
  const clearSelection = useCallback(() => {
    if (!selectionRange || !activeRaw?.columns?.length) return;
    const changes: CellChange[] = [];
    for (let rowIndex = selectionRange.rowStart; rowIndex <= selectionRange.rowEnd; rowIndex += 1) {
      for (let colIndex = selectionRange.colStart; colIndex <= selectionRange.colEnd; colIndex += 1) {
        const column = activeRaw.columns[colIndex];
        const row = activeRaw.rows?.[rowIndex];
        if (!column || !row) continue;
        const before = row.cells?.[column.key] ?? {};
        changes.push({ rowIndex, colIndex, before: { ...before }, after: { value: null } });
      }
    }
  ...
  const copySelection = useCallback(async () => {
    if (!selectionRange || !activeRaw?.columns?.length || !navigator.clipboard?.writeText) return;
    const lines: string[] = [];
    for (let rowIndex = selectionRange.rowStart; rowIndex <= selectionRange.rowEnd; rowIndex += 1) {
      const values: string[] = [];
      for (let colIndex = selectionRange.colStart; colIndex <= selectionRange.colEnd; colIndex += 1) {
        const column = activeRaw.columns[colIndex];
        values.push(rawCellToEditText(column ? activeRaw.rows?.[rowIndex]?.cells?.[column.key] : undefined));
      }
  ...
  const pasteSelection = useCallback(async () => {
    ...
    matrix.forEach((values, rowOffset) => values.forEach((raw, colOffset) => {
      ...
      changes.push({
        ...
        before: { ...before },
        after: {
          style: (before as Record<string, unknown>).style,
          comment: (before as Record<string, unknown>).comment,
          validation: (before as Record<string, unknown>).validation,
          ...parseCellInput(raw),
        },
      });
    }));
    ...
    redoStackRef.current = [];
    applyChanges(changes, 'after');
  }, [activeRaw, applyChanges, selected]);
  ...
  }), [activeRaw, clearSelection, copySelection, cutSelection, pasteSelection, redo, ref, selected, startEditing, undo]);
  ...
  const handleNavigationKey = useCallback(
    ...
    [clearSelection, editingValue, moveSelection, redo, selected, startEditing, undo]
  );
  ...
  const handleContainerKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      if (selected && onSelectionContextMenu && e.key === 'F10' && e.shiftKey) {
        const selection = describeSelection();
        if (selection) {
          e.preventDefault();
          const cell = containerRef.current?.querySelector<HTMLElement>('[role="gridcell"][aria-selected="true"]');
        ...
        <table role="presentation" className="w-full text-xs" /* §27-exempt: edytor komorkowy/arkusz, edycja cell-by-cell — docs/ui-standards/DOKTRYNA_TABELA_NIE_EXCEL.md */>
          <thead className="sticky top-0 z-10">
            <tr className="bg-c-surface-raised">
              <th
                aria-label={t('kimi.excele.selectAll', 'Select all')}
              ...
              {columns.map((col, ci) => (
                <th
                  key={`${col.key}-${ci}`}
                  onClick={() => {
                    setSelected({ rowIndex: 0, colIndex: ci, kind: 'column' });
                    setSelectionEnd({
                      rowIndex: Math.max((activeRaw.rows?.length ?? 1) - 1, 0),
                      colIndex: ci,
                    });
                  }}
                  style={{ width: typeof col.width === 'number' ? `${col.width * 8}px` : undefined, display: col.hidden ? 'none' : undefined }}
                  className={`px-3 py-2 text-left font-medium text-c-text-secondary border-b border-c-border-subtle whitespace-nowrap ${freezeFirstColumn && ci === 0 ? 'sticky left-10 z-[8] bg-c-surface-raised' : ''}`}
                >
                  {col.header || col.key}
                </th>
              ))}
            ...
            {visibleRows.map((row, ri) => (
              <tr key={ri} style={{ height: typeof activeRaw.rows?.[ri]?.height === 'number' ? `${activeRaw.rows[ri].height}px` : undefined, display: activeRaw.rows?.[ri]?.hidden ? 'none' : undefined }} className="border-b border-c-border-subtle hover:bg-c-surface-raised">
                <th
                  scope="row"
                  onClick={() => {
                    setSelected({ rowIndex: ri, colIndex: 0, kind: 'row' });
                    setSelectionEnd({
                      rowIndex: ri,
                      colIndex: Math.max(columns.length - 1, 0),
                    });
                  }}
                  className="sticky left-0 z-[7] w-10 border-r border-c-border-subtle bg-c-surface-raised px-2 py-1.5 text-center font-mono text-[10px] font-normal text-c-text-secondary"
                >
                  ...
                  return (
                    <td
                      ...
                      onClick={(event) => {
                        ...
                        else { setSelected({ rowIndex: ri, colIndex: ci, kind: 'cell' }); setSelectionEnd(null); }
                      ...
                      onContextMenu={(event) => {
                        ...
                        const candidate = { rowIndex: ri, colIndex: ci, kind: 'cell' as const };
                        setSelected(candidate);
                        setSelectionEnd(null);
                        const column = activeRaw.columns![ci];
                        onSelectionContextMenu({
                          x: event.clientX,
                          y: event.clientY,
                          selection: {
                            ...
                            colIndex: ci,
                            kind: 'cell',
                            address: `${activeRaw.name}!${colIndexToLetter(ci)}${excelRowForDataRowIndex(ri)}`,
                            rawValue: rawCellToEditText(activeRaw.rows?.[ri]?.cells?.[column.key]),
                          },
                        });
                      }}
                      ...
                      className={`relative px-3 py-1.5 whitespace-nowrap max-w-[220px] truncate cursor-cell ${
                        isNegativeVariance
                          ...
                          : cell?.isFormula
                            ...
                            : 'text-c-text'
                      } ${freezeFirstColumn && ci === 0 ? 'sticky left-10 z-[4] bg-c-surface' : ''} ${selectionRange && ri >= selectionRange.rowStart && ri <= selectionRange.rowEnd && ci >= selectionRange.colStart && ci <= selectionRange.colEnd ? 'outline outline-2 outline-c-focus outline-offset-[-2px]' : ''}`}
                    >
                      {isEditingThis ? (
                        <input
                          ref={inputRef}
