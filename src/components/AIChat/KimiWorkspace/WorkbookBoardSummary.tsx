...
export const WorkbookBoardSummary: React.FC<{
  sheets: WorkbookGridSheet[];
  rawSheets?: FormulaSheet[] | null;
  activeSheetName?: string;
}> = ({ sheets, rawSheets, activeSheetName }) => {
      ...
      rows: sheet.rows.map((row) =>
        Object.fromEntries(
          sheet.columns.map((column) => [
            ...
            row.cells[column.key]?.error || (row.cells[column.key]?.computed ?? ''),
          ])
        )
      ),
    }));
  ...
  const cards = summary.rows
    .slice(0, 5)
    .map((row) => ({ label: String(row[summaryLabel] ?? ''), value: row[summaryValue] }));
  const scenarioLabel = scenarios.columns[0];
  const scenarioRow = scenarios.rows.find((row) =>
    /korzyści risk-adjusted/i.test(String(row[scenarioLabel] ?? ''))
  );
  const series = scenarioRow
    ? scenarios.columns
        .slice(1)
        .map((column) => ({ label: column, value: asNumber(scenarioRow[column]) }))
  ...
  return (
    <section
      data-testid="workbook-board-summary"
      className="border-b border-c-border-subtle bg-c-surface-raised/40 p-4"
    >
      <div className="grid grid-cols-2 gap-2 lg:grid-cols-5">
        {cards.map((card) => (
          <div
            key={card.label}
            className="rounded-lg border border-c-border-subtle bg-c-surface px-3 py-2 shadow-sm"
          >
            <p className="text-[10px] font-semibold uppercase tracking-wide text-c-text-muted">
              {card.label}
            </p>
            <p className="mt-1 text-lg font-semibold tabular-nums text-c-text">
              {display(card.value)}
            </p>
          </div>
        ))}
      ...
      {series.length >= 2 && max > 0 ? (
        <div className="mt-3 rounded-lg border border-c-border-subtle bg-c-surface p-3">
          <p className="mb-2 text-xs font-semibold text-c-text">
            Risk-adjusted benefits by scenario
          </p>
          <div
            className="flex h-28 items-end gap-4"
            role="img"
            aria-label="Risk-adjusted benefits by scenario"
          >
            {series.map((point) => (
              <div key={point.label} className="flex min-w-0 flex-1 flex-col items-center gap-1">
                <span className="text-[10px] tabular-nums text-c-text-secondary">
                  {display(point.value)}
                </span>
                <div
                  className="w-full max-w-16 rounded-t bg-c-chart-1"
                  style={{ height: `${Math.max(8, (Math.abs(point.value) / max) * 76)}px` }}
                />
