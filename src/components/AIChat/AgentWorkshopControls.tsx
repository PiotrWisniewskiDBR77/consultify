      ...
      sections={[
        {
          ...
          children: (
            <div className="space-y-2">
              ...
              {errorMessage ? (
                <p className="text-[11px] text-c-danger" role="alert">
                  {errorMessage}
                </p>
              ) : null}
            </div>
            ...
            <div className="space-y-2">
              ...
              {currentStep ? (
                <div
                  ...
                  className="rounded-lg border border-c-info bg-[color-mix(in_srgb,var(--c-info)_8%,transparent)] px-2.5 py-2"
                >
                  <div className="text-[9px] font-semibold uppercase tracking-wider text-c-info">
                    Teraz — krok {currentStep.stepIndex + 1} z {plan.totalSteps}
                  </div>
                  <div className="mt-0.5 truncate text-xs font-semibold text-c-text">
                    {stepDisplayName(currentStep)}
                  </div>
                  <div className="truncate text-[10px] text-c-text-muted">
          ...
          children: (
            <div className="space-y-2">
              {awaitingSteps.map((step) => (
                <div key={step.id} className="rounded-lg border border-c-warning/40 p-2">
                  <div className="mb-0.5 text-xs font-semibold text-c-text">
                    {stepDisplayName(step)}
                  </div>
                  <div className="mb-1.5 text-[10px] text-c-text-muted">
        ...
        },
        {
          ...
          children: (
            <ol className="space-y-1.5" data-testid="agent-controls-log">
              {plan.steps.map((step) => {
                const chip = STEP_STATUS_CHIP[step.status];
                return (
                  <li key={step.id} className="flex items-start gap-2">
                    <span className="mt-0.5 w-4 shrink-0 text-center text-[10px] tabular-nums text-c-text-muted">
                      {step.stepIndex + 1}
                    </span>
                    <div className="min-w-0 flex-1">
                      ...
                      {step.errorMessage ? (
                        <div className="mt-0.5 text-[10px] text-c-danger">{step.errorMessage}</div>
                      ) : null}
                    </div>
                  </li>
                );
