...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, className = '' }) => {
    const components = useMemo(
        () => ({
            ...
            h2: ({ children }: any) => (
                <h2 className="text-xl font-semibold text-slate-900 dark:text-white mt-5 mb-3">{children}</h2>
            ),
            h3: ({ children }: any) => (
                <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mt-4 mb-2">{children}</h3>
            ),
            h4: ({ children }: any) => (
                <h4 className="text-base font-semibold text-slate-800 dark:text-slate-200 mt-3 mb-2">{children}</h4>
            ),
            ...
            table: ({ children }: any) => (
                <div className="overflow-x-auto mb-4 rounded-lg border border-slate-200 dark:border-navy-700">
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-navy-700">{children}</table>
                </div>
            ),
            thead: ({ children }: any) => <thead className="bg-slate-50 dark:bg-navy-800">{children}</thead>,
            tbody: ({ children }: any) => (
                <tbody className="divide-y divide-slate-200 dark:divide-navy-700">{children}</tbody>
            ),
            tr: ({ children }: any) => <tr className="hover:bg-slate-50 dark:hover:bg-navy-800/50">{children}</tr>,
            th: ({ children }: any) => (
