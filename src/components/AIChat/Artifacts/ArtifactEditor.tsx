...
export const ArtifactEditor: React.FC<ArtifactEditorProps> = ({ artifact, onSave, onCancel }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200 dark:border-navy-700 bg-slate-50 dark:bg-navy-800/50">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {showPreview
              ? t('artifacts.hidePreview', 'Hide Preview')
              : t('artifacts.showPreview', 'Show Preview')}
          </button>
      ...
      {/* Editor Content */}
      <div className={`flex-1 ${showPreview ? 'grid grid-cols-2 gap-0.5' : ''}`}>
        {/* Textarea */}
        <div className={showPreview ? 'h-full' : 'h-full'}>
          <textarea
            ref={textareaRef}
            value={content}
        ...
        {showPreview && (
          <div className="overflow-auto bg-slate-50 dark:bg-navy-950">
            <ArtifactViewer artifact={{ ...artifact, content }} className="min-h-full" />
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-1.5 border-t border-slate-200 dark:border-navy-700 bg-slate-50 dark:bg-navy-800/50 text-xs text-slate-500 dark:text-slate-400">
        <div className="flex items-center gap-3">
          <span>
            {content.length} {t('artifacts.characters', 'characters')}
          </span>
          <span>
            {content.split('\n').length} {t('artifacts.lines', 'lines')}
          </span>
        </div>
        {hasChanges && (
