...
const PATTERNS: Array<{ pattern: RegExp; mode: CanvasStreamMode }> = [
  ...
  // EN
  {
    pattern: /\b(rewrite|replace|revise)\b[^.]*\b(this|the)?\s*(selection|selected|paragraph|section|text|fragment)\b/i,
    mode: 'replace',
  },
  // PL
  {
    pattern: /\b(przepisz|zamień|zmień|popraw)\b[^.]*\b(to|zaznaczenie|zaznaczony|akapit|fragment|sekcj|tekst)/i,
    mode: 'replace',
  },
  ...
  // EN
  {
    pattern: /\b(write|draft|compose|add|append|continue|expand|insert|put|fill)\b[^.]*\b(in|into|to|on)?\s*(the\s+)?(document|canvas|doc|here|page)\b/i,
    mode: 'append',
  },
  {
    pattern: /\b(write|draft|generate|compose)\s+(me\s+)?(a\s+|an\s+|the\s+)?(document|draft|section|paragraph|summary|outline|report)\b/i,
    mode: 'append',
  },
  // PL
  {
    pattern: /\b(napisz|dopisz|wpisz|dodaj|kontynuuj|rozwiń|uzupełnij|wstaw)\b[^.]*\b(w|do|na)?\s*(dokumen|canvas|kanw|tutaj|stron)/i,
    mode: 'append',
  },
  {
    pattern: /\b(napisz|stwórz|utwórz|wygeneruj|przygotuj)\s+(mi\s+)?(dokument|szkic|sekcj|akapit|podsumowanie|raport|konspekt)/i,
    mode: 'append',
  },
];
