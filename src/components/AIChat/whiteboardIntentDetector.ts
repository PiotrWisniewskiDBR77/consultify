...
const WB_INTENT_PATTERNS: Array<{ pattern: RegExp; action: string }> = [
  // EN patterns
  { pattern: /\b(create|start|open|make)\s+(a\s+)?(brainstorm|whiteboard|workshop|board)\b/i, action: 'wb_add_sticky' },
  { pattern: /\b(organize|cluster|group)\s+(my\s+)?(notes|ideas|stickies|board)\b/i, action: 'wb_add_cluster' },
  ...
  // PL patterns
  { pattern: /\b(stwórz|utwórz|zacznij|otwórz)\s+(burz[eę]|tablicę|warsztat|board)\b/i, action: 'wb_add_sticky' },
  { pattern: /\b(organizuj|grupuj|klasteryzuj)\s+(notatki|pomysły|board)\b/i, action: 'wb_add_cluster' },
