...
import { useToolStore, ToolType, InitiativeDraft } from '@/store/useToolStore';
import { useToolAI } from '@/hooks/discovery/useToolAI';

import { ToolHeader } from './ToolHeader';
import { ToolCanvas } from './ToolCanvas';
import { ToolActionBar } from './ToolActionBar';
import { InitiativeComposer } from './InitiativeComposer';
...
export const ToolWorkspace: React.FC<ToolWorkspaceProps> = ({
  ...
  // Modal states
  const [isInitiativeComposerOpen, setIsInitiativeComposerOpen] = useState(false);
  const [isReportViewOpen, setIsReportViewOpen] = useState(false);
  const [selectedInitiative, setSelectedInitiative] = useState<InitiativeDraft | undefined>(undefined);
  ...
  const handleOpenInitiativeComposer = useCallback((prefill?: InitiativeDraft) => {
    setSelectedInitiative(prefill);
    setIsInitiativeComposerOpen(true);
  }, []);

  const handleCloseInitiativeComposer = useCallback(() => {
    setIsInitiativeComposerOpen(false);
    setSelectedInitiative(undefined);
  }, []);

  const handleInitiativeCreated = useCallback((initiative: any) => {
    console.log('[ToolWorkspace] Initiative created:', initiative);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-navy-950">
      ...
      <ToolHeader
        ...
        onExport={handleOpenReportView}
        onCreateInitiative={() => handleOpenInitiativeComposer()}
        isPolish={isPolish}
      />
      ...
      {/* Initiative Composer Modal */}
      <InitiativeComposer
        isOpen={isInitiativeComposerOpen}
        onClose={handleCloseInitiativeComposer}
        onSuccess={handleInitiativeCreated}
        toolType={toolType}
        sessionId={currentSession.id}
        prefillData={selectedInitiative}
        isPolish={isPolish}
      />
