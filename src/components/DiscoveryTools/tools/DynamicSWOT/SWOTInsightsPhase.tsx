import { AlertTriangle, ArrowRight, Check, Lightbulb, Link2, Sparkles } from 'lucide-react';
import React from 'react';

import { ProposalCardType, SWOTData, ToolSession, useToolStore } from '@/store/useToolStore';
...
export function SWOTInsightsPhase({
  ...
  return (
    <div className="space-y-6">
      ...
      {(tensions.some(t => t.proposalStatus === 'ai-proposed') || moves.some(m => m.proposalStatus === 'ai-proposed')) && (
            ...
            {tensions.length === 0 ? (
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {tensions.map((tension) => {
                  ...
                  return (
                    <div key={tension.id} className="rounded-xl border border-slate-200/70 bg-slate-50/80 p-4 dark:border-navy-700/70 dark:bg-navy-950/40">
                      {tensionContent}
                    </div>
                  );
                })}
            ...
            {moves.length === 0 ? (
              ...
              <div className="space-y-3">
                {moves.map((move) => {
                  ...
                  return (
                    <div key={move.id} className="rounded-xl border border-slate-200/70 bg-slate-50/80 p-3 dark:border-navy-700/70 dark:bg-navy-950/40">
                      {moveContent}
                    </div>
                  );
                })}
