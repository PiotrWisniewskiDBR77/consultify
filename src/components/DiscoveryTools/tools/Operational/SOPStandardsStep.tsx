/**
 * SOPStandardsStep - Define standards and quality criteria
 */
...
interface SOPStandardsStepProps {
  session: ToolSession;
  isPolish: boolean;
}
...
export const SOPStandardsStep: React.FC<SOPStandardsStepProps> = ({ session, isPolish }) => {
  ...
  const items = data.sections?.standards || [];
  ...
  const handleRemove = (itemId: string) => {
    updateInputData({
      sections: {
        ...data.sections,
        standards: items.filter((item) => item.id !== itemId),
      },
    });
  };
...
export default SOPStandardsStep;
