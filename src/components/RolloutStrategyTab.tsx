...
export const RolloutStrategyTab: React.FC<RolloutStrategyTabProps> = ({
  data,
  onUpdate,
  isAdmin,
}) => {
  ...
  const updateListItem = (
    field: 'businessGoals' | 'inScope' | 'outScope',
    index: number,
    value: string
  ) => {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        ...
        {isAdmin && (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold border ${isLocked ? 'bg-slate-800 text-slate-400 border-slate-700' : 'bg-slate-100 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-navy-700 border-slate-300 dark:border-navy-700'}`}
          >
            {isLocked ? (
              <>
        ...
        <div className="bg-white dark:bg-navy-900 p-6 rounded-xl border border-slate-200 dark:border-navy-700 shadow-sm">
          ...
          <div className="mb-6">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-lg p-3"
            />
          </div>
              ...
              {scope.businessGoals.map((g, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    ...
                    onBlur={handleSave}
                    className="flex-1 bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-lg p-2 text-sm"
                    placeholder="Goal..."
                  />
                  <button
                ...
                {scope.outScope.map((item, i) => (
                  <div key={i} className="flex gap-2">
                    <input
                      ...
                      onBlur={handleSave}
                      className="flex-1 bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-lg p-2 text-sm opacity-70"
                    />
                    <button
      ...
      <div className="bg-white dark:bg-navy-900 p-6 rounded-xl border border-slate-200 dark:border-navy-700 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Mock pillars for UI */}
          {['Digital Foundation', 'Customer Experience', 'Operational Excellence'].map(
            (pillar, i) => (
              <div
                key={i}
                className="p-4 bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-lg text-center font-bold"
              >
                {pillar}
              </div>
            )
