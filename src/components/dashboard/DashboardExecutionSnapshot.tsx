...
export const DashboardExecutionSnapshot = React.memo<DashboardExecutionSnapshotProps>(
  ({ session, onNavigate }) => {
    // --- 1. Project Logic & Stats ---
    const initiativeStats = useMemo(() => {
      const inits = session.initiatives || [];
      const total = inits.length;
      const inProgress = inits.filter((i) =>
        ['In Progress', 'To Do', 'Ready'].includes(i.status)
      ).length;
    ...
    const priorityAlerts = useMemo(() => {
      ...
      if (initiativeStats.delayed > 0)
        alerts.push(`${initiativeStats.delayed} initiatives delayed.`);
      // Mock 3rd alert if needed or based on other logic
      if (initiativeStats.delayed > 2)
        alerts.push('Critical dependency blocked in Data Workstream.');
    ...
    return (
      <div className="max-w-6xl mx-auto space-y-6 animate-fade-in pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-navy-900 rounded-xl p-6 border border-slate-200 dark:border-navy-700 shadow-sm relative overflow-hidden">
            <h3 className="text-slate-500 dark:text-slate-400 font-bold text-xs uppercase tracking-wider mb-4">
              Overall Progress
            </h3>
            <div className="flex items-end gap-2 mb-4">
              <span className="text-5xl font-black text-navy-900 dark:text-white">
                {overallProgress}%
              </span>
            </div>
          ...
          <div className="bg-white dark:bg-navy-900 rounded-xl p-6 border border-slate-200 dark:border-navy-700 shadow-sm flex flex-col justify-center relative">
            <div className="absolute top-0 right-0 p-4 opacity-5">
              <Clock size={80} />
            </div>
            <h3 className="text-slate-500 dark:text-slate-400 font-bold text-xs uppercase tracking-wider mb-2">
              Current Phase
            </h3>
          ...
          <div className="bg-red-50 dark:bg-red-900/10 rounded-xl p-6 border border-red-100 dark:border-red-500/20 shadow-sm">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="bg-white dark:bg-navy-900 rounded-xl p-4 border border-slate-200 dark:border-navy-700 shadow-sm">
                <div className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase mb-2">
                  Total Initiatives
                </div>
              ...
              {/* In Progress */}
              <div className="bg-white dark:bg-navy-900 rounded-xl p-4 border border-slate-200 dark:border-navy-700 shadow-sm relative overflow-hidden">
              ...
              <div
                className={`rounded-xl p-4 border shadow-sm ${
                  initiativeStats.delayed > 0
                    ...
                    : 'bg-white border-slate-200 dark:bg-navy-900 dark:border-navy-700'
                }`}
              >
                <div
            ...
            <div className="bg-white dark:bg-navy-900 rounded-xl p-6 border border-slate-200 dark:border-navy-700 shadow-sm">
              ...
              {session.initiatives?.filter((i) => ['In Progress', 'To Do'].includes(i.status))
                .length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-slate-100 dark:border-navy-700 text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider">
