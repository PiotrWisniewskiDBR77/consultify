...
export const FullStep5Workspace: React.FC<FullStep5WorkspaceProps> = ({
    ...
    const getInitiativesByStatus = (status: string) => {
        ...
        return initiatives.filter((i) => {
            ...
            if (status === 'To Do')
                return [InitiativeStatus.DRAFT, InitiativeStatus.PLANNING, InitiativeStatus.APPROVED].includes(
                    i.status,
                );
    ...
    if (selectedInitiative) {
        return <InitiativeTaskBoard initiative={selectedInitiative} onClose={() => setSelectedInitiative(null)} />;
    }
            ...
            <div className="flex-1 overflow-x-auto p-4 md:p-6 bg-slate-50 dark:bg-navy-950">
                <div className="flex gap-3 h-full min-w-[1000px]">
                    {columns.map((col) => (
                        <div
                            key={col.id}
                            className="flex-1 flex flex-col bg-slate-100 dark:bg-navy-950/30 rounded-lg border border-slate-200 dark:border-white/5"
                        >
                            {/* Column Header */}
                            <div
                                className={`p-3 border-b border-slate-200 dark:border-white/5 flex justify-between items-center ${col.id === 'Blocked' ? 'bg-red-50 dark:bg-red-500/5' : ''}`}
                            >
