...
export const FullStep5Workspace: React.FC<FullStep5WorkspaceProps> = ({
  ...
  const columns: { id: string; label: string; color: string }[] = [
    ...
    { id: 'Blocked', label: t.columns.blocked, color: 'border-danger-500/50' },
  ...
  const doneCount = initiatives.filter((i) => i.status === InitiativeStatus.CLOSED).length;
  const blockedCount = initiatives.filter((i) => i.status === InitiativeStatus.IN_EXECUTION).length;
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-navy-900 font-sans">
      {/* Header with KPIs */}
      <div className="h-14 border-b border-slate-200 dark:border-navy-700 flex items-center px-6 bg-white dark:bg-navy-900 shrink-0 gap-8 overflow-x-auto shadow-sm dark:shadow-none">
        ...
        <div className="flex flex-col">
          ...
          <span
            className={`text-lg font-bold ${blockedCount > 0 ? 'text-danger-600 dark:text-danger-400' : 'text-slate-600'}`}
          >
            {blockedCount}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`px-3 py-1.5 rounded text-xs font-bold border transition-colors ${showMyOnly ? 'bg-blue-600 border-blue-500 text-white' : 'bg-slate-50 dark:bg-navy-800 border-slate-200 dark:border-navy-700 text-slate-500 dark:text-slate-400 hover:text-navy-900 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-navy-700'}`}
          >
            My Initiatives
          </button>
          <button
            onClick={() => setShowBlockedOnly(!showBlockedOnly)}
            className={`px-3 py-1.5 rounded text-xs font-bold border transition-colors ${showBlockedOnly ? 'bg-danger-100 dark:bg-danger-900/50 border-danger-500 text-danger-600 dark:text-danger-200' : 'bg-slate-50 dark:bg-navy-800 border-slate-200 dark:border-navy-700 text-slate-500 dark:text-slate-400 hover:text-navy-900 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-navy-700'}`}
          >
            Show Blocked
          </button>
        </div>
      ...
      <div className="flex-1 overflow-x-auto p-4 md:p-6 bg-slate-50 dark:bg-navy-950">
        <div className="flex gap-3 h-full min-w-[1000px]">
          {columns.map((col) => (
            <div
              key={col.id}
              className="flex-1 flex flex-col bg-slate-100 dark:bg-navy-950/30 rounded-lg border border-slate-200 dark:border-navy-700"
            >
              {/* Column Header */}
              <div
                className={`p-3 border-b border-slate-200 dark:border-navy-700 flex justify-between items-center ${col.id === 'Blocked' ? 'bg-danger-50 dark:bg-danger-500/5' : ''}`}
              >
                ...
                {getInitiativesByStatus(col.id).map((init) => (
                  <div
                    ...
                    className={`
                             ...
                             ${col.id === 'Blocked' ? 'border-danger-200 dark:border-danger-500/20' : 'border-slate-200 dark:border-navy-700'}
                           `}
                  >
                    {/* Axis Label */}
                    ...
                    <h5 className="text-navy-900 dark:text-white text-xs font-medium mb-2 leading-snug">
                      {init.name}
                    </h5>
                      ...
                      <div className="flex items-center gap-1.5">
                        {init.ownerBusiness ? (
                          <div
                            ...
                            title={`${init.ownerBusiness.firstName} ${init.ownerBusiness.lastName}`}
                          >
                            <User size={10} />
                            <span className="max-w-[50px] truncate">
                              {init.ownerBusiness.lastName}
                            </span>
                          </div>
                        ) : (
                      ...
                      <div className="flex items-center gap-1.5">
                        {init.endDate ? (
                          <div className="flex items-center gap-1" title={init.endDate}>
                            <Calendar size={10} />
                            <span>
                              {new Date(init.endDate).toLocaleDateString(undefined, {
                                month: 'numeric',
                                day: 'numeric',
                              })}
                            </span>
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-slate-900/5 dark:group-hover:bg-black/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-lg pointer-events-none">
                      <div className="bg-white/90 dark:bg-navy-950/80 px-2 py-1 rounded text-[10px] text-navy-900 dark:text-white backdrop-blur flex items-center gap-1 border border-slate-200 dark:border-navy-700 shadow-sm">
