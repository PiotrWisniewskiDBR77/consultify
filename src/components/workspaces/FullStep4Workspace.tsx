...
export const FullStep4Workspace: React.FC<FullStep4WorkspaceProps> = ({
  ...
  const handleRangeChange = (
    ...
    if (type === 'cost') {
      ...
      onUpdateInitiative({
        ...init,
        estimatedAnnualBenefit: val,
        benefitRange: rangeLabel as BenefitRange,
      });
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-navy-900">
      {/* Header */}
      <div className="h-14 border-b border-white/5 flex flex-col justify-center p-4 bg-navy-900 shrink-0">
        <div className="flex justify-between items-center mb-1">
          <span className="text-xs font-semibold text-white tracking-wide">
            {t.intro.substring(0, 30)}...
          </span>
          <span className="text-[10px] text-slate-500 dark:text-slate-400">STEP 4</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-navy-950/50 border border-white/5 p-3 rounded-xl">
            ...
            <div className="text-xl font-bold text-blue-400">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">{t.summary.years}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end">
          <div className="bg-navy-950 p-1 rounded-lg border border-white/10 flex gap-1">
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-medium flex items-center gap-2 transition-colors ${inputMode === 'range' ? 'bg-purple-600 text-white' : 'text-slate-400 dark:text-slate-500 hover:text-white'}`}
            >
              <Sliders size={14} /> Estimate Ranges
            </button>
            <button
              onClick={() => setInputMode('precise')}
              className={`px-3 py-1.5 rounded text-xs font-medium flex items-center gap-2 transition-colors ${inputMode === 'precise' ? 'bg-purple-600 text-white' : 'text-slate-400 dark:text-slate-500 hover:text-white'}`}
            >
        ...
        <div className="border border-white/10 rounded-xl overflow-hidden bg-navy-950/50 shadow-xl">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-navy-900 border-b border-white/10 text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">
            ...
            <tbody>
              {initiatives.map((init) => (
                <tr
                  key={init.id}
                  className="border-b border-white/5 hover:bg-slate-50 dark:hover:bg-navy-800/20 transition-colors"
                >
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    {inputMode === 'range' ? (
                      ...
                      <div className="relative">
                        <span className="absolute left-2 top-1.5 text-slate-500 dark:text-slate-400 text-xs">
                          $
                        </span>
                        <input
                          type="number"
