...
export const FullStep3Workspace: React.FC<FullStep3WorkspaceProps> = ({
  ...
  const workload = quarters.reduce(
    (acc, q) => {
      ...
      inits.forEach((i) => {
        const effort = i.effortProfile
          ? (i.effortProfile.analytical + i.effortProfile.operational + i.effortProfile.change) / 3
          : 1;
        ...
        const points = i.effortProfile
          ? i.effortProfile.analytical + i.effortProfile.operational + i.effortProfile.change
          : 3;
        totalEffort += points;
  ...
  return (
    <div className="flex flex-col h-full bg-transparent">
      {/* Header */}
      <div className="h-14 border-b border-slate-200 dark:border-navy-700 flex flex-col justify-center px-6 bg-white dark:bg-navy-900 shrink-0">
        <div className="flex justify-between items-center mb-0.5">
          <span className="text-sm font-semibold text-navy-900 dark:text-white tracking-wide truncate max-w-[500px]">
            Strategic Roadmap Builder
          </span>
          <div className="flex items-center gap-3">
            {/* View Toggle */}
            <div className="bg-slate-100 dark:bg-navy-950 p-0.5 rounded-md border border-slate-200 dark:border-navy-700 flex gap-0.5">
              <button
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
        ...
        <div className="flex items-center justify-between bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 rounded-xl p-3 shadow-sm">
        ...
        <div className="bg-white dark:bg-navy-950/50 border border-slate-200 dark:border-navy-700 rounded-xl p-4 shadow-sm dark:shadow-none">
          ...
          <div className="grid grid-cols-4 lg:grid-cols-8 gap-2">
            {quarters.map((q) => {
              const { count, totalEffort, changeEffort } = workload[q] || {
                count: 0,
                totalEffort: 0,
                changeEffort: 0,
              };
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-slate-900/90 dark:bg-navy-950/90 rounded flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10 gap-1 backdrop-blur-sm">
                    ...
                    {isWarning && (
                      <button className="text-[9px] font-bold text-white bg-danger-500 px-2 py-1 rounded-full w-20 hover:scale-105 transition-transform">
                        Lighten
                      </button>
                    )}
                  </div>
                  ...
                  <div className="flex items-end gap-1">
                    <span
                      className={`text-sm font-bold ${isWarning ? 'text-danger-500 dark:text-danger-400' : 'text-navy-900 dark:text-white'}`}
                    >
                      {count}
                    </span>
                  ...
                  {isWarning && (
                    <div className="absolute top-1 right-1">
                      <AlertTriangle size={10} className="text-danger-500 animate-pulse" />
                    </div>
                  )}
                  ...
                  {count > 0 && (
                    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-32 bg-white dark:bg-navy-900 text-slate-900 dark:text-white text-[10px] p-2 rounded shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none z-50 transition-opacity">
                      ...
                      <div className="flex justify-between text-danger-300">
                        <span>Change:</span> <span>{changeEffort} pts</span>
                      </div>
                      {isHighChange && (
                        <div className="mt-1 text-danger-400 font-bold">
                          High Change Fatigue Risk!
                        </div>
                      )}
                      {isOverloaded && (
                        <div className="mt-1 text-danger-400 font-bold">Capacity Overload!</div>
                      )}
                    </div>
                  )}
                </div>
              ...
              <tbody>
                {initiatives.map((init) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      {init.strategicRole ? (
                        <span className="text-[10px] px-2 py-0.5 rounded bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-navy-700">
                          {init.strategicRole}
                        </span>
                      ) : (
                      ...
                      {init.effortProfile ? (
                        <div className="flex items-end gap-1 h-4 w-16">
                          <div
                            ...
                            style={{
                              height: `${(init.effortProfile.analytical / 5) * 100}%`,
                            }}
                            title={`Analytical: ${init.effortProfile.analytical}`}
                          ></div>
                          <div
                            className="bg-emerald-500 w-1/3 rounded-sm"
                            style={{
                              height: `${(init.effortProfile.operational / 5) * 100}%`,
                            }}
                            title={`Operational: ${init.effortProfile.operational}`}
                          ></div>
                          <div
                            className="bg-danger-500 w-1/3 rounded-sm"
                            style={{ height: `${(init.effortProfile.change / 5) * 100}%` }}
                            title={`Change: ${init.effortProfile.change}`}
                          ></div>
                        </div>
                      ) : (
                        <span className="text-[10px] text-slate-600 dark:text-slate-500">-</span>
                    ...
                    <td className="p-3">
                      <select
                        value={init.quarter}
                        onChange={(e) =>
                          onUpdateInitiative({ ...init, quarter: e.target.value as Quarter })
                        }
                        className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 text-[10px] rounded px-1.5 py-1 outline-none focus:border-blue-500 text-navy-900 dark:text-slate-300 w-16"
                      >
                        {quarters.map((q) => (
                    ...
                    <td className="p-3">
                      <select
                        value={init.wave}
                        onChange={(e) =>
                          onUpdateInitiative({ ...init, wave: e.target.value as Wave })
                        }
                        className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 text-[10px] rounded px-1.5 py-1 outline-none focus:border-blue-500 text-navy-900 dark:text-slate-300 w-20"
                      >
                        {waves.map((w) => (
