      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-thin scrollbar-thumb-navy-700">
          ...
          <div className="space-y-3">
            {sessionData.challengesMap && sessionData.challengesMap.length > 0 ? (
              sessionData.challengesMap.map((challenge) => (
                <div
                  key={challenge.id}
                  className="p-3 bg-slate-50/30 dark:bg-navy-950/20 rounded-lg border border-white/5 flex flex-col gap-2"
                >
              ...
              <div className="text-center py-6 text-slate-500 dark:text-slate-400 italic border border-dashed border-white/10 rounded-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              {sessionData.constraints && sessionData.constraints.length > 0 ? (
                sessionData.constraints.map((c) => (
                  <div
                    key={c.id}
                    className="text-sm p-2 bg-slate-50/30 dark:bg-navy-950/20 rounded border border-white/5 flex justify-between"
                  >
          ...
          <div className="bg-navy-950/50 border border-white/10 rounded-xl p-6 relative">
            ...
            <div className="border border-dashed border-white/10 rounded bg-slate-50/30 dark:bg-navy-950/20 h-24 flex items-center justify-center text-xs text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-navy-800/40 cursor-pointer transition-colors">
