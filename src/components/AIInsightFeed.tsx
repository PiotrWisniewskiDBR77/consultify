...
export const AIInsightFeed: React.FC<AIInsightFeedProps> = ({ session }) => {
  ...
  const generateInsights = React.useCallback(async () => {
    ...
    try {
      ...
      const results = await Agent.analyzeSessionForInsights(
        session,
        currentUser?.companyName || 'Client'
      );
      setInsights(results);
    } catch (e) {
    ...
    <div className="bg-white dark:bg-navy-800 rounded-xl shadow-sm border border-slate-200 dark:border-navy-700 overflow-hidden flex flex-col h-full">
      <div className="p-4 border-b border-slate-100 dark:border-navy-700 flex justify-between items-center bg-slate-50/50 dark:bg-white/5">
        ...
        <button
          ...
          disabled={loading}
          className={`p-1.5 rounded hover:bg-white dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 transition-all ${loading ? 'animate-spin' : ''}`}
        >
          <RefreshCw size={14} />
        </button>
      </div>
