...
import type { ActionKind, VerificationResult, WorkspaceCapability, WorkspaceFindingDetail } from './workspaceApi';
...
export const RemediationPanel: React.FC<RemediationPanelProps> = ({
  ...
  const handleProposeAction = useCallback(() => {
    ...
    run(() =>
      workspaceApi.proposeAction(finding.id, { actionKind, title: actionTitle.trim() }).then(() => setActionTitle(''))
    );
  }, [actionTitle, actionKind, finding.id, run]);

  const handleApprove = useCallback((id: string) => run(() => workspaceApi.approveAction(id)), [run]);

  const handleReportImplementation = useCallback(
    (id: string) => run(() => workspaceApi.reportImplementation(id, { note: t('Wdrożono', 'Implemented') })),
    [run, t]
  );
  ...
  return (
    <div data-testid="remediation-panel" className="space-y-5">
      ...
      <section data-testid="chain-link-korekcja" className="space-y-2">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-c-text-muted">{t('Korekcja', 'Correction')}</h4>
        <p className="text-xs text-c-text-muted">
          {t('Usunięcie bezpośredniego skutku niezgodności — nie zamyka ustalenia samo z siebie.', 'Removes the direct effect of the nonconformity — does not close the finding by itself.')}
        </p>
        {correctionActions.map((a) => (
          <div key={a.id} className="flex items-center justify-between rounded-token-sm border border-c-border-subtle px-2 py-1.5 text-xs">
        ...
        {canPropose ? (
          <div className="flex gap-2">
            ...
            />
            <PreviewActionButton variant="neutral" label={t('Dodaj', 'Add')} onClick={handleProposeCorrection} disabled={!correctionTitle.trim()} />
          </div>
        ) : (
          <p className="text-xs text-c-text-muted">{t('Wymaga uprawnienia action.propose.', 'Requires the action.propose permission.')}</p>
        )}
      </section>

      {/* Ogniwo: przyczyna źródłowa */}
      <section data-testid="chain-link-przyczyna-zrodlowa" className="space-y-2">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-c-text-muted">{t('Przyczyna źródłowa', 'Root cause')}</h4>
        {canUpdateFinding ? (
          <div className="flex gap-2">
            <input
              type="text"
              ...
              className="flex-1 rounded-token-sm border border-c-border bg-c-surface px-2 py-1.5 text-sm text-c-text"
            />
            <PreviewActionButton variant="neutral" label={t('Zapisz', 'Save')} onClick={handleSaveRootCause} disabled={!rootCause.trim()} />
          </div>
        ) : (
          <p className="text-xs text-c-text-muted">{finding.rootCause || t('Nie ustalono jeszcze przyczyny źródłowej.', 'Root cause not established yet.')}</p>
        )}
      </section>

      {/* Ogniwo: działanie korygujące */}
      <section data-testid="chain-link-dzialanie-korygujace" className="space-y-2">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-c-text-muted">{t('Działanie korygujące', 'Corrective action')}</h4>
        <p className="text-xs text-c-text-muted">
          {t('Usuwa PRZYCZYNĘ — niezgodność nie może zostać zamknięta bez co najmniej jednego takiego działania.', 'Removes the CAUSE — a nonconformity cannot be closed without at least one such action.')}
        </p>
        {systemicActions.map((a) => (
          <div key={a.id} className="space-y-1 rounded-token-sm border border-c-border-subtle px-2 py-1.5 text-xs">
            ...
            <div className="flex gap-2">
              {canApprove && a.status === 'proposed' && (
                <PreviewActionButton variant="positive" label={t('Zatwierdź', 'Approve')} icon={CheckCircle2} onClick={() => handleApprove(a.id)} />
              )}
              {canReportImplementation && (a.status === 'approved' || a.status === 'in_progress' || a.status === 'overdue') && (
                <PreviewActionButton variant="neutral" label={t('Zgłoś wdrożenie', 'Report implementation')} icon={Wrench} onClick={() => handleReportImplementation(a.id)} />
              )}
            </div>
          </div>
        ))}
          ...
          <div className="space-y-2">
            <div className="flex gap-2">
              ...
              <input
                ...
                className="flex-1 rounded-token-sm border border-c-border bg-c-surface px-2 py-1.5 text-sm text-c-text"
              />
              <PreviewActionButton variant="neutral" label={t('Dodaj', 'Add')} onClick={handleProposeAction} disabled={!actionTitle.trim()} />
            </div>
          </div>
        ) : (
          <p className="text-xs text-c-text-muted">{t('Wymaga uprawnienia action.propose.', 'Requires the action.propose permission.')}</p>
      ...
      <section data-testid="chain-link-wlasciciel-termin" className="space-y-1">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-c-text-muted">{t('Właściciel / termin', 'Owner / due date')}</h4>
        {finding.correctiveActions.length === 0 ? (
          <p className="text-xs text-c-text-muted">{t('Brak działań do przypisania.', 'No actions to assign yet.')}</p>
        ) : (
      ...
      <section data-testid="chain-link-weryfikacja-skutecznosci" className="space-y-2">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-c-text-muted">{t('Weryfikacja skuteczności', 'Effectiveness verification')}</h4>
        {systemicActions.map((a) => {
          const verification = finding.verifications.find((v) => v.correctiveActionId === a.id && v.verificationKind === 'effectiveness');
          const isOwnAction = !!currentUserId && (a.ownerUserId === currentUserId || a.implementedBy === currentUserId);
          return (
            <div key={a.id} className="space-y-1 rounded-token-sm border border-c-border-subtle px-2 py-1.5 text-xs">
              <span className="text-c-text">{a.title}</span>
              {!verification && a.status === 'implemented' && canVerify && (
                <PreviewActionButton variant="neutral" label={t('Zaplanuj weryfikację', 'Plan verification')} onClick={() => handlePlanVerification(a.id)} />
              )}
              ...
              {verification?.performedAt && (
                <StatusChip label={verification.result || '—'} tone={verification.result === 'effective' ? 'success' : 'danger'} />
              )}
            </div>
          );
        })}
        {systemicActions.length === 0 && (
          <p className="text-xs text-c-text-muted">{t('Brak działań korygujących do weryfikacji.', 'No corrective actions to verify yet.')}</p>
        )}
      </section>

      {/* Ogniwo: zamknięcie */}
      <section data-testid="chain-link-zamkniecie" className="space-y-2">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-c-text-muted">{t('Zamknięcie', 'Closure')}</h4>
        {finding.status === 'closed' ? (
          ...
          <div className="space-y-2">
            <textarea
              ...
              rows={2}
            />
            <PreviewActionButton variant="positive" label={t('Zamknij ustalenie', 'Close finding')} icon={Hammer} onClick={handleClose} disabled={!closeNote.trim()} />
          </div>
        ) : (
          <p className="text-xs text-c-text-muted">
            {t('Zamknięcie wymaga roli lead auditor/reviewer i weryfikacji skuteczności działań.', 'Closing requires the lead auditor/reviewer role and verified effectiveness of actions.')}
          </p>
        )}
      </section>
    </div>
