...
import { AlertCircle, CheckCircle, HelpCircle, Info, Table, User } from 'lucide-react';
...
export const RACIMatrix: React.FC<RACIMatrixProps> = ({ projectId }) => {
  ...
  Object.values((matrix as any).matrix).forEach((raciByType: any) => {
    Object.values(raciByType).forEach((users: any) => {
      if (Array.isArray(users)) {
        users.forEach((user: any) => allUsers.add(JSON.stringify(user)));
      }
    });
  });
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {Object.entries(RACI_LABELS).map(([key, value]) => (
                <div key={key} className="flex items-center gap-2">
                  <span
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${value.color}`}
                  >
                    {value.label}
                  </span>
                  <div>
              ...
              {(['R', 'A', 'C', 'I'] as RACIType[]).map((type) => (
                ...
                >
                  <div className="flex flex-col items-center gap-1">
                    <span
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${RACI_LABELS[type].color}`}
                    >
                      {type}
                    </span>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {OBJECT_TYPES.map((objectType) => {
              ...
              return (
                <tr
                  key={objectType}
                  className="hover:bg-gray-50 dark:bg-navy-800 dark:hover:bg-gray-700/30"
                >
