...
export const PhaseIndicator: React.FC<PhaseIndicatorProps> = ({ compact = false }) => {
  const { currentPhase, phaseNumber, totalPhases, gateStatus, isLoading, projectName } =
    usePMOStore();
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 rounded-lg ${getBgColor()} border transition-all`}
    >
      {getStatusIcon()}
