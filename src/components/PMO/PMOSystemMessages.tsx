...
export const PMOSystemMessages: React.FC<PMOSystemMessagesProps> = ({
  ...
  const { systemMessages, blockingIssues, getCriticalMessages, getWarningMessages } =
    usePMOContext();
        ...
        {criticalMessages.slice(0, maxMessages).map((msg, idx) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-danger-400 font-medium">{msg.message}</p>
            </div>
          </div>
        ))}
...
export const PMOWarningBadge: React.FC = () => {
  ...
  return (
    <span className="flex items-center gap-1 px-2 py-0.5 bg-danger-500/20 text-danger-400 rounded text-xs font-medium border border-danger-500/30">
