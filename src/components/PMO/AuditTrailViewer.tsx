...
export const AuditTrailViewer: React.FC<AuditTrailViewerProps> = ({ projectId }) => {
    ...
    const fetchAuditTrail = async () => {
        try {
            setLoading(true);
            const data = await Api.get(`/pmo-domains/projects/${projectId}/audit-trail?range=${dateRange}`);
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Filters */}
            <div className="flex flex-wrap gap-4 bg-white dark:bg-navy-800 rounded-xl p-4 border border-slate-100 dark:border-white/5">
                {/* Search */}
                <div className="flex-1 min-w-[200px] relative">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-navy-900 border border-slate-200 dark:border-white/10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                    />
                </div>
            ...
            {/* Entries List */}
            <div className="bg-white dark:bg-navy-800 rounded-xl border border-slate-100 dark:border-white/5 overflow-hidden">
                {loading ? (
