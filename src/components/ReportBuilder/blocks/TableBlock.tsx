          ...
          <tr className="border-b border-slate-200 dark:border-navy-700">
            {headers.map((h, i) => (
              <th key={i} className="p-1 min-w-[120px] relative group">
                <input
                  className="w-full bg-slate-50 dark:bg-navy-900 border border-transparent hover:border-slate-300 dark:border-navy-700 rounded px-2 py-1 font-bold text-slate-700 dark:text-slate-200 focus:outline-none focus:border-blue-500"
                  value={h}
                  onChange={(e) => updateHeader(i, e.target.value)}
                />
                <button
                  onClick={() => deleteColumn(i)}
                  className="absolute -top-2 right-0 opacity-0 group-hover:opacity-100 bg-danger-100 text-danger-500 rounded-full p-0.5 hover:bg-danger-200"
                  title="Delete Column"
                >
                  <Trash2 size={10} />
                </button>
            ...
            >
              {row.map((cell, cI) => (
                <td key={cI} className="p-1">
                  <input
                    className="w-full bg-transparent border border-transparent hover:border-slate-200 dark:border-navy-700 rounded px-2 py-1 text-slate-600 dark:text-slate-300 focus:outline-none focus:border-blue-500 focus:bg-white dark:focus:bg-navy-950"
                    value={cell}
              ...
              <td className="w-8 text-center">
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-1 text-slate-600 dark:text-slate-500 hover:text-danger-500"
                >
                  <Trash2 size={12} />
                </button>
              </td>
