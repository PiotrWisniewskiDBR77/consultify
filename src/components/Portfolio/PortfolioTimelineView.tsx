...
import { AlertTriangle, ArrowRight, Calendar, ChevronLeft, ChevronRight, ZoomIn, ZoomOut } from 'lucide-react';
import React, { useEffect, useMemo, useRef, useState } from 'react';

import { getAxisColor, getPriorityColors, getStatusColors, TIMELINE_COLORS } from '../../config/portfolioColors';
    ...
    return (
        ...
        >
            ...
            <div className="relative h-full flex items-center gap-2 px-3 overflow-hidden">
                ...
                {/* Name */}
                <span className={`text-sm font-medium truncate ${statusColors.text} ${statusColors.textDark}`}>
                    {initiative.name}
                </span>
...
export const PortfolioTimelineView: React.FC<PortfolioTimelineViewProps> = ({
    ...
    return (
        <div className="h-full flex flex-col">
            {/* Timeline Controls */}
            <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-navy-950">
                ...
                <div className="flex items-center gap-1 bg-white dark:bg-navy-900 rounded-lg p-1 border border-slate-200 dark:border-white/10">
                    <button
                        onClick={() => setZoom(1)}
            ...
            <div ref={containerRef} className="flex-1 overflow-auto">
                <div className="min-w-[800px]">
                    {/* Quarter Headers */}
                    <div className="sticky top-0 z-10 flex bg-white dark:bg-navy-900 border-b border-slate-200 dark:border-white/10">
                        {quarters.map((q, idx) => (
                            <div
                                key={`${q.year}-${q.quarter}`}
                                className="flex-1 px-4 py-3 text-center border-r border-slate-100 dark:border-white/5 last:border-r-0"
                            >
                                <div className="text-sm font-semibold text-navy-900 dark:text-white">Q{q.quarter}</div>
                    ...
                    <div className="relative">
                        ...
                        <div className="absolute inset-0 flex pointer-events-none">
                            {quarters.map((q, idx) => (
                                <div
                                    ...
                                    className="flex-1 border-r border-slate-100 dark:border-white/5 last:border-r-0"
                                />
                            ))}
                        </div>
                        ...
                        {initiativeRows.length === 0 ? (
                            ...
                            initiativeRows.map((row, rowIdx) => (
                                <div
                                    key={rowIdx}
                                    className="relative h-14 border-b border-slate-50 dark:border-white/5"
                                >
                                    {row.map((initiative) => {
                                        const startIdx = getQuarterIndex(initiative.targetQuarter);
            ...
            {/* Legend */}
            <div className="shrink-0 flex items-center gap-6 px-4 py-2 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-navy-950 text-xs text-slate-500 dark:text-slate-400">
