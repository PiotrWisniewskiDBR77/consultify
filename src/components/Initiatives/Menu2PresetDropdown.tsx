  ...
  return (
    <div ref={containerRef} className={`relative ${className}`} data-testid={testId}>
      <button
        ...
        className={`inline-flex h-9 items-center gap-2 rounded-lg border border-c-border-subtle bg-c-surface px-3 text-xs font-medium text-c-text-secondary transition-colors duration-150 hover:bg-c-surface-raised focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-c-focus ${
          isOpen ? 'bg-c-surface-raised text-c-text' : ''
        }`}
      >
        <Filter size={14} className="text-c-text-muted" aria-hidden="true" />
        <span>
          {label}: {selected?.label ?? ''}
        </span>
        {selected?.count !== undefined && (
          <span className="rounded-full bg-c-surface-raised px-1.5 py-0.5 text-[11px] text-c-text-muted">
            {selected.count}
          </span>
        )}
        <ChevronDown
          size={14}
          className={`text-c-text-muted transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
          aria-hidden="true"
        />
      </button>
