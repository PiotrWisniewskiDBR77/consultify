...
export const PlanScenarioSurface: React.FC<Props> = ({
  ...
  /**
   * SCALENIE K3+K5 (07.09): popyt per rola z sekcji „Obciazenie rol" zapisuje sie
   * TYM SAMYM `persistScenario` co daty i zakres. Edytor mial wlasny odczyt planu
   * i wlasny licznik wersji — na jednej karcie dawaloby to dwoch pisarzy jednego
   * agregatu i 409 po pierwszej zmianie po drugiej stronie.
   */
  const changeRoleDemand = async (
    initiativeId: string,
    roleDemand: Array<{ roleId: string; roleLabel: string; fte: number }>
  ): Promise<boolean> => {
    ...
    return persistScenario(
      {
        ...draft,
        windows: draft.windows.map((window) =>
          window.initiativeId === initiativeId ? { ...window, roleDemand } : window
        ),
      },
      'UPDATE'
    );
    ...
    return (
      <>
        <PlanCard
          ...
          onRoleDemandChange={(initiativeId, roleDemand) =>
            changeRoleDemand(initiativeId, roleDemand)
          }
