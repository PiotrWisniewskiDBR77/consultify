...
export const KpisSection: React.FC<InitiativeSectionProps> = ({ sectionType, expanded, onToggle }) => {
  ...
  const addKpi = () => {
    ...
    setKpis([...kpis, { id: `kpi-${Date.now()}`, name: newName, unit: newUnit, baseline: newBaseline, target: newTarget }]);
    setNewName(''); setNewUnit(''); setNewBaseline(''); setNewTarget(''); setShowAdd(false);
  };
  ...
  return (
    <CollapsibleSection
      ...
      expanded={expanded}
      onToggle={onToggle}
      badge={kpis.length > 0 ? <span className="text-xs text-slate-400">{kpis.length}</span> : undefined}
      actions={
        <div className="flex items-center gap-2">
          <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <Plus size={14} /><span>{isPolish ? 'Nowy' : 'New'}</span>
          </motion.button>
          <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
            whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            onClick={async (e) => {
              ...
              if (result?.parsedContent && Array.isArray(result.parsedContent)) {
                setKpis((prev) => [...prev, ...result.parsedContent.map((k: any) => ({
                  name: k.name || '', unit: k.unit || '', baseline: k.baseline || '', target: k.target || '',
                }))]);
              }
            }}
            ...
            {isGeneratingAI === 'kpis' ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}
            <span>AI</span>
          </motion.button>
        </div>
      }
    >
      {showAdd && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder={isPolish ? 'Nazwa KPI...' : 'KPI name...'}
            className="w-full px-3 py-2 rounded-lg bg-white dark:bg-navy-800 border border-slate-200 dark:border-navy-600 text-sm" autoFocus />
          <div className="grid grid-cols-3 gap-2">
            <input type="text" value={newUnit} onChange={(e) => setNewUnit(e.target.value)} placeholder={isPolish ? 'Jednostka' : 'Unit'}
              className="px-3 py-2 rounded-lg bg-white dark:bg-navy-800 border border-slate-200 dark:border-navy-600 text-sm" />
            <input type="text" value={newBaseline} onChange={(e) => setNewBaseline(e.target.value)} placeholder="Baseline"
              className="px-3 py-2 rounded-lg bg-white dark:bg-navy-800 border border-slate-200 dark:border-navy-600 text-sm" />
          ...
          <div className="flex justify-end gap-2">
            ...
            <button onClick={addKpi} disabled={!newName.trim()} className="px-3 py-1.5 text-xs bg-cyan-500 text-white rounded-lg disabled:opacity-50">{isPolish ? 'Dodaj' : 'Add'}</button>
          </div>
        </motion.div>
      )}
      ...
      {kpis.length === 0 && !showAdd ? (
        <div className="text-center py-6 border-2 border-dashed border-slate-200 dark:border-navy-700 rounded-xl">
          ...
          <p className="text-sm text-slate-400">{isPolish ? 'Brak zdefiniowanych KPI' : 'No KPIs defined yet'}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {kpis.map((kpi) => (
            <div key={kpi.id} className="p-3 rounded-xl bg-slate-50/50 dark:bg-navy-800/50 border border-slate-200/50 dark:border-navy-700/50">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{kpi.name}</span>
                <button onClick={() => setKpis(kpis.filter((k) => k.id !== kpi.id))} className="text-slate-400 hover:text-red-500"><X size={14} /></button>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="text-center p-2 rounded-lg bg-slate-100/50 dark:bg-navy-900/50">
                  ...
                  <div className="text-sm font-semibold text-slate-600 dark:text-slate-300">{kpi.baseline || '-'} {kpi.unit}</div>
                </div>
                <div className="text-center p-2 rounded-lg bg-cyan-50/50 dark:bg-cyan-500/5 border border-cyan-200/30 dark:border-cyan-500/20">
                  <div className="text-[10px] text-cyan-500 uppercase mb-0.5">{isPolish ? 'Obecny' : 'Current'}</div>
                  <div className="text-sm font-semibold text-cyan-600 dark:text-cyan-400">{kpi.current || '-'} {kpi.unit}</div>
                </div>
                <div className="text-center p-2 rounded-lg bg-emerald-50/50 dark:bg-emerald-500/5 border border-emerald-200/30 dark:border-emerald-500/20">
                  ...
                  <div className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">{kpi.target || '-'} {kpi.unit}</div>
                </div>
              </div>
            </div>
          ))}
