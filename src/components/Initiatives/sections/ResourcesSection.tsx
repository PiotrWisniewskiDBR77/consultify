...
export const ResourcesSection: React.FC<InitiativeSectionProps> = ({
  sectionType,
  expanded,
  onToggle,
}) => {
    ...
    <CollapsibleSection
      ...
      badge={
        <div className="flex items-center gap-2">
          {resources.length > 0 && (
            <span className="text-xs text-slate-400">{totalFTE.toFixed(1)} FTE</span>
          )}
        </div>
      }
      actions={
            ...
            onClick={async (e) => {
              ...
              if (result?.parsedContent) {
                ...
                if (data.fteAllocation?.length) {
                  setResources((prev) => [
                    ...prev,
                    ...data.fteAllocation.map((a: any, i: number) => ({
                      id: `ai-${Date.now()}-${i}`,
                      name: a.role || '',
                      role: a.role || '',
          ...
          >
            <div className="grid grid-cols-2 gap-2">
              ...
              <input
                ...
                placeholder={isPolish ? 'Rola w projekcie' : 'Project role'}
                className="px-3 py-2 rounded-lg bg-white dark:bg-navy-800 border border-slate-200 dark:border-navy-600 text-sm"
              />
            </div>
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="flex-1"
              />
              <span className="text-sm font-medium text-teal-500 min-w-[40px] text-right">
                {newAlloc}%
              </span>
            </div>
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs bg-teal-500 text-white rounded-lg disabled:opacity-50"
              >
                {isPolish ? 'Dodaj' : 'Add'}
              </button>
            </div>
        ...
        {resources.length === 0 && !showAdd ? (
          ...
          <div className="space-y-2">
            {resources.map((r) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-teal-500/20 flex items-center justify-center text-xs font-bold text-teal-500">
                    {r.allocation}%
                  </div>
                  <div>
                ...
                <button
                  onClick={() => setResources(resources.filter((x) => x.id !== r.id))}
        ...
        <div className="pt-3 border-t border-slate-200 dark:border-navy-700">
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {tools.map((tool, i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-teal-100 dark:bg-teal-500/20 text-teal-700 dark:text-teal-300"
              >
                {tool}
                <button
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-2 rounded-lg text-teal-500 border border-teal-200 dark:border-teal-500/30 disabled:opacity-50"
            >
              <Plus size={16} />
            </button>
          </div>
