...
export const TargetStateSection: React.FC<InitiativeSectionProps> = ({
  ...
  const [targetDescription, setTargetDescription] = useState(typeof targetData === 'object' ? targetData.description || '' : typeof targetData === 'string' ? targetData : '');
  const [successCriteria, setSuccessCriteria] = useState<string[]>(typeof targetData === 'object' ? targetData.successCriteria || [] : []);
  const [deliverables, setDeliverables] = useState<string[]>(typeof targetData === 'object' ? targetData.deliverables || [] : []);
  const [newCriteria, setNewCriteria] = useState('');
  const [newDeliverable, setNewDeliverable] = useState('');

  const filledCount = [targetDescription, successCriteria.length > 0, deliverables.length > 0].filter(Boolean).length;
  ...
  return (
    <CollapsibleSection
      ...
      expanded={expanded}
      onToggle={onToggle}
      badge={filledCount > 0 ? <span className="text-xs text-slate-400">{filledCount}/3</span> : undefined}
      actions={
        <motion.button
        ...
        >
          {isGeneratingAI === 'targetState' ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}
          <span>AI</span>
        </motion.button>
      }
    >
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={isPolish ? 'Opisz pożądany stan końcowy po wdrożeniu inicjatywy...' : 'Describe the desired end state after initiative completion...'}
          />
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mb-2">{isPolish ? 'Mierzalne warunki uznania inicjatywy za zakończoną sukcesem' : 'Measurable conditions for considering the initiative successful'}</p>
          <div className="space-y-2 mb-3">
            {successCriteria.map((c, i) => (
              <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-emerald-50/50 dark:bg-emerald-500/5 border border-emerald-200/50 dark:border-emerald-500/20">
                <div className="w-5 h-5 rounded-full bg-emerald-500/20 flex items-center justify-center text-xs font-bold text-emerald-500">✓</div>
          ...
          <div className="flex gap-2">
            <input
              type="text" value={newCriteria} onChange={(e) => setNewCriteria(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter' && newCriteria.trim()) { setSuccessCriteria([...successCriteria, newCriteria.trim()]); setNewCriteria(''); } }}
            ...
            />
            <button onClick={() => { if (newCriteria.trim()) { setSuccessCriteria([...successCriteria, newCriteria.trim()]); setNewCriteria(''); } }} disabled={!newCriteria.trim()}
        ...
        <div>
          ...
          <div className="space-y-2 mb-3">
            {deliverables.map((d, i) => (
              <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-blue-50/50 dark:bg-blue-500/5 border border-blue-200/50 dark:border-blue-500/20">
                <div className="w-5 h-5 rounded-full bg-blue-500/20 flex items-center justify-center text-xs font-bold text-blue-500">{i + 1}</div>
                <span className="flex-1 text-sm text-slate-700 dark:text-slate-300">{d}</span>
                <button onClick={() => setDeliverables(deliverables.filter((_, j) => j !== i))} className="text-slate-400 hover:text-red-500"><X size={14} /></button>
              </div>
            ))}
          </div>
          <div className="flex gap-2">
            <input
              type="text" value={newDeliverable} onChange={(e) => setNewDeliverable(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter' && newDeliverable.trim()) { setDeliverables([...deliverables, newDeliverable.trim()]); setNewDeliverable(''); } }}
              placeholder={isPolish ? 'Dodaj produkt...' : 'Add deliverable...'}
              className="flex-1 px-3 py-2 rounded-lg bg-slate-50 dark:bg-navy-800 border border-slate-200 dark:border-navy-600 text-sm"
            />
            <button onClick={() => { if (newDeliverable.trim()) { setDeliverables([...deliverables, newDeliverable.trim()]); setNewDeliverable(''); } }} disabled={!newDeliverable.trim()}
