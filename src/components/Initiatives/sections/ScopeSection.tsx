  ...
  return (
    ...
    >
      <div className="space-y-5">
          ...
          <div className="space-y-2 mb-3">
            {inScope.map((item, i) => (
              ...
              >
                ...
                <button
                  onClick={() => setInScope(inScope.filter((_, j) => j !== i))}
        ...
        <div>
          <label className="flex items-center gap-2 mb-2">
            <div className="w-6 h-6 rounded-full bg-slate-500/20 flex items-center justify-center text-xs font-bold text-slate-500">
              ✗
            </div>
            <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">
          ...
          <div className="space-y-2 mb-3">
            {outScope.map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-2 p-2 rounded-lg bg-slate-100/50 dark:bg-slate-500/5 border border-slate-200/50 dark:border-slate-500/20"
              >
                <span className="flex-1 text-sm text-slate-500 dark:text-slate-400 line-through">
                  {item}
                </span>
                <button
                  onClick={() => setOutScope(outScope.filter((_, j) => j !== i))}
                  className="text-slate-400 hover:text-red-500"
                >
                  <X size={14} />
                </button>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={!newOut.trim()}
              className="px-3 py-2 rounded-lg text-slate-500 border border-slate-200 dark:border-slate-500/30 disabled:opacity-50"
            >
              <Plus size={16} />
            </button>
          </div>
        ...
        <div className="pt-4 border-t border-slate-200 dark:border-navy-700">
          ...
          <div className="flex gap-2">
            <input
              type="text"
              value={newKill}
              onChange={(e) => setNewKill(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' && newKill.trim()) {
                  setKillCriteria([...killCriteria, newKill.trim()]);
                  setNewKill('');
                }
              }}
