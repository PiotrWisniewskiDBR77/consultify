...
describe('InitiativesHub smoke', () => {
  it('keeps the accepted-classic bridge absent when its flag is off', async () => {
    ...
    // DEC-653: the action is now a Menu 2 action; with the flag off, the kebab itself must not render.
    expect(screen.queryByTestId('initiatives-menu2-kebab')).not.toBeInTheDocument();
  ...
  it('confirms and calls the accepted-classic bridge when its flag is on', async () => {
    ...
    renderHub();
    // DEC-653: the action now lives in the Menu 2 "More actions" kebab — open it
    // before clicking the menu item.
    fireEvent.click(await screen.findByTestId('initiatives-menu2-kebab'));
