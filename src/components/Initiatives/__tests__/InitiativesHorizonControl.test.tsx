...
describe('F2-1 E1 shared initiative horizon', () => {
  ...
  it('uses weeks for 1/3 months and months for 6/12 months', () => {
    expect(([1, 3] as InitiativesHorizon[]).map(getInitiativesHorizonGranularity)).toEqual(['week', 'week']);
    expect(([6, 12] as InitiativesHorizon[]).map(getInitiativesHorizonGranularity)).toEqual(['month', 'month']);
  });
});
