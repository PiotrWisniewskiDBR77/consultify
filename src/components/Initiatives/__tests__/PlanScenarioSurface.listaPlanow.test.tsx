...
vi.mock('@/i18n', () => ({
  default: { language: 'pl', t: (_key: string, fallback?: string) => fallback ?? _key },
}));
...
describe('P11 — lista planów', () => {
  beforeEach(() => {
    listPlanScenarioRegister.mockResolvedValue({
      scenarios: [
        {
          id: 'plan-123456789',
          name: 'Plan modernizacji zakładu',
          state: 'DRAFT',
          version: 1,
    ...
    readPlanScenario.mockResolvedValue({
      version: 1,
      scenario: {
        scenarioId: 'plan-123456789',
        name: 'Plan modernizacji zakładu',
        scenarioVersion: 1,
        status: 'DRAFT',
        portfolioScenarioId: 'portfolio-123456789',
  ...
  it('pierwszą kolumną jest nazwa planu, a inicjatywy nie stają się wierszami listy', async () => {
    render(
      <MemoryRouter>
        <PlanScenarioSurface
          activePreset="all"
          initiatives={[{ id: 'ie-1', name: 'Inicjatywa nie jest wierszem' }]}
        />
      </MemoryRouter>
    );
  ...
  // [ODMROZENIE 05_INITIATIVES DEC-421] P15 K4 — reader zwracał UUID w polu `author`,
  // co `resolveBusinessDisplayLabel` traktował jak identyfikator techniczny i zawsze
  // podstawiał „Nieznane" (postgresInitiativeReader.ts:1239 przed naprawą). Gdy reader
  // dostarcza prawdziwe imię i nazwisko, kolumna Autor ma je pokazać wprost.
  it('kolumna Autor pokazuje imię i nazwisko, nie „Nieznane", gdy reader je dostarczył', async () => {
    render(
      <MemoryRouter>
        <PlanScenarioSurface activePreset="all" initiatives={[]} />
      </MemoryRouter>
    );
    await screen.findAllByText('Plan modernizacji zakładu');
    expect(screen.getAllByText('Anna Kowalska').length).toBeGreaterThan(0);
    expect(screen.queryByText('Nieznane')).not.toBeInTheDocument();
  });
});
