/**
 * InitiativeCalendar (M13 Depth · Seria R · R3 / M13c).
 *
 * PURE presentational month/week calendar for an initiative's schedule. Takes a
 * normalized ScheduleItem[] (from `buildScheduleItems`) and renders items on
 * their start day. Native HTML5 drag-and-drop reschedules an item to a new day
...
export interface InitiativeCalendarProps {
  items: ScheduleItem[];
  onReschedule?: (item: ScheduleItem, newDateISO: string) => void;
  loading?: boolean;
}
...
export const InitiativeCalendar: React.FC<InitiativeCalendarProps> = ({
  ...
  const statuses = useMemo(
    () => Array.from(new Set(items.map((i) => i.status).filter(Boolean) as string[])),
    [items]
  );
  ...
  const handleDrop = (e: React.DragEvent, dayIso: string) => {
    e.preventDefault();
    const id = e.dataTransfer.getData('text/plain');
    const item = items.find((i) => i.id === id);
    if (item && item.start !== dayIso) onReschedule?.(item, dayIso);
  };
              ...
              <div className="space-y-1">
                {dayItems.map((it) => (
                  <div
                    ...
                    title={it.title}
                    className="flex items-center gap-1 rounded px-1 py-0.5 bg-slate-100 dark:bg-navy-800 cursor-grab active:cursor-grabbing"
                  >
