...
vi.mock('../../../services/api/v8/finance', () => ({ V8FinanceApi: { getSettings: vi.fn(), updateSettings: vi.fn() } }));

const profileResponse = { profile: { defaultTimezone: 'Europe/Warsaw', defaultLanguage: 'pl', dateFormat: 'DD/MM/YYYY' } };
...
describe('AdminOrganizationDefaultsPanel', () => {
  beforeEach(() => {
    ...
    vi.mocked(V8FinanceApi.getSettings).mockResolvedValue({ defaultCurrency: 'PLN', defaultWacc: 10, defaultHorizonYears: 5, version: 3 });
  });
  ...
  it('loads both sources independently and saves profile with GET readback', async () => {
    ...
    await waitFor(() => expect(Api.put).toHaveBeenCalledWith('/organization-profiles/org-1', expect.objectContaining({ defaultTimezone: 'UTC' })));
    expect(Api.get).toHaveBeenCalledTimes(2);
  });

  it('uses optimistic version and response state as finance readback', async () => {
    vi.mocked(V8FinanceApi.updateSettings).mockResolvedValue({ state: { defaultCurrency: 'EUR', defaultWacc: 8, defaultHorizonYears: 4, version: 4 }, receiptId: 'r1', idempotentReplay: false });
    render(<AdminOrganizationDefaultsPanel organizationId="org-1" />);
    ...
    await waitFor(() => expect(V8FinanceApi.updateSettings).toHaveBeenCalledWith(expect.objectContaining({ defaultCurrency: 'EUR' }), 3, expect.any(String)));
    expect(await screen.findByDisplayValue('EUR')).toBeInTheDocument();
  });

  it('keeps profile active when V8 is disabled', async () => {
    vi.mocked(V8FinanceApi.getSettings).mockRejectedValue(Object.assign(new Error('V8_DISABLED'), { data: { code: 'V8_DISABLED' } }));
    render(<AdminOrganizationDefaultsPanel organizationId="org-1" />);
