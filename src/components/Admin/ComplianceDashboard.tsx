...
export function ComplianceDashboard() {
    ...
    const fetchComplianceData = useCallback(async () => {
        ...
        try {
            ...
            setActivities(
                activitiesRes.data.activities.length ? activitiesRes.data.activities : getDefaultActivities(),
            );
    ...
    const filteredMetrics = categoryFilter === 'all' ? metrics : metrics.filter((m) => m.category === categoryFilter);
                                ...
                                {activities.map((activity) => (
                                    <tr key={activity.id}>
                                        <td className="px-6 py-4">
                                            ...
                                            <p className="text-sm text-slate-500">{activity.purpose}</p>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-wrap gap-1">
                                                {activity.dataCategories.map((cat, idx) => (
                                                    <span
                                                        key={idx}
                                                        className="px-2 py-0.5 text-xs bg-slate-100 dark:bg-navy-900 text-slate-600 dark:text-slate-400 rounded"
                                                    >
                                                        {cat}
                                                    </span>
                                                ))}
                    ...
                    <div className="space-y-4">
                        {incidents.length === 0 ? (
                            <div className="bg-white dark:bg-navy-800 rounded-xl border border-slate-200 dark:border-white/10 p-12 text-center">
                                ...
                                <p className="text-slate-500 mt-1">All systems are operating normally</p>
                            </div>
                        ) : (
