...
export const NotificationSettingsAdmin: React.FC<NotificationSettingsAdminProps> = ({
  ...
  const SettingRow = ({
    ...
    <div className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-navy-700 last:border-0">
      ...
      <ToggleSwitch enabled={!!settings[settingKey]} onToggle={() => toggleSetting(settingKey)} />
    </div>
  );
        ...
        <div className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-navy-700">
          <div>
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">
              Days before escalating unresolved items
            </div>
          </div>
          <select
            value={settings.escalation_days}
            onChange={(e) => updateDays(parseInt(e.target.value))}
            className="px-3 py-1.5 bg-slate-100 dark:bg-navy-900 border border-slate-200 dark:border-navy-700 rounded-lg text-sm text-slate-700 dark:text-white"
          >
            {[1, 2, 3, 5, 7, 14].map((d) => (
