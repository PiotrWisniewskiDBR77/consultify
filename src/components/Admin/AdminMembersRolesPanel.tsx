...
export const AdminMembersRolesPanel: React.FC<{
  screen?: 'members' | 'invitations' | 'ownership';
}> = ({ screen }) => {
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      const response = await Api.createAdminOrganizationInvitation(
        orgId,
        email,
        inviteRole,
        commandId
      ...
      if (!receiptId || !readback) {
        throw new Error(
          t(
            'admin.membersRoles.invite.readbackFailed',
            'Invitation was submitted but exact read-back is not available. Retry with the same command.'
          )
        );
      }
      sessionStorage.removeItem(inviteCommandStorageKey(orgId, email, inviteRole));
      const msg = t(
        'admin.membersRoles.invite.added',
        'Invitation for {{email}} is pending acceptance. Delivery: {{delivery}}.',
        {
          email,
      ...
      setInviteNotice(msg);
      if (readback.delivery === 'SENT')
        toast.success(t('admin.membersRoles.invite.addedToast', 'Invitation sent'));
      else
        toast.error(
          t(
            'admin.membersRoles.invite.deliveryFailed',
            'Invitation recorded, but email delivery failed or is unverified.'
          )
        );
      setInviteEmail('');
      ...
      const friendly = recovered
        ? t(
            'admin.membersRoles.invite.recovered',
            'A pending invitation exists. Delivery cannot be repeated until the server resend window opens.'
          )
        : raw ||
          t(
            'admin.membersRoles.invite.failed',
            'Failed to create invitation. Retry uses the same command identity.'
          );
  ...
  const handleInvitationAction = async (invitation: InvitationRow, action: 'resend' | 'revoke') => {
    ...
    try {
      ...
      if (action === 'resend')
        await Api.resendOrganizationInvitation(orgId!, invitation.id, commandId);
      ...
      const message =
        action === 'resend'
          ? t(
              'admin.membersRoles.invitations.resent',
              'Invitation resend recorded. Delivery: {{delivery}}.',
              {
  ...
  const handleRoleChange = async (memberId: string, role: RoleOption, expectedRole: string) => {
    ...
    try {
      ...
      if (!exact || String(exact.role || '').toUpperCase() !== role) {
        throw new Error(
          t(
            'admin.membersRoles.role.readbackFailed',
  ...
  const handleRemove = async (memberId: string, expectedRole: string) => {
    ...
    const member = members.find((row) => String(row.user_id ?? row.id ?? '') === String(memberId));
    ...
    try {
      ...
      if (stillActive) {
        throw new Error(
          t(
            'admin.membersRoles.remove.readbackFailed',
            'Removal command completed without exact member read-back.'
          )
        );
      }
  ...
  const deliveryLabel = (delivery?: InvitationRow['delivery']) => {
    ...
    return t(
      `admin.membersRoles.invitations.deliveryStates.${normalized.toLowerCase()}`,
      normalized
    );
  };
        ...
        if (!canManageTeam) {
          return (
            <span className="text-sm text-c-text-secondary">
              {roleLabels[row.role as RoleOption]}
            </span>
          );
        }
        return (
          <div onClick={(e) => e.stopPropagation()} className="max-w-[160px]">
            <label htmlFor={`admin-member-role-select-${row.memberId}`} className="sr-only">
            ...
            <SelectField
              ...
              onChange={(value) =>
                void handleRoleChange(row.memberId, value as RoleOption, row.role)
              }
              placeholder=""
              options={[
  ...
  const invitationColumns: TableColumn[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <span className="text-c-text-secondary">
          {roleLabels[
            String(row.role_to_assign || row.role || 'MEMBER').toUpperCase() as RoleOption
          ] ||
            row.role_to_assign ||
            row.role}
        </span>
    ...
    },
    {
      ...
      render: (row) => (
        <EntityStatusChip
          status={deliveryToneStatus(row.delivery)}
          label={deliveryLabel(row.delivery)}
        />
      ),
    },
    {
        ...
        return (
          <div className="flex justify-end gap-2">
            <Button
              ...
              onClick={() => void handleInvitationAction(row as InvitationRow, 'resend')}
            >
            ...
            <Button
              ...
              onClick={() => void handleInvitationAction(row as InvitationRow, 'revoke')}
            >
  ...
  return (
    <div className="space-y-4 sm:space-y-6" aria-busy={loading || invitationsLoading}>
      ...
      {(!screen || screen === 'members') && (
        <section
          aria-labelledby="admin-members-title"
          className="rounded-2xl border border-slate-200/60 bg-c-surface p-4 sm:p-5 dark:border-white/[0.08]"
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            {canManageTeam ? (
              ...
              <p role="status" className="text-sm text-c-text-muted">
                {t(
                  'admin.membersRoles.readOnly',
                  'Read-only access. Management controls are available only to workspace owners and admins.'
                )}
              </p>
            )}
          </div>
                ...
                {members.length === 0 ? (
                  ...
                  members.map((member) => {
                    ...
                    return (
                      <article
                        key={memberId}
                        className="rounded-xl border border-slate-200/70 p-4 dark:border-white/[0.08]"
                      >
                        ...
                        <div className="mt-3">
                          <label
                            htmlFor={`admin-mobile-member-role-${memberId}`}
                            className="mb-1 block text-xs font-medium text-c-text-muted"
                          >
                          ...
                          {canManageTeam ? (
                            <SelectField
                              ...
                              onChange={(value) =>
                                void handleRoleChange(memberId, value as RoleOption, role)
                              }
                              placeholder=""
                              options={[
                                { value: 'OWNER', label: roleLabels.OWNER, disabled: true },
      ...
      {(!screen || screen === 'invitations') && (
        <section
          aria-labelledby="admin-invitations-title"
          className="rounded-2xl border border-slate-200/60 bg-c-surface p-4 sm:p-5 dark:border-white/[0.08]"
        >
          ...
          {invitationsLoading && invitations.length === 0 && !invitationLoadError ? (
            ...
            <div
              role="alert"
              data-testid="invitations-load-error"
              className="mt-4 rounded-lg border border-c-danger/40 bg-c-danger/10 p-3 text-sm text-c-text"
            >
              {invitationLoadError}
            </div>
            ...
            <p className="mt-4 text-sm text-c-text-muted">
              {t(
                'admin.membersRoles.invitations.empty',
                'No invitation records for this workspace.'
              )}
            </p>
          ) : (
            <div className="mt-4">
              <div className="space-y-3 sm:hidden">
                {invitations.map((invitation) => {
                  ...
                  const role = String(
                    invitation.role_to_assign || invitation.role || 'MEMBER'
                  ).toUpperCase() as RoleOption;
                    ...
                    >
                      ...
                      <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
                        <div>
                          ...
                          <dd className="mt-1 text-c-text">{roleLabels[role] || role}</dd>
                        </div>
                        <div>
                          <dt className="text-xs text-c-text-muted">
                            {t('admin.membersRoles.columns.status', 'Status')}
                          </dt>
                          <dd className="mt-1">
                            <EntityStatusChip
                              status={String(invitation.status || 'unknown').toLowerCase()}
                            />
                          </dd>
                        </div>
                        <div>
                          <dt className="text-xs text-c-text-muted">
                            {t('admin.membersRoles.invitations.delivery', 'Delivery')}
                          </dt>
                          <dd className="mt-1">
                            <EntityStatusChip
                              status={deliveryToneStatus(invitation.delivery)}
                              label={deliveryLabel(invitation.delivery)}
                            />
                          </dd>
                        </div>
                        <div>
      ...
      {!screen && canManageTeam && (
        <section
          aria-labelledby="admin-invite-code-title"
          className="rounded-2xl border border-slate-200/60 bg-c-surface p-4 sm:p-5 dark:border-white/[0.08]"
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
      ...
      {canManageTeam && (!screen || screen === 'ownership') && <OwnershipManagementView />}
      {removeMemberDialog}
    </div>
  );
};
