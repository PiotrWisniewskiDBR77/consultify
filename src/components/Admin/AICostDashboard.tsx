...
export const AICostDashboard: React.FC = () => {
    ...
    const avgCostPerRequest = providerCount > 0 && totalTokens > 0
        ? (costData?.totalCost || 0) / (totalTokens / 1000)
        : 0;

    const metrics: CostMetric[] = [
        { label: 'Total Cost (MTD)', value: `$${(costData?.totalCost || 0).toFixed(2)}`, change: 0, trend: 'neutral' },
        { label: 'Tokens Used', value: totalTokens.toLocaleString(), change: 0, trend: 'neutral' },
        { label: 'Avg Cost/Request', value: `$${avgCostPerRequest.toFixed(4)}`, change: 0, trend: 'neutral' },
        { label: 'Est. Monthly', value: `$${((costData?.totalCost || 0) * 30 / new Date().getDate()).toFixed(2)}`, change: 0, trend: 'neutral' },
    ];
            ...
            <div className="bg-navy-800 rounded-xl border border-white/10 p-6">
                ...
                {error ? (
                    <div className="text-center py-8 text-red-400">
                        <p>{error}</p>
                        <button onClick={fetchCosts} className="mt-2 text-sm underline">Retry</button>
                    </div>
                ) : Object.keys(costData?.byProvider || {}).length > 0 ? (
                    <div className="space-y-3">
                        {Object.entries(costData?.byProvider || {}).map(([provider, data]) => (
                            <div key={provider} className="flex items-center justify-between p-3 bg-navy-900/50 rounded-lg">
                                <div>
