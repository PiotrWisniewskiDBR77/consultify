        ...
        <div className={`space-y-1 ${className}`}>
            ...
            {(subtitle || trend) && (
                <div className="flex items-center gap-2">
                    ...
                    {subtitle && !trend && <span className="text-xs text-slate-500">{subtitle}</span>}
                </div>
            )}
        </div>
    );
...
export const MetricCardWithCard: React.FC<MetricCardWithCardProps> = ({ variant = 'default', ...props }) => {
    const wrapperClass =
        variant === 'bordered'
