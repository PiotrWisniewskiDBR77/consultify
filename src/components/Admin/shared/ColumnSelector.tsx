...
export const ColumnSelector: React.FC<ColumnSelectorProps> = ({ columns, onChange, onReset, trigger, className }) => {
    ...
    const toggleColumn = useCallback(
        (columnId: string) => {
            onChange(
                columns.map((col) => (col.id === columnId && !col.required ? { ...col, visible: !col.visible } : col)),
            );
        },
        [columns, onChange],
    );
    ...
    return (
        <div className={cn('relative', className)}>
            ...
            {trigger ? (
                ...
                <Button variant="outline" size="sm" onClick={() => setIsOpen(!isOpen)} icon={<Columns size={16} />}>
                ...
                <>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-navy-800 border border-slate-200 dark:border-navy-700 rounded-xl shadow-lg z-50">
                        ...
                        <div className="flex items-center gap-2 p-2 border-b border-slate-200 dark:border-navy-700">
                            <button
                                onClick={showAll}
                                className="flex-1 px-2 py-1.5 text-xs font-medium text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded"
                            >
                                {t('admin.table.showAll', 'Show All')}
                            </button>
                            <button
                                onClick={hideOptional}
                                className="flex-1 px-2 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 dark:hover:bg-navy-700 rounded"
                            >
                            ...
                            {onReset && (
                                <Tooltip content={t('admin.table.resetColumns', 'Reset to default')}>
                                    <button
                                        onClick={onReset}
                                        className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-50 dark:hover:bg-navy-700 rounded"
                                    >
                                        <RefreshCw size={14} />
                                    </button>
                                </Tooltip>
                            ...
                            <div className="space-y-1">
                                {filteredColumns.map((column, index) => (
                                    ...
                                    >
                                        ...
                                        {/* Badges */}
                                        {column.required && (
                                            <span className="px-1 py-0.5 text-xs bg-slate-200 dark:bg-navy-700 text-slate-500 rounded">
