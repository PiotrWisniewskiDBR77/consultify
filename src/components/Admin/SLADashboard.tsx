...
export function SLADashboard() {
    ...
    const loadSLAData = useCallback(async () => {
        setLoading(true);
        try {
            const response = await api.get(`/ai-analytics/sla?range=${timeRange}`);
            if (response.data.success || response.data.metrics) {
                setMetrics(response.data.metrics || generateMockMetrics());
                setBreaches(response.data.breaches || generateMockBreaches());
                setUptimeHistory(response.data.uptimeHistory || generateMockHistory());
            } else {
