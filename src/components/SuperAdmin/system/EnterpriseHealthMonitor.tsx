...
export const EnterpriseHealthMonitor: React.FC = () => {
    ...
    try {
      ...
      const serviceList: ServiceHealth[] = [
        ...
        },
        {
          name: 'AI Services',
          status:
            data.ai?.status === 'online'
              ? 'healthy'
              : data.ai?.status === 'no_keys'
                ? 'degraded'
                : 'down',
          latency: 0,
          lastCheck: data.timestamp,
      ...
      if (data.system) {
        setMetrics({
          cpu: {
            usage: (data.system.loadAvg?.[0] || 0) * 10,
            cores: (data.system.cpus as any)?.length || 4,
          },
          memory: {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={refreshing}
            className="flex items-center gap-2 px-4 py-2 bg-slate-50/30 dark:bg-navy-950/20 hover:bg-slate-100 dark:hover:bg-navy-800/40 border border-white/10 rounded-lg text-slate-300 transition-colors"
          >
              ...
              {[
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span
                        className={`font-medium ${isActive ? 'text-white' : 'text-slate-500 dark:text-slate-400'}`}
                      >
                        {name}
                      </span>
                      {isActive ? (
                    ...
                    <div
                      className={`text-xs mt-1 ${isActive ? `text-${color}-400` : 'text-slate-600 dark:text-slate-400'}`}
                    >
      ...
      {/* Footer */}
      <div className="pt-4 border-t border-white/10 text-xs text-slate-500 dark:text-slate-400 flex items-center justify-between">
        <span>
