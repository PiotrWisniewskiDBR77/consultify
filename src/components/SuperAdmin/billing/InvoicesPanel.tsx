...
export const InvoicesPanel: React.FC = () => {
    ...
    const getStatusBadge = (status: string) => {
        const configs: Record<string, { icon: React.ReactNode; bg: string; text: string }> = {
            draft: { icon: <FileText size={14} />, bg: 'bg-slate-500/20', text: 'text-slate-400' },
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-white/[0.06]">
                                ...
                                <th className="text-left p-4 text-sm font-medium text-slate-400">Organization</th>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
                        <div className="flex items-start justify-between mb-6">
                            <div>
                                <h3 className="text-xl font-semibold text-white">{selectedInvoice.invoice_number}</h3>
                                <p className="text-sm text-slate-400">{selectedInvoice.organization_name}</p>
                            </div>
