...
const TrendIcon: React.FC<{ trend?: 'up' | 'down' | 'neutral' }> = ({ trend }) => {
  switch (trend) {
    ...
    case 'down':
      return <TrendingDown className="w-4 h-4 text-danger-300" />;
    default:
      ...
      {isEditing ? (
        <div className="p-4 bg-slate-50 dark:bg-navy-800 rounded-xl border border-slate-200 dark:border-slate-600 space-y-3 min-w-[200px]">
          ...
          <select
            ...
            onChange={(e) =>
              setEditForm({ ...editForm, variant: e.target.value as MetricCardAttrs['variant'] })
            }
            className="w-full px-2 py-1 border rounded text-sm"
          >
          ...
          <select
            ...
            onChange={(e) =>
              setEditForm({ ...editForm, trend: e.target.value as MetricCardAttrs['trend'] })
            }
            className="w-full px-2 py-1 border rounded text-sm"
          >
        ...
        >
          ...
          {attrs.sublabel && <span className="text-xs opacity-70 mt-1"> {attrs.sublabel} </span>}
        </div>
      )}
    </NodeViewWrapper>
  );
