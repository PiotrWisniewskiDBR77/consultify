...
export const ReportSection: React.FC<ReportSectionProps> = ({
  ...
  return (
    <div
      ref={editorRef}
      className={`${panelMode ? 'fixed inset-4 z-50 bg-white dark:bg-navy-900 rounded-xl shadow-2xl flex flex-col' : ''}`}
    >
      {/* Edit header */}
      <div
        className={`flex items-center justify-between gap-4 ${panelMode ? 'p-4 border-b border-slate-200 dark:border-navy-700' : 'mb-4'}`}
      >
        {/* Title input */}
        <input
          type="text"
      ...
      {/* Panel mode backdrop */}
      {panelMode && <div className="fixed inset-0 bg-black/50 -z-10" onClick={handleCancel} />}
    </div>
  );
};
