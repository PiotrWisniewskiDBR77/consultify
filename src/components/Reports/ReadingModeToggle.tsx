        ...
        className={`
                    ...
                    ${
                      isEnabled
                        ...
                        : 'text-slate-500 hover:text-slate-700 dark:text-slate-300 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5'
                    }
                `}
        title={t('reports.readingModeToggle.readingSettingsTooltip', 'Reading settings')}
      >
        {isEnabled ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
        <span className="text-sm font-medium hidden sm:inline">
          {t('reports.readingModeToggle.readingModeLabel', 'Reading Mode')}
        </span>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-navy-800 rounded-xl shadow-xl border border-slate-200 dark:border-navy-700 p-4 z-50"
            >
              {/* Header */}
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-navy-900 dark:text-white">
                  {t('reports.readingModeToggle.readingSettingsTitle', 'Reading Settings')}
                </h3>
                <button
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-white/5 rounded-lg mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-navy-900 dark:text-white">
                    {t('reports.readingModeToggle.focusModeLabel', 'Focus Mode')}
                  </span>
                </div>
                <button
              ...
              <div className="mb-4">
                <label className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 block">
                  {t('reports.readingModeToggle.fontSizeLabel', 'Font Size')}
                </label>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setFontSize(Math.max(minFontSize, fontSize - 2))}
                    disabled={fontSize <= minFontSize}
                    className="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg disabled:opacity-50"
                  >
              ...
              <div className="mb-4">
                <label className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 block">
                  {t('reports.readingModeToggle.themeLabel', 'Theme')}
                </label>
                  ...
                  {[
                    {
                      id: 'light',
                      icon: Sun,
                      label: t('reports.readingModeToggle.lightThemeLabel', 'Light'),
                    },
                    {
                      id: 'dark',
                      icon: Moon,
                      label: t('reports.readingModeToggle.darkThemeLabel', 'Dark'),
                    },
              ...
              {/* Actions */}
              <div className="flex gap-2 pt-3 border-t border-slate-200 dark:border-navy-700">
                <button
                  onClick={handlePrint}
                ...
                >
                  ...
                  {t('reports.readingModeToggle.printButton', 'Print')}
                </button>
                <button
                  onClick={() => {
                    if (!document.fullscreenElement) {
                ...
                >
                  ...
                  {t('reports.readingModeToggle.fullscreenButton', 'Fullscreen')}
                </button>
              </div>
            </motion.div>
          </>
  ...
  return (
    <button
      ...
      className={`
                ...
                ${className}
            `}
      title={t('reports.readingModeToggle.readingModeTooltip', 'Reading mode')}
    >
      {isEnabled ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
    </button>
  );
