...
export const ReportBuilder: React.FC<ReportBuilderProps> = ({
  ...
  const handleDeleteSection = async (sectionId: string) => {
    const confirmed = window.confirm(
      t('reports.confirmDelete', 'Are you sure you want to delete this section?')
    );
    if (!confirmed) return;
  ...
  const hasEmbeddableData = (section: ReportSectionData) => {
    return (
      ['maturity_overview', 'axis_detail', 'gap_analysis'].includes(section.sectionType) &&
      section.dataSnapshot &&
      Object.keys(section.dataSnapshot).length > 0
    );
  };
    ...
    <div ref={containerRef} className="max-w-4xl mx-auto py-8 px-6">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
          <span>
            {sections.length} {t('reports.sections', 'sections')}
          </span>
          <span>•</span>
          <span>
      ...
      <div className="space-y-4">
            ...
            <div key={section.id} data-section-id={section.id}>
              ...
              <div
                ...
                className={`
                  ...
                  ${isFocused ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-200 dark:border-navy-700'}
                  ...
                  <div className="px-4 pb-4 border-t border-slate-200 dark:border-navy-700">
                    ...
                    <div className="mt-4">
                      <ReportSection
                        ...
                        onContentUpdate={(content, title) =>
                          handleContentUpdate(section.id, content, title)
                        }
              ...
              {!readOnly && (
                <div className="relative my-2">
                  ...
                  {showAddMenu === index && (
                    <div className="absolute left-1/2 -translate-x-1/2 top-8 z-10 w-64 bg-white dark:bg-navy-800 rounded-xl shadow-xl border border-slate-200 dark:border-navy-700 py-2 max-h-80 overflow-auto">
        ...
        {!readOnly && sections.length > 0 && (
          <div className="flex justify-center pt-4">
            <button
              onClick={() =>
                setShowAddMenu(showAddMenu === sections.length ? null : sections.length)
              }
            ...
            {showAddMenu === sections.length && (
              <div className="absolute mt-10 z-10 w-64 bg-white dark:bg-navy-800 rounded-xl shadow-xl border border-slate-200 dark:border-navy-700 py-2 max-h-80 overflow-auto">
                <p className="px-3 py-1.5 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  {t('reports.addSection', 'Add Section')}
                </p>
                {SECTION_TYPES.map(({ type, label, labelPl }) => {
      ...
      {showAddMenu !== null && (
        <div className="fixed inset-0 z-0" onClick={() => setShowAddMenu(null)} />
      )}
    </div>
  );
};
