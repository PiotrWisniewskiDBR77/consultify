...
const PriorityBadge: React.FC<{ gap: number }> = ({ gap }) => {
  ...
  if (gap >= 3) {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-danger-100 dark:bg-danger-500/20 text-danger-700 dark:text-danger-400 rounded-full">
...
const ProgressBar: React.FC<{ actual: number; target: number; maxLevel: number }> = ({
  actual,
  target,
  maxLevel,
}) => {
...
const MaturityOverviewMatrix: React.FC<{ axisData: Record<string, AxisData> }> = ({ axisData }) => {
  ...
  const avgGap =
    totals.count > 0 ? ((totals.targetSum - totals.actualSum) / totals.count).toFixed(1) : '-';
    ...
    <div className="bg-slate-50 dark:bg-navy-800/50 rounded-xl p-4">
      ...
      <div className="overflow-x-auto">
        ...
        >
          ...
          <tfoot>
            <tr className="border-t-2 border-slate-200 dark:border-navy-700 font-medium">
              <td className="pt-3 text-navy-900 dark:text-white">
                {t('reports.average', 'Average')}
              </td>
...
const AxisDetailMatrix: React.FC<{ axisId: string; data: AxisData }> = ({ axisId, data }) => {
  ...
  const areaScores = useMemo(() => {
    ...
    return Object.entries(data.areaScores)
      .map(([areaId, scores]) => {
        const actual = Array.isArray(scores)
          ? scores[0]
          : (scores as { actual?: number })?.actual || 0;
        const target = Array.isArray(scores)
          ? scores[1]
          : (scores as { target?: number })?.target || 0;
        return {
          id: areaId,
          actual,
          target,
  ...
  return (
    <div className="bg-slate-50 dark:bg-navy-800/50 rounded-xl p-4">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div className="bg-danger-50 dark:bg-danger-500/10 rounded-lg p-3 text-center">
          <div className="text-xs text-danger-600 dark:text-danger-400 mb-1">
            {t('reports.criticalGaps', 'Critical Gaps')}
          </div>
          <div className="text-2xl font-bold text-danger-700 dark:text-danger-400">{critical}</div>
        </div>
        <div className="bg-yellow-50 dark:bg-yellow-500/10 rounded-lg p-3 text-center">
          <div className="text-xs text-yellow-600 dark:text-yellow-400 mb-1">
            {t('reports.highGaps', 'High Priority')}
          </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-500/10 rounded-lg p-3 text-center">
          <div className="text-xs text-blue-600 dark:text-blue-400 mb-1">
            {t('reports.totalLevels', 'Total Levels to Gain')}
          </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-navy-700">
              <th className="pb-2 font-medium">{t('reports.axis', 'Axis')}</th>
              <th className="pb-2 font-medium text-center">
                {t('reports.currentVsTarget', 'Current → Target')}
              </th>
              <th className="pb-2 font-medium text-center">{t('reports.gap', 'Gap')}</th>
              <th className="pb-2 font-medium text-center">{t('reports.estTime', 'Est. Time')}</th>
              <th className="pb-2 font-medium text-center">
                {t('reports.complexity', 'Complexity')}
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200 dark:divide-white/5">
