        ...
        <div className="relative h-2 bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden">
            ...
            <div
                className={`h-full rounded-full transition-all ${
                    actual >= target ? 'bg-green-500' : actual >= target * 0.7 ? 'bg-yellow-500' : 'bg-red-500'
                }`}
        ...
        <div className="bg-slate-50 dark:bg-navy-800/50 rounded-xl p-4">
            ...
            <div className="grid grid-cols-4 gap-4 mb-4">
                <div className="bg-white dark:bg-navy-900 rounded-lg p-3 text-center">
                    ...
                    <div className="text-2xl font-bold text-navy-900 dark:text-white">{data.actual || '-'}</div>
                ...
                <div className="bg-white dark:bg-navy-900 rounded-lg p-3 text-center">
                    ...
                    <div className="text-2xl font-bold text-navy-900 dark:text-white">{data.target || '-'}</div>
                    <div className="text-xs text-slate-400">/ {axisConfig.maxLevel}</div>
                </div>
                <div className="bg-white dark:bg-navy-900 rounded-lg p-3 text-center">
                    <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">{t('reports.gap', 'Gap')}</div>
                    <div
...
export const EmbeddedMatrix: React.FC<EmbeddedMatrixProps> = ({ sectionType, axisId, dataSnapshot, axisData }) => {
    switch (sectionType) {
