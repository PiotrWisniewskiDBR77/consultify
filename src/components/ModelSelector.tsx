...
export const ModelSelector: React.FC = () => {
  ...
  const checkLLMConnection = useCallback(async () => {
    ...
    try {
      ...
      if (currentProvider === 'system') {
        ...
        const providerExists = systemProviders.some(
          (p: any) => p.id === currentModelId || p.model_id === currentModelId
        );
  ...
  if (currentUser?.aiConfig?.privateModels) {
    ...
    const privateMatch = currentUser.aiConfig.privateModels.find(
      (pm: { provider: string; modelId: string; apiKey?: string; name: string }) =>
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-navy-900 hover:bg-slate-200 dark:hover:bg-navy-800 border border-slate-200 dark:border-navy-700 rounded-full text-sm transition-colors min-w-[160px]"
      >
          ...
          <div className="max-h-64 overflow-y-auto">
            {loading ? (
              ...
              options.map((opt) => {
                ...
                const isSelected =
                  opt.provider === currentProvider && opt.modelId === currentModelId;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <div
                        className={`w-2 h-2 rounded-full ${isSelected ? 'bg-green-500' : 'bg-slate-400'}`}
                      />
