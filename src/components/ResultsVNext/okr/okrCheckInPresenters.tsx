  ...
  return [
    {
      ...
      render: (row: OkrCheckInDto) => (
        <span className="text-sm text-c-text tabular-nums">
          {formatOkrDate(row.submittedAt, isPolish)}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      filterable: true,
      filterOptions: (
        Object.keys(OKR_CHECKIN_STATUS_TONE) as OkrCheckInDto['ownerDeclaredStatus'][]
      )
      ...
      render: (row: OkrCheckInDto) =>
        row.ownerDeclaredStatus ? (
          <StatusChip
            label={okrCheckInStatusLabel(row.ownerDeclaredStatus, isPolish)}
            tone={OKR_CHECKIN_STATUS_TONE[row.ownerDeclaredStatus]}
          />
        ) : (
    ...
    },
    {
      ...
      width: '260px',
      render: (row: OkrCheckInDto) => (
        <span className="block truncate text-sm text-c-text-secondary">{row.note}</span>
      ),
    },
    {
      id: 'correction',
...
export function buildOkrCheckInRowMenu(
  row: OkrCheckInDto,
  isPolish: boolean,
  handlers: OkrCheckInRowMenuHandlers
): StandardRowMenu {
  return {
    primary: [
      ...
      {
        id: 'correct',
        label: isPolish ? 'Skoryguj' : 'Correct',
...
export function buildOkrCheckInPreview(
  row: OkrCheckInDto,
  deps: OkrCheckInPreviewDeps
  ...
  return {
    title: isPolish
      ? `Check-in — ${formatOkrDate(row.submittedAt, isPolish)}`
      : `Check-in — ${formatOkrDate(row.submittedAt, isPolish)}`,
    onClose,
    meta: {
      pills: [
    ...
    details: {
      ...
      properties: [
        ...
        {
          id: 'previousValue',
          label: isPolish ? 'Wartość poprzednia' : 'Previous value',
          value: (
            <HonestValueCell
              isPolish={isPolish}
              value={previousValue}
              format={(v) => v.toLocaleString(isPolish ? 'pl-PL' : 'en-US')}
            />
          ),
        },
        {
        ...
        },
        {
          ...
          value: row.systemSuggestedStatus
            ? okrCheckInStatusLabel(row.systemSuggestedStatus, isPolish)
            : '—',
        },
        {
          id: 'confidence',
        ...
        {
          id: 'supportRequested',
          label: isPolish ? 'Potrzebne wsparcie' : 'Support requested',
          value: row.supportRequested ?? '—',
        },
        {
          id: 'submittedBy',
          label: isPolish ? 'Zgłosił' : 'Submitted by',
        ...
        {
          id: 'submittedAt',
          label: isPolish ? 'Data zgłoszenia' : 'Submitted at',
          value: formatOkrDate(row.submittedAt, isPolish),
        },
        {
          id: 'cadenceOccurrenceId',
          label: isPolish ? 'ID wystąpienia cyklu' : 'Cadence occurrence id',
          value: row.cadenceOccurrenceId,
          mono: true,
        },
      ],
    },
