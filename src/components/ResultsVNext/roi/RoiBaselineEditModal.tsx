...
export const RoiBaselineEditModal: React.FC<RoiBaselineEditModalProps> = ({
  ...
  useEffect(() => {
    ...
    setCurrentValue(
      baseline?.currentMeasuredValue !== null && baseline?.currentMeasuredValue !== undefined
        ? String(baseline.currentMeasuredValue)
        : ''
    );
    ...
    setGrowthRate(
      baseline?.bauGrowthRatePct !== null && baseline?.bauGrowthRatePct !== undefined
        ? String(baseline.bauGrowthRatePct)
        : ''
    );
    setReferenceValue(
      baseline?.bauReferenceValue !== null && baseline?.bauReferenceValue !== undefined
        ? String(baseline.bauReferenceValue)
        : ''
    );
  ...
  return (
    <Modal
      ...
      description={
        isPolish
          ? 'Zapisuje realny baseline sprawy ROI (PUT .../baseline).'
          : 'Saves the real ROI case baseline (PUT .../baseline).'
      }
      size="lg"
    ...
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-current-value">
              {isPolish ? 'Bieżąca wartość zmierzona' : 'Current measured value'}
            </label>
            <input
              id="roi-baseline-current-value"
              type="number"
              value={currentValue}
              onChange={(e) => setCurrentValue(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-current-value"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-current-unit">
              {isPolish ? 'Jednostka' : 'Unit'}
            </label>
            <input
              id="roi-baseline-current-unit"
              value={currentUnit}
              onChange={(e) => setCurrentUnit(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-current-unit"
            />
          </div>
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-baseline-current-as-of">
            {isPolish ? 'Stan na dzień' : 'Measured as of'}
          </label>
          <input
            id="roi-baseline-current-as-of"
            type="date"
            value={currentAsOf}
            onChange={(e) => setCurrentAsOf(e.target.value)}
            className={FIELD_CLASS}
            data-testid="roi-baseline-current-as-of"
          />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-period-start">
              {isPolish ? 'Początek okresu baseline' : 'Baseline period start'}
            </label>
            <input
              id="roi-baseline-period-start"
              type="date"
              value={periodStart}
              onChange={(e) => setPeriodStart(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-period-start"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-period-end">
              {isPolish ? 'Koniec okresu baseline' : 'Baseline period end'}
            </label>
            <input
              id="roi-baseline-period-end"
              type="date"
              value={periodEnd}
              onChange={(e) => setPeriodEnd(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-period-end"
            />
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-method">
              {isPolish ? 'Metoda projekcji BAU' : 'BAU projection method'}
            </label>
            <select
              id="roi-baseline-method"
              value={method}
              onChange={(e) => setMethod(e.target.value as RoiBaselineProjectionMethod)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-method"
            >
              {ROI_BASELINE_PROJECTION_METHODS.map((m) => (
                <option key={m} value={m}>
                  {roiBaselineProjectionMethodLabel(m, isPolish)}
                </option>
              ))}
            </select>
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-growth-rate">
              {isPolish ? 'Stopa wzrostu BAU (%)' : 'BAU growth rate (%)'}
            </label>
            <input
              id="roi-baseline-growth-rate"
              type="number"
              value={growthRate}
              onChange={(e) => setGrowthRate(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-growth-rate"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-reference-value">
              {isPolish ? 'Wartość referencyjna BAU' : 'BAU reference value'}
            </label>
            <input
              id="roi-baseline-reference-value"
              type="number"
              value={referenceValue}
              onChange={(e) => setReferenceValue(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-reference-value"
            />
          </div>
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-baseline-notes">
            {isPolish ? 'Notatki porównawcze' : 'Comparison notes'}
          </label>
          <textarea
            id="roi-baseline-notes"
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-baseline-notes"
          />
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-source">
              {isPolish ? 'Źródło' : 'Source'}
            </label>
            <input
              id="roi-baseline-source"
              value={source}
              onChange={(e) => setSource(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-source"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-confidence">
              {isPolish ? 'Pewność' : 'Confidence'}
            </label>
            <select
              id="roi-baseline-confidence"
              value={confidence}
              onChange={(e) => setConfidence(e.target.value as RoiConfidenceLevel | '')}
              className={FIELD_CLASS}
              data-testid="roi-baseline-confidence"
            >
              <option value="">—</option>
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-baseline-owner">
              {isPolish ? 'Właściciel' : 'Owner'}
            </label>
            <input
              id="roi-baseline-owner"
              value={ownerUserId}
              onChange={(e) => setOwnerUserId(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-baseline-owner"
            />
          </div>
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-baseline-reason">
            {isPolish ? 'Notatka do audytu (opcjonalnie)' : 'Audit note (optional)'}
          </label>
          <textarea
            id="roi-baseline-reason"
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-baseline-reason"
          />
        </div>
        ...
        {errorMessage ? (
          <div
            ...
            data-testid="roi-baseline-error"
          >
