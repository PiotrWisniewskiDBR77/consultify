...
export const RoiForecastVersionCreateModal: React.FC<RoiForecastVersionCreateModalProps> = ({
  open,
  onClose,
  onSubmit,
  isPolish,
  ...
  return (
    <Modal
      ...
      title={isPolish ? 'Opublikuj wersję prognozy' : 'Publish forecast version'}
      size="sm"
      ...
      footer={
        <>
          ...
          <button
            ...
            disabled={busy}
            data-testid="roi-forecast-version-submit"
    ...
    >
      <div className="space-y-4">
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-forecast-reason">
            {isPolish ? 'Powód (wymagany)' : 'Reason (required)'}
          </label>
          <textarea
            id="roi-forecast-reason"
            autoFocus
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-forecast-reason"
        ...
        <ErrorBanner
          ...
          isPolish={isPolish}
          testId="roi-forecast-version-error"
        />
      </div>
    </Modal>
  );
...
export const RoiActualEntryFormModal: React.FC<RoiActualEntryFormModalProps> = ({
  ...
  const lineOptions =
    entryType === 'cost' ? costLines : entryType === 'benefit' ? benefitLines : [];

  return (
    <Modal
      open={open}
      onClose={busy ? () => {} : onClose}
      title={isPolish ? 'Zarejestruj wykonanie' : 'Record actual entry'}
      size="lg"
      preventOverlayClose={busy}
      preventEscapeClose={busy}
      ...
      footer={
        <>
          ...
          <button
            ...
            onClick={handleSubmit}
            disabled={busy}
            data-testid="roi-actual-entry-submit"
            className={`${MENU_1_PRIMARY_CTA} disabled:cursor-not-allowed disabled:opacity-50`}
          >
            <Plus size={16} />
            <span>
    ...
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-actual-type">
              {isPolish ? 'Typ' : 'Type'}
            </label>
            <select
              id="roi-actual-type"
              value={entryType}
              onChange={(e) => {
                setEntryType(e.target.value as RoiActualEntryType);
                setLineId('');
              }}
              className={FIELD_CLASS}
              data-testid="roi-actual-type"
            >
              {ROI_ACTUAL_ENTRY_TYPES.map((t) => (
                <option key={t} value={t}>
                  {roiActualEntryTypeLabel(t, isPolish)}
                </option>
              ))}
            </select>
          </div>
          {entryType !== 'observation' ? (
            <div>
              <label className={LABEL_CLASS} htmlFor="roi-actual-line">
                {entryType === 'cost'
                  ? isPolish
                    ? 'Pozycja kosztowa'
                    : 'Cost line'
              ...
              <select
                ...
                className={FIELD_CLASS}
                data-testid="roi-actual-line"
              >
                <option value="">—</option>
                {lineOptions.map((l: any) => (
                  <option
                    key={l.costLineId ?? l.benefitLineId}
                    value={l.costLineId ?? l.benefitLineId}
                  >
                    {l.label}
                  </option>
                ))}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-actual-period-start">
              {isPolish ? 'Początek okresu' : 'Period start'}
            </label>
            <input
              id="roi-actual-period-start"
              type="date"
              value={periodStart}
              onChange={(e) => setPeriodStart(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-actual-period-start"
              aria-invalid={periodStartError || undefined}
              aria-describedby={periodStartError ? 'roi-actual-period-start-error' : undefined}
            />
            {periodStartError ? (
              <p id="roi-actual-period-start-error" className="mt-1 text-[11px] text-c-danger">
                {isPolish ? 'Początek okresu jest wymagany' : 'Period start is required'}
              </p>
            ) : null}
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-actual-period-end">
              {isPolish ? 'Koniec okresu' : 'Period end'}
            </label>
            <input
              id="roi-actual-period-end"
              type="date"
              value={periodEnd}
              onChange={(e) => setPeriodEnd(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-actual-period-end"
              aria-invalid={periodEndError || undefined}
              aria-describedby={periodEndError ? 'roi-actual-period-end-error' : undefined}
            />
            {periodEndError ? (
              <p id="roi-actual-period-end-error" className="mt-1 text-[11px] text-c-danger">
                {isPolish ? 'Koniec okresu jest wymagany' : 'Period end is required'}
              </p>
            ) : null}
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-actual-amount">
              {isPolish ? 'Kwota' : 'Amount'}
            </label>
            <input
              id="roi-actual-amount"
              type="number"
              value={amount}
              onChange={(e) => setAmount(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-actual-amount"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-actual-currency">
              {isPolish ? 'Waluta' : 'Currency'}
            </label>
            <input
              id="roi-actual-currency"
              value={currency}
              onChange={(e) => setCurrency(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-actual-currency"
            />
          </div>
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-actual-source">
            {isPolish ? 'Źródło (wymagane)' : 'Source (required)'}
          </label>
          <input
            id="roi-actual-source"
            value={source}
            onChange={(e) => setSource(e.target.value)}
            className={FIELD_CLASS}
            data-testid="roi-actual-source"
        ...
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-actual-notes">
...
export const RoiActualEntryActionModal: React.FC<RoiActualEntryActionModalProps> = ({
  ...
  const title =
    kind === 'correction'
      ? isPolish
        ? 'Koryguj wykonanie'
        : 'Correct actual entry'
      : kind === 'verify'
        ? isPolish
          ? 'Zweryfikuj wykonanie'
          : 'Verify actual entry'
        : isPolish
          ? 'Zakwestionuj wykonanie'
          : 'Dispute actual entry';
  const icon =
    kind === 'correction' ? (
      <Camera size={16} />
    ) : kind === 'verify' ? (
      ...
      footer={
        <>
          ...
          <button
            ...
            onClick={handleSubmit}
            disabled={busy}
            data-testid="roi-actual-action-submit"
            className={`${MENU_1_PRIMARY_CTA} disabled:cursor-not-allowed disabled:opacity-50`}
          >
            {icon}
            <span>
              {busy ? (isPolish ? 'Zapisywanie…' : 'Saving…') : isPolish ? 'Potwierdź' : 'Confirm'}
            </span>
          </button>
        </>
      }
    >
      <div className="space-y-4">
        {kind === 'correction' ? (
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={LABEL_CLASS} htmlFor="roi-correction-amount">
                {isPolish ? 'Nowa kwota' : 'New amount'}
              </label>
              <input
                id="roi-correction-amount"
                type="number"
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
                className={FIELD_CLASS}
                data-testid="roi-correction-amount"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="roi-correction-currency">
                {isPolish ? 'Waluta' : 'Currency'}
              </label>
              <input
                id="roi-correction-currency"
                value={currency}
                onChange={(e) => setCurrency(e.target.value)}
                className={FIELD_CLASS}
                data-testid="roi-correction-currency"
              />
            </div>
          </div>
        ) : null}
        ...
        <ErrorBanner
          ...
          isConflict={isConflict}
          isPolish={isPolish}
          testId="roi-actual-action-error"
        />
      </div>
    </Modal>
  );
...
export const RoiActualSnapshotPublishModal: React.FC<RoiActualSnapshotPublishModalProps> = ({
  open,
  onClose,
  onSubmit,
  isPolish,
  ...
  const handleSubmit = () => {
    setTouched(true);
    if (!asOfPeriodEnd) return;
    onSubmit({ asOfPeriodEnd, reason: reason.trim() || null });
  };
  return (
    <Modal
      open={open}
      onClose={busy ? () => {} : onClose}
      title={isPolish ? 'Opublikuj migawkę wykonania' : 'Publish actual snapshot'}
      size="sm"
      preventOverlayClose={busy}
      preventEscapeClose={busy}
      footer={
        <>
          ...
          <button
            ...
            onClick={handleSubmit}
            disabled={busy}
            data-testid="roi-actual-snapshot-submit"
            className={`${MENU_1_PRIMARY_CTA} disabled:cursor-not-allowed disabled:opacity-50`}
          >
            <Camera size={16} />
            <span>
    ...
    >
      <div className="space-y-4">
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-snapshot-as-of">
            {isPolish ? 'Stan na dzień (wymagane)' : 'As of (required)'}
          </label>
          <input
            id="roi-snapshot-as-of"
            type="date"
            autoFocus
            value={asOfPeriodEnd}
            onChange={(e) => setAsOfPeriodEnd(e.target.value)}
            className={FIELD_CLASS}
            data-testid="roi-snapshot-as-of"
        ...
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-snapshot-reason">
            {isPolish ? 'Notatka (opcjonalnie)' : 'Note (optional)'}
          </label>
          <textarea
            id="roi-snapshot-reason"
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-snapshot-reason"
          />
        </div>
        <ErrorBanner
          message={errorMessage}
          isConflict={isConflict}
          isPolish={isPolish}
          testId="roi-actual-snapshot-error"
        />
      </div>
    </Modal>
  );
...
export const RoiVarianceFormModal: React.FC<RoiVarianceFormModalProps> = ({
  ...
  const [comparisonType, setComparisonType] =
    useState<RoiVarianceComparisonType>('approved_vs_forecast');
  ...
  useEffect(() => {
    if (!open) return;
    setComparisonType('approved_vs_forecast');
    setMetric('npv');
    setApprovalSnapshotId('');
    setForecastVersionId('');
    setActualSnapshotId('');
    setOwnerUserId('');
  }, [open]);

  const needsApproval =
    comparisonType === 'approved_vs_forecast' || comparisonType === 'approved_vs_actual';
  const needsForecast =
    comparisonType === 'approved_vs_forecast' || comparisonType === 'forecast_vs_actual';
  const needsActual =
    comparisonType === 'approved_vs_actual' || comparisonType === 'forecast_vs_actual';
  ...
  return (
    <Modal
      open={open}
      onClose={busy ? () => {} : onClose}
      title={isPolish ? 'Zarejestruj wariancję' : 'Record variance'}
      size="md"
      preventOverlayClose={busy}
      preventEscapeClose={busy}
      footer={
        <>
          ...
          <button
            ...
            onClick={handleSubmit}
            disabled={busy}
            data-testid="roi-variance-submit"
            className={`${MENU_1_PRIMARY_CTA} disabled:cursor-not-allowed disabled:opacity-50`}
          >
            <Plus size={16} />
            <span>
    ...
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-variance-comparison">
              {isPolish ? 'Typ porównania' : 'Comparison type'}
            </label>
            <select
              id="roi-variance-comparison"
              value={comparisonType}
              onChange={(e) => setComparisonType(e.target.value as RoiVarianceComparisonType)}
              className={FIELD_CLASS}
              data-testid="roi-variance-comparison"
            >
              {ROI_VARIANCE_COMPARISON_TYPES.map((c) => (
                <option key={c} value={c}>
                  {roiVarianceComparisonTypeLabel(c, isPolish)}
                </option>
              ))}
            </select>
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-variance-metric">
              {isPolish ? 'Metryka' : 'Metric'}
            </label>
            <select
              id="roi-variance-metric"
              value={metric}
              onChange={(e) => setMetric(e.target.value as RoiCompareMetric)}
              className={FIELD_CLASS}
              data-testid="roi-variance-metric"
            >
              {ROI_COMPARE_METRICS.map((m) => (
                <option key={m} value={m}>
                  {roiCompareMetricLabel(m, isPolish)}
                </option>
              ))}
            </select>
          </div>
        ...
        {needsApproval ? (
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-variance-approval">
              {isPolish ? 'Migawka akceptacji' : 'Approval snapshot'}
            </label>
            <select
              id="roi-variance-approval"
              value={approvalSnapshotId}
              onChange={(e) => setApprovalSnapshotId(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-variance-approval"
            >
              <option value="">—</option>
              {approvalSnapshots.map((s) => (
        ...
        {needsForecast ? (
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-variance-forecast">
              {isPolish ? 'Wersja prognozy' : 'Forecast version'}
            </label>
            <select
              id="roi-variance-forecast"
              value={forecastVersionId}
              onChange={(e) => setForecastVersionId(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-variance-forecast"
            >
              <option value="">—</option>
              {forecastVersions.map((f) => (
                <option key={f.forecastVersionId} value={f.forecastVersionId}>
                  #{f.sequenceNumber}
                </option>
              ))}
            </select>
        ...
        {needsActual ? (
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-variance-actual">
              {isPolish ? 'Migawka wykonania' : 'Actual snapshot'}
            </label>
            <select
              id="roi-variance-actual"
              value={actualSnapshotId}
              onChange={(e) => setActualSnapshotId(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-variance-actual"
            >
              <option value="">—</option>
              {actualSnapshots.map((s) => (
                <option key={s.actualSnapshotId} value={s.actualSnapshotId}>
                  #{s.sequenceNumber}
                </option>
              ))}
            </select>
        ...
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-variance-owner">
            {isPolish ? 'Właściciel' : 'Owner'}
          </label>
          <input
            id="roi-variance-owner"
            value={ownerUserId}
            onChange={(e) => setOwnerUserId(e.target.value)}
            className={FIELD_CLASS}
            data-testid="roi-variance-owner"
          />
        </div>
        <ErrorBanner
          message={errorMessage}
...
export const RoiVarianceStatusModal: React.FC<RoiVarianceStatusModalProps> = ({
  ...
  return (
    <Modal
      open={open}
      onClose={busy ? () => {} : onClose}
      title={isPolish ? 'Zmień status wariancji' : 'Change variance status'}
      ...
      footer={
        <>
          ...
          <button
            ...
            disabled={busy}
            data-testid="roi-variance-status-submit"
            className={`${MENU_1_PRIMARY_CTA} disabled:cursor-not-allowed disabled:opacity-50`}
          >
            <span>
    ...
    >
      <div className="space-y-4">
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-variance-status-select">
            {isPolish ? 'Status' : 'Status'}
          </label>
          <select
            id="roi-variance-status-select"
            value={status}
            onChange={(e) => setStatus(e.target.value as RoiVarianceStatus)}
            className={FIELD_CLASS}
            data-testid="roi-variance-status-select"
          >
            {ROI_VARIANCE_STATUSES.map((s) => (
              <option key={s} value={s}>
                {roiVarianceStatusLabel(s, isPolish)}
              </option>
            ))}
          </select>
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-variance-status-owner">
            {isPolish ? 'Właściciel' : 'Owner'}
          </label>
          <input
            id="roi-variance-status-owner"
            value={ownerUserId}
            onChange={(e) => setOwnerUserId(e.target.value)}
            className={FIELD_CLASS}
            data-testid="roi-variance-status-owner"
          />
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-variance-status-reason">
            {isPolish ? 'Notatka (opcjonalnie)' : 'Note (optional)'}
          </label>
          <textarea
            id="roi-variance-status-reason"
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-variance-status-reason"
          />
        </div>
        <ErrorBanner
          message={errorMessage}
          isConflict={isConflict}
          isPolish={isPolish}
          testId="roi-variance-status-error"
        />
      </div>
    </Modal>
  );
...
export const RoiVarianceCauseFormModal: React.FC<RoiVarianceCauseFormModalProps> = ({
  ...
  const handleSubmit = () => {
    ...
    onSubmit({
      causeCategory: causeCategory.trim(),
      contributionPct: contributionPct.trim() === '' ? null : Number(contributionPct),
  ...
  return (
    <Modal
      open={open}
      onClose={busy ? () => {} : onClose}
      title={isPolish ? 'Dodaj przyczynę' : 'Add cause'}
      description={varianceLabel}
      size="sm"
      preventOverlayClose={busy}
      preventEscapeClose={busy}
      footer={
        <>
          ...
          <button
            ...
            onClick={handleSubmit}
            disabled={busy}
            data-testid="roi-variance-cause-submit"
            className={`${MENU_1_PRIMARY_CTA} disabled:cursor-not-allowed disabled:opacity-50`}
          >
            <Plus size={16} />
            <span>
              {busy ? (isPolish ? 'Zapisywanie…' : 'Saving…') : isPolish ? 'Dodaj' : 'Add'}
            </span>
          </button>
        </>
      }
    >
      <div className="space-y-4">
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-cause-category">
            {isPolish ? 'Kategoria (wymagana)' : 'Category (required)'}
          </label>
          <input
            id="roi-cause-category"
            autoFocus
            value={causeCategory}
            onChange={(e) => setCauseCategory(e.target.value)}
            className={FIELD_CLASS}
            data-testid="roi-cause-category"
        ...
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-cause-contribution">
            {isPolish ? 'Udział %' : 'Contribution %'}
          </label>
          <input
            id="roi-cause-contribution"
            type="number"
            value={contributionPct}
            onChange={(e) => setContributionPct(e.target.value)}
            className={FIELD_CLASS}
            data-testid="roi-cause-contribution"
          />
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-cause-narrative">
            {isPolish ? 'Opis (wymagany)' : 'Narrative (required)'}
          </label>
          <textarea
            id="roi-cause-narrative"
            value={narrative}
            onChange={(e) => setNarrative(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-cause-narrative"
            aria-invalid={narrativeError || undefined}
          />
          {narrativeError ? (
            <p className="mt-1 text-[11px] text-c-danger">
              {isPolish ? 'Opis jest wymagany' : 'Narrative is required'}
            </p>
          ) : null}
        </div>
        <ErrorBanner
          ...
          isConflict={isConflict}
          isPolish={isPolish}
          testId="roi-variance-cause-error"
        />
      </div>
    </Modal>
  );
