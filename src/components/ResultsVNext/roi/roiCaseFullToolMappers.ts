...
export const ROI_SCENARIO_OVERRIDE_TARGET_TYPE_LABELS: Record<
  RoiScenarioOverrideTargetType,
  { pl: string; en: string }
> = {
...
export function roiScenarioOverrideTargetTypeLabel(
  value: RoiScenarioOverrideTargetType,
  isPolish: boolean
): string {
  return isPolish
...
export const ROI_EVIDENCE_LINK_PURPOSE_LABELS: Record<
  RoiEvidenceLinkPurpose,
  { pl: string; en: string }
> = {
...
export function roiEvidenceLinkPurposeLabel(
  value: RoiEvidenceLinkPurpose,
  isPolish: boolean
): string {
  return isPolish
    ? ROI_EVIDENCE_LINK_PURPOSE_LABELS[value].pl
    : ROI_EVIDENCE_LINK_PURPOSE_LABELS[value].en;
}

export const ROI_EVIDENCE_LINK_DISPUTE_STATUS_LABELS: Record<
  RoiEvidenceLinkDisputeStatus,
  { pl: string; en: string }
> = {
...
export function roiEvidenceLinkDisputeStatusLabel(
  value: RoiEvidenceLinkDisputeStatus,
  isPolish: boolean
): string {
  return isPolish
    ? ROI_EVIDENCE_LINK_DISPUTE_STATUS_LABELS[value].pl
    : ROI_EVIDENCE_LINK_DISPUTE_STATUS_LABELS[value].en;
}
...
export const ROI_DATA_QUALITY_STATUS_LABELS: Record<
  RoiDataQualityStatus,
  { pl: string; en: string }
> = {
...
export function roiDataQualityStatusLabel(value: RoiDataQualityStatus, isPolish: boolean): string {
  return isPolish
    ? ROI_DATA_QUALITY_STATUS_LABELS[value].pl
    : ROI_DATA_QUALITY_STATUS_LABELS[value].en;
}
...
export const ROI_VARIANCE_COMPARISON_TYPE_LABELS: Record<
  RoiVarianceComparisonType,
  { pl: string; en: string }
> = {
...
export function roiVarianceComparisonTypeLabel(
  value: RoiVarianceComparisonType,
  isPolish: boolean
): string {
  return isPolish
    ? ROI_VARIANCE_COMPARISON_TYPE_LABELS[value].pl
    : ROI_VARIANCE_COMPARISON_TYPE_LABELS[value].en;
}
...
export const ROI_PIR_OUTCOME_LABELS: Record<RoiPirOutcome, { pl: string; en: string }> = {
  ...
  benefits_partially_realized: {
    pl: 'Korzyści częściowo zrealizowane',
    en: 'Benefits partially realized',
  },
...
export const ROI_PIR_TERESA_DISPOSITION_LABELS: Record<
  RoiPirTeresaDraftDisposition,
  { pl: string; en: string }
> = {
...
export function roiPirTeresaDispositionLabel(
  value: RoiPirTeresaDraftDisposition | null,
  isPolish: boolean
): string {
  if (!value) return '—';
  return isPolish
    ? ROI_PIR_TERESA_DISPOSITION_LABELS[value].pl
    : ROI_PIR_TERESA_DISPOSITION_LABELS[value].en;
}
...
export const ROI_FINANCE_RECONCILIATION_STATUS_LABELS: Record<
  RoiFinanceReconciliationStatus,
  { pl: string; en: string }
> = {
  open: { pl: 'Otwarta', en: 'Open' },
...
export function roiFinanceReconciliationStatusLabel(
  value: RoiFinanceReconciliationStatus,
  isPolish: boolean
): string {
  return isPolish
...
export const ROI_FINANCE_RECONCILIATION_STATUS_TONE: Record<
  RoiFinanceReconciliationStatus,
  RoiFinanceReconciliationTone
> = {
  open: 'warning',
  investigating: 'info',
  resolved: 'success',
...
export function calcRunNpvReason(run: RoiCalculationRun | null, isPolish: boolean): string {
  return npvNotCalculableReason(
    run as unknown as Parameters<typeof npvNotCalculableReason>[0],
    isPolish
  );
}
export function calcRunIrrReason(run: RoiCalculationRun | null, isPolish: boolean): string {
  return irrNotCalculableReason(
    run as unknown as Parameters<typeof irrNotCalculableReason>[0],
    isPolish
  );
}
export function forecastVersionNpvReason(fv: RoiForecastVersion | null, isPolish: boolean): string {
  return npvNotCalculableReason(
    fv as unknown as Parameters<typeof npvNotCalculableReason>[0],
    isPolish
  );
}
export function forecastVersionIrrReason(fv: RoiForecastVersion | null, isPolish: boolean): string {
  return irrNotCalculableReason(
    fv as unknown as Parameters<typeof irrNotCalculableReason>[0],
    isPolish
  );
}
