...
export function buildRoiScenarioColumns(isPolish: boolean): TableColumn[] {
  return [
    ...
    },
    {
      ...
      width: '140px',
      render: (row: RoiScenario) => (
        <StatusChip label={roiScenarioTypeLabel(row.scenarioType, isPolish)} tone="neutral" />
      ),
    },
    {
      id: 'description',
      ...
      width: '260px',
      render: (row: RoiScenario) => (
        <span className="text-sm text-c-text-secondary">{row.description ?? '—'}</span>
      ),
    },
    {
      id: 'frozen',
      label: isPolish ? 'Zamrożony' : 'Frozen',
      width: '110px',
      render: (row: RoiScenario) => (
        <span className="text-sm text-c-text-muted">
          {row.frozenAt ? formatRoiDate(row.frozenAt, isPolish) : isPolish ? 'Nie' : 'No'}
        </span>
      ),
    },
    {
...
export function buildRoiScenarioRowMenu(
  ...
  isPolish: boolean,
  handlers: {
    onPreview: (r: RoiScenario) => void;
    onEdit?: (r: RoiScenario) => void;
    onRemove?: (r: RoiScenario) => void;
  }
): StandardRowMenu {
  const lock = getRoiCaseLockInfo(caseStatus);
...
export function buildRoiScenarioPreview(
  ...
  handlers: {
    ...
    onRemoveOverride?: (o: RoiScenarioOverride) => void;
  }
): StandardPreviewProps {
  ...
  return {
    ...
    headerExtra,
    meta: {
      pills: [{ label: roiScenarioTypeLabel(scenario.scenarioType, isPolish), tone: 'neutral' }],
      recommendation,
    },
    details: {
      showWordCount: false,
      ...
      properties: [
        ...
        },
        {
          id: 'frozen',
          label: isPolish ? 'Zamrożony' : 'Frozen',
          value: scenario.frozenAt
            ? formatRoiDate(scenario.frozenAt, isPolish)
            : isPolish
              ? 'Nie'
              : 'No',
        },
        {
          id: 'updated',
          label: isPolish ? 'Zaktualizowano' : 'Updated',
          value: formatRoiDate(scenario.updatedAt, isPolish),
        },
        {
          id: 'createdBy',
          label: isPolish ? 'Utworzył' : 'Created by',
          value: scenario.createdBy,
          mono: true,
        },
      ],
    },
    ...
    relations,
    relationsEmptyLabel: isPolish
      ? 'Brak nadpisań w tym scenariuszu'
          ...
          informational: [
            {
              id: 'manage-overrides',
              variant: 'neutral',
              label: isPolish ? 'Zarządzaj nadpisaniami' : 'Manage overrides',
              onClick: handlers.onManageOverrides,
            },
          ],
        }
      : undefined,
...
export function buildRoiCalculationRunColumns(isPolish: boolean): TableColumn[] {
  return [
    ...
    {
      id: 'completedAt',
      label: isPolish ? 'Ukończono' : 'Completed',
      width: '140px',
      render: (row: RoiCalculationRun) => (
        <span className="text-sm text-c-text-muted tabular-nums">
          {formatRoiDate(row.completedAt, isPolish)}
        </span>
      ),
    },
  ];
...
export function buildRoiCalculationRunRowMenu(
  row: RoiCalculationRun,
  isPolish: boolean,
  onPreview: (r: RoiCalculationRun) => void
): StandardRowMenu {
  return {
    primary: [{ id: 'open', label: isPolish ? 'Otwórz' : 'Open', onClick: () => onPreview(row) }],
    universalHandlers: { preview: () => onPreview(row) },
  };
}

export function buildRoiCalculationRunPreview(
  ...
  return {
    ...
    details: {
      ...
      properties: [
        ...
        {
          id: 'doubleCounting',
          label: isPolish ? 'Nierozwiązane podwójne liczenie' : 'Unresolved double counting',
          value: run.hasUnresolvedDoubleCounting
            ? isPolish
              ? 'Tak'
              : 'Yes'
            : isPolish
        ...
        {
          id: 'mixedCurrency',
          label: isPolish ? 'Błąd mieszanych walut' : 'Mixed-currency failure',
          value: run.hasMixedCurrencyFailure ? (isPolish ? 'Tak' : 'Yes') : isPolish ? 'Nie' : 'No',
        },
        {
          id: 'warnings',
          label: isPolish ? 'Ostrzeżenia' : 'Warnings',
          value: run.warnings.length > 0 ? String(run.warnings.length) : '—',
        },
        {
          id: 'initiatedBy',
          label: isPolish ? 'Zainicjował' : 'Initiated by',
        ...
        },
        {
          id: 'completedAt',
          label: isPolish ? 'Ukończono' : 'Completed',
          value: formatRoiDate(run.completedAt, isPolish),
        },
      ],
    },
...
export function buildRoiApprovalSnapshotColumns(isPolish: boolean): TableColumn[] {
  return [
    {
      id: 'approvedAt',
      label: isPolish ? 'Zaakceptowano' : 'Approved',
      width: '160px',
      render: (row: RoiApprovalSnapshot) => (
        <span className="text-sm text-c-text tabular-nums">
          {formatRoiDate(row.approvedAt, isPolish)}
        </span>
      ),
    },
    {
      id: 'approvedBy',
      label: isPolish ? 'Zaakceptował' : 'Approved by',
      width: '200px',
      render: (row: RoiApprovalSnapshot) => (
        <span className="text-sm text-c-text-secondary font-mono">{row.approvedBy}</span>
      ),
    },
    {
      id: 'calcRun',
      label: isPolish ? 'Przebieg kalkulacji' : 'Calculation run',
      width: '220px',
      render: (row: RoiApprovalSnapshot) => (
        <span className="text-sm text-c-text-muted font-mono">{row.calculationRunId}</span>
      ),
    },
  ];
}
export function buildRoiApprovalSnapshotRowMenu(
  row: RoiApprovalSnapshot,
  isPolish: boolean,
  onPreview: (r: RoiApprovalSnapshot) => void
): StandardRowMenu {
  return {
    primary: [{ id: 'open', label: isPolish ? 'Otwórz' : 'Open', onClick: () => onPreview(row) }],
    universalHandlers: { preview: () => onPreview(row) },
...
export function buildRoiApprovalSnapshotPreview(
  snapshot: RoiApprovalSnapshot,
  isPolish: boolean,
  onClose: () => void
): StandardPreviewProps {
  const { propertyLabel, valueLabel } = propertyLabels(isPolish);
    ...
    details: {
      ...
      properties: [
        ...
        },
        {
          ...
          value: formatRoiDate(snapshot.approvedAt, isPolish),
        },
        {
          id: 'calcRun',
          label: isPolish ? 'Przebieg kalkulacji' : 'Calculation run',
          value: snapshot.calculationRunId,
          mono: true,
        },
      ],
    },
  ...
  return [
    {
      ...
      render: (row: RoiForecastVersion) => (
        <span className="text-sm text-c-text-muted tabular-nums">{row.sequenceNumber}</span>
      ),
    },
    {
      id: 'status',
    ...
    },
    {
      ...
      render: (row: RoiForecastVersion) => (
        <span className="text-sm text-c-text-muted tabular-nums">
          {formatRoiDate(row.publishedAt, isPolish)}
        </span>
      ),
    },
  ];
}
export function buildRoiForecastVersionRowMenu(
  row: RoiForecastVersion,
  isPolish: boolean,
  onPreview: (r: RoiForecastVersion) => void
): StandardRowMenu {
  return {
    primary: [{ id: 'open', label: isPolish ? 'Otwórz' : 'Open', onClick: () => onPreview(row) }],
    universalHandlers: { preview: () => onPreview(row) },
...
export function buildRoiForecastVersionPreview(
  fv: RoiForecastVersion,
  isPolish: boolean,
  onClose: () => void
): StandardPreviewProps {
  const { propertyLabel, valueLabel } = propertyLabels(isPolish);
  return {
    title: isPolish
      ? `Wersja prognozy #${fv.sequenceNumber}`
      : `Forecast version #${fv.sequenceNumber}`,
    onClose,
    meta: {
      pills: [
        {
          label:
            fv.status === 'completed'
              ...
              : isPolish
                ? 'Niepowodzenie'
                : 'Failed',
          tone: fv.status === 'completed' ? 'success' : 'danger',
        },
      ],
    },
    details: {
      ...
      properties: [
        ...
        {
          id: 'publishedBy',
          label: isPolish ? 'Opublikował' : 'Published by',
          value: fv.publishedBy,
          mono: true,
        },
        {
          id: 'publishedAt',
          label: isPolish ? 'Opublikowano' : 'Published at',
          value: formatRoiDate(fv.publishedAt, isPolish),
        },
      ],
    },
...
function renderBenefitsRealizationPct(
  slot: RoiCaseBenefitsRealizationView['benefitsRealizationPct'],
  isPolish: boolean
): React.ReactNode {
  if (slot.status === 'not_yet_available') {
    const label =
      slot.reason === 'not_yet_approved'
        ? isPolish
          ? 'Jeszcze nie zatwierdzono'
          : 'Not yet approved'
        : isPolish
          ? 'Nie zapisano jeszcze wykonania'
          : 'No actual recorded yet';
    return <span className="text-sm text-c-text-muted">{label}</span>;
  }
  return (
    <HonestValueCell
      ...
      notCalculableReason={
        isPolish
          ? 'Zaakceptowana wartość korzyści wynosi zero, więc procent realizacji jest nieokreślony.'
          : 'Approved benefits are zero, so the realization percentage is undefined.'
      }
    />
  );
}

export function buildRoiCaseViewsRows(
  compare: RoiCompareView | null,
  benefitsRealization: RoiCaseBenefitsRealizationView | null
): RoiCaseViewRowVm[] {
  return [
    ...
    },
    {
      ...
      render: (row: RoiCaseViewRowVm) => {
        if (row.id === 'benefits-realization') {
          ...
          return renderBenefitsRealizationPct(v.benefitsRealizationPct, isPolish);
        }
        return (
          <span className="text-sm text-c-text-secondary">
            {row.compare
              ? isPolish
                ? `${Object.keys(row.compare.metrics ?? {}).length} metryk`
                : `${Object.keys(row.compare.metrics ?? {}).length} metrics`
              : isPolish
                ? 'Brak danych'
                : 'No data'}
          </span>
...
export function buildRoiCaseViewsRowMenu(
  row: RoiCaseViewRowVm,
  isPolish: boolean,
  onPreview: (r: RoiCaseViewRowVm) => void
): StandardRowMenu {
  return {
    primary: [{ id: 'open', label: isPolish ? 'Otwórz' : 'Open', onClick: () => onPreview(row) }],
    universalHandlers: { preview: () => onPreview(row) },
...
export function buildRoiCaseViewsPreview(
  row: RoiCaseViewRowVm,
  isPolish: boolean,
  onClose: () => void
): StandardPreviewProps {
          ...
          ? [
              {
                id: 'approved',
                label: isPolish
                  ? 'Zaakceptowane korzyści finansowe'
              ...
              {
                id: 'pct',
                label: isPolish ? 'Procent realizacji' : 'Realization pct',
                value: renderBenefitsRealizationPct(v.benefitsRealizationPct, isPolish),
              },
            ]
          : [
              {
                id: 'missing',
                label: isPolish ? 'Dane' : 'Data',
                value: isPolish ? 'Brak danych dla tej sprawy.' : 'No data for this case.',
              },
            ],
      },
      ai: aiComingSoon(
        isPolish ? ['Wyjaśnij lukę realizacji'] : ['Explain the realization gap'],
        isPolish
      ),
      relations: [],
    };
  ...
  return {
    title: isPolish
      ? 'Porównanie (zaakceptowane / prognoza / wykonanie)'
      : 'Compare (approved / forecast / actual)',
    onClose,
    meta: { pills: [{ label: isPolish ? 'Widok pochodny' : 'Derived view', tone: 'neutral' }] },
    details: {
      showWordCount: false,
      ...
      valueLabel: isPolish
        ? 'Zaakceptowane / Prognoza / Wykonanie'
          ...
          : [
              {
                id: 'missing',
                label: isPolish ? 'Dane' : 'Data',
                value: isPolish ? 'Brak danych porównawczych.' : 'No compare data.',
              },
            ],
    },
  ...
  return [
    {
      ...
      render: (row: RoiActualEntry) => (
        <StatusChip label={roiActualEntryTypeLabel(row.entryType, isPolish)} tone="neutral" />
      ),
    },
    {
      id: 'period',
      ...
      render: (row: RoiActualEntry) => (
        <span className="text-sm text-c-text-secondary">
          {formatRoiDate(row.periodStart, isPolish)} – {formatRoiDate(row.periodEnd, isPolish)}
        </span>
      ),
    },
    {
      ...
      render: (row: RoiActualEntry) => (
        <HonestValueCell
          ...
          format={(v) =>
            row.currency
              ? formatRoiCurrency(v, row.currency, isPolish)
    ...
    },
    {
      ...
      render: (row: RoiActualEntry) => (
        <StatusChip
          label={roiDataQualityStatusLabel(row.dataQualityStatus, isPolish)}
          tone={ROI_DATA_QUALITY_TONE[row.dataQualityStatus]}
        />
      ),
    },
    {
      ...
      width: '160px',
      render: (row: RoiActualEntry) => (
        <span className="text-sm text-c-text-muted">{row.source}</span>
      ),
    },
    {
      id: 'recordedAt',
      ...
      render: (row: RoiActualEntry) => (
        <span className="text-sm text-c-text-muted tabular-nums">
          {formatRoiDate(row.recordedAt, isPolish)}
        </span>
      ),
    },
  ];
...
export function buildRoiActualEntryRowMenu(
  ...
  isPolish: boolean,
  handlers: {
    onPreview: (r: RoiActualEntry) => void;
    onCorrect: (r: RoiActualEntry) => void;
    onVerify: (r: RoiActualEntry) => void;
    onDispute: (r: RoiActualEntry) => void;
  }
): StandardRowMenu {
  return {
    primary: [
      ...
      {
        id: 'correct',
        label: isPolish ? 'Koryguj' : 'Correct',
        onClick: () => handlers.onCorrect(row),
      },
      {
        id: 'verify',
        label: isPolish ? 'Zweryfikuj' : 'Verify',
        onClick: () => handlers.onVerify(row),
      },
      {
        id: 'dispute',
        label: isPolish ? 'Zakwestionuj' : 'Dispute',
        onClick: () => handlers.onDispute(row),
      },
    ],
    universalHandlers: { preview: () => handlers.onPreview(row) },
  };
}
export function buildRoiActualEntryPreview(
  ...
  return {
    ...
    meta: {
      pills: [
        {
          label: roiDataQualityStatusLabel(entry.dataQualityStatus, isPolish),
          tone: ROI_DATA_QUALITY_TONE[entry.dataQualityStatus] === 'danger' ? 'danger' : 'neutral',
        },
      ],
    },
    details: {
      ...
      properties: [
        {
          id: 'entryType',
          label: isPolish ? 'Typ' : 'Type',
          value: roiActualEntryTypeLabel(entry.entryType, isPolish),
        },
        {
          id: 'period',
          label: isPolish ? 'Okres' : 'Period',
          value: `${formatRoiDate(entry.periodStart, isPolish)} – ${formatRoiDate(entry.periodEnd, isPolish)}`,
        },
        {
          id: 'amount',
          label: isPolish ? 'Kwota' : 'Amount',
          value: (
            <HonestValueCell
              ...
              format={(v) =>
                entry.currency
                  ? formatRoiCurrency(v, entry.currency, isPolish)
                  : formatRoiNumber(v, isPolish)
              }
            />
          ),
        ...
        {
          id: 'correctionOf',
          label: isPolish ? 'Korekta wpisu' : 'Correction of entry',
          value: entry.correctionOfActualEntryId ?? '—',
          mono: true,
        },
        {
          id: 'correctionReason',
          label: isPolish ? 'Powód korekty' : 'Correction reason',
          value: entry.correctionReason ?? '—',
        },
        {
          id: 'recordedBy',
          label: isPolish ? 'Zarejestrował' : 'Recorded by',
          value: entry.recordedBy,
          mono: true,
        },
        {
          id: 'recordedAt',
          label: isPolish ? 'Zarejestrowano' : 'Recorded at',
          value: formatRoiDate(entry.recordedAt, isPolish),
        },
        {
          id: 'verifiedBy',
          label: isPolish ? 'Zweryfikował' : 'Verified by',
          value: entry.verifiedBy ?? '—',
          mono: true,
        },
        {
          id: 'verifiedAt',
          label: isPolish ? 'Zweryfikowano' : 'Verified at',
          value: entry.verifiedAt ? formatRoiDate(entry.verifiedAt, isPolish) : '—',
        },
      ],
    },
...
export function buildRoiActualSnapshotColumns(isPolish: boolean): TableColumn[] {
  return [
    ...
    {
      id: 'asOfPeriodEnd',
      label: isPolish ? 'Stan na' : 'As of',
      width: '140px',
      render: (row: RoiActualSnapshot) => (
        <span className="text-sm text-c-text tabular-nums">
          {formatRoiDate(row.asOfPeriodEnd, isPolish)}
        </span>
      ),
    },
    {
    ...
    {
      id: 'unverified',
      label: isPolish ? 'Niezweryfikowane' : 'Unverified',
      width: '130px',
      align: 'right',
      render: (row: RoiActualSnapshot) => (
        <span className="text-sm tabular-nums text-c-text-muted">{row.unverifiedEntryCount}</span>
      ),
    },
    {
      id: 'disputed',
      label: isPolish ? 'Sporne' : 'Disputed',
      width: '110px',
      align: 'right',
      render: (row: RoiActualSnapshot) => (
        <span className="text-sm tabular-nums text-c-text-muted">{row.disputedEntryCount}</span>
      ),
    },
    {
      id: 'publishedAt',
...
export function buildRoiActualSnapshotRowMenu(
  row: RoiActualSnapshot,
  isPolish: boolean,
  onPreview: (r: RoiActualSnapshot) => void
): StandardRowMenu {
  return {
    primary: [{ id: 'open', label: isPolish ? 'Otwórz' : 'Open', onClick: () => onPreview(row) }],
    universalHandlers: { preview: () => onPreview(row) },
...
export function buildRoiActualSnapshotPreview(
  s: RoiActualSnapshot,
  isPolish: boolean,
  onClose: () => void
): StandardPreviewProps {
  ...
  return {
    title: isPolish
      ? `Migawka wykonania #${s.sequenceNumber}`
      : `Actual snapshot #${s.sequenceNumber}`,
    onClose,
    ...
    details: {
      ...
      properties: [
        ...
        {
          id: 'periodsWith',
          label: isPolish ? 'Okresy z wykonaniem' : 'Periods with actual',
          value: `${s.periodsWithActualCount} / ${s.periodsExpectedCount}`,
        },
        {
          id: 'unverified',
          label: isPolish ? 'Niezweryfikowane wpisy' : 'Unverified entries',
          value: String(s.unverifiedEntryCount),
        },
        {
          id: 'disputed',
          label: isPolish ? 'Sporne wpisy' : 'Disputed entries',
          value: String(s.disputedEntryCount),
        },
        {
          id: 'entriesIncluded',
          label: isPolish ? 'Wpisy uwzględnione' : 'Entries included',
          value: String(s.entryIdsIncluded.length),
        },
        {
          id: 'publishedBy',
          label: isPolish ? 'Opublikował' : 'Published by',
...
export function buildRoiVarianceColumns(isPolish: boolean): TableColumn[] {
  return [
    {
      id: 'metric',
      label: isPolish ? 'Metryka' : 'Metric',
      width: '160px',
      render: (row: RoiVariance) => (
        <span className="text-sm font-medium text-c-text">
          {roiCompareMetricLabel(row.metric, isPolish)}
        </span>
      ),
    },
    {
      id: 'comparisonType',
      label: isPolish ? 'Porównanie' : 'Comparison',
      width: '200px',
      render: (row: RoiVariance) => (
        <span className="text-sm text-c-text-secondary">
          {roiVarianceComparisonTypeLabel(row.comparisonType, isPolish)}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row: RoiVariance) => (
        <StatusChip
          label={roiVarianceStatusLabel(row.status, isPolish)}
          tone={
            ROI_VARIANCE_STATUS_TONE[row.status] === 'success'
              ? 'success'
              : ROI_VARIANCE_STATUS_TONE[row.status] === 'warning'
                ? 'warning'
                : 'neutral'
          }
        />
    ...
    },
    {
      ...
      width: '150px',
      render: (row: RoiVariance) => (
        <span className="text-sm text-c-text-muted font-mono">{row.ownerUserId ?? '—'}</span>
      ),
    },
    {
      id: 'updatedAt',
...
export function buildRoiVarianceRowMenu(
  ...
  isPolish: boolean,
  handlers: {
    onPreview: (r: RoiVariance) => void;
    onEditStatus: (r: RoiVariance) => void;
    onAddCause: (r: RoiVariance) => void;
  }
): StandardRowMenu {
  return {
    primary: [
      ...
      {
        id: 'status',
        label: isPolish ? 'Zmień status' : 'Change status',
        onClick: () => handlers.onEditStatus(row),
      },
      {
        id: 'cause',
        label: isPolish ? 'Dodaj przyczynę' : 'Add cause',
        onClick: () => handlers.onAddCause(row),
      },
    ],
    universalHandlers: { preview: () => handlers.onPreview(row) },
  };
...
export function buildRoiVariancePreview(
  ...
  return {
    title: isPolish
      ? `Wariancja: ${roiCompareMetricLabel(variance.metric, isPolish)}`
      : `Variance: ${roiCompareMetricLabel(variance.metric, isPolish)}`,
    onClose: handlers.onClose,
    meta: {
      pills: [
        {
          label: roiVarianceStatusLabel(variance.status, isPolish),
          tone:
    ...
    details: {
      ...
      properties: [
        {
          id: 'comparisonType',
          label: isPolish ? 'Typ porównania' : 'Comparison type',
          value: roiVarianceComparisonTypeLabel(variance.comparisonType, isPolish),
        },
        {
          id: 'baseline',
        ...
        },
        {
          id: 'owner',
          label: isPolish ? 'Właściciel' : 'Owner',
          value: variance.ownerUserId ?? '—',
          mono: true,
        },
        {
          id: 'createdBy',
          label: isPolish ? 'Utworzył' : 'Created by',
          value: variance.createdBy,
          mono: true,
        },
      ],
    },
...
export function buildRoiPirColumns(isPolish: boolean): TableColumn[] {
  return [
    ...
    },
    {
      ...
      render: (row: RoiPostInvestmentReview) => (
        <StatusChip
          label={roiPirStatusLabel(row.status, isPolish)}
    ...
    {
      id: 'outcome',
      label: isPolish ? 'Wynik' : 'Outcome',
      width: '220px',
      render: (row: RoiPostInvestmentReview) => (
        <span className="text-sm text-c-text-secondary">
          {roiPirOutcomeLabel(row.outcome, isPolish)}
        </span>
      ),
    },
    {
      id: 'startedAt',
      label: isPolish ? 'Rozpoczęto' : 'Started',
      width: '140px',
      render: (row: RoiPostInvestmentReview) => (
        <span className="text-sm text-c-text-muted tabular-nums">
          {formatRoiDate(row.startedAt, isPolish)}
        </span>
      ),
    },
    {
      id: 'finalizedAt',
      label: isPolish ? 'Zamknięto' : 'Finalized',
      width: '140px',
      render: (row: RoiPostInvestmentReview) => (
        <span className="text-sm text-c-text-muted tabular-nums">
          {row.finalizedAt ? formatRoiDate(row.finalizedAt, isPolish) : '—'}
        </span>
      ),
    },
  ];
...
export function buildRoiPirRowMenu(
  ...
  const lockedNote = isPolish
    ? 'PIR jest zamknięty — tylko do odczytu.'
    : 'PIR is finalized — read only.';
  ...
  return {
    primary: [
      { id: 'open', label: isPolish ? 'Otwórz' : 'Open', onClick: () => handlers.onPreview(row) },
      ...(editable && handlers.onEditDraft
        ? [
            {
              id: 'edit-draft',
              label: isPolish ? 'Edytuj szkic' : 'Edit draft',
              onClick: () => handlers.onEditDraft!(row),
            },
          ]
        : []),
      ...
      ...(handlers.onAskTeresa
        ...
        : []),
      ...(editable &&
      row.teresaDraftLessonsPayload &&
      !row.teresaDraftDisposition &&
      handlers.onTeresaDisposition
        ? [
            {
              id: 'teresa-disposition',
              label: isPolish ? 'Decyzja o szkicu Teresy' : 'Teresa draft decision',
              onClick: () => handlers.onTeresaDisposition!(row),
            },
          ]
        : []),
    ],
...
export function buildRoiPirPreview(
  ...
  return {
    ...
    meta: {
      pills: [
        {
          label: roiPirStatusLabel(pir.status, isPolish),
          tone: pir.status === 'finalized' ? 'success' : 'neutral',
        },
      ],
    },
    details: {
      ...
      properties: [
        {
          id: 'outcome',
          label: isPolish ? 'Wynik' : 'Outcome',
          value: roiPirOutcomeLabel(pir.outcome, isPolish),
        },
        {
          id: 'lessons',
          label: isPolish ? 'Wnioski' : 'Lessons learned',
          value: pir.lessonsLearned ?? '—',
        },
        {
          id: 'recommendation',
          label: isPolish ? 'Rekomendacja' : 'Recommendation',
          value: pir.recommendation ?? '—',
        },
        {
          id: 'waiver',
          label: isPolish ? 'Powód zniesienia wymogu wariancji' : 'Open-variance waiver reason',
          value: pir.openVarianceWaiverReason ?? '—',
        },
        {
          id: 'teresaDisposition',
          label: isPolish ? 'Decyzja o szkicu Teresy' : 'Teresa draft disposition',
          value: pir.teresaDraftDisposition
            ? isPolish
              ? {
                  accepted: 'Zaakceptowano',
                  rejected: 'Odrzucono',
                  edited_then_accepted: 'Zmieniono i zaakceptowano',
                }[pir.teresaDraftDisposition]
              : pir.teresaDraftDisposition
            : '—',
        },
        {
          id: 'startedBy',
          label: isPolish ? 'Rozpoczął' : 'Started by',
          value: pir.startedBy,
          mono: true,
        },
        {
          id: 'startedAt',
          label: isPolish ? 'Rozpoczęto' : 'Started at',
          value: formatRoiDate(pir.startedAt, isPolish),
        },
        {
          id: 'finalizedBy',
          label: isPolish ? 'Zamknął' : 'Finalized by',
          value: pir.finalizedBy ?? '—',
          mono: true,
        },
        {
          id: 'finalizedAt',
          label: isPolish ? 'Zamknięto' : 'Finalized at',
          value: pir.finalizedAt ? formatRoiDate(pir.finalizedAt, isPolish) : '—',
        },
      ],
    },
    ai: aiComingSoon(
      isPolish ? ['Wygeneruj wnioski (Teresa)'] : ['Draft lessons (Teresa)'],
      isPolish
    ),
    relations: [],
  };
...
export function buildRoiFinanceLinkColumns(isPolish: boolean): TableColumn[] {
  return [
    {
      id: 'financeArtifactType',
      label: isPolish ? 'Typ artefaktu' : 'Artifact type',
      width: '160px',
      render: (row: RoiFinanceLink) => (
        <span className="text-sm font-medium text-c-text">{row.financeArtifactType}</span>
      ),
    },
    {
      id: 'financeArtifactId',
      label: isPolish ? 'ID artefaktu' : 'Artifact ID',
      width: '200px',
      render: (row: RoiFinanceLink) => (
        <span className="text-sm text-c-text-muted font-mono">{row.financeArtifactId}</span>
      ),
    },
    {
      id: 'linkPurpose',
      label: isPolish ? 'Cel powiązania' : 'Link purpose',
      width: '160px',
      render: (row: RoiFinanceLink) => (
        <span className="text-sm text-c-text-secondary">{row.linkPurpose}</span>
      ),
    },
    {
      id: 'asOf',
      ...
      render: (row: RoiFinanceLink) => (
        <span className="text-sm text-c-text-muted tabular-nums">
          {formatRoiDate(row.asOf, isPolish)}
        </span>
      ),
    },
    {
      id: 'linkedBy',
      label: isPolish ? 'Powiązał' : 'Linked by',
      width: '150px',
      render: (row: RoiFinanceLink) => (
        <span className="text-sm text-c-text-muted font-mono">{row.linkedBy}</span>
      ),
    },
  ];
}
export function buildRoiFinanceLinkRowMenu(
  row: RoiFinanceLink,
  caseStatus: RoiCaseStatus,
  isPolish: boolean,
  handlers: { onPreview: (r: RoiFinanceLink) => void; onRemove?: (r: RoiFinanceLink) => void }
): StandardRowMenu {
  const lock = getRoiCaseLockInfo(caseStatus);
  const lockReason = lock ? (isPolish ? lock.reason.pl : lock.reason.en) : undefined;
  return {
...
export function buildRoiFinanceLinkPreview(
  link: RoiFinanceLink,
  isPolish: boolean,
  onClose: () => void
): StandardPreviewProps {
      ...
      properties: [
        {
          id: 'artifactId',
          label: isPolish ? 'ID artefaktu' : 'Artifact ID',
          value: link.financeArtifactId,
          mono: true,
        },
        {
          id: 'versionId',
          label: isPolish ? 'ID wersji' : 'Version ID',
          value: link.financeVersionId,
          mono: true,
        },
        {
          id: 'mappingVersion',
          label: isPolish ? 'Wersja mapowania' : 'Mapping version',
          value: String(link.mappingVersion),
        },
        { id: 'source', label: isPolish ? 'Źródło' : 'Source', value: link.source },
        {
          id: 'asOf',
          label: isPolish ? 'Stan na' : 'As of',
          value: formatRoiDate(link.asOf, isPolish),
        },
        {
          id: 'semanticUnit',
          label: isPolish ? 'Jednostka semantyczna' : 'Semantic unit',
          value: link.semanticUnit ?? '—',
        },
  ...
  return [
    {
      ...
      render: (row: RoiFinanceReconciliation) => (
        <StatusChip
          label={roiFinanceReconciliationStatusLabel(row.status, isPolish)}
          tone={
            ROI_FINANCE_RECONCILIATION_STATUS_TONE[row.status] === 'success'
              ? 'success'
              : ROI_FINANCE_RECONCILIATION_STATUS_TONE[row.status] === 'warning'
                ? 'warning'
                : 'neutral'
          }
        />
    ...
    {
      id: 'divergenceReason',
      label: isPolish ? 'Powód rozbieżności' : 'Divergence reason',
      width: '240px',
      render: (row: RoiFinanceReconciliation) => (
        <span className="text-sm text-c-text-secondary">{row.divergenceReason ?? '—'}</span>
      ),
    },
    {
      id: 'openedAt',
      label: isPolish ? 'Otwarto' : 'Opened',
      width: '130px',
      render: (row: RoiFinanceReconciliation) => (
        <span className="text-sm text-c-text-muted tabular-nums">
          {formatRoiDate(row.openedAt, isPolish)}
        </span>
      ),
    },
  ];
}
export function buildRoiFinanceReconciliationRowMenu(
  row: RoiFinanceReconciliation,
  isPolish: boolean,
  handlers: {
    onPreview: (r: RoiFinanceReconciliation) => void;
    onEditStatus: (r: RoiFinanceReconciliation) => void;
  }
): StandardRowMenu {
...
export function buildRoiFinanceReconciliationPreview(
  r: RoiFinanceReconciliation,
  isPolish: boolean,
  onClose: () => void
): StandardPreviewProps {
  const { propertyLabel, valueLabel } = propertyLabels(isPolish);
  return {
    ...
    meta: {
      pills: [
        {
          label: roiFinanceReconciliationStatusLabel(r.status, isPolish),
    ...
    details: {
      ...
      properties: [
        {
          id: 'financeLinkId',
          label: isPolish ? 'Powiązanie Finance' : 'Finance link',
          value: r.financeLinkId,
          mono: true,
        },
        {
          id: 'roiValue',
        ...
        },
        {
          id: 'divergenceReason',
          label: isPolish ? 'Powód rozbieżności' : 'Divergence reason',
          value: r.divergenceReason ?? '—',
        },
        {
          id: 'openedBy',
          label: isPolish ? 'Otworzył' : 'Opened by',
          value: r.openedBy,
          mono: true,
        },
        {
          id: 'openedAt',
          label: isPolish ? 'Otwarto' : 'Opened at',
          value: formatRoiDate(r.openedAt, isPolish),
        },
        {
          id: 'resolvedBy',
          label: isPolish ? 'Rozwiązał' : 'Resolved by',
          value: r.resolvedBy ?? '—',
          mono: true,
        },
        {
          id: 'resolvedAt',
          label: isPolish ? 'Rozwiązano' : 'Resolved at',
          value: r.resolvedAt ? formatRoiDate(r.resolvedAt, isPolish) : '—',
        },
        {
          id: 'resolutionNotes',
          label: isPolish ? 'Notatki rozwiązania' : 'Resolution notes',
          value: r.resolutionNotes ?? '—',
        },
      ],
    },
    ai: aiComingSoon(isPolish ? ['Wkrótce'] : ['Coming soon'], isPolish),
