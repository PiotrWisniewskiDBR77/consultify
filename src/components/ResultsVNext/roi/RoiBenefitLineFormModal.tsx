...
export const RoiBenefitLineFormModal: React.FC<RoiBenefitLineFormModalProps> = ({
  ...
  useEffect(() => {
    ...
    setRampPeriods(
      b?.rampPeriods !== null && b?.rampPeriods !== undefined ? String(b.rampPeriods) : ''
    );
    ...
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-benefit-category">
              {isPolish ? 'Kategoria' : 'Category'}
            </label>
            <input
              id="roi-benefit-category"
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-benefit-category"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-benefit-label">
              {isPolish ? 'Nazwa pozycji' : 'Benefit line label'}
            </label>
            <input
              id="roi-benefit-label"
              autoFocus
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-benefit-label"
        ...
        <label className={CHECKBOX_LABEL_CLASS}>
          <input
            ...
            data-testid="roi-benefit-is-financial"
          />
        ...
        {isFinancial ? (
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={LABEL_CLASS} htmlFor="roi-benefit-amount">
                {isPolish ? 'Kwota' : 'Amount'}
              </label>
              <input
                id="roi-benefit-amount"
                type="number"
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
                className={FIELD_CLASS}
                data-testid="roi-benefit-amount"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="roi-benefit-currency">
                {isPolish ? 'Waluta' : 'Currency'}
              </label>
              <select
                id="roi-benefit-currency"
                value={currency}
                onChange={(e) => setCurrency(e.target.value)}
                className={FIELD_CLASS}
                data-testid="roi-benefit-currency"
              >
                {CURRENCIES.map((c) => (
        ...
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-benefit-timing-type">
            {isPolish ? 'Typ harmonogramu' : 'Timing type'}
          </label>
          <select
            id="roi-benefit-timing-type"
            value={timingType}
            onChange={(e) => setTimingType(e.target.value as RoiTimingType)}
            className={FIELD_CLASS}
            data-testid="roi-benefit-timing-type"
          >
        ...
        {timingType === 'one_time' ? (
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-benefit-one-time-date">
              {isPolish ? 'Data' : 'Date'}
            </label>
            <input
              id="roi-benefit-one-time-date"
              type="date"
              value={oneTimeDate}
              onChange={(e) => setOneTimeDate(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-benefit-one-time-date"
            />
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={LABEL_CLASS} htmlFor="roi-benefit-recurrence-start">
                {isPolish ? 'Początek' : 'Start'}
              </label>
              <input
                id="roi-benefit-recurrence-start"
                type="date"
                value={recurrenceStart}
                onChange={(e) => setRecurrenceStart(e.target.value)}
                className={FIELD_CLASS}
                data-testid="roi-benefit-recurrence-start"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="roi-benefit-recurrence-end">
                {isPolish ? 'Koniec' : 'End'}
              </label>
              <input
                id="roi-benefit-recurrence-end"
                type="date"
                value={recurrenceEnd}
                onChange={(e) => setRecurrenceEnd(e.target.value)}
                className={FIELD_CLASS}
                data-testid="roi-benefit-recurrence-end"
              />
            </div>
            <div>
              <label className={LABEL_CLASS} htmlFor="roi-benefit-cadence">
                {isPolish ? 'Cykl' : 'Cadence'}
              </label>
              <select
                id="roi-benefit-cadence"
                value={recurrenceCadence}
                onChange={(e) => setRecurrenceCadence(e.target.value as RoiRecurrenceCadence | '')}
                className={FIELD_CLASS}
                data-testid="roi-benefit-cadence"
              >
                <option value="">—</option>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-benefit-ramp">
              {isPolish ? 'Okresy narastania' : 'Ramp periods'}
            </label>
            <input
              id="roi-benefit-ramp"
              type="number"
              ...
              min="1"
              value={rampPeriods}
              onChange={(e) => setRampPeriods(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-benefit-ramp"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-benefit-double-counting-group">
              {isPolish ? 'Grupa podwójnego liczenia' : 'Double-counting group'}
            </label>
            <input
              id="roi-benefit-double-counting-group"
              value={doubleCountingGroup}
              onChange={(e) => setDoubleCountingGroup(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-benefit-double-counting-group"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-benefit-confidence">
              {isPolish ? 'Pewność' : 'Confidence'}
            </label>
            <select
              id="roi-benefit-confidence"
              value={confidence}
              onChange={(e) => setConfidence(e.target.value as RoiConfidenceLevel | '')}
              className={FIELD_CLASS}
              data-testid="roi-benefit-confidence"
            >
              <option value="">—</option>
        ...
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-benefit-double-counting-note">
            {isPolish ? 'Rozstrzygnięcie podwójnego liczenia' : 'Double-counting resolution note'}
          </label>
          <textarea
            id="roi-benefit-double-counting-note"
            value={doubleCountingNote}
            onChange={(e) => setDoubleCountingNote(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-benefit-double-counting-note"
          />
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-benefit-description">
            {isPolish ? 'Opis' : 'Description'}
          </label>
          <textarea
            id="roi-benefit-description"
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-benefit-description"
          />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-benefit-source">
              {isPolish ? 'Źródło' : 'Source'}
            </label>
            <input
              id="roi-benefit-source"
              value={source}
              onChange={(e) => setSource(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-benefit-source"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-benefit-owner">
              {isPolish ? 'Właściciel' : 'Owner'}
            </label>
            <input
              id="roi-benefit-owner"
              value={ownerUserId}
              onChange={(e) => setOwnerUserId(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-benefit-owner"
            />
          </div>
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-benefit-reason">
            {isPolish ? 'Notatka do audytu (opcjonalnie)' : 'Audit note (optional)'}
          </label>
          <textarea
            id="roi-benefit-reason"
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-benefit-reason"
          />
        </div>
        ...
        {errorMessage ? (
          <div
            ...
            data-testid="roi-benefit-line-error"
          >
