...
export const RoiAssumptionFormModal: React.FC<RoiAssumptionFormModalProps> = ({
  ...
  useEffect(() => {
    ...
    setDownsideValue(
      a?.downsideValue !== null && a?.downsideValue !== undefined ? String(a.downsideValue) : ''
    );
    setUpsideValue(
      a?.upsideValue !== null && a?.upsideValue !== undefined ? String(a.upsideValue) : ''
    );
    ...
    setSensitivityRank(
      a?.sensitivityRank !== null && a?.sensitivityRank !== undefined
        ? String(a.sensitivityRank)
        : ''
    );
    setNotes(a?.notes ?? '');
    setReason('');
    ...
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-category">
              {isPolish ? 'Kategoria' : 'Category'}
            </label>
            <input
              id="roi-assumption-category"
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-category"
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-label">
              {isPolish ? 'Nazwa założenia' : 'Assumption label'}
            </label>
            <input
              id="roi-assumption-label"
              autoFocus
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-label"
        ...
        <div className="grid grid-cols-4 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-base">
              {isPolish ? 'Wartość bazowa' : 'Base value'}
            </label>
            <input
              id="roi-assumption-base"
              type="number"
              value={baseValue}
              onChange={(e) => setBaseValue(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-base"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-downside">
              {isPolish ? 'Pesymistyczna' : 'Downside'}
            </label>
            <input
              id="roi-assumption-downside"
              type="number"
              value={downsideValue}
              onChange={(e) => setDownsideValue(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-downside"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-upside">
              {isPolish ? 'Optymistyczna' : 'Upside'}
            </label>
            <input
              id="roi-assumption-upside"
              type="number"
              value={upsideValue}
              onChange={(e) => setUpsideValue(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-upside"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-unit">
              {isPolish ? 'Jednostka' : 'Unit'}
            </label>
            <input
              id="roi-assumption-unit"
              value={unit}
              onChange={(e) => setUnit(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-unit"
            />
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-confidence">
              {isPolish ? 'Pewność' : 'Confidence'}
            </label>
            <select
              id="roi-assumption-confidence"
              value={confidence}
              onChange={(e) => setConfidence(e.target.value as RoiConfidenceLevel | '')}
              className={FIELD_CLASS}
              data-testid="roi-assumption-confidence"
            >
              <option value="">—</option>
          ...
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-sensitivity">
              {isPolish ? 'Ranga wrażliwości' : 'Sensitivity rank'}
            </label>
            <input
              id="roi-assumption-sensitivity"
              type="number"
              step="1"
              value={sensitivityRank}
              onChange={(e) => setSensitivityRank(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-sensitivity"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-owner">
              {isPolish ? 'Właściciel' : 'Owner'}
            </label>
            <input
              id="roi-assumption-owner"
              value={ownerUserId}
              onChange={(e) => setOwnerUserId(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-owner"
            />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-evidence">
              {isPolish ? 'Dowód (referencja)' : 'Evidence ref'}
            </label>
            <input
              id="roi-assumption-evidence"
              value={evidenceRef}
              onChange={(e) => setEvidenceRef(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-evidence"
            />
          </div>
          <div>
            <label className={LABEL_CLASS} htmlFor="roi-assumption-source">
              {isPolish ? 'Źródło' : 'Source'}
            </label>
            <input
              id="roi-assumption-source"
              value={source}
              onChange={(e) => setSource(e.target.value)}
              className={FIELD_CLASS}
              data-testid="roi-assumption-source"
            />
          </div>
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-assumption-notes">
            {isPolish ? 'Notatki' : 'Notes'}
          </label>
          <textarea
            id="roi-assumption-notes"
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-assumption-notes"
          />
        </div>
        <div>
          <label className={LABEL_CLASS} htmlFor="roi-assumption-reason">
            {isPolish ? 'Notatka do audytu (opcjonalnie)' : 'Audit note (optional)'}
          </label>
          <textarea
            id="roi-assumption-reason"
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            className={TEXTAREA_CLASS}
            data-testid="roi-assumption-reason"
          />
        </div>
        ...
        {errorMessage ? (
          <div
            ...
            data-testid="roi-assumption-error"
          >
