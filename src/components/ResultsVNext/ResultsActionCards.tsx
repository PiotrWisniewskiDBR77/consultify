...
export function ResultsActionCards() {
  ...
  return <section aria-label={t('results.actionCards.aria', 'Action cards — Results')} className="space-y-3 p-4">{cards.map((card) => <ActionCard key={card.id} card={card} />)}</section>;
}
