...
export const FullPilotWorkspace: React.FC<FullPilotWorkspaceProps> = ({
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-navy-900 p-6 rounded-xl border border-slate-200 dark:border-c-border-subtle">
          ...
          {isEditing ? (
            ...
            <ul className="list-disc list-inside space-y-2 text-slate-700 dark:text-slate-300">
              {pilotInitiative.scopeIn?.map((item, i) => <li key={i}>{item}</li>) || (
                <li className="text-slate-500 italic">No scope items defined</li>
              )}
            </ul>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-navy-900 p-6 rounded-xl border border-slate-200 dark:border-c-border-subtle">
          ...
          {isEditing ? (
            ...
            <ul className="list-disc list-inside space-y-2 text-slate-700 dark:text-slate-300">
              {pilotInitiative.scopeOut?.map((item, i) => <li key={i}>{item}</li>) || (
                <li className="text-slate-500 italic">No exclusion items defined</li>
              )}
            </ul>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-4 bg-slate-50 dark:bg-navy-950 rounded-lg text-center">
          ...
          <div className="text-lg font-semibold">
            {pilotInitiative.sponsor?.firstName || 'Not Assigned'}
          </div>
        </div>
        <div className="p-4 bg-slate-50 dark:bg-navy-950 rounded-lg text-center border-2 border-blue-500/20">
          ...
          <div className="text-lg font-semibold">
            {pilotInitiative.ownerBusiness?.firstName || 'Not Assigned'}
          </div>
        </div>
        <div className="p-4 bg-slate-50 dark:bg-navy-950 rounded-lg text-center">
          <div className="text-sm text-slate-500 uppercase font-bold mb-2">Tech Lead</div>
          <div className="text-lg font-semibold">
            {pilotInitiative.ownerExecution?.firstName || 'Not Assigned'}
          </div>
        </div>
      </div>
    </div>
          ...
          <button
            onClick={() =>
              setEditData({
                ...
                milestones: [
                  ...(editData.milestones || []),
                  { name: 'New Milestone', date: '', status: 'pending' },
                ],
              })
            }
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'dashboard':
        return (
          <div className="p-10 text-center text-slate-500">Dashboard Visualization Placeholder</div>
        );
      case 'evaluation':
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div
              className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-500/20 p-8 rounded-2xl flex flex-col items-center justify-center text-center cursor-pointer hover:scale-105 transition-transform"
              onClick={onNextStep}
            >
      ...
      <div className="h-20 border-b border-c-border-subtle px-8 flex items-center justify-between bg-navy-900/50 backdrop-blur-sm sticky top-0 z-10">
        ...
        <div className="flex items-center gap-4">
          {isEditing ? (
            <button
              onClick={handleSave}
              className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg text-sm font-bold transition-colors"
            >
