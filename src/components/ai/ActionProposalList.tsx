...
export const ActionProposalList: React.FC<ActionProposalListProps> = ({
  proposals,
  onSelect,
  selectedId,
}) => {
  ...
  const getRiskColor = (risk: string) => {
    switch (risk) {
      ...
      default:
        return 'text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-navy-800/300/10 border-slate-500/20';
    }
  };
      ...
      {proposals.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-navy-900 rounded-xl border border-dashed border-slate-200 dark:border-navy-700">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            No pending action proposals found.
          </p>
        </div>
      ) : (
          ...
          <button
            ...
            className={`w-full text-left p-4 rounded-xl border transition-all duration-200 group relative overflow-hidden ${
              selectedId === proposal.proposal_id
                ...
                : 'bg-white dark:bg-navy-900 border-slate-200 dark:border-navy-700 hover:border-indigo-400 dark:hover:border-indigo-500/50 hover:bg-slate-50 dark:hover:bg-white/5'
            }`}
          >
