...
export const ActionAuditTrail: React.FC<ActionAuditTrailProps> = ({
    ...
    return (
      <div className="space-y-4 animate-pulse">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-24 bg-slate-100 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-navy-700"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      {records.length === 0 ? (
        <div className="text-center py-12 opacity-50 border border-dashed border-slate-200 dark:border-navy-700 rounded-xl">
        ...
        records.map((record) => (
          <div
            key={record.id}
            className="p-4 bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 rounded-xl shadow-sm hover:shadow-md transition-shadow"
          >
