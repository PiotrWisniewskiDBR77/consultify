      ...
      {diff.map((line, idx) => (
        ...
        >
          ...
          <span className="w-6 flex-shrink-0">
            ...
            {line.type === 'removed' && <Minus className="w-4 h-4 text-danger-600" />}
            {line.type === 'modified' && <ArrowRight className="w-4 h-4 text-yellow-600" />}
          </span>
          <div className="flex-1 min-w-0">
            {line.type === 'modified' ? (
              <>
                <div className="line-through text-danger-600">{line.originalLine}</div>
                <div className="text-green-600">{line.suggestedLine}</div>
              </>
            ) : line.type === 'removed' ? (
              <span className="text-danger-600 line-through">{line.originalLine}</span>
            ) : line.type === 'added' ? (
