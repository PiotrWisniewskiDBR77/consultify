...
export const DigitizationToolTab: React.FC<DigitizationToolTabProps> = ({ analysis, onUpdate }) => {
  ...
  const [pendingChanges, setPendingChanges] = useState<
    Map<string, { currentLevel: number; targetLevel: number }>
  >(new Map());
  ...
  const saveChanges = useCallback(async () => {
    ...
    try {
      ...
      toast.success('Changes zapisane');
    } catch (e) {
  ...
  return (
    <div className="flex h-full">
      {/* Axis Sidebar */}
      <div className="w-72  border-slate-200 dark:border-navy-700 bg-white dark:bg-navy-900 shrink-0 overflow-y-auto">
        <div className="p-4">
      ...
      {showKeyboardHelp && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white dark:bg-navy-900 rounded-xl p-6 max-w-md w-full mx-4 shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center">
                <Keyboard size={20} className="text-emerald-500" />
              </div>
              <h3 className="text-lg font-bold text-navy-900 dark:text-white">Keyboard Shortcuts</h3>
            </div>
            <div className="space-y-3">
      ...
      <div className="fixed bottom-4 right-4 z-40">
        ...
        >
          <Keyboard size={14} />
          <span>Press ? to see shortcuts</span>
        </button>
      </div>
    </div>
  );
