  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      const result = await Api.post('/api/economics/budgets', {
        title: title.trim(), periodStart, periodEnd, granularity,
      });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-navy-900 rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
        ...
        <div>
          <label className="text-xs text-slate-500">{t('finance.prediction.budgetName', 'Nazwa')}</label>
          <input value={title} onChange={(e) => setTitle(e.target.value)}
            placeholder={t('finance.prediction.namePlaceholder', 'e.g., Budget 2026 Q1-Q4') as string}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-slate-500">{t('finance.prediction.periodStart', 'Okres od')}</label>
            <input type="month" value={periodStart} onChange={(e) => setPeriodStart(e.target.value)}
              className="mt-1 w-full px-3 py-2 border border-slate-200 dark:border-navy-600 rounded-lg text-sm bg-white dark:bg-navy-800" />
          </div>
          <div>
            <label className="text-xs text-slate-500">{t('finance.prediction.periodEnd', 'Okres do')}</label>
            <input type="month" value={periodEnd} onChange={(e) => setPeriodEnd(e.target.value)}
              className="mt-1 w-full px-3 py-2 border border-slate-200 dark:border-navy-600 rounded-lg text-sm bg-white dark:bg-navy-800" />
          </div>
        </div>
        <div>
          <label className="text-xs text-slate-500">{t('finance.prediction.granularity', 'Granulacja')}</label>
