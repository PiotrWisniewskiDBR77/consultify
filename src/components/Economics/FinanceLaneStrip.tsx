...
import type { FinanceLaneRun, FinanceLaneStep, KpiLinkageStatus } from '../../services/api/v8/finance';
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <div className="h-8 inline-flex items-center gap-1.5 rounded-full px-2.5 text-[11px] font-medium border whitespace-nowrap bg-white/60 text-slate-600 border-slate-200/60 dark:bg-white/[0.02] dark:text-slate-300 dark:border-white/[0.06]">
        <span className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${KPI_DOT_COLORS[activeLaneRun.kpiLinkageStatus]}`} />
        <span>{kpiLabels[activeLaneRun.kpiLinkageStatus]}</span>
      </div>

      <div className="h-8 inline-flex items-center gap-1.5 rounded-full px-2.5 text-[11px] font-medium border whitespace-nowrap bg-white/60 text-slate-600 border-slate-200/60 dark:bg-white/[0.02] dark:text-slate-300 dark:border-white/[0.06]">
        <span className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${activeLaneRun.versionType === 'actual' ? 'bg-violet-400' : 'bg-sky-400'}`} />
        <span>{activeLaneRun.versionType === 'actual' ? t('finance.lane.version.actual', 'Actual') : t('finance.lane.version.current', 'Current')}</span>
      </div>
    </div>
  );
};
