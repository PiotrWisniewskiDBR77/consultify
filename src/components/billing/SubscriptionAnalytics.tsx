...
import { formatListCurrency, formatListDate, formatListNumber, localeListy } from '../../utils/listDateFormat';
...
export const SubscriptionAnalytics: React.FC = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      setError(t('billing.analytics.loadError', 'Failed to load analytics data'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const formatCurrency = (amount: number, decimals = 0) =>
    formatListCurrency(amount / 100, 'USD', {
  ...
  const formatPercent = (value: string | number) => {
    ...
    const sign = num >= 0 ? '+' : '';
    return `${sign}${formatListNumber(num, '0', {
  ...
  const formatMonth = (dateStr: string) =>
    new Intl.DateTimeFormat(localeListy(), { month: 'short', year: 'numeric' }).format(
      new Date(dateStr + '-01')
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-900 dark:text-white">
          {t('billing.analytics.title', 'Subscription analytics')}
        </h2>
        <div className="flex items-center gap-2">
          ...
          >
            <option value={7}>{t('billing.analytics.last7Days', 'Last 7 days')}</option>
            <option value={30}>{t('billing.analytics.last30Days', 'Last 30 days')}</option>
            <option value={90}>{t('billing.analytics.last90Days', 'Last 90 days')}</option>
            <option value={180}>{t('billing.analytics.last6Months', 'Last 6 months')}</option>
            <option value={365}>{t('billing.analytics.lastYear', 'Last year')}</option>
          </select>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-navy-800 rounded-xl p-4 border border-slate-200 dark:border-navy-700">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
              {t('billing.analytics.mrrLabel', 'Monthly recurring revenue (MRR)')}
            </span>
          ...
          {mrrTrend?.summary && (
            ...
            >
              ...
              <span className="text-slate-500 dark:text-slate-400 ml-1">
                {t('billing.analytics.vsPreviousPeriod', 'vs previous period')}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-navy-800 rounded-xl p-4 border border-slate-200 dark:border-navy-700">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
              {t('billing.analytics.arrLabel', 'Annual recurring revenue (ARR)')}
            </span>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
            {t('billing.analytics.activeSubscriptions', '{{count}} active subscriptions', {
        ...
        <div className="bg-white dark:bg-navy-800 rounded-xl p-4 border border-slate-200 dark:border-navy-700">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
              {t('billing.analytics.churnRate', 'Churn rate')}
            </span>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">{t('billing.analytics.monthlyMrrChurn', 'Monthly MRR churn')}</p>
        </div>
        ...
        <div className="bg-white dark:bg-navy-800 rounded-xl p-4 border border-slate-200 dark:border-navy-700">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
              {t('billing.analytics.ltvLabel', 'Customer lifetime value (LTV)')}
            </span>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
            {t('billing.analytics.arpaLabel', 'Average revenue per account (ARPA)')}: {formatCurrency(ltv?.arpa || 0)}/{t('billing.analytics.monthShort', 'mo.')}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-navy-800 rounded-xl p-6 border border-slate-200 dark:border-navy-700">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">{t('billing.analytics.mrrTrend', 'MRR trend')}</h3>
        ...
        <div className="bg-white dark:bg-navy-800 rounded-xl p-6 border border-slate-200 dark:border-navy-700">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">
            {t('billing.analytics.revenueByPlan', 'Revenue by plan')}
          </h3>
          <div className="space-y-4">
                ...
                <div key={plan.plan_id}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm text-slate-500 dark:text-slate-400">
                      {t('billing.analytics.subscribersCount', '{{count}} subscribers', {
            ...
            {(!mrr?.byPlan || mrr.byPlan.length === 0) && (
              <div className="text-center py-8 text-slate-500 dark:text-slate-400">
                {t('billing.analytics.noPlanDistribution', 'No plan revenue data available')}
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-navy-800 rounded-xl p-6 border border-slate-200 dark:border-navy-700">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">
            {t('billing.analytics.churnAnalysis', 'Churn analysis')}
          </h3>
          <div className="overflow-x-auto">
            ...
            >
              <thead>
                <tr className="text-left text-xs text-slate-500 dark:text-slate-400">
                  <th className="pb-2">{t('billing.analytics.month', 'Month')}</th>
                  <th className="pb-2 text-right">{t('billing.analytics.churned', 'Churned')}</th>
                  <th className="pb-2 text-right">{t('billing.analytics.mrrLost', 'MRR lost')}</th>
                  <th className="pb-2 text-right">{t('billing.analytics.rate', 'Rate')}</th>
                </tr>
              </thead>
            ...
            {(!churn?.data || churn.data.length === 0) && (
              <div className="text-center py-8 text-slate-500 dark:text-slate-400">
                {t('billing.analytics.noChurnData', 'No churn data available')}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-navy-800 rounded-xl p-6 border border-slate-200 dark:border-navy-700">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">
            {t('billing.analytics.expansionRevenue', 'Expansion revenue')}
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="text-left text-xs text-slate-500 dark:text-slate-400">
                  <th className="pb-2">{t('billing.analytics.month', 'Month')}</th>
                  <th className="pb-2 text-right">{t('billing.analytics.expansion', 'Expansion')}</th>
                  <th className="pb-2 text-right">{t('billing.analytics.contraction', 'Contraction')}</th>
                  <th className="pb-2 text-right">{t('billing.analytics.net', 'Net')}</th>
                </tr>
              </thead>
              <tbody className="text-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          {(!cohorts?.cohorts || cohorts.cohorts.length === 0) && (
            <div className="col-span-full text-center py-8 text-slate-500 dark:text-slate-400">
              {t('billing.analytics.noCohortData', 'No cohort data available')}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-navy-800 rounded-xl p-6 border border-slate-200 dark:border-navy-700">
        <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">
          {t('billing.analytics.customerValueMetrics', 'Customer value metrics')}
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="text-center p-4 rounded-lg bg-primary-50 dark:bg-primary-900/20">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">{t('billing.analytics.lifetimeValue', 'Lifetime value')}</p>
          ...
          <div className="text-center p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">
              {t('billing.analytics.arpaLabel', 'Average revenue per account (ARPA)')}
            </p>
            <p className="text-xl font-bold text-blue-600 dark:text-blue-400">
              {formatCurrency(ltv?.arpa || 0)}/{t('billing.analytics.monthShort', 'mo.')}
            </p>
          </div>
          <div className="text-center p-4 rounded-lg bg-green-50 dark:bg-green-900/20">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">{t('billing.analytics.avgRevenuePerCustomer', 'Avg. revenue per customer')}</p>
            <p className="text-xl font-bold text-green-600 dark:text-green-400">
              {formatCurrency(ltv?.avgRevenuePerCustomer || 0)}
            </p>
          </div>
          <div className="text-center p-4 rounded-lg bg-amber-50 dark:bg-amber-900/20">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">{t('billing.analytics.avgLifespan', 'Avg. lifespan')}</p>
            <p className="text-xl font-bold text-amber-600 dark:text-amber-400">
              {ltv?.avgLifespanMonths || 0} {t('billing.analytics.monthShort', 'mo.')}
            </p>
          </div>
          <div className="text-center p-4 rounded-lg bg-slate-50 dark:bg-slate-700">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">{t('billing.analytics.monthlyChurn', 'Monthly churn')}</p>
