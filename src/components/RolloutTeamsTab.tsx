  ...
  const defaultRoles = [
    {
      ...
      responsibilities: 'Budget approval, strategic alignment, conflict resolution.',
    },
    {
      role: 'Program Manager',
      responsibilities: 'Day-to-day execution, risk management, reporting.',
    },
    { role: 'PMO Lead', responsibilities: 'Standards, tools, reporting, administrative support.' },
    {
      role: 'Change Manager',
      responsibilities: 'Communication, training, stakeholder engagement.',
    },
  ];

  const roles =
    data?.governance?.roles && data.governance.roles.length > 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-navy-900 rounded-xl border border-slate-200 dark:border-navy-700 overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-100 dark:border-navy-700 bg-slate-50 dark:bg-navy-800/50 flex justify-between items-center">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {roles.map((item, i) => (
                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-white/5">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-sm italic cursor-not-allowed">
      ...
      <div className="bg-white dark:bg-navy-900 rounded-xl border border-slate-200 dark:border-navy-700 p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {workstreams.map((ws, i) => (
            <div
              key={ws.id}
              className="border border-slate-200 dark:border-navy-700 rounded-lg p-4 hover:border-blue-500/50 transition-colors bg-slate-50 dark:bg-navy-950"
            >
              <div className="flex justify-between items-start mb-3">
                <input
                  ...
                  className="font-bold bg-transparent border-b border-transparent hover:border-slate-300 dark:border-navy-700 focus:border-blue-500 outline-none w-full mr-2"
                />
                <button
          ...
          {workstreams.length === 0 && (
            <div className="col-span-full py-8 text-center text-slate-400 dark:text-slate-500 italic border border-dashed border-slate-200 dark:border-navy-700 rounded-lg">
