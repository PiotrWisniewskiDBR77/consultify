/**
 * SIRI session runtime — HTTP-backed (T4, 2026-08-13).
 *
 * ★ SAME KERNEL, SAME ENDPOINTS AS DRD. This class is the SIRI-branded twin
 * of `src/method-core/methods/drd/drdHttpSessionRuntime.ts` — same
 * `@/method-core/api/methodCoreApi` calls, same server
 * (`server/src/routes/method-core.routes.ts`), same read/write discipline
 * (every read re-fetches, every write goes over HTTP first). The kernel
 * itself carries no `if (methodId === 'siri')` branch (see
 * `server/src/method-core/MethodSessionService.ts` header) — the ONLY thing
 * that makes this "the SIRI runtime" is that callers construct/`create()` it
 * with `methodPackId: 'siri'` (`SIRI_METHOD_PACK_ID`,
 * `./compileSiriPack.ts`). Every method below is otherwise pack-agnostic —
 * duplicated here (not imported from the `drd` folder) only because this
 * program's scope for T4 forbids touching `src/method-core/methods/drd/**`.
 *
 * ★ NOT YET WIRED INTO `SIRIAssessmentEditor.tsx` — mirrors DRD's own
 * current state exactly (`DrdHttpSessionRuntime`'s header: "NOT YET WIRED
 * INTO `DrdMethodWorkspaceScreen.tsx`"). CLAUDE.md rule #7 ("Piotr nigdy nie
 * jest pierwszym testerem wizualnym") requires a clean dev-render screenshot
 * + Piotr's acceptance, gated by a default-OFF flag, before any runtime swap
 * changes what Piotr sees. Wiring this into the SIRI editor is deliberately
 * left for that gated step, not done blind here.
 *
...
export interface SiriHttpRuntimeState {
   ...
   * reconstructed from any other localStorage content. TIER (SIRI
   * Prioritisation Matrix) is deliberately NOT computed here — it is a
   * separate view (`siriTierView.ts`) gated on `session.state === 'frozen'`,
   * never run against live/unfrozen data (ASSESSMENT_KB_SIRI.md §4). */
   ...
   * a known gap tracked alongside the DRD runtime, not a silent fabrication). */
...
export class SiriHttpSessionRuntime {
  ...
  async refresh(): Promise<void> {
    ...
    try {
      ...
      if (session.state === 'frozen' || session.state === 'closed') {
        ...
        // Session left frozen/closed (e.g. after a reopen path) — an Output
        // from a PRIOR state must never linger and be mistaken for the
        // current one.
        output = null;
      }
  ...
  // -- creation (mirrors DrdHttpSessionRuntime.create) -----------------------
  ...
  /** `unitId` is one of the 16 SIRI dimensions (`SIRI_PRIORITISATION_AREAS`,
   * `@/services/siriStructure`), `level` a Band 0–5. The kernel event store
   * itself does not enforce no-leapfrog — that guard lives client-side in
   * `siriAdapter.resolveOpenLevels` / `siriWorkspaceView.ts`, which callers
   * MUST consult before calling this. */
  ...
  /** `evidenceType: 'factory_observation'` is a SIRI-distinct Evidence Item
   * type (see `siriWorkspaceView.ts` — "factory observation as a distinct
   * Evidence Item type") but this method accepts any evidenceType string;
   * the kernel event schema itself is method-agnostic. */
   ...
   * (or otherwise resolve) before retrying, never auto-overwrite. Also the
   * "restart/reopen" path: `to: 'active'` from a `frozen` session creates a
   * NEW session revision server-side (see `MethodSessionService`); this
   * response still reports the ORIGINAL (still-frozen) session — the caller
   * must list/refresh to discover the new revision's id, matching the
   * kernel's own documented behaviour. */
  ...
  /** Freezes the session -> Output. TIER (SIRI Prioritisation Matrix) may
   * only be computed AFTER this resolves and `output` is non-null — see
   * `siriTierView.siriTierAvailability()`, which this runtime does not call
   * itself (a separate screen owns that gate). */
  ...
  // -- Teresa --------------------------------------------------------------
  // Note: kernel-level, Teresa may never CONFIRM a Band on SIRI's behalf
  // (siriWorkspaceView.ts "Teresa never confirms a Band" — the confirming-
  // actor guard accepts only participant/approver). This runtime only
  // relays preview/commit round-trips; it does not weaken that guard.
   ...
   * automatically; only a direct user action may drop queued writes without
  ...
  /**
   * TEST/HARNESS ONLY — synthetically overlays a state patch so unit tests
   * and a future dev-render harness can reach offline/conflict/recovery
   * deterministically without a genuinely flaky network. Mirrors
   * `DrdHttpSessionRuntime.debugForceState` — same escape-hatch shape, no
