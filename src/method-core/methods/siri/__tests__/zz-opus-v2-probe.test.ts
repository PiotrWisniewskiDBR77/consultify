...
const frozen = {
  vertical_integration: 2,
  horizontal_integration: 1,
  integrated_product_lifecycle: 3,
  shop_floor_automation: 2,
  enterprise_automation: 1,
  facility_automation: 2,
  shop_floor_connectivity: 3,
  enterprise_connectivity: 2,
  facility_connectivity: 1,
  shop_floor_intelligence: 1,
  enterprise_intelligence: 2,
  facility_intelligence: 1,
  workforce_learning: 3,
  leadership_competency: 2,
  strategy_governance: 2,
  inter_intra_collaboration: 1,
};

describe('OPUS PROBE — COORD-08 zero cichej zmiany', () => {
  it('★ BEZ FLAGI domyslna sciezka to legacy_v1', () => {
    const r = siriAdapter.prioritise!({
      frozenUnitLevels: frozen,
      parameters: {
        sessionState: 'frozen',
  ...
  it('★ prioritise() ODRZUCA dane niezamrozone', () => {
    expect(() =>
      siriAdapter.prioritise!({ frozenUnitLevels: {} as never, parameters: {} })
    ).toThrow();
  });

  it('trzy presety sumuja sie do 1 i sa te z Figure 12', () => {
    expect(SIRI_PM_WEIGHT_PRESETS.strategic).toEqual({ cost: 0.3, kpi: 0.4, proximity: 0.3 });
    expect(SIRI_PM_WEIGHT_PRESETS.tactical).toEqual({ cost: 0.45, kpi: 0.3, proximity: 0.25 });
    expect(SIRI_PM_WEIGHT_PRESETS.operational).toEqual({ cost: 0.6, kpi: 0.2, proximity: 0.2 });
  ...
  it('★ v2 obcina ujemny Proximity do zera (defekt 2 naprawiony)', () => {
    ...
    const legacy = calculateImpactValue({
      areaId: 'x',
      costRelevance: 0,
      costProfile: 0,
      kpiRelevance: 0,
