...
const RechartsProvider: React.FC<{ children: (recharts: any) => React.ReactNode }> = ({ children }) => {
  return (
    <Suspense fallback={<div className="flex items-center justify-center p-8">Loading chart...</div>}>
      <RechartsLoader>{children}</RechartsLoader>
    </Suspense>
  );
};

const RechartsLoader: React.FC<{ children: (recharts: any) => React.ReactNode }> = ({ children }) => {
