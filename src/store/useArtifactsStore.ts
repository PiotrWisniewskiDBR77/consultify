...
export const parseArtifactsFromResponse = (response: string): Artifact[] => {
    ...
    while ((match = artifactPatternWithLang.exec(response)) !== null) {
        ...
        if (isValidArtifactType(type)) {
            ...
            artifacts.push(createArtifact(type as Artifact['type'], title.trim(), content.trim(), { language }));
        }
    }
    ...
    while ((match = jsonPattern.exec(response)) !== null) {
        try {
            ...
            if (artifactDef.type && artifactDef.content) {
                artifacts.push(
                    createArtifact(artifactDef.type, artifactDef.title || 'Untitled', artifactDef.content, artifactDef),
                );
            }
        } catch (e) {
            // Invalid JSON, skip
