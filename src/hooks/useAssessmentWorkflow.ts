...
export const useAssessmentWorkflow = (assessmentId: string | null): UseAssessmentWorkflowReturn => {
  ...
  const fetchVersions = useCallback(async () => {
    ...
    const versionsData = await apiCall<{ versions: AssessmentVersion[] }>(
  ...
  const fetchHistory = useCallback(async () => {
    ...
    const historyData = await apiCall<{ history: WorkflowTransition[] }>(
  ...
  const canReject =
    workflowStatus?.status === 'IN_REVIEW' || workflowStatus?.status === 'AWAITING_APPROVAL';
