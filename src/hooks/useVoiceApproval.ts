...
function parseVoiceCommand(text: string): ParsedCommand {
  ...
  const rejectWithReason = normalized.match(
    /^(?:reject|odrzuć|odrzuc)\s+(?:because|bo|ponieważ)?\s*(.+)$/i
  );
  if (rejectWithReason) {
...
function getVoiceResponse(
  ...
  return (
    responses[language]?.[commandType] ||
    responses.en[commandType] ||
    responses.en[COMMAND_TYPES.UNKNOWN]
  );
}
  ...
  const refreshPendingActions = useCallback(async () => {
    ...
    try {
      const response = (await (api as any).get?.(
        `/ai/actions/pending?projectId=${projectId || ''}`
      )) || {
        data: {},
      };
