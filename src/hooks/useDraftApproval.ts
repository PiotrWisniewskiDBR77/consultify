...
export function useDraftApproval(options: UseDraftApprovalOptions = {}): UseDraftApprovalReturn {
  ...
  const approveDraft = useCallback(
    async (draftId: string, notes?: string, modifications?: any): Promise<boolean> => {
      try {
   ...
   */
  const getDraftsForEntity = useCallback(
    async (entityType: string, entityId: string): Promise<AIDraft[]> => {
      try {
