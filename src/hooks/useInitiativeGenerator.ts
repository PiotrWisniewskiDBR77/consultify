...
interface UseInitiativeGeneratorResult {
  ...
  applyTemplate: (
    templateId: string,
    charter: Partial<AIGeneratedCharter>
  ) => Promise<AIGeneratedCharter>;
}
...
export function useInitiativeGenerator(assessmentId?: string): UseInitiativeGeneratorResult {
  ...
  const generateCharter = useCallback(
    async (request: AICharterRequest): Promise<AIGeneratedCharter> => {
  ...
  const regenerateSection = useCallback(
    async (charterId: string, section: string, context: any): Promise<any> => {
      try {
