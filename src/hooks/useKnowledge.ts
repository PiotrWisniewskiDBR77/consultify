...
async function searchArticles(query: string, lang: string, limit = 10): Promise<KbArticleListItem[]> {
    if (!query || query.length < 2) return [];
    const res = await fetch(`${API_BASE}/search?q=${encodeURIComponent(query)}&lang=${lang}&limit=${limit}`);
    if (!res.ok) throw new Error('Failed to search articles');
    const data = await res.json();
    return data.articles;
}

async function fetchContextual(moduleId: string, lang: string, token?: string): Promise<KbArticleListItem[]> {
