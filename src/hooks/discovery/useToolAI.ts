...
import { getSystemPrompt, getStepQuestion } from '@/components/DiscoveryTools/ai/toolPrompts';
...
export const useToolAI = ({ toolType }: UseToolAIOptions): UseToolAIReturn => {
  ...
  // Get the appropriate system prompt with organization context
  const buildSystemPrompt = useCallback(() => {
    const basePrompt = getSystemPrompt(toolType);
...
=== END CONTEXT ===`;
  ...
  }, [buildSystemPrompt, currentSession, currentStep, getStepDefinitions, startStream]);
...
{"attractiveness": 3, "summary": "executive summary", "insights": ["insight 1", "insight 2"], "initiatives": [{"title": "...", "description": "...", "type": "...", "rationale": "..."}]}`;
  ...
  const getStepOpeningQuestionFn = useCallback((): string => {
    const stepDefs = getStepDefinitions();
    const currentStepDef = stepDefs[currentStep - 1];

    if (!currentStepDef) return '';

    // Use imported getStepQuestion, fallback to hardcoded for backwards compatibility
    const question = getStepQuestion(toolType, currentStepDef.id, 'en');
    if (question) return question;

    // Fallback for tools not yet in toolPrompts.ts
    ...
    return fallbackQuestions[toolType]?.[currentStepDef.id] || '';
  ...
  return {
    ...
    generateSummary,
    abortStream,
    getStepOpeningQuestion: getStepOpeningQuestionFn,
  };
};
