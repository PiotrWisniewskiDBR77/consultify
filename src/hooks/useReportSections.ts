...
export const useReportSections = (reportId: string | null): UseReportSectionsReturn => {
  ...
  const updateSection = useCallback(
    async (sectionId: string, content: string, title?: string): Promise<boolean> => {
      ...
      try {
        ...
        setSections((prev) =>
          prev.map((s) =>
            s.id === sectionId ? { ...s, version: result.version, updatedAt: result.updatedAt } : s
          )
        );
    ...
    async (
      ...
      try {
        ...
        const response = await fetch(
          `/api/assessment-reports/${reportId}/sections/${sectionId}/ai`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
