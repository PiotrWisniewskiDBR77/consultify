...
export function usePartnerEcosystem(): UsePartnerEcosystemReturn {
  ...
  const submitCommissionInquiry = useCallback(
    async (type: string, message: string): Promise<void> => {
  ...
  const updateDirectoryProfile = useCallback(
    async (profile: Partial<DirectoryProfile>): Promise<void> => {
  ...
  const completeAcademyModule = useCallback(
    async (moduleId: string, score: number): Promise<void> => {
      // In production: await Api.completeAcademyModule(moduleId, score);
      setAcademyModules((prev) =>
        prev.map((m) =>
          m.id === moduleId ? { ...m, completedAt: new Date().toISOString(), score } : m
        )
      );
    },
    []
  ...
  const requestClientAccess = useCallback(
    async (clientId: string, accessLevel: string): Promise<void> => {
