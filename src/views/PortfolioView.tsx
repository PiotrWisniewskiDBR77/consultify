...
export const PortfolioView: React.FC = () => {
  ...
  const [fullInitiative, setFullInitiative] = useState<FullInitiative | null>(null);
  const [isLoadingFullInitiative, setIsLoadingFullInitiative] = useState(false);

  // Users state (for tasks and modals)
    ...
    async (showRefreshIndicator = false) => {
      ...
      try {
        ...
        if (filters.priority?.length)
          filters.priority.forEach((p: any) => params.append('priority', p));
  ...
  // Handler for opening full initiative detail modal
  const handleOpenFullDetail = useCallback(async (initiative: PortfolioInitiative) => {
    setIsLoadingFullInitiative(true);
    try {
      // Fetch full initiative data with all details
      const fullData = await Api.get(`/initiatives/${initiative.id}`);
      setFullInitiative(fullData);
      setIsSidePanelOpen(false);
      setIsFullModalOpen(true);
    } catch (error: any) {
      console.error('[PortfolioView] Failed to fetch full initiative:', error);
      toast.error('Failed to load initiative details');
    } finally {
      setIsLoadingFullInitiative(false);
    }
  }, []);

  // Handler for saving initiative from full modal
  const handleSaveInitiative = useCallback(
    async (updatedInitiative: FullInitiative) => {
      try {
        await Api.put(`/initiatives/${updatedInitiative.id}`, updatedInitiative);
        // Update in the list
        setInitiatives((prev: any) =>
          prev.map((i: any) => (i.id === updatedInitiative.id ? { ...i, ...updatedInitiative } : i))
        );
        ...
        console.error('[PortfolioView] Failed to save initiative:', error);
        toast.error(error.response?.data?.error || 'Failed to save initiative');
      }
    },
    [fetchData]
  );
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        console.error('[PortfolioView] Failed to fetch users:', error);
      }
    };
    fetchUsers();
  }, []);
  ...
  const handleStatusChange = useCallback(
    async (initiativeId: string, newStatus: InitiativeStatus) => {
      try {
        ...
        setInitiatives((prev: any) =>
          prev.map((i: any) => (i.id === initiativeId ? { ...i, status: newStatus } : i))
        );
  ...
  const handleQuickUpdate = useCallback(
    async (initiativeId: string, updates: Partial<PortfolioInitiative>) => {
      try {
        ...
        setInitiatives((prev: any) =>
          prev.map((i: any) => (i.id === initiativeId ? { ...i, ...updates } : i))
        );
      } catch (error: any) {
      ...
      <div className="flex flex-wrap items-center gap-3 p-4 bg-slate-50 dark:bg-navy-900/50 rounded-lg">
        ...
        {projects.length > 0 && (
          <select
            ...
            className="px-3 py-1.5 text-sm bg-white dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-lg text-navy-900 dark:text-white"
          >
        ...
        <select
          ...
          onChange={(e) =>
            handleFilterChange('priority', e.target.value ? [e.target.value] : undefined)
          }
          className="px-3 py-1.5 text-sm bg-white dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-lg text-navy-900 dark:text-white"
        >
      ...
      return (
        <div className="flex flex-col items-center justify-center h-96 text-center">
          ...
          <h3 className="text-xl font-semibold text-navy-900 dark:text-white mb-2">
            {t('portfolio.emptyState.title', 'No initiatives yet')}
          </h3>
    ...
    <SplitLayout title="Portfolio & Roadmap" currentView={AppView.PORTFOLIO}>
      <div className="h-full flex flex-col bg-slate-50 dark:bg-navy-950 overflow-hidden">
        ...
        <div className="shrink-0 px-6 py-4 bg-white dark:bg-navy-900 border-b border-slate-200 dark:border-navy-700">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="p-2 text-slate-400 hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg transition-colors">
                <Download size={18} />
              </button>
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-64 pl-9 pr-4 py-1.5 text-sm rounded-lg border border-slate-200 dark:border-navy-700 bg-white dark:bg-navy-950 text-navy-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
              </div>
        ...
        <div className="flex-1 overflow-hidden px-6 pb-6">
          <div className="h-full bg-white dark:bg-navy-900 rounded-xl border border-slate-200 dark:border-navy-700 overflow-hidden">
            {renderContent()}
          </div>
        </div>
        ...
        <InitiativeSidePanel
          ...
          onUpdate={(updated) => {
            setInitiatives((prev: any) =>
              prev.map((i: any) => (i.id === updated.id ? updated : i))
            );
          }}
