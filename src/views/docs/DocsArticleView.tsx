    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <nav className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-6">
                ...
                <Link to={`/docs/${categorySlug}`} className="hover:text-purple-600 dark:hover:text-purple-400">
                ...
                <article className="flex-1 min-w-0">
                    ...
                    <div className="prose prose-slate dark:prose-invert prose-headings:scroll-mt-20 prose-a:text-purple-600 dark:prose-a:text-purple-400 prose-code:before:content-none prose-code:after:content-none max-w-none">
                        <ReactMarkdown
                            ...
                            components={{
                                h1: ({ children }) => {
                                    const id = (children as string)?.toString().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
                                ...
                                code({ node, className, children, ...props }) {
                                    ...
                                    const inline = !match;
                                    return inline ? (
                                        <code className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-navy-800 text-sm" {...props}>
                                            {children}
                                        </code>
                                    ) : (
                                        <SyntaxHighlighter
