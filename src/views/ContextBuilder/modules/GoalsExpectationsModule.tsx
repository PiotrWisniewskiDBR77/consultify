...
export const GoalsExpectationsModule: React.FC = () => {
    ...
    // Handlers
    const createHandler = (listName: 'kpis' | 'inScope' | 'outScope' | 'noGo', currentItems: DynamicListItem[]) => ({
        ...
        <div className="space-y-6">
            ...
            <div className="flex border-b border-slate-200 dark:border-white/10 space-x-6 overflow-x-auto">
                {tabs.map((tab) => (
                    <button
                        key={tab.id}
                        onClick={() => setActiveTab(tab.id as 'intent' | 'metrics' | 'scope' | 'nogo' | 'expectations')}
                        className={`
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {[
                                    ...
                                    <label
                                        key={p.id}
                                        className="relative flex items-center gap-3 border border-slate-200 dark:border-white/10 p-4 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5 transition-all group"
                                    >
                                        <input
                                            type="checkbox"
