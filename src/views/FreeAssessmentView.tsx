            ...
            {[1, 2, 3].map((step) => {
                ...
                return (
                    <div key={step} className="flex items-center">
                        <div
                            className={`flex items-center justify-center w-6 h-6 rounded-full border transition-all ${isActive ? 'bg-purple-600 border-purple-500 text-white' : isCompleted ? 'bg-green-500/20 border-green-500/30 text-green-400' : 'bg-navy-900 border-white/10 text-slate-500'}`}
                        >
                            {isCompleted ? <Check size={12} /> : <span className="text-[10px] font-bold">{step}</span>}
                        </div>
                        <span
                            className={`ml-2 text-[10px] font-medium hidden lg:block ${isActive ? 'text-white' : 'text-slate-500'}`}
                        >
                            {step === 1 ? 'Profile & IT' : step === 2 ? 'Goals' : 'Challenges'}
                        </span>
                        {step < 3 && (
                            <div className={`w-8 h-px mx-2 ${isCompleted ? 'bg-green-500/30' : 'bg-white/10'}`}></div>
                        )}
                    </div>
                );
            })}
...
export const FreeAssessmentView: React.FC = () => {
    ...
    }, [currentAppView, sessionData.step2Completed, sessionData.step3Completed, currentStep, messages]);
