      ...
      {[1, 2, 3].map((step) => {
        ...
        return (
          <div key={step} className="flex items-center">
            <div
              className={`flex items-center justify-center w-6 h-6 rounded-full border transition-all ${isActive ? 'bg-purple-600 border-purple-500 text-white' : isCompleted ? 'bg-green-500/20 border-green-500/30 text-green-400' : 'bg-navy-900 border-white/10 text-slate-500 dark:text-slate-400'}`}
            >
              {isCompleted ? (
                <Check size={12} />
              ) : (
            ...
            <span
              className={`ml-2 text-[10px] font-medium hidden lg:block ${isActive ? 'text-white' : 'text-slate-500 dark:text-slate-400'}`}
            >
              {step === 1 ? 'Profile & IT' : step === 2 ? 'Goals' : 'Challenges'}
            </span>
            {step < 3 && (
              <div
                className={`w-8 h-px mx-2 ${isCompleted ? 'bg-green-500/30' : 'bg-white/10'}`}
              ></div>
            )}
          </div>
        );
...
export const FreeAssessmentView: React.FC = () => {
  ...
  const chat = translate('chat.freeAssessment', { returnObjects: true }) as {
  ...
  useEffect(() => {
    if (
      ...
      messages.length === 0 &&
      currentAppView === AppView.QUICK_STEP1_PROFILE &&
      currentUser
    ) {
  ...
  const handleOptionSelect = (option: ChatOption, isMultiSelect?: boolean) => {
    ...
    if (isStep1(currentStep)) {
      ...
      currentStep === AssessmentStep.STRATEGIC_GOALS ||
      currentStep === AssessmentStep.SUCCESS_CRITERIA
    ) {
      // Step 2 Logic
      processStep2Logic(option.value, option.label);
    } else if (
      currentStep === AssessmentStep.CHALLENGES_MAP ||
      currentStep === AssessmentStep.CONSTRAINTS
    ) {
      // Step 3 Logic
  ...
  const isStep1 = (step: AssessmentStep) => {
    const step1Steps = [
      ...
      AssessmentStep.INDUSTRY,
      AssessmentStep.SIZE,
      AssessmentStep.COUNTRY,
      AssessmentStep.BUSINESS_MODEL,
      AssessmentStep.CORE_PROCESSES,
      AssessmentStep.IT_LANDSCAPE,
    ...
    switch (currentStep) {
      ...
      case AssessmentStep.BUSINESS_MODEL:
        setProfileData((prev: any) => ({
          ...prev,
          businessModel: { type: [value], description: '' },
        }));
      ...
      case AssessmentStep.IT_LANDSCAPE:
        setProfileData((prev: any) => ({
          ...prev,
          itLandscape: { ...prev.itLandscape, erp: value },
        }));
  ...
  const startStep2Flow = () => {
    ...
    addAiMessage(
      "Let's define your Strategic Goals. What is the #1 priority for this transformation?",
      [
  ...
  const processStep3Logic = (value: string) => {
    if (currentStep === AssessmentStep.CHALLENGES_MAP) {
      ...
      if (value !== 'None') {
        ...
        setSessionData({
          ...sessionData,
          constraints: [...(sessionData.constraints || []), newConstraint],
          step3Completed: true,
        });
      } else {
      ...
      // Done with Module 1
      addAiMessage(
        'Module 1 Complete! We have your profile, goals, and challenges. Ready to create your account and start the full Assessment?',
