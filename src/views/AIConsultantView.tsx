...
export const AIConsultantView: React.FC<AIConsultantViewProps> = ({ session }) => {
  ...
  const suggestedPrompts = [
    {
      ...
      prompt:
        'Analyze the potential impact of our initiatives on key operational KPIs based on the roadmap.',
    },
    {
      icon: <DollarSign size={16} />,
      text: 'Explain Financial ROI',
      prompt:
        'Break down the ROI calculation. How does the $120k cost translate to the projected benefits?',
    },
    {
      icon: <Activity size={16} />,
      text: 'Operational Recs',
      prompt:
        "What operational changes should we prioritize to unblock the 'Data Foundation' initiative?",
    },
  ];
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-navy-900 rounded-xl overflow-hidden border border-slate-200 dark:border-navy-700 shadow-sm relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Your Digital Executive Partner
            </p>
          </div>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="text-center py-12 opacity-60">
            ...
            <p className="text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
              I can analyze your transformation data, explain financial projections, or provide
      ...
      {/* Input Area */}
      <div className="p-4 bg-white dark:bg-navy-900 border-t border-slate-200 dark:border-navy-700 space-y-4">
        {/* Suggested Prompts */}
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {suggestedPrompts.map((p, i) => (
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-navy-800/40 hover:bg-indigo-50 dark:bg-navy-800 dark:hover:bg-indigo-900/30 border border-slate-200 dark:border-navy-700 rounded-full text-xs font-medium text-slate-600 dark:text-slate-300 transition-colors whitespace-nowrap"
            >
              {p.icon}
              {p.text}
            </button>
        ...
        <div className="relative">
          <input
            ...
            className="w-full pl-4 pr-12 py-3 bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all dark:text-white"
          />
          <button
