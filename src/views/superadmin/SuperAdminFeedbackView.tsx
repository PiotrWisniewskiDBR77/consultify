      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 dark:text-slate-500 mt-1">
            Manage incoming reports and ideas from users.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredFeedback.length === 0 ? (
            ...
            filteredFeedback.map((item) => (
              <div
                key={item.id}
                className="bg-navy-800 border border-slate-700 rounded-xl p-4 hover:border-slate-600 transition-colors"
              >
                    ...
                    <div
                      className={`px-3 py-1.5 rounded-lg text-xs font-bold text-center border ${
                        item.status === 'NEW'
                          ...
                          : item.status === 'RESOLVED'
                            ? 'bg-slate-700/50 text-slate-400 dark:text-slate-500 border-slate-700'
                            : 'bg-slate-700/50 text-slate-400 dark:text-slate-500 border-slate-700'
                      }`}
                    >
                      {item.status}
