...
export const PolicyEnforcementTab: React.FC = () => {
  ...
  const enforcementColumns: TableColumn[] = useMemo(
    () => [
      {
        id: 'domain',
        label: 'Domain',
        render: (row: TableRow) => (
          <span className="font-medium text-slate-900 dark:text-white">{row.domain}</span>
        ),
      },
      {
        id: 'desiredState',
        label: 'Desired',
        render: (row: TableRow) => (
          <span className="text-slate-600 dark:text-slate-300">{row.desiredState}</span>
        ),
      },
      {
        id: 'appliedState',
        label: 'Applied',
        render: (row: TableRow) => (
          <span className="text-slate-600 dark:text-slate-300">{row.appliedState}</span>
        ),
      },
      {
        id: 'drift',
      ...
      },
      {
        ...
        label: 'Severity',
        render: (row: TableRow) => {
          const severity = getDriftSeverity(row as EnforcementRow);
          return (
            <span
      ...
      },
      {
        ...
        label: 'Repair path',
        render: (row: TableRow) => {
          const target = getRepairTarget(row as EnforcementRow);
          return (
            <a
              href={target.href}
  ...
  const enforcementRows: TableRow[] = useMemo(() => rows.map((row) => ({ ...row })), [rows]);
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-navy-700 dark:bg-navy-900">
          ...
          <div className="mt-4">
            <StandardTable
              ...
              empty={{ title: 'No enforcement data available.' }}
              persistKey="superadmin.aiPlatform.policyEnforcement"
              canvasClassName="p-0"
            />
          </div>
        </div>
