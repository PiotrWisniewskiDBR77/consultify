...
export const LLMObservatoryTab: React.FC = () => {
  ...
  const loadData = async (nextRange: TimeRange = range) => {
    try {
      setRefreshing(true);
      const response = await Api.getAIOperationsLLMObservatory(nextRange);
      const data = (response?.data || response) as ObservatoryPayload;
      setPayload(data);
      ...
      console.error('[LLMObservatoryTab] Failed to load observatory data:', error);
      toast.error(
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-4">
        ...
        <div className={`${cardClass} p-4`}>
          ...
          <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            {t('superadmin.ai.observatory.providers', 'Providers')}: {summary?.providersUsed ?? 0}
          </div>
        </div>
        ...
        <div className={`${cardClass} p-5`}>
          ...
          <div className="mt-4 space-y-3 max-h-[420px] overflow-y-auto">
            {filteredIncidents.length === 0 ? (
              <div className="text-sm text-slate-500 dark:text-slate-400 py-8 text-center">
                {t('superadmin.ai.observatory.noIncidents', 'No incidents recorded in this period')}
              </div>
            ) : (
