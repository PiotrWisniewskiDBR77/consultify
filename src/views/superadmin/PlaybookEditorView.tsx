...
export const PlaybookEditorView: React.FC<PlaybookEditorViewProps> = ({ templateId, onBack }) => {
  ...
  const handleAddNode = (type: PlaybookNodeType) => {
    ...
    const newNode: PlaybookNode = {
      ...
      type,
      title:
        type === PlaybookNodeType.BRANCH
          ? 'Branch'
          : type === PlaybookNodeType.CHECK
            ? 'Check'
            : 'New Action',
      data: {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100 dark:bg-navy-800">
      {/* Header */}
      <div className="bg-white dark:bg-navy-900 border-b border-gray-200 px-4 py-3 flex items-center gap-4">
        <button
          onClick={onBack}
          className="p-1.5 text-gray-500 hover:text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:bg-navy-800 rounded"
        >
          <ArrowLeft size={20} />
        </button>
