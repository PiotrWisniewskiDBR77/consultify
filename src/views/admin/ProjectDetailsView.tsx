    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-wrap justify-between items-start gap-4 bg-gradient-to-r from-primary-600 to-secondary-700 dark:bg-navy-900/40 dark:from-transparent dark:to-transparent backdrop-blur-md p-8 rounded-xl border border-primary-500/20 dark:border-navy-700 shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex items-center gap-3 mt-4 sm:mt-0 z-10">
          ...
          <button
            onClick={onBack}
            className="flex items-center gap-2 px-4 py-2 bg-navy-950 border border-white/5 text-slate-400 dark:text-slate-500 hover:text-white rounded-xl transition-all text-sm font-semibold"
          >
      ...
      <div className="mt-8 min-h-[500px]">
            ...
            <div className="lg:col-span-2 space-y-6">
              {isEditing ? (
                <form
                  ...
                  className="bg-navy-900 border border-white/10 rounded-xl p-8 space-y-6 shadow-xl"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {project.assessments?.length === 0 ? (
                ...
                project.assessments?.map((item: any) => (
                  <div
                    key={item.id}
                    className="bg-navy-900 border border-white/10 p-6 rounded-xl hover:bg-navy-800 transition-all group"
                  >
        ...
        {activeTab === 'documents' && (
          <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
            {/* Upload Panel */}
            <div className="bg-navy-900 border border-white/10 p-8 rounded-xl shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {project.documents?.length === 0 ? (
                ...
                project.documents?.map((item: any) => (
                  <div
                    key={item.id}
                    className="bg-navy-900 border border-white/10 p-4 rounded-xl hover:border-blue-500/30 transition-all flex justify-between items-center group"
                  >
                    <div className="flex items-center gap-3">
