...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ users, projects }) => {
  ...
  const { currentOrganization, setCurrentView } = useAppStore();
  ...
  // Calculate growth based on activity ratio (percentage of active vs total)
  // Positive if more than 80% active, negative if less
  const userGrowth = users.length > 0 ? Math.round((activeUsers / users.length) * 100) - 80 : 0;
  const projectGrowth =
    projects.length > 0 ? Math.round((activeProjects / projects.length) * 100) - 80 : 0;
  ...
  const loadUpcomingEvents = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin-data/scheduled-events/${currentOrganization.id}?limit=5`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        }
      );
  ...
  const quickActions = [
    ...
    {
      icon: Settings,
      label: t('admin.dashboard.settings', 'Settings'),
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="admin-metric">
          <div className="flex items-center gap-2">
            <Send size={14} className="text-slate-500 dark:text-slate-400" />
            <span className="admin-metric-label">
        ...
        {/* Revenue - Links to Billing */}
        <div
          className="admin-metric cursor-pointer"
          onClick={() => navigate('/admin/billing')}
          title={t('admin.dashboard.viewBilling', 'View Billing Details')}
        >
          <div className="flex items-center gap-2">
            <DollarSign size={14} className="text-slate-500 dark:text-slate-400" />
            <span className="admin-metric-label">{t('admin.dashboard.billing', 'Billing')}</span>
          </div>
          <p className="admin-metric-value text-blue-500 dark:text-blue-400">
            {t('admin.dashboard.viewDetails', 'View Details')}
          </p>
          <p className="admin-metric-subtitle">
            {t('admin.dashboard.manageSubscription', 'manage subscription')}
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {quickActions.map((action, idx) => (
            <button
              ...
              className="flex items-center gap-3 p-3 bg-slate-50 hover:bg-slate-100 dark:bg-white/[0.02] dark:hover:bg-white/[0.05] border border-slate-200 dark:border-navy-700 rounded-lg transition-colors text-slate-600 hover:text-navy-900 dark:text-slate-400 dark:hover:text-white"
            >
              <action.icon size={16} />
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="p-3 bg-slate-50 dark:bg-white/[0.02] rounded-lg">
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t('admin.dashboard.uptime', 'Uptime')}
              </p>
            ...
            <div className="p-3 bg-slate-50 dark:bg-white/[0.02] rounded-lg">
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t('admin.dashboard.lastCheck', 'Last Check')}
              </p>
              <p className="text-lg font-medium text-navy-900 dark:text-white mt-0.5">
...
const ScheduleEventModal: React.FC<ScheduleEventModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-navy-700 flex justify-between items-center">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-slate-900 dark:text-white"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-navy-700 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-slate-900 dark:text-white resize-none"
            />
          </div>
