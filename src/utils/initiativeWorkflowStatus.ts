...
export function getWorkflowStatusForInitiative(
  initiative:
    | { status?: string | null; displayStatus?: string | null }
    | null
    | undefined
): string {
...
export function hasInitiativeStatusReadDrift(
  ...
  const raw = String(initiative?.status ?? '').trim().toUpperCase();
  const disp = String(initiative?.displayStatus ?? '').trim().toUpperCase();
