...
export function adaptIdeaActions(): FederatedActionEntry[] {
  return IDEA_ACTION_REGISTRY.map((action) => ({
    ...
    effect: action.destructive
      ? 'DESTRUCTIVE_MUTATION'
      : action.mutates
        ? 'REVERSIBLE_MUTATION'
        : 'READ',
    roles: [action.permission?.requiredRole ?? 'AUTHENTICATED'],
...
export function adaptChatActions(): FederatedActionEntry[] {
  return CHAT_ACTION_DEFINITIONS.map((action) => {
    ...
    return {
      actionId: `chat.${action.type.toLowerCase()}@1`,
      version: 1,
      module: 'CHAT',
      surface: 'chat-action-registry',
      mountedMutationId: mutates ? `chat:${action.type}` : null,
      effect: mutates ? 'PROPOSAL' : 'READ',
      roles: ['CAPABILITY_CHECKED'],
      tenantScope: 'ORGANIZATION',
      preview: mutates ? 'REQUIRED' : 'NOT_REQUIRED',
      confirm: mutates ? 'REQUIRED' : 'NOT_REQUIRED',
      idempotency: mutates ? 'proposal-id' : null,
      receipt: mutates ? 'proposal-receipt' : null,
      auditEvent: mutates ? 'chat.action.proposed' : null,
      compensation: mutates ? 'reject-proposal-before-write' : 'read-only',
      uiExecutor: `chatActionRegistry:${action.type}`,
      teresaExecutor: `chatActionRegistry:${action.type}`,
      mvpDisposition: 'SUPPORTED',
    };
  });
}
