...
import { CreditCard, Download, Calendar, TrendingUp, DollarSign, Users, RefreshCw, FileText, Mail, User as UserIcon, Building2, MapPin, Check, X, Zap, ArrowRight, Package, Sparkles, ChevronRight, Crown, AlertCircle } from 'lucide-react';
import { Api } from '../../services/api';
import { Invoice, OrganizationOwnership, BillingAddress, User } from '../../types';
          ...
          <div className="space-y-2">
            {loadingInvoices ? (
              ...
              invoices.map((invoice: Invoice, i: number) => (
                <div key={invoice.id || i} className="flex items-center justify-between p-3 bg-slate-50 hover:bg-slate-100 dark:bg-white/[0.02] dark:hover:bg-white/[0.04] rounded-lg transition-colors">
                  <div>
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`admin-status ${invoice.status === 'Paid' || invoice.status === 'paid'
      ...
      <div className="admin-card p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Billing Address</p>
              <p className="text-sm font-medium text-navy-900 dark:text-white">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
                ...
                <button onClick={() => setShowPlanModal(false)} className="text-slate-400 hover:text-navy-900 dark:hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
                ...
                {planComparison && selectedPlanId && selectedPlanId !== billing?.plan && (
                  <div className="mt-6 p-4 bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-xl">
                    <h4 className="text-sm font-medium text-navy-900 dark:text-white mb-3">
                      {planComparison.comparison.isUpgrade ? 'Upgrade Summary' : 'Downgrade Summary'}
                    </h4>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
                ...
                <button onClick={() => { setShowAddonsModal(false); setSelectedAddon(null); }} className="text-slate-400 hover:text-navy-900 dark:hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
                    ...
                    {addons.map((addon) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <p className="font-semibold text-navy-900 dark:text-white">${addon.price}</p>
                          <p className="text-xs text-slate-500">{addon.recurring ? '/month' : 'one-time'}</p>
                        </div>
                        <ChevronRight size={16} className="text-slate-400 group-hover:text-[var(--admin-accent)] transition-colors" />
                      </div>
                    ))}
                    {addons.length === 0 && (
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-white/[0.02] rounded-xl border border-slate-200 dark:border-white/10">
                      ...
                      <div className="mt-4 flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="w-12 text-center font-medium text-white">{addonQuantity}</span>
                          <button
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-200 dark:border-white/10 flex justify-between items-center">
                        ...
                        <span className="text-xl font-bold text-navy-900 dark:text-white">
                          ...
                          {selectedAddon.recurring && <span className="text-sm font-normal text-slate-500">/month</span>}
                        </span>
                      </div>
                    </div>
                    ...
                    {selectedAddon.type === 'tokens' && (
                      <div className="flex items-start gap-3 p-3 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-500/20 rounded-lg">
                        ...
                        <p className="text-sm text-amber-700 dark:text-amber-300">
                          Token packages are added to your monthly limit immediately and do not roll over.
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm shadow-2xl">
            ...
            >
              <div className="p-6 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
                ...
                <button onClick={() => setShowBillingModal(false)} className="text-slate-400 hover:text-navy-900 dark:hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6 overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-slate-400 mb-1.5">Billing Name</label>
                    <input
                      type="text"
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-slate-400 mb-1.5">Billing Email</label>
                    <input
                      type="email"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-400 mb-1.5">Tax ID</label>
                    <input
                      type="text"
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-white/5">
                  <h4 className="text-sm font-medium text-navy-900 dark:text-white mb-4">Billing Address</h4>
                  <div className="space-y-4">
                    <input
                      type="text"
