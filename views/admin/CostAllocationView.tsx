...
import {
  Building2, DollarSign, PieChart, Plus, Edit2, Trash2, RefreshCw,
  Check, X, Download, TrendingUp, AlertCircle, Users, Briefcase
} from 'lucide-react';
...
const COLORS = ['#64748b', '#94a3b8', '#475569', '#334155', '#1e293b', '#0f172a', '#3b82f6', '#10b981'];
  ...
  const loadCostCenters = async () => {
    ...
    setTimeout(() => {
      ...
      setAllocations([
        ...
        { id: 'a-3', cost_center_id: 'cc-1', cost_center_name: 'Engineering', amount: 137.50, resource_type: 'AI Tokens', date: '2026-01-01' },
        { id: 'a-4', cost_center_id: 'cc-4', cost_center_name: 'DBR77 Project', amount: 256.75, resource_type: 'AI Tokens', date: '2026-01-01' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="admin-card p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-slate-500 uppercase tracking-wider">Current Spend</span>
          </div>
          <p className="text-2xl font-semibold text-white">${totalSpend.toFixed(2)}</p>
          <p className="text-xs text-slate-500">{((totalSpend / totalBudget) * 100).toFixed(1)}% of budget</p>
        </div>

        <div className="admin-card p-4">
          <div className="flex items-center gap-2 mb-2">
            <Building2 size={14} className="text-slate-500" />
            <span className="text-xs text-slate-500 uppercase tracking-wider">Cost Centers</span>
          </div>
          <p className="text-2xl font-semibold text-white">{costCenters.length}</p>
          <p className="text-xs text-slate-500">{costCenters.filter(cc => cc.is_active).length} active</p>
        </div>

        <div className="admin-card p-4">
          <div className="flex items-center gap-2 mb-2">
            <AlertCircle size={14} className="text-slate-500" />
            <span className="text-xs text-slate-500 uppercase tracking-wider">Over Budget</span>
          </div>
          <p className="text-2xl font-semibold text-white">
            {costCenters.filter(cc => cc.percentage_used >= 90).length}
          </p>
          <p className="text-xs text-slate-500">cost centers</p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.05]">
                ...
                <th className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider py-3 px-4">Code</th>
                <th className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider py-3 px-4">Type</th>
                <th className="text-right text-xs font-medium text-slate-500 uppercase tracking-wider py-3 px-4">Budget</th>
                <th className="text-right text-xs font-medium text-slate-500 uppercase tracking-wider py-3 px-4">Spent</th>
                <th className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider py-3 px-4 w-40">Usage</th>
                ...
                <tr key={center.id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      {center.type === 'department' && <Users size={14} className="text-slate-500" />}
                      {center.type === 'project' && <Briefcase size={14} className="text-slate-500" />}
                      <span className="text-sm text-white">{center.name}</span>
                    </div>
                  </td>
                  <td className="py-3 px-4">
                    <span className="text-sm text-slate-400 font-mono">{center.code}</span>
                  </td>
                  <td className="py-3 px-4">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{allocation.resource_type} • {allocation.date}</p>
                </div>
              </div>
              <span className="text-sm font-medium text-white">${allocation.amount.toFixed(2)}</span>
            </div>
          ))}
        </div>
      </div>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, budget_monthly: parseFloat(e.target.value) || 0 })}
                    className="admin-input w-full"
                    placeholder="500"
                    min="0"
                    step="10"
