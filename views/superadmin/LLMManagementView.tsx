 ...
 * LLM Management View
 * 
 * SuperAdmin interface for managing LLM providers, model routing,
...
export const LLMManagementView: React.FC = () => {
    ...
    const tabs = [
        { id: 'providers' as LLMConfigTab, label: 'LLM Providers', icon: Cpu },
        { id: 'routing' as LLMConfigTab, label: 'Model Routing', icon: Settings },
        { id: 'usage' as LLMConfigTab, label: 'Usage & Costs', icon: BarChart3 },
        { id: 'health' as LLMConfigTab, label: 'System Health', icon: Activity },
    ];
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-400">Configure AI providers, routing, and monitor usage</p>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div className="space-y-4">
                                    {[
                                        ...
                                        <div key={item.tier} className="flex items-center gap-4 p-4 bg-navy-950/50 rounded-lg border border-white/5">
                                            ...
                                            <select className="bg-navy-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-cyan-500 outline-none">
                ...
                {activeTab === 'usage' && (
                    <div className="p-8 overflow-y-auto h-full">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                            <StatCard icon={Zap} label="Tokens Today" value={usageStats?.user?.tokens_used_today?.toLocaleString() || '0'} color="text-yellow-400" />
                            <StatCard icon={DollarSign} label="Cost (30 days)" value={`$${(costStats?.totals?.costUsd || 0).toFixed(4)}`} color="text-emerald-400" />
                            <StatCard icon={Activity} label="Requests (30 days)" value={costStats?.totals?.requests?.toLocaleString() || '0'} color="text-blue-400" />
                            <StatCard icon={TrendingUp} label="Cache Hit Rate" value={`${usageStats?.cache?.hitRate || 0}%`} color="text-purple-400" />
                        </div>
                        ...
                        <form onSubmit={handleProviderSubmit} className="space-y-4">
                            ...
                            <div className="pt-4 flex gap-3">
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 bg-cyan-600/20 hover:bg-cyan-600/40 text-cyan-300 border border-cyan-500/50 rounded flex items-center gap-2"
                                >
                                    <Wifi size={16} /> Test
                                </button>
                                <button type="submit" className="flex-1 py-2 bg-cyan-600 hover:bg-cyan-500 text-white font-medium rounded">Save Provider</button>
                            </div>
                        </form>
                    </div>
                </div>
