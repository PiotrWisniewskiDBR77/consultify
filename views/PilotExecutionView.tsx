import { Filter, Kanban, List as ListIcon, Plus, ShieldCheck, Sparkles } from 'lucide-react';
...
export const PilotExecutionView: React.FC = () => {
  ...
  const renderKanbanColumn = (status: TaskStatus | string, label: string) => {
    const columnTasks = tasks.filter(
      (t) => t.status === status || String(t.status).toLowerCase() === String(status).toLowerCase()
    );
    return (
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      const message =
        `**Validation Report for: ${selectedInitiative.name}**\n\n` +
    ...
    <SplitLayout title="Pilot Execution Board" onSendMessage={handleAiChat}>
      <div className="w-full h-full bg-navy-900 flex flex-col overflow-hidden">
        ...
        <div className="h-16 border-b border-white/5 flex items-center px-6 gap-4 bg-navy-800 shrink-0 overflow-x-auto">
          <span className="text-xs uppercase font-bold text-slate-500 shrink-0">
            Active Programs:
          </span>
          ...
          {activeInitiatives.length === 0 && (
            <span className="text-xs text-slate-600 italic">
              No pilots active. Go to Step 3 to launch one.
            </span>
          )}
        </div>
