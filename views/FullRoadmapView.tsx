...
import { AppView, FullInitiative, InitiativeStatus, Quarter, SessionMode, User, Wave } from '../types';
...
export const FullRoadmapView: React.FC = () => {
    ...
    const generateRoadmap = useCallback(async () => {
        ...
        try {
            ...
            await Api.saveSession(
                currentUser!.id,
                SessionMode.FULL,
                { ...fullSession, initiatives: scheduledInitiatives },
    ...
    }, [fullSession.initiatives.length, generateRoadmap]);
        ...
        <SplitLayout title="Module 3: Strategic Roadmap">
            <div className="w-full h-full relative">
                <div className="w-full h-full bg-slate-50 dark:bg-navy-950 flex flex-col p-6 overflow-y-auto gap-6 relative">
                    ...
                    <RoadmapSummary summary={summary} isLoading={isSummaryLoading} />
                    ...
                    <div className="absolute inset-0 z-50 bg-slate-500/50 dark:bg-navy-950/90 backdrop-blur-sm flex items-center justify-center p-8">
                        <div className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-white/10 rounded-2xl max-w-4xl w-full p-8 shadow-2xl transition-colors">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 mb-8">
                                ...
                                rollout. Select <strong>one</strong> high-impact, low-complexity initiative to validate
                                the strategy.
                            </p>
                                    ...
                                    .map((init) => (
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                {init.complexity === 'Low' && (
                                                    <span className="text-[10px] bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-400 px-2 py-0.5 rounded">
                                                        Low Risk
                                                    </span>
                                                )}
                                            </div>
                                            <h4 className="text-lg font-bold text-navy-900 dark:text-white mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-300">
                                                {init.name}
                                            </h4>
                                            <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2">
                                                {init.description || init.summary}
                                            </p>
                                        </div>
                                    ))}
                            </div>
                            ...
                            <div className="flex justify-end gap-4">
                                <button
                                    onClick={() => {
                                        setShowPilotDecision(false); /* Skip Logic? */
                                    }}
