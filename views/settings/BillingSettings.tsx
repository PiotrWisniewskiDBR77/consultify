import { Cpu, CreditCard, Globe } from 'lucide-react';
...
export const BillingSettings: React.FC<BillingSettingsProps> = ({ currentUser }) => {
  ...
  const tokenPercentage =
    usage.tokenLimit > 0 ? Math.round((usage.tokensUsed / usage.tokenLimit) * 100) : 0;
  const storagePercentage =
    usage.storageLimit > 0 ? Math.round((usage.storageUsed / usage.storageLimit) * 100) : 0;
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">
                  ...
                  {(usage.tokenLimit || 0).toLocaleString()}
                </span>
                <span
                  className={`font-medium ${tokenPercentage >= 80 ? 'text-orange-400' : 'text-slate-300'}`}
                >
                  {tokenPercentage}%
                </span>
              </div>
          ...
          <div className="bg-navy-900 border border-white/10 rounded-xl p-5">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span
                  className={`font-medium ${storagePercentage >= 80 ? 'text-orange-400' : 'text-slate-300'}`}
                >
                  {storagePercentage}%
                </span>
              </div>
