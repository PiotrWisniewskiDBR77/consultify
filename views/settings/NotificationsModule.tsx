                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">
                            {t('settings.schedule.quietHoursDesc', 'Pause non-urgent notifications during set hours')}
                        </p>
                    </div>
                    <button
            ...
            <div className="p-4 bg-white dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10">
                <div className="flex items-center justify-between">
                    ...
                    <button className={`w-12 h-6 rounded-full transition-colors bg-purple-600`}>
                        <div className={`w-5 h-5 bg-white rounded-full transform transition-transform translate-x-6`} />
                    </button>
                </div>
            </div>
        </div>
...
export const NotificationsModule: React.FC<NotificationsModuleProps> = ({ initialTab, currentUser, onUpdateUser }) => {
    ...
    return (
        <TabLayout
            ...
            subtitle={t('settings.modules.notificationsDesc', 'Manage how and when you receive notifications')}
        >
            {renderContent()}
        </TabLayout>
    );
