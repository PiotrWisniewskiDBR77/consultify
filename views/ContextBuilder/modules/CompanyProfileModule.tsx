...
export const CompanyProfileModule: React.FC = () => {
    ...
    // MOCK DATA STATES
    const [processes, setProcesses] = useState<DynamicListItem[]>([
        { id: '1', name: 'Order to Cash', criticality: 'High', notes: 'Manual entry bottlenecks' },
        { id: '2', name: 'Procurement', criticality: 'Medium', notes: 'Partial SAP integration' }
    ]);

    const [stakeholders, setStakeholders] = useState<DynamicListItem[]>([
        { id: '1', role: 'COO', influence: 'High', owner: 'John Doe' },
        { id: '2', role: 'Plant Manager', influence: 'High', owner: 'Jane Smith' }
    ]);

    const [initiatives, setInitiatives] = useState<DynamicListItem[]>([
        { id: '1', name: 'ERP Upgrade 2023', area: 'IT', result: 'Delayed', why: 'Budget cuts' }
    ]);

    // AI Suggestions State for In-Context Display
    const [showContextBanner, setShowContextBanner] = useState(true);

    // AI Suggestions moved to Notifications
    // We mock triggering it once on mount
    const { addNotification, currentUser } = useAppStore();

    React.useEffect(() => {
        // Mock checking if we should show an insight (e.g. only once)
        // In a real app this would come from the backend or a "Seen" check
        const insightId = 'insight-industry-update-2024';

        // Simple mock: timeout to simulate analysis finishing
        const timer = setTimeout(() => {
            addNotification({
                ...
                data: {
                    ...
                    link: '/context/profile' // Or handled via action
                }
            });
    ...
    const processHandlers = createHandler(setProcesses);
    const stakeholderHandlers = createHandler(setStakeholders);
    const initiativeHandlers = createHandler(setInitiatives);
    ...
    const SelectionGroup = ({ label, options, name, icon: Icon }: { label: string, options: string[], name: string, icon?: any }) => (
    ...
    const SelectField = ({ label, options }: { label: string, options: string[] }) => (
        <div>
            ...
            <div className="relative">
                <select className="w-full appearance-none px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-navy-900 text-sm font-medium text-navy-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-shadow">
            ...
            {showContextBanner && activeTab === 'snapshot' && (
                <div className="bg-gradient-to-r from-purple-50 to-white dark:from-purple-900/20 dark:to-navy-900 border border-purple-100 dark:border-purple-800/50 rounded-xl p-4 flex items-start gap-4 shadow-sm relative overflow-hidden animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="flex flex-col gap-2 z-10 shrink-0">
                        <button className="px-4 py-1.5 bg-purple-600 text-white text-xs font-semibold rounded-lg hover:bg-purple-700 shadow-sm transition-all shadow-purple-200 dark:shadow-none whitespace-nowrap">Accept</button>
                        <button
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <SelectField label="Industry" options={['Manufacturing', 'Logistics', 'Retail', 'Energy', 'Healthcare']} />
                                    <SelectField label="Sub-Industry" options={['Automotive', 'Electronics', 'FMCG', 'Pharma', 'Aerospace']} />
                                </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="bg-slate-50 dark:bg-navy-800/50 rounded-xl p-5 border border-slate-100 dark:border-white/5 space-y-5">
                                    <SelectField label="Employees (FTE)" options={['1-50', '51-200', '201-500', '501-1000', '1000-5000', '5000+']} />
                                    <SelectField label="Annual Revenue" options={['< $10M', '$10M - $50M', '$50M - $250M', '$250M - $1B', '> $1B']} />
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mb-3">
                                        <label className="cursor-pointer">
                                            <input type="radio" name="footprint" className="peer sr-only" />
                                            <div className="p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-navy-900 peer-checked:ring-2 peer-checked:ring-purple-500 peer-checked:border-purple-500 transition-all text-center">
                                                <div className="font-bold text-navy-900 dark:text-white mb-1">Single Site</div>
                                                <div className="text-[10px] text-slate-500">Centralized ops</div>
                                            </div>
                                        </label>
                                        <label className="cursor-pointer">
                                            <input type="radio" name="footprint" className="peer sr-only" />
                                            <div className="p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-navy-900 peer-checked:ring-2 peer-checked:ring-purple-500 peer-checked:border-purple-500 transition-all text-center">
                                                <div className="font-bold text-navy-900 dark:text-white mb-1">Multi-Site</div>
                                                <div className="text-[10px] text-slate-500">Distributed ops</div>
                                            </div>
                                        </label>
                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                {[
                                    ...
                                    <label key={model.id} className="relative cursor-pointer group">
                                        <input type="radio" name="prodModel" className="peer sr-only" />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        ...
                                        <div className="flex gap-2">
                                            {['1 Shift', '2 Shifts', '3 Shifts', '4-Brigade (24/7)'].map(shift => (
                                                <label key={shift} className="flex-1 cursor-pointer">
                                                    <input type="radio" name="shifts" className="peer sr-only" />
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <label className="text-xs font-bold text-slate-500 uppercase">Automation Level</label>
                                        </div>
                                        <input type="range" className="w-full accent-purple-600 h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer dark:bg-slate-700" />
                            ...
                            {[
                                ...
                                <label key={constraint.id} className="relative group cursor-pointer">
                                    ...
                                    <div className="h-full p-5 rounded-2xl border border-slate-200 dark:border-white/5 bg-white dark:bg-navy-900 hover:border-purple-300 dark:hover:border-purple-500/50 transition-all peer-checked:border-purple-500 peer-checked:ring-1 peer-checked:ring-purple-500/50 peer-checked:bg-purple-50/30 dark:peer-checked:bg-purple-900/10 shadow-sm">
                                        ...
                                        <div className="grid grid-rows-[0fr] peer-checked:grid-rows-[1fr] transition-all duration-300 ease-in-out">
                                            <div className="overflow-hidden">
                                                <div className="pt-4 mt-2 border-t border-slate-100 dark:border-white/5">
                                                    <AITextArea
                                                        value={constraintDetails[constraint.id] || ''}
                                                        onChange={(e) => setConstraintDetails(prev => ({ ...prev, [constraint.id]: e.target.value }))}
