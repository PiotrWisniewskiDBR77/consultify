...
import { ArrowRight, Lock, AlertCircle, X, ChevronLeft, Sparkles } from 'lucide-react';
...
export const AuthView: React.FC<AuthViewProps> = ({ initialStep, targetMode, onAuthSuccess, onBack }) => {
  ...
  const renderDemoRedirect = () => (
    <div className="text-center space-y-6 animate-in fade-in zoom-in-95 duration-300">
      ...
      <div>
        <h2 className="text-2xl font-bold text-navy-900 dark:text-white mb-2">{t('auth.demoMode', 'Demo Mode')}</h2>
        <p className="text-slate-500 dark:text-slate-400 text-sm max-w-xs mx-auto leading-relaxed">
          {t('auth.demoModeDescription', 'For commercial purposes, please contact our sales team. You can explore the platform in demo mode now.')}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <a
          ...
          className="w-full py-2.5 bg-white dark:bg-navy-800 border border-slate-200 dark:border-white/10 text-navy-900 dark:text-white font-semibold rounded-lg hover:border-purple-300 dark:hover:border-purple-500/30 transition-all text-center text-sm"
        >
      ...
      <button
        onClick={() => { setShowDemoRedirect(false); setError(null); }}
        className="text-purple-600 dark:text-purple-400 hover:text-purple-500 dark:hover:text-purple-300 font-medium hover:underline text-sm transition-colors"
      >
        {t('auth.back', 'Back')}
      </button>
  ...
  const renderCodeEntry = () => (
    <div className="space-y-8">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-bold text-navy-900 dark:text-white mb-2">{t('auth.unlockFull')}</h2>
        <p className="text-slate-500 dark:text-slate-400 text-sm max-w-xs mx-auto">{t('auth.enterCode')}</p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleRegister} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('auth.firstName')} <span className="text-purple-500 dark:text-purple-400">*</span></label>
            <input
              required
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('auth.lastName')} <span className="text-purple-500 dark:text-purple-400">*</span></label>
            <input
              required
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('auth.email')} <span className="text-purple-500 dark:text-purple-400">*</span></label>
          <input
            type="email"
            required
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('auth.phone')}</label>
          <input
            type="tel"
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('auth.company')} <span className="text-purple-500 dark:text-purple-400">*</span></label>
          <input
            required
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('auth.accessCode')} <span className="text-slate-400 font-normal">({t('auth.optional')})</span></label>
          <input
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('auth.password')} <span className="text-purple-500 dark:text-purple-400">*</span></label>
          <input
            type="password"
            required
            minLength={8}
      ...
      <div className="text-center pt-3 space-y-2">
        <div className="text-sm text-slate-500 dark:text-slate-400">
          {t('auth.haveAccount')}{' '}
          <button onClick={() => setStep(AuthStep.LOGIN)} className="text-purple-600 dark:text-purple-400 hover:text-purple-500 dark:hover:text-purple-300 font-medium hover:underline">{t('auth.logIn')}</button>
        </div>
  ...
  const renderLogin = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-navy-900 dark:text-white mb-2">{t('auth.welcomeBack')}</h2>
        <p className="text-slate-500 dark:text-slate-400 text-sm">{t('auth.signInText')}</p>
      </div>

      <form onSubmit={handleLogin} className="space-y-5">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('auth.email')}</label>
          <input
            type="email"
            required
            value={formData.email}
      ...
      <div className="text-center pt-4 text-sm text-slate-500 dark:text-slate-400">
        {t('auth.noAccount')}{' '}
        <button onClick={() => setStep(AuthStep.REGISTER)} className="text-purple-600 dark:text-purple-400 hover:text-purple-500 dark:hover:text-purple-300 font-medium hover:underline">{t('auth.createOne')}</button>
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-full bg-slate-50 dark:bg-navy-950 p-6 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="mt-8 flex flex-col items-center gap-3 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
        <a
          href="/"
          className="flex items-center gap-2 px-4 py-2 bg-white/50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg text-slate-600 dark:text-slate-300 hover:text-purple-600 dark:hover:text-purple-400 hover:border-purple-300 dark:hover:border-purple-500/30 text-sm transition-all group"
        >
