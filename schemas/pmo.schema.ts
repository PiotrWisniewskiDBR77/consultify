...
export const CreateDecisionSchema = z.object({
  ...
  title: z.string().min(1, 'Decision title is required').max(255, 'Title too long').trim(),
  description: z.string().min(10, 'Please provide more detail').max(5000, 'Description too long'),
  status: z.enum(['pending', 'approved', 'rejected', 'deferred']).optional().default('pending'),
...
export const CreateRAIDItemSchema = z.object({
  ...
  status: z.enum(['open', 'in_progress', 'mitigated', 'closed']).optional().default('open'),
...
export const UpdateRAIDItemSchema = CreateRAIDItemSchema.partial().omit({
  projectId: true,
  type: true,
});
...
export const CreateStageGateSchema = z.object({
  projectId: z.string().uuid(),
  name: z.string().min(1, 'Gate name is required').max(100, 'Name too long').trim(),
...
export const UpdateGovernanceSettingsSchema = z.object({
  ...
  standardsCompliance: z
    .array(z.enum(['ISO21500', 'PMBOK7', 'PRINCE2', 'IPMA', 'CUSTOM']))
    .optional(),
});
