...
Target persona: COO / plant director weighing footprint, equipment, and intralogistics investments  
Funnel stage: Decision  
Core problem: brownfield expansion debates mix floor space, equipment, and labor fixes in one conversation, so the winning story is often the easiest slide, not the least fragile under stress  
Main promise: a comparison checklist that forces layout, flow, constraint, and cash signals into one table before you expand walls or buy machines

before you expand capacity in a brownfield factory, compare options under the same demand and supply lens on constraint time migration, intralogistics travel and handling load, WIP and floor congestion, changeover and mix sensitivity, ramp duration, cash timing, and ability to unwind if demand shifts. Run paired scenarios for each serious path, not only the preferred narrative. Brownfield expansion is a geometry problem and a flow problem. Treat it as both before you spend.
