...
Target persona: Production manager / Shift manager / Continuous improvement lead  
...
1. **Economic trigger**  
   Confirmed output or uptime loss on a named constraint with two or more occurrences in a review window

2. **Risk trigger**  
   Trend toward interlock, hold, or scrap threshold per plant standards

3. **Trust trigger**  
   Conflicting narratives between shifts about the same signal or override pattern

4. **Compliance trigger**  
   Evidence or retention rules require a reconstructable timeline

## Step sequence: from visibility to structured loop

1. **Stabilize and contain** using existing escalation and override rules  
2. **Capture the IoT slice**: timestamps, signals, operator notes, photos if allowed  
3. **Charter the problem** with a single owner, scope boundary, and time box  
4. **Run the method** your plant uses: A3, 8D light, DMAIC slice, or equivalent  
5. **Close with standards update** if definitions, training, or thresholds must change  
6. **Log integration** if the fix requires CMMS, engineering change, or IT-OT work
...
If every blip becomes a project, you will drown.

If no blip becomes a project, you will relive the same week forever.

Use triggers, owners, and evidence discipline to choose.
