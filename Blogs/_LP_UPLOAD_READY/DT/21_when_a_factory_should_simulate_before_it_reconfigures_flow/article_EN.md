...
You should simulate before reconfiguring flow when the change can move constraints, alter handoffs, or change how work accumulates between stations.

If the change is cosmetic or isolated, a lighter review may be enough.

If it changes how the system behaves under load, simulation is the cheapest place to discover mistakes.

## Direct answer

Simulate first when at least one of these is true:

- the new flow shares a bottleneck or buffer with other lines
- staffing, shift patterns, or batching logic changes
- you are rebalancing work to hit a new takt or mix
- intralogistics paths or supermarket sizing change
- the business case assumes a specific throughput or lead time
...
- where queues form when variability returns
- how a "small" move shifts the system constraint
- whether a faster local step creates starvation upstream
- how changeovers or batch breaks propagate

A Digital Twin in this context is not a 3D showcase.
...
1. a credible process sequence with realistic cycle time ranges
2. changeover and failure assumptions stated as ranges, not single points
3. demand or order mix scenarios that reflect peak and slump
4. staffing rules that match how the line is actually run
...
Run at least three scenario families:

- baseline current flow
- proposed flow under expected demand
- proposed flow under stress demand or worst-case mix
...
Simulation is not a moral obligation.
