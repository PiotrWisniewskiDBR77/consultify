...
Target persona: Project Manager / Buyer Owner  
Funnel stage: Consideration to Decision transition  
Core problem: early vendor meetings create energy, then teams lose weeks to unstructured follow-up, duplicate questions, and silent internal drift  
Main promise: a post-meeting operating rhythm that converts conversations into comparable artifacts and dated decisions

The first vendor meetings are easy.

They feel like progress.

The hard part starts when the room empties.

Momentum dies when follow-up is informal:

- questions scatter across email
- assumptions live in personal notes
- internal alignment quietly reverses
- suppliers optimize for the loudest voice
...
After each supplier interaction, publish internally:

- date and attendees
- what was shown
- what was promised as a next step
- open questions owned by names
- deltas versus the brief (what changed)

If this record does not exist, you do not have organizational memory.

You have vibes.

## Artifact 2: a living assumptions log (shared, short)

Assumptions are not shameful.

They are inventory.

Keep a shared log with three columns:

| Assumption | owner to verify | due date |

If the log grows past one page, split into "pre-decision" versus "post-award discovery."
...
1. review open questions and assign owners
2. update assumptions log
3. refresh comparison matrix with new facts only
4. confirm the decision timeline still matches plant reality
5. send one consolidated clarification batch to suppliers when needed
...
Meetings create excitement.

Discipline creates decisions.
