# Co powinien zawierac dobry plan mobilizacji po stronie producenta po przyznaniu

Target persona: sponsor operacji odpowiedzialny za gotowosc po przyznaniu  
Funnel stage: Delivery (mobilizacja)  
Core problem: dostawcy mobilizuja sie, podczas gdy zaklad nadal traktuje projekt jako problem na pozniej  
Main promise: plan mobilizacji po stronie producenta, ktory nazywa daty, wlascicieli i zaleznosci, ktorych integrator nie przejmie za ciebie

Przyznanie to nie tylko start pracy dostawcy.

To start pracy zakladu, ktorej nie mozesz delegowac.
...
- dni zero do trzydziestu: zaleznosci zamkniete lub oznaczone na czerwono, testy interfejsow zaplanowane  
- dni trzydziesci do szescdziesieciu: proby szkolen, szablony dowodow akceptacji uzgodnione  
- punkt kontrolny dzien szescdziesiat: go lub no-go dla agresywnych dat uruchomienia  
...
Mobilizacja to miejsce, gdzie porownane oferty spotykaja rzeczywistosc zakladu; plan producenta to ubezpieczenie, ze ta rzeczywistosc byla budzetowana.
