  ...
  [routesDir, controllersDir, servicesDir].forEach((scanDir) => {
    ...
    walkDir(scanDir, (filePath) => {
      ...
      while ((match = dynamicRegex.exec(content)) !== null) {
        ...
        if (
          fs.existsSync(absoluteTarget + '.ts') ||
        ...
        if (!exists) {
          ...
          if (!lineContent.trim().startsWith('//')) {
            ...
            const destructureMatch = lineContent.match(
              /(?:const|let|var)\s+\{\s*([\w:\s,]+)\s*\}\s*=/
            );
      ...
      // 2. Static imports
      const staticRegex =
        /import\s+(?:(\w+)|(?:\*\s+as\s+(\w+))|\{\s*([\w\s,]+)\s*\})\s+from\s+['"]([^'"]+)\.js['"];/g;
      while ((match = staticRegex.exec(content)) !== null) {
        ...
        if (!exists) {
          ...
          if (!content.includes(`// Stubbed missing`)) {
            // Weak check, but prevents double stubbing same line slightly
