...
<ModalTabs tabs={tabs} activeTab={activeTab} onTabChange={setActiveTab} accentColor="blue" />;
```
...
<ModalContainer isOpen={isOpen} onClose={onClose} maxWidth="5xl" height="full">
  {/* Modal content */}
</ModalContainer>;
```
    ...
    <ModalContainer isOpen={isOpen} onClose={onClose} maxWidth="3xl">
      ...
      <div className="flex-1 p-6">
        <InputGroup label="Name">
          <FormInput value={formData.name} onChange={(e) => setField('name', e.target.value)} />
        </InputGroup>
      </div>
...
_Dokumentacja wygenerowana automatycznie przez Cursor AI dla projektu ANTYGRACITY/Consultinity_
