    ...
    const projects = [
        {
            ...
            contextData: JSON.stringify({
                description: 'Comprehensive DRD 7-axis digital maturity assessment for manufacturing operations',
                type: 'DRD'
            }),
...
async function seedDRDAssessment() {
    ...
    const drdAxisScores = {
        processes: { actual: 3.2, target: 5.0, areaScores: { '1A': [3, 5], '1B': [3, 5], '1C': [4, 5], '1D': [3, 5] } },
        digitalProducts: { actual: 2.8, target: 5.5, areaScores: { '2A': [3, 5], '2B': [2, 6], '2C': [3, 5], '2D': [3, 6] } },
        businessModels: { actual: 3.5, target: 5.0, areaScores: { '3A': [3, 5], '3B': [4, 5], '3C': [3, 5], '3D': [4, 5] } },
        dataManagement: { actual: 2.1, target: 5.0, areaScores: { '4A': [2, 5], '4B': [2, 5], '4C': [2, 5], '4D': [2, 5] } },
        culture: { actual: 4.0, target: 5.5, areaScores: { '5A': [4, 5], '5B': [4, 6], '5C': [4, 5], '5D': [4, 6] } },
        cybersecurity: { actual: 3.0, target: 5.0, areaScores: { '6A': [3, 5], '6B': [3, 5], '6C': [3, 5], '6D': [3, 5] } },
        aiMaturity: { actual: 2.0, target: 4.5, areaScores: { '7A': [2, 4], '7B': [2, 5], '7C': [2, 4], '7D': [2, 5], '7E': [2, 4] } }
    };
    ...
    `, [
        ...
        JSON.stringify(['processes', 'digitalProducts', 'businessModels', 'dataManagement', 'culture', 'cybersecurity', 'aiMaturity']),
        overallAsIs,
        overallToBe,
        overallGap,
        1,
    ...
    console.log(`     Overall: As-Is ${overallAsIs} → To-Be ${overallToBe} (Gap: ${overallGap.toFixed(2)})`);
...
async function seedLeanAssessment() {
    ...
    console.log(`     Overall Score: ${overallScore.toFixed(2)} / 5.0 (Benchmark: ${industryBenchmark})`);
...
async function seedInitiatives() {
    ...
    const initiatives = [
        // DRD Initiatives (8)
        {
            ...
            summary: 'Deploy computer vision AI system for automated quality inspection on production lines, reducing defect rates by 40%.',
            roi: 280,
            cost: 450000
        },
        {
            ...
            value: 'High',
            owner: IDS.USER_CFO,
            summary: 'Implement centralized data lake architecture to unify siloed data sources and enable advanced analytics.',
            roi: 200,
            cost: 800000
        },
        {
            ...
            value: 'High',
            owner: IDS.USER_COO,
            summary: 'Migrate legacy on-premise ERP system to cloud-based SAP S/4HANA for improved scalability and real-time insights.',
            roi: 150,
            cost: 1200000
        },
        {
            ...
            value: 'Medium',
            owner: IDS.USER_PM,
            summary: 'Launch interactive digital product catalog with 3D visualization and AR capabilities for B2B customers.',
            roi: 180,
            cost: 300000
        },
        {
            ...
            value: 'Critical',
            owner: IDS.USER_ADMIN,
            summary: 'Implement Zero Trust security architecture across all digital assets and manufacturing systems.',
            roi: 120,
            cost: 500000
        },
        {
            ...
            value: 'Medium',
            owner: IDS.USER_INNOVATION,
            summary: 'Develop customer portal for order tracking, documentation access, and support ticket management.',
            roi: 160,
            cost: 250000
        },
        {
            ...
            summary: 'Establish comprehensive data governance policies, data stewardship roles, and quality monitoring processes.',
            roi: 100,
            cost: 150000
        },
        {
            ...
            value: 'Medium',
            owner: IDS.USER_INNOVATION,
            summary: 'Launch company-wide innovation program including hackathons, idea management platform, and innovation labs.',
            roi: 140,
            cost: 200000
        },
        // Lean 4.0 Initiatives (4)
        {
            ...
            value: 'High',
            owner: IDS.USER_PM,
            summary: 'Implement structured Kaizen events targeting waste elimination on Assembly Line A, targeting 25% efficiency gain.',
            roi: 220,
            cost: 180000
        },
        {
            ...
            value: 'Medium',
            owner: IDS.USER_COO,
            summary: 'Deploy digital Kanban system for inventory management and production scheduling across 3 facilities.',
            roi: 180,
            cost: 120000
        },
        {
            ...
            value: 'Medium',
            owner: IDS.USER_PM,
            summary: 'Establish regular Gemba walk routines for leadership team to identify improvement opportunities on the shop floor.',
            roi: 90,
            cost: 50000
        },
        {
            ...
            value: 'High',
            owner: IDS.USER_ANALYST,
            summary: 'Deploy real-time visual management dashboards showing OEE, production metrics, and quality KPIs on shop floor displays.',
            roi: 160,
            cost: 100000
        }
    ];
...
async function seedTasks() {
    ...
    const initiativeIds = [
        IDS.INIT_01, IDS.INIT_02, IDS.INIT_03, IDS.INIT_04, IDS.INIT_05, IDS.INIT_06,
        IDS.INIT_07, IDS.INIT_08, IDS.INIT_09, IDS.INIT_10, IDS.INIT_11, IDS.INIT_12
    ];

    const userIds = [
        IDS.USER_ADMIN, IDS.USER_COO, IDS.USER_CFO,
        IDS.USER_INNOVATION, IDS.USER_PM, IDS.USER_ANALYST
    ];
    ...
    for (const initId of initiativeIds) {
        ...
        const projectId = initId.includes('init-00') && parseInt(initId.slice(-1)) <= 8 ? IDS.PROJ_DRD : IDS.PROJ_LEAN;
...
async function seedSessionData() {
    ...
    const drdSessionData = {
        ...
        assessment: {
            processes: { actual: 3.2, target: 5.0, justification: 'Current processes are partially digitized with manual workarounds.' },
            digitalProducts: { actual: 2.8, target: 5.5, justification: 'Limited digital product offerings, mostly traditional catalog.' },
            businessModels: { actual: 3.5, target: 5.0, justification: 'Traditional B2B model with emerging digital channels.' },
            dataManagement: { actual: 2.1, target: 5.0, justification: 'Data silos across departments, no unified data strategy.' },
            culture: { actual: 4.0, target: 5.5, justification: 'Open to change, strong leadership support for transformation.' },
            cybersecurity: { actual: 3.0, target: 5.0, justification: 'Basic security measures in place, needs modern framework.' },
            aiMaturity: { actual: 2.0, target: 4.5, justification: 'Limited AI adoption, mostly manual analytics.' }
        },
        roadmap: {
    ...
    `, [
        ...
        IDS.PROJ_LEAN,
        'full',
        JSON.stringify(leanSessionData)
    ]);
...
async function seedNotifications() {
    ...
    const notifications = [
        ...
        },
        {
            ...
            message: 'Your Data Management maturity (2.1) is significantly below target (5.0). Consider prioritizing Data Governance Framework initiative.',
            isRead: 0
        },
        {
            userId: IDS.USER_ADMIN,
        ...
        },
        {
            ...
            message: 'Michael Chen mentioned you in AI-Powered Quality Inspection: "Emily, can you review the vendor proposals?"',
            isRead: 0
        },
        {
            userId: IDS.USER_PM,
        ...
        },
        {
            ...
            message: 'Based on your Lean 4.0 assessment, implementing Kaizen events could yield 25% efficiency gains.',
            isRead: 0
        },
        {
            userId: IDS.USER_COO,
        ...
        },
        {
            ...
            message: 'David Miller requested your review on "Technical Design" for AI-Powered Quality Inspection.',
            isRead: 0
        },
        {
            userId: IDS.USER_ANALYST,
...
async function verify() {
    ...
    // Sessions
    const sessionCount = await dbGet(`SELECT COUNT(*) as count FROM sessions WHERE project_id IN (?, ?)`, [IDS.PROJ_DRD, IDS.PROJ_LEAN]);
    checks.push({
        name: 'Sessions',
    ...
    // Notifications
    const notifCount = await dbGet(`SELECT COUNT(*) as count FROM notifications WHERE organization_id = ?`, [IDS.ORG]);
    checks.push({
        name: 'Notifications',
    ...
    // DRD Assessment
    const drdAssess = await dbGet(`SELECT * FROM maturity_assessments WHERE id = ?`, [IDS.ASSESS_DRD]);
    checks.push({
        name: 'DRD Assessment',
        passed: !!drdAssess,
        details: drdAssess ? `As-Is: ${drdAssess.overall_as_is}, To-Be: ${drdAssess.overall_to_be}` : 'NOT FOUND'
    });

    // Lean Assessment
    const leanAssess = await dbGet(`SELECT * FROM rapid_lean_assessments WHERE id = ?`, [IDS.ASSESS_LEAN]);
    checks.push({
