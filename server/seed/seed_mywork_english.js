...
const SAMPLE_DECISIONS = [
  ...
  },
  {
    ...
    description:
      'Approve the proposed scope changes for Phase 2 of the digital transformation project',
...
async function seedMyWorkEnglishData() {
  ...
  try {
    ...
    const project = await dbGet('SELECT id FROM projects WHERE organization_id = ? LIMIT 1', [
      organizationId,
    ]);
    ...
    // Verify data
    const taskVerify = await dbGet(
      'SELECT COUNT(*) as count FROM tasks WHERE organization_id = ?',
      [organizationId]
    );
    const decisionVerify = await dbGet(
