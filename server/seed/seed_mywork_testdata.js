...
const MY_WORK_TASKS = [
  // 🔴 BLOCKING PHASE tasks
  {
    ...
    description:
      'Required documentation to pass the Design to Execution gate. Blocks initiative approval.',
    status: 'in_progress',
    priority: 'urgent',
    stepPhase: 'design',
    taskType: 'VALIDATION',
  ...
  },
  {
    ...
    description:
      'Sponsor must approve the budget before moving to the pilot phase. Critical for the schedule.',
    status: 'todo',
    priority: 'urgent',
    stepPhase: 'design',
    taskType: 'DECISION',
  ...
  },
  {
    ...
    description:
      'Team allocation and budget plan for the execution phase. Requires PMO acceptance.',
    status: 'blocked',
    priority: 'high',
    stepPhase: 'design',
    taskType: 'ANALYSIS',
  ...
  // 🟠 BLOCKING INITIATIVE tasks
  {
    ...
    description:
      'Third-party API is not responding according to specification. Blocking IoT initiative progress.',
    status: 'in_progress',
    priority: 'high',
    stepPhase: 'execution',
    taskType: 'BUILD',
  ...
  },
  {
    ...
    description:
      '3 critical bugs identified during acceptance testing. No acceptance without fixes.',
    status: 'in_progress',
    priority: 'urgent',
    stepPhase: 'pilot',
    taskType: 'BUILD',
  ...
  },
  {
    ...
    description:
      'Client requests additional dashboards. Requires decision on scope and budget extension.',
    status: 'todo',
    priority: 'medium',
    stepPhase: 'execution',
    taskType: 'DECISION',
  ...
  // ⚫ OVERDUE tasks
  {
    ...
    description:
      'Report summarizing discovery phase for AI initiative. Deadline passed 5 days ago.',
    status: 'in_progress',
    priority: 'high',
    stepPhase: 'design',
    taskType: 'ANALYSIS',
...
const MY_WORK_NOTIFICATIONS = [
  // CRITICAL
  {
    ...
    message:
      'Smart Factory Initiative has been blocked due to resource shortage. Immediate intervention required.',
  ...
  },
  {
    ...
    message:
      'AI analysis indicates 85% probability of E-Commerce Platform project delay by 2 weeks.',
...
const MY_WORK_DECISIONS = [
  {
    title: 'Select CRM Platform Vendor',
    description:
      'Strategic decision to choose between Salesforce and Microsoft Dynamics 365. Impacts budget and timeline of the whole program.',
    status: 'PENDING',
    dueDate: 3,
    relatedObjectType: 'INITIATIVE',
  ...
  },
  {
    ...
    description:
      'Client requests additional BI dashboards. Scope extension will increase budget by 15% and extend timeline by 3 weeks.',
    status: 'PENDING',
    dueDate: 4,
    relatedObjectType: 'INITIATIVE',
  ...
  },
  {
    ...
    description:
      'Big Bang vs Phased Migration for Legacy system. Big Bang = higher risk, lower cost. Phased = safer, more expensive.',
    status: 'PENDING',
    dueDate: 6,
    relatedObjectType: 'INITIATIVE',
  ...
  },
  {
    ...
    description:
      'Decision to move from pilot to production. All technical criteria met, awaiting business approval.',
    status: 'PENDING',
    dueDate: 7,
    relatedObjectType: 'INITIATIVE',
...
async function seedMyWorkTestData() {
  ...
  try {
    ...
    const user = await dbGet(
      `SELECT id, organization_id FROM users WHERE email LIKE '%piotr%' LIMIT 1`
    );
    if (!user) {
    ...
    let project = await dbGet(`SELECT id FROM projects WHERE organization_id = ? LIMIT 1`, [
      organizationId,
    ]);
    if (!project) {
    ...
    await dbRun(
      `DELETE FROM tasks WHERE title LIKE '%PMO Test%' OR title IN (${MY_WORK_TASKS.map(() => '?').join(',')})`,
      MY_WORK_TASKS.map((t) => t.title)
    );
    await dbRun(
      `DELETE FROM notifications WHERE title LIKE '%Test%' OR type IN ('INITIATIVE_BLOCKED', 'BOTTLENECK_DETECTED', 'WEEKLY_DIGEST')`
    );
    ...
    for (const notifDef of MY_WORK_NOTIFICATIONS) {
      ...
      const priority =
        notifDef.severity === 'CRITICAL'
          ? 'high'
          : notifDef.severity === 'WARNING'
            ? 'normal'
            : 'low';
      ...
      await dbRun(
        ...
        [
          ...
          notifDef.title,
          notifDef.message,
          notifDef.relatedObjectType || null,
          relatedObjectId,
          priority,
          notifDef.actionUrl ? 1 : 0,
          notifDef.actionUrl || null,
      ...
      for (const decisionDef of MY_WORK_DECISIONS) {
        ...
        await dbRun(
          ...
          [
            decisionId,
            project.id,
            decisionDef.relatedObjectType,
            relatedObjectId,
            userId,
            decisionDef.status,
            decisionDef.title,
            decisionDef.description,
          ]
        );
    ...
    try {
      ...
      for (let i = 0; i < focusCandidates.length; i++) {
        ...
        await dbRun(
          `INSERT INTO focus_tasks (id, user_id, task_id, order_index, created_at)
                    VALUES (?, ?, ?, ?, datetime('now'))`,
        ...
        await dbRun(
          `INSERT INTO focus_time_blocks (id, focus_task_id, start_time, end_time, created_at)
                    VALUES (?, ?, ?, ?, datetime('now'))`,
    ...
    console.log(
      `   - 🔴 Blocking Phase: ${MY_WORK_TASKS.filter((t) => t.labels.includes('BLOCKING_PHASE')).length}`
    );
    console.log(
      `   - 🟠 Blocking Initiative: ${MY_WORK_TASKS.filter((t) => t.labels.includes('BLOCKING_INITIATIVE')).length}`
    );
    console.log(
      `   - 🟡 Awaiting Decision: ${MY_WORK_TASKS.filter((t) => t.labels.includes('AWAITING_DECISION')).length}`
    );
    console.log(`   - ⚫ Overdue: ${MY_WORK_TASKS.filter((t) => t.dueDateOffset < 0).length}`);
    console.log(`   Notifications: ${notificationsCreated}`);
    console.log(
      `   - 🔴 Critical: ${MY_WORK_NOTIFICATIONS.filter((n) => n.severity === 'CRITICAL').length}`
    );
    console.log(
      `   - 🟡 Warning: ${MY_WORK_NOTIFICATIONS.filter((n) => n.severity === 'WARNING').length}`
    );
    console.log(
      `   - 🔵 Info: ${MY_WORK_NOTIFICATIONS.filter((n) => n.severity === 'INFO').length}`
    );
...
export default seedMyWorkTestData;
