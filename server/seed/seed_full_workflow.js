...
async function seedFullWorkflow() {
  ...
  try {
    ...
    const orgs = await dbAll(
      `SELECT id, name FROM organizations WHERE name LIKE '%DBR77%' LIMIT 1`
    );
    ...
    // Get projects
    const projects = await dbAll(`SELECT id, name FROM projects WHERE organization_id = ?`, [
      orgId,
    ]);
    ...
    let initiatives = await dbAll(
      `SELECT id, name, project_id, status, axis FROM initiatives WHERE organization_id = ?`,
      [orgId]
    );
    ...
    const waves = [
      {
        ...
        description: 'Infrastructure and core capabilities',
        months: 0,
        duration: 3,
        status: 'ACTIVE',
      },
      {
        ...
        description: 'Core digital systems implementation',
        months: 3,
        duration: 4,
        status: 'PLANNED',
      },
      ...
      {
        name: 'Wave 4: AI & Analytics',
        description: 'Advanced AI and analytics capabilities',
        months: 11,
        duration: 4,
        status: 'PLANNED',
      },
      {
        name: 'Wave 5: Optimization',
    ...
    const approvedInitiatives = initiatives.filter(
    ...
    for (const gate of gateTypes) {
      ...
      await dbRun(
        ...
        [
          ...
          gate.status,
          completionCriteria,
          gate.status !== 'NOT_READY' ? piotr.id : null,
        ]
      );
    ...
    const decisionTypes = [
      {
        ...
        title: 'Approve Digital Process Automation',
        status: 'APPROVED',
        objType: 'INITIATIVE',
      },
      {
        type: 'INITIATIVE_APPROVAL',
        title: 'Approve Data Lake Implementation',
        status: 'APPROVED',
        objType: 'INITIATIVE',
      },
      {
        type: 'PHASE_TRANSITION',
        title: 'Move to Roadmap Phase',
        status: 'APPROVED',
        objType: 'PHASE',
      },
      {
        type: 'UNBLOCK',
        title: 'Resource Allocation for AI Team',
        status: 'PENDING',
        objType: 'INITIATIVE',
      },
      {
        type: 'INITIATIVE_APPROVAL',
        title: 'Approve Cloud Migration Phase 2',
        status: 'PENDING',
        objType: 'INITIATIVE',
      },
    ...
    for (let i = 0; i < decisionTypes.length; i++) {
      ...
      const relatedId =
        dec.objType === 'INITIATIVE' && approvedInitiatives[i]
          ? approvedInitiatives[i].id
          : projectId;
    ...
    let taskCount = 0;
    for (const init of approvedInitiatives.slice(0, 5)) {
      // First 5 initiatives
    ...
    const notificationTemplates = [
      {
        ...
        message: 'Your approval is needed for Cloud Migration Phase 2',
      },
      {
        type: 'gate_ready',
        title: 'Stage Gate Ready',
        message: 'APPROVAL_GATE is ready for review',
      },
      {
        type: 'initiative_blocked',
        title: 'Initiative Blocked',
        message: 'AI Predictive Analytics is waiting for dependencies',
      },
      {
        type: 'task_overdue',
        title: 'Task Overdue',
        message: 'Documentation task is 2 days overdue',
      },
      {
        type: 'milestone_approaching',
        title: 'Milestone Approaching',
        message: 'Wave 1 completion in 5 days',
      },
    ];
