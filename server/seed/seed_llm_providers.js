...
db.serialize(() => {
  ...
  const stmt = db.prepare(
    `INSERT OR REPLACE INTO llm_providers (id, name, provider, model_id, api_key, is_active, is_default, cost_per_1k, visibility) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );

  providers.forEach((p) => {
    db.run(
      'DELETE FROM llm_providers WHERE provider = ? AND model_id = ?',
