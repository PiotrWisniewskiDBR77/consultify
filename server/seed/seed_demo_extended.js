...
async function seedDemoExtended() {
  ...
  try {
    ...
    const orgs = await dbAll(
      `SELECT id, name FROM organizations WHERE name LIKE '%TechVision%' OR name LIKE '%Demo%' LIMIT 1`
    );
    ...
    if (initiatives.length < 15) {
      ...
      const additionalInitiatives = [
        ...
        {
          name: 'Employee Experience Portal',
          status: 'DRAFT',
          priority: 'MEDIUM',
          axis: 'culture',
        },
    ...
    const waves = [
      {
        ...
        desc: 'Core infrastructure and security',
        monthsStart: 0,
        duration: 4,
      },
      {
        name: 'Data & Analytics',
        desc: 'Data platform and analytics capabilities',
        monthsStart: 4,
        duration: 5,
      },
      {
        name: 'Digital Products',
        desc: 'Customer-facing digital products',
        monthsStart: 9,
        duration: 6,
      },
      {
        name: 'AI & Automation',
        desc: 'AI/ML deployment and automation',
        monthsStart: 15,
        duration: 5,
      },
      {
        name: 'Scale & Optimize',
        desc: 'Performance optimization and scaling',
        monthsStart: 20,
        duration: 4,
      },
    ];
    ...
    const decisions = [
      {
        ...
        title: 'Approve ERP Modernization Budget',
        status: 'APPROVED',
      },
    ...
    for (let i = 0; i < decisions.length; i++) {
      ...
      await dbRun(
        ...
        [
          ...
          dec.status,
          dec.title,
          `Decision details for: ${dec.title}`,
        ]
      );
    }
    ...
    const changeRequests = [
      ...
      {
        title: 'Budget increase for AI tools',
        type: 'BUDGET',
        status: 'SUBMITTED',
        risk: 'MEDIUM',
      },
      ...
      for (const template of taskTemplates) {
        ...
        await dbRun(
          ...
          [
            ...
            template.status,
            template.status === 'IN_PROGRESS' ? 'HIGH' : 'MEDIUM',
            assignee.id,
            demoUser.id,
          ]
        );
    ...
    const notifications = [
      {
        ...
        message: 'ML Governance Framework needs your approval',
      },
      { type: 'gate_ready', title: 'Gate Ready', message: 'Approval Gate is ready for review' },
      {
        type: 'task_assigned',
        title: 'New Task',
        message: 'You have been assigned to Development Sprint 2',
      },
      {
        type: 'risk_escalated',
        title: 'Risk Alert',
        message: 'Resource Availability risk has been escalated',
      },
      { type: 'milestone_achieved', title: 'Milestone', message: 'Design Gate has been passed!' },
      {
        type: 'change_request',
        title: 'Change Request',
        message: 'New change request for additional security features',
      },
    ];
