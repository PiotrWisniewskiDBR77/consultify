...
async function seedTestAssessments() {
  ...
  try {
    ...
    const project = await new Promise((resolve, reject) => {
      db.get(
        `SELECT id, name FROM projects WHERE organization_id = ? LIMIT 1`,
        [organizationId],
        (err, row) => {
          if (err) reject(err);
          else resolve(row);
    ...
    const user = await new Promise((resolve, reject) => {
      db.get(
        `SELECT id, first_name, last_name FROM users WHERE organization_id = ? LIMIT 1`,
        [organizationId],
        (err, row) => {
          if (err) reject(err);
          else resolve(row);
    ...
    await new Promise((resolve, reject) => {
      db.run(
        `DELETE FROM assessment_workflows WHERE organization_id = ?`,
        [organizationId],
        (err) => (err ? reject(err) : resolve())
      );
    });
    ...
    await new Promise((resolve, reject) => {
      db.run(`DELETE FROM maturity_assessments WHERE project_id = ?`, [projectId], (err) =>
    ...
    console.log(
      `       Overall As-Is: ${overallAsIs.toFixed(1)}, To-Be: ${overallToBe.toFixed(1)}, Gap: ${overallGap.toFixed(1)}`
    );
