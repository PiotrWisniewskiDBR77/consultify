  ...
  try {
    ...
    for (let i = 0; i < Math.min(12, 15 - orgs.length); i++) {
      ...
      await dbRun(
        `
                ...
                VALUES (?, ?, ?, ?, ?)
            `,
        [orgId, orgNames[i] || `Test Org ${i + 1}`, plan, status, randomDate(90)]
      );
    ...
    for (let i = 0; i < 100; i++) {
      ...
      await dbRun(
        ...
        [
          ...
          netRevenue,
          type === 'purchase'
            ? `Token package purchase - ${Math.abs(tokens).toLocaleString()} tokens`
            : type === 'bonus'
    ...
    const plansData = [
      ...
      },
      {
        ...
        token_limit: 500000,
        storage_limit_gb: 50,
        features: JSON.stringify(['basic', 'advanced']),
      },
      {
        name: 'Enterprise',
        price_monthly: 199,
        ...
        features: JSON.stringify(['basic', 'advanced', 'premium']),
      },
    ];
