...
async function seedTechVisionWorkflow() {
  ...
  try {
    ...
    if (!orgs || orgs.length === 0) {
      console.log(
        '⚠️ TechVision Industries not found or has no users. Run seed_demo_complete.js first.'
      );
    ...
    const users = await dbAll(
      `SELECT id, first_name, last_name, email FROM users WHERE organization_id = ?`,
      [orgId]
    );
    ...
    const waves = [
      {
        ...
        desc: 'Core infrastructure, IT/OT integration',
        start: -1,
        dur: 4,
        status: 'ACTIVE',
      },
      {
        name: 'Phase 2: Connect',
        desc: 'IoT sensors, data collection, connectivity',
        start: 3,
        dur: 5,
        status: 'PLANNED',
      },
      {
        ...
        desc: 'Analytics, dashboards, insights',
        start: 8,
        dur: 5,
        status: 'PLANNED',
      },
      {
        name: 'Phase 4: Automate',
        desc: 'AI/ML, predictive systems, automation',
        start: 13,
        dur: 6,
        status: 'PLANNED',
      },
      {
        ...
        desc: 'Digital twins, autonomous systems',
        start: 19,
        dur: 5,
        status: 'PLANNED',
      },
    ...
    const decisions = [
      {
        ...
        title: 'Approve Smart Factory Budget €2.5M',
        status: 'APPROVED',
      },
      {
        type: 'INITIATIVE_APPROVAL',
        title: 'Approve IoT Platform Vendor Selection',
        status: 'APPROVED',
      },
    ...
    const risks = [
      ...
      {
        title: 'AI Model Accuracy Issues',
        type: 'delivery',
        severity: 'high',
        likelihood: 'medium',
        status: 'open',
      ...
      {
        title: 'Change Fatigue in Operations',
        type: 'change_fatigue',
        severity: 'medium',
        likelihood: 'high',
        status: 'mitigating',
      ...
      {
        title: 'Regulatory Compliance (NIS2)',
        type: 'delivery',
        severity: 'high',
        likelihood: 'low',
        status: 'open',
    ...
    const changeRequests = [
      {
        title: 'Expand IoT sensor coverage to Warehouse B',
        type: 'SCOPE',
        status: 'APPROVED',
        risk: 'MEDIUM',
      },
      {
        title: 'Accelerate AI deployment by 2 months',
        type: 'SCHEDULE',
        status: 'APPROVED',
        risk: 'HIGH',
      },
      {
        title: 'Add real-time alerting to dashboard',
        type: 'SCOPE',
        status: 'SUBMITTED',
        risk: 'LOW',
      },
      {
        title: 'Increase cloud compute budget +€150K',
        type: 'BUDGET',
        status: 'SUBMITTED',
        risk: 'MEDIUM',
      },
      {
        title: 'Include carbon tracking in Digital Twin',
        type: 'SCOPE',
        status: 'DRAFT',
        risk: 'LOW',
      },
      {
        title: 'Postpone Quality Vision to Q2',
        type: 'SCHEDULE',
        status: 'REJECTED',
        risk: 'MEDIUM',
      },
    ...
    const inProgressTasks = await dbAll(
      `SELECT id FROM tasks WHERE organization_id = ? AND status = 'IN_PROGRESS' LIMIT 5`,
      [orgId]
    );
    ...
    const notifications = [
      {
        ...
        message: 'Digital Twin Investment needs your approval',
      },
      {
        type: 'gate_ready',
        title: 'Pilot Gate Ready',
        message: 'Pilot Gate is ready for final review',
      },
      {
        type: 'risk_escalated',
        title: 'Risk Escalated',
        message: 'IoT Security Risk has been escalated to critical',
      },
      {
        type: 'task_overdue',
        title: 'Task Overdue',
        message: 'Security Audit for Smart Factory is overdue',
      },
      {
        type: 'initiative_blocked',
        title: 'Initiative Blocked',
        message: 'Digital Twin Factory waiting on dependencies',
      },
      {
        type: 'change_approved',
        title: 'Change Approved',
        message: 'IoT sensor expansion has been approved',
      },
      {
        type: 'milestone_achieved',
        title: 'Milestone',
        message: 'Approval Gate passed - Pilot can begin!',
      },
    ];
...
export default seedTechVisionWorkflow;
