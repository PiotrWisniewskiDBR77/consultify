...
async function checkAndTriggerAlerts() {
  ...
  try {
    ...
    const orgs: Array<{ id: string }> =
      (await (deps.db.all<{ id: string }>('SELECT id FROM organizations WHERE status = ?', [
        'active',
...
async function generatePayAsYouGoInvoices() {
  ...
  try {
    ...
    const orgs: Array<{ organization_id: string }> =
      (await (deps.db.all<{ organization_id: string }>(
        `SELECT os.organization_id
             FROM organization_seats os
             WHERE os.billing_model IN('pay_as_you_go', 'hybrid')`,
        []
      ) as Promise<Array<{ organization_id: string }>>)) || [];
