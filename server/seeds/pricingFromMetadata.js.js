      ...
      if (row) {
        ...
        db.run(
          `UPDATE subscription_plans SET 
                        ...
                        is_active = 1
                    WHERE id = ?`,
          [plan.name, plan.monthlyPrice, plan.aiCreditsMonthly, plan.overagePrice, plan.id],
          function (updateErr) {
      ...
      (err, rows) => {
        if (err) {
          ...
          rows.forEach((row) => {
            ...
            console.log(
              `    Monthly: €${row.price_monthly} | AI Credits: ${row.token_limit}/month`
            );
          });
        }
        resolve(rows);
...
export { seedPricingPlans, loadPricingMetadata, verifyPlans, seedTrialConfig };
