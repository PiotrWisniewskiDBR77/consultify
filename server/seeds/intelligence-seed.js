...
const SAMPLE_INSIGHTS = [
  {
    ...
    content: JSON.stringify({
      description:
        'Primary goal is to increase operational efficiency by 30% through process automation and digital transformation initiatives.',
  ...
  },
  {
    ...
    content: JSON.stringify({
      description:
        'Eliminate redundant manual data entry processes by implementing automated data capture and integration systems.',
  ...
  },
  {
    ...
    content: JSON.stringify({
      description:
        'Integration with existing legacy ERP system may be more complex than initially estimated due to outdated APIs.',
      probability: 'medium',
      impact: 'high',
      mitigation:
        'Engage integration specialist early in the project, conduct thorough API assessment in Phase 1',
    }),
    confidence: 'medium',
    status: 'confirmed',
  ...
  },
  {
    ...
    content: JSON.stringify({
      description:
        'Key team members may have limited availability during Q4 due to year-end activities.',
      probability: 'high',
      impact: 'medium',
      mitigation:
        'Plan critical milestones outside Q4, secure commitment from management for dedicated resources',
    }),
    confidence: 'high',
    status: 'confirmed',
    pmo_domain: 'RISK_ISSUE_MANAGEMENT',
  },
  {
    ...
    content: JSON.stringify({
      description:
        'We assume the full project budget will be approved by the board before end of Q1.',
      validatedBy: 'CFO',
      validationDate: null,
    }),
    confidence: 'medium',
  ...
  },
  {
    ...
    content: JSON.stringify({
      description:
        'IT department will have the cloud infrastructure ready for deployment by project start.',
      validatedBy: 'CTO',
      validationDate: null,
    }),
    confidence: 'medium',
  ...
  },
  {
    ...
    content: JSON.stringify({
      description:
        'System must be operational by December 2025 to align with fiscal year reporting requirements.',
      type: 'time',
  ...
  },
  {
    ...
    content: JSON.stringify({
      description:
        'At least 80% of target users actively using the system within 3 months of launch.',
...
const SAMPLE_MESSAGES = [
  {
    role: 'ai',
    content:
      'Witaj! Jestem Twoim asystentem Project Intelligence. Pomogę Ci zebrać i zorganizować wiedzę o projekcie. Zacznijmy od kontekstu - opowiedz mi o tym projekcie.',
  },
  {
    role: 'user',
    content:
      'Projekt dotyczy transformacji cyfrowej naszego działu operacyjnego. Chcemy zwiększyć efektywność o 30% poprzez automatyzację procesów.',
  },
  {
    role: 'ai',
    content:
      'Rozumiem - transformacja cyfrowa z ambitnym celem 30% wzrostu efektywności. To solidny cel. Wykryłem to jako główny cel projektu. Kto jest sponsorem tego projektu i kto będzie odpowiedzialny za realizację?',
  },
  {
    role: 'user',
    content: 'Sponsorem jest John Smith, nasz CFO. Project Managerem będzie Anna Kowalska z PMO.',
  },
  {
    role: 'ai',
    content:
      'Świetnie, mamy kluczowych interesariuszy. Dodałem Johna Smitha jako Sponsora Wykonawczego i Annę Kowalską jako Project Managera. Jakie widzisz główne ryzyka dla tego projektu?',
  },
  {
    role: 'user',
    content:
      'Największym ryzykiem jest integracja ze starym systemem ERP - może być bardziej skomplikowana niż zakładamy. Poza tym, w Q4 zespół może być mniej dostępny przez zamknięcie roku.',
  },
];
  ...
  for (let i = 0; i < SAMPLE_MESSAGES.length; i++) {
    ...
    await db.runAsync(
      `
            ...
            VALUES (?, ?, ?, ?, ?)
        `,
      [msgId, session.id, msg.role, msg.content, timestamp]
    );
  }
...
export { seedIntelligenceData, SAMPLE_INSIGHTS, SAMPLE_MESSAGES };
