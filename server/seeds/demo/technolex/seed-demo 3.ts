  ...
  private async seedTranslations(): Promise<void> {
    ...
    for (const locale of locales) {
      ...
      if (translations.translations?.initiatives) {
        for (const [initRef, trans] of Object.entries(
          translations.translations.initiatives
        ) as any) {
...
async function main() {
  ...
  console.log(
    `  • Tasks: ${initiatives.initiatives.reduce((sum: number, i: any) => sum + (i.tasks?.length || 0), 0)}`
  );
