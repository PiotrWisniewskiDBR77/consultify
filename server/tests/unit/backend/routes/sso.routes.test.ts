...
describe('SSO routes', () => {
    ...
    it('POST /sso/superadmin/google/config upserts and returns success', async () => {
        ...
        const res = await request(app).post('/sso/superadmin/google/config').send({
            organizationId: 'org-1',
    ...
    it('PUT /sso/superadmin/config/:id/toggle updates status', async () => {
        mockDb.run.mockResolvedValue({});
        const res = await request(app).put('/sso/superadmin/config/test-id/toggle').send({ isActive: false });
        expect(res.status).toBe(200);
        expect(res.body.success).toBe(true);
    ...
    it('DELETE /sso/superadmin/config/:id removes config', async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.success).toBe(true);
        expect(mockDb.run).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM sso_configs'), ['test-id']);
    });
});
/**
