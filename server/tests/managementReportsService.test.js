...
describe('ManagementReportsService', () => {
  ...
  describe('generateTeamMeetingReport', () => {
    it('should generate a team meeting report for a project', async () => {
      ...
      db.all.mockImplementation((sql, params, callback) => {
        if (sql.includes('FROM tasks')) {
          ...
          callback(null, [{ id: 'r1', status: 'OPEN', severity: 'HIGH' }]);
        } else {
          callback(null, []);
        }
      });
    ...
    it('should handle AI service failure gracefully', async () => {
      AIExecutiveReporting.generateNarrative.mockRejectedValueOnce(
    ...
    it('should generate a steering committee report for portfolio', async () => {
      ...
      db.all.mockImplementation((sql, params, callback) => {
        if (sql.includes('FROM projects')) {
          ...
          callback(null, [{ id: 'd1', status: 'PENDING', priority: 'HIGH' }]);
        } else if (sql.includes('FROM risks')) {
          callback(null, [{ id: 'r1', status: 'OPEN', severity: 'HIGH' }]);
        } else {
          callback(null, []);
  ...
  describe('finalizeReport', () => {
    ...
    it('should throw error if report requires approval but not approved', async () => {
      ...
      await expect(ManagementReportsService.finalizeReport('report1', 'user1')).rejects.toThrow(
        'Report must be approved before finalization'
      );
    });
  });
  ...
  describe('unlockReport', () => {
    it('should unlock a finalized report with reason', async () => {
      ...
      const result = await ManagementReportsService.unlockReport(
        'report1',
        'admin1',
        'Correction needed'
      );
