...
describe('ReportVersionService', () => {
  ...
  describe('compareVersions', () => {
    ...
    it('should throw error if versions are same', async () => {
      await expect(ReportVersionService.compareVersions('report1', 1, 1)).rejects.toThrow(
        'Cannot compare same versions'
      );
    });
  });
  ...
  describe('restoreVersion', () => {
    it('should create new draft from old version', async () => {
      ...
      db.get.mockImplementation((sql, params, callback) => {
        if (
          sql.includes('FROM management_report_versions WHERE report_id') &&
          params &&
          params[1] === 1
        ) {
          callback(null, {
    ...
    it('should throw error if report is finalized', async () => {
      ...
      await expect(ReportVersionService.restoreVersion('report1', 1, 'user1')).rejects.toThrow(
        'Cannot restore version of finalized report'
      );
    });
  });
