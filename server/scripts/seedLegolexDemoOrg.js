...
async function seedSettlements() {
  ...
  for (const attr of attributions) {
    ...
    console.log(
      `   ✓ Settlement Entry: $${attr.revenue_amount} → $${(attr.revenue_amount * 0.15).toFixed(2)} to LexPartners`
    );
  }
}
...
async function seedHelpEvents() {
  ...
  for (const scenario of scenarios) {
    ...
    console.log(
      `   ✓ Help Event: ${scenario.user.split('-').pop()} -> ${scenario.pb} (${scenario.status})`
    );
  }
}
...
async function seedProjectAndData() {
  ...
  const initiatives = [
    {
      id: IDS.INIT_001,
      proj: IDS.PROJ_CORE,
      name: 'AI Contract Automation',
      axis: 'technology',
      status: 'EXECUTING',
      value: 'High',
      owner: IDS.USER_PIOTR,
    },
    {
      id: IDS.INIT_002,
      proj: IDS.PROJ_CORE,
  ...
  for (let idx = 0; idx < initiatives.length; idx++) {
    ...
    for (let i = 0; i < numTasks; i++) {
      ...
      const userId =
        idx === 8 && i % 2 === 0 ? IDS.USER_JULIA : i % 2 === 0 ? init.owner : IDS.USER_PIOTR;
...
async function verify() {
  ...
  checks.push({
    ...
    passed: !!partner,
    details: partner
      ? `${partner.name} (${partner.partner_type}, ${partner.default_revenue_share_percent}%)`
      : 'NOT FOUND',
  });

  // Promo Codes
  const promoCodes = await dbAll(`SELECT code FROM promo_codes WHERE id IN(?, ?)`, [
    IDS.PROMO_DEMO,
    IDS.PROMO_PARTNER,
  ]);
  checks.push({
  ...
  // Attribution Events
  const attributions = await dbAll(
    `SELECT source_type FROM attribution_events WHERE organization_id = ? `,
    [IDS.ORG]
  );
  checks.push({
  ...
  const orgEvents = await dbAll(
    `SELECT event_type FROM organization_events WHERE organization_id = ? `,
    [IDS.ORG]
  );
  checks.push({
  ...
  // Settlement Period
  const settlement = await dbGet(`SELECT * FROM settlement_periods WHERE id = ? `, [
    IDS.SETTLEMENT_PERIOD,
  ]);
  checks.push({
    ...
    details: settlement
      ? `${settlement.status} (Revenue: $${settlement.total_revenue}, Payout: $${settlement.total_settlements})`
      : 'NOT FOUND',
  });

  // Partner Settlements
  const settlements = await dbAll(
    `SELECT SUM(settlement_amount) as total FROM partner_settlements WHERE settlement_period_id = ? `,
    [IDS.SETTLEMENT_PERIOD]
  );
  checks.push({
  ...
  if (helpTableExists) {
    const helpEvents = await dbAll(
      `SELECT event_type, COUNT(*) as count FROM help_events WHERE organization_id = ? GROUP BY event_type`,
      [IDS.ORG]
    );
    checks.push({
      name: 'Help Events',
  ...
  if (metricsTableExists) {
    const metricEvents = await dbAll(
      `SELECT event_type, COUNT(*) as count FROM metrics_events WHERE organization_id = ? GROUP BY event_type`,
      [IDS.ORG]
    );
    checks.push({
      name: 'Metrics (Step 7)',
  ...
  for (const check of checks) {
    ...
    if (!check.passed && check.name !== 'Help Events' && check.name !== 'Metrics (Step 7)')
      allPassed = false;
  }
