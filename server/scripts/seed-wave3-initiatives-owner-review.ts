...
const USERS = Object.freeze([
  { id: IDS.owner, org: IDS.mainOrg, email: 'w3.initiatives.owner@local.test', role: 'OWNER', membership: 'ACTIVE', purpose: 'owner candidate acceptance and governed profile command', password: 'Wave3IniOwner!2026' },
  { id: IDS.admin, org: IDS.mainOrg, email: 'w3.initiatives.admin@local.test', role: 'ADMIN', membership: 'ACTIVE', purpose: 'allowed same-tenant admin alternate', password: 'Wave3IniAdmin!2026' },
  { id: IDS.projectActor, org: IDS.mainOrg, email: 'w3.initiatives.project.actor@local.test', role: 'MEMBER', membership: 'ACTIVE', purpose: 'system-portfolio PROJECT_MANAGER for capability-bound review', password: 'Wave3IniProject!2026' },
  { id: IDS.member, org: IDS.mainOrg, email: 'w3.initiatives.member@local.test', role: 'MEMBER', membership: 'ACTIVE', purpose: 'role-denied profile command', password: 'Wave3IniMember!2026' },
  { id: IDS.inactive, org: IDS.mainOrg, email: 'w3.initiatives.inactive@local.test', role: 'ADMIN', membership: 'REVOKED', purpose: 'inactive membership denial', password: 'Wave3IniInactive!2026' },
  { id: IDS.foreignOwner, org: IDS.foreignOrg, email: 'w3.initiatives.foreign@local.test', role: 'OWNER', membership: 'ACTIVE', purpose: 'foreign-tenant non-disclosure', password: 'Wave3IniForeign!2026' },
]);

function fail(message: string): never { throw new Error(`[W3 Initiatives fixture] BLOCKED: ${message}`); }
...
function context() {
  ...
  if (!databaseName.startsWith(DB_PREFIX) || !/^consultify_w3_initiatives_owner_[a-z0-9_]+$/.test(databaseName)) {
  ...
  if (COMMAND === 'seed') {
    ...
    if (!path.isAbsolute(MANIFEST_PATH) || MANIFEST_PATH.includes('://')) fail('INITIATIVES_OWNER_FIXTURE_MANIFEST must be an absolute local filesystem path');
...
function requireYes() { if (CONFIRM !== 'YES') fail('seed/reset requires INITIATIVES_OWNER_FIXTURE_CONFIRM=YES'); }
...
function manifest(databaseName: string, ownershipNonce: string, dynamic: Record<string, unknown> | null = null, readback: Record<string, unknown> | null = null) {
  return {
    ...
    boundaries: {
      candidateReplay: 'same initiative, no duplicate', profileReplay: 'same receipt',
      profileCollision: 'IDEMPOTENCY_PAYLOAD_COLLISION', staleProfile: 'PROFILE_VERSION_CONFLICT',
      foreignCandidate: 'not found / zero write', foreignProfile: 'INITIATIVE_NOT_FOUND',
      memberProfile: 'INITIATIVE_PROFILE_ROLE_REQUIRED', inactiveProfile: 'INITIATIVE_PROFILE_ROLE_REQUIRED',
      executionReplay: 'same link', executionCollision: 'execution_idempotency_payload_conflict',
      staleExecution: 'execution_runtime_initiative_stale_or_not_found', foreignExecution: 'execution_runtime_initiative_stale_or_not_found',
    },
    dynamic, readback,
  };
}
...
function persistManifest(manifestPath: string, payload: ReturnType<typeof manifest>) {
  ...
  try { fd = fs.openSync(manifestPath, 'wx', 0o600); fs.writeFileSync(fd, bytes, 'utf8'); }
...
async function seedBase(ownershipNonce: string) {
  ...
  try {
    ...
    for (const user of USERS) {
      ...
      await c.query(`INSERT INTO users(id,organization_id,email,password,first_name,last_name,role,status,language,timezone) VALUES($1,$2,$3,$4,'Initiatives','Fixture',$5,'active','pl','Europe/Warsaw')`, [user.id, user.org, user.email, hash, user.role]);
      await c.query(`INSERT INTO organization_members(id,organization_id,user_id,role,status) VALUES($1,$2,$3,$4,$5)`, [`membership-${user.id}`, user.org, user.id, user.role, user.membership]);
    }
    await c.query(
...
async function runCanonicalJourney() {
  Object.assign(process.env, { NODE_ENV: 'test', DB_TYPE: 'postgres', MOCK_DB: 'false', RUN_DB_TESTS: '1', POSTGRES_SKIP_INIT_IN_TEST: '1', DATABASE_URL: TARGET_URL, REQUIRE_INITIATIVE_PROJECT: 'true' });
  ...
  const accepted = await candidates.acceptCandidate(undefined, IDS.candidate, { orgId: IDS.mainOrg, userId: IDS.owner, fill: false });
  if (!accepted?.receiptPersisted || !accepted.initiativeId) fail(`candidate acceptance failed: ${JSON.stringify(accepted)}`);
  const initiativeId = accepted.initiativeId;
  const replay = await candidates.acceptCandidate(undefined, IDS.candidate, { orgId: IDS.mainOrg, userId: IDS.owner, fill: false });
  if (!replay?.receiptPersisted || replay.initiativeId !== initiativeId) fail('candidate replay did not return the durable initiative');
  const foreignCandidate = await candidates.acceptCandidate(undefined, IDS.candidate, { orgId: IDS.foreignOrg, userId: IDS.foreignOwner, fill: false });
  if (foreignCandidate !== null) fail('foreign candidate acceptance did not fail closed');

  const profileInput = { summary: 'Owner-reviewed plan: pilot na jednej linii, KPI czasu planowania i terminowości.', expectedVersion: 1, idempotencyKey: 'w3-ini-profile-v1' };
  const profile = await profiles.updateInitiativeProfile(IDS.mainOrg, initiativeId, IDS.owner, profileInput) as { version: number; idempotentReplay: boolean };
  if (profile.version !== 2 || profile.idempotentReplay) fail('canonical profile update failed');
  const profileReplay = await profiles.updateInitiativeProfile(IDS.mainOrg, initiativeId, IDS.owner, profileInput) as { version: number; idempotentReplay: boolean };
  ...
  const expectedErrors: Array<[() => Promise<unknown>, string]> = [
    [() => profiles.updateInitiativeProfile(IDS.mainOrg, initiativeId, IDS.owner, { ...profileInput, summary: 'Collision payload' }), 'IDEMPOTENCY_PAYLOAD_COLLISION'],
    [() => profiles.updateInitiativeProfile(IDS.mainOrg, initiativeId, IDS.owner, { summary: 'Stale payload', expectedVersion: 1, idempotencyKey: 'w3-ini-profile-stale-v1' }), 'PROFILE_VERSION_CONFLICT'],
    [() => profiles.updateInitiativeProfile(IDS.foreignOrg, initiativeId, IDS.foreignOwner, { summary: 'Foreign payload', expectedVersion: 2, idempotencyKey: 'w3-ini-profile-foreign-v1' }), 'INITIATIVE_NOT_FOUND'],
    [() => profiles.updateInitiativeProfile(IDS.mainOrg, initiativeId, IDS.member, { summary: 'Member payload', expectedVersion: 2, idempotencyKey: 'w3-ini-profile-member-v1' }), 'INITIATIVE_PROFILE_ROLE_REQUIRED'],
    [() => profiles.updateInitiativeProfile(IDS.mainOrg, initiativeId, IDS.inactive, { summary: 'Inactive payload', expectedVersion: 2, idempotencyKey: 'w3-ini-profile-inactive-v1' }), 'INITIATIVE_PROFILE_ROLE_REQUIRED'],
  ];
  for (const [operation, code] of expectedErrors) {
    try { await operation(); fail(`boundary ${code} unexpectedly succeeded`); }
  ...
  let projectId = '';
  try {
    projectId = String((await c.query(`SELECT project_id FROM initiatives WHERE id=$1 AND organization_id=$2`, [initiativeId, IDS.mainOrg])).rows[0]?.project_id || '');
  ...
  const linkInput = { organizationId: IDS.mainOrg, initiativeId, caseId: IDS.executionCase, sourceVersion: 1, actorId: IDS.owner, idempotencyKey: 'w3-ini-execution-link-v1' };
  ...
  const executionErrors: Array<[() => Promise<unknown>, string]> = [
    [() => execution.linkRuntimeInitiativeToExecutionCase({ ...linkInput, caseId: '05000000-0000-4000-8000-000000000099' }), 'execution_idempotency_payload_conflict'],
    [() => execution.linkRuntimeInitiativeToExecutionCase({ ...linkInput, sourceVersion: 2, idempotencyKey: 'w3-ini-execution-stale-v1' }), 'execution_runtime_initiative_stale_or_not_found'],
    [() => execution.linkRuntimeInitiativeToExecutionCase({ ...linkInput, organizationId: IDS.foreignOrg, actorId: IDS.foreignOwner, idempotencyKey: 'w3-ini-execution-foreign-v1' }), 'execution_runtime_initiative_stale_or_not_found'],
  ];
  for (const [operation, message] of executionErrors) {
    try { await operation(); fail(`boundary ${message} unexpectedly succeeded`); }
    catch (error) { if ((error as Error).message !== message) throw error; }
  }
...
async function readback(databaseName: string, dynamic: Record<string, unknown> | null = null) {
  const c = new pg.Client({ connectionString: TARGET_URL }); await c.connect();
  try {
    const initiativeId = String(dynamic?.initiativeId || (await c.query(`SELECT initiative_id FROM initiative_candidates WHERE id=$1`, [IDS.candidate])).rows[0]?.initiative_id || '');
...
async function seed(ctx: ReturnType<typeof context>) {
  requireYes(); const c = new pg.Client({ connectionString: ctx.admin.toString() }); await c.connect();
...
async function reset(ctx: ReturnType<typeof context>) {
  requireYes(); const c = new pg.Client({ connectionString: ctx.admin.toString() }); await c.connect();
  try { if (await databaseExists(c, ctx.databaseName)) await c.query(`DROP DATABASE "${ctx.databaseName}" WITH (FORCE)`); console.log(JSON.stringify({ fixture: 'W3-INITIATIVES-OWNER-v1', databaseName: ctx.databaseName, dropped: true, catalogAbsent: !(await databaseExists(c, ctx.databaseName)) }, null, 2)); } finally { await c.end(); }
}

const ctx = context();
if (COMMAND === 'seed') await seed(ctx); else if (COMMAND === 'readback') await readback(ctx.databaseName); else await reset(ctx);
