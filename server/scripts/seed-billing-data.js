...
async function seedBillingData() {
    ...
    try {
        ...
        // Find admin user
        const admin = await get(`SELECT id, email FROM users WHERE organization_id = ? AND role IN ('ADMIN', 'OWNER') LIMIT 1`, [orgId]);
        ...
        for (const inv of invoices) {
            await run(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            `, [
                inv.id, orgId, inv.stripeId, inv.status, inv.amountDue, inv.amountPaid,
        ...
        for (const alert of spendingAlerts) {
            await run(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `, [alert.id, orgId, alert.type, alert.threshold, alert.thresholdType, alert.action, JSON.stringify(alert.notifyEmails), alert.isActive]);
        }
