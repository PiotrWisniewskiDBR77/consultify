...
function assertManifestShape(manifest: OwnerBindingManifest): void {
  ...
  if (manifest.operation === "ROLLBACK")
    requiredString(manifest.applyRunId, "applyRunId");
  ...
  if (
    ...
    throw new Error(
      "issuedAt/expiresAt must describe a valid positive time window",
    );
  }
  if (Date.now() > expiresAt) throw new Error("Manifest has expired");
  if (issuedAt > Date.now() + 5 * 60_000)
    throw new Error("Manifest issuedAt is in the future");
  ...
  for (const mapping of manifest.mappings) {
    const partnerId = requiredString(
      mapping.partnerOrganizationId,
      "partnerOrganizationId",
    );
    const ownerId = requiredString(
      mapping.ownerOrganizationId,
      "ownerOrganizationId",
    );
    if (partnerIds.has(partnerId))
      throw new Error(`Duplicate Partner organization ${partnerId}`);
    if (ownerIds.has(ownerId))
      throw new Error(`Duplicate active owner tenant ${ownerId}`);
...
export function verifyManifest(
  ...
  if (manifest.signature.keyId !== key.keyId)
    throw new Error("Manifest signing key id mismatch");
  if (key.secret.length < 32)
    throw new Error("Binding signing key must be at least 32 characters");
  ...
  const mappings = [...manifest.mappings]
    ...
    .sort((left, right) =>
      left.partnerOrganizationId.localeCompare(right.partnerOrganizationId),
    );
...
async function assertActor(
  ...
  if (actor.rowCount !== 1)
    throw new Error(`Actor ${actorUserId} does not exist`);
...
async function lockAndValidateMappings(
  ...
  const byId = new Map(
    partners.rows.map((row) => [row.id, row.owner_organization_id]),
  );
  ...
  if (operation === "APPLY") {
    ...
    if (occupied.rowCount) {
      throw new Error(
        `Owner tenant collision: ${occupied.rows[0].owner_organization_id}`,
      );
    }
  }
  for (const mapping of mappings) {
    ...
    if (operation === "APPLY" && current !== null) {
      throw new Error(
        `Partner ${mapping.partnerOrganizationId} is already bound`,
      );
    }
...
export async function executeManifest(params: {
  ...
  if (!params.write) {
    ...
    await lockAndValidateMappings(
      params.pool,
      params.manifest.operation,
      verified.mappings,
    );
    if (params.manifest.operation === "ROLLBACK") {
      const applyReceipt = await readReceipt(
        params.pool,
        params.manifest.applyRunId!,
      );
      if (!applyReceipt || applyReceipt.operation !== "APPLY")
        throw new Error("Apply receipt not found");
      if (
        canonicalJson(applyReceipt.mappings_json) !==
        canonicalJson(verified.mappings)
      ) {
  ...
  try {
    ...
    for (const mapping of verified.mappings) {
      ...
      if (result.rowCount !== 1)
        throw new Error("Binding state changed during transaction");
      changed += 1;
      if (params.injectFailureAfter === changed)
        throw new Error("Injected owner-binding failure");
    }
    await client.query(
...
async function main(): Promise<void> {
  ...
  const manifestPath = path.resolve(
    requiredString(args.manifest, "--manifest"),
  );
  ...
  const expectedDatabase = requiredString(
    args["expect-database"],
    "--expect-database",
  );
  const actualDatabase = target.pathname.replace(/^\/+/, "");
  if (target.hostname !== expectedHost || actualDatabase !== expectedDatabase) {
    throw new Error(
      "Database target does not match the explicitly approved host/database",
    );
  }
  const manifest = JSON.parse(
    fs.readFileSync(manifestPath, "utf8"),
  ) as OwnerBindingManifest;
  ...
  if (args.apply && args.rollback)
    throw new Error("Choose only one of --apply or --rollback");
  const key = {
  ...
  try {
    const live = await pool.query<{ database: string }>(
      "SELECT current_database() database",
    );
    if (live.rows[0].database !== expectedDatabase)
    ...
    console.log(
      `${write ? manifest.operation : "DRY_RUN"} ${result.replay ? "REPLAY" : "OK"}`,
    );
