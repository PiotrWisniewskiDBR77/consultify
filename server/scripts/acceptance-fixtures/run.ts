...
async function main() {
  ...
  try {
    ...
    const identity = await client.query(`SELECT o.organization_type, u.organization_id, lower(u.email) email, u.status FROM organizations o JOIN users u ON u.id=$2 WHERE o.id=$1`, [organizationId,userId]);
    const actor = identity.rows[0];
    if (!actor || actor.organization_id !== organizationId || actor.organization_type !== 'DEMO' || actor.status !== 'active') throw new Error('Target must be an active user in an organization marked DEMO');
