...
export function buildFixturePlan(ctx: FixtureContext): FixtureStatement[] {
  ...
  const presentationSlides = [
    { intent:'cover', key_message:'Decyzja o przyspieszeniu realizacji korzyści', content:{ title:'Realizacja korzyści transformacji', subtitle:'Materiał dla komitetu sterującego' } },
    { intent:'performance_overview', key_message:'Zweryfikowana realizacja wynosi 76% wobec celu 90%', content:{ kpis:[{label:'Realizacja',value:'76%'},{label:'Cel',value:'90%'},{label:'Jakość danych',value:'Zweryfikowana'}], context:'Największa luka dotyczy adopcji nowego sposobu pracy.' } },
    { intent:'executive_summary', key_message:'Program ma dodatnią wartość i wymaga kontrolowanego planu naprawczego', content:{ key_findings:['NPV: 612 tys. PLN','IRR: 31,4%','Okres zwrotu: 7,8 miesiąca'], recommendation:'Kontynuować program z miesięcznym przeglądem KPI i niezależnym potwierdzeniem finansowym.' } },
    { intent:'roadmap', key_message:'Trzy kroki prowadzą od zweryfikowanej bazy do utrwalonego rezultatu', content:{ phases:[{title:'Zweryfikować bazę',timing:'Sierpień',owner:'Piotr Wiśniewski'},{title:'Zamknąć lukę realizacji',timing:'Wrzesień',owner:'Właściciele inicjatyw'},{title:'Potwierdzić korzyści',timing:'Październik',owner:'Finance + Results'}] } },
    { intent:'risk_management', key_message:'Ryzyko adopcji jest jawne i ma przypisaną kontrolę', content:{ risks:[{risk:'Adopcja poniżej planu',likelihood:'Średnie',impact:'Wysoki',mitigation:'Miesięczny pomiar, właściciel i plan naprawczy'},{risk:'Niespójne dane korzyści',likelihood:'Niskie',impact:'Wysoki',mitigation:'Niezależne uzgodnienie Finance i KPI'}] } },
    { intent:'next_steps', key_message:'Komitet powinien zatwierdzić kontrolowany plan realizacji', content:{ actions:[{action:'Zatwierdzić plan naprawczy',owner:'Komitet sterujący',deadline:'14 sierpnia 2026'},{action:'Uruchomić miesięczny przegląd',owner:'Piotr Wiśniewski',deadline:'31 sierpnia 2026'}], closing_message:'Decyzja uruchamia realizację; nie wysyła komunikacji zewnętrznej.' } },
  ];
  ...
  return [
    row('case', `INSERT INTO projects (id, organization_id, name, status) VALUES ($1,$2,$3,'active') ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name`, [project,ctx.organizationId,'Program poprawy realizacji korzyści'], 'projects','id',project,ctx.organizationId),
    ...
    row('artifact', `INSERT INTO presentation_decks (id,organization_id,project_id,title,description,status,generated_by,slide_count,outline_json,unified_json) VALUES ($1,$2,$3,$4,$5,'ready',$6,6,$7,$8) ON CONFLICT (id) DO UPDATE SET title=EXCLUDED.title,status='ready',slide_count=6,outline_json=EXCLUDED.outline_json,unified_json=EXCLUDED.unified_json`, [deck,ctx.organizationId,project,'Przegląd korzyści transformacji','Materiał decyzyjny dla komitetu sterującego.',ctx.userId,JSON.stringify(presentationSlides.map(slide=>({title:slide.key_message}))),JSON.stringify({meta:{project:'Program poprawy realizacji korzyści',client:'Komitet sterujący',template:'corporate',language:'pl',confidentiality:'internal'},slides:presentationSlides,marker:ACCEPTANCE_NAMESPACE})], 'presentation_decks','id',deck,ctx.organizationId),
