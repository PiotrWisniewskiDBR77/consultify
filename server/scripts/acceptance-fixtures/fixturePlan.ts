...
export function buildFixturePlan(ctx: FixtureContext): FixtureStatement[] {
  ...
  return [
    ...
    // DEC-424: 'EXECUTING' był poprawny przed migracją P12 (20262103_p12_initiative_status_slownik.sql);
    // initiatives_status_check_p12 dopuszcza dziś wyłącznie 7 kodów z server/src/constants/initiativeStatuses.ts.
    row('shared', `INSERT INTO initiatives (id,organization_id,name,status) VALUES ($1,$2,$3,'IN_EXECUTION') ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name`, [initiative,ctx.organizationId,'Poprawa realizacji korzyści programu'], 'initiatives','id',initiative,ctx.organizationId),
