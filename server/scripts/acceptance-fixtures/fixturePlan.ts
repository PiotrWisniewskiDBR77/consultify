...
export function buildFixturePlan(ctx: FixtureContext): FixtureStatement[] {
  ...
  return [
    ...
    row('artifact', `INSERT INTO presentation_decks (id,organization_id,project_id,title,description,status,generated_by,slide_count,outline_json,unified_json) VALUES ($1,$2,$3,$4,$5,'ready',$6,6,$7,$8) ON CONFLICT (id) DO UPDATE SET title=EXCLUDED.title,status='ready',slide_count=6,outline_json=EXCLUDED.outline_json,unified_json=EXCLUDED.unified_json`, [deck,ctx.organizationId,project,'Przegląd korzyści transformacji','Materiał decyzyjny dla komitetu sterującego.',ctx.userId,JSON.stringify([{title:'Executive decision'},{title:'Baseline'},{title:'Value case'},{title:'Execution plan'},{title:'Risks and controls'},{title:'Decision requested'}]),JSON.stringify({marker:ACCEPTANCE_NAMESPACE,slides:[{title:'Transformation benefits — owner review',subtitle:'Decision briefing'},{title:'Verified baseline',bullets:['Current realization: 76%','Target: 90%','Data quality: verified']},{title:'Value case',bullets:['PLN 900k benefit','PLN 500k investment','Controlled monthly evidence']},{title:'Execution plan',bullets:['Validate baseline','Close benefit gap','Review monthly']},{title:'Risks and controls',bullets:['Adoption risk','Named owner','Dated recovery plan']},{title:'Decision requested',bullets:['Approve controlled recovery plan']}]} )], 'presentation_decks','id',deck,ctx.organizationId),
