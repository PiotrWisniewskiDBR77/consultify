...
async function seedDBR77Data() {
  ...
  try {
    ...
    let org = await get('SELECT * FROM organizations WHERE id = ? OR name LIKE ? LIMIT 1', [
      'org-dbr77-test',
      '%DBR77%',
    ]);
    if (!org) {
    ...
    const securityEvents = [
      {
        ...
        severity: 'medium',
        userIdx: 0,
        details: 'PII (email address) detected and automatically redacted in AI request',
        hoursAgo: 2,
        resolved: 1,
      },
      {
        ...
        severity: 'low',
        userIdx: 1,
        details: 'User exceeded daily AI request limit (100/100)',
        hoursAgo: 5,
        resolved: 1,
      },
      {
        ...
        severity: 'low',
        userIdx: 2,
        details: 'Attempted access to premium AI model without proper tier',
        hoursAgo: 12,
        resolved: 1,
      },
      {
        ...
        severity: 'medium',
        userIdx: 0,
        details: 'Phone number detected and masked in conversation context',
        hoursAgo: 24,
        resolved: 1,
      },
      {
        ...
        severity: 'medium',
        userIdx: null,
        details: 'Organization AI budget reached 95% threshold',
        hoursAgo: 48,
        resolved: 0,
      },
      {
        ...
        severity: 'high',
        userIdx: 1,
        details: 'Potential prompt injection attempt detected and blocked',
        hoursAgo: 72,
        resolved: 0,
      },
    ];
    ...
    for (const event of securityEvents) {
      const userId =
        event.userIdx !== null && userIds[event.userIdx] ? userIds[event.userIdx] : null;
      await run(
        `
    ...
    const usageData = [
      { userIdx: 0, requests: 456, tokens: 185000, cost: 18.5, tier: 'PREMIUM' },
      { userIdx: 1, requests: 234, tokens: 89000, cost: 8.5, tier: 'STANDARD' },
      { userIdx: 2, requests: 89, tokens: 32000, cost: 2.1, tier: 'BUDGET' },
    ];
    ...
    for (let i = 0; i < projects.length; i++) {
      const projectUsage = [
        { requests: 234, tokens: 89000, cost: 8.5 },
        { requests: 156, tokens: 58000, cost: 5.4 },
        { requests: 89, tokens: 32000, cost: 2.9 },
      ][i] || { requests: 50, tokens: 20000, cost: 1.5 };
    ...
    const auditTableExists = await get(
    ...
    if (auditTableExists) {
      const auditEvents = [
        ...
        { type: 'PROJECT_CREATE', description: 'Created project: AI Integration Initiative' },
        {
          ...
          description: 'Updated AI settings: increased daily token limit to 50,000',
        },
        {
          type: 'USER_ROLE_CHANGE',
          description: 'Changed role for team member from USER to PROJECT_MANAGER',
        },
        {
          type: 'TASK_COMPLETE',
          description: 'Completed task: Database schema migration for Q1 release',
        },
        { type: 'INVITATION_SEND', description: 'Sent invitation to external consultant' },
        {
          type: 'REPORT_GENERATE',
          description: 'Generated monthly progress report for stakeholders',
        },
        {
          type: 'AI_MODEL_CHANGE',
          description: 'Changed default AI model from GPT-4o-mini to GPT-4o',
        },
        {
          type: 'SECURITY_SETTING_CHANGE',
          description: 'Enabled 2FA requirement for all admin users',
        },
    ...
    const loginHistoryExists = await get(
    ...
    const sessionsTableExists = await get(
    ...
    if (sessionsTableExists) {
      ...
      await run(
        'DELETE FROM user_sessions WHERE user_id IN (SELECT id FROM users WHERE organization_id = ?)',
        [orgId]
      );
    ...
    // 9. Create invoices
    console.log('\n🧾 Creating invoices...');
    const invoicesTableExists = await get(
    ...
    if (invoicesTableExists) {
      ...
      const invoices = [
        { amount: 299.0, status: 'paid', monthsAgo: 0 },
        { amount: 299.0, status: 'paid', monthsAgo: 1 },
        { amount: 299.0, status: 'paid', monthsAgo: 2 },
        { amount: 299.0, status: 'paid', monthsAgo: 3 },
      ];
      ...
      for (let i = 0; i < invoices.length; i++) {
        ...
        await run(
          ...
          [
            ...
            inv.amount,
            inv.status,
            `https://pay.stripe.com/invoice/INV-2026-00${i + 1}`,
          ]
        );
      }
    ...
    const complianceReports = [
      ...
      {
        name: 'Performance Monitoring Assessment',
        standard: 'PMBOK7',
        status: 'compliant',
        findings: 0,
      },
    ...
    // Find owner user
    const ownerUser =
      existingUsers.find((u) => u.role === 'ADMIN' || u.is_owner === 1) || existingUsers[0];
    ...
    const existingMethod = await get('SELECT * FROM payment_methods WHERE organization_id = ?', [
      orgId,
    ]);
    if (!existingMethod) {
      await run(
    ...
    const existingGroups = await all('SELECT * FROM user_groups WHERE organization_id = ?', [
      orgId,
    ]);
      ...
      const groups = [
        {
          ...
          description: 'Software developers and architects',
          color: '#3B82F6',
          members: 4,
        },
        {
        ...
        {
          name: 'Leadership',
          description: 'C-level and department heads',
          color: '#8B5CF6',
          members: 2,
        },
      ];
    ...
    const templates = [
      {
        name: 'DBR77 AI Governance Framework',
        description: 'Organization-specific AI governance based on ISO 21500',
        basedOn: 'ISO21500',
        sections: 4,
        checkpoints: 12,
      },
      {
        ...
        description: 'Extended GDPR framework with AI-specific controls',
        basedOn: 'GDPR',
        sections: 5,
        checkpoints: 18,
      },
