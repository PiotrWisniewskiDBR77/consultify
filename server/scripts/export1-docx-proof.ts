...
import { applyDocBaseThemeContract } from '../src/services/export/docx/docBaseThemePostprocessor.js';
import { renderDocumentSchemaToDocxBuffer } from '../src/services/documentStudio/documentDocxRenderer.js';

const outputDir = path.resolve('docs/program/EXPORT_1_DOCX_20260916/dowody');
...
const buffer = await applyDocBaseThemeContract(await renderDocumentSchemaToDocxBuffer(schema));
const output = path.join(outputDir, 'northwind-client-final-report.docx');
await writeFile(output, buffer);
console.log(JSON.stringify({ output, bytes: buffer.length, sections: schema.sections.length }));
