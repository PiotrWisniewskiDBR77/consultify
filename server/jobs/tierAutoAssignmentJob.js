...
class TierAutoAssignmentJob {
     ...
     */
    async logTierChange({ userId, organizationId, previousTier, newTier, reason, tokensThisMonth, triggeredBy }) {
        return new Promise((resolve) => {
            db.run(
                `INSERT INTO tier_change_log 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
                [uuidv4(), userId, organizationId, previousTier, newTier, reason, tokensThisMonth, triggeredBy],
                (err) => {
                    if (err) {
    ...
    async sendTierChangeNotification(user, oldTier, newTier, changeType) {
        ...
        // For now, just log it
        console.log(`[TierAutoAssignment] ${changeType === 'promoted' ? '📈' : '📉'} User ${user.userName} (${user.userId}) ${changeType} from ${oldTier} to ${newTier}`);
    ...
    async runGlobal() {
        ...
        try {
            ...
            console.log(`[TierAutoAssignment] Job completed in ${duration}ms. ${summary.totalChanges} tier changes.`);
            return summary;
