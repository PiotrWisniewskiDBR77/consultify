...
const AIWatchdog = {
        ...
        for (const signal of analysis.signals) {
            if (signal.probability >= CONFIG.highAlertThreshold) {
                const alert = await AIWatchdog.createAlert({
                    ...
                    severity: signal.severity === 'critical' ? ALERT_SEVERITY.CRITICAL :
                              signal.severity === 'high' ? ALERT_SEVERITY.HIGH :
    ...
    checkMilestones: async (projectId) => {
        return new Promise((resolve) => {
            ...
            `, [projectId, now], async (err, milestones) => {
                ...
                for (const milestone of milestones) {
                    ...
                    if (!exists) {
                        const alert = await AIWatchdog.createAlert({
                            ...
                            severity: daysOverdue > 14 ? ALERT_SEVERITY.HIGH :
                                      daysOverdue > 7 ? ALERT_SEVERITY.MEDIUM :
                                      ALERT_SEVERITY.LOW,
    ...
    isDuplicateAlert: async (alertData) => {
        return new Promise((resolve) => {
            db.get(`
                ...
                AND signal_type = ?
                AND created_at > datetime('now', '-24 hours')
            `, [alertData.projectId, alertData.type, alertData.signalType || null], (err, row) => {
                resolve(!!row);
            });
        });
    },
    ...
    generateDailyBriefings: async () => {
        ...
        const alreadyGenerated = await new Promise((resolve) => {
            ...
            `, [ALERT_TYPES.DAILY_BRIEFING, todayStart], (err, row) => {
                resolve(!!row);
            });
        });
    ...
    generateOrgBriefing: async (organizationId) => {
        ...
        try {
            ...
            const result = await coordinator.processQuery(
                ...
                Key alerts: ${summary.recentAlerts?.slice(0, 3).map(a => a.title).join(', ') || 'None'}
    ...
    checkAndEscalate: async () => {
        ...
        return new Promise((resolve) => {
            db.all(`
                ...
                AND status = 'new'
                AND created_at < ?
            `, [escalationTime.toISOString()], async (err, alerts) => {
    ...
    notifyOrgAdmins: async (alert) => {
        return new Promise((resolve) => {
            ...
            `, [alert.organization_id], (err, admins) => {
    ...
    logRun: async (runId, results, durationMs) => {
        return new Promise((resolve) => {
            ...
            `, [runId, JSON.stringify(results), durationMs], resolve);
        });
    },
