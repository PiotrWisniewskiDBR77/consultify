-- Ported from: 900_prod_missing_tables_hotfix.sql
...
DO $$
BEGIN
  -- Some environments created webhook_deliveries without the success column.
  -- Guard index creation to keep this migration idempotent.
  IF EXISTS (
    SELECT 1
    ...
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_webhook_deliveries_success ON webhook_deliveries(success)';
  END IF;
END $$;
