...
-- Additive approach: creates a new parallel table with the extended CHECK constraint.
-- Application code should write to v8_calendar_items_extended_itemtype for new item types.
...
CREATE TABLE IF NOT EXISTS v8_calendar_items_extended_itemtype (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_v8_calendar_items_extended_source_ref
  ON v8_calendar_items_extended_itemtype(organization_id, source_id, source_object_ref);
