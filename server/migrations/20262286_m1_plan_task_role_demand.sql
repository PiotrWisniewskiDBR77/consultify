-- M1 Plan workload: backfill plan roleDemand from real task estimates.
-- Idempotent: recomputes only windows that do not already contain roleDemand.
...
WITH all_windows AS (
  SELECT
    ...
    COALESCE(jsonb_array_length(w.value->'roleDemand'), 0) AS role_demand_count
  ...
  SELECT
    ...
    jsonb_agg(
      CASE
        WHEN aw.role_demand_count = 0 AND rd.demand_json IS NOT NULL
          THEN jsonb_set(aw.window_json, '{roleDemand}', rd.demand_json, true)
        ELSE aw.window_json
      END
...
SELECT COUNT(*) AS plan_scenarios_with_task_role_demand FROM updated;
