...
const BaselineService = {
    ...
    captureBaseline: async (roadmapId, projectId, userId, rationale) => {
        ...
        const initiatives = await new Promise((resolve, reject) => {
            db.all(`SELECT initiative_id, planned_start_date, planned_end_date, sequence_position 
        ...
        const currentVersion = await new Promise((resolve, reject) => {
            db.get(`SELECT MAX(version) as maxVer FROM schedule_baselines WHERE roadmap_id = ?`,
                [roadmapId], (err, row) => {
                    if (err) reject(err);
        ...
        return new Promise((resolve, reject) => {
            ...
            db.run(sql, [id, roadmapId, projectId, currentVersion, JSON.stringify(snapshots), userId, rationale], function (err) {
                if (err) return reject(err);

                // Update roadmap baseline version
                db.run(`UPDATE roadmaps SET current_baseline_version = ?, last_baselined_at = CURRENT_TIMESTAMP, status = 'BASELINED' WHERE id = ?`,
    ...
    getBaseline: async (roadmapId, version) => {
        return new Promise((resolve, reject) => {
            ...
            db.get(sql, version ? [roadmapId, version] : [roadmapId], (err, row) => {
                if (err) return reject(err);
    ...
    getBaselineHistory: async (roadmapId) => {
        return new Promise((resolve, reject) => {
            db.all(`SELECT id, version, approved_by, approved_at, rationale FROM schedule_baselines 
    ...
    calculateVariance: async (roadmapId, baselineVersion = null) => {
        ...
        const actuals = await new Promise((resolve, reject) => {
            db.all(`SELECT ri.initiative_id, ri.actual_start_date, ri.actual_end_date, i.name
