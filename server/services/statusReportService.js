/**
 * Status Report Service - PMO Status Reporting
 * 
...
const StatusReportService = {
    /**
     * Generate a new status report
     */
    generateReport: async (orgId, initiativeId, periodType, userId, options = {}) => {
        const id = uuidv4();
        const now = new Date();

        // Calculate period dates
        const { periodStart, periodEnd, periodLabel } = StatusReportService.calculatePeriod(periodType, options.periodDate);
        ...
        const reportData = await StatusReportService.gatherReportData(initiativeId, orgId, periodStart, periodEnd);

        // Calculate section statuses
        const sections = StatusReportService.calculateSectionStatuses(reportData);

        // Calculate overall status
        const overallStatus = StatusReportService.calculateOverallStatus(sections);

        // Generate narrative content
        const narrative = await StatusReportService.generateNarrative(initiative, reportData, sections);
        ...
        `, [
            id, orgId, initiativeId, initiative.project_id,
            periodType, periodStart, periodEnd, periodLabel,
            overallStatus, reportData.trend || 'STABLE', JSON.stringify(sections),
            narrative.executiveSummary, 
            JSON.stringify(narrative.accomplishments),
            JSON.stringify(narrative.nextSteps),
            JSON.stringify(narrative.escalations),
            narrative.risksAndIssues, narrative.recommendations,
            reportData.progress, reportData.budgetConsumedPercent,
            reportData.tasksCompleted, reportData.tasksTotal,
            reportData.openRisks, reportData.openIssues, reportData.pendingDecisions,
            options.method || 'MANUAL', 'DRAFT', userId, nowStr, nowStr
        ]);
        ...
        for (const [sectionName, sectionData] of Object.entries(sections)) {
            ...
            `, [
                uuidv4(), id, sectionName, sectionData.status,
                sectionData.content,
                JSON.stringify(sectionData.highlights || []),
                JSON.stringify(sectionData.issues || []),
                nowStr
            ]);
        }
     ...
     * Calculate period dates
     */
    calculatePeriod: (periodType, referenceDate = new Date()) => {
        ...
        switch (periodType) {
            case 'WEEKLY':
                ...
                const weekNum = Math.ceil((periodStart.getDate() + new Date(periodStart.getFullYear(), 0, 1).getDay()) / 7);
     ...
     */
    gatherReportData: async (initiativeId, orgId, periodStart, periodEnd) => {
        // Initiative progress
        const initiative = await queryHelpers.queryOne(`
        ...
        // Task statistics
        const taskStats = await queryHelpers.queryOne(`
            SELECT 
                COUNT(*) as total,
        ...
        `, [initiativeId]) || { total: 0, completed: 0, in_progress: 0, blocked: 0 };

        // Tasks completed this period
        const periodTasks = await queryHelpers.queryOne(`
            SELECT COUNT(*) as completed_this_period
            FROM tasks 
            WHERE initiative_id = ? 
              AND status = 'DONE'
              AND updated_at >= ? AND updated_at <= ?
        `, [initiativeId, periodStart, periodEnd]) || { completed_this_period: 0 };

        // Budget data (if exists)
        let budgetData = { consumedPercent: 0, isOverBudget: false };
        try {
            ...
            // Budget service might not be available
        }
        ...
        `, [initiativeId]) || { open_risks: 0, open_issues: 0, high_priority: 0 };

        // Pending decisions
        const decisionStats = await queryHelpers.queryOne(`
            SELECT COUNT(*) as pending
            FROM decisions 
            WHERE related_object_id = ? AND status = 'PENDING'
        `, [initiativeId]) || { pending: 0 };

        // Calculate trend based on previous report
        const previousReport = await queryHelpers.queryOne(`
     ...
     * Calculate section statuses
     */
    calculateSectionStatuses: (data) => {
        const sections = {};

        // Schedule section
        const scheduleStatus = data.tasksBlocked > 0 ? 'RED' :
            data.tasksInProgress > data.tasksTotal * 0.5 && data.progress < 50 ? 'AMBER' : 'GREEN';
        sections[SECTION_NAMES.SCHEDULE] = {
            status: scheduleStatus,
            content: `${data.tasksCompleted}/${data.tasksTotal} tasks completed (${data.progress}% progress)`,
            highlights: data.tasksCompletedThisPeriod > 0 
                ? [`Completed ${data.tasksCompletedThisPeriod} tasks this period`] 
                : [],
            issues: data.tasksBlocked > 0 
                ? [`${data.tasksBlocked} tasks currently blocked`] 
                : []
        };

        // Budget section
        const budgetStatus = data.isOverBudget ? 'RED' :
            data.budgetConsumedPercent >= 90 ? 'AMBER' : 'GREEN';
        sections[SECTION_NAMES.BUDGET] = {
            status: budgetStatus,
            content: `${data.budgetConsumedPercent}% of budget consumed`,
            highlights: data.budgetConsumedPercent < 80 
                ? ['Budget on track'] 
                : [],
            issues: data.isOverBudget 
        ...
        // Risks section
        const risksStatus = data.highPriorityItems >= 3 ? 'RED' :
            data.openRisks >= 3 || data.highPriorityItems >= 1 ? 'AMBER' : 'GREEN';
        sections[SECTION_NAMES.RISKS] = {
            status: risksStatus,
            content: `${data.openRisks} open risks, ${data.openIssues} open issues`,
            highlights: data.openRisks === 0 
                ? ['No open risks'] 
                : [],
            issues: data.highPriorityItems > 0 
     ...
     * Generate narrative content
     */
    generateNarrative: async (initiative, data, sections) => {
        ...
        // Executive summary
        const statusWord = sections.SCHEDULE?.status === 'GREEN' ? 'on track' :
            sections.SCHEDULE?.status === 'AMBER' ? 'at risk' : 'off track';
        const executiveSummary = `${initiative.name} is currently ${statusWord} with ${data.progress}% completion. ` +
            `${data.tasksCompleted} of ${data.tasksTotal} tasks completed. ` +
            (data.openIssues > 0 ? `${data.openIssues} open issues require attention.` : 'No critical issues.');

        // Risks and issues summary
        const risksAndIssues = data.openRisks > 0 || data.openIssues > 0
        ...
        if (data.trend === 'DECLINING') {
            recommendations = 'Consider reviewing resource allocation and timeline. Progress trend is declining.';
        } else if (data.isOverBudget) {
            recommendations = 'Budget overrun detected. Recommend immediate cost review and approval for additional funds or scope adjustment.';
        } else if (data.highPriorityItems > 0) {
            recommendations = 'Focus on resolving high-priority risks and issues before proceeding with new work.';
        } else {
     ...
     */
    listReports: async (initiativeId, orgId, options = {}) => {
        ...
        `;
        const params = [initiativeId, orgId];
        ...
        const reports = await queryHelpers.queryAll(sql, params);
     ...
     * Update report
     */
    updateReport: async (reportId, updates, userId) => {
        const allowedFields = [
            'executive_summary', 'accomplishments', 'next_steps', 'escalations',
            'risks_and_issues', 'recommendations', 'overall_status', 'status'
        ];
     ...
     * Approve report
     */
    approveReport: async (reportId, userId) => {
     ...
     */
    createDistribution: async (reportId, recipientData) => {
        const id = uuidv4();
        const { recipientId, recipientEmail, recipientType, distributionMethod } = recipientData;

        await queryHelpers.queryRun(`
            INSERT INTO report_distributions (
                id, report_id, recipient_id, recipient_email, 
                recipient_type, distribution_method, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [
            id, reportId, recipientId, recipientEmail,
            recipientType || 'STAKEHOLDER', distributionMethod || 'EMAIL',
     ...
     */
    markDistributionSent: async (distributionId) => {
        await queryHelpers.queryRun(`
            UPDATE report_distributions SET sent_at = ? WHERE id = ?
