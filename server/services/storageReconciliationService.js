    ...
    runReconciliation: async () => {
        ...
        for (const orgDir of orgDirs) {
            ...
            for (const projDir of projectDirs) {
                if (projDir.name === '.trash')
                    continue; // Skip trash for now
        ...
        const id = uuidv4();
        db.run(`INSERT INTO storage_audit_logs (id, organization_id, action, files_scanned, discrepancies_found) VALUES (?, 'system', 'daily_reconciliation', ?, ?)`, [id, scandFiles, discrepancies]);
        console.log(`[Reconciliation] Completed. Discrepancies found: ${discrepancies}`);
    },
};
export default StorageReconciliationService;
//# sourceMappingURL=storageReconciliationService.js.map