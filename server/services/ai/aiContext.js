...
const INDUSTRY_PROFILES = {
    manufacturing: {
        ...
        keyTransformationAreas: ['Smart Factory', 'Predictive Maintenance', 'Supply Chain Visibility', 'Quality 4.0'],
        typicalChallenges: [
    ...
    retail: {
        ...
        keyTransformationAreas: ['Omnichannel', 'Personalization', 'Inventory Optimization', 'Last-mile Delivery'],
        typicalChallenges: [
    ...
    healthcare: {
        ...
        keyTransformationAreas: ['Telemedicine', 'EHR Integration', 'Clinical Decision Support', 'Patient Engagement'],
        typicalChallenges: [
    ...
    technology: {
        ...
        keyTransformationAreas: ['Cloud-Native', 'DevOps/MLOps', 'Product-Led Growth', 'AI Integration'],
        typicalChallenges: [
    ...
    services: {
        ...
        keyTransformationAreas: ['Client Experience', 'Knowledge Management', 'Automation', 'Remote Collaboration'],
        typicalChallenges: [
...
class ContextBuilder {
     ...
     */
    async buildReportContext(params) {
    ...
    // =========================================================================
    _buildCompanyProfile(organization, projectOrAssessment) {
    ...
    _inferIndustry(orgName, context) {
        ...
        if (name.includes('bank') || name.includes('finans') || name.includes('ubezp'))
            return 'financial';
        if (name.includes('tech') || name.includes('soft') || name.includes('it'))
            return 'technology';
        if (name.includes('szpital') || name.includes('med') || name.includes('health'))
            return 'healthcare';
        if (name.includes('logist') || name.includes('transport'))
            return 'logistics';
        if (name.includes('energ') || name.includes('power'))
            return 'energy';
    ...
    _determineCompanySize(employeeInfo) {
        if (!employeeInfo)
            return 'sme';
        const count = typeof employeeInfo === 'number' ? employeeInfo : parseInt(employeeInfo.replace(/[^\d]/g, ''), 10);
        if (isNaN(count)) {
            // Try string matching
            const str = String(employeeInfo).toLowerCase();
            if (str.includes('startup') || str.includes('mały'))
                return 'startup';
            if (str.includes('średni') || str.includes('sme'))
                return 'sme';
            if (str.includes('duży') || str.includes('enterprise'))
                return 'enterprise';
            return 'sme';
        }
        if (count <= 50)
    ...
    // =========================================================================
    _buildIndustryContext(industryKey) {
    ...
    // =========================================================================
    _buildRegulatoryContext(industryKey, location) {
    ...
    _generateComplianceRecommendation(industryKey, regulations) {
        const priorityRegulations = regulations.filter((r) => r.name === 'GDPR' || r.name === 'NIS2' || r.name === 'DORA');
    ...
    // =========================================================================
    _buildAssessmentContext(assessment) {
    ...
    _performGapAnalysis(axisData) {
        ...
        return {
            ...
            allGaps: gaps,
            estimatedTotalTransformationMonths: Math.max(...gaps.map((g) => g.estimatedMonths), 0) + Math.floor(gaps.length * 1.5), // Overlap factor
        };
    }
    _calculateGapPriority(actual, target) {
    ...
    _estimateTransformationTime(actual, target) {
        const gap = (target || 0) - (actual || 0);
        if (gap <= 0)
            return 0;
    ...
    _assessComplexity(axisId, actual, target) {
        const gap = (target || 0) - (actual || 0);
        const complexAxes = ['culture', 'businessModels', 'aiMaturity'];
        if (complexAxes.includes(axisId) && gap >= 2)
            return 'HIGH';
        if (gap >= 3)
            return 'HIGH';
        if (gap >= 2)
    ...
    _analyzeMaturityProfile(axisData, industryContext) {
        const parsed = this._parseJSON(axisData) || axisData || {};
        ...
        if (avgScore >= industryBenchmark + 1)
            positioning = 'LEADER';
        else if (avgScore >= industryBenchmark + 0.5)
            positioning = 'ABOVE_AVERAGE';
        else if (avgScore <= industryBenchmark - 1)
            positioning = 'LAGGARD';
        else if (avgScore <= industryBenchmark - 0.5)
    ...
    async _fetchUser(userId) {
        return new Promise((resolve) => {
            db.get('SELECT id, email, name, role FROM users WHERE id = ?', [userId], (err, row) => resolve(err ? null : row));
        });
    }
    async _fetchOrganization(orgId) {
        return new Promise((resolve) => {
            db.get('SELECT id, name, transformation_context FROM organizations WHERE id = ?', [orgId], (err, row) => resolve(err ? null : row));
        });
    }
    async _fetchProject(projectId) {
        return new Promise((resolve) => {
            db.get('SELECT id, name, status, context, phase FROM projects WHERE id = ?', [projectId], (err, row) => resolve(err ? null : row));
        });
    }
    async _fetchAssessment(assessmentId) {
        return new Promise((resolve) => {
            db.get(`SELECT a.*, o.name as org_name, o.transformation_context as org_context
                 FROM assessments a
                 LEFT JOIN organizations o ON a.organization_id = o.id
                 WHERE a.id = ?`, [assessmentId], (err, row) => resolve(err ? null : row));
        });
    }
    async _fetchFullAssessment(assessmentId) {
        ...
        return new Promise((resolve) => {
            ...
            db.get(`SELECT 
                 ...
                 WHERE m.id = ?`, [assessmentId], (err, row) => {
                if (err || !row) {
                    ...
                    db.get(`SELECT 
                             ...
                             LEFT JOIN organizations o ON p.organization_id = o.id
                             WHERE a.id = ?`, [assessmentId], (err2, row2) => {
                        if (err2 || !row2) {
                            resolve(null);
                        }
                        else {
