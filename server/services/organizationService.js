...
const OrganizationService = {
    ...
    createOrganization: async ({ userId, name, _email, attribution = null, }) => {
        ...
        return new Promise((resolve, reject) => {
            deps.db.serialize(() => {
                ...
                deps.db.run(`INSERT INTO organizations (
                     ...
                     VALUES (?, ?, 'active', 'TRIAL', 0, ?, ?, 1, 'MEDIUM', 'SUGGEST_ONLY', ?)`, [orgId, name, userId, now, attributionJson], function (err) {
                    if (err) {
                ...
                deps.db.run(`INSERT INTO organization_members (id, organization_id, user_id, role, status, created_at)
                     VALUES (?, ?, ?, ?, 'ACTIVE', ?)`, [deps.uuidv4(), orgId, userId, 'OWNER', now], function (err) {
                    if (err) {
                        deps.db.run('ROLLBACK');
                        return reject(err);
                    }
    ...
    getOrganization: async (orgId) => {
        await initDeps();
        return new Promise((resolve, reject) => {
            deps.db.get(`SELECT id, name, status, billing_status, token_balance, created_at 
                 FROM organizations WHERE id = ?`, [orgId], (err, row) => {
                if (err)
                    return reject(err);
                if (!row)
    ...
    addMember: async ({ organizationId, userId, role, invitedBy }) => {
        ...
        return new Promise((resolve, reject) => {
            deps.db.run(`INSERT INTO organization_members (id, organization_id, user_id, role, status, invited_by_user_id)
                 VALUES (?, ?, ?, ?, 'ACTIVE', ?)`, [id, organizationId, userId, role, invitedBy], function (err) {
                if (err) {
    ...
    getMembers: async (orgId) => {
        await initDeps();
        return new Promise((resolve, reject) => {
            deps.db.all(`SELECT m.id, m.user_id, m.role, m.status, m.created_at, u.first_name, u.last_name, u.email
                 FROM organization_members m
                 JOIN users u ON m.user_id = u.id
                 WHERE m.organization_id = ?`, [orgId], (err, rows) => {
                if (err)
                    return reject(err);
                resolve(rows || []);
            });
    ...
    getUserOrganizations: async (userId) => {
        await initDeps();
        return new Promise((resolve, reject) => {
            deps.db.all(`SELECT o.id, o.name, o.billing_status, m.role
                 FROM organizations o
                 JOIN organization_members m ON o.id = m.organization_id
                 WHERE m.user_id = ?`, [userId], (err, rows) => {
                if (err)
                    return reject(err);
                resolve(rows || []);
            });
    ...
    activateBilling: async (orgId) => {
        ...
        return new Promise((resolve, reject) => {
            deps.db.serialize(() => {
                ...
                deps.db.run(`UPDATE organizations 
                     SET billing_status = 'ACTIVE', 
                         organization_type = 'PAID',
                         status = 'active',
                         token_balance = IFNULL(token_balance, 0) + ? 
                     WHERE id = ?`, [INITIAL_TOKENS, orgId], function (err) {
                    if (err) {
                        deps.db.run('ROLLBACK');
                        return reject(err);
                    }
                ...
                deps.db.run(`INSERT OR REPLACE INTO organization_billing (organization_id, status, updated_at)
                     VALUES (?, 'ACTIVE', CURRENT_TIMESTAMP)`, [orgId], function (err) {
                    if (err) {
                        deps.db.run('ROLLBACK');
                        return reject(err);
                    }
    ...
    getAISettings: async (orgId) => {
        await initDeps();
        return new Promise((resolve, reject) => {
            deps.db.get(`SELECT ai_assertiveness_level, ai_autonomy_level 
                FROM organizations WHERE id = ?`, [orgId], (err, row) => {
                if (err)
                    return reject(err);
                resolve(row || {
    ...
    removeMember: async ({ organizationId, userId }) => {
        await initDeps();
        return new Promise((resolve, reject) => {
            deps.db.run(`DELETE FROM organization_members 
                 WHERE organization_id = ? AND user_id = ?`, [organizationId, userId], function (err) {
                if (err)
                    return reject(err);
                if (this.changes === 0) {
    ...
    updateMemberRole: async ({ organizationId, userId, role, }) => {
        ...
        return new Promise((resolve, reject) => {
            deps.db.run(`UPDATE organization_members 
                 SET role = ? 
                 WHERE organization_id = ? AND user_id = ?`, [role, organizationId, userId], function (err) {
                if (err)
                    return reject(err);
                if (this.changes === 0) {
                    return reject(new Error('Member not found'));
    ...
    getMemberRole: async (organizationId, userId) => {
        await initDeps();
        return new Promise((resolve, reject) => {
            deps.db.get(`SELECT role FROM organization_members 
                 WHERE organization_id = ? AND user_id = ?`, [organizationId, userId], (err, row) => {
                if (err)
                    return reject(err);
