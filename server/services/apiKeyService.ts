...
const ApiKeyService: ApiKeyServiceInterface = {
  ...
  async checkRateLimit(keyId: string, type: 'minute' | 'day' = 'minute'): Promise<RateLimitResult> {
    ...
    const key = (await dbGet(
      `SELECT rate_limit_per_minute, rate_limit_per_day FROM api_keys WHERE id = ?`,
      [keyId]
    )) as Record<string, unknown>;
    const limit =
      type === 'minute'
        ? (key.rate_limit_per_minute as number)
        : (key.rate_limit_per_day as number);
  ...
  async getKeyUsage(keyId: string, days: number = 30): Promise<UsageStatistics> {
    ...
    return {
      usage,
      totals: totals || { total_requests: 0, avg_response_time: 0, total_errors: 0 },
      endpoints,
    };
  },
  ...
  async updateKey(
    ...
    const key = (await dbGet(`SELECT organization_id FROM api_keys WHERE id = ?`, [keyId])) as {
      organization_id: string;
    } | null;
    AuditService.logSystemEvent(
      ...
      'api_key',
      keyId,
      (key?.organization_id as any) || null,
      {
  ...
  async revokeKey(
    ...
    const key = (await dbGet(`SELECT organization_id, name FROM api_keys WHERE id = ?`, [
      keyId,
    ])) as { organization_id: string; name: string } | null;
    if (!key) {
