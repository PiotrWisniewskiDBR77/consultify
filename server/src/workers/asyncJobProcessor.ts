...
const AsyncJobProcessor = {
   ...
   * @param {any} job - BullMQ job object
  ...
  processDecisionExecution: async (job: any) => {
    ...
    try {
      ...
      const execResult = await ActionExecutionAdapter.executeDecision(decisionId, 'SYSTEM', {
        dry_run: false,
      });
   ...
   * @param {any} job - BullMQ job object
   * @returns {Promise<any>} Advance result
   */
...
export default AsyncJobProcessor;
