...
describe.skipIf(!REAL_DB)(
  'Method Kernel — roles/approvals HTTP surface (S2) — real PostgreSQL',
  () => {
    let app: Express;
    ...
    beforeAll(async () => {
      ...
      await pool.query(
        ...
        [ORG, 'S2 roles/approvals HTTP test org']
      );
      await pool.query(
        `INSERT INTO organizations (id, name) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING`,
        [OTHER_ORG, 'S2 roles/approvals HTTP test org (other tenant)']
      );
      for (const [id, org] of [
        [OWNER, ORG],
        [APPROVER, ORG],
    ...
    async function driveToInReviewViaHttp(sessionId: string): Promise<void> {
      ...
      if (grant.status !== 201 && grant.status !== 200) {
        throw new Error(
          `driveToInReviewViaHttp: self-grant lead_assessor failed: ${grant.status} ${JSON.stringify(grant.body)}`
        );
      }
      for (const to of ['prepared', 'active', 'in_review']) {
        ...
        if (res.status !== 200) {
          throw new Error(
            `driveToInReviewViaHttp: transition to ${to} failed: ${res.status} ${JSON.stringify(res.body)}`
          );
        }
      }
    }
    ...
    it('P1. assigning a role over HTTP is visible in the roster and needs zero SQL', async () => {
      ...
      expect(list.body.roles).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ userId: RESPONDENT, role: 'respondent' }),
        ])
      );
    ...
    it('R1b. [DENY] the self-elevation guard cannot be bypassed by claiming a spoofed actor identity in the body', async () => {
      ...
      const res = await request(app)
        ...
        .send({
          userId: OWNER,
          role: 'approver',
          assignedBy: RESPONDENT,
          onBehalfOf: RESPONDENT,
          actorUserId: RESPONDENT,
        });
      expect(res.status).toBe(403);
      expect(res.body.error).toBe('self_elevation_forbidden');
    ...
    it('R2. [DENY] an actor without the approver role cannot approve/freeze a session in review', async () => {
      ...
      const approvals = await pool.query(`SELECT 1 FROM method_approvals WHERE session_id = $1`, [
        sessionId,
      ]);
    ...
    it('R5. revoking a role appends a "revoked" event; the "assigned" event stays exactly as it was', async () => {
      ...
      const revokedEvent = respondentHistory.find(
        (e: { action: string }) => e.action === 'revoked'
      );
    ...
    it('V1. an unknown role in the closed METHOD_PROCESS_ROLES set is rejected with 400', async () => {
      const sessionId = await createSession(ownerToken);
      const res = await assignRoleHttp(
        sessionId,
        ownerToken,
        RESPONDENT,
        'super_admin_definitely_not_a_role'
      );
      expect(res.status).toBe(400);
    });
    ...
    it('V3. no Authorization header on any new S2 route is refused with 401', async () => {
      ...
      const res2 = await request(app)
        .post(`/api/method/sessions/${sessionId}/roles`)
        .send({ userId: RESPONDENT, role: 'respondent' });
