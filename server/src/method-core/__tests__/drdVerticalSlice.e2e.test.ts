...
describe.skipIf(!REAL_DB)(
  'DRD vertical slice — pełna ścieżka E2E przez prawdziwy HTTP (real PostgreSQL, T3)',
  () => {
    ...
    beforeAll(async () => {
      ...
      await pool.query(
        ...
        [ORG, 'T3 DRD vertical slice org']
      );
      await pool.query(
        `INSERT INTO organizations (id, name) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING`,
        [OTHER_ORG, 'T3 DRD vertical slice org (other tenant)']
      );
      for (const [id, org] of [
        [OWNER, ORG],
        [APPROVER, ORG],
    ...
    afterAll(async () => {
      ...
      await pool.query(`DELETE FROM users WHERE id = ANY($1)`, [
        [OWNER, APPROVER, PARTICIPANT, OTHER_ORG_USER],
      ]);
    ...
    it('1. Library: GET /packs pokazuje zarejestrowaną, wydaną (released) paczkę metody', async () => {
      ...
      const pack = res.body.packs.find(
        (p: { packId: string; version: string }) =>
          p.packId === PACK_ID && p.version === PACK_VERSION
      );
    ...
    it('3. Przypisanie uczestników przez HTTP: lead_assessor (self), approver, evidence_owner; guardy S2 działają', async () => {
      ...
      const rosterPairs = roster.body.roles.map(
    ...
    it('5. Wpis ręczny (ANSWER_DRAFTED/REVISED/CONFIRMED) + dowód (EVIDENCE_ATTACHED) na dwóch osiach', async () => {
      ...
      const draft1A = await request(app)
        ...
        .send({
          ...
          payload: {
            questionId: 'q1A',
            answerText: 'Wersja robocza — poziom 2',
            answerState: 'draft',
          },
        });
      ...
      const draft1B = await request(app)
        ...
        .send({
          ...
          payload: {
            questionId: 'q1B',
            answerText: 'Wersja robocza — poziom 1',
            answerState: 'draft',
          },
        });
    ...
    it('6. Autosave-safe replay: ten sam Idempotency-Key dwa razy = JEDEN wpis, nie dwa', async () => {
      ...
      const events = await request(app)
        .get(`/api/method/sessions/${rootSessionId}/events`)
        .set('Authorization', `Bearer ${ownerToken}`);
      const matching = events.body.events.filter(
        (e: { id: string }) => e.id === first.body.event.id
      );
    ...
    it('13. Korekta na nowej rewizji: poprawiony wpis + nowy dowód na 1A, potem in_review', async () => {
      const revised = await request(app)
        ...
        .send({
          ...
          payload: {
            questionId: 'q1A',
            answerState: 'revised',
            reason: 'T3: doprecyzowano po send back',
          },
        });
      ...
      const evidence = await request(app)
        ...
        .send({
          ...
          unitId: '1A',
          payload: {
            evidenceId: `ev-1a-rev-${randomUUID()}`,
            evidenceType: 'document',
            strength: 'E3',
          },
        });
    ...
    it('14. Approval na nowej rewizji tworzy Output v2; v1 istnieje nadal, teraz superseded', async () => {
      ...
      const byId = new Map(
        fromV1.body.revisions.map((r: { id: string; status: string }) => [r.id, r.status])
      );
    ...
    it('15. Report: POST /outputs/:v2/report tworzy snapshot z realnej treści v2', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({
          title: 'T3 DRD Report v2',
          content: { executiveSummary: `Report for ${v2OutputId}` },
        });
      expect(res.status).toBe(201);
    ...
    it('16. Presentation: POST /outputs/:v2/presentation tworzy snapshot z tej SAMEJ treści v2', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({
          title: 'T3 DRD Presentation v2',
          content: { executiveSummary: `Presentation for ${v2OutputId}` },
        });
      expect(res.status).toBe(201);
    ...
    it('18. RESTART: druga instancja aplikacji odnajduje i otwiera KAŻDY artefakt wyłącznie po id', async () => {
      ...
      const dbV1 = await pool.query(`SELECT content_hash FROM method_outputs WHERE id = $1`, [
        v1OutputId,
      ]);
      const dbV2 = await pool.query(`SELECT content_hash FROM method_outputs WHERE id = $1`, [
        v2OutputId,
      ]);
      ...
      const presentationsList = await request(restartedApp)
        ...
        .set('Authorization', `Bearer ${ownerToken}`);
      expect(presentationsList.body.presentations.map((p: { id: string }) => p.id)).toContain(
        presentationId
      );
    ...
    it('19. Lineage (po restarcie): sesja -> rewizje -> Output -> Report/Presentation -> Initiative Proposal, kompletny łańcuch', async () => {
      ...
      expect(sessionIds).toEqual(
        fromRevision.body.sessions.map((s: { id: string }) => s.id).sort()
      );

      const outputIds = fromRoot.body.outputs
        .map((o: { output: { id: string } }) => o.output.id)
        .sort();
      expect(outputIds).toEqual([v1OutputId, v2OutputId].sort());

      const v1Node = fromRoot.body.outputs.find(
        (o: { output: { id: string } }) => o.output.id === v1OutputId
      );
      const v2Node = fromRoot.body.outputs.find(
        (o: { output: { id: string } }) => o.output.id === v2OutputId
      );
