...
describe.skipIf(!REAL_DB)(
  'Method Kernel — client/server contract parity (real HTTP, real PostgreSQL)',
  () => {
    let app: Express;
    ...
    beforeAll(async () => {
      ...
      await pool.query(
        ...
        [ORG, 'T2 client-contract test org']
      );
      await pool.query(
        `INSERT INTO organizations (id, name) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING`,
        [OTHER_ORG, 'T2 client-contract test org (other tenant)']
      );
      for (const [id, org] of [
        [OWNER, ORG],
        [APPROVER, ORG],
    ...
    async function addFinding(sessionId: string, unitId = '1A'): Promise<void> {
      ...
      if (res.status !== 201) {
        throw new Error(
          `addFinding: EVIDENCE_ATTACHED failed: ${res.status} ${JSON.stringify(res.body)}`
        );
      }
      await request(app)
        .post(`/api/method/sessions/${sessionId}/events`)
    ...
    async function postInitiativeDraft(
      output: Record<string, unknown>
    ): Promise<Record<string, unknown>> {
    ...
    it('9. listPresentations(): a posted Presentation parses correctly and is absent from listReports()', async () => {
      actAs(ownerToken);
      const { output } = await fullFreezeFlow();
      const presentation = await postArtefact(
        output.id as string,
        'presentation',
        'T2 presentation'
      );
    ...
    it('10. getPresentationSnapshot(id): content round-trips; getReportSnapshot on the same id refuses with 404 (kind mismatch, not a wrong row)', async () => {
      ...
      expect(detail!.content).toEqual({
        executiveSummary: `presentation for ${output.id as string}`,
      });
      ...
      await expect(getReportSnapshot(presentation.id as string)).rejects.toMatchObject({
        status: 404,
      });
    });
    ...
    it('17. an empty result set is [] with total 0, never an error, on every list() client function', async () => {
      ...
      const presentations = await listPresentations({
        sessionId: `no-such-session-${randomUUID()}`,
      });
