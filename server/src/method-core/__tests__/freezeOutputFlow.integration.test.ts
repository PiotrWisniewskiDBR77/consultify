...
describe.skipIf(!REAL_DB)(
  'P0B — freeze -> Output -> approval -> Report -> Initiative (real PostgreSQL)',
  () => {
    let app: Express;
    ...
    beforeAll(async () => {
      ...
      await pool.query(
        ...
        [ORG, 'P0B freeze/Output HTTP test org']
      );
      await pool.query(
        `INSERT INTO organizations (id, name) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING`,
        [OTHER_ORG, 'P0B freeze/Output HTTP test org (other tenant)']
      );
      for (const [id, org] of [
        [OWNER, ORG],
        [APPROVER, ORG],
    ...
    async function addEvidenceAndAnswer(sessionId: string, unitId = '1A'): Promise<void> {
      ...
      if (evidence.status !== 201) {
        throw new Error(
          `addEvidenceAndAnswer: EVIDENCE_ATTACHED failed: ${evidence.status} ${JSON.stringify(evidence.body)}`
        );
      }
      ...
      if (answer.status !== 201) {
        throw new Error(
          `addEvidenceAndAnswer: ANSWER_CONFIRMED failed: ${answer.status} ${JSON.stringify(answer.body)}`
        );
      }
    }
    ...
    async function createFrozenSessionWithOutput(): Promise<{ sessionId: string; output: any }> {
      ...
      if (freeze.status !== 200) {
        throw new Error(
          `createFrozenSessionWithOutput: freeze failed: ${freeze.status} ${JSON.stringify(freeze.body)}`
        );
      }
    ...
    it('1. freezing with a stale expectedVersion is refused 409, and no snapshot/Output is written', async () => {
      ...
      const before = await pool.query(
        `SELECT version, state, frozen_snapshot_id FROM method_sessions WHERE id = $1`,
        [sessionId]
      );
      ...
      const outputs = await pool.query(`SELECT id FROM method_outputs WHERE session_id = $1`, [
        sessionId,
      ]);
    ...
    it('2. retrying freeze with the SAME Idempotency-Key returns the identical session+Output, no second write', async () => {
      ...
      const snapshots = await pool.query(`SELECT id FROM method_snapshots WHERE session_id = $1`, [
        sessionId,
      ]);
    ...
    it('3. freeze creates exactly one immutable method_outputs row, readable via GET, no PUT/PATCH/DELETE route exists', async () => {
      ...
      const rows = await pool.query(`SELECT * FROM method_outputs WHERE session_id = $1`, [
        sessionId,
      ]);
      ...
      const del = await request(app)
        .delete(`/api/method/outputs/${output.id}`)
        .set('Authorization', `Bearer ${ownerToken}`);
    ...
    it('5. the Output traces back to the exact session revision and frozen snapshot that produced it', async () => {
      ...
      const snapshotRow = await pool.query(
        `SELECT session_id FROM method_snapshots WHERE id = $1`,
    ...
    it('6. appending a new event to the (now frozen) session leaves the Output content_hash byte-identical', async () => {
      ...
      const append = await request(app)
        ...
        .send({
          ...
          unitId: '1A',
          payload: {
            evidenceId: `ev-post-freeze-${randomUUID()}`,
      ...
      const dbRow = await pool.query(`SELECT content_hash FROM method_outputs WHERE id = $1`, [
        output.id,
      ]);
    ...
    it('8. reopening a frozen session (frozen -> active) creates a NEW session revision; the old session and its Output are untouched', async () => {
      const { sessionId: originalSessionId, output: originalOutput } =
        await createFrozenSessionWithOutput();

      const originalBefore = await pool.query(
        `SELECT state, frozen_snapshot_id, version FROM method_sessions WHERE id = $1`,
        [originalSessionId]
      );
      const outputBefore = await pool.query(
        `SELECT content_hash FROM method_outputs WHERE id = $1`,
        [originalOutput.id]
      );
      ...
      const outputAfter = await pool.query(
        `SELECT content_hash FROM method_outputs WHERE id = $1`,
        [originalOutput.id]
      );
    ...
    it('11. a Presentation renders from the SAME Output as the Report — same artefact, structured content, never an image', async () => {
      ...
      const reportContent = {
        executiveSummary: 'Report view',
        findings: output.findings.map((f: any) => f.unitId),
      };
      const reportRes = await request(app)
        .post(`/api/method/outputs/${output.id}/report`)
        .set('Authorization', `Bearer ${ownerToken}`)
    ...
    it('12. an Initiative Proposal Draft is created from the approved Output, structurally unable to become a Registered Initiative', async () => {
      ...
      const registerAttempt = await request(app)
        .post(
          `/api/method/outputs/${output.id}/initiative-drafts/${draftRes.body.draft.id}/register`
        )
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({});
    ...
    it('13. reopening + re-freezing supersedes the OLD Report/Presentation/Initiative Draft across the whole lineage, not just the newest session', async () => {
      const { sessionId: originalSessionId, output: outputA } =
        await createFrozenSessionWithOutput();
      ...
      const reportBRow = await pool.query(
        `SELECT status FROM method_report_snapshots WHERE id = $1`,
    ...
    describe('14. cross-org and unauthorized rejection', () => {
      ...
      it('14c. Report/Presentation/Initiative-draft creation: wrong org -> 404 (Output invisible), no Authorization -> 401', async () => {
        ...
        const reportRows = await pool.query(
          `SELECT id FROM method_report_snapshots WHERE output_id = $1`,
          [output.id]
        );
    ...
    it('15. an interruption between freeze (snapshot durable) and Output (never written) self-heals on the next freeze call', async () => {
      ...
      const postOutputs = await pool.query(`SELECT id FROM method_outputs WHERE session_id = $1`, [
        sessionId,
      ]);
    ...
    describe('★ demo bypass discipline (methodology_review pack)', () => {
      ...
      it('a demo-bypassed Output and its Report carry an explicit, visible demonstration marker — never indistinguishable from production', async () => {
        ...
        try {
          ...
          const outputRow = await pool.query(
            `SELECT demo_bypass_active FROM method_outputs WHERE id = $1`,
            [output.id]
          );
          ...
          const reportRow = await pool.query(
            `SELECT demo_bypass_active FROM method_report_snapshots WHERE id = $1`,
