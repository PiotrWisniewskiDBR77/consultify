...
describe.skipIf(!REAL_DB)(
  'Method Kernel — downstream listing/lookup HTTP surface (real PostgreSQL)',
  () => {
    let app: Express;
    ...
    beforeAll(async () => {
      ...
      await pool.query(
        ...
        [ORG, 'S1 method-core listing test org']
      );
      await pool.query(
        `INSERT INTO organizations (id, name) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING`,
        [OTHER_ORG, 'S1 method-core listing test org (other tenant)']
      );
      for (const [id, org] of [
        [OWNER, ORG],
        [APPROVER, ORG],
    ...
    it('2. GET /outputs/:id returns a snapshot byte-identical to the frozen database row', async () => {
      ...
      const dbRow = await pool.query(
        `SELECT content_hash, scope FROM method_outputs WHERE id = $1`,
        [output.id]
      );
      ...
      const responseFindings = [
        ...(res.body.output.findings as Array<{ businessMeaning: string }>),
      ].sort((a, b) => a.businessMeaning.localeCompare(b.businessMeaning));
    ...
    it('5. GET /outputs pagination: limit + offset page through the full set with no gaps or duplicates', async () => {
      ...
      const created = [
        await fullFreezeFlow({ projectId }),
        await fullFreezeFlow({ projectId }),
        await fullFreezeFlow({ projectId }),
      ];
      ...
      const seenIds = [...page1.body.outputs, ...page2.body.outputs].map(
        (o: { id: string }) => o.id
      );
    ...
    it('14. GET /initiative-drafts (list + :id) works and is tenant-isolated', async () => {
      ...
      const otherList = await request(app)
        ...
        .set('Authorization', `Bearer ${otherOrgToken}`);
      expect(otherList.body.initiativeDrafts.map((d: { id: string }) => d.id)).not.toContain(
        draft.id
      );
    ...
    it('16. GET /sessions/:id/lineage: reachable from EITHER the root or a reopened revision, and refused across tenants', async () => {
      ...
      const sessionIdsFromRevision = fromRevision.body.sessions
