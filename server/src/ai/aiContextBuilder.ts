...
const AIContextBuilder = {
  ...
  _getTasks: (orgId) => {
    return new Promise((resolve, reject) => {
      db.all(
        `SELECT id, initiative_id, assignee_id, status, priority, blocked_reason FROM tasks WHERE organization_id = ?`,
        [orgId],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows || []);
  ...
  _getInitiatives: (orgId) => {
    return new Promise((resolve, reject) => {
      db.all(
        `SELECT id, name, status, blocked_reason, updated_at FROM initiatives WHERE organization_id = ?`,
        [orgId],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows || []);
  ...
  _getHelpEvents: (orgId) => {
    return new Promise((resolve, reject) => {
      db.all(
        `SELECT user_id, playbook_key, event_type, created_at FROM help_events WHERE organization_id = ?`,
        [orgId],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows || []);
  ...
  _getMetrics: (orgId) => {
    return new Promise((resolve, reject) => {
      db.all(
        `SELECT event_type, created_at FROM metrics_events WHERE organization_id = ?`,
        [orgId],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows || []);
  ...
  _getLifecycleEvents: (orgId) => {
    return new Promise((resolve, reject) => {
      db.all(
        `SELECT event_type, metadata, created_at FROM organization_events WHERE organization_id = ? ORDER BY created_at DESC LIMIT 20`,
        [orgId],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows || []);
    ...
    tasks.forEach((t) => {
      ...
      if (t.assignee_id) {
        ...
        if (t.status === 'blocked' || t.status === 'BLOCKED')
          stats.user_load[t.assignee_id].blocked++;
      }
    });
