...
const AIPlaybookExecutor = {
  ...
  advanceRun: async (runId, userId) => {
    ...
    const allComplete = updatedRun.steps.every(
      (s) =>
        s.status === 'EXECUTED' ||
        s.status === 'SKIPPED' ||
    ...
    // Check for failures
    const hasFailed = updatedRun.steps.some((s) => s.status === 'FAILED' && !s.isOptional);
...
VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ...
  dryRunRoute: async (runId) => {
    ...
    if (stepType === 'BRANCH' || stepType === 'CHECK' || stepType === 'AI_ROUTER') {
      ...
      const context = await AIPlaybookRoutingEngine.buildContext(run.id, run.organizationId);
      const templateStep = await AIPlaybookRoutingEngine.getTemplateStep(
        currentStep.templateStepId
      );
