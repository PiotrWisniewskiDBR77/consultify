...
const SignalEngine = {
    ...
    Object.keys(user_load).forEach((userId) => {
      ...
      if (load.total >= 5 && load.completed === 0 && help.ratio < 0.2) {
        signals.push({
          ...
          explanation:
            'High task volume combined with zero output and low help engagement suggests a stall or lack of understanding of the tool/process.',
        });
      }
    });
    return signals;
    ...
    initiatives.forEach((init: any) => {
      if (init.is_blocked || init.stale_days > 7) {
        ...
        signals.push({
          ...
          explanation:
            'Initiatives that stop moving for more than 7 days represent a loss of momentum and hidden friction.',
        });
      }
    });
    return signals;
  ...
  _detectLowHelpAdoption: (context: any) => {
    ...
    if (totalStarted > 5 && globalRatio < 0.3) {
      signals.push({
        ...
        explanation:
          'High dropout rate in playbooks suggests either the help content is not engaging or users are getting distracted before completion.',
      });
    }
    return signals;
  },
  ...
  _detectStrongTeamMembers: (context: any) => {
    ...
    const stdDev = Math.sqrt(
      completions.map((x) => Math.pow(x - avg, 2)).reduce((a, b) => a + b, 0) / completions.length
    );
      ...
      if (load.completed > avg + stdDev && load.completed > 2) {
        signals.push({
          ...
          evidence: {
            ...
            standard_deviations_above:
              Math.round(((load.completed - avg) / (stdDev || 1)) * 100) / 100,
          },
          explanation:
            'Identifying high-output individuals allows for better knowledge sharing and potential promotion/mentoring roles.',
        });
      }
    });
    return signals;
