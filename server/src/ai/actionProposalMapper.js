...
const ActionProposalMapper = {
  ...
  mapSignalToProposals: (signal, recommendation, simulation) => {
    ...
    switch (signal.type) {
      case 'USER_AT_RISK':
        // Mapping: TASK_CREATE
        const tcId = generateProposalId('tc', signal.entity_id, signal.type);
        proposals.push({
          proposal_id: tcId,
          correlation_id: `corr-${tcId}`,
          ...
          risk_level: 'LOW',
          expected_impact:
            simulation?.narrative || 'Improve user retention and initial value realization.',
          simulation: {
        ...
        const paId = generateProposalId('pa', signal.entity_id, signal.type);
        proposals.push({
          proposal_id: paId,
          correlation_id: `corr-${paId}`,
          origin_signal: signal.type,
          origin_recommendation: recommendation?.title || 'N/A',
      ...
      case 'BLOCKED_INITIATIVE':
        ...
        const msId = generateProposalId('ms', signal.entity_id, signal.type);
        proposals.push({
          proposal_id: msId,
          correlation_id: `corr-${msId}`,
          origin_signal: signal.type,
          origin_recommendation: recommendation?.title || 'N/A',
          ...
          risk_level: 'MEDIUM',
          expected_impact:
            simulation?.narrative || 'Unfreeze dependencies and recover lost momentum.',
          simulation: {
            assumptions: simulation?.assumptions || [],
            expected_direction: 'positive',
          },
      ...
      case 'LOW_HELP_ADOPTION':
        // Mapping: PLAYBOOK_ASSIGN
        const haId = generateProposalId('ha', signal.entity_id, signal.type);
        proposals.push({
          proposal_id: haId,
          correlation_id: `corr-${haId}`,
          origin_signal: signal.type,
          origin_recommendation: recommendation?.title || 'N/A',
          ...
          expected_impact:
            ...
            'Identify and fix patterns where users get stuck in help docs.',
          simulation: {
            assumptions: simulation?.assumptions || [],
            expected_direction: 'positive',
          },
      ...
      case 'STRONG_TEAM_MEMBER':
        ...
        const rsId = generateProposalId('rs', signal.entity_id, signal.type);
        proposals.push({
          proposal_id: rsId,
          correlation_id: `corr-${rsId}`,
          origin_signal: signal.type,
          origin_recommendation: recommendation?.title || 'N/A',
          ...
          expected_impact:
            simulation?.narrative ||
            'Scale best practices by formalizing high-performer contributions.',
          simulation: {
            assumptions: simulation?.assumptions || [],
            expected_direction: 'positive',
          },
