...
const ActionDecisionService = {
  ...
  recordDecision: async (data: any) => {
    const { proposal_id, organization_id, decision, decided_by_user_id, reason, modified_payload } =
      data;

    if (!proposal_id || !decision || !decided_by_user_id || !organization_id) {
      throw new Error(
        'Missing required decision fields: proposal_id, organization_id, decision, or decided_by_user_id'
      );
    }
    ...
    const hasActiveApproval = existingDecisions.some(
      (d: any) => d.decision === 'APPROVED' || d.decision === 'MODIFIED'
    );
    ...
    if (decision === 'MODIFIED') {
      ...
      const allowlist =
        (ActionDecisionService.MODIFIED_ALLOWLIST as any)[proposal.action_type] || [];
      ...
      for (const key of keys) {
        if (allowlist.includes(key)) {
          ...
          const error = new Error(
            `Field '${key}' is not allowed to be modified for action type ${proposal.action_type}`
          );
          (error as any).status = 400;
          throw error;
        }
          ...
          (async () => {
            try {
              ...
              if (proposal.simulation) {
                assumptions.push(
                  `Impact simulation: ${proposal.simulation.impact_summary || 'analyzed'}`
                );
              }
              if (policyRuleId) {
