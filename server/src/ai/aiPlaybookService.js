...
const AIPlaybookService = {
  ...
  getTemplateByKey: async (key) => {
    return new Promise((resolve, reject) => {
      deps.db.get(
        `SELECT * FROM ai_playbook_templates WHERE key = ?`,
        [key],
        async (err, template) => {
  ...
  createDraftTemplate: async ({
    ...
    const id = `apt-${deps.uuidv4()}`;
    const graphJson =
      typeof templateGraph === 'string' ? templateGraph : JSON.stringify(templateGraph || {});
  ...
  updateDraftTemplate: async (id, updates) => {
    ...
    if (template.status && template.status !== 'DRAFT') {
      throw new Error(
        `Cannot edit template with status '${template.status}'. Only DRAFT templates can be edited.`
      );
    }
    ...
    if (updates.templateGraph !== undefined) {
      ...
      values.push(
        typeof updates.templateGraph === 'string'
          ? updates.templateGraph
          : JSON.stringify(updates.templateGraph)
      );
    }
    ...
    return new Promise((resolve, reject) => {
      deps.db.run(
        `UPDATE ai_playbook_templates SET ${setClauses.join(', ')} WHERE id = ?`,
        values,
        function (err) {
          if (err) reject(err);
  ...
  initiateRun: async ({ templateId, organizationId, initiatedBy, contextSnapshot = {} }) => {
    ...
    const steps = await new Promise((resolve, reject) => {
      deps.db.all(
        `SELECT * FROM ai_playbook_template_steps WHERE template_id = ? ORDER BY step_order`,
        [templateId],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows || []);
  ...
  getRun: async (runId) => {
    return new Promise((resolve, reject) => {
      deps.db.get(
        `SELECT r.*, t.key as template_key, t.title as template_title
  ...
  updateRunStatus: async (runId, status, extraFields = {}) => {
    ...
    return new Promise((resolve, reject) => {
      deps.db.run(
        `UPDATE ai_playbook_runs SET ${setClauses.join(', ')} WHERE id = ?`,
        values,
        function (err) {
          if (err) reject(err);
          else resolve(this.changes > 0);
  ...
  updateStepStatus: async (stepId, status, { decisionId = null, executionId = null } = {}) => {
    ...
    return new Promise((resolve, reject) => {
      deps.db.run(
        `UPDATE ai_playbook_run_steps SET ${setClauses.join(', ')} WHERE id = ?`,
        values,
        function (err) {
          if (err) reject(err);
          else resolve(this.changes > 0);
   ...
   */
  updateRunStepWithRouting: async (
    stepId,
    { outputs, evaluationTrace, selectedNextStepId, statusReason }
  ) => {
    const setClauses = [];
    const values = [];
  ...
  getNextStepInRun: async (runId, currentTemplateStepId) => {
    ...
    const currentStep = await AIPlaybookService.getRunStepByTemplateStepId(
      runId,
      ...
      deps.db.get(
        ...
        (err, stats) => {
          ...
          resolve({
            ...
            avgExecutionTimeMins: stats?.avg_duration_mins
              ? Math.round(stats.avg_duration_mins)
              : null,
            totalRuns,
            completedRuns,
  ...
  getTemplateAnalytics: async (
    templateId,
    { limit = 100, eventType = null, dateFrom = null, dateTo = null } = {}
  ) => {
