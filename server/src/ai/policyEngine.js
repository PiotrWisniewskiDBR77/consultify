...
const PolicyEngine = {
  ...
  evaluatePolicy: async ({ proposal, organizationId }) => {
    ...
    if (!isEnabled) {
      return {
        matched: false,
        decision: null,
        reason: 'Policy Engine globally disabled',
        rule_id: null,
      };
    }
    ...
    if (PolicyEngine.ALWAYS_MANUAL_ACTION_TYPES.includes(proposal.action_type)) {
      return {
        matched: false,
        decision: null,
        reason: `${proposal.action_type} always requires manual approval`,
        rule_id: null,
      };
    }
    ...
    for (const rule of rules) {
      const conditionsMatch = await PolicyEngine.evaluateConditions(
        rule.conditions,
        proposal,
        organizationId
      );
  ...
  getGlobalStatus: async () => {
    return new Promise((resolve) => {
      db.get(`SELECT * FROM ai_policy_settings WHERE id = 'singleton'`, [], (err, row) => {
        if (err || !row) {
