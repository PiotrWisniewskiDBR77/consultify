...
// @ts-expect-error - aiQueueModule from @ts-nocheck file, type cannot be determined
...
const AsyncJobService = {
  ...
  enqueuePlaybookAdvance: async ({
    ...
    const existingJob = await AsyncJobService.findActiveJob(
  ...
  getJob: async (jobId: any, organizationId: any): Promise<AsyncJob | null> => {
    return new Promise((resolve, reject) => {
      const query =
        organizationId === 'SUPERADMIN_BYPASS'
          ? `SELECT * FROM async_jobs WHERE id = ?`
          : `SELECT * FROM async_jobs WHERE id = ? AND organization_id = ?`;

      const params = organizationId === 'SUPERADMIN_BYPASS' ? [jobId] : [jobId, organizationId];
  ...
  updateJobStatus: async (jobId: any, status: any, metadata: any = {}) => {
    ...
    return new Promise<void>((resolve, reject) => {
      db.run(`UPDATE async_jobs SET ${updates.join(', ')} WHERE id = ?`, params, (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
  ...
  retryJob: async (jobId: any, organizationId: any) => {
    ...
    if (job.status !== JOB_STATUSES.FAILED && job.status !== JOB_STATUSES.DEAD_LETTER) {
      const error = new Error(
        `Cannot retry job in ${job.status} state. Only FAILED or DEAD_LETTER jobs can be retried.`
      );
    ...
    if (job.error_code && !isRetryable(job.error_code)) {
      const error = new Error(`Cannot retry job with non-retryable error: ${job.error_code}`);
    ...
    const payload =
      job.type === JOB_TYPES.EXECUTE_DECISION
        ? {
            decisionId: job.entity_id,
  ...
  cancelJob: async (jobId: any, organizationId: any) => {
    ...
    if (job.status !== JOB_STATUSES.QUEUED) {
      const error = new Error(
        `Cannot cancel job in ${job.status} state. Only QUEUED jobs can be cancelled.`
      );
      (error as any).code = 'JOB_INVALID_STATE';
      throw error;
    }
  ...
  incrementAttempts: async (jobId: string): Promise<number> => {
    await new Promise<void>((resolve, reject) => {
      db.run(
        `UPDATE async_jobs SET attempts = attempts + 1 WHERE id = ?`,
        [jobId],
        (err: Error | null) => {
          if (err) reject(err);
          else resolve();
    ...
    const job = await new Promise<{ attempts: number } | null>((resolve, reject) => {
      db.get(
