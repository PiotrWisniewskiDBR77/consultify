...
function loadConfig(): AppConfig {
    ...
    if (!result.success) {
        ...
        console.warn('[Config] Using defaults for invalid values. Fix configuration errors before deploying to production.');
        return AppConfigSchema.parse({
            ...rawConfig,
            JWT_SECRET: rawConfig.JWT_SECRET.length >= 32 ? rawConfig.JWT_SECRET : 'supersecretkey_change_this_in_production'.repeat(2),
        });
    }
