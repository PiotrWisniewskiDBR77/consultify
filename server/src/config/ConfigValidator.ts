...
const AppConfigSchema = JWTConfigSchema.merge(ServerConfigSchema)
    ...
    .refine(
        (data) => {
            ...
            const googleHasAny = data.GOOGLE_CLIENT_ID || data.GOOGLE_CLIENT_SECRET || data.GOOGLE_CALLBACK_URL;
            const googleHasAll = data.GOOGLE_CLIENT_ID && data.GOOGLE_CLIENT_SECRET && data.GOOGLE_CALLBACK_URL;
            if (googleHasAny && !googleHasAll) return false;

            // LinkedIn OAuth validation
            const linkedinHasAny = data.LINKEDIN_CLIENT_ID || data.LINKEDIN_CLIENT_SECRET || data.LINKEDIN_CALLBACK_URL;
            const linkedinHasAll = data.LINKEDIN_CLIENT_ID && data.LINKEDIN_CLIENT_SECRET && data.LINKEDIN_CALLBACK_URL;
