 ...
 */
router.post('/', verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
    const orgId = req.user?.organizationId;
    if (!orgId) {
        return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    const runResult1 = await dbRun(sql, [id, orgId, name, description || null, leadId || null, now]);
 ...
 */
router.put('/:id', verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const orgId = req.user?.organizationId;
    if (!orgId) {
        return res.status(401).json({ error: 'Unauthorized' });
 ...
 */
router.delete('/:id', verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const orgId = req.user?.organizationId;
    if (!orgId) {
        return res.status(401).json({ error: 'Unauthorized' });
    ...
    // Delete team
    const result = await dbRun('DELETE FROM teams WHERE id = ? AND organization_id = ?', [id, orgId]);
 ...
 */
router.post('/:id/members', verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const orgId = req.user?.organizationId;
    if (!orgId) {
        return res.status(401).json({ error: 'Unauthorized' });
    ...
    const team = await dbGet<{ id: string }>('SELECT id FROM teams WHERE id = ? AND organization_id = ?', [id, orgId]);
    ...
    const user = await dbGet<{ id: string }>('SELECT id FROM users WHERE id = ? AND organization_id = ?', [userId, orgId]);
 ...
 */
router.delete('/:id/members/:userId', verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = await dbRun('DELETE FROM team_members WHERE team_id = ? AND user_id = ?', [id, userId]);
