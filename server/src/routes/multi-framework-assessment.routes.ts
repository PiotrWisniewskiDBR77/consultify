...
router.get('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  res.json(await dbAll(`SELECT id, title, frameworks, overall_score, status, created_at
    FROM multi_framework_assessments WHERE organization_id = ? ORDER BY created_at DESC`, [orgId]) || []);
}));

router.post('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const { title, frameworks } = req.body;
  const id = uuidv4();
  await dbRun(`INSERT INTO multi_framework_assessments (id, organization_id, title, frameworks, status, created_at)
    VALUES (?, ?, ?, ?, 'in_progress', datetime('now'))`,
    [id, req.user?.organizationId, title || 'MFA Assessment', JSON.stringify(frameworks || ['SIRI', 'ADMA', 'CMMI'])]);
  res.status(201).json({ success: true, id });
}));

router.get('/:id', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const assessment = await dbAll(`SELECT * FROM multi_framework_assessments WHERE id = ?`, [req.params.id]);
