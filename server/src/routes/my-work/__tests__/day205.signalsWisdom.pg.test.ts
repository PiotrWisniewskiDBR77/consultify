...
describe(
  'Day205 R2 signal decisions become organization wisdom through real Gateway',
  NO_RETRY,
  () => {
    ...
    beforeAll(async () => {
      ...
      await sql.connect();
      await sql.query(
        `INSERT INTO organizations (id,name,plan,status) VALUES ($1,'Day205 R2','enterprise','active')`,
        [organizationId]
      );
      await sql.query(
    ...
    it('preserves mutation responses and persists non-empty snooze and dismiss claims', async () => {
      ...
      const resolved = JSON.stringify(
        await organizationContextService.buildResolvedContext(organizationId)
      );
