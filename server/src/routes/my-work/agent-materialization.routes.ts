...
function sendError(res: any, error: unknown) {
  ...
  const status = code.includes('NOT_FOUND')
    ...
    : code.includes('COLLISION') ||
      ...
      ? 409
      : code.includes('MEMBERSHIP') || code.includes('FORBIDDEN')
        ? 403
        : code.startsWith('MYW_AGENT_')
          ? 422
          : 500;
...
router.get(
  '/agent-materialization/source/:planId',
  asyncHandler(async (req: AuthRequest, res) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    try {
      res.json(
        await getAgentPlanSourceIdentity(identity.orgId, String(req.params.planId), identity.userId)
      );
    } catch (error) {
      sendError(res, error);
    }
...
router.get(
  '/agent-materialization/proposals',
  asyncHandler(async (req: AuthRequest, res) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const parsed = z
      .object({ sourcePlanId: z.string().min(1).max(128).optional() })
      .safeParse(req.query);
    if (!parsed.success)
      return res.status(400).json({ status: 'fail', error: { code: 'MYW_AGENT_INPUT_INVALID' } });
    try {
      res.json(
        await listMaterializationProposals({
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    const parsed = z
      .object({
        sourcePlanId: z.string().min(1).max(128),
        sourceVersion: z.number().int().positive(),
        sourceHash: hash,
        targetKind: z.enum(['task', 'decision', 'notebook']),
        content,
    ...
    if (!parsed.success)
      return res.status(400).json({ status: 'fail', error: { code: 'MYW_AGENT_INPUT_INVALID' } });
    try {
      const result = await createMaterializationProposal({
        ...parsed.data,
        organizationId: identity.orgId,
...
router.post(
  '/agent-materialization/proposals/:proposalId/decision',
  asyncHandler(async (req: AuthRequest, res) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const parsed = z
    ...
    try {
      res.json(
        await decideMaterializationProposal({
          ...parsed.data,
...
router.post(
  '/agent-materialization/proposals/:proposalId/materialize',
  asyncHandler(async (req: AuthRequest, res) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const parsed = z
