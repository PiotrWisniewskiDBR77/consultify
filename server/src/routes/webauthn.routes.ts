...
router.post('/register/begin', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  const user = await dbGet<any>('SELECT email, first_name, last_name FROM users WHERE id = ?', [userId]);
...
router.post('/register/finish', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
...
router.get('/credentials', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.delete('/credentials/:credentialId', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
