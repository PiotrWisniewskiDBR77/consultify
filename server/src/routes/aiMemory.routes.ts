 ...
 */
router.get('/context', verifyToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id || req.user?.userId;
        ...
        const preferences = memories.filter(m =>
            m.key && ['preferred_language', 'communication_style', 'response_length', 'timezone'].includes(m.key)
        );
        const context = memories.filter(m =>
            m.key && ['role_context', 'project_focus', 'expertise_level'].includes(m.key)
        );
        const custom = memories.filter(m =>
            m.key && !preferences.some(p => p.key === m.key) && !context.some(c => c.key === m.key)
        );
 ...
 */
router.put('/:key', verifyToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id || req.user?.userId;
 ...
 */
router.delete('/:key', verifyToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id || req.user?.userId;
        const { key } = req.params;
 ...
 */
router.post('/bulk', verifyToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id || req.user?.userId;
        const organizationId = req.user?.organizationId || req.user?.organization_id;
        ...
        for (const memory of memories) {
            const { key, value, source = 'inferred', confidence = 0.8 } = memory as {
                key?: string;
                value?: string;
                source?: string;
                confidence?: number;
            ...
            `, [userId, key]) as { id?: string; confidence?: number } | null;

            const valueStr = typeof value === 'string' ? value.trim() : String(value);

            // Only update if new confidence is higher or source is explicit
            if (existing?.id) {
                if (source === 'explicit' || (confidence && existing.confidence && confidence > existing.confidence)) {
                    await dbRun(`
                        UPDATE ai_user_memory
 ...
 */
router.post('/parse', verifyToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
