/**
 * AiSecurity Routes — mounted at /api/ai/security.
 *
 * RECOVERY (self-import wrapper family): the previous `createLazyRoute('./ai-security.js')` wrapper
...
router.use((_req, res) => {
  res.status(503).json({
    ...
    status: false,
    type: 'not_configured',
    message: 'AI security endpoints are not implemented in this deployment.',
  });
});
