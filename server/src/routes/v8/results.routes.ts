...
/** P04-B: derive KPI role from DB — never trust client-supplied headers. */
async function p04KpiRoleFromDB(req: AuthRequest): Promise<KpiPermissionRole> {
  const userId = req.user?.id;
  const orgId = req.user?.organizationId;
  const kpiId = req.params?.kpiId ?? req.params?.id ?? req.body?.kpiId;

  if (!userId || !orgId) return 'viewer';

  // For create operations (no kpiId yet), grant kpi_owner to org admins/owners.
  if (!kpiId) {
    const orgRole = String(req.user?.role ?? '').toLowerCase();
    return orgRole === 'owner' || orgRole === 'administrator' || orgRole === 'admin'
      ? 'kpi_owner'
      : 'viewer';
  }

  const row = await dbGet<{ owner_user_id?: string | null }>(
    `SELECT owner_user_id FROM initiative_kpis WHERE id = ? AND COALESCE(organization_id, (SELECT organization_id FROM initiatives WHERE id = initiative_id)) = ? LIMIT 1`,
    [kpiId, orgId]
  ).catch(() => null);

  if (!row) return 'viewer';
  if (row.owner_user_id === userId) return 'kpi_owner';

  // Non-owners with org membership can at minimum comment.
  const orgRole = String(req.user?.role ?? '').toLowerCase();
  return orgRole === 'owner' || orgRole === 'administrator' || orgRole === 'admin'
    ? 'commenter'
    : 'viewer';
...
async function p04AssertKpiPermission(
  ...
  const role = await p04KpiRoleFromDB(req);
