...
import {
  getPinnedLegacyValuationIdentity,
  readCanonicalLegacyValuationInputs,
  writeCanonicalLegacyPeers,
  writeCanonicalLegacyValuationDepth,
  writeCanonicalLegacyWacc,
} from '../../../services/finance/canonical/valuationLegacySuccessorService.js';
...
import {
  ...
  ValuationRegistrationError,
} from '../../../services/finance/canonical/valuationRegistrationService.js';
import {
  buildWaccByTerminalGGrid,
  getSensitivityGrid,
  type SensitivityAxisValues,
  writeSensitivityGrid,
} from '../../../services/finance/canonical/valuationSensitivityService.js';
...
import {
  loadWaccInputs,
  upsertWaccInputs,
  type UpsertWaccInputsParams,
} from '../../../services/finance/canonical/valuationWaccService.js';
import {
...
const valuationRegistrationSchema = z
  .object({
  ...
  .superRefine((body, ctx) => {
    ...
    if (body.sourceType === 'manual' && body.sourceId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['sourceId'],
        message: 'manual registration cannot bind sourceId',
      });
    }
  });

router.post(
  '/valuation/registrations',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (/^Source .* not found$/.test(message))
        return sendError(res, 404, 'SOURCE_NOT_FOUND', 'Valuation source not found');
      if (/must be approved before it can seed a valuation$/.test(message))
        return sendError(res, 409, 'SOURCE_NOT_APPROVED', message);
      throw error;
    }
  })
);
...
router.get(
  '/valuation/legacy/:legacyId/input-identity',
  requireActiveMembership,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { organizationId } = getV8Context(req);
    const identity = await getPinnedLegacyValuationIdentity(
      organizationId,
      String(req.params.legacyId || '')
    );
    if (!identity)
      return sendError(
        res,
        ...
        'No exact canonical valuation identity'
      );
...
router.get(
  '/valuation/legacy/:legacyId/inputs',
  requireActiveMembership,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { organizationId } = getV8Context(req);
    try {
      const result = await readCanonicalLegacyValuationInputs(
        organizationId,
        String(req.params.legacyId || '')
      );
      return res.status(200).json({
        data: {
          artifactId: result.identity.artifact_id,
          businessVersionId: result.identity.business_version_id,
          workingRevisionId: result.identity.working_revision_id,
          workingRevisionVersion: Number(result.identity.working_revision_version),
      ...
      const code = String(error?.code || 'CANONICAL_INPUT_READ_FAILED');
      return sendError(res, 409, code, String(error?.message || code));
    }
  })
);

router.delete(
  '/valuation/legacy/:legacyId',
  requireFinanceEditorMembership,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { organizationId, userId } = getV8Context(req);
    const body = req.body ?? {};
    ...
    if (!body.expected || typeof body.expected !== 'object')
      return sendError(res, 400, 'INVALID_BODY', 'expected canonical identity is required');
    const reason = typeof body.reason === 'string' ? body.reason : 'User discarded valuation';
    try {
      const result = await discardCanonicalLegacyValuation({
        organizationId,
        userId,
        legacyId: String(req.params.legacyId || ''),
      ...
      const code = String(error?.code || 'CANONICAL_VALUATION_DISCARD_FAILED');
      const status =
        code === 'NOT_FOUND'
          ? 404
          : code === 'IDEMPOTENCY_KEY_REQUIRED' || code === 'INVALID_REASON'
            ? 400
            : 409;
...
router.post(
  '/valuation/legacy/:legacyId/compute',
  requireFinanceEditorMembership,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { organizationId, userId } = getV8Context(req);
    const idempotencyKey = String(req.headers['x-idempotency-key'] || '').trim();
    if (!idempotencyKey)
      return sendError(res, 400, 'IDEMPOTENCY_KEY_REQUIRED', 'x-idempotency-key is required');
    try {
      const computed = await runCanonicalLegacyValuationCompute({
        organizationId,
        userId,
        legacyId: String(req.params.legacyId || ''),
        idempotencyKey,
      ...
      return res.status(200).json({
        data: {
          artifactId: computed.identity.artifact_id,
          businessVersionId: computed.identity.business_version_id,
          workingRevisionId: computed.identity.working_revision_id,
          workingRevisionVersion: Number(computed.identity.working_revision_version),
          jobId: computed.result.job.id,
          enterpriseValue: computed.result.enterpriseValue,
          equityValue: computed.result.equityValue,
          terminalValue: computed.result.terminalValue,
          wacc: 'wacc' in computed.result ? computed.result.wacc : undefined,
          replay: computed.replay,
        },
        meta: financeV2Meta(),
      });
    } catch (error: any) {
      const code = String(error?.code || 'CANONICAL_COMPUTE_FAILED');
      return sendError(
        res,
        code.includes('MISSING') ? 422 : 409,
        code,
...
router.post(
  '/valuation/legacy/:legacyId/negotiation-pack',
  requireFinanceEditorMembership,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { organizationId, userId } = getV8Context(req);
    const body = req.body ?? {};
    ...
    if (!body.expected || typeof body.expected !== 'object')
      return sendError(res, 400, 'INVALID_BODY', 'expected canonical identity is required');
    try {
      const result = await generateCanonicalLegacyNegotiationPack({
        organizationId,
        userId,
        legacyId: String(req.params.legacyId || ''),
        expected: body.expected,
      ...
      const code = String(error?.code || 'CANONICAL_NEGOTIATION_PACK_FAILED');
      const status =
        code === 'LEGACY_IDENTITY_UNMAPPED'
          ? 404
          : code === 'IDEMPOTENCY_KEY_REQUIRED'
            ? 400
            : code === 'CANONICAL_RESULTS_NOT_READY'
              ? 422
              : 409;
      return sendError(res, status, code, String(error?.message || code));
    }
...
router.post(
  '/valuation/legacy/:legacyId/export/pptx',
  requireFinanceEditorMembership,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { organizationId, userId } = getV8Context(req);
    const body = req.body ?? {};
    ...
    const theme = ['minimal', 'modern'].includes(body.theme) ? body.theme : 'corporate';
    const confidentiality = ['public', 'internal'].includes(body.confidentiality)
      ? body.confidentiality
      : 'confidential';
    try {
      const result = await exportCanonicalLegacyValuationPptx({
        organizationId,
        userId,
        legacyId: String(req.params.legacyId || ''),
        expected: body.expected,
        idempotencyKey,
        options: { language, theme, confidentiality },
      });
      return res.status(200).json({ data: result, meta: financeV2Meta() });
    } catch (error: any) {
      const code = String(error?.code || 'CANONICAL_PPTX_EXPORT_FAILED');
      const status =
        code === 'LEGACY_IDENTITY_UNMAPPED'
          ? 404
          : code === 'IDEMPOTENCY_KEY_REQUIRED'
...
router.put(
  '/valuation/legacy/:legacyId/depth',
  requireFinanceEditorMembership,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { organizationId, userId } = getV8Context(req);
    const body = req.body ?? {};
    ...
    if (
      ...
      !['managerial', 'banking'].includes(body.depth)
    )
      return sendError(res, 400, 'INVALID_BODY', 'depth and expected identity are required');
    try {
      const result = await writeCanonicalLegacyValuationDepth({
        organizationId,
        userId,
        legacyId: String(req.params.legacyId || ''),
        depth: body.depth,
      ...
      const code = String(error?.code || 'CANONICAL_DEPTH_WRITE_FAILED');
      const status =
        code === 'LEGACY_IDENTITY_UNMAPPED'
          ? 404
          : code === 'INVALID_DEPTH' || code === 'IDEMPOTENCY_KEY_REQUIRED'
            ? 400
            : 409;
      return sendError(res, status, code, String(error?.message || code));
    }
...
for (const [suffix, writer] of [
  ['assumptions', writeCanonicalLegacyWacc],
  ['peers', writeCanonicalLegacyPeers],
] as const) {
  router.put(
    `/valuation/legacy/:legacyId/${suffix}`,
    requireFinanceEditorMembership,
    asyncHandler(async (req: AuthRequest, res: Response) => {
      const { organizationId, userId } = getV8Context(req);
      const body = req.body ?? {};
      ...
      if (
        ...
        return sendError(
          ...
          400,
          'INVALID_BODY',
          'typed payload and expected identity are required'
        );
      try {
        ...
        const code = String(error?.code || 'CANONICAL_INPUT_WRITE_FAILED');
        return sendError(
          res,
          code === 'NEEDS_DECISION' ? 422 : 409,
          code,
          String(error?.message || code)
        );
      }
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (typeof body.businessVersionId !== 'string' || !body.businessVersionId.trim()) {
      return sendError(
        ...
        400,
        'INVALID_BODY',
        'businessVersionId is required (create it first via POST /artifacts with artifactType=VALUATION_CASE)'
      );
    }
    if (typeof body.name !== 'string' || !body.name.trim()) {
      return sendError(res, 400, 'INVALID_BODY', 'name is required');
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (
      ...
      return sendError(
        ...
        400,
        'INVALID_BODY',
        'description, when provided, must be a string or null'
      );
    }
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!result.ok) {
      const status =
        result.code === 'SAME_VARIANT'
          ? 400
          : result.code === 'INVALID_STATUS'
            ? 409
            : result.code === 'SNAPSHOT_FAILED'
              ? 500
              : 404; // CASE_NOT_FOUND / VARIANT_NOT_IN_CASE / ORGANIZATION_MISMATCH -> 404, fail-closed
      return sendError(res, status, result.code, result.message);
    }
    return res.status(200).json({
      data: {
        ...
        variantA: {
          businessVersionId: result.variantA.businessVersionId,
          name: result.variantA.name,
          enterpriseValue: result.variantA.enterpriseValue,
        },
        variantB: {
          businessVersionId: result.variantB.businessVersionId,
          name: result.variantB.name,
          enterpriseValue: result.variantB.enterpriseValue,
        },
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    return res.status(200).json({
      data: {
        ...
        weightedRecommendation: computeWeightedRecommendation(methods),
      },
      meta: financeV2Meta(),
    });
  })
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (
      body.methodType === 'OTHER_WITH_POLICY' &&
      (typeof body.applicabilityPolicyRef !== 'string' || !body.applicabilityPolicyRef.trim())
    ) {
      return sendError(
        res,
        400,
        'INVALID_BODY',
        'applicabilityPolicyRef is required for methodType=OTHER_WITH_POLICY'
      );
    }
    const variant = await getVariant(organizationId, businessVersionId);
    if (!variant) return sendError(res, 404, 'NOT_FOUND', 'Variant not found');
    ...
    const result = await findOrCreateMethod({
      ...
      applicabilityPolicyRef:
        typeof body.applicabilityPolicyRef === 'string' ? body.applicabilityPolicyRef : null,
    });
    if (!result.ok) return sendError(res, 404, result.code, result.message);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    for (let i = 0; i < body.updates.length; i++) {
      ...
      if (typeof u?.methodId !== 'string' || typeof u?.isInRecommendationBasket !== 'boolean') {
        return sendError(
          ...
          400,
          'INVALID_BODY',
          `updates[${i}] requires methodId (string) and isInRecommendationBasket (boolean)`
        );
      }
      ...
      if (u.isInRecommendationBasket) {
        if (typeof u.weightPct !== 'number' || !Number.isFinite(u.weightPct) || u.weightPct <= 0) {
          return sendError(
            ...
            400,
            'INVALID_BODY',
            `updates[${i}]: weightPct must be a positive number when isInRecommendationBasket=true`
          );
        }
        ...
        return sendError(
          ...
          400,
          'INVALID_BODY',
          `updates[${i}]: weightPct must be null/omitted when isInRecommendationBasket=false (cross-checks are never weighted, DEC-FIN-005)`
        );
      }
      updates.push({
        methodId: u.methodId,
        isInRecommendationBasket: u.isInRecommendationBasket,
        weightPct: u.isInRecommendationBasket ? u.weightPct : null,
      });
    }
    ...
    try {
      ...
      return res.status(200).json({
        data: {
          methods: result.methods.map(mapMethod),
          weightedRecommendation: computeWeightedRecommendation(result.methods),
        },
        meta: financeV2Meta(),
      });
    } catch (err: unknown) {
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!wacc)
      return sendError(res, 404, 'NOT_FOUND', 'No WACC inputs recorded for this variant yet');
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!Array.isArray(body.projectionYears) || body.projectionYears.length === 0) {
      return sendError(
        ...
        400,
        'INVALID_BODY',
        'projectionYears must be a non-empty array of {fiscalYear, periodIds[]}'
      );
    }
    for (let i = 0; i < body.projectionYears.length; i++) {
      const y = body.projectionYears[i];
      if (
        typeof y?.fiscalYear !== 'number' ||
        !Array.isArray(y?.periodIds) ||
        y.periodIds.some((p: unknown) => typeof p !== 'string')
      ) {
        return sendError(
          res,
          400,
          'INVALID_BODY',
          `projectionYears[${i}] requires fiscalYear (number) and periodIds (array of strings)`
        );
      }
    }
    ...
    const params: RunDcfFcffValuationParams = {
      ...
      openingWorkingCapital:
        typeof body.openingWorkingCapital === 'number' ? body.openingWorkingCapital : null,
      terminal: {
        gPct: typeof body.terminal.gPct === 'number' ? body.terminal.gPct : undefined,
      },
      directCashTaxRatePct:
        typeof body.directCashTaxRatePct === 'number' ? body.directCashTaxRatePct : undefined,
      valuationAsOfDate:
        typeof body.valuationAsOfDate === 'string' ? body.valuationAsOfDate : undefined,
    };
    ...
    try {
      ...
      if (
        code === 'P0001' &&
        /finance_valuation_wacc_inputs: parent business_version .* is APPROVED and immutable; UPDATE not permitted/.test(
          message
        )
      ) {
        return sendError(
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!bridge)
      return sendError(res, 404, 'NOT_FOUND', 'No EV->Equity bridge recorded for this variant yet');
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (
      ...
      typeof body.enterpriseValueDecimal !== 'number' ||
      ...
      return sendError(
        ...
        400,
        'INVALID_BODY',
        'asOfDate (string), enterpriseValueDecimal (number) and components (array) are required'
      );
    }
    ...
    for (let i = 0; i < body.components.length; i++) {
      ...
      if (
        ...
        return sendError(
          ...
          400,
          'INVALID_BODY',
          `components[${i}] requires sequenceOrder (number), componentKind, sign, amountDecimal (number) and asOfDate (string)`
        );
      }
      components.push({
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (typeof body.rowAxisVariable !== 'string' || typeof body.columnAxisVariable !== 'string') {
      return sendError(
        ...
        400,
        'INVALID_BODY',
        'rowAxisVariable and columnAxisVariable are required strings'
      );
    }
    ...
    if (
      !Array.isArray(wacc) ||
      wacc.length !== 5 ||
      wacc.some((n: unknown) => typeof n !== 'number')
    ) {
      return sendError(res, 400, 'INVALID_BODY', 'axes.wacc must be an array of exactly 5 numbers');
    }
    if (
      !Array.isArray(terminalG) ||
      terminalG.length !== 5 ||
      terminalG.some((n: unknown) => typeof n !== 'number')
    ) {
      return sendError(
        res,
        400,
        'INVALID_BODY',
        'axes.terminalG must be an array of exactly 5 numbers'
      );
    }
    if (
      ...
      body.years.some(
        (y: unknown) =>
          typeof (y as { fcff?: unknown })?.fcff !== 'number' ||
          typeof (y as { fiscalYear?: unknown })?.fiscalYear !== 'number'
      )
    ) {
      return sendError(
        res,
        400,
        'INVALID_BODY',
        'years must be a non-empty array of {fiscalYear, fcff}'
      );
    }
    if (
      typeof body.fcffTerminalYear !== 'number' ||
      typeof body.baseWaccPct !== 'number' ||
      typeof body.baseGPct !== 'number'
    ) {
      return sendError(
        res,
        400,
        'INVALID_BODY',
        'fcffTerminalYear, baseWaccPct and baseGPct (numbers) are required'
      );
    }

    const axes: SensitivityAxisValues = {
      wacc: wacc as [number, number, number, number, number],
      terminalG: terminalG as [number, number, number, number, number],
    };
    ...
    return res.status(200).json({
      data: {
        gridId: written.gridId,
        gridLabel: body.gridLabel,
        gridStatus: 'COMPLETE',
        baseRowIndex: built.baseRowIndex,
        baseColIndex: built.baseColIndex,
        cells: built.cells,
      },
      meta: financeV2Meta(),
    });
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!result.ok)
      return sendError(
        res,
        statusForAdvisorError(result.code),
        result.code,
        result.message,
        result.currentStatus ? { currentStatus: result.currentStatus } : undefined
      );
    return res.status(200).json({
      data: {
