...
describe.skipIf(!REAL_PG)(
  'Finance v2 Pakiet B — artifacts/versions/compute (real HTTP + real PostgreSQL)',
  () => {
    ...
    it('GET /artifacts/:id/capabilities reflects role + status via lifecycleService.allowedActionsFromStatus', async () => {
      ...
      const asAdmin = await request(app).get(
      ...
      const viewerRes = await request(appAs('some_unmapped_role')).get(
        `/api/v8/finance-v2/artifacts/${artifactId}/capabilities`
      );
    ...
    async function legacyUnknownManifestId(): Promise<string> {
      ...
      if (!row)
        throw new Error('LEGACY_UNKNOWN engine manifest not seeded — migrations not applied?');
    ...
    it('POST /compute/jobs without Idempotency-Key -> 400 IDEMPOTENCY_KEY_REQUIRED', async () => {
      ...
      const res = await request(app).post('/api/v8/finance-v2/compute/jobs').send({
        jobType: 'BASELINE_COMPUTE',
        inputArtifactId: artifactId,
        inputRevisionHash: 'h',
        engineManifestId,
