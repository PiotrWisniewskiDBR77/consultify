...
describe.skipIf(!REAL_PG)(
  'ID BRIDGE — GET /artifacts/resolve-legacy/:legacyTable/:legacyId',
  () => {
    ...
    it('1) RESOLVED: an AUTO_MIGRATE alias for a real artifact returns canonical artifactId + businessVersionId', async () => {
      ...
      const res = await request(app).get(
        `/api/v8/finance-v2/artifacts/resolve-legacy/financial_models/${legacyId}`
      );
    ...
    it('3) QUARANTINED: an alias with mapping_confidence=QUARANTINE surfaces the reason, distinct from NOT_MIGRATED', async () => {
      ...
      const app = appWithContext(orgId);
      const res = await request(app).get(
        `/api/v8/finance-v2/artifacts/resolve-legacy/valuations/${legacyId}`
      );
    ...
    it('4) cross-tenant: an alias belonging to another organization is invisible (NOT_MIGRATED, not a leak)', async () => {
      ...
      const app = appWithContext(orgId);
      const res = await request(app).get(
        `/api/v8/finance-v2/artifacts/resolve-legacy/financial_analyses/${legacyId}`
      );
    ...
    it('5) unknown legacyTable -> 400 INVALID_LEGACY_TABLE', async () => {
      const app = appWithContext(orgId);
      const res = await request(app).get(
        `/api/v8/finance-v2/artifacts/resolve-legacy/not_a_real_table/${randomUUID()}`
      );
        ...
        verifyClient.query(
         ...
         VALUES ($1, $2, NULL, $3, $4, $5, $6, NULL)`,
          [
            'financial_models',
            legacyId,
            foreignArtifact.artifact.artifact_id,
            orgId,
            foreignArtifact.businessVersion.business_version_id,
            'AUTO_MIGRATE',
          ]
        )
