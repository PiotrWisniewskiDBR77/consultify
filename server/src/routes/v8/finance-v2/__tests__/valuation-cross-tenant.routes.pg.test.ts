...
describe.skipIf(!REAL_PG)(
  'Finance v2 Pakiet B3 — valuation cross-tenant matrix (real HTTP + real PostgreSQL)',
  () => {
    ...
    it('POST /valuation/cases/:caseId/variants — org B posting a variant against org A caseId -> 404 CASE_NOT_FOUND, no row written', async () => {
      const caseId = await makeCaseAsA();
      const artifact = await request(appB)
        .post('/api/v8/finance-v2/artifacts')
        .send({ artifactType: 'VALUATION_CASE' });
      ...
      const res = await request(appB)
        .post(`/api/v8/finance-v2/valuation/cases/${caseId}/variants`)
        .send({ businessVersionId: bvId, name: 'Hijack attempt' });
      expect(res.status).toBe(404);
    ...
    it('GET/PATCH /valuation/variants/:id — org B on org A variant -> 404 both ways, SQL confirms name unchanged', async () => {
      ...
      const patchRes = await request(appB)
        .patch(`/api/v8/finance-v2/valuation/variants/${bvId}`)
        .send({ name: 'Hijacked Name' });
      expect(patchRes.status).toBe(404);

      const sqlRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ name: string; organization_id: string }>(
          `SELECT name, organization_id FROM finance_valuation_variants WHERE business_version_id = ?`,
          [bvId]
        )
      );
    ...
    it('GET/POST /valuation/variants/:id/methods and POST .../basket — org B on org A variant -> 404 all three, zero rows for org B', async () => {
      ...
      const listRes = await request(appB).get(
      ...
      const basketRes = await request(appB)
        .post(`/api/v8/finance-v2/valuation/variants/${bvId}/methods/basket`)
        .send({
          updates: [{ methodId: randomUUID(), isInRecommendationBasket: true, weightPct: 100 }],
        });
      expect(basketRes.status).toBe(404);

      const sqlRows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_valuation_methods WHERE business_version_id = ? AND organization_id = ?`,
          [bvId, orgB]
        )
      );
    ...
    it('GET method (terminal/sensitivity) with a methodId belonging to org A, requested by org B -> 404, zero leakage', async () => {
      const caseId = await makeCaseAsA();
      const bvId = await makeVariantAsA(caseId);
      const methodRes = await request(appA)
        .post(`/api/v8/finance-v2/valuation/variants/${bvId}/methods`)
        .send({ methodType: 'DCF_FCFF' });
      ...
      await valuationComputeService.setMethodResult({
        ...
        resultEvDecimal: 7_000_000,
      });

      const terminalRes = await request(appB).get(
      ...
      const sqlGrids = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_valuation_sensitivity_grids WHERE method_id = ? AND organization_id = ?`,
          [methodId, orgB]
        )
      );
    ...
    it('GET/PUT /valuation/variants/:id/wacc-inputs — org B on org A variant -> 404 both ways, no row for org B', async () => {
      ...
      const getRes = await request(appB).get(
      ...
      const sqlRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ id: string }>(
          `SELECT id FROM finance_valuation_wacc_inputs WHERE business_version_id = ?`,
          [bvId]
        )
      );
      expect(sqlRow).toBeNull();
    ...
    it('POST /valuation/variants/:id/compute/dcf — org B attempting to compute org A variant -> 404, no compute_jobs row for org B', async () => {
      ...
      const res = await request(appB)
        ...
        .send({
          entityId: randomUUID(),
          projectionYears: [{ fiscalYear: 2027, periodIds: [randomUUID()] }],
    ...
    it('GET /valuation/variants/:id/results — org B on org A variant -> 404 (ORGANIZATION_MISMATCH mapped fail-closed), zero data leaked in the body', async () => {
      ...
      const methodRes = await request(appA)
        ...
        .send({ methodType: 'DCF_FCFF' });
      await valuationComputeService.setMethodResult({
        methodId: methodRes.body.data.methodId,
        readiness: 'READY',
        resultValueStatus: 'PRESENT_NONZERO',
        resultEvDecimal: 42_000_000,
      });
    ...
    it('GET/PUT /valuation/variants/:id/bridge — org B on org A variant -> 404 both ways, SQL confirms no bridge row for org B', async () => {
      ...
      const getRes = await request(appB).get(
        `/api/v8/finance-v2/valuation/variants/${bvId}/bridge`
      );
      expect(getRes.status).toBe(404);
    ...
    it('POST /valuation/variants/:id/advisor/generate and GET .../advisor — org B on org A variant -> 404 both, zero advisor rows for org B', async () => {
      ...
      await valuationComputeService.setMethodResult({
        ...
        readiness: 'READY',
        resultValueStatus: 'PRESENT_NONZERO',
        resultEvDecimal: 15_000_000,
      });

      const genRes = await request(appB)
        .post(`/api/v8/finance-v2/valuation/variants/${bvId}/advisor/generate`)
        .send({});
      expect(genRes.status).toBe(404);

      const listRes = await request(appB).get(
        `/api/v8/finance-v2/valuation/variants/${bvId}/advisor`
      );
      expect(listRes.status).toBe(404);

      const sqlRows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_valuation_advisor_outputs WHERE business_version_id = ? AND organization_id = ?`,
          [bvId, orgB]
        )
      );
      expect(sqlRows.length).toBe(0);
    ...
    it('POST /valuation/cases/:caseId/compare-variants — org B supplying org A caseId + org A variant ids -> 404 (ORGANIZATION_MISMATCH), no advisor rows written for either side', async () => {
      ...
      const sqlRows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_valuation_advisor_outputs WHERE business_version_id = ? AND is_comparison = true`,
          [bvA]
        )
      );
