...
describe.skipIf(!REAL_PG)(
  'Finance v2 P0 fix — approve role gate + capability/endpoint consistency (real HTTP + real PostgreSQL)',
  () => {
    ...
    async function verifyIndependently(businessVersionId: string): Promise<any> {
      const r = await verifyClient.query(
        `SELECT * FROM finance_business_versions WHERE business_version_id = $1`,
        [businessVersionId]
      );
    ...
    async function createArtifactViaHttp(app: express.Express, artifactType: string) {
      ...
      if (res.status !== 201)
        throw new Error(`createArtifactViaHttp failed: ${res.status} ${JSON.stringify(res.body)}`);
      return res.body.data as {
        artifactId: string;
        currentBusinessVersion: { businessVersionId: string; version: number; status: string };
      };
    }
    ...
    async function driveToInReview(artifactType = 'HISTORICAL_ANALYSIS') {
      ...
      if (r.status !== 200)
        throw new Error(`submit_for_review failed: ${r.status} ${JSON.stringify(r.body)}`);
      ...
      if (r.status !== 200)
        throw new Error(`start_review failed: ${r.status} ${JSON.stringify(r.body)}`);
      version = r.body.data.version;
    ...
    type ActionCase = {
      ...
      call: (
        app: express.Express,
        ids: { artifactId: string; businessVersionId: string; version: number }
    ...
    const ACTION_CASES: ActionCase[] = [
      ...
      { action: 'archive', fromStatus: 'APPROVED', call: transitionCall('archive') },
      {
        ...
        fromStatus: 'APPROVED',
        reason: 'bad data',
        call: transitionCall('invalidate', 'bad data'),
      },
      {
        action: 'reopen',
        fromStatus: 'APPROVED',
    ...
    async function driveToStatus(
      ...
      const step = async (action: string, byApp: express.Express, reason?: string) => {
        ...
        if (res.status !== 200)
          throw new Error(
            `driveToStatus step ${action} failed: ${res.status} ${JSON.stringify(res.body)}`
          );
      ...
      if (targetStatus === 'APPROVED') {
        ...
        if (approveRes.status !== 200)
          throw new Error(
            `driveToStatus approve failed: ${approveRes.status} ${JSON.stringify(approveRes.body)}`
          );
    ...
    for (const testRole of ['viewer', 'preparer'] as const) {
      ...
      const casesForRole =
        testRole === 'viewer'
          ? ACTION_CASES
          : ACTION_CASES.filter(
              (c) => !['submit_for_review', 'withdraw', 'resume_editing'].includes(c.action)
            );
      ...
      for (const c of casesForRole) {
        it(`capability<->endpoint agree for role=${testRole} action=${c.action} (from ${c.fromStatus}): capabilities correctly excludes it AND the real endpoint rejects it (403 FORBIDDEN), row unchanged`, async () => {
          ...
          const capRes = await request(testApp).get(
            `/api/v8/finance-v2/artifacts/${st.artifactId}/capabilities`
          );
          const capSaysAllowed =
            Array.isArray(capRes.body?.data?.allowedActions) &&
            capRes.body.data.allowedActions.includes(c.action);
