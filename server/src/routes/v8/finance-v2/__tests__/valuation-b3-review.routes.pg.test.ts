...
describe.skipIf(!REAL_PG)(
  'Finance v2 Pakiet B3 — valuation, independent review supplement (real HTTP + real PostgreSQL)',
  () => {
    ...
    async function makeVariant(
      caseId: string,
      name = `Review Variant ${randomUUID().slice(0, 8)}`
    ) {
    ...
    it('★ REGRESSION (found+fixed in review): a REPEAT POST .../sensitivity for the SAME (methodId, gridLabel) returns the SAME gridId as the first call, and that gridId owns exactly 25 cells — not an orphaned id', async () => {
      ...
      const gridRows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_valuation_sensitivity_grids WHERE method_id = ? AND grid_label = 'regression'`,
          [methodId]
        )
      );
      ...
      const getRes = await request(app).get(
        `/api/v8/finance-v2/valuation/methods/${methodId}/sensitivity/regression`
      );
    ...
    it('GET .../methods/:methodId/sensitivity/:gridLabel — same-org but nonexistent gridLabel -> 404 NOT_FOUND (route ran); wrong-org methodId -> 404 NOT_FOUND (method ownership check ran first)', async () => {
      ...
      const noSuchLabel = await request(app).get(
        `/api/v8/finance-v2/valuation/methods/${methodId}/sensitivity/does-not-exist`
      );
      ...
      const wrongOrg = await request(otherApp).get(
        `/api/v8/finance-v2/valuation/methods/${methodId}/sensitivity/does-not-exist`
      );
    ...
    it('★ KONTROLA NEGATYWNA (DB layer, weight-sum=100): a raw SQL basket totalling 60% — bypassing setBasketWeights()/the route entirely — is rejected AT COMMIT by trg_finance_valuation_methods_weight_sum', async () => {
      ...
      await expect(
        withPinnedPostgresTransaction((tx) =>
          tx.queryRun(
            `UPDATE finance_valuation_methods SET is_in_recommendation_basket = true, weight_pct = 60 WHERE id = ?`,
            [mA]
          )
        )
    ...
    it('★ TRZY STANY, nigdy ze sobą pomylone: MISSING (brak danych) vs. PRESENT_ZERO (prawdziwe zero) vs. PRESENT_NONZERO — GET .../methods i GET .../results rozróżniają wszystkie trzy jednocześnie', async () => {
      ...
      // bypassed.
      const zeroResult = await valuationComputeService.setMethodResult({
        methodId: mZero,
        readiness: 'READY',
        resultValueStatus: 'PRESENT_ZERO',
        resultEvDecimal: 0,
      });
      expect(zeroResult.ok).toBe(true);
      const nonzeroResult = await valuationComputeService.setMethodResult({
        methodId: mNonzero,
        readiness: 'READY',
        resultValueStatus: 'PRESENT_NONZERO',
        resultEvDecimal: 4_200_000,
      });
      ...
      const byType = (t: string) =>
        methodsRes.body.data.methods.find((m: any) => m.methodType === t);
    ...
    it('DISCOVERY: a second, byte-identical POST .../compute/dcf call (same entity/years/terminal, hence same idempotency key in computeJobService) — documents whether the endpoint replays idempotently or errors', async () => {
      ...
      const requestBody = {
        entityId,
        projectionYears: [{ fiscalYear: 2029, periodIds: [periodId] }],
      ...
      const jobCountBefore = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ n: string }>(
          `SELECT count(*)::text AS n FROM compute_jobs WHERE organization_id = ? AND job_type = 'VALUATION_COMPUTE'`,
          [orgId]
        )
      );
