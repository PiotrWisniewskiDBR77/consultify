...
describe.skipIf(!REAL_PG)(
  'Finance v2 ROUTES_EXPOSURE — Export/Import (real HTTP + real PostgreSQL)',
  () => {
    ...
    it('MOUNT PROOF: valid context + REAL router, random artifact/version -> 404 WITH {code:"NOT_FOUND"}', async () => {
      const res = await request(appA).get(
        `/api/v8/finance-v2/export/statement-pack/${randomUUID()}/${randomUUID()}`
      );
    ...
    it('POST /import/preview — after editing the Value cell, shows a real toChange diff', async () => {
      const editedRows = parsedRows.map((r: any) =>
        r['Value'] !== undefined ? { ...r, Value: 250 } : r
      );
      const res = await request(appA)
        .post('/api/v8/finance-v2/import/preview')
    ...
    it('POST /import/apply — same request replays its immutable receipt; same key with changed payload fails closed', async () => {
      ...
      const editedRows = parsedRows.map((r: any) =>
        r['Value'] !== undefined ? { ...r, Value: 999 } : r
      );
      const first = await request(appA).post('/api/v8/finance-v2/import/apply').send({
        artifactId,
        businessVersionId: bvId,
    ...
    it("CROSS-TENANT: org B exporting org A's Statement Pack -> 404 NOT_FOUND", async () => {
      const res = await request(appB).get(
        `/api/v8/finance-v2/export/statement-pack/${artifactId}/${bvId}`
      );
      expect(res.status).toBe(404);
      expect(res.body).toHaveProperty('code', 'NOT_FOUND');
    });
