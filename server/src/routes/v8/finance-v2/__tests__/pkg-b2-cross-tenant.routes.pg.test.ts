...
describe.skipIf(!REAL_PG)(
  'Finance v2 Pakiet B2 — cross-tenant matrix (real HTTP + real PostgreSQL)',
  () => {
    ...
    it('GET /statements/:id/lines — org B on org A Statement Pack -> 404, SQL confirms lines still exist for org A', async () => {
      ...
      const mapRes = await request(appA)
        ...
        .send({
          ...
          rawLines: [
            { lineItem: 'Cash', periodId, entityCode, currency: 'PLN', value: 100, sourceRef: {} },
          ],
      ...
      const rows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ organization_id: string }>(
          `SELECT organization_id FROM finance_stmt_lines WHERE business_version_id = ?`,
          [bvId]
        )
      );
    ...
    it('POST /statements/:id/reconcile — org B attempting to reconcile org A version -> 404, SQL confirms zero reconciliation runs for org B', async () => {
      ...
      const res = await request(appB)
        ...
        .send({ sourceSystem: 'attack', mappingResults: [] });
      expect(res.status).toBe(404);

      const rows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_reconciliation_runs WHERE business_version_id = ? AND organization_id = ?`,
          [bvId, orgB]
        )
      );
    ...
    it('GET /baseline/:id/assumptions and POST — org B on org A Baseline Model -> 404 both ways, SQL confirms zero assumption rows for org B', async () => {
      ...
      const rows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_baseline_assumptions WHERE business_version_id = ? AND organization_id = ?`,
          [bvId, orgB]
        )
      );
      expect(rows.length).toBe(0);
    ...
    it('POST /prediction/:id/preflight — org B on org A scenario -> 404, SQL confirms zero preflight runs for org B', async () => {
      ...
      const rows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_prediction_preflight_runs WHERE business_version_id = ? AND organization_id = ?`,
          [scenarioBvId, orgB]
        )
      );
      expect(rows.length).toBe(0);
    ...
    it('POST /artifacts/:id/rename (D3) — org B renaming org A artifact -> 404, SQL confirms natural_key unchanged', async () => {
      const created = await request(appA)
        .post('/api/v8/finance-v2/artifacts')
        .send({ artifactType: 'HISTORICAL_ANALYSIS', naturalKey: 'Original Name' });
      const artifactId = created.body.data.artifactId;

      const crossRename = await request(appB)
        .post(`/api/v8/finance-v2/artifacts/${artifactId}/rename`)
        .send({ naturalKey: 'Hijacked Name' });
      expect(crossRename.status).toBe(404);

      const row = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ natural_key: string; organization_id: string }>(
          `SELECT natural_key, organization_id FROM finance_artifacts WHERE artifact_id = ?`,
          [artifactId]
        )
      );
      ...
      const legit = await request(appA)
        .post(`/api/v8/finance-v2/artifacts/${artifactId}/rename`)
        .send({ naturalKey: 'Renamed By Owner' });
    ...
    it('GET /versions/:id/lineage and /exceptions/open — org B never sees org A rows (empty result, not an error leak)', async () => {
      ...
      const exceptionsRes = await request(appB).get(
        `/api/v8/finance-v2/exceptions/open?artifactId=${artifactId}`
      );
