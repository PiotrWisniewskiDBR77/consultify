...
describe.skipIf(!REAL_PG)(
  'INDEPENDENT VERIFIER — Pakiet B3 claim #8 (DCF idempotent replay, post-fix)',
  () => {
    ...
    async function buildFixture() {
      ...
      const requestBody = {
        entityId,
        projectionYears: [{ fiscalYear: 2031, periodIds: [periodId] }],
        openingWorkingCapital: 120_000,
        terminal: { gPct: 2 },
      };
      ...
      const spy = vi
        ...
        .mockImplementationOnce(async (completeParams) => {
          ...
          if (runningStatus !== 'running') {
            throw new Error(
              `fixture: expected job ${completeParams.jobId} to be 'running' at interception time, was '${runningStatus}'`
            );
          }
