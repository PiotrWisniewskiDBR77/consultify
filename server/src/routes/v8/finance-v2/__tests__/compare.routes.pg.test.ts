...
describe.skipIf(!REAL_PG)('Finance v2 ROUTES_EXPOSURE — Compare engine (real HTTP + real PostgreSQL)', () => {
  ...
  describe('the other five Compare axes — versions / entities / scenarios / valuation-methods / actual-vs-forecast', () => {
    ...
    beforeAll(async () => {
      ...
      const entityRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ entity_code: string }>(`SELECT entity_code FROM finance_stmt_entities WHERE id = ?`, [entityId])
      );
      ...
      const engineManifestRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ engine_manifest_id: string }>(`SELECT engine_manifest_id FROM finance_business_versions WHERE business_version_id = ?`, [stmtBvId])
      );
      if (!engineManifestRow) throw new Error('fixture setup: stmtBvId engine_manifest_id not found');
      ...
      const revenueLineRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ id: string }>(`SELECT id FROM financial_statement_lines WHERE line_code = 'REVENUE' AND organization_id IS NULL LIMIT 1`)
      );
      if (!revenueLineRow) throw new Error('fixture setup: no seeded REVENUE line');
      const revenueLineId = revenueLineRow.id;

      const baselineArtifact = await av.createArtifact({ organizationId: orgA, artifactType: 'BASELINE_MODEL', createdBy: userA });
      ...
      if (!baselineEntityRow) throw new Error('fixture setup: baseline entity insert returned no row');
      await withPinnedPostgresTransaction((tx) =>
        tx.queryRun(
          ...
          [orgA, baselineBv, revenueLineId, baselineEntityRow.id, scenarioPeriodId, 11_943_750.0, userA]
        )
      );

      const baseScenarioArtifact = await av.createArtifact({ organizationId: orgA, artifactType: 'PREDICTION_SCENARIO', createdBy: userA });
      ...
      if (!edge.ok) throw new Error(`fixture setup: MODEL_TO_SCENARIO edge insert failed: ${edge.code}`);

      const downsideArtifact = await av.createArtifact({ organizationId: orgA, artifactType: 'PREDICTION_SCENARIO', createdBy: userA });
      ...
      if (!downsideEntityRow) throw new Error('fixture setup: downside entity insert returned no row');
      await withPinnedPostgresTransaction((tx) =>
        tx.queryRun(
          ...
          [orgA, downsideBv, revenueLineId, downsideEntityRow.id, scenarioPeriodId, 11_602_500.0, userA]
        )
      );

      // --- compareValuationMethods: one variant, two methods, real EV values --
      const caseRes = await request(appA).post('/api/v8/finance-v2/valuation/cases').send({ name: `J1 compare fixture ${randomUUID().slice(0, 8)}` });
      if (caseRes.status !== 201) throw new Error(`fixture setup: POST /valuation/cases failed: ${caseRes.status} ${JSON.stringify(caseRes.body)}`);
      const caseId = caseRes.body.data.caseId as string;
      const valuationArtifact = await request(appA).post('/api/v8/finance-v2/artifacts').send({ artifactType: 'VALUATION_CASE' });
      valuationBvId = valuationArtifact.body.data.currentBusinessVersion.businessVersionId as string;
      ...
      if (variantRes.status !== 201) throw new Error(`fixture setup: POST .../variants failed: ${variantRes.status} ${JSON.stringify(variantRes.body)}`);

      const methodARes = await request(appA).post(`/api/v8/finance-v2/valuation/variants/${valuationBvId}/methods`).send({ methodType: 'DCF_FCFF' });
      const methodAId = methodARes.body.data.methodId as string;
      const methodBRes = await request(appA).post(`/api/v8/finance-v2/valuation/variants/${valuationBvId}/methods`).send({ methodType: 'ASSET_BASED' });
      ...
      await withPinnedPostgresTransaction((tx) =>
        tx.queryRun(`UPDATE finance_valuation_methods SET readiness = 'READY', result_value_status = 'PRESENT_NONZERO', result_ev_decimal = '50000000' WHERE id = ?`, [methodAId])
      );
      await withPinnedPostgresTransaction((tx) =>
        tx.queryRun(`UPDATE finance_valuation_methods SET readiness = 'READY', result_value_status = 'PRESENT_NONZERO', result_ev_decimal = '47000000' WHERE id = ?`, [methodBId])
      );
      ...
      const forecastArtifact = await av.createArtifact({ organizationId: orgA, artifactType: 'BASELINE_MODEL', createdBy: userA });
      ...
      if (!forecastEntityRow) throw new Error('fixture setup: AvF forecast entity insert returned no row');
      await withPinnedPostgresTransaction((tx) =>
        tx.queryRun(
          `INSERT INTO finance_baseline_outputs (
             organization_id, business_version_id, statement_type, canonical_line_id, entity_id, period_id,
    ...
    it('POST /compare/versions — bv1(100) vs bv2(130) of the SAME artifact, matched via entity_code, relationship=B_IS_DIRECT_CHILD_OF_A; SQL confirms bv2 really holds 130', async () => {
      ...
      const sqlRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ value_decimal: string }>(`SELECT value_decimal FROM finance_stmt_lines WHERE business_version_id = ? AND entity_id = ?`, [bv2, entity2Id])
      );
    ...
    it('CROSS-TENANT compare/versions: org B claims its OWN organizationId but org A\'s real businessVersionIdA -> 404 ARTIFACT_NOT_FOUND', async () => {
      const res = await request(appB)
        .post('/api/v8/finance-v2/compare/versions')
        .send({ artifactType: 'STATEMENT_PACK', artifactId: stmtArtifactId, businessVersionIdA: stmtBvId, businessVersionIdB: bv2, entityCode });
      expect(res.status).toBe(404);
      expect(res.body).toHaveProperty('code', 'ARTIFACT_NOT_FOUND');
    });
    ...
    it('POST /compare/entities — entity1(100) vs entity3(40) SAME period/line within stmtBvId; SQL confirms entity3 really holds 40', async () => {
      const res = await request(appA)
        ...
        .send({ artifactRef: artifactRefFor(orgA), periodId: periodIdA, entityIdA: entityId, entityIdB: entity3Id, consolidationScope: 'STANDALONE', accumulationBasis: 'FULL_YEAR' });
      expect(res.status).toBe(200);
      const { data } = res.body;
      ...
      const sqlRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ value_decimal: string }>(`SELECT value_decimal FROM finance_stmt_lines WHERE business_version_id = ? AND entity_id = ?`, [stmtBvId, entity3Id])
      );
    ...
    it('CROSS-TENANT compare/entities: org B forging artifactRef.organizationId=orgA -> 403 ORGANIZATION_MISMATCH', async () => {
      const res = await request(appB)
        .post('/api/v8/finance-v2/compare/entities')
        .send({ artifactRef: artifactRefFor(orgA), periodId: periodIdA, entityIdA: entityId, entityIdB: entity3Id });
      expect(res.status).toBe(403);
      expect(res.body).toHaveProperty('code', 'ORGANIZATION_MISMATCH');
    });
    ...
    it('POST /compare/scenarios — Base (11,943,750 via STANDARD_BASE -> finance_baseline_outputs passthrough) vs Downside (11,602,500), matches WP-D08\'s published delta exactly', async () => {
      ...
      const res = await request(appA)
        ...
        .send({ businessVersionIdBase: baseScenarioBv, businessVersionIdOther: downsideBv, consolidationScope: 'CONSOLIDATED' });
      expect(res.status).toBe(200);
      const { data } = res.body;
      ...
      const sqlRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ value_decimal: string }>(`SELECT value_decimal FROM finance_prediction_outputs WHERE business_version_id = ?`, [downsideBv])
      );
    ...
    it('POST /compare/valuation-methods — DCF_FCFF(50,000,000) vs ASSET_BASED(47,000,000) within the SAME variant; SQL confirms both result_ev_decimal values', async () => {
      const res = await request(appA)
        ...
        .send({ businessVersionId: valuationBvId, methodTypeA: 'DCF_FCFF', methodTypeB: 'ASSET_BASED' });
      expect(res.status).toBe(200);
      const { data } = res.body;
      ...
      const sqlRows = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ method_type: string; result_ev_decimal: string }>(`SELECT method_type, result_ev_decimal FROM finance_valuation_methods WHERE business_version_id = ? ORDER BY method_type`, [
          valuationBvId,
        ])
      );
      expect(sqlRows.find((r) => r.method_type === 'DCF_FCFF')?.result_ev_decimal).toBe('50000000');
      expect(sqlRows.find((r) => r.method_type === 'ASSET_BASED')?.result_ev_decimal).toBe('47000000');
    });
    ...
    it('POST /compare/actual-vs-forecast — Actual STATEMENT_PACK(100) vs Forecast BASELINE_MODEL(80), same entity_code/period, accumulationBasis ignored on match; SQL confirms the forecast row', async () => {
      const res = await request(appA)
        ...
        .send({
          ...
          forecastArtifactRef: { organizationId: orgA, artifactId: forecastBvId, businessVersionId: forecastBvId, artifactType: 'BASELINE_MODEL', naturalKey: null },
          entityCode,
          periodIds: [periodIdA],
          accumulationBasis: 'FULL_YEAR',
          consolidationScope: 'STANDALONE',
      ...
      const sqlRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ value_decimal: string }>(`SELECT value_decimal FROM finance_baseline_outputs WHERE business_version_id = ?`, [forecastBvId])
      );
