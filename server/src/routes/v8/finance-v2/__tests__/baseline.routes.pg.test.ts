...
describe.skipIf(!REAL_PG)('Finance v2 Pakiet B2 — baseline (real HTTP + real PostgreSQL)', () => {
  ...
  it('POST /baseline/:id/assumptions batch-writes; GET reads back with the full value contract; a second POST upserts (not duplicates)', async () => {
    ...
    const sqlRow = await withPinnedPostgresTransaction((tx) =>
      tx.queryOne<{ value_decimal: string }>(
        `SELECT value_decimal FROM finance_baseline_assumptions WHERE id = ?`,
        [assumptionId]
      )
    );
    ...
    const countRow = await withPinnedPostgresTransaction((tx) =>
      tx.queryOne<{ count: string }>(
        `SELECT count(*)::text AS count FROM finance_baseline_assumptions WHERE business_version_id = ?`,
        [bvId]
      )
    );
  ...
  it('POST /baseline/:id/assumptions with an invalid scheduleType -> 400 INVALID_BODY, no row written', async () => {
    ...
    const res = await request(app)
      ...
      .send({
        assumptions: [
          {
            scheduleType: 'not_a_real_schedule_type',
            driverCode: 'X',
            entityId: randomUUID(),
            periodId,
            rule: 'FIXED_VALUE',
  ...
  it('POST /baseline/:id/compute on a fresh Baseline Model with no STATEMENT_TO_MODEL lineage edge -> 404 NO_SOURCE_STATEMENT_PACK_EDGE (the real first gate `loadContext` checks, before even looking for a finance_baseline_models row)', async () => {
    ...
    const res = await request(app)
      ...
      .send({
        entityId: randomUUID(),
        forecastPeriodIds: [periodId],
        openingBalanceSheetPeriodId: periodId,
      });
  ...
  describe('POST /baseline/:id/compute — real happy path (GoldCo fixture, real circularity solve)', () => {
    ...
    it('converges to a real EV/output row set — {jobId, jobStatus, periodsComputed:12, monthlyResults}; SQL confirms 372 finance_baseline_outputs rows and finance_business_versions state', async () => {
      ...
      const engineManifestId = (await withPinnedPostgresTransaction((tx) =>
      ...
      await withPinnedPostgresTransaction((tx) =>
        tx.queryRun(
          ...
          [
            ...
            orgId,
            bvId2,
            'J1 LUKA 2 fixture — GoldCo FY2026 explicit monthly horizon',
            userId,
          ]
        )
      );
      ...
      const res = await request(app).post(`/api/v8/finance-v2/baseline/${bvId2}/compute`).send({
      ...
      const outputCount = await withPinnedPostgresTransaction((tx) =>
    ...
    it("CROSS-TENANT: org B computing org A's real Baseline Model business_version_id -> 404, zero finance_baseline_outputs rows for org B", async () => {
      ...
      const res = await request(appB).post(`/api/v8/finance-v2/baseline/${bvId2}/compute`).send({
        entityId: happyEntityId,
        forecastPeriodIds: happyForecastPeriodIds,
        openingBalanceSheetPeriodId: happyOpeningPeriodId,
      });
      ...
      const orgBOutputs = await withPinnedPostgresTransaction((tx) =>
        tx.queryAll<{ id: string }>(
          `SELECT id FROM finance_baseline_outputs WHERE business_version_id = ? AND organization_id = ?`,
          [bvId2, orgB]
        )
      );
