...
describe.skipIf(!REAL_PG)('Finance v2 Pakiet B — cross-tenant matrix (real HTTP + real PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    ...
    await withPinnedPostgresTransaction((tx) =>
      tx.queryRun(`INSERT INTO organizations (id, name) VALUES (?, ?), (?, ?)`, [orgA, 'PkgB Tenant A', orgB, 'PkgB Tenant B'])
    );
  ...
  it('GET /artifacts/:id — org B reading org A artifact -> 404, SQL confirms row still belongs to org A', async () => {
    ...
    const row = await withPinnedPostgresTransaction((tx) =>
      tx.queryOne<{ organization_id: string }>(`SELECT organization_id FROM finance_artifacts WHERE artifact_id = ?`, [artifactId])
    );
  ...
  it('GET /artifacts/:id/capabilities — org B on org A artifact -> 404', async () => {
    ...
    const res = await request(appB).get(`/api/v8/finance-v2/artifacts/${artifactId}/capabilities`);
    expect(res.status).toBe(404);
  });
  ...
  it('POST /versions/:id/compute-snapshot — org B on org A version -> 404 NOT_FOUND, SQL confirms zero snapshots created', async () => {
    ...
    const res = await request(appB).post(`/api/v8/finance-v2/versions/${bvId}/compute-snapshot`).send({});
    expect(res.status).toBe(404);

    const snapshots = await withPinnedPostgresTransaction((tx) =>
      tx.queryAll<{ compute_snapshot_id: string }>(`SELECT compute_snapshot_id FROM finance_compute_snapshots WHERE working_revision_id IN (SELECT working_revision_id FROM finance_working_revisions WHERE business_version_id = ?)`, [bvId])
    );
  ...
  it('GET /compute/jobs/:id — org B reading org A job -> 404, SQL confirms job untouched', async () => {
    ...
    const row = await withPinnedPostgresTransaction((tx) =>
      tx.queryOne<{ status: string; organization_id: string }>(`SELECT status, organization_id FROM compute_jobs WHERE id = ?`, [jobId])
    );
  ...
  it('POST /compute/jobs/:id/cancel — org B cancelling org A job -> 404, SQL confirms job still queued (not cancelled)', async () => {
    ...
    const enqueueRes = await request(appA)
      ...
      .send({ jobType: 'BASELINE_COMPUTE', inputArtifactId: artifactId, inputRevisionHash: 'xh2', engineManifestId });
    const jobId = enqueueRes.body.data.jobId;

    const crossCancel = await request(appB).post(`/api/v8/finance-v2/compute/jobs/${jobId}/cancel`).send({ reason: 'malicious cross-tenant cancel' });
    ...
    const legitCancel = await request(appA).post(`/api/v8/finance-v2/compute/jobs/${jobId}/cancel`).send({ reason: 'real owner cancel' });
