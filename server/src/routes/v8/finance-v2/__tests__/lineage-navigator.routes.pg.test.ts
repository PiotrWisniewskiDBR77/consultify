...
describe.skipIf(!REAL_PG)(
  'Finance v2 ROUTES_EXPOSURE — Lineage Navigator (real HTTP + real PostgreSQL)',
  () => {
    ...
    it('MOUNT PROOF: valid context + REAL router, random business_version_id -> 404 WITH {code:"NOT_FOUND"}', async () => {
      const res = await request(appA).get(
        `/api/v8/finance-v2/versions/${randomUUID()}/lineage-navigator`
      );
    ...
    it('GET /versions/:baselineBvId/lineage-navigator — mid-chain node has both a parent AND a child group', async () => {
      ...
      expect(relatedPanel.children.map((g: any) => g.artifactType)).toEqual([
        'PREDICTION_SCENARIO',
      ]);
    });
