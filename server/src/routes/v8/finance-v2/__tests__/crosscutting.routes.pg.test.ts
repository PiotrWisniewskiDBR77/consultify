...
describe.skipIf(!REAL_PG)(
  'Finance v2 Pakiet B2 — crosscutting freshness-events + exceptions/inbox (real HTTP + real PostgreSQL)',
  () => {
    ...
    async function approveDraft(
      ...
      const submitted = await av.transition({
      ...
      const started = await av.transition({
        organizationId: orgId,
        businessVersionId: bvId,
        action: 'start_review',
        actorId: approverId,
      ...
      await withPinnedPostgresTransaction((tx) =>
        tx.queryRun(
          `UPDATE finance_business_versions SET freshness = 'CURRENT', freshness_reason = NULL, stale_since = NULL WHERE business_version_id = ?`,
          [bvId]
        )
      );
    ...
    beforeAll(async () => {
      ...
      exceptionLedgerService =
        await import('../../../../services/finance/canonical/exceptionLedgerService.js');
      financeV2Router = (await import('../index.js')).default;

      await withPinnedPostgresTransaction((tx) =>
        tx.queryRun(`INSERT INTO organizations (id, name) VALUES (?, ?), (?, ?)`, [
          orgA,
          'Crosscutting Tenant A',
          orgB,
          'Crosscutting Tenant B',
        ])
      );
      ...
      const stmtVersionAfterApprove = await approveDraft(
        orgA,
        stmt.businessVersion.business_version_id,
        stmt.businessVersion.version,
        approverA
      );
      ...
      if (!edge.ok)
        throw new Error(`fixture setup: STATEMENT_TO_ANALYSIS edge insert failed: ${edge.code}`);
      ...
      const reopened = await av.reopenVersion({
        organizationId: orgA,
        businessVersionId: stmt.businessVersion.business_version_id,
        actorId: approverA,
        role: 'approver',
        expectedVersion: stmtVersionAfterApprove,
        reason: 'J1 freshness-events fixture',
      });
      if (!reopened.ok)
        throw new Error(`fixture setup: reopen failed: ${reopened.code} ${reopened.message}`);
      ...
      const reapproveVersion = await approveDraft(
        orgA,
        childBvId,
      ...
      const ledgerRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ id: string }>(
          `SELECT id FROM finance_lineage_freshness_events WHERE organization_id = ? AND target_version_id = ?`,
          [orgA, analysisBvId]
        )
      );
      if (!ledgerRow)
        throw new Error(
          'fixture setup: expected finance_lineage_freshness_events row not found — propagation did not fire'
        );
      ledgerRowId = ledgerRow.id;

      // --- exceptions/inbox fixture: one real tie_out_fail exception.
      const inboxArtifact = await av.createArtifact({
        organizationId: orgA,
        artifactType: 'STATEMENT_PACK',
        createdBy: userA,
      });
    ...
    it('GET /versions/:id/freshness-events — real propagation ledger row round-trips through HTTP, matches independent SQL read', async () => {
      const res = await request(appA).get(
        `/api/v8/finance-v2/versions/${analysisBvId}/freshness-events`
      );
      ...
      const sqlRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ reason_code: string; previous_state: string; new_state: string }>(
          `SELECT reason_code, previous_state, new_state FROM finance_lineage_freshness_events WHERE id = ?`,
          [ledgerRowId]
        )
      );
      ...
      const analysisRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ freshness: string; freshness_reason: string }>(
          `SELECT freshness, freshness_reason FROM finance_business_versions WHERE business_version_id = ?`,
          [analysisBvId]
        )
      );
    ...
    it('GET /versions/:id/freshness-events?limit=not-a-number — non-numeric limit -> 400 INVALID_QUERY', async () => {
      const badRes = await request(appA).get(
        `/api/v8/finance-v2/versions/${analysisBvId}/freshness-events?limit=not-a-number`
      );
    ...
    it('GET /exceptions/inbox — real tie_out_fail row surfaces with the correct category/severity/deep link, SQL confirms the raw exception row', async () => {
      const res = await request(appA).get(
        `/api/v8/finance-v2/exceptions/inbox?artifactId=${inboxArtifactId}`
      );
      expect(res.status).toBe(200);
      expect(res.body.data).toHaveLength(1);
      ...
      const sqlRow = await withPinnedPostgresTransaction((tx) =>
        tx.queryOne<{ severity: string; reason_code: string; state: string }>(
          `SELECT severity, reason_code, state FROM finance_exceptions_current WHERE exception_group_id = ?`,
          [exceptionGroupId]
        )
      );
