...
describe.sequential(
  'finance-value.routes.ts — requireActiveMembership wall (real Postgres, real auth)',
  () => {
    let pool: Pool;
    ...
    async function insertMembership(userId: string, organizationId: string, status: string) {
      await pool.query(
       ...
       VALUES($1,$2,$3,'ADMIN',$4,$5)`,
        [
          `${prefix}-${userId}-${organizationId}-membership`,
          organizationId,
          userId,
          status,
    ...
    afterAll(async () => {
      ...
      try {
        ...
        expect(trgProbe.rowCount, 'exact append-only guard count').toBe(
          APPEND_ONLY_TRIGGERS.length
        );
        for (const row of trgProbe.rows) {
          expect(row.tgenabled, `guard ${row.tgname} must be ENABLED (O) BEFORE teardown`).toBe(
            'O'
          );
        }
        ...
        await cleanupClient.query(
          `DELETE FROM finance_post_investment_reviews WHERE organization_id IN ($1,$2)`,
          [org, foreignOrg]
        );
        await cleanupClient.query(
          `DELETE FROM roi_realized_values WHERE organization_id IN ($1,$2)`,
          [org, foreignOrg]
        );
        await cleanupClient.query(
          `DELETE FROM value_capture_gates WHERE organization_id IN ($1,$2)`,
          [org, foreignOrg]
        );
        await cleanupClient.query(
          `DELETE FROM value_ledger_entries WHERE organization_id IN ($1,$2)`,
          [org, foreignOrg]
        );
        await cleanupClient.query(`DELETE FROM value_baselines WHERE organization_id IN ($1,$2)`, [
          org,
          foreignOrg,
        ]);
        await cleanupClient.query(`DELETE FROM financial_model_versions WHERE model_id=$1`, [
          modelId,
        ]);
        ...
        await cleanupClient.query(`DELETE FROM users WHERE organization_id IN ($1,$2)`, [
          org,
          foreignOrg,
        ]);
        await cleanupClient.query(`DELETE FROM organizations WHERE id IN ($1,$2)`, [
          org,
          foreignOrg,
        ]);
    ...
    for (const w of writers) {
      describe.sequential(`membership wall — ${w.label}`, () => {
        ...
        it('denies a caller whose JWT claims membership in `org` they do not hold, even though genuinely ACTIVE in foreignOrg', async () => {
          ...
          expect(
            res.status,
            `${w.label}: foreign-claim status must be exactly ${w.foreignClaim.status}`
        ...
        it('fails CLOSED (503) when the membership lookup itself fails, writes nothing, and recovers on the next request', async () => {
          ...
          spy.mockImplementation(async (...args: Parameters<typeof DbPromise.get>) => {
            ...
            if (sql.includes('from organization_members') && hits < 1) {
              hits += 1;
              throw new Error(
                'bounded membership lookup failure (financeValue.membershipGate.pg.test)'
              );
            }
        ...
        it('same signed token is denied on the NEXT request after membership is revoked (no per-token caching)', async () => {
          ...
          expect(
            await w.countBusinessRows(org),
            'the revoked-token retry must not add a second row'
          ).toBe(before + 1);
        });
      });
    }
    ...
    describe.sequential(
      'POST /post-investment-reviews — active success + body-spoof isolation',
      () => {
    ...
    describe.sequential('membership wall — POST /capture/gates/:id/advance (advanceGate)', () => {
      ...
      it('denies a caller whose JWT claims membership in `org` they do not hold', async () => {
        ...
        expect(res.status, 'foreign-claim advance must be exactly 404 (tenant-scoped miss)').toBe(
          404
        );
    ...
    // =========================================================================
    describe.sequential(
      'reads and calculators remain reachable without an ACTIVE membership row',
      () => {
        it('GET /ledger/baselines/active succeeds for a caller with NO membership row', async () => {
          const res = await request(app)
            .get(`${BASE}/ledger/baselines/active`)
            .query({
              initiativeId: `${prefix}-nonexistent-init`,
              kpiId: `${prefix}-nonexistent-kpi`,
            })
            .set(bearer(noMembershipUser, org));
          expect(res.status).toBe(200);
