/**
 * LANE B-FRESH W2 PHASE 2 — finance-planning.routes.ts has ZERO business
 * writers (and, unlike finance-intelligence.routes.ts, ZERO READ handlers
 * too — all 17 handlers are pure-function CALCULATOR endpoints; see the
 * file's own header: "All 6 engines are pure functions (no DB, no I/O...)").
 * This suite exists to LOCK THAT IN against a REAL Postgres, not to fake a
 ...
 * file's handler classification at 17 = 0 READ + 17 CALCULATOR + 0
 ...
 * — see that file's header for the full precedent citations):
...
if (!REAL_DB_REQUESTED) {
  throw new Error(
    '[financePlanning.membershipGate.pg.test] Refusing to proceed: requires RUN_DB_TESTS=1, ' +
...
if (process.env.JWT_SECRET !== CANONICAL_JWT_SECRET) {
  throw new Error(
    `[financePlanning.membershipGate.pg.test] JWT_SECRET must be exactly "${CANONICAL_JWT_SECRET}" ` +
...
const SUITE_LOCK_NAME = 'finance-planning-membership-gate-lockin';

const prefix = `finplan-${randomUUID().slice(0, 8)}`;
...
describe.sequential('finance-planning.routes.ts — zero-writer lock-in (real Postgres)', () => {
  let pool: Pool;
  ...
  beforeAll(async () => {
    ...
    if (!serverDb.startsWith(REQUIRED_DB_PREFIX)) {
      throw new Error(
        `[financePlanning.membershipGate.pg.test] refusing to run: server-side ` +
    ...
    const router = (await import('../finance-planning.routes.js')).default;
    ...
    // to prove finance-planning's OWN handlers never consult
    ...
    app.use('/api/v8/finance-planning', router);
  }, 120_000);
  ...
  it('the compiled router has exactly 17 route handlers (matches the pinned static denominator)', () => {
    expect(routerHandlerCount).toBe(17);
  });

  // Representative CALCULATOR handlers — this file has ZERO READ handlers
  // (0 GET routes exist; see financeV8MutationInventory.test.ts), so unlike
  // finance-intelligence there is no READ handler to pick here. Picked from
  // two different underlying engines (cashForecastService,
  // rollingForecastService) to span more than one of the file's 6 clusters.
  it('POST /cash-forecast (CALCULATOR) succeeds for a caller with NO membership row', async () => {
    const res = await request(app)
      .post('/api/v8/finance-planning/cash-forecast')
      .set(bearer(noMembershipUser, org))
      .send({ openingCash: 10000, periods: [{ inflows: 2000, outflows: 1500 }] });
    expect(res.status).toBe(200);
    expect(res.body?.data?.forecast).toBeDefined();
  });

  it('POST /rolling-forecast/reforecast (CALCULATOR) succeeds for a REVOKED-membership caller', async () => {
    const res = await request(app)
      .post('/api/v8/finance-planning/rolling-forecast/reforecast')
      .set(bearer(revokedUser, org))
      .send({
        plan: [{ period: '2026-01', value: 100 }],
        actuals: [{ period: '2026-01', value: 90 }],
      });
    expect(res.status).toBe(200);
  ...
  it('exercising this file\'s handlers writes ZERO rows to financial_models / financial_analyses / value_baselines', async () => {
    ...
    await request(app)
      .post('/api/v8/finance-planning/cash-forecast')
      .set(bearer(noMembershipUser, org))
      .send({ openingCash: 5000, periods: [{ inflows: 100, outflows: 50 }] });
    await request(app)
      .post('/api/v8/finance-planning/rolling-forecast/reforecast')
      .set(bearer(revokedUser, org))
