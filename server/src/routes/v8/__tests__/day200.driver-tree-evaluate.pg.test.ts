...
describe.sequential('day200: DriverTreePanel — POST /driver-tree/evaluate on the real Gateway V8 stack (real Postgres)', () => {
  let pool: Pool;
  ...
  it('returns 200 with the exact node/formula shape DriverTreePanel.tsx sends, and computes the real value', async () => {
    ...
    expect(res.body?.data?.order).toEqual(expect.arrayContaining(['customers', 'arpu', 'revenue']));
