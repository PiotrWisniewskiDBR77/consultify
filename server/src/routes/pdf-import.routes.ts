...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const id = uuidv4();
    await dbRun(
      `INSERT INTO pdf_imports (id, organization_id, user_id, filename, target_type, project_id, status, created_at)
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json(
      (await dbAll(
        `SELECT id, filename, target_type, status, page_count, created_at
    FROM pdf_imports WHERE organization_id = ? ORDER BY created_at DESC LIMIT 50`,
        [orgId]
      )) || []
    );
  })
...
router.get(
  ...
  isAuthenticated,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const record = await dbAll(
      `SELECT id, filename, status, page_count, error_message
    FROM pdf_imports WHERE id = ?`,
      [req.params.id]
    );
