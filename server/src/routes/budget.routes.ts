...
router.post('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  `, [id, orgId, projectId, category || 'general', plannedAmount || 0, currency || 'USD', periodStart, periodEnd]);
...
router.put('/:id', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (plannedAmount !== undefined) { updates.push('planned_amount = ?'); params.push(plannedAmount); }
  if (actualAmount !== undefined) { updates.push('actual_amount = ?'); params.push(actualAmount); }
