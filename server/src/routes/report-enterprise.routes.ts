...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const schema = z.object({
      ...
      citationPolicy: z.enum(['required', 'recommended', 'optional']).optional(),
    });
    ...
    const pack = await reportEnterpriseService.createSourcePack(identity.orgId, identity.userId, {
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    let item;
    try {
      item = await reportEnterpriseService.addSourcePackItem(
        identity.orgId,
        req.params.packId,
        parsed.data
      );
...
router.post(
  '/reports/:reportId/data-bindings',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const schema = z.object({
      sectionId: z.string().min(1),
      bindingType: z.enum(['kpi', 'finance', 'custom']).optional(),
    ...
    const binding = await reportEnterpriseService.createDataBinding(identity.orgId, {
      reportId: req.params.reportId,
      ...parsed.data,
    });
...
router.post(
  '/data-bindings/:bindingId/refresh',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    ...
    const ok = await reportEnterpriseService.refreshDataBinding(
      identity.orgId,
...
router.post(
  '/data-bindings/:bindingId/approve',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const ok = await reportEnterpriseService.approveDataBinding(
      identity.orgId,
      req.params.bindingId,
      identity.userId
    );
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const bindings = await reportEnterpriseService.getDataBindings(
      identity.orgId,
      req.params.reportId
    );
    res.json({ bindings });
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const tpl = await reportEnterpriseService.createTemplate(
      identity.orgId,
      identity.userId,
      parsed.data
    );
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const ok = await reportEnterpriseService.publishTemplate(
      identity.orgId,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const policy = await reportEnterpriseService.createBrandVoicePolicy(
      identity.orgId,
      identity.userId,
      parsed.data
    );
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = await reportEnterpriseService.validateAgainstBrandVoice(
      identity.orgId,
      parsed.data.text
    );
    res.json(result);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const proposal = await reportEnterpriseService.createAIProposal(identity.orgId, {
      reportId: req.params.reportId,
      ...parsed.data,
    });
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const proposals = await reportEnterpriseService.getAIProposals(
      identity.orgId,
      req.params.reportId,
      status
    );
...
router.post(
  '/reports/:reportId/distribution-schedules',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const schema = z.object({
    ...
    const schedule = await reportEnterpriseService.createDistributionSchedule(
      identity.orgId,
      identity.userId,
      { reportId: req.params.reportId, ...parsed.data }
    );
...
router.post(
  '/distribution-schedules/:scheduleId/approve',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const ok = await reportEnterpriseService.approveDistribution(
      identity.orgId,
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const schedules = await reportEnterpriseService.getDistributionSchedules(
      identity.orgId,
      req.params.reportId
    );
    res.json({ schedules });
...
router.get(
  '/distribution-schedules/:scheduleId/log',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const identity = requireUser(req, res);
    if (!identity) return;
    const logs = await reportEnterpriseService.getDistributionLog(
      identity.orgId,
      req.params.scheduleId
    );
    res.json({ logs });
