...
router.get('/status', verifyToken, isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[MFA] Failed to get MFA status:', error);
    res.status(500).json({ error: 'Failed to get MFA status', message: error.message });
  }
});
...
router.post('/setup', verifyToken, isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[MFA] Failed to setup MFA:', error);
    res.status(500).json({ error: 'Failed to setup MFA', message: error.message });
  }
});
...
router.post('/verify-setup', verifyToken, isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[MFA] Failed to verify MFA setup:', error);
    res.status(500).json({ error: 'Failed to verify MFA setup', message: error.message });
  }
});
...
router.post('/disable', verifyToken, isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[MFA] Failed to disable MFA:', error);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    `,
      ...
      logger.error('[MFA] Failed to regenerate backup codes:', error);
      res.status(500).json({ error: 'Failed to regenerate backup codes', message: error.message });
    }
  }
);
