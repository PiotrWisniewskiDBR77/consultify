...
router.get('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  res.json(await dbAll(`SELECT id, title, description, source, priority, status, estimated_impact, created_at
    FROM generated_initiatives WHERE organization_id = ? ORDER BY priority DESC, created_at DESC`, [orgId]) || []);
}));

router.post('/generate', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const orgId = req.user?.organizationId;
  const userId = req.user?.id;
  const { source, context, assessmentId } = req.body;
  const id = uuidv4();
  await dbRun(`INSERT INTO generated_initiatives (id, organization_id, title, description, source, priority, status, assessment_id, created_by, created_at)
