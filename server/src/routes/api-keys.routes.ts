...
router.post('/', verifyToken, verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const expiresAt = expiresInDays
    ? new Date(Date.now() + expiresInDays * 86400000).toISOString() : null;
  ...
  `, [id, orgId, name, keyHash, keyPrefix, JSON.stringify(permissions || ['read']), expiresAt, userId]);

  logger.info(`[APIKeys] Created key: ${name} for org ${orgId}`);
  // Show full key only once at creation
  res.status(201).json({
    success: true, id, apiKey, name, keyPrefix,
...
router.put('/:id', verifyToken, verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  await dbRun(`UPDATE api_keys SET ${updates.join(', ')} WHERE id = ? AND organization_id = ?`, params);
...
router.delete('/:id', verifyToken, verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const orgId = req.user?.organizationId;
  await dbRun('UPDATE api_keys SET is_active = 0, updated_at = datetime(\'now\') WHERE id = ? AND organization_id = ?', [id, orgId]);
  logger.info(`[APIKeys] Revoked key ${id}`);
  res.json({ success: true });
}));

router.get('/:id/usage', verifyToken, verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
