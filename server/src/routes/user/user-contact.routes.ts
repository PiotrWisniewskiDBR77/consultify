...
router.get('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const contact = await dbGet('SELECT phone, address, city, country, postal_code, linkedin, website FROM user_contact WHERE user_id = ?', [req.user?.id]);
  res.json(contact || {});
}));
...
router.put('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  await dbRun(`INSERT INTO user_contact (user_id, phone, address, city, country, postal_code, linkedin, website, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
    ON CONFLICT(user_id) DO UPDATE SET phone=?, address=?, city=?, country=?, postal_code=?, linkedin=?, website=?, updated_at=datetime('now')`,
    [req.user?.id, phone, address, city, country, postalCode, linkedin, website,
      phone, address, city, country, postalCode, linkedin, website]);
