...
router.get('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  res.json(await dbAll(`SELECT id, title, description, target_date, progress_pct, status, created_at
    FROM user_goals WHERE user_id = ? ORDER BY target_date ASC`, [req.user?.id]) || []);
}));
...
router.post('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const id = uuidv4();
  await dbRun(`INSERT INTO user_goals (id, user_id, title, description, target_date, progress_pct, status, created_at)
    VALUES (?, ?, ?, ?, ?, 0, 'active', datetime('now'))`, [id, req.user?.id, title, description || '', targetDate]);
...
router.delete('/:id', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  await dbRun('DELETE FROM user_goals WHERE id = ? AND user_id = ?', [req.params.id, req.user?.id]);
  res.json({ success: true });
}));
