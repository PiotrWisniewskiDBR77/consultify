...
router.get('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const settings = await dbGet('SELECT shortcuts FROM user_keyboard_shortcuts WHERE user_id = ?', [req.user?.id]);
...
router.put('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  await dbRun(`INSERT INTO user_keyboard_shortcuts (user_id, shortcuts, updated_at) VALUES (?, ?, datetime('now'))
    ON CONFLICT(user_id) DO UPDATE SET shortcuts = ?, updated_at = datetime('now')`, [req.user?.id, json, json]);
