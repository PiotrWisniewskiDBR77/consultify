...
router.post('/upload', verifyToken, isAuthenticated, upload.single('file'), asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  logger.info(`[FinanceStatements] Uploaded ${file.originalname} → statement ${statementId} type=${detection.statementType}`);
...
router.post('/:id/detect', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const stmt = await getStatementOrFail(req.params.id, res);
  if (!stmt) return;

  const text = stmt.notes || '';
  if (!text) return res.status(400).json({ error: 'No extracted text available — re-upload the PDF' });
  ...
  await dbRun(
    ...
    [detection.statementType === 'UNKNOWN' ? stmt.statement_type : detection.statementType,
     detection.periodStart, detection.periodEnd, detection.periodLabel,
     detection.currency, detection.scaling, detection.confidence, req.params.id]
  );

  res.json({ statementId: req.params.id, detection });
}));

router.post('/:id/extract', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const stmt = await getStatementOrFail(req.params.id, res);
  if (!stmt) return;
...
router.post('/:id/map', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const stmt = await getStatementOrFail(req.params.id, res);
  if (!stmt) return;
...
router.put('/:id/values', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const stmt = await getStatementOrFail(req.params.id, res);
  if (!stmt) return;
  ...
  await updateStatementStatus(req.params.id, 'mapped', validationResult.status, validationResult.messages);
...
router.post('/:id/validate', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const stmt = await getStatementOrFail(req.params.id, res);
  if (!stmt) return;
...
router.post('/:id/confirm', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const stmt = await getStatementOrFail(req.params.id, res);
  if (!stmt) return;
...
router.get('/canonical-lines', verifyToken, isAuthenticated, asyncHandler(async (_req: AuthRequest, res: Response) => {
...
router.delete('/:id', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (stmt.status === 'confirmed') {
    return res.status(400).json({ error: 'Cannot delete confirmed statement. Archive it instead.' });
  }
...
router.get('/ratios/catalog', verifyToken, isAuthenticated, asyncHandler(async (_req: AuthRequest, res: Response) => {
  res.json(getRatioCatalog());
}));

router.get('/:id/ratios', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const orgId = req.user?.organizationId;
  try {
...
router.post('/ratios/growth', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.put('/benchmarks', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const orgId = req.user?.organizationId;
  const { ratioCode, industry, region, companySize, periodYear, p25, median, p75, targetMin, targetMax, sourceLabel } = req.body;
  if (!ratioCode) return res.status(400).json({ error: 'ratioCode required' });

  const id = await upsertBenchmark({
    organizationId: orgId!,
    ratioCode, industry, region, companySize, periodYear,
    p25, median, p75, targetMin, targetMax, sourceLabel,
...
async function getStatementOrFail(id: string, res: Response): Promise<any | null> {
  ...
  if (!rows?.length) { res.status(404).json({ error: 'Statement not found' }); return null; }
  return rows[0];
}
