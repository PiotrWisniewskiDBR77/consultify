...
if (typeof user_availabilityRoutesJS === 'function') {
  ...
  router.use(user_availabilityRoutesJS as unknown as unknown as unknown as RequestHandler);
} else if (
  user_availabilityRoutesJS &&
  typeof (user_availabilityRoutesJS as { handle?: unknown }).handle === 'function'
) {
  // If it's a router function, use it
  router.use(user_availabilityRoutesJS as unknown as unknown as unknown as RequestHandler);
} else {
