...
const GOOGLE_REDIRECT_ENV =
  process.env.GOOGLE_OIDC_REDIRECT_URI || 'http://localhost:3000/api/sso/google/callback';
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    await dbRun(`UPDATE sso_configs SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [
      newStatus,
      id,
    ]);
