 ...
 * and deletes organization/user/membership fixtures and briefly renames
 * `organization_members` to simulate a database failure.
 */
...
mountedSuite('Audits — ACTIVE membership enforced on all four mounts (real Gateway, real HS256)', () => {
  ...
  const tokenActive = () =>
    sign({ id: USER_ACTIVE, email: `${USER_ACTIVE}@local`, organizationId: ORG, organization_id: ORG, role: 'OWNER' });
  const tokenNoMembership = () =>
    sign({ id: USER_NO_MEMBERSHIP, email: `${USER_NO_MEMBERSHIP}@local`, organizationId: ORG, organization_id: ORG, role: 'MEMBER' });
  const tokenForeign = () =>
    sign({ id: USER_ACTIVE, email: `${USER_ACTIVE}@local`, organizationId: FOREIGN_ORG, organization_id: FOREIGN_ORG, role: 'OWNER' });
  ...
  /** Rows that must not appear as a side effect of any denied request. */
  async function auditWriteCount(): Promise<number> {
    const row = await auditsDb.auditGet<{ n: string }>(
      `SELECT ((SELECT count(*) FROM audit_programs WHERE organization_id = ANY($1)) +
               (SELECT count(*) FROM audit_packs    WHERE organization_id = ANY($1)))::text AS n`,
      [[ORG, FOREIGN_ORG]],
    );
  ...
  beforeAll(async () => {
    ...
    for (const org of [ORG, FOREIGN_ORG]) {
      await auditsDb.auditRun(`INSERT INTO organizations (id) VALUES ($1) ON CONFLICT DO NOTHING`, [org]);
    }
    for (const user of [USER_ACTIVE, USER_NO_MEMBERSHIP, USER_SUPERADMIN]) {
      await auditsDb.auditRun(`INSERT INTO users (id) VALUES ($1) ON CONFLICT DO NOTHING`, [user]);
    }
  ...
  it.each(MOUNT_PROBES)('$mount — a user with NO membership row is refused', async ({ path }) => {
    const res = await request(app).get(path).set('Authorization', `Bearer ${tokenNoMembership()}`);
  ...
  it('a token claiming a FOREIGN tenant cannot read that tenant\'s data (claim is server-resolved, not trusted)', async () => {
    ...
    const foreignProgramId = `aprog_mount_foreign_${RUN}`;
    ...
    await auditsDb.auditRun(
      ...
      [foreignProgramId, FOREIGN_ORG, foreignProgramName, 'aud-mount-foreign-seeder'],
    );
    ...
    try {
      ...
      // Whatever the status, the foreign tenant's row must not appear.
      expect(JSON.stringify(res.body)).not.toContain(foreignProgramName);
      expect(JSON.stringify(res.body)).not.toContain(foreignProgramId);
    } finally {
      await auditsDb.auditRun(`DELETE FROM audit_programs WHERE id = $1`, [foreignProgramId]);
    }
  });
  ...
  it('a caller with no ACTIVE membership anywhere cannot escalate by claiming any org', async () => {
    ...
    for (const org of [ORG, FOREIGN_ORG]) {
      ...
      const res = await request(app).get('/api/audits/packs').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(403);
      expect(res.body).toMatchObject({ code: 'ORG_MEMBERSHIP_REVOKED' });
    }
  });
  ...
  it.each(MOUNT_PROBES)(
    ...
    async ({ path }) => {
      await setMembership('ACTIVE');
      // Real fault injection: make the lookup genuinely fail by removing the
      // relation the guard queries. Reversible, and only ever on the declared
      // disposable database.
      await assertDisposableDatabase();
      await auditsDb.auditRun(`ALTER TABLE organization_members RENAME TO organization_members_hidden`);
      try {
        const res = await request(app).get(path).set('Authorization', `Bearer ${tokenActive()}`);
        expect(res.status).toBe(403);
        expect(res.body).toMatchObject({ code: 'ORG_MEMBERSHIP_REVOKED' });
      } finally {
        await auditsDb.auditRun(`ALTER TABLE organization_members_hidden RENAME TO organization_members`);
      }
      // The guard recovers once the table is readable again.
      const recovered = await request(app).get(path).set('Authorization', `Bearer ${tokenActive()}`);
      expect(recovered.body?.code).not.toBe('ORG_MEMBERSHIP_REVOKED');
    },
  );

  it('none of the denied requests created any audit row (zero mutation)', async () => {
    const before = await auditWriteCount();
    for (const { path } of MOUNT_PROBES) {
      await request(app).get(path).set('Authorization', `Bearer ${tokenNoMembership()}`);
      await request(app).get(path).set('Authorization', `Bearer ${tokenSuperAdminNoMembership()}`);
      await request(app).get(path).set('Authorization', `Bearer ${tokenForeign()}`);
    }
    expect(await auditWriteCount()).toBe(before);
  });
});
