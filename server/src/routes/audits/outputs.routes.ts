  ...
  route('GET /outputs/diff', async (req, res) => {
    ...
    if (!a || !b) {
      res
        ...
        .json({
          success: false,
          error: 'Parametry a i b są wymagane',
          code: 'AUDIT_DIFF_PARAMS_MISSING',
        });
      return;
    }
...
router.get(
  ...
  route('GET /outputs/:id', async (req, res) => {
    ...
    if (!output) {
      res
        .status(404)
        .json({ success: false, error: 'Output nie został znaleziony', code: 'AUDIT_NOT_FOUND' });
      return;
    }
  ...
  route('POST /outputs/:id/supersede', async (req, res) => {
    ...
    if (!newOutputId || typeof newOutputId !== 'string') {
      res
        ...
        .json({
          success: false,
          error: 'newOutputId jest wymagany',
          code: 'AUDIT_NEW_OUTPUT_ID_REQUIRED',
        });
      return;
    }
    const output = await outputService.supersedeOutput(
