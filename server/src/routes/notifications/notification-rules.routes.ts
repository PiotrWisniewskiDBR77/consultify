...
router.post('/', verifyToken, verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const {
    name, description, eventType, conditions, actions, priority = 0
  } = req.body;
  ...
  `, [
    id, orgId, name, description || '', eventType,
 ...
 */
router.post('/:id/toggle', verifyToken, verifyAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const orgId = req.user?.organizationId;
 ...
 */
router.get('/meta/event-types', verifyToken, asyncHandler(async (req: AuthRequest, res: Response) => {
  const eventTypes = [
