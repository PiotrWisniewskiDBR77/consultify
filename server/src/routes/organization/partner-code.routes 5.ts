    ...
    try {
        ...
        return res.json({
            ...
            data: {
                ...
                discountMonths: discount?.end_date ? Math.ceil(
                    (new Date(discount.end_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24 * 30)
                ) : null,
...
router.post('/partner-code', authenticateToken, async (req: Request, res: Response) => {
    try {
        ...
        const existingAttribution = await partnerReferralService.getAttributionByOrganization(organizationId);
        if (existingAttribution) {
            return res.status(400).json({
                success: false,
                error: 'Organization already has a partner attribution. Remove it first to apply a new code.',
            });
        }
        ...
        // Log the action
        logger.info(`Partner code ${partnerCode} applied to organization ${organizationId} by user ${userId}`);
...
router.delete('/partner-code', authenticateToken, async (req: Request, res: Response) => {
    try {
        ...
        await partnerReferralService.updateAttributionStatus(
            existingAttribution.id,
            'EXPIRED'
        );
        ...
        // Log the action
        logger.info(`Partner attribution removed from organization ${organizationId} by user ${userId}`);
