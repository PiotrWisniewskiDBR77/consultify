...
const ensureTable = async () => {
  await dbRun(`
    ...
    `);
  await dbRun(
    `CREATE INDEX IF NOT EXISTS idx_approved_domains_org ON approved_domains(organization_id)`
  );
};
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    try {
      await dbRun(`DELETE FROM approved_domains WHERE id = ? AND organization_id = ?`, [
        domainId,
        orgId,
      ]);
