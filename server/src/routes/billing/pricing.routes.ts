 ...
 */
router.post('/', verifyToken, verifySuperAdmin, asyncHandler(async (req: Request, res: Response) => {
  const {
    name, description, priceMonthly, priceYearly, currency = 'USD',
    features, limits, displayOrder
  } = req.body;
  ...
  `, [
    id, name, description || '', priceMonthly, priceYearly || priceMonthly * 10,
    currency, JSON.stringify(features || []), JSON.stringify(limits || {}),
    displayOrder || 0
  ]);
 ...
 */
router.put('/:id', verifyToken, verifySuperAdmin, asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const {
    name, description, priceMonthly, priceYearly, currency,
    features, limits, isActive, displayOrder
  } = req.body;
  ...
  if (priceMonthly !== undefined) { updates.push('price_monthly = ?'); params.push(priceMonthly); }
  if (priceYearly !== undefined) { updates.push('price_yearly = ?'); params.push(priceYearly); }
 ...
 */
router.delete('/:id', verifyToken, verifySuperAdmin, asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
 ...
 */
router.get('/compare/all', asyncHandler(async (req: Request, res: Response) => {
  const tiers = await dbAll(`
