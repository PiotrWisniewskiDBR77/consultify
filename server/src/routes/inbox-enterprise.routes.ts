...
router.post('/connectors/ingest', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ sourceChannel: z.string(), sourceId: z.string().optional(), payloadJson: z.string().optional(), targetUserId: z.string().optional(), senderEmail: z.string().optional(), senderName: z.string().optional(), subject: z.string().optional() });
...
router.post('/connectors/:itemId/route', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.get('/routing-rules', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const channel = req.query.channel as string | undefined;
  res.json({ rules: await inboxEnterpriseService.listRoutingRules(id.orgId, channel) });
}));

router.put('/routing-rules/:ruleId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json(await inboxEnterpriseService.updateRoutingRule(id.orgId, req.params.ruleId, req.body));
}));

router.delete('/routing-rules/:ruleId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.get('/focus/boards', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json({ boards: await inboxEnterpriseService.getFocusBoards(id.userId, id.orgId) });
}));

router.put('/focus/boards/:boardId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json(await inboxEnterpriseService.updateFocusBoard(req.params.boardId, id.userId, req.body));
}));

router.post('/focus/boards/:boardId/items', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ inboxItemId: z.string().optional(), sourceEntityType: z.string().optional(), sourceEntityId: z.string().optional(), title: z.string(), priority: z.string().optional(), plannedDate: z.string().optional(), timeEstimateMinutes: z.number().optional(), sortOrder: z.number().optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
...
router.get('/focus/boards/:boardId/items', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const status = req.query.status as string | undefined;
  res.json({ items: await inboxEnterpriseService.getFocusItems(req.params.boardId, status) });
}));

router.post('/focus/items/:itemId/complete', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json(await inboxEnterpriseService.completeFocusItem(req.params.itemId));
}));

router.delete('/focus/items/:itemId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.get('/focus/templates', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.post('/triage', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ inboxItemId: z.string(), suggestedPriority: z.string().optional(), suggestedSection: z.string().optional(), suggestedAction: z.string().optional(), confidenceScore: z.number(), reasoning: z.string().optional(), originalPriority: z.string().optional(), originalSection: z.string().optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
  const r = await inboxEnterpriseService.triageInboxItem(id.orgId, p.data);
  res.status(201).json(r);
}));

router.post('/triage/:triageId/accept', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json(await inboxEnterpriseService.acceptTriage(req.params.triageId));
}));

router.post('/triage/:triageId/reject', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json(await inboxEnterpriseService.rejectTriage(req.params.triageId));
}));

router.post('/triage/:triageId/undo', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json(await inboxEnterpriseService.undoTriage(req.params.triageId));
}));

router.get('/triage/log', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.get('/triage/config', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.put('/triage/config', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ autoTriageEnabled: z.boolean().optional(), confidenceThreshold: z.number().optional(), allowedActions: z.array(z.string()).optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
...
router.get('/table', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const filters = {
...
router.get('/items/:itemId/preview', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const preview = await inboxEnterpriseService.getInboxItemPreview(id.userId, req.params.itemId);
  if (!preview) { res.status(404).json({ error: 'Item not found' }); return; }
  res.json(preview);
}));
