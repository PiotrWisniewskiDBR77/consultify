...
router.get('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  let query = 'SELECT id, title, description, url, thumbnail_url, duration, category, status, created_at FROM videos WHERE organization_id = ?';
...
router.post('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const id = uuidv4();
  await dbRun(`INSERT INTO videos (id, organization_id, title, description, url, thumbnail_url, duration, category, status, created_at)
