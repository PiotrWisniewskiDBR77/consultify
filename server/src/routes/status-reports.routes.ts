...
router.get('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  let query = 'SELECT id, project_id, title, content, health, period, created_by, created_at FROM status_reports WHERE organization_id = ?';
...
router.post('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const id = uuidv4();
  await dbRun(`INSERT INTO status_reports (id, organization_id, project_id, title, content, health, period, created_by, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
    [id, orgId, projectId, title || 'Status Report', JSON.stringify(content || {}), health || 'green', period || 'weekly', userId]);
