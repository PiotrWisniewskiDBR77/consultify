...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.status(201).json(
      await enterprisePlatformService.createConnector(id.orgId, {
        ...p.data,
        createdBy: id.userId,
      })
    );
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!id) return;
    res.json(
      await enterprisePlatformService.updateConnector(id.orgId, req.params.connectorId, req.body)
    );
  })
);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = await enterprisePlatformService.healthCheckConnector(
      id.orgId,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = await enterprisePlatformService.processQueueItem(
      id.orgId,
      req.params.itemId,
      p.data.success,
      p.data.errorMessage
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res
      .status(201)
      .json(await enterprisePlatformService.recordMetric({ ...p.data, organizationId: id.orgId }));
  })
);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      metrics: await enterprisePlatformService.getMetrics(id.orgId, req.params.metricName, since),
    });
  })
);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res
      .status(201)
      .json(await enterprisePlatformService.createSlo({ ...p.data, organizationId: id.orgId }));
  })
);
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = await enterprisePlatformService.updateSloStatus(
      id.orgId,
      req.params.sloId,
      p.data.currentPercentage,
      p.data.budgetRemaining
    );
    if (!result.ok) {
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res
      .status(201)
      .json(await enterprisePlatformService.recordTrace({ ...p.data, organizationId: id.orgId }));
  })
);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.status(201).json(
      await enterprisePlatformService.createDrDrill({
        ...p.data,
        organizationId: id.orgId,
        conductedBy: id.userId,
      })
    );
  })
);
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const s = z.object({
      status: z.string(),
      resultsJson: z.record(z.string(), z.unknown()).optional(),
    });
    const p = s.safeParse(req.body);
    if (!p.success) {
      res.status(400).json({ error: p.error.message });
    ...
    const result = await enterprisePlatformService.updateDrDrill(
      id.orgId,
      req.params.drillId,
      p.data
    );
    if (!result.ok) {
      res.status(404).json({ error: 'DR drill not found' });
      return;
    }
    res.json(result);
  })
