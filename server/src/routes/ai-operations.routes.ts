        ...
        `).catch(() => [])) as Array<{
            ...
            res.json({
                ...
                data: trends.map((t) => ({
                    ...
                    successRate:
                        t.requests && t.requests > 0 ? (((t.successful || 0) / t.requests) * 100).toFixed(2) : '100',
                })),
            });
        } catch (error: unknown) {
...
router.get(
    ...
    asyncHandler(async (_req: AuthRequest, res: Response) => {
        ...
        `).catch(() => ({ latency_ms: 0 }))) as { latency_ms?: number };
            ...
            const availability =
                metrics.total && metrics.total > 0 ? ((metrics.successful || 0) / metrics.total) * 100 : 100;
            const errorRate =
        ...
        `).catch(() => [])) as Array<{
            ...
            res.json({
                ...
                data: history.map((h) => ({
                    date: h.date,
                    availability:
                        h.total && h.total > 0 ? parseFloat((((h.successful || 0) / h.total) * 100).toFixed(2)) : 100,
