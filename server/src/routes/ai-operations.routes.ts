 ...
 */
router.get('/mission-control/status', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
        ...
        const errorRatePercent = errorRate.total && errorRate.total > 0
        ...
        res.json({
            success: true,
            data: {
                status: parseFloat(errorRatePercent) < 5 ? 'healthy' : parseFloat(errorRatePercent) < 15 ? 'degraded' : 'critical',
 ...
 */
router.get('/mission-control/providers', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
 ...
 * Get active alerts
 */
router.get('/mission-control/alerts', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
 ...
 * Resolve an alert
 */
router.post('/mission-control/alerts/:id/resolve', verifyToken, requireRole(['super_admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/performance/metrics', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            data: {
                ...
                successRate: metrics.total_requests && metrics.total_requests > 0
 ...
 */
router.get('/performance/trends', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            data: trends.map(t => ({
                ...
                successRate: t.requests && t.requests > 0
 ...
 * Get cost summary
 */
router.get('/costs/summary', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
 ...
 */
router.get('/costs/trends', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const { period = 'month' } = req.query;
 ...
 */
router.get('/costs/by-user', verifyToken, requireRole(['super_admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        ...
        const topUsers = await dbAll(`
            ...
            ORDER BY cost DESC
            LIMIT ?
        `, [parseInt(limit as string)]).catch(() => []) as Array<{
            user_id?: string;
            user_name?: string;
            email?: string;
            tokens?: number;
 ...
 */
router.get('/sla/status', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
        ...
        const availability = metrics.total && metrics.total > 0
            ? ((metrics.successful || 0) / metrics.total) * 100
            : 100;
        const errorRate = metrics.total && metrics.total > 0
 ...
 */
router.get('/sla/history', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            data: history.map(h => ({
                date: h.date,
                availability: h.total && h.total > 0
 ...
 * Get usage analytics
 */
router.get('/analytics/usage', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const { period = 'month' } = req.query;
 ...
 */
router.get('/analytics/insights', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
        ...
        if (latencyComparison.baseline && latencyComparison.baseline > 0 && latencyComparison.recent && latencyComparison.recent > latencyComparison.baseline * 1.5) {
            insights.push({
                type: 'warning',
 ...
 */
router.get('/summary', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
        const [status, costs, performance] = await Promise.all([
            ...
            `).catch(() => ({ requests_today: 0, errors_today: 0 })) as { requests_today?: number; errors_today?: number },
            dbGet(`
        ...
        const availability = status.requests_today && status.requests_today > 0
