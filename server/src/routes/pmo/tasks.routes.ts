...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    if (!orgId || !userId) { res.status(401).json({ error: 'Unauthorized' }); return; }
    ...
    if (existing) {
      res.status(409).json({ error: `Field key "${def.fieldKey}" already exists for entity type "${def.entityType}"` });
      return;
    }
    ...
    await DbPromise.run(
      `INSERT INTO task_custom_field_schemas (id, organization_id, entity_type, field_key, field_type, label, required, options_json, default_value_json, validation_json, is_active, sort_order, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
      [
        id, orgId, def.entityType, def.fieldKey, def.fieldType, def.label,
        def.required ? 1 : 0,
...
router.put(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    if (upd.label !== undefined) { sets.push('label = ?'); params.push(upd.label); }
    if (upd.required !== undefined) { sets.push('required = ?'); params.push(upd.required ? 1 : 0); }
    if (upd.options !== undefined) { sets.push('options_json = ?'); params.push(JSON.stringify(upd.options)); }
    if (upd.defaultValue !== undefined) { sets.push('default_value_json = ?'); params.push(JSON.stringify(upd.defaultValue)); }
    if (upd.validation !== undefined) { sets.push('validation_json = ?'); params.push(JSON.stringify(upd.validation)); }
    if (upd.isActive !== undefined) { sets.push('is_active = ?'); params.push(upd.isActive ? 1 : 0); }
    if (upd.sortOrder !== undefined) { sets.push('sort_order = ?'); params.push(upd.sortOrder); }
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const { initiativeId, projectId, snapshotType = 'manual' } = req.body as {
      initiativeId?: string;
      projectId?: string;
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const { initiativeId, projectId, limit = '20', offset = '0' } = req.query as {
      initiativeId?: string;
      projectId?: string;
      limit?: string;
      offset?: string;
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const snapshot = await DbPromise.get<{ tasks_json: string; initiative_id?: string; project_id?: string }>(
    ...
    const compareFields = ['title', 'status', 'priority', 'due_date', 'started_at', 'estimated_hours', 'progress', 'assignee_id'];
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const depsBySucc = new Map<string, Array<{ fromTaskId: string; type: string; lagDays: number }>>();
    ...
    const overdueCritical = result.tasks.filter(t =>
      t.isCritical && t.endDate && new Date(t.endDate) < new Date() &&
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const taskRows = await DbPromise.all<{
      ...
      started_at: string | null; due_date: string | null;
      estimated_hours: number | null; assignee_id: string | null;
    }>(
    ...
    if (taskRows.length === 0) {
      res.json({
        criticalPathLength: 0, totalTasks: 0, criticalTaskCount: 0,
        scheduleRiskPercent: 0,
        slackDistribution: { zero: 0, low: 0, medium: 0, high: 0 },
        bottlenecks: [], atRiskTasks: [],
    ...
    const cpTasks: CriticalPathTask[] = taskRows.map(t => {
      ...
      return {
        id: t.id, title: t.title, startDate: t.started_at, endDate: t.due_date,
        durationDays, status: t.status, dependencies: depsBySucc.get(t.id) ?? [],
      };
    });
    ...
    let baselineVariance: { plannedDays: number; currentDays: number; varianceDays: number } | undefined;
    try {
        ...
        if (baselineTasks.length > 0) {
          ...
          for (const bt of baselineTasks) {
            if (bt.started_at && bt.due_date) {
              ...
              if (!isNaN(s) && !isNaN(e)) {
                maxBaselineDays = Math.max(maxBaselineDays, Math.ceil((e - s) / (1000 * 60 * 60 * 24)));
              }
            }
          }
    ...
    const scheduleRiskPercent = totalTasks > 0 ? Math.round((criticalTaskCount / totalTasks) * 100) : 0;
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const { taskId, hours, date, description } = req.body as {
      taskId: string; hours: number; date: string; description?: string;
    };
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const { taskId, userId, from, to } = req.query as {
      taskId?: string; userId?: string; from?: string; to?: string;
    };
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const { taskId, userId, allocatedHours, weekStart } = req.body as {
      taskId: string; userId: string; allocatedHours: number; weekStart: string;
    };
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const { taskId, userId, weekStart, weekEnd } = req.query as {
      taskId?: string; userId?: string; weekStart?: string; weekEnd?: string;
    };
    ...
    if (weekEnd) { sql += ' AND week_start <= ?'; params.push(weekEnd); }
