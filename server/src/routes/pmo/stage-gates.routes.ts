...
router.post('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  `, [
    id, orgId, name, description || '', stageOrder || 0,
    JSON.stringify(criteria || []), requiredApprovers || 1, userId
  ]);
...
router.put('/:id', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (stageOrder !== undefined) { updates.push('stage_order = ?'); params.push(stageOrder); }
  if (criteria !== undefined) { updates.push('criteria = ?'); params.push(JSON.stringify(criteria)); }
  if (requiredApprovers !== undefined) { updates.push('required_approvers = ?'); params.push(requiredApprovers); }
 ...
 */
router.get('/project/:projectId', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.post('/project/:projectId/:gateId/approve', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.post('/project/:projectId/:gateId/reject', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const { projectId, gateId } = req.params;
  const userId = req.user?.id;
