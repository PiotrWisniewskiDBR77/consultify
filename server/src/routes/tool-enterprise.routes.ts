...
router.get('/knowledge/search', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  ...
  res.json({ results: await toolEnterpriseService.searchKnowledge(id.orgId, query, toolSessionId, limit) });
}));

router.get('/knowledge', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const toolSessionId = req.query.toolSessionId as string | undefined;
...
router.get('/rag/queries', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const toolSessionId = req.query.toolSessionId as string | undefined;
...
router.post('/entitlements', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ packKey: z.string(), packName: z.string(), licensedTools: z.array(z.string()).optional(), maxSessionsPerMonth: z.number().optional(), maxConcurrentUsers: z.number().optional(), validFrom: z.string().optional(), validUntil: z.string().optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
  res.status(201).json(await toolEnterpriseService.createEntitlement(id.orgId, p.data));
}));

router.get('/entitlements', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json({ entitlements: await toolEnterpriseService.getEntitlements(id.orgId) });
}));

router.get('/entitlements/check/:toolKey', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json(await toolEnterpriseService.checkEntitlement(id.orgId, req.params.toolKey));
}));

router.post('/usage/log', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ toolKey: z.string(), entitlementId: z.string().optional(), action: z.string().optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
  res.status(201).json(await toolEnterpriseService.logToolUsage(id.orgId, { ...p.data, userId: id.userId }));
}));

router.get('/usage/stats', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.post('/entitlements/:entitlementId/deactivate', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
