...
router.use(asyncHandler(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  if (!organizationId || !userId) return res.status(401).json({ success: false, code: 'UNAUTHORIZED' });
  ...
  if (!membership || String(membership.status).toUpperCase() !== 'ACTIVE') return res.status(403).json({ success: false, code: 'ADMIN_MEMBERSHIP_REQUIRED' });
  if (!['OWNER', 'ADMIN'].includes(String(membership.role).toUpperCase())) return res.status(403).json({ success: false, code: 'ADMIN_ACCESS_REQUIRED' });
  next();
}));
...
router.get('/', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  return res.json({ success: true, data: await serviceAccountService.listServiceAccounts(organizationId) });
}));

router.post('/', asyncHandler(async (req: AuthRequest, res: Response) => {
  const organizationId = String(req.user?.organizationId || '');
  const name = String(req.body?.name || '').trim();
  const scopes = Array.isArray(req.body?.scopes) ? req.body.scopes.map(String).filter(Boolean) : [];
  if (!name || scopes.length === 0) return res.status(400).json({ success: false, error: 'name and scopes are required' });
...
router.delete('/:id', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (result.rows.length === 0) return res.status(404).json({ success: false, error: 'Service account not found' });
