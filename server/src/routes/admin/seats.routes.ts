...
import {
  ...
  getSeatHistory,
  toggleAutoAddSeats,
} from '../../services/seatManagementService.js';
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    return res.json({
      success: true,
      transactions: await getSeatHistory(organizationId, limit),
      limit,
    });
  })
);
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!Number.isInteger(threshold) || threshold < 1 || threshold > 100)
      return res
        .status(400)
        .json({ success: false, error: 'threshold must be an integer from 1 to 100' });
