...
router.get('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  res.json(await dbAll(`SELECT id, name, frameworks, status, progress_pct, created_at
    FROM multi_framework_workflows WHERE organization_id = ? ORDER BY created_at DESC`, [orgId]) || []);
}));
...
router.post('/', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const id = uuidv4();
  await dbRun(`INSERT INTO multi_framework_workflows (id, organization_id, name, frameworks, status, progress_pct, created_at)
