...
router.get('/settings', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  const settings = await dbGet(`SELECT settings FROM daily_brief_settings WHERE user_id = ?`, [userId]);
  res.json(settings?.settings ? JSON.parse(settings.settings) : {
    enabled: true, sendTime: '08:00', includeCalendar: true, includeTasks: true,
    includeDecisions: true, email: false
  });
}));

router.put('/settings', verifyToken, isAuthenticated, asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
