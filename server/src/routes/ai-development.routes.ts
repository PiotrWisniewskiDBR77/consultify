...
interface KnowledgeServiceInterface {
    ...
    addCandidate?: (content: string, reasoning?: string, source?: string, relatedAxis?: string, originContext?: string) => Promise<string>;
 ...
 */
router.get('/prompts', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
 ...
 */
router.get('/prompts/categories', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
 ...
 */
router.get('/prompts/:id', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
 ...
 * Create new prompt
 */
router.post('/prompts', verifyToken, requireRole(['super_admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
 ...
 * Update prompt
 */
router.put('/prompts/:id', verifyToken, requireRole(['super_admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
        const { name, category, description, template, variables, is_active } = req.body;
        const userId = req.user?.id;
 ...
 */
router.post('/prompts/:id/test', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/knowledge/candidates', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.post('/knowledge/candidates', verifyToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.put('/knowledge/candidates/:id/status', verifyToken, requireRole(['super_admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.get('/knowledge/approved', verifyToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.get('/intelligence/config', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            success: true,
            data: config?.value ? JSON.parse(config.value) : {
 ...
 */
router.put('/intelligence/config', verifyToken, requireRole(['super_admin']), asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
...
router.get('/summary', verifyToken, requireRole(['super_admin', 'admin']), asyncHandler(async (_req: AuthRequest, res: Response) => {
    try {
        const [promptStats, experimentStats, knowledgeStats] = await Promise.all([
            ...
            `).catch(() => ({ total: 0, running: 0, completed: 0 })) as Promise<{ total?: number; running?: number; completed?: number }>,
            dbGet(`
                SELECT 
                    COUNT(*) as total,
            ...
            `).catch(() => ({ total: 0, approved: 0, pending: 0 })) as Promise<{ total?: number; approved?: number; pending?: number }>
        ]);
