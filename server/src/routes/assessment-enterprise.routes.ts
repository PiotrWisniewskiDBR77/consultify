...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const r = await assessmentEnterpriseService.createFinding(id.orgId, {
      ...p.data,
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const filters = {
      ...
      findingType: req.query.findingType as string,
      clauseRef: req.query.clauseRef as string,
    };
    res.json({
      findings: await assessmentEnterpriseService.getFindings(
        id.orgId,
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!id) return;
    res.json(
      await assessmentEnterpriseService.updateFinding(id.orgId, req.params.findingId, req.body)
    );
  })
);

router.post(
  '/findings/:findingId/capa',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const s = z.object({
    ...
    const r = await assessmentEnterpriseService.createCapaAction(id.orgId, {
      ...p.data,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const r = await assessmentEnterpriseService.mapEvidenceToClause(id.orgId, {
      ...p.data,
      mappedBy: id.userId,
    });
    res.status(201).json(r);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!id) return;
    const filters = {
      evidenceId: req.query.evidenceId as string,
...
router.get(
  '/evidence/clause-coverage/:frameworkId',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    res.json({
      coverage: await assessmentEnterpriseService.getClauseCoverage(
        id.orgId,
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      log: await assessmentEnterpriseService.getEvidenceAccessLog(id.orgId, evidenceId, limit),
    });
  })
);
...
router.post(
  '/assessments/:assessmentId/scoring-proposals',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const s = z.object({
    ...
    const r = await assessmentEnterpriseService.createScoringProposal(id.orgId, {
      ...p.data,
      assessmentId: req.params.assessmentId,
    });
    res.status(201).json(r);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      proposals: await assessmentEnterpriseService.getScoringProposals(
        id.orgId,
        req.params.assessmentId,
        status
      ),
...
router.post(
  '/scoring-proposals/:proposalId/review',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    ...
    res.json(
      await assessmentEnterpriseService.reviewScoringProposal(req.params.proposalId, {
        ...p.data,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const s = z.object({
      ...
      name: z.string(),
      description: z.string().optional(),
      goldenItems: z.array(z.record(z.string(), z.unknown())).optional(),
    });
    const p = s.safeParse(req.body);
    if (!p.success) {
      res.status(400).json({ error: p.error.message });
    ...
    const r = await assessmentEnterpriseService.createEvalDataset(id.orgId, {
      ...p.data,
      createdBy: id.userId,
    });
    res.status(201).json(r);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      datasets: await assessmentEnterpriseService.getEvalDatasets(id.orgId, frameworkId),
    });
  })
);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const r = await assessmentEnterpriseService.createEvalRun(id.orgId, {
      ...p.data,
      runBy: id.userId,
    });
    res.status(201).json(r);
...
router.get(
  '/eval/runs/:runIdA/compare/:runIdB',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const r = await assessmentEnterpriseService.compareEvalRuns(
      req.params.runIdA,
      req.params.runIdB
    );
    if (!r) {
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const r = await assessmentEnterpriseService.requestReview(id.orgId, {
      ...p.data,
      assessmentId: req.params.assessmentId,
    });
    res.status(201).json(r);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      reviews: await assessmentEnterpriseService.getReviews(
        id.orgId,
        req.params.assessmentId,
        versionId
      ),
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json(
      await assessmentEnterpriseService.rejectReview(req.params.reviewId, id.userId, p.data)
    );
  })
);

router.get(
  '/assessments/:assessmentId/versions/:fromVersionId/diff/:toVersionId',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const diff = await assessmentEnterpriseService.getVersionDiff(
      id.orgId,
      req.params.assessmentId,
    ...
    if (!diff) {
      res.status(404).json({ error: 'Version(s) not found' });
      return;
    }
    res.json(diff);
  })
