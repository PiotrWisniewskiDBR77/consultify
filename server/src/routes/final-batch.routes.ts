...
router.post('/ideas/:ideaId/export', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ exportType: z.string(), exportFormat: z.string(), watermarkText: z.string().optional(), includeMetadata: z.boolean().optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
  res.status(201).json(await finalBatchService.requestExport(id.orgId, { ...p.data, ideaId: req.params.ideaId, requestedBy: id.userId }));
}));

router.get('/ideas/:ideaId/exports', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json({ exports: await finalBatchService.getExports(id.orgId, req.params.ideaId) });
}));

router.post('/exports/:exportId/complete', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.get('/cohort-policy', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.put('/cohort-policy', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ minCohortSize: z.number().optional(), suppressionEnabled: z.boolean().optional(), noiseMethod: z.string().optional(), noiseMagnitude: z.number().optional(), auditAllQueries: z.boolean().optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
...
router.get('/cohort-privacy/audit', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.post('/framework-mappings', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ frameworkKey: z.string(), frameworkName: z.string(), version: z.string().optional(), dimensionKey: z.string(), dimensionName: z.string(), percentileP25: z.number().optional(), percentileP50: z.number().optional(), percentileP75: z.number().optional(), percentileP90: z.number().optional(), whatGoodLooksLike: z.string().optional(), dataSource: z.string().optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
  res.status(201).json(await finalBatchService.upsertFrameworkMapping(p.data));
}));

router.get('/framework-mappings', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.get('/gap-analyses', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const frameworkKey = req.query.frameworkKey as string | undefined;
  res.json({ analyses: await finalBatchService.getGapAnalyses(id.orgId, frameworkKey) });
}));

router.post('/gap-analyses/:gapId/initiatives', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ dimensionKey: z.string(), initiativeId: z.string() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
  res.status(201).json(await finalBatchService.linkGapToInitiative(req.params.gapId, p.data.dimensionKey, p.data.initiativeId));
}));

router.get('/gap-analyses/:gapId/initiatives', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  res.json({ links: await finalBatchService.getGapInitiativeLinks(req.params.gapId) });
}));

router.put('/gap-analyses/:gapId/status', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const s = z.object({ status: z.string(), autoInitiativesCreated: z.number().optional() });
  const p = s.safeParse(req.body); if (!p.success) { res.status(400).json({ error: p.error.message }); return; }
  res.json(await finalBatchService.updateGapAnalysisStatus(req.params.gapId, p.data.status, p.data.autoInitiativesCreated));
}));
...
router.get('/playbooks', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const domain = req.query.domain as string | undefined;
  res.json({ playbooks: await finalBatchService.getPlaybooks(domain, id.orgId) });
}));

router.get('/playbooks/:playbookId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.put('/playbooks/:playbookId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
...
router.get('/playbooks/:playbookId/executions', asyncHandler(async (req: AuthRequest, res: Response) => {
  const id = requireUser(req, res); if (!id) return;
  const limit = req.query.limit ? Number(req.query.limit) : 50;
  res.json({ executions: await finalBatchService.getPlaybookExecutions(req.params.playbookId, limit) });
}));
