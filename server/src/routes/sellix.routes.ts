  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    try {
      await adminAuditService.logAction({
        adminId,
        actionType: 'sellix_config_updated',
...
router.post(
  '/test-event',
  requireConfirmation('sellix_test_event', 'high'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { organizationId, score, tier } = req.body;
    if (!organizationId) {
      res.status(422).json({ error: 'organizationId required' });
      return;
    }
    const result = await sendReadinessSignal(
      organizationId,
      score ?? 85,
      tier ?? 'READY',
      [],
    ...
    try {
      await adminAuditService.logAction({
        ...
        actionType: 'sellix_test_event_sent',
        details: { organizationId, result },
      });
    } catch {
      /* best-effort */
    }
...
router.get(
  ...
  asyncHandler(async (_req: AuthRequest, res: Response) => {
    const [deliveries, events] = await Promise.all([
      getDeliveryStatus(20),
      getRecentInboundEvents(20),
    ]);
...
sellixWebhookRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (config?.webhookSecret) {
      ...
      if (!valid) {
        logger.warn('[Sellix] Invalid webhook signature');
