...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const r = await presentationEnterpriseService.createBinding(id.orgId, {
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!id) return;
    res.json({
      bindings: await presentationEnterpriseService.getBindings(id.orgId, req.params.deckId),
    });
  })
);

router.post(
  '/bindings/:bindingId/refresh',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const s = z.object({ valueHash: z.string() });
    const p = s.safeParse(req.body);
    if (!p.success) {
      res.status(400).json({ error: p.error.message });
      return;
    }
    const ok = await presentationEnterpriseService.refreshBinding(
      id.orgId,
...
router.post(
  '/bindings/:bindingId/approve',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const ok = await presentationEnterpriseService.approveBinding(
      id.orgId,
      req.params.bindingId,
      id.userId
    );
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.status(201).json(
      await presentationEnterpriseService.createExportQA(id.orgId, {
        deckId: req.params.deckId,
        ...p.data,
      })
    );
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (!id) return;
    res.json({
      results: await presentationEnterpriseService.getExportQAResults(id.orgId, req.params.deckId),
    });
  })
);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const s = z.object({
      ...
      consultingPackType: z.string().optional(),
    });
    const p = s.safeParse(req.body);
    if (!p.success) {
      res.status(400).json({ error: p.error.message });
    ...
    res
      .status(201)
      .json(
        await presentationEnterpriseService.createTemplateGovernance(id.orgId, id.userId, p.data)
      );
  })
);

router.post(
  '/template-governance/:governanceId/publish',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const ok = await presentationEnterpriseService.publishTemplateVersion(
      id.orgId,
      req.params.governanceId,
      id.userId
    );
    if (!ok) {
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res
      .status(201)
      .json(await presentationEnterpriseService.createPPTXImport(id.orgId, id.userId, p.data));
  })
);

router.patch(
  '/pptx-imports/:importId',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const s = z.object({
      deckId: z.string().optional(),
      mappingData: z.array(z.unknown()).optional(),
      status: z.string(),
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res
      .status(201)
      .json(
        await presentationEnterpriseService.joinCollabSession(
          id.orgId,
          req.params.deckId,
          id.userId
        )
...
router.post(
  '/collab/:sessionId/presence',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    const s = z.object({
      cursorPosition: z.record(z.string(), z.unknown()).optional(),
      activeSlideIndex: z.number().int().optional(),
    });
    const p = s.safeParse(req.body);
    if (!p.success) {
      res.status(400).json({ error: p.error.message });
...
router.get(
  '/decks/:deckId/collab/active',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const id = requireUser(req, res);
    if (!id) return;
    res.json({
      collaborators: await presentationEnterpriseService.getActiveCollaborators(
        id.orgId,
        req.params.deckId
      ),
    });
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const s = z.object({
      ...
      entitlementScope: z.string().optional(),
    });
    const p = s.safeParse(req.body);
    if (!p.success) {
      res.status(400).json({ error: p.error.message });
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      media: await presentationEnterpriseService.getMediaLibrary(id.orgId, rightsFilter),
    });
  })
);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.status(201).json(
      await presentationEnterpriseService.logMediaUsage(id.orgId, {
