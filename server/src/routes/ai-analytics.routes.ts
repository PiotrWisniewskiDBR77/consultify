...
async function getOrganizationBudget(organizationId: string): Promise<{ monthly_ai_budget?: number } | null> {
    ...
    `, [organizationId]) as Promise<{ monthly_ai_budget?: number } | null>;
}
 ...
 */
router.get('/costs', asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            totals: {
                ...totals,
                successRate: totals.total_requests && totals.total_requests > 0
            ...
            budget: budget ? {
                ...
                utilization: budget.monthly_ai_budget && budget.monthly_ai_budget > 0
        ...
        if (aiLogger?.error) {
            aiLogger.error('AIAnalytics', `costs error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
...
router.get('/usage', asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (aiLogger?.error) {
            aiLogger.error('AIAnalytics', `usage error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
 ...
 */
router.get('/quotas', asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        const organizationId = req.user?.organizationId;
        ...
        if (aiLogger?.error) {
            aiLogger.error('AIAnalytics', `quotas error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
 ...
 */
router.get('/performance', asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            contextUtilization: cacheStats && cacheStats.total && cacheStats.total > 0
        ...
        if (aiLogger?.error) {
            aiLogger.error('AIAnalytics', `performance error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
 ...
 */
router.post('/alerts/configure', asyncHandler(async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (aiLogger?.error) {
            aiLogger.error('AIAnalytics', `alerts configure error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
