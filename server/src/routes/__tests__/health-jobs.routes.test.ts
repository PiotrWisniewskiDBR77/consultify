...
const app = () => {
  const a = express();
  a.use('/api/admin/health-panel', routes);
  return a;
};
describe('health jobs route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('requires auth', async () => {
    user = null;
    expect((await request(app()).get('/api/admin/health-panel/jobs')).status).toBe(401);
  });
  it('requires admin membership', async () => {
    dbGet.mockResolvedValue({ role: 'MEMBER', status: 'ACTIVE' });
    expect((await request(app()).get('/api/admin/health-panel/jobs')).status).toBe(403);
  });
  it('uses token tenant and exposes no platform fields', async () => {
    const response = await request(app()).get(
      '/api/admin/health-panel/jobs?status=failed&limit=999'
    );
    ...
    expect(JSON.stringify(response.body)).not.toMatch(/host|cpu|memory|credential/i);
  });
});
