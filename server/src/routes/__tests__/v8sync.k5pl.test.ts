/**
 * K5pl-229 (Wpis 231 pkt 3 / Wpis 261, DEC-690) — v8/sync.routes.ts.
 *
 * POST /integrations/:id/configure and POST /integrations/:id/reauth both return
 * 501 with a stable `code: GOVERNED_CONNECTOR_NOT_APPROVED` when the governed
 * connector approval check fails. The client localizes that code via
 * apiErrorFallbacks + errors.<CODE>; the redundant Polish `error:` was removed.
 * This suite pins the wire contract (body == { code }, no Polish diacritics) and
 * the i18n parity of the code, so re-introducing a Polish `error:` fails.
 */
...
// Membership guard passthrough — the 501 fires inside the handler, not the guard.
vi.mock('../../middleware/auditsStrictMembership.middleware.js', () => ({
  requireActiveAuditsMembership: (_req: any, _res: any, next: any) => next(),
}));

// integrations row for an oauth2 connector ('gmail'); the governed-auth throw
// happens before any write, so run/get are inert.
vi.mock('../../utils/DbPromise.js', () => ({
  all: async () => [{ id: 'int-1', connector_id: 'gmail', config: '{}', status: 'pending' }],
  get: async () => undefined,
...
beforeAll(async () => {
  const router = (await import('../v8/sync.routes.js')).default;
  app = express();
  app.use(express.json());
  // Replicate attachV8Context: the real getV8Context reads req.v8Context.
...
describe('K5pl-229 v8/sync.routes GOVERNED_CONNECTOR_NOT_APPROVED', () => {
  it('POST /integrations/:id/configure → 501 body == { code } with no Polish diacritics', async () => {
  ...
  it('POST /integrations/:id/reauth → 501 body == { code } with no Polish diacritics', async () => {
