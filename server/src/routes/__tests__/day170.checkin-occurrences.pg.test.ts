...
describe(
  'Day 170 check-in occurrence picker through real ApiGateway and PostgreSQL',
  NO_RETRY,
  () => {
    let app: Express;
    let sql: Client;
    ...
    async function buildScenario(scenario: 'set-before-cycle' | 'set-after-cycle') {
      ...
      const emptyOccurrences = await get(
        owner,
        organizationId,
      ...
      const notFound = await get(
        owner,
        organizationId,
        `/key-results/${randomUUID()}/checkin-occurrences`,
        404
      );
      ...
      expect(
        listed.body.occurrences.map((row: any) => ({
          windowStart: row.windowStart,
          windowEnd: row.windowEnd,
        }))
      ).toEqual(
        rawOccurrences.rows.map((row) => ({
      ...
      evidence.push({
        scenario,
        step: 'listed occurrences before check-in',
        status: listed.status,
        response: listed.body,
        database: rawOccurrences.rows,
      });

      const checkIn = await post(
        owner,
        organizationId,
        `/key-results/${keyResultIds[0]}/check-ins`,
        {
      ...
      const listedAfter = await get(
        owner,
        organizationId,
        `/key-results/${keyResultIds[0]}/checkin-occurrences`
      );
      expect(
        listedAfter.body.occurrences.find(
          (row: any) => row.cadenceOccurrenceId === occurrenceIds[0]
        )?.used
      ).toBe(true);
