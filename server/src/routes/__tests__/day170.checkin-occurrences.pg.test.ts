...
describe('Day 170 check-in occurrence picker through real ApiGateway and PostgreSQL', NO_RETRY, () => {
  let app: Express;
  let sql: Client;
  ...
  const get = async (token: string, organizationId: string, path: string, expectedStatus = 200) => {
  ...
  async function buildScenario(scenario: 'set-before-cycle' | 'set-after-cycle') {
    ...
    await sql.query(
      ...
      [organizationId, `Day170 ${scenario}`]
    );
    for (const [userId, role] of [
      ...
      await sql.query(
        `INSERT INTO users
         ...
         VALUES ($1, $2, $3, 'x', 'Day', '170', $4, 'active', now())`,
        [userId, organizationId, `${userId}@day170.test`, role]
      );
      await sql.query(
    ...
    const emptyOccurrences = await get(owner, organizationId, `/key-results/${keyResultIds[0]}/checkin-occurrences`);
    expect(emptyOccurrences.body).toEqual({ occurrences: [] });
    ...
    const listed = await get(owner, organizationId, `/key-results/${keyResultIds[0]}/checkin-occurrences`);
    const rawOccurrences = await sql.query(
      `SELECT cadence_occurrence_id, window_start::text, window_end::text
         FROM okr_vnext_checkin_occurrences
        WHERE cycle_id = $1 ORDER BY window_start`,
      [cycle.cycleId]
    );
    ...
    const checkIn = await post(owner, organizationId, `/key-results/${keyResultIds[0]}/check-ins`, {
      ...
      newValue: 50,
      note: `Day170 ${scenario} HTTP check-in`,
      idempotencyKey: randomUUID(),
    });
    ...
    const listedAfter = await get(owner, organizationId, `/key-results/${keyResultIds[0]}/checkin-occurrences`);
    expect(listedAfter.body.occurrences.find((row: any) => row.cadenceOccurrenceId === occurrenceIds[0])?.used).toBe(true);
    const rawCheckIn = await sql.query(
      `SELECT cadence_occurrence_id FROM okr_vnext_checkins
        WHERE key_result_id = $1 AND correction_of_checkin_id IS NULL`,
      [keyResultIds[0]]
    );
    expect(rawCheckIn.rows[0].cadence_occurrence_id).toBe(occurrenceIds[0]);
