...
const ARTIFACT = '/private/tmp/cx-day326-konta-serwisowe-artefakty/service-accounts-r1-matrix.json';
...
describe(
  'Day 326 service-account matrix through real ApiGateway, JWT and PostgreSQL',
  NO_RETRY,
  () => {
    let app: Express;
    let sql: Client;
    ...
    beforeAll(async () => {
      ...
      await sql.connect();
      await sql.query('DELETE FROM tp_service_accounts WHERE organization_id = $1', [UUID_ORG]);
      await sql.query(
    ...
    it('R1 GET non-UUID pairs foreign denial with the measured owner empty response', async () => {
      expect((await sql.query("SELECT id FROM organizations WHERE id = 'system'")).rowCount).toBe(
        1
      );
      const foreign = await call(
        'get-text',
    ...
    it('R1 POST non-UUID pairs foreign denial with the gated owner response and unchanged database', async () => {
      const foreign = await call(
        'post-text',
        'foreign',
        'post',
      ...
      expect(owner.response.body).toMatchObject({
        code: 'INVALID_IDENTIFIER',
        errorCode: 'INVALID_IDENTIFIER',
        correlationId: 'day326-post-text-owner',
      });
    ...
    it('R1 DELETE non-UUID pairs foreign denial with the gated owner response and preserved UUID row', async () => {
      ...
      expect(owner.response.body).toMatchObject({
        code: 'INVALID_IDENTIFIER',
        errorCode: 'INVALID_IDENTIFIER',
        correlationId: 'day326-delete-text-owner',
      });
      expect(owner.databaseAfter).toEqual(owner.databaseBefore);
    });

    it('R2 gives every exercised in-router error branch an errorCode and correlationId', async () => {
      const probes = [
        await request(app)
          .get(`/api/admin/service-accounts?orgId=${UUID_ORG}`)
          .set('Authorization', tokenFor.foreign)
          .set('X-Correlation-ID', 'day326-boundary'),
        await request(app)
          .post('/api/admin/service-accounts')
          .set('Authorization', tokenFor.uuid)
          .set('X-Correlation-ID', 'day326-invalid-body')
          .send({}),
        await request(app)
          .delete(`/api/admin/service-accounts/${randomUUID()}`)
          .set('Authorization', tokenFor.uuid)
          .set('X-Correlation-ID', 'day326-not-found'),
      ];
      expect(probes.map((probe) => probe.status)).toEqual([403, 400, 404]);
      for (const probe of probes) {
        expect(probe.body.errorCode, JSON.stringify(probe.body)).toEqual(expect.any(String));
        expect(probe.body.correlationId, JSON.stringify(probe.body)).toEqual(expect.any(String));
      }
    });
    ...
    it('KONTRAKT DLA DYŻURU 326 — wcześniejsza odpowiedź 401 ma pełną kopertę', async () => {
      ...
      expect(response.body).toMatchObject({
        ...
        correlationId: 'day326-unauthorized',
      });
    });

    it('R2 rejects non-UUID POST and DELETE before PostgreSQL with stable envelopes', async () => {
      for (const [method, path, body] of [
        ['post', '/api/admin/service-accounts', { name: 'blocked', scopes: ['records:read'] }],
        ['delete', `/api/admin/service-accounts/${randomUUID()}`, undefined],
      ] as const) {
        const before = await rowsFor(UUID_ORG);
        const pending = request(app)
          [method](path)
          .set('Authorization', tokenFor.text)
          .set('X-Correlation-ID', `day326-r2-${method}`);
        const response = body ? await pending.send(body) : await pending;
        expect(response.status).toBe(400);
        expect(response.body).toMatchObject({
          code: 'INVALID_IDENTIFIER',
          errorCode: 'INVALID_IDENTIFIER',
          correlationId: `day326-r2-${method}`,
        });
        expect(await rowsFor(UUID_ORG)).toEqual(before);
      }
    });

    it('R2 preserves successful UUID GET POST and DELETE behavior', async () => {
      await sql.query(
        `INSERT INTO tp_service_accounts
         (organization_id, name, token_hash, token_prefix, scopes, created_by)
       VALUES ($1, 'day326-r2-existing', $2, 'day326_r2e', ARRAY['records:read'], $3)`,
        [UUID_ORG, 'd'.repeat(64), UUID_OWNER]
      );
      ...
      expect(getResponse.body.data).toEqual(
        expect.arrayContaining([expect.objectContaining({ name: 'day326-r2-existing' })])
      );

      const postResponse = await request(app)
        .post('/api/admin/service-accounts')
        .set('Authorization', tokenFor.uuid)
        .send({ name: 'day326-r2-continuity', scopes: ['records:read'] });
      expect(postResponse.status).toBe(201);
      const createdId = postResponse.body.data.account.id;
      expect(await rowsFor(UUID_ORG)).toEqual(
        expect.arrayContaining([expect.objectContaining({ id: createdId })])
      );

      const deleteResponse = await request(app)
        .delete(`/api/admin/service-accounts/${createdId}`)
        .set('Authorization', tokenFor.uuid);
      expect(deleteResponse.status).toBe(204);
      expect(await rowsFor(UUID_ORG)).not.toEqual(
        expect.arrayContaining([expect.objectContaining({ id: createdId })])
      );
    });
  }
