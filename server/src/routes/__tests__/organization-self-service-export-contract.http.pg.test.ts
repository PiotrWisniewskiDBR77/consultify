...
describe.skipIf(!enabled)('CODEX6 E4 safe and complete tenant export contract', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(`INSERT INTO ${quoteIdentifier(rootTable)}(id,organization_id,access_token,payload,metadata) VALUES ('own-root',$1,$2,'own-payload',jsonb_build_object('refresh_token',$3::text,'legal_value','kept')),('other-root',$4,'other-token','other-payload','{}')`, [orgId, sentinels.accessToken, sentinels.nestedToken, otherOrgId]);
