...
describe.skipIf(!enabled)('DAY157 provenance predicate and sandbox revert — real PG', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(`INSERT INTO organizations(id,name,plan,status) VALUES($1,'Day157','enterprise','active')`, [org]);
  ...
  it('materializes three traced rows and reverts exactly the predicate match', async () => {
    ...
    await pool.query(`INSERT INTO tasks(id,organization_id,title,source) VALUES($1,$2,'manual task','manual')`, [randomUUID(), org]);
    await pool.query(`INSERT INTO decisions(id,organization_id,title) VALUES($1,$2,'manual decision')`, [randomUUID(), org]);
    await pool.query(
    ...
    await pool.query(`DELETE FROM tasks WHERE organization_id=$1 AND source_type='myw_agent_proposal'`, [org]);
    await pool.query(`DELETE FROM decisions WHERE organization_id=$1 AND source_type='myw_agent_proposal'`, [org]);
    await pool.query(
    ...
    console.log(`DAY157_COUNTS count_before=${countBefore} count_reverted=${countReverted} count_after=${countAfter}`);
    ...
    expect((await pool.query(`SELECT count(*)::int n FROM tasks WHERE organization_id=$1`, [org])).rows[0].n).toBe(1);
    expect((await pool.query(`SELECT count(*)::int n FROM decisions WHERE organization_id=$1`, [org])).rows[0].n).toBe(1);
    expect((await pool.query(`SELECT count(*)::int n FROM notebook_pages WHERE organization_id=$1`, [org])).rows[0].n).toBe(1);
  }, 30_000);
});
