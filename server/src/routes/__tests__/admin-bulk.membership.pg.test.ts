...
const org = `${prefix}-org`,
  admin = `${prefix}-admin`,
  importedEmail = `${prefix}@test.invalid`;
...
describe.skipIf(!enabled)('admin bulk user plus membership atomic writer realPG', () => {
  ...
  it('commits user and ACTIVE membership together and cold-reads both', async () => {
    ...
    const response = await request(app)
      ...
      .send({ users: [{ email: importedEmail, role: 'MANAGER' }] });
    ...
    try {
      const rows = await cold.query(
        `SELECT u.email,u.role user_role,m.role membership_role,m.status
      ...
      expect(rows.rows).toEqual([
        { email: importedEmail, user_role: 'MANAGER', membership_role: 'MEMBER', status: 'ACTIVE' },
      ]);
    } finally {
      await cold.end();
    }
  ...
  it('keeps duplicate import failure atomic with no second user or membership', async () => {
    const before = (
      await pool.query(`SELECT count(*)::int n FROM users WHERE organization_id=$1`, [org])
    ).rows[0].n;
    const bearer = jwt.sign(
      {
        id: admin,
    ...
    const response = await request(app)
      ...
      .send({ users: [{ email: importedEmail, role: 'MANAGER' }] });
    expect(response.status).toBe(200);
    expect(response.body).toMatchObject({ success: 0, failed: 1 });
    const paired = await pool.query(
      `SELECT count(*)::int n FROM users u JOIN organization_members m ON m.user_id=u.id WHERE u.organization_id=$1 AND u.email=$2`,
      [org, importedEmail]
    );
