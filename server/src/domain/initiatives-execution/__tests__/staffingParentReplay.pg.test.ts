...
describe('CODEX2B staffing parent replay boundary', { retry: 0, sequential: true }, () => {
  ...
  for (const channel of ['legacy', 'native'] as const) {
    it(
      `${channel} role create replay cannot claim a child after its parent was deleted`,
      { retry: 0 },
      async () => {
        process.env.ENABLE_INITIATIVE_UNIFIED_WRITE = 'true';
        const id = await createPlan(`Replay parent ${channel}`);
        const roleId = `replay-role-${channel}-${run}`;
        const runtime = `/api/initiatives/runtime-v1/initiatives/${initiative}/staffing-plans/${id}`;
        const send = () =>
          channel === 'legacy'
            ? request(app)
                .post(`${base}/${id}/roles`)
                .set(auth())
                .set('Idempotency-Key', `role-replay-${channel}-${run}`)
                .send({ roleName: 'Replay child' })
            : request(app)
                .post(`${runtime}/roles/${roleId}`)
                .set(auth())
                .send({
                  roleName: 'Replay child',
                  expectedVersion: 0,
                  clientRequestId: `role-replay-${channel}-${run}`,
                });
          ...
          expect(
            (
              await request(app)
                .delete(runtime)
                .set(auth())
                .send({ expectedVersion: 1, clientRequestId: `delete-parent-${run}` })
            ).status
          ).toBe(200);
        const replay = await send();
        const rows = await pool.query(
          'SELECT id FROM staffing_plan_roles WHERE staffing_plan_id=$1',
          [id]
        );
        expect({ status: replay.status, rows: rows.rowCount }).toEqual({ status: 404, rows: 0 });
      }
    );
  }
});
