...
class AlertAggregator {
    ...
    async processAlert(
        ...
        if (!group) {
            ...
            if (timeSinceFirst < this.deduplicationWindowMs && group.alert.count > MAX_ALERTS_PER_WINDOW) {
    ...
    getStats(): {
        ...
        const recentAlerts = this.alertHistory.filter((a) => now - a.lastOccurrence < recentWindow).length;
