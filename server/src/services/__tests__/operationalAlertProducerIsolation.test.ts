...
const state = vi.hoisted(() => ({
  runs: [] as Array<{ sql: string; params: unknown[] }>,
  sendCalls: 0,
  signalCalls: 0,
  dispatchCalls: 0,
}));
...
vi.mock('../operationalAlertRepairService.js', () => ({
  enqueueOperationalAlertRepairIntent: vi.fn(async () => {
    state.signalCalls++;
    throw new Error('forced alert store down');
  }),
}));
vi.mock('../caseWorkspace/eventOutboxService.js', () => ({
  dispatchPendingEvents: vi.fn(async () => {
    state.dispatchCalls++;
    return { claimed: 1, delivered: 1, failed: 0, failedEventIds: [] };
  }),
...
vi.mock('../caseWorkspace/eventInboxService.js', () => ({
  runInboxReconciliationSweep: vi.fn(async () => ({ scanned: 0 })),
}));

import {
  getMissedCaseWorkspaceDurableAlertSignals,
  runOutboxWorkerTick,
} from '../caseWorkspace/outboxWorker.js';
import NotificationOutboxService, {
  getMissedNotificationDurableAlertSignals,
...
describe('OPS alert producer failure isolation', () => {
  ...
  it('Notification commits SENT and calls business send once despite forced signal failure', async () => {
    ...
    await expect(NotificationOutboxService.drainOnce({ limit: 1 })).resolves.toMatchObject({
      processed: 1,
      sent: 1,
      failed: 0,
    });
  ...
  it('Case preserves the primary tick result and dispatches once despite forced signal failure', async () => {
    ...
    await expect(runOutboxWorkerTick({ organizationId: 'org-1' })).resolves.toMatchObject({
      claimed: 1,
      delivered: 1,
      failed: 0,
      timedOut: false,
