...
import {
  ...
  linkRuntimeInitiativeToExecutionCase,
  resolveExecutionSpineAuthority,
...
const link = {
  ...
  runtime_initiative_id: 'initiative-1',
  runtime_execution_case_id: 'case-1',
  source_version: 7,
...
describe('execution spine authority resolver', () => {
  ...
  it('returns the decision-A projection only after exact tenant/runtime validation', async () => {
    ...
    await expect(
      resolveExecutionSpineAuthority({
        ...
        runtimeInitiativeId: 'initiative-1',
        runtimeExecutionCaseId: 'case-1',
        expectedVersion: 3,
        requireActive: true,
      })
  ...
  it.each([
    ...
    [
      'ambiguous identity',
      [link, { ...link, link_id: 'link-2' }],
      'execution_authority_ambiguous_conflict',
    ],
  ...
  it('rejects an inactive or mismatched Runtime-v1 successor', async () => {
    query.mockResolvedValueOnce({ rows: [link] }).mockResolvedValueOnce({
      rows: [
        {
          ...runtime,
          case_payload: { initiativeId: 'different', state: 'CLOSED' },
        },
      ],
    });
    await expect(
  ...
  it('retires the legacy writer only through an explicit identity alias', async () => {
    ...
    await expect(
      linkInitiativeToExecutionCase({
        organizationId: 'org-a',
        initiativeId: 'legacy-initiative',
        caseId: 'legacy-case',
        actorId: 'actor-1',
  ...
  it('maps mounted resolver failures to stable fail-closed HTTP classes', () => {
    expect(classifyDomainCode('execution_authority_not_found')).toEqual({
      status: 404,
      code: 'execution_authority_not_found',
    });
    expect(classifyDomainCode('execution_authority_version_conflict')).toEqual({
      status: 409,
      code: 'execution_authority_version_conflict',
    });
    expect(classifyDomainCode('execution_authority_runtime_not_active')).toEqual({
      status: 409,
      code: 'execution_authority_runtime_not_active',
    });
    expect(classifyDomainCode('execution_authority_version_invalid')).toEqual({
      status: 400,
      code: 'execution_authority_version_invalid',
    });
  });
  ...
  it('fails when an explicit legacy alias is already bound to another canonical link', async () => {
    query
      ...
      .mockResolvedValueOnce({
        rows: [
          { version: 7, payload_json: { lifecycleState: 'IN_EXECUTION', projectId: 'project-1' } },
        ],
      })
      .mockResolvedValueOnce({
        rows: [{ payload_json: { initiativeId: 'initiative-1', state: 'ACTIVE' } }],
      })
      .mockResolvedValueOnce({ rows: [{ '?column?': 1 }] })
      .mockResolvedValueOnce({ rows: [link] })
      .mockResolvedValueOnce({
        rows: [{ initiative_project_id: 'project-1', case_project_id: 'project-1' }],
      })
      .mockResolvedValueOnce({ rows: [] });
    await expect(
      linkRuntimeInitiativeToExecutionCase({
        ...
        legacyInitiativeId: 'legacy-initiative',
        legacyCaseId: 'legacy-case',
      })
