...
describePg('execution action governance real PostgreSQL', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `DROP TRIGGER IF EXISTS trg_exe_action_audit_test_failure ON execution_action_audit`
    );
  ...
  afterAll(async () => {
    ...
    await pool
      .query(`DROP TRIGGER IF EXISTS trg_exe_action_audit_test_failure ON execution_action_audit`)
      .catch(() => undefined);
    await pool
      .query(`DROP FUNCTION IF EXISTS exe_action_audit_test_failure()`)
      .catch(() => undefined);
      ...
      executeGovernedExecutionAction({
        ...
        operation: () =>
          withPgTransaction(async (client) => {
            ...
            await client.query(
              `INSERT INTO exe_action_uow_probe (organization_id,action_id,kind,target_id)
          VALUES (?,?,?,?),(?,?,?,?)`,
              [
                orgId,
                actionId,
  ...
  it.each(ACTIONS)(
    'rolls back domain and outbox when audit insert fails for %s',
    async (actionId) => {
          ...
          operation: () =>
            withPgTransaction(async (client) => {
              ...
              await client.query(
                `INSERT INTO exe_action_uow_probe (organization_id,action_id,kind,target_id)
          VALUES (?,?,?,?),(?,?,?,?)`,
                [orgId, actionId, 'domain', targetId, orgId, actionId, 'outbox', targetId]
              );
              return true;
            }),
        })
      ...
      const rows = await pool.query(
        `SELECT kind FROM exe_action_uow_probe WHERE organization_id=$1 AND target_id=$2`,
        [orgId, targetId]
      );
  ...
  it('fails closed for a hidden action and audits denial', async () => {
    await expect(
      executeGovernedExecutionAction({
        organizationId: orgId,
        actionId: 'execution.initiative.delete',
    ...
    const row = await pool.query(
      `SELECT outcome,reason_code FROM execution_action_audit WHERE organization_id=$1 AND target_id='hidden-target'`,
      [orgId]
    );
    expect(row.rows[0]).toMatchObject({
      outcome: 'DENIED',
      reason_code: 'execution_action_hidden',
    });
  });

  it('rejects direct UPDATE and DELETE and survives cold readback', async () => {
    const audit = await pool.query(
      `SELECT audit_id FROM execution_action_audit WHERE organization_id=$1 LIMIT 1`,
      [orgId]
    );
    const auditId = audit.rows[0].audit_id;
    await expect(
      pool.query(`UPDATE execution_action_audit SET reason_code='tamper' WHERE audit_id=$1`, [
        auditId,
      ])
    ).rejects.toThrow('append-only');
    await expect(
      pool.query(`DELETE FROM execution_action_audit WHERE audit_id=$1`, [auditId])
    ).rejects.toThrow('append-only');
    const cold = new Pool({ connectionString: databaseUrl });
    const count = await cold.query(
      `SELECT count(*)::int AS n FROM execution_action_audit WHERE organization_id=$1`,
      [orgId]
    );
    await cold.end();
