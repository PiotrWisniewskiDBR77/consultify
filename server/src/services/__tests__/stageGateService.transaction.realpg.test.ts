...
describe('PMO E3 stage-gate transaction', NO_RETRY, () => {
  ...
  const actorId = `f23-rollback-user-${suffix}`;
  ...
  beforeAll(async () => {
    ...
    await pool.query(`INSERT INTO organizations(id,name) VALUES($1,$2)`, [organizationId, 'F23 rollback proof']);
    await pool.query(`INSERT INTO users(id,email,organization_id) VALUES($1,$2,$3)`, [actorId, `${actorId}@test.invalid`, organizationId]);
    await pool.query(`INSERT INTO users(id,email,organization_id) VALUES($1,$2,$3)`, [requesterId, `${requesterId}@test.invalid`, organizationId]);
    await pool.query(
      ...
      [
        ...
        JSON.stringify({ strategicGoals: ['Goal'], challenges: ['Challenge'], constraints: ['Constraint'] }),
        actorId,
      ]
    );
    await pool.query(
    ...
    await pool.query(`CREATE FUNCTION ${triggerName}() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'controlled phase failure'; END $$`);
    await pool.query(`CREATE TRIGGER ${triggerName} BEFORE UPDATE ON projects FOR EACH ROW WHEN (NEW.id = '${projectId}') EXECUTE FUNCTION ${triggerName}()`);
  });
  ...
  it('rolls back the gate receipt when the phase update fails', async () => {
    await expect(
      passGate(projectId, GATE_TYPES.READINESS_GATE, actorId, 'rollback proof', 'OWNER', {
        organizationId,
    ...
    expect((await pool.query('SELECT current_phase FROM projects WHERE id=$1', [projectId])).rows[0].current_phase).toBe('Context');
  });
});
