...
vi.mock('../../utils/DbPromise.js', () => ({
  ...
  all: vi.fn(async (sql: string, params: unknown[]) => {
    ...
    return state.records
      .filter((row) => !cursorAt || row.created_at > cursorAt || (row.created_at === cursorAt && row.id > cursorId!))
...
const params = {
  organizationId: 'org-a', artifactId: 'artifact-1', originRuntime: 'sheet', originRecordId: 'table-1',
};
...
describe('sheet artifact content adapter', () => {
  beforeEach(() => {
    state.table = { id: 'table-1', base_id: 'base-1', name: 'Data', description: null, schema_version: 3 };
    state.fields = [{ id: 'field-1', name: 'Value', field_type: 'text', options: {}, is_computed: false, field_order: 0 }];
    state.views = [{ id: 'view-1', name: 'Grid', view_type: 'grid', visible_field_ids: ['field-1'], config: {}, is_default: true, ordinal: 0 }];
    state.records = [];
    state.queries = [];
    state.artifact = null;
    state.origin = null;
  ...
  it.each([
    ['preview', 50, 50, false], ['preview', 51, 50, true],
    ['full', 500, 500, false], ['full', 501, 500, true],
    ...
    const result = await resolveSheetArtifactContent({ ...params, mode, limit: mode === 'preview' ? 50 : 500 });
  ...
  it('uses the (created_at,id) keyset without skips or duplicates on timestamp ties', async () => {
    ...
    const second = await resolveSheetArtifactContent({ ...params, limit: 2, cursor: firstJson.page.nextCursor });
    expect(firstJson.records.map((row: any) => row.id)).toEqual(['record-0001', 'record-0002']);
    expect((second!.envelope.contentJson as any).records.map((row: any) => row.id)).toEqual(['record-0003']);
    await expect(resolveSheetArtifactContent({ ...params, cursor: 'not-a-cursor' })).rejects.toMatchObject({
      statusCode: 400, code: SHEET_CONTENT_ERROR_CODES.INVALID_CURSOR,
    });
  });
  ...
  it('canonicalizes JSON object key order but changes page hash for values, schema, views and rows', async () => {
    ...
    const hash = async () => ((await resolveSheetArtifactContent(params))!.envelope.contentJson as any).revision.pageHash;
  ...
  it('labels page hash as weak snapshot semantics and never as datasetRevision', async () => {
    ...
    expect(json.revision).toMatchObject({ strength: 'weak', scope: 'page-snapshot', datasetRevision: null });
  ...
  it('stops at record boundaries and returns stable 413 for one oversized row', async () => {
    ...
    await expect(resolveSheetArtifactContent({ ...params, payloadByteCap: 100 })).rejects.toMatchObject({
      statusCode: 413, code: SHEET_CONTENT_ERROR_CODES.RECORD_TOO_LARGE,
    });
  });
  ...
  it('escapes pipes/newlines/null and canonicalizes nested values in Markdown', async () => {
    state.fields = [
      ...state.fields,
      { id: 'field-2', name: 'Other', field_type: 'text', options: {}, is_computed: false, field_order: 1 },
    ];
  ...
  it('emits deterministic SQL ordering and cursor shape', async () => {
    state.records = [record(1)];
    await resolveSheetArtifactContent({ ...params, cursor: encodeSheetCursor({ createdAt: record(0).created_at, id: record(0).id }) });
    expect(state.queries.find((sql) => sql.includes('FROM tp_fields'))).toContain('field_order ASC, name ASC, id ASC');
    expect(state.queries.find((sql) => sql.includes('FROM tp_views'))).toContain('is_default DESC, ordinal ASC NULLS LAST, name ASC, id ASC');
    expect(state.queries.find((sql) => sql.includes('FROM tp_records'))).toContain('(created_at, id) > (?, ?)');
  });
  ...
  it('resolves the exact sheet origin through the artifact registry read-back', async () => {
    ...
    const resolved = await resolveArtifactContent({ artifactId: 'artifact-1', organizationId: 'org-a' });
