...
describe.skipIf(!REAL_PG)('EXE-FLOW material-command outbox consumer', () => {
  ...
  afterAll(async () => {
    ...
    await db.query('DELETE FROM ie_outbox_delivery_receipts WHERE organization_id=ANY($1)', [
      [orgA, orgB],
    ]);
  ...
  it('claims tenant-scoped exactly once under 8-way concurrency and persists immutable receipt', async () => {
    ...
    const outcomes = await Promise.all(
      Array.from({ length: 8 }, () => consumeNextInitiativeExecutionEvent({ organizationId: orgA }))
    );
    expect(outcomes.filter((v) => v?.status === 'DELIVERED')).toHaveLength(1);
    expect(outcomes.find(Boolean)?.eventId).toBe(eventA);
    const receipt = await db.query(
      `SELECT * FROM ie_outbox_delivery_receipts WHERE source_event_id=$1`,
      [eventA]
    );
    expect(receipt.rows).toHaveLength(1);
    expect(receipt.rows[0].payload_version).toBe(1);
    expect(
      (await db.query(`SELECT processed_at FROM ie_outbox_events WHERE id=$1`, [eventB])).rows[0]
        .processed_at
    ).toBeNull();
    await expect(
      db.query(
        `UPDATE ie_outbox_delivery_receipts SET event_type='tampered' WHERE source_event_id=$1`,
        [eventA]
      )
  ...
  it('reclaims stale processing and cold-reads the same receipt', async () => {
    ...
    await db.query(
      `UPDATE ie_outbox_events SET processing_started_at=now()-interval '10 minutes' WHERE id=$1`,
      [event]
    );
    ...
    const row = await cold.query(
      `SELECT r.receipt_id,e.processed_at FROM ie_outbox_delivery_receipts r JOIN ie_outbox_events e ON e.id=r.source_event_id WHERE r.source_event_id=$1`,
      [event]
    );
    await cold.end();
  ...
  it('fails unsupported versions closed and supports tenant-scoped DLQ redrive', async () => {
    ...
    expect(await consumeNextInitiativeExecutionEvent({ organizationId: orgA })).toMatchObject({
      status: 'DEAD_LETTER',
    });
    expect(
      (
        await db.query(
          `SELECT count(*)::int n FROM ie_outbox_delivery_receipts WHERE source_event_id=$1`,
          [unsupported]
        )
      ).rows[0].n
    ).toBe(0);
    ...
    expect(
      await consumeNextInitiativeExecutionEvent({
        organizationId: orgA,
        __testForceFailure: new Error('forced'),
      })
