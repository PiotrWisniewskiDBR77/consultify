...
describe.skipIf(!real)('RESULTS-W25/W26/W32 canonical recovery cards (real PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const [org, actor, kpi, kase] of [
      [orgA, actorA, kpiA, caseA],
      [orgB, actorB, kpiB, caseB],
    ]) {
      ...
      await pool.query(
        ...
        VALUES($1,$2,$3,100,'HIGHER_IS_BETTER','PERCENT_FROM_TARGET',0.1,0.2)`,
        [kpi, org, kpi]
      );
      await pool.query(
  ...
  it('creates once under concurrency and rejects changed replay, foreign case and revoked replay', async () => {
    ...
    const [first, second] = await Promise.all([
      createRecoveryCard({
        ...ctx(),
        caseId: caseA,
        initialPatch: { hypothesis: 'Initial pinned cause', priority: 'HIGH' },
        idempotencyKey: key,
      }),
      createRecoveryCard({
        ...ctx(),
    ...
    expect(first.result).toMatchObject({
      hypothesis: 'Initial pinned cause',
      priority: 'HIGH',
      version: 2,
    });
    expect(
      (
        await pool.query(
          `SELECT count(*)::int n FROM kpi_recovery_cards WHERE deviation_case_id=$1`,
          [caseA]
        )
      ).rows[0].n
    ).toBe(1);
    await expect(
      createRecoveryCard({
        ...
        caseId: caseA,
        initialPatch: { hypothesis: 'Initial pinned cause', priority: 'HIGH' },
        idempotencyKey: `${tag}-other-key`,
      })
    ).rejects.toMatchObject({ code: 'RECOVERY_CARD_ALREADY_EXISTS' });
    await expect(
      createRecoveryCard({
        ...ctx({ organizationId: orgB, actorUserId: actorB }),
        caseId: caseA,
        idempotencyKey: `${tag}-foreign`,
      })
    ...
    await expect(
      createRecoveryCard({ ...ctx(), caseId: caseA, idempotencyKey: key })
  ...
  it('updates with CAS, exact replay and event-failure rollback', async () => {
    const updated = await updateRecoveryCard({
      ...ctx(),
      cardId,
      expectedVersion: 2,
      patch: { hypothesis: 'Pinned cause', priority: 'HIGH' },
      idempotencyKey: `${tag}-update`,
    });
    expect(updated.result).toMatchObject({
      hypothesis: 'Pinned cause',
      priority: 'HIGH',
      version: 3,
    });
    const replay = await updateRecoveryCard({
      ...ctx(),
      cardId,
      expectedVersion: 2,
      patch: { hypothesis: 'Pinned cause', priority: 'HIGH' },
    ...
    await expect(
      updateRecoveryCard({
        ...
        cardId,
        expectedVersion: 3,
        patch: { hypothesis: 'Must rollback' },
        idempotencyKey: `${tag}-rollback`,
      })
    ).rejects.toThrow('forced card event failure');
    ...
    expect(
      (await pool.query(`SELECT hypothesis,version FROM kpi_recovery_cards WHERE id=$1`, [cardId]))
        .rows[0]
  ...
  it('closes only from a fresh green measurement and replays the identical receipt', async () => {
    await expect(
      closeRecoveryCard({
        ...
        idempotencyKey: `${tag}-no-evidence`,
      })
    ...
    const key = `${tag}-close`;
    const closed = await closeRecoveryCard({
      ...ctx(),
      cardId,
      expectedVersion: 3,
      evidenceText: 'Fresh KPI returned to target',
      effectivenessRating: 'EFFECTIVE',
      idempotencyKey: key,
    });
    ...
    const replay = await closeRecoveryCard({
      ...ctx(),
      cardId,
      expectedVersion: 3,
      evidenceText: 'Fresh KPI returned to target',
