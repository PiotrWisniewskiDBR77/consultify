...
const orgA = `${tag}-org-a`,
  orgB = `${tag}-org-b`;
const actorA = `${tag}-actor-a`,
  actorB = `${tag}-actor-b`,
  foreignOwner = `${tag}-foreign-owner`;
const kpiA = `${tag}-kpi-a`,
  kpiB = `${tag}-kpi-b`;
const caseA = `${tag}-case-a`,
  caseB = `${tag}-case-b`;
const cardA = `${tag}-card-a`,
  cardB = `${tag}-card-b`;
...
describe.skipIf(!real)('RESULTS-W27..W31 canonical recovery children (real PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const [org, actor, kpi, kase, card] of [
      [orgA, actorA, kpiA, caseA, cardA],
      [orgB, actorB, kpiB, caseB, cardB],
    ]) {
      ...
      await pool.query(
        ...
        [`${actor}-membership`, org, actor]
      );
      await pool.query(`INSERT INTO initiative_kpis(id,organization_id,name) VALUES($1,$2,$3)`, [
        kpi,
        org,
        kpi,
      ]);
      await pool.query(
        `INSERT INTO kpi_deviation_cases(id,kpi_id,organization_id,period_start,severity,owner_user_id)
        VALUES($1,$2,$3,'2026-08-01','RED',$4)`,
        [kase, kpi, org, actor]
      );
      await pool.query(
        `INSERT INTO kpi_recovery_cards(id,organization_id,deviation_case_id,kpi_id,created_by)
        VALUES($1,$2,$3,$4,$5)`,
        [card, org, kase, kpi, actor]
      );
    }
    await pool.query(
    ...
    await pool.query(
      ...
      [`${foreignOwner}-membership`, orgB, foreignOwner]
    );
  });
  ...
  afterAll(async () => {
    ...
    await pool.query(`DELETE FROM rvn_platform_events WHERE organization_id IN ($1,$2)`, [
      orgA,
      orgB,
    ]);
    await pool.query(`DELETE FROM tasks WHERE organization_id IN ($1,$2)`, [orgA, orgB]);
    await pool.query(`DELETE FROM rvn_kpi_recovery_checkpoints WHERE organization_id IN ($1,$2)`, [
      orgA,
      orgB,
    ]);
    await pool.query(`DELETE FROM rvn_kpi_recovery_actions WHERE organization_id IN ($1,$2)`, [
      orgA,
      orgB,
    ]);
    await pool.query(`DELETE FROM kpi_time_series WHERE organization_id IN ($1,$2)`, [orgA, orgB]);
    await pool.query(`DELETE FROM kpi_recovery_cards WHERE organization_id IN ($1,$2)`, [
      orgA,
      orgB,
    ]);
    await pool.query(`DELETE FROM kpi_deviation_cases WHERE organization_id IN ($1,$2)`, [
      orgA,
      orgB,
    ]);
  ...
  it('creates exactly once under concurrency and rejects changed-payload/foreign-owner/foreign-card', async () => {
    ...
    expect(
      (
        await pool.query(
          `SELECT count(*)::int n FROM rvn_kpi_recovery_actions WHERE organization_id=$1`,
          [orgA]
        )
      ).rows[0].n
    ).toBe(1);
    ...
    await expect(
      createRecoveryAction({
        ...
        idempotencyKey: `${tag}-foreign-owner`,
      })
    ).rejects.toMatchObject({ code: 'ASSIGNEE_NOT_ACTIVE_MEMBER' });
    await expect(
      createRecoveryAction({
        ...ctx({ organizationId: orgB, actorUserId: actorB, cardId: cardA }),
        actionType: 'DURABLE',
        title: 'Foreign card',
        idempotencyKey: `${tag}-foreign-card`,
      })
  ...
  it('CAS updates allow one winner and replay authorization denies a revoked actor', async () => {
    const created = await createRecoveryAction({
      ...ctx(),
      actionType: 'DURABLE',
      title: 'CAS action',
      ownerUserId: actorA,
      idempotencyKey: `${tag}-cas-create`,
    });
    const races = await Promise.allSettled([
      updateRecoveryAction({
        ...ctx(),
        actionId: created.result.id,
        expectedVersion: 1,
        status: 'DONE',
        idempotencyKey: `${tag}-cas-a`,
      }),
      updateRecoveryAction({
        ...ctx(),
        actionId: created.result.id,
    ...
    await expect(
      ...
      name: 'CommandCapabilityDeniedError',
      code: 'COMMAND_CAPABILITY_DENIED',
    });
    await pool.query(
      `INSERT INTO organization_members(id,organization_id,user_id,role,status)
      VALUES($1,$2,$3,'ADMIN','ACTIVE')`,
      [`${actorA}-membership-restored`, orgA, actorA]
    );
  });
  ...
  it('links one canonical task atomically, preserves exact owner, and rolls back task+link on event failure', async () => {
    const created = await createRecoveryAction({
      ...
      title: 'Task action',
      ownerUserId: actorA,
      idempotencyKey: `${tag}-task-create`,
    });
    ...
    const task = await pool.query(
      `SELECT organization_id,assignee_id,source_type,source_id FROM tasks WHERE id=$1`,
      [links[0].result.linkedTaskId]
    );
    expect(task.rows[0]).toMatchObject({
      organization_id: orgA,
      assignee_id: actorA,
      source_type: 'kpi_recovery_action',
      source_id: created.result.id,
    });

    const rollbackAction = await createRecoveryAction({
      ...ctx(),
      actionType: 'IMMEDIATE',
      title: 'Rollback action',
      ownerUserId: actorA,
      idempotencyKey: `${tag}-rollback-create`,
    });
    ...
    await expect(
      linkRecoveryActionTask({
        ...ctx(),
        actionId: rollbackAction.result.id,
        expectedVersion: 1,
        idempotencyKey: `${tag}-rollback-link`,
      })
    ...
    expect(
      (
        await pool.query(`SELECT linked_task_id FROM rvn_kpi_recovery_actions WHERE action_id=$1`, [
          rollbackAction.result.id,
        ])
      ).rows[0].linked_task_id
    ).toBeNull();
    expect(
      (
        await pool.query(`SELECT count(*)::int n FROM tasks WHERE source_id=$1`, [
          rollbackAction.result.id,
        ])
      ).rows[0].n
    ).toBe(0);
  ...
  it('checkpoint create/replay is exact; foreign measurement is rejected and concurrent resolve has one winner', async () => {
    ...
    await pool.query(
      `INSERT INTO kpi_time_series(id,kpi_id,organization_id,value,period_start)
      VALUES($1,$2,$3,10,'2026-08-20')`,
      [foreignMeasurement, kpiB, orgB]
    );
    await expect(
      resolveRecoveryCheckpoint({
        ...ctx(),
        checkpointId: a.result.id,
        expectedVersion: 1,
        status: 'MET',
        kpiTimeSeriesId: foreignMeasurement,
        idempotencyKey: `${tag}-foreign-measurement`,
      })
    ).rejects.toMatchObject({ name: 'AtomicWriteAggregateNotFoundError' });
    const measurement = `${tag}-measurement-a`;
    await pool.query(
      `INSERT INTO kpi_time_series(id,kpi_id,organization_id,value,period_start)
      VALUES($1,$2,$3,12,'2026-08-21')`,
      [measurement, kpiA, orgA]
    );
    const races = await Promise.allSettled([
      resolveRecoveryCheckpoint({
        ...ctx(),
        ...
        expectedVersion: 1,
        status: 'MET',
        kpiTimeSeriesId: measurement,
        idempotencyKey: `${tag}-resolve-a`,
      }),
      resolveRecoveryCheckpoint({
        ...ctx(),
        checkpointId: a.result.id,
