...
class PermissionsMatrixServiceClass {
    ...
    async updateRolePermissions(roleId: string, permissions: any[]): Promise<boolean> {
        ...
        for (const p of permissions) {
            await this.db.run('INSERT INTO role_permissions (role_id, permission_key, enabled) VALUES (?, ?, ?)', [roleId, p.key, p.enabled ? 1 : 0]);
        }
        return true;
    }

    async togglePermission(roleId: string, permissionKey: string, enabled: boolean): Promise<boolean> {
    ...
    async copyRolePermissions(sourceRole: string, targetRole: string): Promise<boolean> {
        const sourcePerms = await this.db.all('SELECT permission_key, enabled FROM role_permissions WHERE role_id = ?', [sourceRole]);
        await this.db.run('DELETE FROM role_permissions WHERE role_id = ?', [targetRole]);
        for (const p of sourcePerms) {
            await this.db.run('INSERT INTO role_permissions (role_id, permission_key, enabled) VALUES (?, ?, ?)', [targetRole, p.permission_key, p.enabled]);
        }
        return true;
    }

    async compareRoles(role1: string, role2: string): Promise<any> {
        const p1 = await this.db.all('SELECT permission_key, enabled FROM role_permissions WHERE role_id = ?', [role1]);
        const p2 = await this.db.all('SELECT permission_key, enabled FROM role_permissions WHERE role_id = ?', [role2]);
        return { role1, role2, diff: [] }; // Minimal implementation
    }

    async getStats(): Promise<any> {
        return await this.db.get('SELECT COUNT(DISTINCT role_id) as role_count, COUNT(*) as total_permissions FROM role_permissions');
    }
}
