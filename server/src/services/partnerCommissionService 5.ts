 ...
 */
export async function createCommission(params: CreateCommissionParams): Promise<CommissionTransaction> {
    const {
        partnerOrgId,
        attributionId,
        organizationId,
    ...
    try {
        await DbPromise.run(
            ...
            `INSERT INTO partner_commission_transactions 
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [id, partnerOrgId, attributionId || null, organizationId, transactionType,
             transactionDate, billingPeriodStart || null, billingPeriodEnd || null,
             grossAmount, commissionRate, commissionAmount, currency,
             invoiceId || null, stripePaymentId || null, stripeInvoiceId || null, notes || null]
        );

        logger.info(`[PartnerCommissionService] Created commission: €${commissionAmount} for partner ${partnerOrgId}`);
...
export async function approveCommissions(
    ...
    try {
        ...
        logger.info(`[PartnerCommissionService] Approved ${result.changes} commissions by ${approvedBy}`);
...
export async function cancelCommission(
    ...
    try {
        ...
        if (result.changes && result.changes > 0) {
            logger.info(`[PartnerCommissionService] Cancelled commission ${commissionId} by ${cancelledBy}`);
        }
...
export async function getEarningsSummary(partnerOrgId: string): Promise<EarningsSummary> {
    try {
        ...
        const totalEarned = (stats?.total_pending || 0) + (stats?.total_approved || 0) + (stats?.total_paid || 0);
...
export async function requestPayout(params: PayoutRequest): Promise<Payout | null> {
    ...
    try {
        ...
        if (netAmount < threshold) {
            logger.warn(`[PartnerCommissionService] Payout amount €${netAmount} below threshold €${threshold}`);
            return null;
        }

        // Determine period
        const periodStart = approvedCommissions[0].transaction_date.split('T')[0];
        const periodEnd = approvedCommissions[approvedCommissions.length - 1].transaction_date.split('T')[0];
        ...
        await DbPromise.run(
            ...
            `INSERT INTO partner_payouts 
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'EUR', ?, 'PENDING', ?, ?, ?)`,
            [payoutId, partnerOrgId, payoutAccountId || null, periodStart, periodEnd,
             grossAmount, fees, netAmount, approvedCommissions.length,
             partner?.payout_method || 'BANK_TRANSFER', requestedBy || null, notes || null]
        );
        ...
        logger.info(`[PartnerCommissionService] Created payout request: €${netAmount} for partner ${partnerOrgId}`);
