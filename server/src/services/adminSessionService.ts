...
class AdminSessionServiceClass {
    ...
    async revokeSession(sessionId: string): Promise<boolean> {
        const result = await this.db.run('UPDATE admin_sessions SET is_active = 0 WHERE id = ?', [sessionId]);
    ...
    async getSessionStats(): Promise<any> {
        return await this.db.get('SELECT COUNT(*) as active_sessions FROM admin_sessions WHERE is_active = 1');
    }
}
