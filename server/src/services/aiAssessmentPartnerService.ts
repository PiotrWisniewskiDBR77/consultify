...
class AIAssessmentPartnerService {
  ...
  _getKeyActivities(axisId, level) {
    ...
    return (
      activities[axisId]?.[level] || ['Assess current capabilities', 'Define improvement roadmap']
    );
  }
  ...
  async suggestEvidence(axisId, score, context = {}) {
    ...
    try {
      ...
      const parsed = jsonMatch
        ...
        : this._getFallbackEvidence(axisId, score);
      const evidence = validateOrFallback(
        parsed,
        EvidenceSchema,
        this._getFallbackEvidence(axisId, score)
      );
  ...
  async generateBenchmarkCommentary(assessment, benchmarks, options = {}) {
    const scores = Object.entries(assessment)
      ...
      .map(([key, val]) => ({
        ...
        vsIndustry: benchmarks[key]
          ? val.actual - (benchmarks[key].median || benchmarks[key].average)
          : null,
      }));
  ...
  async generateInitiativesFromGaps(gapAnalysis, constraints = {}) {
    ...
    const prompt = `
            ...
            ${sortedGaps
              ...
              .map(
                (g) =>
                  `- ${g.axisName || g.axis}: Gap ${g.gap} levels (current: ${g.currentScore}, target: ${g.targetScore})`
              )
              .join('\n')}
  ...
  _getTargetReasoning(axisId, current, target, ambitionLevel) {
    ...
    if (gap === 1)
      return `Konserwatywny cel: osiągnięcie poziomu ${target} w ${axis?.name || axisId} jest realistyczne w ciągu 6-9 miesięcy.`;
    if (gap === 2)
      return `Zbalansowany cel: przejście na poziom ${target} wymaga średnioterminowego programu transformacji (12-18 miesięcy).`;
  ...
  _getFallbackExecutiveSummary(scores, avgActual, avgTarget) {
    ...
    return {
      summary:
        `Organizacja osiągnęła średni poziom dojrzałości cyfrowej ${avgActual.toFixed(1)}/7. ` +
