...
const base = (requestId: string) => ({
  ...
  writerId: 'FIN-W02',
  organizationId: org,
  tenantResolution: 'resolved' as const,
  requestId,
  userId: `${prefix}-user`,
  method: 'POST',
  routePath: '/api/financial-modeling/models/x/approve',
  accessKind: 'legacy_uncovered_writer' as const,
  successorPath: '/api/v8/finance',
  legacyTable: null,
  legacyId: null,
  canonicalArtifactId: null,
  canonicalBusinessVersionId: null,
  canonicalWorkingRevisionId: null,
...
describe.skipIf(!enabled)('B1 durable legacy intent lifecycle realPG', () => {
  ...
  it('registers durably before terminal completion and completes the same intent', async () => {
    ...
    const cold = await db.query(
      `SELECT status,terminal_result FROM legacy_cutover_signal_intents WHERE intent_id=$1`,
      [registered.intentId]
    );
    ...
    const terminal = await db.query(
      `SELECT status,terminal_status,terminal_result FROM legacy_cutover_signal_intents WHERE intent_id=$1`,
      [registered.intentId]
    );
  ...
  it('deduplicates an exact request and rejects a fingerprint collision', async () => {
    ...
    await expect(
      registerLegacyCutoverIntent({ ...input, routePath: '/different' }, 'key-2')
  ...
  it('repairs abandoned registered work conservatively, never as passed', async () => {
    ...
    await db.query(
      `UPDATE legacy_cutover_signal_intents SET created_at=now()-interval '2 hours' WHERE intent_id=$1`,
      [row.intentId]
    );
    expect(await repairAbandonedLegacyCutoverIntents(60, 10)).toBeGreaterThanOrEqual(1);
    const repaired = await db.query(
      `SELECT status,terminal_result,completion_source FROM legacy_cutover_signal_intents WHERE intent_id=$1`,
      [row.intentId]
    );
    expect(repaired.rows[0]).toEqual({
      status: 'ABORTED_UNKNOWN',
      terminal_result: 'aborted_unknown',
      completion_source: 'repair',
    });
  });
  ...
  it('fences concurrent repair workers to one terminal transition', async () => {
    ...
    const terminal = await db.query(
      `SELECT status,fencing_version FROM legacy_cutover_signal_intents WHERE intent_id=$1`,
      [row.intentId]
    );
  ...
  it('rejects direct SQL mutation of a terminal intent', async () => {
    ...
    await expect(
      db.query(
        `UPDATE legacy_cutover_signal_intents SET status='REGISTERED',terminal_result=NULL,completed_at=NULL WHERE intent_id=$1`,
        [row.intentId]
      )
