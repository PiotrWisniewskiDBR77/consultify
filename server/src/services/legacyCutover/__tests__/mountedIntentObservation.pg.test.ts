...
const enabled =
  process.env.RUN_DB_TESTS === '1' && process.env.MOCK_DB === 'false' && /^postgres/.test(url);
...
const token = (id: string, role = 'ADMIN') => ({
...
async function waitFor(pool: Pool, requestId: string) {
  for (let i = 0; i < 80; i++) {
    const row = (
      await pool.query(`SELECT * FROM legacy_cutover_signal_intents WHERE request_id=$1`, [
        requestId,
      ])
    ).rows[0];
    if (row) return row;
...
describe.skipIf(!enabled)('B1 mounted durable intent and strict membership realPG', () => {
  ...
  beforeAll(async () => {
    ...
    for (const [id, status] of [
      ...
      await pool.query(
        `INSERT INTO organization_members(id,organization_id,user_id,role,status,created_at) VALUES($1,$2,$3,'ADMIN',$4,$5)`,
        [`${prefix}-${id}-m`, org, id, status, now]
      );
    const financeRouter = (await import('../../../routes/financial-modeling.routes.js')).default;
    fin = express();
    fin.use(express.json());
    fin.use('/api/financial-modeling', financeRouter);
    fin.use((e: any, _q: any, r: any, _n: any) =>
      r.status(500).json({ error: String(e?.message || e) })
    );
    const { superAdminPartnerRouter } = await import('../../../routes/partners.routes.js');
    partner = express();
    partner.use(express.json());
    partner.use('/api/superadmin/partner-settlements', superAdminPartnerRouter);
    partner.use((e: any, _q: any, r: any, _n: any) =>
      r.status(500).json({ error: String(e?.message || e) })
    );
    const { default: v8 } = await import('../../../routes/v8/index.js');
    results = express();
    results.use(express.json());
    results.use('/api/v8', v8);
    results.use((e: any, _q: any, r: any, _n: any) =>
      r.status(500).json({ error: String(e?.message || e) })
    );
  }, 120_000);
  afterAll(async () => {
    ...
    await pool.query(`DELETE FROM initiative_kpis WHERE organization_id=$1 AND name LIKE $2`, [
      org,
      `${prefix}%`,
    ]);
  ...
  it('registers intent before the real Finance handler can continue, then completes the same row', async () => {
    ...
    await lock.query('LOCK TABLE financial_models IN ACCESS EXCLUSIVE MODE');
    const requestId = `${prefix}-paused`;
    const pending = request(fin)
      .post(`/api/financial-modeling/models/${prefix}-missing/approve`)
      .set(token(active))
      .set('x-request-id', requestId)
      .send({})
      .then((r) => r);
    const registered = await waitFor(pool, requestId);
    ...
    for (let i = 0; i < 80; i++) {
      const row = (
        await pool.query(`SELECT * FROM legacy_cutover_signal_intents WHERE intent_id=$1`, [
          registered.intent_id,
        ])
      ).rows[0];
      if (row.status === 'COMPLETED') {
        expect(row.terminal_status).toBe(404);
        expect(row.terminal_result).toBe('passed');
        return;
      }
      await new Promise((resolve) => setTimeout(resolve, 25));
    }
  ...
  it('rejects revoked Results membership before intent or mutation', async () => {
    ...
    expect(
      (
        await pool.query(
          `SELECT count(*)::int n FROM legacy_cutover_signal_intents WHERE request_id=$1`,
          [requestId]
        )
      ).rows[0].n
    ).toBe(0);
  ...
  it('lets an ACTIVE signed Results caller reach the real handler and completes its intent', async () => {
    ...
    const response = await request(results)
      ...
      .set(token(active))
      .set('x-request-id', requestId)
      .send({ name: `${prefix}-active-kpi` });
    expect([401, 403, 503]).not.toContain(response.status);
    const intent = await waitFor(pool, requestId);
    for (let i = 0; i < 80; i++) {
      const row = (
        await pool.query(
          `SELECT status,terminal_status FROM legacy_cutover_signal_intents WHERE intent_id=$1`,
          [intent.intent_id]
        )
      ).rows[0];
      if (row.status === 'COMPLETED') {
        expect(row.terminal_status).toBe(response.status);
        return;
      }
      await new Promise((resolve) => setTimeout(resolve, 25));
    }
  ...
  it('does not let SUPERADMIN bypass missing ACTIVE membership on Partner', async () => {
    ...
    const response = await request(partner)
      ...
      .set(token(superNoMember, 'SUPERADMIN'))
      .set('x-request-id', requestId)
      .send({ commissionIds: [] });
    expect(response.status).toBe(403);
    expect(response.body).toMatchObject({ code: 'ORG_MEMBERSHIP_REVOKED' });
  ...
  it('fails 503 before Finance handler when durable registration is unavailable', async () => {
    ...
    await pool.query(
      `ALTER TABLE legacy_cutover_signal_intents RENAME TO legacy_cutover_signal_intents_b1_hold`
    );
    try {
      const response = await request(fin)
        .post(`/api/financial-modeling/models/${prefix}-db-down/approve`)
        .set(token(active))
        .set('x-request-id', `${prefix}-db-down`)
        .send({});
      expect(response.status).toBe(503);
      expect(response.body).toMatchObject({ code: 'LEGACY_CUTOVER_INTENT_UNAVAILABLE' });
    } finally {
      await pool.query(
        `ALTER TABLE legacy_cutover_signal_intents_b1_hold RENAME TO legacy_cutover_signal_intents`
      );
    }
  });
  ...
  it('cold pool reads exact completed Finance intent', async () => {
    ...
    try {
      const rows = await cold.query(
        `SELECT status,terminal_status,terminal_result FROM legacy_cutover_signal_intents WHERE request_id=$1`,
        [`${prefix}-paused`]
      );
      expect(rows.rows).toEqual([
        { status: 'COMPLETED', terminal_status: 404, terminal_result: 'passed' },
      ]);
    } finally {
      await cold.end();
    }
