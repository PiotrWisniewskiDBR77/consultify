...
describe.skipIf(!REAL_PG)(
  'Materials (presentations mount) cutover guard (fresh real PostgreSQL)',
  () => {
    let pool: Pool;
    ...
    it('registers the export writers as observed with no successor route, not disabled', () => {
      const w06 = MATERIALS_PRESENTATIONS_CUTOVER.writers.find(
        (w) => w.writerId === 'MATERIALS-W06'
      );
      const w07 = MATERIALS_PRESENTATIONS_CUTOVER.writers.find(
        (w) => w.writerId === 'MATERIALS-W07'
      );
      const w08 = MATERIALS_PRESENTATIONS_CUTOVER.writers.find(
        (w) => w.writerId === 'MATERIALS-W08'
      );
      ...
      expect(MATERIALS_PRESENTATIONS_CUTOVER.writers.every((w) => w.state !== 'disabled')).toBe(
        true
      );
    });
