...
export const hasFinanceEditRole = (role: unknown): boolean =>
  FINANCE_EDIT_ROLES.has(String(role || '').toLowerCase());
...
export async function requireActiveMembership(
  ...
  const userId = String(
    anyReq?.v8Context?.userId || anyReq?.user?.id || anyReq?.userId || ''
  ).trim();
...
export async function requireFinanceEditorMembership(
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> {
  ...
  try {
    const membership = await DbPromise.get<{ status?: string; role?: string }>(
    ...
    if (!hasFinanceEditRole(membership?.role)) {
      res.status(403).json({ success: false, code: FINANCE_EDIT_FORBIDDEN_CODE });
      return;
    }
  } catch (error) {
    logger.error('[Finance] editor membership verification unavailable', error);
    res.status(503).json({ success: false, code: MEMBERSHIP_UNVERIFIABLE_CODE });
    return;
  }
  next();
