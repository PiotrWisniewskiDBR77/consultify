...
export const FINANCE_STATEMENTS_CUTOVER: LegacyCutoverDomainConfig = {
  ...
  writers: [
    ...
    },
    {
      ...
      path: /^\/[^/]+\/detect\/?$/,
      state: 'disabled',
      successor: '/api/v8/finance/statements/:statementId/detect',
      reason:
        'All mounted detect callers use the independently mounted tenant-scoped V8 workflow. The compatibility fallback to this duplicate metadata writer was removed; failures remain visible.',
    },
    {
      writerId: 'FS-W04',
      method: 'POST',
      path: /^\/[^/]+\/extract\/?$/,
      state: 'disabled',
      successor: '/api/v8/finance/statements/:statementId/extract',
      reason:
        'All mounted extract callers use the V8 workflow, which owns ingest-run, source-artifact, quality-run and staged multi-section persistence. The duplicate legacy door is retired fail-closed.',
    },
    {
      writerId: 'FS-W05',
      method: 'POST',
      path: /^\/[^/]+\/map\/?$/,
      state: 'disabled',
      successor: '/api/v8/finance/statements/:statementId/map',
      reason:
        'All mounted map callers use the V8 workflow with persisted candidates, mapping tiers and coverage policy. The compatibility fallback is removed and this sibling writer is retired.',
    },
    {
      writerId: 'FS-W06',
      method: 'PUT',
      path: /^\/[^/]+\/values\/?$/,
      state: 'disabled',
      successor: '/api/v8/finance/statements/:statementId/values',
      reason:
        'All mounted value-save callers use the tenant-scoped V8 save flow and never retry onto this sibling write. The duplicate legacy door is retired fail-closed.',
    },
    {
      writerId: 'FS-W07',
      method: 'POST',
      path: /^\/[^/]+\/validate\/?$/,
      state: 'disabled',
      successor: '/api/v8/finance/statements/:statementId/confirm',
      reason:
        'No mounted caller requires a separate validation mutation. The governed V8 confirm command computes validation and readiness inside the same tenant-scoped transaction before canonical registration, so the split delete/reinsert validation door is retired fail-closed.',
    },
    {
      writerId: 'FS-W08',
      method: 'POST',
      ...
      successor: '/api/v8/finance/statements/:statementId/confirm',
      reason:
        'Every mounted confirmation now resolves the current source receipt and values version and invokes governed V8 confirmation with an Idempotency-Key. That transaction checks membership, CAS, source provenance, manual decisions, validation/readiness and canonical registration; the legacy snapshot writer is retired.',
    },
    {
      writerId: 'FS-W09',
      method: 'POST',
