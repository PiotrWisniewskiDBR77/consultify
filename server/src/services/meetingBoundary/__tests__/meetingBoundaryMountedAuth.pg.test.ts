...
describe('MTG-BVP-001 mounted production router with real auth and PostgreSQL', () => {
  ...
  beforeAll(async () => {
    ...
    for (const [id, name] of [[orgA, 'Meeting Auth A'], [orgB, 'Meeting Auth B']]) {
      await pool.query(
    ...
    for (const [id, org, role] of [
      [memberA, orgA, 'USER'], [adminA, orgA, 'ADMIN'], [adminB, orgB, 'ADMIN'],
    ]) {
      await pool.query(
  ...
  afterAll(async () => {
    await pool.query(`DELETE FROM artifact_handoff_receipts WHERE organization_id IN ($1,$2)`, [orgA, orgB]);
    await pool.query(`DELETE FROM artifact_handoff_proposals WHERE organization_id IN ($1,$2)`, [orgA, orgB]);
    await pool.query(`DELETE FROM meeting_notes WHERE organization_id IN ($1,$2)`, [orgA, orgB]);
    await pool.query(`DELETE FROM meeting_follow_ups WHERE meeting_id IN (SELECT id FROM meetings WHERE organization_id IN ($1,$2))`, [orgA, orgB]);
  ...
  it('creates agenda/materials metadata and proposes durable notes through real member auth', async () => {
    ...
    const generated = await request(app)
      ...
      .set(auth(token(memberA, orgA, 'USER')))
      .send({ transcript: `${prefix}: decision approve pilot. action item prepare evidence.`, idempotencyKey: `${prefix}-idem` });
  ...
  it('approves once through real admin auth and cold-reads one immutable receipt', async () => {
    ...
    const [first, concurrent] = await Promise.all([
      request(app).post(`/api/meeting/${meetingId}/notes/${noteId}/decision`).set(auth(adminToken)).send({ action: 'approve' }),
      request(app).post(`/api/meeting/${meetingId}/notes/${noteId}/decision`).set(auth(adminToken)).send({ action: 'approve' }),
    ]);
    expect(
