...
class AccessPolicyServiceClass {
    ...
    public setDependencies(newDeps: Partial<AccessPolicyDeps>): void {
        ...
        if (newDeps.SeatManagementService) this.deps.SeatManagementService = newDeps.SeatManagementService;
    }
    ...
    async checkAccess(
        ...
        try {
            ...
            if (!orgInfo) return { allowed: false, reason: 'Organization not found', errorCode: 'ORG_NOT_FOUND' };
            if (!orgInfo.isActive)
    ...
    async canInviteUsers(organizationId: string, requestingUserId: string): Promise<CanInviteUsersResult> {
        ...
        if (orgInfo.organizationType === ORG_TYPES.DEMO) return { allowed: false, reasonCode: 'DEMO_READ_ONLY' };
        if (trialStatus.expired && orgInfo.organizationType !== ORG_TYPES.PAID)
    ...
    async getSeatAvailability(organizationId: string): Promise<SeatAvailability> {
        ...
        return { maxSeats: limits.maxUsers, currentSeats, seatsRemaining: Math.max(0, limits.maxUsers - currentSeats) };
    }
...
export const getSeatAvailabilityEnhanced = serviceInstance.getSeatAvailabilityEnhanced.bind(serviceInstance);
