...
export async function exportValuationPptx(params: {
  ...
  if (String(row.status || '').toUpperCase() !== 'APPROVED') throw new Error('Valuation must be APPROVED to export');
  ...
  if (sensTable) {
    slides.push({
      ...
      content: {
        ...
        body:
          lang === 'pl'
            ? 'Tabela sensitivity: EV przy różnych WACC i g.'
            : 'Sensitivity table: EV under varying WACC and g.',
        tables: [sensTable],
