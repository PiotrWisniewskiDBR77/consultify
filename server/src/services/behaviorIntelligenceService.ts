...
export async function checkOptOut(userId: string): Promise<boolean> {
  try {
    const row = await dbGet<{ behavior_analytics_enabled?: number }>(
      `SELECT behavior_analytics_enabled FROM users WHERE id = ?`,
      [userId]
    );
...
export async function ingestJourneyBatch(
  ...
  for (const event of events) {
    try {
      await ingestJourneyEvent(userId, organizationId, event);
      ingested++;
    } catch (err) {
      logger.warn('Failed to ingest journey event:', err);
    }
  }
  return { ingested };
}
...
const ACTIVATION_RULES: Record<string, (name: string) => boolean> = {
  ...
  C: (n) =>
    n.includes('report_generated') ||
    n.includes('initiative_created') ||
    n.includes('assessment_completed'),
};
...
async function updateActivationStatus(userId: string, event: JourneyEvent): Promise<void> {
  try {
    ...
    if (!existing?.user_id) {
      await dbRun(
        `INSERT OR IGNORE INTO user_activation_status (user_id, current_phase, first_event_at, last_event_at, updated_at)
         VALUES (?, 'A', datetime('now'), datetime('now'), datetime('now'))`,
        [userId]
      );
    }
    ...
    if (ACTIVATION_RULES.B?.(event.eventName) && (currentPhase === 'A' || currentPhase === 'B'))
      newPhase = 'C';
    ...
    if (newPhase !== currentPhase) {
      ...
      if (newPhase === 'D' && existing?.first_event_at) {
        updates.push(
          `total_ttv_ms = CAST((julianday('now') - julianday(first_event_at)) * 86400000 AS INTEGER)`
        );
      }
    }
    await dbRun(`UPDATE user_activation_status SET ${updates.join(', ')} WHERE user_id = ?`, [
      userId,
    ]);
  } catch (err) {
    logger.warn('Failed to update activation status:', err);
  }
}
...
export async function getAdoptionMetrics(userId: string, organizationId?: string) {
  const [eventCounts, featureTop, activation, logins7d, logins30d, aiUsage] = await Promise.all([
    ...
    dbGet<{ current_phase: string }>(
      `SELECT current_phase FROM user_activation_status WHERE user_id = ?`,
      [userId]
    ),
    dbGet<{ cnt: number }>(
      `SELECT COUNT(*) as cnt FROM login_history WHERE user_id = ? AND login_at >= datetime('now', '-7 days')`,
      [userId]
    ),
    dbGet<{ cnt: number }>(
      `SELECT COUNT(*) as cnt FROM login_history WHERE user_id = ? AND login_at >= datetime('now', '-30 days')`,
      [userId]
    ),
    dbGet<{ cnt: number }>(
      `SELECT COUNT(*) as cnt FROM ai_usage_logs WHERE user_id = ? AND created_at >= datetime('now', '-30 days')`,
      [userId]
    ),
  ]);
  ...
  const engagementScore = Math.min(
    100,
    Math.round(featureUseCount * 2 + milestoneCount * 10 + loginCount7d * 5 + aiUsageCount * 1.5)
  );
  return {
    userId,
    totalEvents,
...
export async function generateChurnWarnings(organizationId: string): Promise<ChurnWarning[]> {
  ...
  const noLoginUsers = await dbAll<{ id: string; last_login: string }>(
    `SELECT u.id, u.last_login FROM users u
     ...
     AND (u.last_login IS NULL OR u.last_login < datetime('now', '-14 days'))`,
    [organizationId]
  );
  for (const user of noLoginUsers) {
    ...
    if (!existing) {
      const id = uuidv4();
      const daysSince = user.last_login
      ...
      await dbRun(
        ...
        [
          ...
          organizationId,
          user.id,
          daysSince > 30 ? 'HIGH' : 'MEDIUM',
          `User has not logged in for ${daysSince} days`,
          JSON.stringify({ daysSinceLogin: daysSince }),
        ]
      );
      warnings.push({
        id,
        ...
        status: 'ACTIVE',
        details_json: JSON.stringify({ daysSinceLogin: daysSince }),
      ...
      if (!existing) {
        ...
        await dbRun(
          ...
          [
            id,
            organizationId,
            `Activity dropped ${Math.round((1 - ratio) * 100)}% (7d vs 30d avg)`,
            JSON.stringify({
              events7d: usageDrop.events_7d,
              events30d: usageDrop.events_30d,
        ...
        warnings.push({
          id,
          organization_id: organizationId,
          warning_type: 'USAGE_DROP',
          severity: 'HIGH',
          status: 'ACTIVE',
          details_json: JSON.stringify({ ratio }),
          created_at: new Date().toISOString(),
        });
      }
    }
...
export async function getChurnPrediction(organizationId: string) {
  const [activeWarnings, loginActivity, eventActivity, orgUsers] = await Promise.all([
    dbAll<ChurnWarning>(
      `SELECT * FROM churn_warnings WHERE organization_id = ? AND status = 'ACTIVE'`,
      [organizationId]
    ),
    dbGet<{ cnt: number }>(
      `SELECT COUNT(DISTINCT user_id) as cnt FROM login_history WHERE organization_id = ? AND login_at >= datetime('now', '-7 days')`,
      [organizationId]
    ),
  ...
  if (warningCount > 0) {
    const highW = activeWarnings.filter(
      (w) => w.severity === 'HIGH' || w.severity === 'CRITICAL'
    ).length;
    const w = Math.min(40, warningCount * 10 + highW * 15);
    riskScore += w;
    factors.push({
      factor: 'Active warnings',
      weight: w,
      signal: `${warningCount} active (${highW} high/critical)`,
    });
  }
  ...
  if (loginRatio < 0.3) {
    ...
    factors.push({
      ...
      weight: w,
      signal: `${activeLogins}/${totalUsers} users active in 7d`,
    });
  }
  ...
  if (olderEvents > 5 && recentEvents < olderEvents * 0.25) {
    ...
    factors.push({
      ...
      weight: 20,
      signal: `${recentEvents} events (7d) vs ${olderEvents} (prev 23d)`,
    });
  }
  ...
  const healthTrend =
    recentEvents > olderEvents * 0.3 ? 'stable' : recentEvents > 0 ? 'declining' : 'inactive';
...
export async function calculateHealthScore(organizationId: string) {
  ...
  return {
    ...
    dimensions: {
      activation: activatedPct,
      featureAdoption: Math.min(100, featureCount * 10),
      engagement: prediction.overallScore,
      retention: prediction.churnRisk === 'LOW' ? 90 : prediction.churnRisk === 'MEDIUM' ? 60 : 30,
    },
  };
}
...
export async function getOrgBehaviorSummary(organizationId: string) {
  const [funnel, dau, wau, mau, topFeatures, warnings] = await Promise.all([
    dbAll<{ current_phase: string; cnt: number }>(
      `SELECT uas.current_phase, COUNT(*) as cnt FROM user_activation_status uas JOIN users u ON u.id = uas.user_id WHERE u.organization_id = ? GROUP BY uas.current_phase`,
      [organizationId]
    ),
    dbGet<{ cnt: number }>(
      `SELECT COUNT(DISTINCT user_id) as cnt FROM login_history WHERE organization_id = ? AND login_at >= datetime('now', '-1 day')`,
      [organizationId]
    ),
    dbGet<{ cnt: number }>(
      `SELECT COUNT(DISTINCT user_id) as cnt FROM login_history WHERE organization_id = ? AND login_at >= datetime('now', '-7 days')`,
    ...
    dbGet<{ cnt: number }>(
      `SELECT COUNT(DISTINCT user_id) as cnt FROM login_history WHERE organization_id = ? AND login_at >= datetime('now', '-30 days')`,
      [organizationId]
    ),
    dbAll<{ event_name: string; cnt: number }>(
      `SELECT event_name, COUNT(*) as cnt FROM journey_events WHERE organization_id = ? AND event_type = 'feature_use' AND created_at >= datetime('now', '-30 days') GROUP BY event_name ORDER BY cnt DESC LIMIT 10`,
      [organizationId]
    ),
    dbAll<ChurnWarning>(
      `SELECT * FROM churn_warnings WHERE organization_id = ? AND status = 'ACTIVE' ORDER BY created_at DESC`,
      [organizationId]
    ),
  ]);
...
export default {
  ...
  ingestJourneyBatch,
  getAdoptionMetrics,
  generateChurnWarnings,
  getChurnPrediction,
  calculateHealthScore,
  getOrgBehaviorSummary,
};
