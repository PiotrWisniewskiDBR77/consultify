...
export async function getAgentPlanSourceIdentity(
...
export async function createMaterializationProposal(input: {
  ...
  sourceHash: string;
  targetKind: MaterializationTarget;
  ...
  return withPgTransaction(async (tx) => {
    ...
    if (existing.rows[0]) {
      if (existing.rows[0].request_digest !== requestDigest)
        throw new Error('MYW_AGENT_IDEMPOTENCY_COLLISION');
    ...
    if (!Number.isFinite(expires.getTime()) || expires.getTime() <= Date.now())
      throw new Error('MYW_AGENT_EXPIRY_INVALID');
...
export async function decideMaterializationProposal(input: {
  ...
  return withPgTransaction(async (tx) => {
    ...
    if (proposal.requester_id === input.approverId)
      throw new Error('MYW_AGENT_SELF_APPROVAL_FORBIDDEN');
    if (proposal.source_hash !== input.sourceHash)
      throw new Error('MYW_AGENT_SOURCE_HASH_MISMATCH');
...
export async function materializeApprovedProposal(input: {
  ...
  const claimed = await withPgTransaction(async (tx) => {
    ...
    const receipt = await tx.query<any>(
      `SELECT * FROM myw_agent_materialization_receipts WHERE proposal_id=?`,
      [input.proposalId]
    );
    ...
    if (
      ...
      proposal.state_version !== input.expectedStateVersion
    )
      throw new Error('MYW_AGENT_MATERIALIZATION_STALE');
    if (new Date(proposal.expires_at).getTime() <= Date.now())
      throw new Error('MYW_AGENT_PROPOSAL_EXPIRED');
    const actual = await sourceIdentity(tx, input.organizationId, proposal.source_plan_id);
    if (
      actual.sourceVersion !== Number(proposal.source_version) ||
      actual.sourceHash !== proposal.source_hash
    ) {
      throw new Error('MYW_AGENT_SOURCE_DRIFT');
    }
    if (!receipt.rows[0]) {
  ...
  if ('busy' in claimed) {
    for (let attempt = 0; attempt < 250; attempt += 1) {
      ...
      if (receipt?.status === 'FAILED')
        throw new Error(receipt.last_error_code || 'MYW_AGENT_MATERIALIZATION_FAILED');
  ...
  try {
    ...
    if (proposal.target_kind === 'task') {
      ...
      const decision = await decisionService.createDecision({
        ...
        description: String(content.description || ''),
        type: 'APPROVAL',
        decisionMakerId: proposal.approver_id,
        stakeholderIds: [proposal.requester_id],
        createdBy: proposal.requester_id,
        idempotencyKey: commandKey,
        sourceType: 'myw_agent_proposal',
        sourceId: sourceIdentityValue,
      });
      ...
      const note = await createNotebookNote({
        ...
        title,
        body: String(content.body || content.description || ''),
    ...
    await withPgTransaction(async (tx) =>
      tx.query(
       ...
       WHERE proposal_id=? AND status='RUNNING' AND lease_owner=?`,
        [
          String(error instanceof Error ? error.message : error).slice(0, 128),
          proposal.proposal_id,
          workerId,
        ]
      )
