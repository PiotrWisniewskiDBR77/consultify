  ...
  generateRecommendations(scores: ADKARScores): ADKARRecommendation[] {
    ...
    gaps.forEach((gap) => {
      ...
      switch (gap.dimension) {
        ...
        case 'ability':
          recommendation =
            'Offer hands-on practice, coaching, and support resources to build skills.';
          priority = gap.gap > 1.5 ? 'high' : 'medium';
          break;
        case 'reinforcement':
          recommendation =
            'Establish recognition systems, celebrate wins, and ensure change sustainability.';
          priority = gap.gap > 1.5 ? 'high' : 'medium';
          break;
      }
...
interface ADKARAssessmentParsed extends Omit<
  ADKARAssessmentRow,
  'questionnaire_responses' | 'ai_recommendations'
> {
...
export const calculateScores = (responses: ADKARResponses) =>
  adkarServiceInstance.calculateScores(responses);
export const identifyGaps = (scores: ADKARScores, threshold?: number) =>
  adkarServiceInstance.identifyGaps(scores, threshold);
export const generateRecommendations = (scores: ADKARScores) =>
  adkarServiceInstance.generateRecommendations(scores);
export const getReadinessLevel = (scores: ADKARScores) =>
  adkarServiceInstance.getReadinessLevel(scores);
...
export const getAssessment = (assessmentId: string) =>
  adkarServiceInstance.getAssessment(assessmentId);
