...
class ThreatIntelligenceServiceClass {
  ...
  async getThreats(filters: any = {}): Promise<any[]> {
    ...
    return await this.db.all(
      'SELECT * FROM threat_intelligence ORDER BY created_at DESC LIMIT ? OFFSET ?',
      [limit, offset]
    );
  }
  ...
  async blockThreat(id: string): Promise<boolean> {
    const result = await this.db.run('UPDATE threat_intelligence SET is_blocked = 1 WHERE id = ?', [
      id,
    ]);
    return (result as any).changes > 0;
  }
  ...
  async unblockThreat(id: string): Promise<boolean> {
    const result = await this.db.run('UPDATE threat_intelligence SET is_blocked = 0 WHERE id = ?', [
      id,
    ]);
    return (result as any).changes > 0;
  }
  ...
  async checkIPReputation(ip: string): Promise<any> {
    const threat = await this.db.get(
      'SELECT * FROM threat_intelligence WHERE type = "ip" AND indicator = ?',
      [ip]
    );
    return {
      ip,
      score: threat ? (threat.severity === 'high' ? 90 : 50) : 0,
  ...
  async checkDomainReputation(domain: string): Promise<any> {
    const threat = await this.db.get(
      'SELECT * FROM threat_intelligence WHERE type = "domain" AND indicator = ?',
      [domain]
    );
    return {
      domain,
  ...
  async getBlockedIPs(): Promise<string[]> {
    const rows = await this.db.all(
      'SELECT indicator FROM threat_intelligence WHERE type = "ip" AND is_blocked = 1'
    );
  ...
  async getBlockedDomains(): Promise<string[]> {
    const rows = await this.db.all(
      'SELECT indicator FROM threat_intelligence WHERE type = "domain" AND is_blocked = 1'
    );
    return rows.map((r: any) => r.indicator);
  }
