...
class OrganizationMetadataServiceClass {
    ...
    async getMetadata(orgId: string): Promise<any[]> {
        return await this.db.all('SELECT * FROM organization_metadata WHERE organization_id = ?', [orgId]);
    }

    async setMetadata(orgId: string, key: string, value: string, valueType: string = 'string', category: string = 'general', isSensitive: boolean = false): Promise<void> {
        await this.db.run(
    ...
    async deleteMetadata(orgId: string, key: string): Promise<boolean> {
        const result = await this.db.run('DELETE FROM organization_metadata WHERE organization_id = ? AND key = ?', [orgId, key]);
