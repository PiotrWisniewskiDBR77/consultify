...
const safe = (a: number | undefined, b: number | undefined, op: (a: number, b: number) => number): number | null => {
...
export const RATIO_CATALOG: RatioDefinition[] = [
  // ── Liquidity ──
  {
    code: 'CURRENT_RATIO', name: 'Current Ratio', namePl: 'Wskaźnik bieżącej płynności',
    category: 'liquidity', formula: 'Current Assets / Current Liabilities',
    ...
    thresholds: { warn: 1.2, critical: 1.0, direction: 'higher_better' }, unit: 'x',
  },
  {
    code: 'QUICK_RATIO', name: 'Quick Ratio', namePl: 'Wskaźnik szybkiej płynności',
    category: 'liquidity', formula: '(Current Assets − Inventory) / Current Liabilities',
    ...
    thresholds: { warn: 0.8, critical: 0.5, direction: 'higher_better' }, unit: 'x',
  },
  {
    code: 'CASH_RATIO', name: 'Cash Ratio', namePl: 'Wskaźnik gotówkowej płynności',
    category: 'liquidity', formula: 'Cash / Current Liabilities',
    ...
    thresholds: { warn: 0.2, critical: 0.1, direction: 'higher_better' }, unit: 'x',
  },

  // ── Profitability ──
  {
    code: 'GROSS_MARGIN', name: 'Gross Margin', namePl: 'Marża brutto',
    category: 'profitability', formula: 'Gross Margin / Revenue × 100',
    ...
    thresholds: { warn: 20, critical: 10, direction: 'higher_better' }, unit: '%',
  },
  {
    code: 'OPERATING_MARGIN', name: 'Operating Margin', namePl: 'Marża operacyjna',
    category: 'profitability', formula: 'EBIT / Revenue × 100',
    ...
    thresholds: { warn: 5, critical: 0, direction: 'higher_better' }, unit: '%',
  },
  {
    code: 'NET_MARGIN', name: 'Net Margin', namePl: 'Marża netto',
    category: 'profitability', formula: 'Net Income / Revenue × 100',
    ...
    thresholds: { warn: 3, critical: 0, direction: 'higher_better' }, unit: '%',
  },
  {
    code: 'EBITDA_MARGIN', name: 'EBITDA Margin', namePl: 'Marża EBITDA',
    category: 'profitability', formula: 'EBITDA / Revenue × 100',
    ...
    thresholds: { warn: 10, critical: 5, direction: 'higher_better' }, unit: '%',
  },
  {
    code: 'ROA', name: 'Return on Assets', namePl: 'Zwrot z aktywów (ROA)',
    category: 'profitability', formula: 'Net Income / Total Assets × 100',
    ...
    thresholds: { warn: 3, critical: 1, direction: 'higher_better' }, unit: '%',
  },
  {
    code: 'ROE', name: 'Return on Equity', namePl: 'Zwrot z kapitału własnego (ROE)',
    category: 'profitability', formula: 'Net Income / Equity × 100',
    ...
    thresholds: { warn: 8, critical: 3, direction: 'higher_better' }, unit: '%',
  },

  // ── Leverage ──
  {
    code: 'DEBT_TO_EQUITY', name: 'Debt-to-Equity', namePl: 'Wskaźnik zadłużenia do kapitału',
    category: 'leverage', formula: 'Total Liabilities / Equity',
    ...
    thresholds: { warn: 2.0, critical: 3.0, direction: 'lower_better' }, unit: 'x',
  },
  {
    code: 'DEBT_RATIO', name: 'Debt Ratio', namePl: 'Wskaźnik zadłużenia',
    category: 'leverage', formula: 'Total Liabilities / Total Assets',
    ...
    thresholds: { warn: 0.6, critical: 0.8, direction: 'lower_better' }, unit: 'x',
  },
  {
    code: 'INTEREST_COVERAGE', name: 'Interest Coverage', namePl: 'Wskaźnik pokrycia odsetek',
    category: 'leverage', formula: 'EBIT / Interest Expense',
    ...
    thresholds: { warn: 3.0, critical: 1.5, direction: 'higher_better' }, unit: 'x',
  },

  // ── Efficiency ──
  {
    code: 'INVENTORY_TURNOVER', name: 'Inventory Turnover', namePl: 'Rotacja zapasów',
    category: 'efficiency', formula: 'COGS / Inventory',
    ...
    thresholds: { warn: 4, critical: 2, direction: 'higher_better' }, unit: 'x',
  },
  {
    code: 'AR_DAYS', name: 'AR Days (DSO)', namePl: 'Dni rotacji należności (DSO)',
    category: 'efficiency', formula: '(AR / Revenue) × 365',
    ...
    thresholds: { warn: 60, critical: 90, direction: 'lower_better' }, unit: 'days',
  },
  {
    code: 'AP_DAYS', name: 'AP Days (DPO)', namePl: 'Dni rotacji zobowiązań (DPO)',
    category: 'efficiency', formula: '(AP / COGS) × 365',
    ...
    thresholds: { warn: 90, critical: 120, direction: 'lower_better' }, unit: 'days',
  },
  {
    code: 'CCC', name: 'Cash Conversion Cycle', namePl: 'Cykl konwersji gotówki (CCC)',
    category: 'efficiency', formula: 'DSO + DIO − DPO',
...
export async function computeRatios(
  ...
  const stmtRows = await dbAll(`SELECT * FROM financial_statements WHERE id = ?`, [statementId]) as any[];
  ...
  for (const def of RATIO_CATALOG) {
    ...
    const coveragePct = def.requiredLines.length > 0
      ? Math.round(((def.requiredLines.length - missingLines.length) / def.requiredLines.length) * 100)
      : 100;
    ...
    ratios.push({
      ...
      ...(bm ? {
        benchmark: {
          ...
          targetMin: bm.target_min, targetMax: bm.target_max,
  ...
  return {
    ...
    ratios,
    categories,
    coverageSummary: { total: ratios.length, computed, na: ratios.length - computed, coveragePct: Math.round((computed / ratios.length) * 100) },
  };
}
...
export async function computeGrowthRatios(
  ...
  const addGrowth = (lineCode: string, name: string, namePl: string) => {
    ...
    growthRatios.push({
      code: `${lineCode}_GROWTH`, name, namePl, category: 'growth',
      value, status, formula: `(Current − Previous) / |Previous| × 100`,
      formulaDescription: `Year-over-year change in ${name}`,
      formulaDescriptionPl: `Zmiana rok do roku: ${namePl}`,
      unit: '%', coveragePct: missingLines.length === 0 ? 100 : 0, missingLines,
    });
  };
...
async function persistRatioSnapshots(statementId: string, organizationId: string, ratios: ComputedRatio[]): Promise<void> {
  ...
  for (const r of ratios) {
    await dbRun(
      `INSERT INTO financial_ratio_snapshots (id, organization_id, statement_id, ratio_code, ratio_value, status, coverage_pct, missing_lines)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [uuidv4(), organizationId, statementId, r.code, r.value, r.status, r.coveragePct, r.missingLines.length > 0 ? JSON.stringify(r.missingLines) : null]
    );
  }
}
...
export async function upsertBenchmark(params: {
  ...
  await dbRun(
    ...
    [id, params.organizationId, params.ratioCode, params.industry || null,
     params.region || null, params.companySize || null, params.periodYear || null,
     params.p25 ?? null, params.median ?? null, params.p75 ?? null,
     params.targetMin ?? null, params.targetMax ?? null,
     params.sourceLabel || null, params.createdBy || null]
  );
  return id;
}

export async function getBenchmarks(organizationId: string): Promise<any[]> {
  return (await dbAll(`SELECT * FROM financial_ratio_benchmarks WHERE organization_id = ? ORDER BY ratio_code`, [organizationId])) || [];
}
