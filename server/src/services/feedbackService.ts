...
---`
  ...
  async voteFeedback(
    ...
    try {
      ...
      // Update votes count
      await this.dbRun(`UPDATE feedback_items SET votes_count = votes_count + 1 WHERE id = ?`, [
        feedbackId,
      ]);
   ...
   */
  async updateFeatureRoadmap(
    itemId: string,
    updates: FeatureRoadmapUpdate
...
export const getLearningExamples = (contextType: string) =>
  feedbackService.getLearningExamples(contextType);
export const consolidateLearning = () => feedbackService.consolidateLearning();
export const getFeedbackItems = (filters?: FeedbackFilters) =>
  feedbackService.getFeedbackItems(filters);
...
export const getFeatureRoadmap = (status?: string | null) =>
  feedbackService.getFeatureRoadmap(status);
