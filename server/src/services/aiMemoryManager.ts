...
export const AIMemoryManager = {
     ...
     */
    recordProjectMemory: async (projectId: string, memoryType: MemoryType, content: any, userId: string) => {
        const id = uuidv4();
    ...
    buildProjectMemorySummary: async (projectId: string) => {
        ...
        const transitions = await AIMemoryManager.getProjectMemory(projectId, MemoryType.PHASE_TRANSITION, 3);
        const recommendations = await AIMemoryManager.getProjectMemory(projectId, MemoryType.RECOMMENDATION, 5);
     ...
     */
    getRelevantMemory: async (projectId: string, query: string, limit = 10, minRelevance = 0.1): Promise<any[]> => {
    ...
    getOrganizationMemory: async (organizationId: string) => {
        const row: any = await get(`SELECT * FROM ai_organization_memory WHERE organization_id = ?`, [organizationId]);
    ...
    clearOrganizationMemory: async (organizationId: string) => {
        const result = await run(`DELETE FROM ai_organization_memory WHERE organization_id = ?`, [organizationId]);
        return { deleted: result.changes };
    },
    ...
    getPersonalizationProfile: async (userId: string | null): Promise<PersonalizationProfile> => {
        ...
        const row: any = await get(`SELECT preferences FROM user_ai_preferences WHERE user_id = ?`, [userId]);
     ...
     */
    updatePersonalizationProfile: async (userId: string, preferences: Partial<PersonalizationProfile>) => {
