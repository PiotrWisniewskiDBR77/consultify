...
/** Flatten an arbitrary facts object into a set of stringified values for a
 * loose "does this number appear in the facts pool" check (R5 approximation). */
function flattenFactValues(facts: Record<string, unknown>): string[] {
  const out: string[] = [];
...
export function validateNumbersFromEngine(input: ValidatableConclusion): ValidatorResult {
  ...
  const pool = flattenFactValues(input.facts)
