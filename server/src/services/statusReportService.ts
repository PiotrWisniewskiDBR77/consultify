...
export class StatusReportService {
     ...
     */
    async generateReport(orgId: string, initiativeId: string, periodType: string, userId: string, options: any = {}) {
        const id = uuidv4();
        const now = new Date();

        // Calculate period dates
        const { periodStart, periodEnd, periodLabel } = this.calculatePeriod(periodType, options.periodDate);
     ...
     */
    async gatherReportData(initiativeId: string, orgId: string, periodStart: string, periodEnd: string) {
        // Initiative progress
        const initiative = await DbPromise.get<any>(
            this.db,
            `
    ...
    calculateSectionStatuses(data: any) {
        ...
        // Budget section
        const budgetStatus = data.isOverBudget ? 'RED' : data.budgetConsumedPercent >= 90 ? 'AMBER' : 'GREEN';
        ...
        sections[SECTION_NAMES.RISKS] = {
            ...
            issues:
                data.highPriorityItems > 0 ? [`${data.highPriorityItems} high-priority items require attention`] : [],
        };
    ...
    async getReport(reportId: string, orgId: string) {
        ...
        return {
            ...
            createdBy: report.created_by_first ? `${report.created_by_first} ${report.created_by_last}` : null,
    ...
    async updateReport(reportId: string, updates: any, userId: string) {
        ...
        for (const [key, value] of Object.entries(updates)) {
            ...
            if (allowedFields.includes(snakeKey)) {
                ...
                params.push(
                    ['accomplishments', 'next_steps', 'escalations'].includes(snakeKey) ? JSON.stringify(value) : value,
                );
            }
        }
        ...
        await DbPromise.run(this.db, `UPDATE status_reports SET ${setClauses.join(', ')} WHERE id = ?`, params);
