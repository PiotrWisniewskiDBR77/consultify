/**
 * DRD Industry Reference Profiles (expert hypothesis v1) — server mirror.
 *
 * Server-local copy of `src/services/assessmentKnowledge/drdIndustryProfiles.ts`.
 * Duplicated intentionally (same reasoning as `drdReportService.ts`): production
 * runs `node dist/src/index.js`, whose `dist/` contains only `server/src/**` — a
 * cross-tree import of the FE `.ts` source would resolve in dev (tsx) but be
 * absent in the compiled prod bundle. Keep the two copies in sync.
 *
 ...
 * - Wiring into the client report is prepared on branch
 *   `oxford/o6-profiles-report`, pending owner decision P3 (publish
 *   immediately with annotation vs after calibration).
 */
