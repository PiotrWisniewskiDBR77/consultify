...
export type ConsultantInviteType =
  (typeof CONSULTANT_INVITE_TYPES)[keyof typeof CONSULTANT_INVITE_TYPES];
...
class ConsultantServiceClass {
  ...
  async getLinkedOrganizations(consultantId: string): Promise<LinkedOrganization[]> {
    ...
    return rows.map((row) => {
      const parsed = row.permission_scope
        ? (JSON.parse(row.permission_scope) as Record<string, unknown>)
        : {};
      return {
        ...row,
  ...
  async verifyAccess(
    ...
    | (Omit<ConsultantOrgLink, 'permission_scope'> & { permission_scope: Record<string, unknown> })
    | null
  > {
    ...
    if (inviteType === 'TRIAL_ORG' || inviteType === 'TRIAL') {
      ...
      if (!orgId) {
        if (!targetOrganizationId)
          throw new Error('Organization context required to link consultant');
      }
  ...
  async revokeLink(linkId: string): Promise<void> {
    await this.deps.db.run(`UPDATE consultant_org_links SET status = 'REVOKED' WHERE id = ?`, [
      linkId,
    ]);
  }
}
...
export const getConsultantProfile = (userId: string) =>
  consultantServiceInstance.getConsultantProfile(userId);
...
export const createInvite = (params: CreateInviteParams) =>
  consultantServiceInstance.createInvite(params);
export const validateInvite = (code: string) => consultantServiceInstance.validateInvite(code);
export const recordInviteUsage = (code: string) =>
  consultantServiceInstance.recordInviteUsage(code);
