 ...
 */
import type { DesignTokens, LayoutResult, UnifiedSlide, UnifiedReportMeta, PerformanceOverviewContent } from '../types.js';
...
export function KpiDashboardLayout(
  ...
  elements.push(SlideTitle({ text: slide.key_message || (meta.language === 'pl' ? 'Przegląd Wyników' : 'Performance Overview') }, tokens));
  ...
  if (c.context) {
    elements.push(
      BodyText({
        text: c.context,
        position: { x: g.contentX, y: kpiY + kpiH + 0.2, w: g.contentW, h: g.contentH - kpiH - 0.5 },
        color: tokens.colors.textSecondary,
      }, tokens)
    );
  }
