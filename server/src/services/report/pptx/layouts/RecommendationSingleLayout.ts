 ...
 */
import type { DesignTokens, LayoutResult, UnifiedSlide, UnifiedReportMeta, RecommendationSingleContent } from '../types.js';
...
export function RecommendationSingleLayout(
  ...
  elements.push(SlideTitle({ text: slide.key_message || (meta.language === 'pl' ? 'Rekomendacja' : 'Recommendation') }, tokens));
  ...
  const cardElements = RecommendationCard({
    ...
    priority: c.priority,
    timeline: c.timeline,
    position: { x: g.contentX + 0.5, y: g.contentY + 0.1, w: g.contentW - 1, h: g.contentH - 0.2 },
  }, tokens);
