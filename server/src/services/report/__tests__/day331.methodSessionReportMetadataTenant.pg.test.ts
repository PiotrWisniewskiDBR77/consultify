...
describe('Day331 method-session report metadata tenant guard', NO_RETRY, () => {
  ...
  beforeAll(async () => {
    ...
    await sql.connect();
    await sql.query(
      `INSERT INTO organizations (id,name) VALUES ($1,'Day331 owner'),($2,'Day331 foreign')`,
      [ownerOrg, foreignOrg]
    );
    await sql.query(`INSERT INTO users (id,organization_id,email,role) VALUES ($1,$2,$3,'user')`, [
      ownerUser,
      ownerOrg,
      `${ownerUser}@example.test`,
    ]);
    await sql.query(
  ...
  it('allows the owner tenant and refuses a foreign overwrite without changing the cold read', async () => {
    const { methodSessionReportMetadataService: service } =
      await import('../methodSessionReportMetadataService.js');
  ...
  it('keeps the owner row when owner and foreign tenant save concurrently', async () => {
    const { methodSessionReportMetadataService: service } =
      await import('../methodSessionReportMetadataService.js');
    const input = (organizationId: string, studyScope: string) => ({
      sessionId,
    ...
    const [ownerResult, foreignResult] = await Promise.allSettled([
      service.save(input(ownerOrg, 'owner concurrent value')),
      service.save(input(foreignOrg, 'foreign concurrent value')),
    ]);
    expect(ownerResult.status).toBe('fulfilled');
    expect(foreignResult.status).toBe('rejected');
    ...
    expect(stored.rows).toEqual([
      { organization_id: ownerOrg, study_scope: 'owner concurrent value' },
    ]);
  });
});
