...
const auditService = {
  ...
  async getRecordHistory(
    ...
    try {
      const result = await db.query(
        `SELECT ae.id, ae.entity_id, ae.actor_id, ae.event_type,
                ae.before_data, ae.after_data, ae.metadata, ae.created_at,
                u.name AS user_name
         FROM tp_audit_events ae
         LEFT JOIN users u ON u.id = ae.actor_id
         WHERE ae.entity_type = 'record' AND ae.entity_id = $1
      ...
      return result.rows.map((row: any) => {
        const before = parseJsonSafe(row.before_data);
        const after = parseJsonSafe(row.after_data);
        const beforeData = before?.data ? parseJsonSafe(before.data) : before;
        const afterData = after?.data ? parseJsonSafe(after.data) : after;
        ...
        return {
          ...
          action,
          changes: extractFieldChanges(beforeData, afterData),
    ...
    try {
      let query = `
        SELECT ae.id, ae.event_type, ae.entity_type, ae.entity_id,
               ae.actor_id, ae.before_data, ae.after_data, ae.metadata, ae.created_at,
               u.name AS actor_name
        FROM tp_audit_events ae
        LEFT JOIN users u ON u.id = ae.actor_id
        WHERE ae.metadata->>'table_id' = $1
      `;
      const params: unknown[] = [tableId];

      if (options?.since) {
        params.push(options.since);
        query += ` AND ae.created_at > $${params.length}`;
      }

      params.push(limit);
      query += ` ORDER BY ae.created_at DESC LIMIT $${params.length}`;
