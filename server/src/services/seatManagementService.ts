...
export async function purchaseSeats(
    ...
    if (config.max_seats && config.max_seats > 0) {
        ...
        if (newTotal > config.max_seats) {
            throw new Error(`Cannot purchase ${quantity} seats. Maximum seats limit is ${config.max_seats}`);
        }
    }
 ...
 */
export async function autoAddSeatOnInvite(orgId: string, userId: string): Promise<AutoAddSeatResult> {
    const config = await getSeatConfiguration(orgId);
 ...
 */
export async function getSeatHistory(orgId: string, limit: number = 50): Promise<SeatTransactionRow[]> {
