...
class AdminAuditServiceClass {
    ...
    async getLogs(filters: any = {}): Promise<any> {
        ...
        const logs = await this.db.all('SELECT * FROM admin_audit_logs ORDER BY created_at DESC LIMIT ? OFFSET ?', [limit, offset]);
        return logs;
    }

    async resolveLog(id: string, resolvedBy: string): Promise<boolean> {
        const result = await this.db.run('UPDATE admin_audit_logs SET status = ?, resolved_by = ? WHERE id = ?', ['resolved', resolvedBy, id]);
    ...
    async getRecentHighRisk(limit: number = 5): Promise<any[]> {
        return await this.db.all('SELECT * FROM admin_audit_logs WHERE risk_score >= 60 ORDER BY created_at DESC LIMIT ?', [limit]);
    }

    async cleanupOldLogs(days: number = 90): Promise<number> {
        const result = await this.db.run("DELETE FROM admin_audit_logs WHERE status = 'resolved' AND created_at < date('now', '-' || ? || ' days')", [days]);
