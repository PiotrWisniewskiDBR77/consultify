...
class FinanceEnterpriseService {
  ...
  async createModelVersion(
    ...
    await queryHelpers.queryRun(
      `INSERT INTO financial_model_versions
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?)`,
      [id, orgId, data.modelId, versionNumber, data.parentVersionId || null, data.scenarioLabel || 'base', assumptionsSnapshot, eventsSnapshot, outputsSnapshot, userId, now]
    );

    return { id, modelId: data.modelId, versionNumber, parentVersionId: data.parentVersionId || null, scenarioLabel: data.scenarioLabel || 'base', status: 'draft', createdBy: userId, createdAt: now };
  }
  ...
  async compareVersions(orgId: string, fromVersionId: string, toVersionId: string) {
    const from = await queryHelpers.queryOne<any>(`SELECT * FROM financial_model_versions WHERE id = ? AND organization_id = ?`, [fromVersionId, orgId]);
    const to = await queryHelpers.queryOne<any>(`SELECT * FROM financial_model_versions WHERE id = ? AND organization_id = ?`, [toVersionId, orgId]);
  ...
  async createDimension(orgId: string, data: { dimensionName: string; dimensionType?: string; hierarchy?: unknown[] }): Promise<Dimension> {
    const id = uuidv4();
    await queryHelpers.queryRun(
      `INSERT INTO financial_dimensions (id, organization_id, dimension_name, dimension_type, hierarchy) VALUES (?, ?, ?, ?, ?)`,
      [id, orgId, data.dimensionName, data.dimensionType || 'custom', JSON.stringify(data.hierarchy || [])]
    );
    return { id, dimensionName: data.dimensionName, dimensionType: data.dimensionType || 'custom', hierarchy: data.hierarchy || [] };
  }

  async getDimensions(orgId: string): Promise<Dimension[]> {
    const rows = (await queryHelpers.queryAll<any>(
      `SELECT * FROM financial_dimensions WHERE organization_id = ? ORDER BY created_at`, [orgId]
    )) || [];
    return rows.map((r: any) => ({ id: r.id, dimensionName: r.dimension_name, dimensionType: r.dimension_type, hierarchy: safeJsonArray(r.hierarchy) }));
  }

  async createAllocation(orgId: string, data: {
    modelId: string; sourceDimensionId?: string; targetDimensionId?: string;
    allocationMethod?: string; allocationRules?: Record<string, unknown>; amount?: number; period?: string;
  }): Promise<{ id: string }> {
    const id = uuidv4();
    await queryHelpers.queryRun(
      `INSERT INTO financial_allocations (id, organization_id, model_id, source_dimension_id, target_dimension_id, allocation_method, allocation_rules, amount, period)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, orgId, data.modelId, data.sourceDimensionId || null, data.targetDimensionId || null, data.allocationMethod || 'proportional', JSON.stringify(data.allocationRules || {}), data.amount || null, data.period || null]
    );
    return { id };
  }
  ...
  async getAllocations(orgId: string, modelId: string) {
    const rows = (await queryHelpers.queryAll<any>(
      `SELECT * FROM financial_allocations WHERE organization_id = ? AND model_id = ? ORDER BY created_at`, [orgId, modelId]
    )) || [];
    return rows.map((r: any) => ({
      id: r.id, modelId: r.model_id, sourceDimensionId: r.source_dimension_id, targetDimensionId: r.target_dimension_id,
      allocationMethod: r.allocation_method, allocationRules: safeJson(r.allocation_rules), amount: r.amount, period: r.period,
    }));
  }

  async createConsolidation(orgId: string, userId: string, data: {
    name: string; sourceModelIds: string[]; consolidationRules?: Record<string, unknown>;
  }): Promise<{ id: string }> {
    const id = uuidv4();
    await queryHelpers.queryRun(
      `INSERT INTO financial_consolidations (id, organization_id, name, source_model_ids, consolidation_rules, created_by)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [id, orgId, data.name, JSON.stringify(data.sourceModelIds), JSON.stringify(data.consolidationRules || {}), userId]
    );
    return { id };
  }
  ...
  async getConsolidations(orgId: string) {
    const rows = (await queryHelpers.queryAll<any>(
      `SELECT * FROM financial_consolidations WHERE organization_id = ? ORDER BY created_at DESC`, [orgId]
    )) || [];
    return rows.map((r: any) => ({
      id: r.id, name: r.name, sourceModelIds: safeJsonArray(r.source_model_ids),
      consolidationRules: safeJson(r.consolidation_rules), resultSnapshot: safeJson(r.result_snapshot),
  ...
  async createBudgetVersion(orgId: string, userId: string, data: {
    modelId: string; budgetType?: string; fiscalYear: number; versionLabel?: string;
    plannedData: Record<string, unknown>;
    ...
    await queryHelpers.queryRun(
      `INSERT INTO financial_budget_versions
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?)`,
      [id, orgId, data.modelId, data.budgetType || 'annual', data.fiscalYear, data.versionLabel || 'v1', JSON.stringify(data.plannedData), userId, now, now]
    );
    return { id, modelId: data.modelId, budgetType: data.budgetType || 'annual', fiscalYear: data.fiscalYear, versionLabel: data.versionLabel || 'v1', plannedData: data.plannedData, actualData: {}, varianceData: {}, status: 'draft' };
  }
  ...
  async updateBudgetActuals(orgId: string, budgetVersionId: string, actualData: Record<string, unknown>): Promise<boolean> {
    ...
    for (const key of Object.keys(actualData)) {
      ...
      variance[key] = { planned: p, actual: a, delta: a - p, pct: p !== 0 ? ((a - p) / Math.abs(p) * 100) : 0 };
    }

    await queryHelpers.queryRun(
      `UPDATE financial_budget_versions SET actual_data = ?, variance_data = ?, updated_at = ? WHERE id = ? AND organization_id = ?`,
      [JSON.stringify(actualData), JSON.stringify(variance), new Date().toISOString(), budgetVersionId, orgId]
    );
    return true;
  }
  ...
  async createForecastCycle(orgId: string, data: {
    modelId: string; cycleName: string; cycleType?: string; forecastHorizonMonths?: number;
  }): Promise<{ id: string }> {
    const id = uuidv4();
    await queryHelpers.queryRun(
      `INSERT INTO financial_forecast_cycles (id, organization_id, model_id, cycle_name, cycle_type, forecast_horizon_months)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [id, orgId, data.modelId, data.cycleName, data.cycleType || 'monthly', data.forecastHorizonMonths || 12]
    );
    return { id };
  }
  ...
  async getVarianceAlerts(orgId: string, budgetVersionId: string) {
    ...
    return rows.map((r: any) => ({
      id: r.id, lineItem: r.line_item, period: r.period,
      plannedAmount: r.planned_amount, actualAmount: r.actual_amount,
      variancePct: r.variance_pct, severity: r.severity,
  ...
  async createConnector(orgId: string, userId: string, data: {
    connectorType: string; name: string; config: Record<string, unknown>; syncDirection?: string;
  }): Promise<Connector> {
    const id = uuidv4();
    await queryHelpers.queryRun(
      `INSERT INTO financial_connectors (id, organization_id, connector_type, name, config, sync_direction, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [id, orgId, data.connectorType, data.name, JSON.stringify(data.config), data.syncDirection || 'import', userId]
    );
    return { id, connectorType: data.connectorType, name: data.name, config: data.config, syncDirection: data.syncDirection || 'import', lastSyncAt: null, lastSyncStatus: 'never' };
  }

  async getConnectors(orgId: string): Promise<Connector[]> {
    const rows = (await queryHelpers.queryAll<any>(
      `SELECT * FROM financial_connectors WHERE organization_id = ? ORDER BY created_at DESC`, [orgId]
    )) || [];
    return rows.map((r: any) => ({
      id: r.id, connectorType: r.connector_type, name: r.name,
      config: safeJson(r.config), syncDirection: r.sync_direction,
      lastSyncAt: r.last_sync_at, lastSyncStatus: r.last_sync_status,
    }));
  }

  async logSync(orgId: string, connectorId: string, data: {
    syncType: string; recordsProcessed: number; recordsCreated: number;
    recordsUpdated: number; recordsErrors: number; errorDetails?: unknown[];
  }): Promise<{ id: string }> {
    const id = uuidv4();
    const now = new Date().toISOString();
    await queryHelpers.queryRun(
      `INSERT INTO financial_sync_log (id, organization_id, connector_id, sync_type, records_processed, records_created, records_updated, records_errors, error_details, started_at, completed_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, orgId, connectorId, data.syncType, data.recordsProcessed, data.recordsCreated, data.recordsUpdated, data.recordsErrors, JSON.stringify(data.errorDetails || []), now, now]
    );
  ...
  async getSyncLog(orgId: string, connectorId: string, limit: number = 20) {
    ...
    return rows.map((r: any) => ({
      id: r.id, syncType: r.sync_type, recordsProcessed: r.records_processed,
      recordsCreated: r.records_created, recordsUpdated: r.records_updated,
      recordsErrors: r.records_errors, errorDetails: safeJsonArray(r.error_details),
  ...
  async createValuationSnapshot(orgId: string, userId: string, data: {
    modelId: string; valuationMethod?: string; inputs: Record<string, unknown>; outputs: Record<string, unknown>;
  }): Promise<ValuationSnapshot> {
    const id = uuidv4();
    const now = new Date().toISOString();
    const assumptionsHash = crypto.createHash('sha256').update(JSON.stringify(data.inputs)).digest('hex').substring(0, 16);
    ...
    await queryHelpers.queryRun(
      `INSERT INTO financial_valuation_snapshots (id, organization_id, model_id, valuation_method, assumptions_hash, inputs, outputs, version, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, orgId, data.modelId, data.valuationMethod || 'dcf', assumptionsHash, JSON.stringify(data.inputs), JSON.stringify(data.outputs), version, userId, now]
    );
    ...
    return { id, modelId: data.modelId, valuationMethod: data.valuationMethod || 'dcf', inputs: data.inputs, outputs: data.outputs, version, createdBy: userId, createdAt: now };
  }
  ...
  async getValuationSnapshots(orgId: string, modelId: string): Promise<ValuationSnapshot[]> {
    ...
    return rows.map((r: any) => ({
      id: r.id, modelId: r.model_id, valuationMethod: r.valuation_method,
      inputs: safeJson(r.inputs), outputs: safeJson(r.outputs),
  ...
  async getValuationAudit(orgId: string, snapshotId: string) {
    ...
    return rows.map((r: any) => ({
      ...
      changeDetail: safeJson(r.change_detail), createdAt: r.created_at,
    }));
  }
  ...
  async createAIAssumption(orgId: string, data: {
    modelId: string; assumptionKey: string; assumptionValue: string;
    confidence?: number; sourceCitations?: unknown[]; aiModelUsed?: string;
    ...
    await queryHelpers.queryRun(
      `INSERT INTO financial_ai_assumptions (id, organization_id, model_id, assumption_key, assumption_value, confidence, source_citations, ai_model_used)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, orgId, data.modelId, data.assumptionKey, data.assumptionValue, data.confidence || 0.7, JSON.stringify(data.sourceCitations || []), data.aiModelUsed || null]
    );
    return { id, modelId: data.modelId, assumptionKey: data.assumptionKey, assumptionValue: data.assumptionValue, confidence: data.confidence || 0.7, sourceCitations: data.sourceCitations || [], status: 'proposed' };
  }
  ...
  async getAIAssumptions(orgId: string, modelId: string): Promise<AIAssumption[]> {
    ...
    return rows.map((r: any) => ({
      id: r.id, modelId: r.model_id, assumptionKey: r.assumption_key,
      assumptionValue: r.assumption_value, confidence: r.confidence,
      sourceCitations: safeJsonArray(r.source_citations), status: r.status,
    }));
  }
  ...
  async createROILink(orgId: string, data: {
    modelId: string; initiativeId?: string; benefitId?: string; assumptionIds?: string[];
  }): Promise<ROILink> {
    const id = uuidv4();
    await queryHelpers.queryRun(
      `INSERT INTO financial_roi_links (id, organization_id, model_id, initiative_id, benefit_id, assumption_ids)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [id, orgId, data.modelId, data.initiativeId || null, data.benefitId || null, JSON.stringify(data.assumptionIds || [])]
    );
    return { id, modelId: data.modelId, initiativeId: data.initiativeId || null, benefitId: data.benefitId || null, realizedValue: null, linkStatus: 'projected' };
  }
  ...
  async getROILinks(orgId: string, modelId: string): Promise<ROILink[]> {
    ...
    return rows.map((r: any) => ({
      id: r.id, modelId: r.model_id, initiativeId: r.initiative_id,
      benefitId: r.benefit_id, realizedValue: r.realized_value, linkStatus: r.link_status,
    }));
  }

  async captureRealizedValue(orgId: string, linkId: string, data: {
    realizedValue: number; evidence: unknown[];
  }): Promise<boolean> {
    const result = await queryHelpers.queryRun(
...
function mapModelVersionRow(r: any): ModelVersion {
  return {
    id: r.id, modelId: r.model_id, versionNumber: r.version_number,
    parentVersionId: r.parent_version_id, scenarioLabel: r.scenario_label,
    status: r.status, createdBy: r.created_by, createdAt: r.created_at,
  };
}
...
function mapBudgetVersionRow(r: any): BudgetVersion {
  return {
    id: r.id, modelId: r.model_id, budgetType: r.budget_type,
    fiscalYear: r.fiscal_year, versionLabel: r.version_label,
    plannedData: safeJson(r.planned_data), actualData: safeJson(r.actual_data),
    varianceData: safeJson(r.variance_data), status: r.status,
  };
}
