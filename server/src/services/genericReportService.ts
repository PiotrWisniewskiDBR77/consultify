...
class GenericReportService {
     ...
     */
    async updateReportContent(reportId: string, { ocrText, aiSummary, keyFindings, suggestedTags }: any) {
        const sql = `
            UPDATE generic_assessment_reports
            SET ocr_text = ?,
                ai_summary = ?,
    ...
    async linkToInitiative(reportId: string, initiativeId: string, organizationId: string) {
        try {
            ...
            if (!linkedInitiatives.includes(initiativeId)) {
                ...
                await DbPromise.run(this.db, sql, [JSON.stringify(linkedInitiatives), reportId, organizationId]);
                return { success: true };
            }
