...
describe('MAT-POL-001 approved engine lock', () => {
  it('binds the four approved in-process engines to the installed lock versions and MIT', () => {
    ...
    const lockPath = fs.existsSync(rootLockPath)
      ? rootLockPath
  ...
  it('fails closed for unapproved or version-unbound engines', () => {
    expect(() => requireApprovedExportEngine('native:sharp-svg')).toThrow(
      'MAT_EXPORT_ENGINE_NOT_APPROVED'
    );
    expect(() => requireApprovedExportEngine('external:unknown')).toThrow(
      'MAT_EXPORT_ENGINE_NOT_APPROVED'
    );
  });
});
...
const REAL_PG =
  ...
  process.env.MAT_POL_FIXTURE_OPT_IN === '1' &&
  /^mat_pol_[a-z0-9_]+$/.test(CALLER_PREFIX) &&
...
describe.skipIf(!REAL_PG)('MAT-POL-001 migration17 storage contract (real PG)', () => {
  ...
  afterAll(async () => {
    ...
    try {
      ...
      await lockClient.query(
        `DELETE FROM artifact_export_receipts WHERE export_receipt_id LIKE 'mat-pol-%'`
      );
      await lockClient.query(
        `DELETE FROM presentation_templates WHERE id LIKE 'mat-pol-template-%'`
      );
  ...
  it('keeps historical provenance unknown and rejects a policy receipt outside the approved lock', async () => {
    ...
    try {
      ...
      expect(
        templateDefaults.rows.every((row) => String(row.column_default).includes('unknown'))
      ).toBe(true);
  ...
  it('quarantines unknown provenance and resolves only after an explicit approved record', async () => {
    ...
    try {
      ...
      await expect(
        pool.query(`UPDATE presentation_templates SET provenance_status='approved' WHERE id=$1`, [
          id,
        ])
      ).rejects.toMatchObject({ code: '23514' });
      await pool.query(
      ...
      await expect(
        pool.query(
          `UPDATE artifact_export_receipts SET status='failed', failure_code='X', completed_at=now(),
                render_engine_version='9.5.0' WHERE export_receipt_id=$1`,
          [id]
        )
      ).rejects.toMatchObject({ code: '55000' });
      const row = await pool.query(
        `SELECT status,render_engine_version FROM artifact_export_receipts WHERE export_receipt_id=$1`,
        [id]
      );
  ...
  it('rolls back a forced cleanup failure while the retained advisory guard remains held', async () => {
    ...
    try {
      ...
      await expect(
        client.query(`SELECT definitely_missing_cleanup_column FROM artifact_export_receipts`)
    ...
    const residue = await pool.query(
      `SELECT count(*)::int AS n FROM artifact_export_receipts WHERE export_receipt_id=$1`,
      [id]
    );
