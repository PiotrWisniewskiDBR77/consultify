...
export class TagService {
    ...
    async addTagToContent(
        ...
        if (result.changes > 0) {
            await this.deps.db.run('UPDATE content_tags SET usage_count = usage_count + 1 WHERE id = ?', [tagId]);
            return true;
        }
        return false;
    }
    ...
    async removeTagFromContent(contentId: string, contentType: string, tagId: string): Promise<boolean> {
        ...
        if (result.changes > 0) {
            await this.deps.db.run('UPDATE content_tags SET usage_count = MAX(0, usage_count - 1) WHERE id = ?', [
                tagId,
            ]);
            return true;
        }
