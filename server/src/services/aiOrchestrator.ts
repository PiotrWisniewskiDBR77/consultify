...
export const AIOrchestrator = {
    ...
    processMessage: async (
        ...
        // 1. Build context
        const context = await _AIContextBuilder.buildContext(userId, organizationId, projectId, options);
...
USER MESSAGE: ${userMessage}`;
    ...
    postProcessResponse: async (responseText: string, responseContext: any) => {
        ...
        if (responseContext?.explanation) {
            const footer = _AIExplainabilityService.buildExplainabilityFooter(responseContext.explanation);
            if (footer) {
