...
export function buildDeterministicInitiativeBudgetFromPrompt(
  ...
  const moneyFormat =
    currency === 'EUR' ? '#,##0.00 [$EUR]' : currency === 'USD' ? '$#,##0.00' : '#,##0.00 [$PLN]';
  ...
  const inputRows = [
    {
      cells: {
        parameter: { value: 'Approved funding input' },
        value: { value: totalBudget, style: { numberFormat: moneyFormat } },
      },
    },
    {
      cells: {
        parameter: { value: 'Monthly plan' },
        value: { value: monthlyPlan, style: { numberFormat: moneyFormat } },
      },
    },
  ...
  return {
    title,
    description:
      'Deterministic 12-month initiative budget with plan, actuals, variance, cumulative totals and utilization formulas.',
    author: 'Consultify Teresa',
    metadata: { generationMode: 'deterministic_initiative_budget', currency },
    sheets: [
      {
        name: 'Inputs',
        purpose: 'Auditable source assumptions and actual costs.',
        columns: [
      ...
      {
        name: 'Monthly Budget',
        purpose: 'Formula-driven monthly plan-versus-actual model.',
        columns: [
      ...
      {
        name: 'Summary',
        purpose: 'Formula-driven annual management summary.',
        columns: [
        ...
        rows: [
          {
            cells: {
              metric: { value: 'Total budget' },
              value: { formula: "'Inputs'!B2", style: { numberFormat: moneyFormat } },
            },
            isSummary: true,
          },
          {
            cells: {
              metric: { value: 'Annual plan' },
              value: {
                formula: "SUM('Monthly Budget'!B2:B13)",
                style: { numberFormat: moneyFormat },
              },
            },
            isSummary: true,
          },
          {
            cells: {
              metric: { value: 'Annual actual' },
              value: {
                formula: "SUM('Monthly Budget'!C2:C13)",
                style: { numberFormat: moneyFormat },
              },
            },
            isSummary: true,
          ...
          {
            cells: {
              metric: { value: 'Annual variance' },
              value: { formula: 'B3-B4', style: { numberFormat: moneyFormat } },
            },
            isSummary: true,
          },
          {
            cells: {
              ...
              value: { formula: 'IF(B3=0,0,B4/B3)', style: { numberFormat: '0.0%' } },
            },
            isSummary: true,
          },
        ],
