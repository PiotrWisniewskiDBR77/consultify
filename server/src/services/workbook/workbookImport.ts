...
function scalarValue(value: ExcelJS.CellValue): string | number | boolean | null {
  ...
  if (typeof value === 'object' && 'result' in value)
    return scalarValue(value.result as ExcelJS.CellValue);
  return String(value);
}

export async function importWorkbookBuffer(
  buffer: Buffer,
  filename: string
): Promise<WorkbookSchema> {
      ...
      columns.forEach((column, columnOffset) => {
        ...
        const formula =
          value && typeof value === 'object' && 'formula' in value
            ...
            : undefined;
        const comment = sourceCell.note
          ? typeof sourceCell.note === 'string'
            ? sourceCell.note
            : sourceCell.note.texts?.map((part) => part.text).join('')
          : undefined;
        const formulaResult =
          value && typeof value === 'object' && 'result' in value ? value.result : null;
        cells[column.key] = {
          value: formula ? scalarValue(formulaResult as ExcelJS.CellValue) : scalarValue(value),
          formula,
          style: importedStyle(sourceCell),
          comment,
        };
      });
