...
describe('benefitsRealization premium workbook', () => {
  it('builds a five-layer, formula-driven board model without placeholders', async () => {
    const schema = buildBenefitsRealizationSchema({
      programName: 'Northstar',
      currencyCode: 'EUR',
    ...
    expect(workbook.getWorksheet('Executive Summary')?.getCell('B2').value).toMatchObject({
      formula: "'Assumptions'!B4",
    });
    expect(workbook.getWorksheet('Executive Summary')?.getCell('B5').value).toMatchObject({
      formula: expect.stringContaining("'Assumptions'!B4"),
    });
    expect(workbook.getWorksheet('Benefits Register')?.getCell('C2').value).toMatchObject({
      formula: expect.stringContaining('Assumptions'),
    });
    ...
    expect(workbook.getWorksheet('Assumptions')?.getCell('B4').value).toMatchObject({
      formula: 'SUM(B2:B3)',
    });
    expect(workbook.getWorksheet('Assumptions')?.getCell('D5').value).toBe('CRM + revenue ledger');
    expect(workbook.getWorksheet('Scenario Model')?.getCell('B2').value).toMatchObject({
      formula: "'Assumptions'!B10",
    });
    expect(workbook.getWorksheet('Scenario Model')?.getCell('C3').value).toMatchObject({
      formula: expect.stringContaining('Benefits Register'),
    });
    expect(workbook.getWorksheet('Monthly Tracking')?.getCell('E13').value).toMatchObject({
      formula: 'SUM($C$2:C13)',
    });
    expect(workbook.getWorksheet('Monthly Tracking')?.getCell('D2').font.color?.argb).toBe(
      'FFC2415D'
    );
    expect(
      (
        workbook.getWorksheet('Monthly Tracking') as unknown as
          | { conditionalFormattings: Array<{ ref: string }> }
          | undefined
      )?.conditionalFormattings[0]?.ref
    ).toBe('D2:D13');
  });
