...
export const P07_CAPTURE_ENTRIES = {
  ...
  open: [
    'notebook_list',
    'search_results',
    'backlink_context_panel',
    'stable_deeplink',
  ],
  forbidden: [
    'no_top_level_module_outside_my_work',
    'no_dead_inbox',
  ],
} as const;
...
export const P07_ATTACHMENT_ERROR_TAXONOMY = {
  ...
  quota_exceeded: { retryable: false, userMessage: 'Storage quota exceeded — free space or upgrade' },
  ...
  storage_unavailable: { retryable: true, userMessage: 'Storage temporarily unavailable — retry later' },
  processing_failed: { retryable: true, userMessage: 'Processing failed — retry or contact support' },
...
export const P07_ANTI_DUPLICATE_RULES = {
  no_parallel_attachment_system: 'Notebook uses one canonical attachment lifecycle — no parallel uploader',
  no_search_v2_index: 'Search uses canonical FTS/embeddings — no parallel index per module',
  no_dead_inbox: 'No parallel dead inbox — every capture uses status=inbox with immediate next step',
  no_parallel_link_model: 'No second link representation for note↔artifact — use canonical link_graph_edges',
} as const;
...
export const P07_DEGRADED_SCENARIOS: ReadonlyArray<{
  ...
  { id: 1, scenario: 'Upload failed: network/timeout', userVisibleState: 'failed(network)', nextAction: 'CTA "Retry upload" — no loss of file reference' },
  { id: 2, scenario: 'Upload blocked: policy/size/type', userVisibleState: 'blocked(policy)', nextAction: 'Show reason + "what next" (change file/size/ask admin)' },
  { id: 3, scenario: 'Processing stuck/slow', userVisibleState: 'processing', nextAction: 'Show progress + "wait / cancel / retry later"' },
  { id: 4, scenario: 'Preview/readback unavailable', userVisibleState: 'attachment visible, preview unavailable', nextAction: 'Banner "preview unavailable" + download/open externally' },
  { id: 5, scenario: 'Semantic search degraded', userVisibleState: 'banner "semantic unavailable"', nextAction: 'Fallback to keyword search — no silent "0 results"' },
  { id: 6, scenario: 'Index stale / delayed', userVisibleState: 'banner "results may be delayed"', nextAction: 'Option to refresh' },
  { id: 7, scenario: 'Deeplink target missing', userVisibleState: 'degraded page "note not found / deleted"', nextAction: 'Search by id + activity pointers' },
  { id: 8, scenario: 'Link target permission denied', userVisibleState: 'link visible, marked degraded(permission)', nextAction: 'Instruction "request access / capture context"' },
  { id: 9, scenario: 'Concurrent edit conflict', userVisibleState: 'conflict resolution UI (versions)', nextAction: 'Explicit conflict resolution — no silent overwrite' },
  { id: 10, scenario: 'AI unavailable', userVisibleState: 'AI actions disabled with explanation', nextAction: 'User edit and core notebook continue working' },
] as const;
...
export const P07_ACCEPTANCE_CHECKLIST = [
  ...
  { id: 2, requirement: 'note_id as durable identity + stable deeplink (§2.3.2)', section: '§2.3.2' },
  { id: 3, requirement: 'Provenance language (source/user_edit/ai_transform) with "no silent loss" (§2.3.3)', section: '§2.3.3' },
  { id: 4, requirement: 'Attachment lifecycle with states + error taxonomy + retry (§2.3.4)', section: '§2.3.4' },
  { id: 5, requirement: 'Search baseline with declared filters (§2.3.5)', section: '§2.3.5' },
  { id: 6, requirement: 'Linking + handoff payload (Radar/Inicjatywy/Teresa) (§2.3.6)', section: '§2.3.6' },
  { id: 7, requirement: 'Anti-duplicate gate (attachments/search/inbox/links) (§2.3.7)', section: '§2.3.7' },
  { id: 8, requirement: 'Degraded posture (8+ scenarios) (§2.3.8)', section: '§2.3.8' },
  { id: 9, requirement: 'Non-goals explicit (no Notion DB parity, no new module, no silent AI writing)', section: '§2.2' },
