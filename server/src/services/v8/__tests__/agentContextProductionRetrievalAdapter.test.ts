...
describe('agent production retrieval adapter', () => {
  ...
  it('maps bounded native chunks and actual relevance into policy revalidation', async () => {
    ...
    await retrieveAndRevalidateTransformationContext({
  ...
  it('fails closed on retrieval error and does not write lineage', async () => {
    ...
    expect(revalidateMock).toHaveBeenCalledWith(
      expect.objectContaining({ candidates: [], retrievalFailureReason: 'retrieval unavailable' })
    );
  ...
  it('requires a pinned project before any retrieval', async () => {
    const client = {
      query: vi.fn().mockResolvedValue({ rows: [{ ...owner, project_id: null }] }),
    } as any;
    await retrieveAndRevalidateTransformationContext({
      transformationCaseId: 'case-1',
      organizationId: 'org-1',
    ...
    expect(revalidateMock).toHaveBeenCalledWith(
      expect.objectContaining({ retrievalFailureReason: 'agent_context_project_required' })
    );
  });
});
