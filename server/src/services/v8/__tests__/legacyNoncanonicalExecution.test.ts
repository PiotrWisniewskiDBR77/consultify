...
describe('legacy noncanonical execution isolation', () => {
  ...
  it('classifies a genuinely legacy execution with structured telemetry', async () => {
    ...
    const { assertLegacyNoncanonicalExecution } =
      await import('../../../ai/legacyNoncanonicalExecution.js');
    await expect(
      assertLegacyNoncanonicalExecution({
    ...
    expect(logInfo).toHaveBeenCalledWith(
      '[LegacyExecution] classified legacy_noncanonical',
      expect.objectContaining({ entrypoint: 'playbook_executor' })
    );
  });
