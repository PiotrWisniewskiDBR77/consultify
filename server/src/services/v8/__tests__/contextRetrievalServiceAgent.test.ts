...
vi.mock('../../ragService.js', () => ({
  default: { hybridSearch: hybridMock },
  hybridSearch: hybridMock,
}));
...
describe('ContextRetrievalService Agent hardening', () => {
  ...
  it('requires a project for the Agent workflow before querying', async () => {
    ...
    ).rejects.toThrow('agent_context_project_required');
  ...
  it('enforces project membership in SQL and preserves hybrid relevance', async () => {
    ...
    allMock.mockResolvedValueOnce([
      { id: 'doc-1', ai_visibility: 'allowed', sensitivity: 'internal' },
    ]);
    hybridMock.mockResolvedValue([
      {
        ...
        chunkIndex: 1,
        hybridScore: 0.87,
        metadata: { nativeSourceLocator: { page: 2 } },
      },
    ]);
  ...
  it('returns no candidate for an inaccessible project document', async () => {
    ...
    const result = await retrieveContext({
      ...
      workflow: 'agent_execution',
      workflowMode: 'selected_material_plus_selected_context',
      selectedDocumentIds: ['foreign-doc'],
      retrievalQuery: 'secret',
    });
