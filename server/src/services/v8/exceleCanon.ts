...
export const P23_AI_PROPOSAL_RULES = {
  ...
  propose: 'AI/agent generates proposal (schema/formulas/transforms) as explicit plan/diff without modifying the artifact',
  ...
  hardInvariant: 'approve(run) ≠ review(artifact) — run approval is about execution (apply); review is about quality/publish/governance',
...
export const P23_ERROR_TAXONOMY = {
  ...
  schema_mismatch: {
    retryable: false,
    userMessage: 'Column structure does not match the expected schema. Review the mapping and approve.',
    recovery: ['accept_mapping', 'fix_input', 'reject'],
  },
  type_coercion_error: {
    retryable: false,
    userMessage: 'Some values could not be converted to the expected type (e.g. text in a number column).',
    recovery: ['apply_with_coercion', 'fix_input', 'reject'],
  },
  validation_failed: {
    retryable: false,
    userMessage: 'Semantic validation failed (e.g. totals do not add up). Review the checks before applying.',
    recovery: ['override', 'fix_input', 'reject'],
  },
  transform_failed: {
    retryable: true,
    userMessage: 'A data cleaning/transformation step failed. Rolling back to pre-transform snapshot.',
    recovery: ['retry', 'skip_step', 'rollback'],
  },
  formula_error: {
    retryable: false,
    userMessage: 'Formula error detected (syntax, references, or #DIV/0!). Affected cells are highlighted.',
    recovery: ['fix_formula', 'remove_formula', 'apply_partial'],
  },
  formula_cycle_detected: {
    retryable: false,
    userMessage: 'Circular reference detected in formulas. Please resolve the cycle before applying.',
...
export const P23_WORKBOOK_CAPABILITIES = {
  ...
  formulaSupport: [
    ...
    'cross_sheet_references', 'percentage_of_total', 'growth_calculations',
  ],
  formattingSupport: [
    'bold', 'italic', 'font_size', 'font_color', 'background_color',
    'number_format', 'alignment', 'wrap_text', 'borders',
    'freeze_panes', 'merged_cells', 'alternating_rows',
    'tab_colors', 'cell_comments', 'summary_rows',
  ],
...
export const P23_ANTI_DUPLICATE_RULES = {
  ...
  noSecondLibrary: 'Sheet artifacts land in Outputs Library (P19) — no parallel "sheet registry v2"',
  noSecondStageEnums: 'Trust-state inherited from P18 — Excele does not invent stage enums',
  singleIdentity: 'Each Sheet has one identity and one reopen path consistent with artifact canon (P18/P19)',
  workbooksMustRegister: 'Workbooks generated by P23-D MUST register in artifact registry to be visible in Outputs Library',
} as const;
...
export const P23_ACCEPTANCE_CHECKLIST = [
  { id: 1, requirement: 'Lane definition: Excele = deliverable sheet lane, not BI/ETL/tables OS', section: '§2.3' },
  { id: 2, requirement: 'No silent apply: propose→review→apply is explicit; apply is a run requiring approve(run)', section: '§2.3' },
  { id: 3, requirement: 'approve(run) ≠ review(artifact): separation cited and linked to P17 + P18', section: '§2.3' },
  { id: 4, requirement: 'Bounded import posture: explicit limits on formats/sizes (no overclaim)', section: '§2.3' },
  { id: 5, requirement: 'Validation + error taxonomy: 10 error classes with recovery paths', section: '§7.1' },
  { id: 6, requirement: 'MISSING INPUT flags: evidence mapping has explicit MISSING INPUT where evidence lacks', section: '§6' },
  { id: 7, requirement: 'Governance: home = Outputs Library (P19); trust-state = P18; no parallel truths', section: '§7' },
  { id: 8, requirement: 'Anti-duplicate: no parallel tables OS (P15); no second library/registry', section: '§7' },
  { id: 9, requirement: 'Degraded posture: 10+ scenarios with recovery and no ghost artifacts', section: '§7.1' },
  { id: 10, requirement: 'Index readiness: EXECUTION_INDEX #23 set to verified(evidence)', section: '§10' },
  { id: 11, requirement: 'Evidence ledger filled: §10 with commit refs, tests, staging proof, known limits', section: '§10' },
] as const;
