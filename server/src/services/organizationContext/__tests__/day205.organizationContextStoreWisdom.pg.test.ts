...
describe('Day205 R1 organization store wisdom through real Gateway and PostgreSQL', NO_RETRY, () => {
  ...
  afterAll(async () => {
    ...
    await sql.query(`DELETE FROM organization_context_store WHERE organization_id=$1`, [organizationId]);
  ...
  it('writes three object claims and exposes their content in resolved manual context', async () => {
    ...
    expect(response.body).toMatchObject({ ok: true, companyProfileOwnership: 'organization_profiles' });
  ...
  it('keeps the CLOSED_FINAL response contract when the parallel claim writer fails', async () => {
    ...
    expect(response.body).toMatchObject({ ok: true, companyProfileOwnership: 'organization_profiles' });
    expect(response.body.version).toEqual(expect.any(String));
    const stored = await sql.query(`SELECT goals_json FROM organization_context_store WHERE organization_id=$1`, [organizationId]);
