...
describe.skipIf(!REAL_DB)('ORG-BVP-001 — pinned Chat/Idea consumers (mounted realPG)', () => {
  ...
  const foreignPayload: GovernedSnapshotPayload = {
    organizationId: orgB,
    schemaVersion: 1,
    claims: [],
  };
  ...
  const foreignRef = {
    ...
    version: 1,
    contentHash: computeContentHash(foreignPayload),
  };
  ...
  async function mountApp(): Promise<Express> {
    const [{ default: v8Router }, { default: ideaRouter }, { default: orgRouter }] =
      await Promise.all([
  ...
  beforeAll(async () => {
    ...
    for (const [organizationId, name] of [
      [orgA, 'Pinned consumer A'],
      [orgB, 'Pinned consumer B'],
    ]) {
      await pool.query(
    ...
    await pool.query(
      `INSERT INTO conversations (id,user_id,organization_id,title) VALUES ($1,$2,$3,'Pinned Chat')`,
      [conversationId, ownerA, orgA]
    );
    await pool.query(
      `INSERT INTO conversation_messages (id,conversation_id,role,content,metadata) VALUES ($1,$2,'ai','Pinned governed context response https://example.test/source','{}'::jsonb)`,
      [messageId, conversationId]
    );
    await pool.query(
      `INSERT INTO my_ideas (id,user_id,organization_id,title,body,tags) VALUES ($1,$2,$3,'Pinned Idea','Pinned body','[]')`,
      [ideaId, ownerA, orgA]
    );
  ...
  afterAll(async () => {
    if (!pool) return;
    await pool.query(
      `ALTER TABLE organization_context_consumer_bindings DISABLE TRIGGER trg_org_context_consumer_binding_immutable`
    );
    await pool.query(
      `DELETE FROM organization_context_consumer_bindings WHERE organization_id = ANY($1)`,
      [[orgA, orgB]]
    );
    await pool.query(
      `ALTER TABLE organization_context_consumer_bindings ENABLE TRIGGER trg_org_context_consumer_binding_immutable`
    );
  ...
  it('governed Chat fails closed on missing/foreign/hash drift, then persists exact ref and replays once', async () => {
    ...
    const foreign = await request(app)
      .post(path)
      .set(bearer(tokenA))
      .send({ ...base, governedSnapshotRef: foreignRef });
    expect(foreign.status).toBe(404);
    const badHash = await request(app)
      .post(path)
      .set(bearer(tokenA))
      .send({ ...base, governedSnapshotRef: { ...ref1, contentHash: '0'.repeat(64) } });
    ...
    const results = await Promise.all(
      Array.from({ length: 8 }, () =>
        request(app)
          .post(path)
          .set(bearer(tokenA))
          .send({ ...base, governedSnapshotRef: ref1 })
      )
    );
  ...
  it('governed Idea requires the same exact ref and persists an immutable audit receipt', async () => {
    ...
    const first = await request(app)
      .post(path)
      .set(bearer(tokenA))
      .send({ ...base, governedSnapshotRef: ref2 });
    ...
    await expect(
      pool.query(
        `UPDATE organization_context_consumer_bindings SET snapshot_version=99 WHERE binding_id=$1`,
        [ideaBindingId]
      )
    ).rejects.toMatchObject({ message: expect.stringContaining('immutable') });
    await expect(
      pool.query(`DELETE FROM organization_context_consumer_bindings WHERE binding_id=$1`, [
        ideaBindingId,
      ])
    ).rejects.toMatchObject({ message: expect.stringContaining('immutable') });
    expect(
  ...
  it('cold process/readback proves exact refs and restart replay never resolves latest/live context again', async () => {
    ...
    const rows = await cold.query(
      `SELECT b.consumer_kind,b.consumer_record_id,b.snapshot_id,b.snapshot_version,
        ...
        WHERE b.binding_id = ANY($1) ORDER BY b.consumer_kind`,
      [[chatBindingId, ideaBindingId]]
    );
    await cold.end();
    ...
    const chatReplay = await request(app)
      ...
      .send({
        messageId,
        targetKind: 'document',
        idempotencyKey: id('chat_idem'),
        governedSnapshotRef: ref1,
      });
    ...
    const counts = await pool.query(
      `SELECT count(*)::int AS n FROM organization_context_consumer_bindings WHERE organization_id=$1`,
      [orgA]
    );
