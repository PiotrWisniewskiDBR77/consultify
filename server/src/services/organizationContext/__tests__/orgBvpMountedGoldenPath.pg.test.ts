...
describe.skipIf(!REAL_DB)(
  'ORG-BVP-001 — mounted organization golden path (real PostgreSQL)',
  () => {
        ...
        await pool.query(
         ...
         VALUES ($1,$2,$3,$4,$5)`,
          [
            ...
            userId,
            role,
            userId === staleId ? 'INACTIVE' : 'ACTIVE',
          ]
        );
      }
      await pool.query(
    ...
    afterAll(async () => {
      ...
      await pool.query(`DELETE FROM users WHERE id = ANY($1)`, [
        [ownerId, memberId, staleId, foreignOwnerId],
      ]);
    ...
    it('reopens the exact tenant snapshot through HTTP and a fresh PostgreSQL connection', async () => {
      ...
      expect(
        JSON.parse(cold.rows[0].snapshot_json).claims.map((claim: any) => claim.claimId)
      ).toContain(claimId);
