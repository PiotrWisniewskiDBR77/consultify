...
export function adaptDynamicSwotActions(): FederatedActionEntry[] {
  return Object.values(TERESA_CAPABILITY_REGISTRY).map(({ definition, handler }) => ({
    actionId: `dynamic-swot.${definition.id}@1`,
    version: 1,
    module: 'DYNAMIC_SWOT',
    surface: 'dynamic-swot-teresa-kernel',
    mountedMutationId:
      handler && definition.producesProposal ? `dynamic-swot:${definition.id}` : null,
    effect: definition.producesProposal ? 'PROPOSAL' : 'READ',
...
export function adaptExecutionActions(
  policies: readonly ExecutionActionPolicy[]
): FederatedActionEntry[] {
  return policies.map((policy) => ({
    actionId: `${policy.actionId}@1`,
    version: 1,
    module: policy.targetType.startsWith('case') ? 'CASE_WORKSPACE' : 'EXECUTION',
    surface: 'execution-action-registry',
    mountedMutationId: policy.actionId,
    effect: policy.destructive ? 'DESTRUCTIVE_MUTATION' : 'REVERSIBLE_MUTATION',
    roles: [policy.minimumRole],
    tenantScope: 'ORGANIZATION',
    preview: 'REQUIRED',
    confirm: 'REQUIRED',
    idempotency: 'requestId',
    receipt: 'execution_action_audit',
    auditEvent: 'execution_action_audit',
    compensation: policy.destructive
      ? 'explicit-irreversible-boundary'
      : 'domain-compensating-command',
