...
const SECTION_TEMPLATES: Record<string, Record<string, { title: string; prompt: string }>> = {
  ...
  insight_detail: {
    en: {
      ...
      prompt: 'Expand on each approved insight with evidence links.',
    },
    pl: {
      title: 'Szczegółowe wnioski',
      prompt: 'Rozwiń każdy zatwierdzony wniosek z linkami do dowodów.',
    },
  },
  assumptions: {
    en: {
      title: 'Assumptions & Unknowns',
      prompt: 'Document assumptions, unknowns, and counterpoints.',
    },
    pl: {
      title: 'Założenia i niewiadome',
      prompt: 'Udokumentuj założenia, niewiadome i kontrargumenty.',
    },
  },
  recommendations: {
    en: {
      title: 'Recommendations',
      prompt: 'Provide actionable recommendations based on findings.',
    },
  ...
  next_steps: {
    ...
    pl: {
      ...
      prompt: 'Przedstaw proponowane następne kroki z harmonogramem.',
    },
  },
};
...
export async function generateSponsorReport(params: {
  ...
  if (assessmentId) {
    assessmentData = await dbGet(`SELECT * FROM assessments WHERE id = ? AND organization_id = ?`, [
      assessmentId,
      organizationId,
    ]);
  }
...
function buildSections(
  ...
  const insightSummary = insights.map((i: any) => {
    const sc =
      typeof i.structured_content === 'string'
        ? JSON.parse(i.structured_content || '{}')
  ...
  const execTemplate =
    SECTION_TEMPLATES.executive_summary[lang] || SECTION_TEMPLATES.executive_summary.en;
  const execContent =
    insightSummary.length > 0
      ? insightSummary
          .slice(0, 5)
          .map((i: any) => `• ${i.title}: ${i.finding}`)
          .join('\n')
      : lang === 'pl'
  ...
  sections.push({
    ...
    content: execContent,
    order: order++,
    evidenceSources: insights.map((i: any) => i.id),
    sectionType: 'executive_summary',
  });

  if (insightSummary.length > 0) {
    const findingsTemplate =
      SECTION_TEMPLATES.key_findings[lang] || SECTION_TEMPLATES.key_findings.en;
    const findingsContent = insightSummary
      .sort((a: any, b: any) => (b.confidence || 0) - (a.confidence || 0))
      .slice(0, 8)
      .map(
        (i: any, idx: number) =>
          `${idx + 1}. **${i.title}** (${lang === 'pl' ? 'pewność' : 'confidence'}: ${i.confidence}%)\n   ${i.finding}`
      )
      .join('\n\n');

    sections.push({
      id: uuidv4(),
      title: findingsTemplate.title,
  ...
  if (insightSummary.some((i: any) => i.recommendation)) {
    const recsTemplate =
      SECTION_TEMPLATES.recommendations[lang] || SECTION_TEMPLATES.recommendations.en;
  ...
  const assumptionsTemplate =
    SECTION_TEMPLATES.assumptions[lang] || SECTION_TEMPLATES.assumptions.en;
  sections.push({
    id: uuidv4(),
    title: assumptionsTemplate.title,
    content:
      lang === 'pl'
  ...
  for (const i of insights) {
    const sc =
      typeof i.structured_content === 'string'
        ...
        : i.structured_content || {};
    const assumptions =
      typeof i.assumptions === 'string'
        ? JSON.parse(i.assumptions || '[]')
        : i.assumptions || sc.assumptions || [];
    if (Array.isArray(assumptions)) result.push(...assumptions.map(String));
  }
  if (result.length === 0) {
    result.push(
      lang === 'pl' ? 'Wnioski oparte na dostępnych danych' : 'Findings based on available data'
    );
  }
  ...
  for (const i of insights) {
    const sc =
      typeof i.structured_content === 'string'
        ...
        : i.structured_content || {};
    const unknowns =
      typeof i.unknowns === 'string'
        ? JSON.parse(i.unknowns || '[]')
        : i.unknowns || sc.unknowns || [];
  ...
  for (const i of insights) {
    const sc =
      typeof i.structured_content === 'string'
        ...
        : i.structured_content || {};
    const cp =
      typeof i.counterpoints === 'string'
        ? JSON.parse(i.counterpoints || '[]')
        : i.counterpoints || sc.counterpoints || [];
...
export async function getReportForExport(
  reportId: string,
  organizationId: string
): Promise<SponsorReportData | null> {
