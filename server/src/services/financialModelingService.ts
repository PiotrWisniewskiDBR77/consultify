...
const PL_LINES = ['REVENUE', 'COGS', 'GROSS_PROFIT', 'OPEX', 'EBITDA', 'DEPRECIATION', 'EBIT', 'INTEREST_EXPENSE', 'EBT', 'TAX', 'NET_INCOME'] as const;
const BS_LINES = ['CASH', 'AR', 'INVENTORY', 'CURRENT_ASSETS', 'PPE_GROSS', 'ACCUM_DEPRECIATION', 'PPE_NET', 'TOTAL_ASSETS', 'AP', 'CURRENT_LIABILITIES', 'LONG_TERM_DEBT', 'TOTAL_LIABILITIES', 'EQUITY_CAPITAL', 'RETAINED_EARNINGS', 'TOTAL_EQUITY', 'TOTAL_LIABILITIES_EQUITY'] as const;
const CF_LINES = ['NET_INCOME_CF', 'DEPRECIATION_ADDBACK', 'WC_CHANGES', 'OPERATING_CF', 'CAPEX_CF', 'INVESTING_CF', 'DEBT_DRAWDOWN_CF', 'DEBT_REPAYMENT_CF', 'EQUITY_CF', 'DIVIDEND_CF', 'FINANCING_CF', 'NET_CHANGE_CASH', 'OPENING_CASH', 'CLOSING_CASH'] as const;

const LINE_NAMES: Record<string, string> = {
  REVENUE: 'Revenue', COGS: 'COGS', GROSS_PROFIT: 'Gross Profit', OPEX: 'Operating Expenses',
  EBITDA: 'EBITDA', DEPRECIATION: 'Depreciation & Amortization', EBIT: 'EBIT',
  INTEREST_EXPENSE: 'Interest Expense', EBT: 'EBT (Earnings Before Tax)', TAX: 'Income Tax', NET_INCOME: 'Net Income',
  CASH: 'Cash', AR: 'Accounts Receivable', INVENTORY: 'Inventory', CURRENT_ASSETS: 'Current Assets',
  PPE_GROSS: 'PPE (Gross)', ACCUM_DEPRECIATION: 'Accumulated Depreciation', PPE_NET: 'PPE (Net)',
  TOTAL_ASSETS: 'Total Assets', AP: 'Accounts Payable', CURRENT_LIABILITIES: 'Current Liabilities',
  LONG_TERM_DEBT: 'Long-term Debt', TOTAL_LIABILITIES: 'Total Liabilities',
  EQUITY_CAPITAL: 'Equity Capital', RETAINED_EARNINGS: 'Retained Earnings', TOTAL_EQUITY: 'Total Equity',
  TOTAL_LIABILITIES_EQUITY: 'Total Liabilities + Equity',
  NET_INCOME_CF: 'Net Income', DEPRECIATION_ADDBACK: 'Depreciation Add-back', WC_CHANGES: 'Working Capital Changes',
  OPERATING_CF: 'Operating Cash Flow', CAPEX_CF: 'Capital Expenditures', INVESTING_CF: 'Investing Cash Flow',
  DEBT_DRAWDOWN_CF: 'Debt Drawdown', DEBT_REPAYMENT_CF: 'Debt Repayment', EQUITY_CF: 'Equity Injection',
  DIVIDEND_CF: 'Dividends', FINANCING_CF: 'Financing Cash Flow',
  NET_CHANGE_CASH: 'Net Change in Cash', OPENING_CASH: 'Opening Cash', CLOSING_CASH: 'Closing Cash',
};
...
function generatePeriods(startDate: string, horizonMonths: number, granularity: string): Array<{ date: string; label: string }> {
...
function expandEventToAmounts(event: ModelEvent, periods: Array<{ date: string }>): Map<string, number> {
  ...
  for (const period of periods) {
    ...
    const growthFactor = event.growth_rate !== 0
      ? Math.pow(1 + event.growth_rate / 100, monthsElapsed / 12)
      : 1;

    let periodAmount = event.amount * growthFactor;
...
export async function computeModel(modelId: string): Promise<ComputeResult> {
  ...
  const events = ((await dbAll(
    ...
    ...e,
    posting_rules: typeof e.posting_rules === 'string' ? JSON.parse(e.posting_rules) : e.posting_rules || {},
  ...
  const assumptions = typeof model.assumptions_json === 'string' ? JSON.parse(model.assumptions_json) : model.assumptions_json || {};

  // Initialize period outputs
  const outputs: PeriodOutput[] = periods.map(p => ({
    date: p.date, label: p.label,
  ...
  for (let pi = 0; pi < outputs.length; pi++) {
    ...
    let totalRevenue = 0, totalCOGS = 0, totalOPEX = 0, totalDepr = 0, totalInterest = 0, totalTax = 0;
    let totalCapex = 0, totalDebtDrawdown = 0, totalDebtRepayment = 0;
    let totalWCChange = 0, totalEquityInjection = 0, totalDividend = 0;
    ...
    for (const event of events) {
      ...
      switch (event.event_type) {
        case 'revenue': totalRevenue += amt; break;
        case 'cogs': totalCOGS += Math.abs(amt); break;
        case 'opex': totalOPEX += Math.abs(amt); break;
        case 'depreciation_run': totalDepr += Math.abs(amt); break;
        case 'interest_accrual': totalInterest += Math.abs(amt); break;
        case 'tax_accrual': case 'tax_payment': totalTax += Math.abs(amt); break;
        case 'capex_purchase': totalCapex += Math.abs(amt); break;
        case 'debt_drawdown': totalDebtDrawdown += amt; break;
        case 'debt_repayment': totalDebtRepayment += Math.abs(amt); break;
        case 'wc_change': totalWCChange += amt; break;
        case 'equity_injection': totalEquityInjection += amt; break;
        case 'dividend': totalDividend += Math.abs(amt); break;
      }
    }
    ...
    out.cf.FINANCING_CF = totalDebtDrawdown - totalDebtRepayment + totalEquityInjection - totalDividend;
  ...
  for (const out of outputs) {
    ...
    validations.push({
      ...
      message: bsDiff < 0.01 ? 'Assets = Liabilities + Equity ✓' : `BS mismatch: diff = ${round2(bsDiff)}`,
      periodDate: out.date,
    });
    ...
    validations.push({
      ...
      message: cfDiff < 0.01 ? 'ΔCash = OCF + ICF + FCF ✓' : `Cash tie-out mismatch: diff = ${round2(cfDiff)}`,
      periodDate: out.date,
    });
    ...
    validations.push({
      ...
      message: cashBsCfDiff < 0.01 ? 'BS Cash = CF Closing Cash ✓' : `Cash discrepancy: ${round2(cashBsCfDiff)}`,
      periodDate: out.date,
    });
  }
...
export async function persistComputeResult(modelId: string, result: ComputeResult, scenario: string = 'base'): Promise<void> {
  // Clear previous outputs and validations
  await dbRun(`DELETE FROM financial_model_outputs WHERE model_id = ? AND scenario = ?`, [modelId, scenario]);
    ...
    for (const [type, lines] of [['P&L', period.pl], ['BS', period.bs], ['CF', period.cf]] as const) {
      for (const [code, value] of Object.entries(lines)) {
        await dbRun(
          ...
          [uuidv4(), modelId, period.date, period.label, type, code, LINE_NAMES[code] || code, round2(value as number), scenario]
        );
      }
    }
  }
  ...
  for (const v of result.validations) {
    await dbRun(
      ...
      [uuidv4(), modelId, v.periodDate || null, v.checkCode, v.checkName, v.status, v.expected, v.actual, v.difference, v.message]
    );
  }
  ...
  if (modelStatus) {
    await dbRun(`UPDATE financial_models SET updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [modelId]);
  }
}
...
export async function createModel(params: {
  organizationId: string; projectId?: string; initiativeId?: string;
  name: string; description?: string; currency?: string;
  horizonMonths?: number; startDate: string; granularity?: string;
  scenario?: string; assumptions?: Record<string, any>; createdBy: string;
}): Promise<string> {
  const id = uuidv4();
  await dbRun(
    ...
    [id, params.organizationId, params.projectId || null, params.initiativeId || null,
     params.name, params.description || null, params.currency || 'PLN',
     params.horizonMonths || 60, params.startDate, params.granularity || 'monthly',
     params.scenario || 'base', JSON.stringify(params.assumptions || {}), params.createdBy]
  );
  return id;
}
...
export async function getModel(modelId: string): Promise<any> {
  ...
  model.assumptions_json = typeof model.assumptions_json === 'string' ? JSON.parse(model.assumptions_json) : model.assumptions_json;
  return model;
}
...
export async function updateModel(modelId: string, updates: Record<string, any>): Promise<void> {
  const allowedFields = ['name', 'description', 'currency', 'horizon_months', 'start_date', 'granularity', 'scenario', 'status'];
  const sets: string[] = [];
  const vals: any[] = [];
...
export async function approveModel(modelId: string, userId: string): Promise<{ success: boolean; error?: string }> {
  ...
  const snapshot = JSON.stringify({ periods: result.periods, validations: result.validations, computedAt: new Date().toISOString() });
  await dbRun(
...
export async function addEvent(params: {
  modelId: string; eventType: EventType; name: string; description?: string;
  amount: number; periodStart: string; periodEnd?: string;
  recurrence?: string; growthRate?: number;
  cfClassification: CfClassification; postingRules?: Record<string, any>;
  parameters?: Record<string, any>; sortOrder?: number; createdBy?: string;
}): Promise<string> {
  const id = uuidv4();
  await dbRun(
    ...
    [id, params.modelId, params.eventType, params.name, params.description || null,
     params.amount, params.periodStart, params.periodEnd || null,
     params.recurrence || 'one_time', params.growthRate || 0,
     params.cfClassification, JSON.stringify(params.postingRules || {}),
     JSON.stringify(params.parameters || {}), params.sortOrder || 0, params.createdBy || null]
  );
  // Reset model to draft on event change
  await dbRun(`UPDATE financial_models SET status = 'draft', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'approved'`, [params.modelId]);
  return id;
}

export async function updateEvent(eventId: string, updates: Record<string, any>): Promise<void> {
  const event = await dbGet(`SELECT model_id FROM financial_model_events WHERE id = ?`, [eventId]) as any;
  if (!event) return;

  const allowedFields = ['name', 'description', 'amount', 'period_start', 'period_end', 'recurrence', 'growth_rate', 'cf_classification', 'sort_order', 'is_active', 'event_type'];
  const sets: string[] = [];
  const vals: any[] = [];
  for (const [k, v] of Object.entries(updates)) {
    if (allowedFields.includes(k)) { sets.push(`${k} = ?`); vals.push(v); }
  }
  if (updates.posting_rules) { sets.push('posting_rules = ?'); vals.push(JSON.stringify(updates.posting_rules)); }
  if (updates.parameters) { sets.push('parameters = ?'); vals.push(JSON.stringify(updates.parameters)); }
  if (sets.length === 0) return;
  sets.push('updated_at = CURRENT_TIMESTAMP');
  vals.push(eventId);
  await dbRun(`UPDATE financial_model_events SET ${sets.join(', ')} WHERE id = ?`, vals);
  await dbRun(`UPDATE financial_models SET status = 'draft', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'approved'`, [event.model_id]);
}
...
export async function listEvents(modelId: string): Promise<any[]> {
  const rows = ((await dbAll(`SELECT * FROM financial_model_events WHERE model_id = ? ORDER BY sort_order, created_at`, [modelId])) || []) as any[];
  return rows.map(r => ({
    ...r,
    posting_rules: typeof r.posting_rules === 'string' ? JSON.parse(r.posting_rules) : r.posting_rules,
...
export async function getValidations(modelId: string): Promise<any[]> {
  return ((await dbAll(`SELECT * FROM financial_model_validations WHERE model_id = ? ORDER BY period_date, check_code`, [modelId])) || []) as any[];
}
