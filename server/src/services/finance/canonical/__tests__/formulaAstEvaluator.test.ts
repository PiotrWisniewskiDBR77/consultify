...
describe('evaluateFormula — CURRENT_RATIO shape (ratio, POINT_IN_TIME, SHOW_WITH_FLAG)', () => {
  ...
  it('DIVISION_BY_ZERO: denominator exactly 0 -> NOT_APPLICABLE, never a fabricated number', async () => {
    const result = await evaluateFormula(
      ast,
      {
    ...
    expect(result.qualityFlag).toBe('DIVISION_BY_ZERO');
  });
  ...
  it('a MISSING numerator/denominator never fabricates a value (propagates MISSING)', async () => {
    const result = await evaluateFormula(
      ast,
      {
...
describe('evaluateFormula — DSO shape (root op is divide, not ratio, but policy still applies at the root)', () => {
  ...
  it('a nested divide-by-zero (REVENUE/DAYS_IN_PERIOD side) never leaks past this evaluator as a number', async () => {
    ...
    // REVENUE/0 (DAYS_IN_PERIOD=0, a degenerate fixture) -> inner divide is DIVISION_BY_ZERO
    // (NOT_APPLICABLE) -> outer divide's right operand is NOT_APPLICABLE -> propagates, never a number.
    expect(result.status).toBe('NOT_APPLICABLE');
    expect(result.value).toBeNull();
  });
