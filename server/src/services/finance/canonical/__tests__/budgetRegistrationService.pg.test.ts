...
describe.skipIf(!REAL_PG)('budgetRegistrationService (real PostgreSQL)', () => {
    ...
    (
      await client.query(
        `SELECT
          ...
          (SELECT count(*)::int FROM finance_budget_initiative_link_receipts
             WHERE organization_id=$1) link_receipts,
  ...
  it('links a same-tenant zero-valued initiative with exact CAS and replay', async () => {
    const budget = await registerBudget(command(`link-${randomUUID()}`));
  ...
  it('migration fails closed before DDL on an occupied partial receipt identity', async () => {
    const migration = fs.readFileSync(
      path.resolve(
        process.cwd(),
        'server/migrations/20261056_finance_budget_document_import_command.sql'
      ),
      'utf8'
    );
    await client.query('BEGIN');
  ...
  it('DatabaseInitializer boot fails hostile W39 and never blesses its ledger row', async () => {
    ...
    const target = '20261056_finance_budget_document_import_command.sql';
    ...
    try {
      ...
      try {
        ...
        expect(boot.message).toMatch(
          /Table Platform migration 20261056_finance_budget_document_import_command\.sql failed: ECO-W39 owned migration identity already exists/
        );
      } finally {
