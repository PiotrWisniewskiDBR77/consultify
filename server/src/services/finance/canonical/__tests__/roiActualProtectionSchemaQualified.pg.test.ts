...
describe.skipIf(!REAL_PG)(
  'ROI-E007 CO-4 — ROI Actual protection on every physical instance',
  () => {
    let raw: pg.Client;
    ...
    for (const schema of ['public', 'v8'] as const) {
      describe(`${schema}.v8_roi_realization_entries`, () => {
        ...
        it('fixture row is inserted AND confirmed present (a trigger needs a row to fire on)', async () => {
          ...
          expect(
            kpiInsert.rowCount,
            `FK prerequisite ${schema}.v8_kpi_definitions did not insert`
          ).toBe(1);
          ...
          expect(
            entryInsert.rowCount,
            `fixture INSERT into ${schema}.v8_roi_realization_entries failed`
          ).toBe(1);
          ...
          expect(
            rows,
            `no fixture row in ${schema}.v8_roi_realization_entries — every trigger probe below would be vacuous`
          ).toHaveLength(1);
        ...
        it('DELETE is physically rejected, and the row survives', async () => {
          ...
          await expect(
            raw.query(`DELETE FROM ${schema}.v8_roi_realization_entries WHERE entry_id = $1`, [
              entryId,
            ])
          ).rejects.toThrow(APPEND_ONLY);
    ...
    describe('public.roi_realized_values', () => {
      ...
      it('fixture row is inserted AND confirmed present', async () => {
        ...
        expect(
          rows,
          'no fixture row in public.roi_realized_values — every trigger probe below would be vacuous'
        ).toHaveLength(1);
      ...
      it('UPDATE of realized_savings is physically rejected, and the stored value does not move', async () => {
        ...
        expect(after).toHaveLength(1);
        expect(
          after[0].realized_savings,
          'public.roi_realized_values was SILENTLY OVERWRITTEN'
        ).toBe(ORIGINAL);
      });
