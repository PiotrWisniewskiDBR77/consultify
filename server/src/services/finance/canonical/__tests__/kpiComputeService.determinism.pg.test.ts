...
describe.skipIf(!REAL_PG)(
  'W3-determinism kpiComputeService — content_semantic_hash is stable across retried recomputes of unchanged content',
  () => {
    ...
    it('10 retried recomputes of the SAME (unchanged) Analysis all produce the SAME content_semantic_hash', async () => {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        if (!result.ok)
          throw new Error(
            `computeAnalysisKpis failed on run ${i}: ${result.code} — ${result.message}`
          );
      ...
      expect(
        distinctHashes.size,
        `expected ALL 10 retried recomputes of unchanged content to share ONE content_semantic_hash, got ${distinctHashes.size} distinct values: ${[...distinctHashes].join(', ')}`
      ).toBe(1);
    }, 120_000);
  }
);
