...
describe.skipIf(!REAL_PG)(
  'PKG-A determinism — finance_prediction_impact_chain / finance_prediction_financing row order churn',
  () => {
    ...
    beforeAll(async () => {
      ...
      const revenueLine = await t((tx) =>
        tx.queryOne<{ id: string }>(
          `SELECT id FROM financial_statement_lines WHERE line_code = 'REVENUE'`
        )
      );
          ...
          tx.queryRun(
           ...
           VALUES (?, ?, ?, 'FACILITY_DRAWDOWN', ?, ?, ?, ?, ?)`,
            [
              ...
              entityId,
              periodId,
              JSON.stringify({ amount: 1000 + i, rate: 0.01 * (i + 1) }),
              userId,
      ...
      for (let i = 1; i <= 10; i++) {
        if (i > 1) {
          ...
          for (const id of impactChainIds) {
            await t((tx) =>
              tx.queryRun(
                `UPDATE finance_prediction_impact_chain SET assumption_label = assumption_label WHERE id = ?`,
                [id]
              )
            );
          }
      ...
      for (let i = 1; i <= 10; i++) {
        if (i > 1) {
          for (const id of financingIds) {
            await t((tx) =>
              tx.queryRun(
                `UPDATE finance_prediction_financing SET updated_at = now() WHERE id = ?`,
                [id]
              )
            );
          }
    ...
    it('NEGATIVE CONTROL: the raw (un-sorted) SQL-order array is NOT guaranteed to pick the same "first FACILITY_DRAWDOWN" — Array.prototype.find on raw order is what the pre-fix code did', async () => {
      ...
      console.warn(
        `[PKG-A churn] raw-scan-order first FACILITY_DRAWDOWN id=${rawFirstDrawdownId}; canonical (created_at-earliest) id=${canonicalFirstDrawdownId} — ${
          rawFirstDrawdownId === canonicalFirstDrawdownId
            ? 'happened to match on this run'
            : 'DIVERGED on this run, exactly the pre-fix risk'
        }`
      );
