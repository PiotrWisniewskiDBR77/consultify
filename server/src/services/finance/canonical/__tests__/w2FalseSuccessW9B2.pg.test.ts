...
describe.skipIf(!REAL_PG)(
  'W2 — false success (W9-B-2): completeJobSuccess() NOT_RUNNING must not report success (real PostgreSQL)',
  () => {
        ...
        tx.queryRun(
         ...
         VALUES (?, ?, ?, 12, 'DEBT_MATURITY', ?, 50, 1, false, true, ?)`,
          [
            ...
            orgId,
            bvId,
            'W2 false-success fixture — GoldCo FY2026 monthly horizon',
            userId,
          ]
        )
      );
     ...
     */
    function interceptFirstCompleteJobSuccessWithRealCancel(reason: string): {
      ...
      const spy = vi
        ...
        .mockImplementationOnce(async (params) => {
          ...
          if (!cancelled)
            throw new Error(
              `fixture: cancelJob() did not affect job ${params.jobId} (already terminal?)`
            );
          spy.mockRestore();
    ...
    beforeAll(async () => {
      ...
      await t((tx) =>
        tx.queryRun(`INSERT INTO organizations (id, name) VALUES (?, ?)`, [
          orgId,
          'W2 False Success Org (GoldCo-scale)',
        ])
      );
    ...
    describe('baselineComputeService.runBaselineCompute()', () => {
      it('job cancelled between claim() and commit -> JOB_NOT_RUNNING, NOT ok:true, and no compute_job_outputs row', async () => {
        ...
        const { capturedJobId } = interceptFirstCompleteJobSuccessWithRealCancel(
          'W2 negative control: baseline cancelled mid-flight'
        );
    ...
    describe('kpiComputeService.computeAnalysisKpis()', () => {
      it('job cancelled between claim() and commit -> JOB_NOT_RUNNING, NOT ok:true, and no compute_job_outputs row', async () => {
        ...
        const { capturedJobId } = interceptFirstCompleteJobSuccessWithRealCancel(
          'W2 negative control: KPI cancelled mid-flight'
        );
    ...
    describe('predictionComputeService.runPredictionCompute() — STANDARD_BASE branch (runStandardBase)', () => {
      it('job cancelled between claim() and commit -> JOB_NOT_RUNNING, NOT ok:true, and no compute_job_outputs row', async () => {
        ...
        const { capturedJobId } = interceptFirstCompleteJobSuccessWithRealCancel(
          'W2 negative control: prediction cancelled mid-flight'
        );
    ...
    describe('valuationComputeService.runDcfFcffValuation()', () => {
      it('job cancelled between claim() and commit -> JOB_NOT_RUNNING, NOT ok:true (was: canonical instance — completed.ok checked but ok:true returned anyway), no compute_job_outputs row', async () => {
        ...
        const { capturedJobId } = interceptFirstCompleteJobSuccessWithRealCancel(
          'W2 negative control: valuation cancelled mid-flight'
        );
    ...
    describe('OUTPUT_ALREADY_COMMITTED is treated as idempotent-safe, NOT as a failure (unlike NOT_RUNNING)', () => {
      it('baselineComputeService: a pre-existing compute_job_outputs row for the SAME job_id (job still running) does not fail the call', async () => {
        ...
        if (!result.ok)
          throw new Error(
            'unreachable — OUTPUT_ALREADY_COMMITTED wrongly treated as a hard failure'
          );
