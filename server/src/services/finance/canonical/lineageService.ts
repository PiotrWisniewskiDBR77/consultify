...
/**
 * Wynik rozwiązania id z żądania `bulk-read` do kanonicznych `business_version_id`.
 * Przewód E3 do listy (97_ODBIOR_W1_W2.md §6 STOP3 / §9(b)): `FinanceHub.tsx`
 * karmi listę identyfikatorami legacy (`financial_statement_packs.id` itd.),
 * nie kanonicznym `business_version_id` — most to `finance_artifact_aliases`
 * (13 wierszy, WP-C03 backfill).
 */
export interface LineageIdResolution {
  /** Unikatowe kanoniczne `business_version_id`, po których faktycznie odpytujemy `finance_lineage_edges`. */
  canonicalIds: string[];
  /** `requestedId -> business_version_id` dla KAŻDEGO id, które udało się rozwiązać — czy to była już wersja kanoniczna, czy legacy id z aliasem. */
  resolvedVersionIds: Record<string, string>;
  /** Id z żądania bez żadnego kanonicznego odpowiednika (ani bezpośrednio, ani przez alias) — honest gap (Z23), nigdy cicho pominięte. */
  unresolvedIds: string[];
}

/**
 * Rozwiązuje mieszankę id (kanoniczne `business_version_id` LUB legacy id z
 * jednej z czterech tabel `financial_statement_packs`/`financial_analyses`/
 * `financial_models`/`valuations`) do kanonicznych `business_version_id`,
 * jednym zapytaniem per źródło, w jednej transakcji, zawężone do
 * `organizationId` z kontekstu (tenant boundary — pułapka §0.2e(e): podanie
 * legacy id wprost do `finance_lineage_edges` dałoby uczciwą, ale bezużyteczną
 * pustkę zamiast realnych krawędzi).
 */
async function resolveLineageRequestIds(
  organizationId: string,
  requestedIds: readonly string[]
): Promise<LineageIdResolution> {
  if (requestedIds.length === 0) {
    return { canonicalIds: [], resolvedVersionIds: {}, unresolvedIds: [] };
  }
  const ids = [...requestedIds];
  const { directRows, aliasRows } = await withPinnedPostgresTransaction(async (tx) => {
    const directRows = await tx.queryAll<{ business_version_id: string }>(
      `SELECT business_version_id FROM finance_business_versions
        WHERE organization_id = ? AND business_version_id = ANY(?)`,
      [organizationId, ids]
    );
    const aliasRows = await tx.queryAll<{ legacy_id: string; business_version_id: string }>(
      `SELECT legacy_id, business_version_id FROM finance_artifact_aliases
        WHERE organization_id = ? AND legacy_id = ANY(?) AND business_version_id IS NOT NULL`,
      [organizationId, ids]
    );
    return { directRows, aliasRows };
  });
  const directIds = new Set(directRows.map((row) => row.business_version_id));
  // Ostatni alias per legacy_id wygrywa (zgodnie z legacyIdBridgeService.ts —
  // `uq_finance_alias_legacy` teoretycznie dopuszcza więcej niż jeden wiersz
  // per (legacy_table, legacy_id) pod różnym legacy_version).
  const aliasMap = new Map<string, string>();
  for (const row of aliasRows) aliasMap.set(row.legacy_id, row.business_version_id);

  const resolvedVersionIds: Record<string, string> = {};
  const unresolvedIds: string[] = [];
  for (const id of requestedIds) {
    if (directIds.has(id)) {
      resolvedVersionIds[id] = id;
    } else if (aliasMap.has(id)) {
      resolvedVersionIds[id] = aliasMap.get(id)!;
    } else {
      unresolvedIds.push(id);
    }
  }
  const canonicalIds = [...new Set(Object.values(resolvedVersionIds))];
  return { canonicalIds, resolvedVersionIds, unresolvedIds };
}

export interface LineageBulkReadResult {
  edges: LineageEdgeRow[];
  /** patrz `LineageIdResolution.resolvedVersionIds` — klient filtruje `edges` po tej mapie, nie po `businessVersionIds` z żądania wprost. */
  resolvedVersionIds: Record<string, string>;
  /** patrz `LineageIdResolution.unresolvedIds` — kolumna „Source statement" renderuje dla tych id `finance.statements.noSourceStatement`, nigdy pustkę/404. */
  unresolvedIds: string[];
}

/**
 * Direct lineage for many business versions in one organization-scoped query.
 * Przewód E3 do listy: przyjmuje mieszankę kanonicznych `business_version_id`
 * i legacy id, rozwiązuje aliasy PRZED odpytaniem `finance_lineage_edges`
 * (97_ODBIOR_W1_W2.md §9(b) — „rozwiązać aliasy w `getLineageForBusinessVersions`
 * po stronie serwera zamiast czekać na pełny »jeden magazyn«").
 */
export async function getLineageForBusinessVersions(
  organizationId: string,
  businessVersionIds: readonly string[]
): Promise<LineageBulkReadResult> {
  if (businessVersionIds.length === 0) {
    return { edges: [], resolvedVersionIds: {}, unresolvedIds: [] };
  }
  const { canonicalIds, resolvedVersionIds, unresolvedIds } = await resolveLineageRequestIds(
    organizationId,
    businessVersionIds
  );
  ...
  const edges = await withPinnedPostgresTransaction((tx) =>
    tx.queryAll<LineageEdgeRow>(
      ...
      [organizationId, canonicalIds, canonicalIds]
    )
  );
  return { edges, resolvedVersionIds, unresolvedIds };
}
