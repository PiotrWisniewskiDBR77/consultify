...
import { loadContext, type LoadedContext, requireAssumption } from './baselineComputeService.js';
import {
 ...
 */
export function sortOverlapSourcesById<T extends { source_id: string }>(
  sources: readonly T[]
): T[] {
...
export function buildAssumptionSetSemanticHash(
  driverOverrideRows: readonly Pick<
    DriverOverrideRow,
    'id' | 'schedule_type' | 'driver_code' | 'entity_id' | 'period_id' | 'value_decimal'
  >[],
  impactChainRows: readonly Pick<
    ImpactChainRow,
    'id' | 'statement_line_id' | 'amount_kind' | 'amount_decimal' | 'sign' | 'start_period_id'
  >[]
): string {
...
export async function runPreflight(params: RunPreflightParams): Promise<RunPreflightResult> {
  ...
  const overlaps = await withPinnedPostgresTransaction((tx) =>
    tx.queryAll<OverlapRow>(`SELECT * FROM finance_prediction_detect_overlaps(?)`, [
  ...
  const needsBaselineCtx = overlaps.some((o) =>
    o.sources.some((s) => s.source_type === 'DRIVER_OVERRIDE')
  );
  if (needsBaselineCtx) {
  ...
  for (const overlap of overlaps) {
    ...
    for (const source of orderedSources) {
      if (source.source_type === 'DRIVER_OVERRIDE') {
        ...
        if (row && baselineCtx) {
          layer2Deltas.push(
            computeDriverOverrideLayer2Delta(
              row,
              lineCodeById.get(overlap.canonical_line_id) ?? '',
              baselineCtx
            )
          );
        } else {
    ...
    findings.push({
      ...
      layer1:
        overlap.combined_impact_decimal === null ? null : Number(overlap.combined_impact_decimal),
  ...
  const assumptionSetSemanticHash = buildAssumptionSetSemanticHash(
    driverOverrideRows,
    impactChainRows
  );
  ...
  await withPinnedPostgresTransaction(async (tx) => {
    ...
    if (currentRun) {
      await tx.queryRun(
        `UPDATE finance_prediction_preflight_runs SET superseded_by_preflight_run_id = ? WHERE id = ?`,
        [preflightRunId, currentRun.id]
      );
    }
...
async function resolveBaselineModelVersion(
  predictionScenarioBusinessVersionId: string
): Promise<string | null> {
 ...
 */
function computeDriverOverrideLayer2Delta(
  row: DriverOverrideRow,
  lineCode: string,
  ctx: LoadedContext
): number {
  const overrideValue = row.value_decimal === null ? null : Number(row.value_decimal);
  const baselineValue =
    row.baseline_value_decimal === null
      ? requireAssumption(ctx, row.schedule_type, row.driver_code)
      : Number(row.baseline_value_decimal);
  ...
  const priorYearRevenue =
    period && period.fiscal_month !== null
      ? ctx.revenueHistoryByFiscalYearMonth.get(`${period.fiscal_year - 1}-${period.fiscal_month}`)
      : undefined;
  const revenueGrowth = requireAssumption(ctx, 'revenue_pvm', 'REVENUE_GROWTH_YOY');
  const revenue =
    priorYearRevenue !== undefined
      ? computeRevenuePvm({
  ...
  switch (row.schedule_type) {
    case 'revenue_pvm': {
      ...
      const base = computeRevenuePvm({
        priorYearSameMonthRevenue: priorYearRevenue,
        annualGrowthRate: baselineValue,
      });
      const withOverride = computeRevenuePvm({
        priorYearSameMonthRevenue: priorYearRevenue,
        annualGrowthRate: overrideValue,
      });
    ...
    case 'cogs_opex': {
      ...
      const base = computeCogsOpex({
        revenue,
        cogsRatio: isOpexDriver ? cogsRatio : baselineValue,
        opexRatio: isOpexDriver ? baselineValue : opexRatio,
      });
    ...
    case 'capex_depreciation': {
      ...
      return lineCode === 'DEPRECIATION'
        ? withOverride.depreciation - base.depreciation
        : withOverride.capex - base.capex;
    }
    case 'wc_dso_dio_dpo': {
      ...
      const pick = (driver: string, base: number) =>
        row.driver_code === driver
          ? { base: baselineValue, override: overrideValue }
          : { base, override: base };
      const dsoV = pick('DSO_DAYS', dso);
      const dioV = pick('DIO_DAYS', dio);
      const dpoV = pick('DPO_DAYS', dpo);
      const base = computeWcDsoDioDpo({
        revenue,
        cogs,
        daysInPeriod: days,
        dsoDays: dsoV.base,
      ...
      const withOverride = computeWcDsoDioDpo({
        revenue,
        cogs,
        daysInPeriod: days,
        dsoDays: dsoV.override,
        dioDays: dioV.override,
        dpoDays: dpoV.override,
      });
...
function computeImpactChainLayer2Delta(
  row: ImpactChainRow,
  initiative: InitiativeDefaultsRow | undefined
): number {
