...
class PartnerServiceClass {
  ...
  ): Promise<PartnerWithMetadata & { createdAt: string }> {
    const {
      name,
      partnerType,
      email = null,
  ...
  ): Promise<PartnerWithMetadata | null> {
    const fields: string[] = [];
  ...
  ): Promise<Array<PartnerWithMetadata & { createdAt?: string }>> {
  ...
  async createAgreement(params: {
    ...
    logger.info(
      `[PartnerService] Created agreement for partner ${partnerId}: ${revenueSharePercent}%`
    );
...
export const updatePartner = (id: string, updates: PartnerUpdateParams) =>
...
export const getByPartnerCode = (partnerCode: string) =>
  partnerService.getByPartnerCode(partnerCode);
