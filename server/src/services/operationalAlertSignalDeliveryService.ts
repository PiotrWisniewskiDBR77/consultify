...
export async function recordOperationalAlertSignal(input: OperationalAlertSignalInput) {
  ...
  if (process.env.OPERATIONAL_ALERT_DURABLE_ENABLED !== 'true') {
    if (
      !(
        ...
        input.sourceType === 'operator_positive_control' &&
  ...
  return withPgTransaction(async (tx) => {
    await tx.query(`SELECT pg_advisory_xact_lock(hashtext(?))`, [
      `ops-signal:${normalized.organizationId}:${normalized.idempotencyKey}`,
    ]);
    const found = await tx.query<any>(
      `SELECT * FROM operational_alert_signals WHERE organization_id=? AND idempotency_key=?`,
      [normalized.organizationId, normalized.idempotencyKey]
    );
    if (found.rows[0]) {
      if (found.rows[0].input_fingerprint !== hash)
        throw new Error('OPS_ALERT_IDEMPOTENCY_COLLISION');
      return found.rows[0];
    }
...
async function measure(
  ...
  const result = await tx.query<any>(
    ...
    FROM operational_alert_signals WHERE organization_id=? AND kind=?
      AND occurred_at > ?::timestamptz - (? * interval '1 millisecond') AND occurred_at <= ?::timestamptz`,
    [org, kind, now, spec.windowMs, now]
  );
  const row = result.rows[0];
  if (kind === 'WRITE_FAILURE_RATE')
    return {
      active: row.total > 0 && row.failures / row.total >= spec.threshold,
      value: row.total ? row.failures / row.total : 0,
      correlationId: row.correlation_id,
    };
  if (kind === 'REPEATED_AUTH_DENIALS')
    return {
      active: row.denials >= spec.threshold,
      value: row.denials,
      correlationId: row.correlation_id,
    };
  if (kind === 'DB_SATURATION')
    return {
      active:
        row.sample_count > 1 &&
        Number(row.min_value) >= spec.threshold &&
        new Date(row.first_at).getTime() <= new Date(now).getTime() - spec.windowMs + 1000,
      value: Number(row.max_value ?? 0),
      correlationId: row.correlation_id,
    };
  return {
    active: Number(row.max_value ?? 0) >= spec.threshold,
    value: Number(row.max_value ?? 0),
    correlationId: row.correlation_id,
  };
}

export async function evaluateOperationalAlertWindows(params: {
  organizationId: string;
  evaluatorId: string;
  now?: string;
}) {
  ...
  return withPgTransaction(async (tx) => {
    ...
    for (const kind of Object.keys(SPECS) as OperationalAlertKind[]) {
      ...
      const selected = await tx.query<any>(
        `SELECT * FROM operational_alert_tenant_states WHERE organization_id=? AND kind=? FOR UPDATE`,
        [org, kind]
      );
      const old = selected.rows[0];
      const wasActive = old?.status === 'ACTIVE';
      const transition =
        metric.active && !wasActive ? 'DETECTED' : !metric.active && wasActive ? 'RECOVERED' : null;
      const nextStatus = metric.active
        ? 'ACTIVE'
      ...
      const correlationId =
        metric.correlationId || `ops-eval:${org}:${kind}:${new Date(now).toISOString()}`;
      const state = await tx.query<any>(
        ...
        VALUES(?,?,?,?,?,?::jsonb) ON CONFLICT DO NOTHING`,
          [
            ...
            JSON.stringify({
              ...
              correlationId,
              value: metric.value,
              threshold: SPECS[kind].threshold,
              occurredAt: now,
            }),
          ]
        );
...
export async function evaluateAllOperationalAlertOrganizations(params: {
  evaluatorId: string;
  now?: string;
}) {
  const organizations = await withPgTransaction(
    async (tx) =>
      (
        await tx.query<{ organization_id: string }>(
          `SELECT DISTINCT organization_id FROM operational_alert_signals WHERE occurred_at > COALESCE(?::timestamptz,now()) - interval '24 hours'`,
          [params.now ?? null]
        )
      ).rows
  );
  ...
  for (const row of organizations)
    result.push(
      ...(await evaluateOperationalAlertWindows({
...
export async function listOperationalAlertDeliveryState(params: { organizationId?: string } = {}) {
  return withPgTransaction(async (tx) => {
    ...
    const values = params.organizationId
      ? [required(params.organizationId, 'OPS_ALERT_ORGANIZATION_INVALID')]
      : [];
    const states = await tx.query<any>(
      `SELECT * FROM operational_alert_tenant_states ${where} ORDER BY organization_id,kind`,
      values
    );
    const deliveries = await tx.query<any>(
      `SELECT delivery_id,organization_id,kind,transition,state_version,status,attempt_count,max_attempts,available_at,delivered_at,last_error FROM operational_alert_delivery_outbox ${where} ORDER BY created_at DESC LIMIT 200`,
      values
    );
...
export async function acknowledgeRecoveredTenantAlert(params: {
  organizationId: string;
  kind: OperationalAlertKind;
  evaluatorId: string;
}) {
  return withPgTransaction(async (tx) => {
    const selected = await tx.query<any>(
      `SELECT * FROM operational_alert_tenant_states WHERE organization_id=? AND kind=? FOR UPDATE`,
      [required(params.organizationId, 'OPS_ALERT_ORGANIZATION_INVALID'), params.kind]
    );
    ...
    if (selected.rows[0].status !== 'RECOVERED' && selected.rows[0].status !== 'ACKNOWLEDGED')
      throw new Error('OPS_ALERT_INCIDENT_NOT_RECOVERED');
    if (selected.rows[0].status === 'ACKNOWLEDGED') return selected.rows[0];
    const updated = await tx.query<any>(
      `UPDATE operational_alert_tenant_states SET status='ACKNOWLEDGED',acknowledged_at=now(),evaluator_id=?,version=version+1,updated_at=now() WHERE organization_id=? AND kind=? RETURNING *`,
      [
        required(params.evaluatorId, 'OPS_ALERT_EVALUATOR_ID_INVALID'),
...
export async function claimOperationalAlertDeliveries(params: {
  workerId: string;
  organizationId?: string;
  limit?: number;
  leaseMs?: number;
  ...
  values.push(
    params.limit ?? 20,
    required(params.workerId, 'OPS_ALERT_WORKER_INVALID'),
    params.leaseMs ?? 30000
  );
  return withPgTransaction(
    ...
    FROM candidates c WHERE o.delivery_id=c.delivery_id RETURNING o.*`,
          values
        )
      ).rows
  );
...
export async function completeOperationalAlertDelivery(params: {
  deliveryId: string;
  workerId: string;
  ...
  return withPgTransaction(async (tx) => {
    const selected = await tx.query<any>(
      `SELECT * FROM operational_alert_delivery_outbox WHERE delivery_id=? AND status='PROCESSING' AND lease_owner=? FOR UPDATE`,
      [params.deliveryId, params.workerId]
    );
    if (!selected.rows[0]) throw new Error('OPS_ALERT_DELIVERY_LEASE_LOST');
    const digest = fingerprint(params.responseBody ?? '');
    await tx.query(
      `INSERT INTO operational_alert_delivery_receipts(delivery_id,organization_id,response_status,response_digest) VALUES(?,?,?,?) ON CONFLICT(delivery_id) DO NOTHING`,
      [params.deliveryId, selected.rows[0].organization_id, params.responseStatus, digest]
    );
    return (
      await tx.query<any>(
        `UPDATE operational_alert_delivery_outbox SET status='DELIVERED',delivered_at=now(),lease_owner=NULL,lease_expires_at=NULL,updated_at=now() WHERE delivery_id=? RETURNING *`,
        [params.deliveryId]
      )
    ).rows[0];
  });
...
export async function failOperationalAlertDelivery(params: {
  deliveryId: string;
  workerId: string;
  error: string;
  now?: string;
  ...
  return withPgTransaction(async (tx) => {
    ...
    return (
      await tx.query<any>(
        `UPDATE operational_alert_delivery_outbox SET status=?,attempt_count=?,available_at=?::timestamptz+(LEAST(300000,1000*power(2,?))*interval '1 millisecond'),last_error=?,lease_owner=NULL,lease_expires_at=NULL,updated_at=now() WHERE delivery_id=? RETURNING *`,
        [
...
export async function deliverOperationalAlerts(params: {
  endpoint?: string;
  workerId: string;
  organizationId?: string;
  ...
  if (
    ...
    return { disabled: true, delivered: 0, failed: 0 };
  const endpoint = params.endpoint ?? process.env.OPERATIONAL_ALERT_DELIVERY_ENDPOINT;
  let target: URL;
  try {
    target = new URL(endpoint ?? '');
  } catch {
    throw new Error('OPS_ALERT_TRANSPORT_NOT_AUTHORIZED');
  }
  if (
  ...
  for (const row of claimed) {
    try {
      const response = await (params.fetchImpl ?? fetch)(target, {
        method: 'POST',
        redirect: 'error',
        headers: { 'content-type': 'application/json', 'x-ops-delivery-id': row.delivery_id },
        body: JSON.stringify(row.payload_json),
      });
      if (!response.ok) throw new Error(`HTTP_${response.status}`);
      await completeOperationalAlertDelivery({
      ...
      await failOperationalAlertDelivery({
        deliveryId: row.delivery_id,
        workerId: params.workerId,
