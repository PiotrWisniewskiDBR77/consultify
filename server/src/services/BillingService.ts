...
class BillingServiceClass {
  ...
  async recordInvoice(orgId: string, stripeInvoice: StripeTypes.Invoice) {
    await this.#ensureInitialized();
...
export const createUserPlan = (planData: CreateUserPlanData) =>
  billingServiceInstance.createUserPlan(planData);
...
export const getOrganizationBilling = (orgId: string) =>
  billingServiceInstance.getOrganizationBilling(orgId);
...
export const cancelSubscription = (orgId: string) =>
  billingServiceInstance.cancelSubscription(orgId);
...
export const getPaymentMethod = (paymentMethodId: string) =>
  billingServiceInstance.getPaymentMethod(paymentMethodId);
...
export const incrementDiscountCodeUsage = (codeId: string) =>
  billingServiceInstance.incrementDiscountCodeUsage(codeId);
