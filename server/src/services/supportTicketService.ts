...
class SupportTicketServiceClass {
    ...
    async updateTicket(id: string, updates: any): Promise<boolean> {
        ...
        params.push(id);
        const result = await this.db.run(`UPDATE support_tickets SET ${fields.join(', ')} WHERE id = ?`, params);
        return (result as any).changes > 0;
    }

    async addComment(ticketId: string, userId: string, text: string, isInternal: boolean = false): Promise<any> {
        const id = this.uuidv4();
        await this.db.run(
