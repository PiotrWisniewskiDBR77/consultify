...
export class TokenBillingServiceClass {
  ...
  async getMargin(sourceType: string): Promise<BillingMargin | null> {
    return this.dbGet<BillingMargin>('SELECT * FROM billing_margins WHERE source_type = ?', [
      sourceType,
    ]);
  }
  ...
  async getPackages(): Promise<TokenPackage[]> {
    return this.dbAll<TokenPackage>(
      'SELECT * FROM token_packages WHERE is_active = 1 ORDER BY sort_order'
    );
  }
  ...
  async getBalance(userId: string): Promise<UserBalance> {
    const row = await this.dbGet<UserBalance>(
      'SELECT * FROM user_token_balance WHERE user_id = ?',
      [userId]
    );
    return (
      row || {
