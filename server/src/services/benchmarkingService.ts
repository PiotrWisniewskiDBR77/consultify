...
class BenchmarkingService {
  ...
  static getBenchmark(industry: string) {
    ...
    return (
      (BenchmarkingService.INDUSTRY_BENCHMARKS as any)[key] ||
      BenchmarkingService.INDUSTRY_BENCHMARKS.OTHER
    );
  }
  ...
  static getDTITier(score: number) {
    ...
    if (score >= 60)
      return { tier: 'Advancer', description: 'Making significant digital progress' };
