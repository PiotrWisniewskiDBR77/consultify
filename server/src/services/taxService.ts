...
class TaxServiceClass {
    ...
    if (useStripeTax && this.stripe) {
      try {
        ...
        return {
          ...
          taxRate:
            (((taxCalc as any).tax_breakdown?.[0]?.tax_rate_details?.percentage_decimal ||
              0) as number) * 100,
          taxType: ((taxCalc as any).tax_breakdown?.[0]?.tax_rate_details?.tax_type ||
            'sales_tax') as string | undefined,
   ...
   */
  private async validateWithVIES(
    vatNumber: string,
    countryCode: string
...
export const getTaxRates = (options?: {
  country?: string;
  taxType?: string;
...
export const isEUCountry = (countryCode: string | null | undefined) =>
  taxService.isEUCountry(countryCode);
