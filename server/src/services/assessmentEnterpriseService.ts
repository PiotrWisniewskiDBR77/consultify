...
class AssessmentEnterpriseService {
  ...
  async createFinding(
    ...
    await queryHelpers.queryRun(
      ...
      [
        ...
        orgId,
        data.assessmentId,
        data.findingType ?? 'nonconformity',
        data.severity ?? 'minor',
        data.clauseRef ?? null,
  ...
  async getFindings(
    ...
    if (filters?.findingType) {
      conditions.push(`finding_type=$${idx++}`);
      params.push(filters.findingType);
    }
    if (filters?.clauseRef) {
      conditions.push(`clause_ref=$${idx++}`);
      params.push(filters.clauseRef);
    }
  ...
  async updateCapaAction(
    ...
    if (data.verificationResult !== undefined) {
      sets.push(`verification_result=$${idx++}`);
      params.push(data.verificationResult);
    }
  ...
  async getScoringProposals(orgId: string, assessmentId: string, status?: string) {
    ...
    return queryHelpers.queryAll(
      sql,
      status ? [orgId, assessmentId, status] : [orgId, assessmentId]
    );
  }

  async reviewScoringProposal(
    proposalId: string,
    data: {
  ...
  async createEvalDataset(
    ...
    await queryHelpers.queryRun(
      ...
      [
        ...
        data.name,
        data.description ?? null,
        data.goldenItems ? JSON.stringify(data.goldenItems) : '[]',
        data.createdBy,
      ]
    );
    return { id };
  ...
  async createEvalRun(
    ...
    await queryHelpers.queryRun(
      ...
      [
        ...
        data.recall ?? null,
        data.f1Score ?? null,
        data.detailsJson ? JSON.stringify(data.detailsJson) : '{}',
        data.runBy,
      ]
    );
    return { id };
  ...
  async getReviews(orgId: string, assessmentId: string, versionId?: string) {
    ...
    return queryHelpers.queryAll(
      sql,
      versionId ? [orgId, assessmentId, versionId] : [orgId, assessmentId]
    );
  }
