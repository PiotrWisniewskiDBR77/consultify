...
class ToolEnterpriseService {
  ...
  async updateTemplate(
    ...
    if (data.schemaJson !== undefined) {
      sets.push(`schema_json=$${idx++}`);
      params.push(JSON.stringify(data.schemaJson));
    }
    if (data.defaultConfig !== undefined) {
      sets.push(`default_config=$${idx++}`);
      params.push(JSON.stringify(data.defaultConfig));
    }
    if (data.isOrgCurated !== undefined) {
      sets.push(`is_org_curated=$${idx++}`);
      params.push(data.isOrgCurated ? 1 : 0);
    }
  ...
  async searchKnowledge(orgId: string, query: string, toolSessionId?: string, limit: number = 10) {
    ...
    return queryHelpers.queryAll(
      sql,
      toolSessionId ? [orgId, toolSessionId, likeQuery, limit] : [orgId, likeQuery, limit]
    );
  }
  ...
  async createEntitlement(
    ...
    await queryHelpers.queryRun(
      ...
      [
        ...
        data.maxSessionsPerMonth ?? null,
        data.maxConcurrentUsers ?? null,
        data.validFrom ?? null,
  ...
  async checkEntitlement(
    orgId: string,
    toolKey: string
  ): Promise<{ allowed: boolean; reason?: string; entitlementId?: string }> {
    const entitlements = await queryHelpers.queryAll<{
      id: string;
