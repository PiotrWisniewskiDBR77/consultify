...
class DecisionService {
  ...
  async createDecision(input: CreateDecisionInput): Promise<Decision> {
    if (input.idempotencyKey && input.sourceType === 'myw_agent_proposal')
      return this.createIdempotentDecision(input);
  ...
  private async createIdempotentDecision(input: CreateDecisionInput): Promise<Decision> {
    ...
    const targetId = await withPgTransaction(async (tx) => {
      await tx.query(`SELECT pg_advisory_xact_lock(hashtext(?))`, [
        `decision-command:${input.organizationId}:${commandKey}`,
      ]);
      const existing = await tx.query<any>(
        `SELECT id,source_type,source_id FROM decisions
        WHERE organization_id=? AND idempotency_key=? AND source_type='myw_agent_proposal'`,
        [input.organizationId, commandKey]
      );
      if (existing.rows[0]) {
        if (
          existing.rows[0].source_type !== input.sourceType ||
          existing.rows[0].source_id !== input.sourceId
        ) {
          throw new Error('DECISION_IDEMPOTENCY_COLLISION');
        }
      ...
      await tx.query(
        `INSERT INTO decisions (id,organization_id,project_id,initiative_id,task_id,title,description,type,
    ...
    if (input.faultInjection === 'AFTER_HISTORY')
      throw new Error('DECISION_TEST_CRASH_AFTER_HISTORY');
    ...
    await withPgTransaction(async (tx) => {
      await tx.query(
        ...
        [
          ...
          commandKey,
          targetId,
          JSON.stringify({ decisionMakerId: input.decisionMakerId, title: input.title }),
        ]
      );
    });
