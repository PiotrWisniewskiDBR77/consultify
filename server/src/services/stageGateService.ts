 ...
 */
export async function evaluateGate(projectId: string, gateType: GateType): Promise<GateEvaluationResult> {
...
async function checkContextReadiness(projectId: string): Promise<boolean> {
    ...
    try {
        ...
        const hasChallenges = ctx.challenges && Array.isArray(ctx.challenges) && ctx.challenges.length > 0;
...
export async function passGate(projectId: string, gateType: GateType, userId: string, notes?: string): Promise<GatePassageResult> {
    ...
    await DbPromise.run(db, sql, [id, projectId, gateType, fromPhase, toPhase, userId, notes || null]);

    // Update project phase
    if (toPhase) {
        await DbPromise.run(db, `UPDATE projects SET current_phase = ? WHERE id = ?`, [toPhase, projectId]);
    }
