...
export class AIPipeline {
  ...
  private buildSystemPrompt(capability: AICapability, ctx: any, request: AIPipelineRequest): string {
    ...
    // 8. Behavioral instructions (pass request for showReasoning check)
    parts.push(this.buildBehavioralInstructions(capability, ctx, request));
    
    // 9. Web search context if available
    if (ctx?.external?.webSearchContext) {
      parts.push(ctx.external.webSearchContext);
    }
    
    // 10. Knowledge Hub context if available
    if (ctx?.knowledgeHub?.summary) {
      parts.push(ctx.knowledgeHub.summary);
    }
    
    // 11. Custom instructions if available
    if (ctx?.instructions?.merged) {
      parts.push('\n## INSTRUKCJE NIESTANDARDOWE');
      parts.push(ctx.instructions.merged);
    }
...
Użytkownik może zapytać o te akcje - możesz mu pomóc je przejrzeć i zatwierdzić.`;
  ...
  private buildBehavioralInstructions(capability: AICapability, ctx: any, request?: AIPipelineRequest): string {
    ...
    // Add thinking/reasoning instruction if enabled
    if ((request?.options as any)?.showReasoning) {
      instructions.push('\n## PROCES MYŚLENIA');
      instructions.push('Przed odpowiedzią, przedstaw swój proces myślenia w tagach <thinking>...</thinking>.');
      instructions.push('W sekcji thinking:');
      instructions.push('- Przeanalizuj pytanie użytkownika');
      instructions.push('- Rozważ dostępny kontekst');
      instructions.push('- Zaplanuj strukturę odpowiedzi');
      instructions.push('Przykład: <thinking>Analizuję pytanie o zadania... Widzę 5 zadań w kontekście... Skupię się na priorytetowych...</thinking>');
    }
