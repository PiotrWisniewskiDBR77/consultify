...
describe('Day 213 explicit organization scope for service inserters on real PostgreSQL', NO_RETRY, () => {
  ...
  it('knowledgeIndexer.insertDocument writes organization scope explicitly', async () => {
    ...
    const result = await pool.query('SELECT scope FROM knowledge_docs WHERE id = $1', [indexerDocId]);
