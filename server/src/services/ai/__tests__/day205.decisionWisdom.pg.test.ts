...
describe('Day205 R3 chat recommendation enters decision memory on real PostgreSQL', NO_RETRY, () => {
  ...
  beforeAll(async () => {
    ...
    await sql.connect();
    await sql.query(`INSERT INTO organizations (id,name,plan,status) VALUES ($1,'Day205 R3','enterprise','active')`, [organizationId]);
    await sql.query(
    ...
    generateSectionSpy = vi.spyOn(decisionService, 'generateSection').mockImplementation(
      async (_id: string, sectionKey: any) => {
        ...
        if (sectionKey === 'risk') {
          return {
            ...
            content: '',
            isJson: true,
            parsedContent: { risks: [{ title: 'Capacity', probability: 'medium', impact: 'high' }] },
            model: 'day205-test',
            tokensUsed: 0,
          };
        }
  ...
  afterAll(async () => {
    ...
    if (!sql) return;
    await sql.query(`DELETE FROM ai_decision_outcomes WHERE organization_id=$1`, [organizationId]);
  ...
  it('records the recommendation, remains pending until outcome, then becomes findable', async () => {
    ...
    let memoryRows: Array<{ id: string; decision_summary: string; recommendation_text: string; outcome_status: string }> = [];
