...
describe.skipIf(!enabled)('DEC-AGT-AUTONOMY durable dispatch — real PG + Redis', () => {
  ...
  it('OFF creates neither receipt nor Redis job', async () => {
    const off = await service.dispatchAgentTask(
      { ...input, dispatchKey: `off-${tag}` },
      { env: {} as NodeJS.ProcessEnv }
    );
    expect(off.status).toBe('DISABLED');
    const rows = await pool.query(
      `SELECT count(*)::int n FROM ai_agent_job_receipts WHERE dispatch_key=$1`,
      [`off-${tag}`]
    );
  ...
  it('8-way same key converges on one receipt and one stable Bull job; collision fails', async () => {
    const results = await Promise.all(
      Array.from({ length: 8 }, () => service.dispatchAgentTask(input))
    );
    expect(new Set(results.map((r) => r.receiptId)).size).toBe(1);
    const rows = await pool.query(
      `SELECT * FROM ai_agent_job_receipts WHERE organization_id=$1 AND dispatch_key=$2`,
      [input.organizationId, input.dispatchKey]
    );
    expect(rows.rows).toHaveLength(1);
    const { default: queue } = await import('../../../queues/aiQueue.js');
    expect(await queue.getJob(rows.rows[0].bull_job_id)).not.toBeNull();
    await expect(service.dispatchAgentTask({ ...input, planId: `other-${tag}` })).rejects.toThrow(
      'AGENT_DISPATCH_PAYLOAD_COLLISION'
    );
  });

  it('tenant identity, durable failure, explicit redrive, reclaim and cold read are fail closed', async () => {
    const receipt = await pool.query(
      `SELECT * FROM ai_agent_job_receipts WHERE organization_id=$1`,
      [input.organizationId]
    );
    const row = receipt.rows[0];
    const claimed = await service.claimAgentTask({
      ...input,
    ...
    await expect(
      service.claimAgentTask({
        ...input,
        organizationId: `foreign-${tag}`,
        receiptId: row.receipt_id,
        payloadDigest: row.payload_digest,
        workerId: 'worker-b',
      })
    ).rejects.toThrow();
    const second = await service.claimAgentTask({
      ...input,
      receiptId: row.receipt_id,
      payloadDigest: row.payload_digest,
      workerId: 'worker-b',
    ...
    const terminal = await cold.query(
      `SELECT status,attempt_count FROM ai_agent_job_receipts WHERE receipt_id=$1`,
      [row.receipt_id]
    );
    await cold.end();
    ...
    expect(
      (
        await pool.query(`SELECT event_type FROM ai_agent_job_attempts WHERE receipt_id=$1`, [
          row.receipt_id,
        ])
  ...
  it('keeps enqueue failure recoverable, isolates a second tenant and reclaims an expired crash lease', async () => {
    ...
    expect(
      (
        await service.dispatchAgentTask(pending, {
          beforeEnqueue: async () => {
    ...
    const pendingRow = await pool.query(
      `SELECT status FROM ai_agent_job_receipts WHERE organization_id=$1 AND dispatch_key=$2`,
    ...
    const foreign = {
      planId: `foreign-plan-${tag}`,
      organizationId: `foreign-org-${tag}`,
      userId: `foreign-user-${tag}`,
    ...
    const crashRow = await pool.query(`SELECT * FROM ai_agent_job_receipts WHERE receipt_id=$1`, [
      dispatched.receiptId,
    ]);
    await service.claimAgentTask({
      ...crash,
      receiptId: crashRow.rows[0].receipt_id,
      payloadDigest: crashRow.rows[0].payload_digest,
      workerId: 'worker-crashed',
    });
    await pool.query(
      `UPDATE ai_agent_job_receipts SET lease_expires_at=now()-interval '1 second' WHERE receipt_id=$1`,
      [crashRow.rows[0].receipt_id]
    );
    const reclaimed = await service.claimAgentTask({
      ...crash,
      receiptId: crashRow.rows[0].receipt_id,
      payloadDigest: crashRow.rows[0].payload_digest,
  ...
  it('keeps OFF redrive immutable and converges two operators through one recoverable Redis-down transition', async () => {
    ...
    const row = (
      await pool.query(`SELECT * FROM ai_agent_job_receipts WHERE receipt_id=$1`, [
        dispatched.receiptId,
      ])
    ).rows[0];
    await service.claimAgentTask({
    ...
    const beforeEvents = await pool.query(
      `SELECT count(*)::int n FROM ai_agent_job_attempts WHERE receipt_id=$1`,
      [row.receipt_id]
    );
    expect(
      (
        await service.redriveAgentTask(row.receipt_id, 'operator-off', {
          env: {} as NodeJS.ProcessEnv,
        })
      ).status
    ).toBe('DISABLED');
    ...
    expect(
      (
        await pool.query(`SELECT count(*)::int n FROM ai_agent_job_attempts WHERE receipt_id=$1`, [
          row.receipt_id,
        ])
      ).rows[0].n
    ).toBe(beforeEvents.rows[0].n);

    const redisDown = async () => {
      throw new Error('REDIS_DOWN');
    };
  ...
  it('rejects literal UPDATE and DELETE of the append-only attempt ledger', async () => {
    const attempt = await pool.query(
      `SELECT attempt_id FROM ai_agent_job_attempts ORDER BY created_at LIMIT 1`
    );
    expect(attempt.rows[0]?.attempt_id).toBeTruthy();
    await expect(
      pool.query(`UPDATE ai_agent_job_attempts SET worker_id='tampered' WHERE attempt_id=$1`, [
        attempt.rows[0].attempt_id,
      ])
    ).rejects.toThrow(/append.only/i);
    await expect(
      pool.query(`DELETE FROM ai_agent_job_attempts WHERE attempt_id=$1`, [
        attempt.rows[0].attempt_id,
      ])
    ).rejects.toThrow(/append.only/i);
  });
  ...
  it('links Bull exhaustion to durable FAILED, explicit redrive and terminal success', async () => {
    ...
    try {
      ...
      await expect(firstJob!.waitUntilFinished(queueEvents, 30_000)).rejects.toThrow(
      ...
      expect(
        (await service.redriveAgentTask(dispatched.receiptId!, 'operator-recovery')).status
      ).toBe('ENQUEUED');
