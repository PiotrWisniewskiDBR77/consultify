...
export async function dispatchAgentTask(
  ...
  options: {
    ...
    replaceExistingJob?: boolean;
  } = {}
): Promise<{
  status: 'DISABLED' | 'ENQUEUED' | 'REPLAY' | 'PENDING';
  receiptId?: string;
  bullJobId?: string;
}> {
  ...
  const receipt = await withPgTransaction(async (tx) => {
    await tx.query(`SELECT pg_advisory_xact_lock(hashtext(?))`, [
      `agent-dispatch:${id.organizationId}:${id.dispatchKey}`,
    ]);
    ...
    if (existing.rows[0]) {
      if (existing.rows[0].payload_digest !== id.payloadDigest)
        throw new Error('AGENT_DISPATCH_PAYLOAD_COLLISION');
  ...
  try {
    ...
    await withPgTransaction(async (tx) => {
      ...
      const enqueuePromise = aiQueue.add(
...
export async function claimAgentTask(
  input: AgentDispatchInput & { receiptId: string; payloadDigest: string; workerId: string }
) {
  const id = identity(input);
  ...
  return withPgTransaction(async (tx) => {
    ...
    if (!row) {
      const current = await tx.query<ReceiptRow>(
        `SELECT * FROM ai_agent_job_receipts WHERE receipt_id=?`,
        [input.receiptId]
      );
      if (current.rows[0]?.status === 'SUCCEEDED')
        return { replayed: true, attemptNo: current.rows[0].attempt_count };
      throw new Error('AGENT_DISPATCH_CLAIM_CONFLICT');
    }
    await tx.query(
      `INSERT INTO ai_agent_job_attempts(receipt_id,attempt_no,event_type,worker_id) VALUES(?,?,'CLAIMED',?)`,
...
export async function finishAgentTask(
  ...
  const code = ok
    ? null
    : String(error instanceof Error ? error.message : error || 'AGENT_TASK_FAILED').slice(0, 128);
  return withPgTransaction(async (tx) => {
    ...
    await tx.query(
      `INSERT INTO ai_agent_job_attempts(receipt_id,attempt_no,event_type,worker_id,error_code) VALUES(?,?,?, ?,?)`,
      [receiptId, row.rows[0].attempt_count, ok ? 'SUCCEEDED' : 'FAILED', workerId, code]
    );
    operationalAlerts.recordWrite({
      correlationId: receiptId,
      tenantId: row.rows[0].organization_id,
      actorId: row.rows[0].user_id,
      sourceId: row.rows[0].plan_id,
