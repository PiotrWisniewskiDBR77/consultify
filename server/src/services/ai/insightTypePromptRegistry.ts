...
export const INSIGHT_TYPE_PROMPT_REGISTRY: Record<InsightFormulaSectionId, InsightTypePromptSpec> = {
  'executive-summary': {
    ...
    envelope: '60–130 słów, ≥3 zdania. Zero rozgrzewki („W trakcie rozmów…" = kasacja). Ton: zarząd czyta 20 s.',
    wzorzec:
...
const REPAIR_HINTS: Record<string, string> = {
  ...
  'insight.tension_two_sided': 'Napięcie musi mieć dowód po OBU stronach (A vs B, każda z H#) + so-what trade-offu.',
  'insight.pattern_multisource': 'Wzorzec wymaga zasięgu ≥2 niezależne źródła/sesje — podaj zasięg jawnie.',
  'insight.model_heldby': 'Model myślowy: podaj held_by (kto trzyma) + implikację dla programu.',
  'insight.power_implication': 'Dynamika władzy: podaj stance (za/przeciw) + implikację dla sponsora (konkretny ruch).',
  'insight.qc_multivoice': 'Porównanie cytatów: ≥2 dosłowne cytaty z RÓŻNYCH ról na jedną oś + so-what kontrastu.',
};
