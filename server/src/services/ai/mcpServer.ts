...
export const ToolSchemas: Record<string, Omit<ToolEntry, 'handler'>> = {
  ...
  generate_deliverable: {
    ...
    description:
      ...
      'For "zrób diagram procesu / przepływ" use type="process_flow"; for "zrób tablicę / whiteboard" use type="whiteboard" (only available when canvas-tools generation is enabled; fall back to a document/sheet if the type is rejected). ' +
      'TABLE vs SHEET disambiguation — read carefully: a request for a TABLE (a list/portfolio/board of items with rows, e.g. "zrób tabelę portfela inicjatyw", "tabela pomysłów", "lista zadań z kolumnami", "burza pomysłów w tabeli", "make a table of ...") is type="table" (an interactive Ideas Table). Only use type="sheet" for a NUMERIC SPREADSHEET / financial model / Excel workbook with formulas, calculations or scenario math (e.g. "arkusz finansowy", "model w Excelu", "budżet z formułami"). When in doubt between the two, prefer type="table". DO NOT answer a "zrób tabelę …" request by writing a Markdown table inline — CALL this tool with type="table" so a real Ideas Table opens on the right. ' +
