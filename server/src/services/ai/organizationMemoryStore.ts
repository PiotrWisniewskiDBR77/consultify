...
type ModelRouterCtor = new () => {
  getProviderConfig: (model: string, tier: string) => Promise<unknown>;
};
...
export class OrganizationMemoryStore {
  ...
  async addSuccessPattern(
    ...
    const { projectId, title, description, keyFactors, metrics, recommendations, createdBy } =
      successData;
  ...
  async addBestPractice(
    ...
    const { title, description, steps, benefits, applicableContexts, industry, createdBy } =
      practiceData;
...
Return JSON:
{
  ...
  "bestPractices": [{ "title": "...", "description": "...", "steps": [...] }]
      ...
      aiLogger.info(
        'OrgMemoryStore',
        `Extracted ${extractedCount} patterns from project ${projectData.id}`
      );
