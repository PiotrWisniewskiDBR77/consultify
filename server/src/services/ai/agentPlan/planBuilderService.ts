...
const CURATED_PLAYBOOKS: Record<string, (label: string) => PlanBuilderStep[]> = {
  ...
  'growth-paths': (label) => [
    step(
      'search_knowledge_base',
      { query: 'growth strategy: market penetration, market development, product development, diversification' },
  ...
  'sop-builder': (label) => [
    ...
    step(
      ...
      { title: 'SOP Draft', content: 'Draft SOP generated by the SOP Builder agent — review before publishing.', entry_type: 'action_item' },
  ...
  'a3-problem-solving': (label) => [
    ...
    step(
      'find_similar_decisions',
      { description: 'Root cause analysis for a recurring operational problem', decision_type: 'process' },
      `${label}: check how similar root causes were tackled before`
    ),
    step(
      'create_notebook_entry',
      { title: 'A3 Problem Solving Draft', content: 'Draft A3 (background, current condition, root cause, countermeasures) — review before finalizing.', entry_type: 'research' },
  ...
  'dms-builder': (label) => [
    ...
    step(
      'create_notebook_entry',
      { title: 'Daily Management System Draft', content: 'Draft daily management routine (tiered huddles, KPI boards) — review before rollout.', entry_type: 'action_item' },
  ...
  'pain-explorer': (label) => [
    ...
    step(
      'find_similar_decisions',
      { description: 'Prioritizing operational pain points for resolution', decision_type: 'process' },
...
const WAVE_DEFAULT_PLAYBOOKS: Record<string, (label: string) => PlanBuilderStep[]> = {
  'wave-1': (label) => [
    step('search_knowledge_base', { query: `${label}: relevant internal context` }, `${label}: gather internal context`),
    step('get_assessment_data', { include_benchmarks: true }, `${label}: pull maturity scores`),
    step('compare_benchmarks', { industry: 'general', metric_type: 'operational' }, `${label}: benchmark against peers`),
    step(
      'generate_report_section',
  ...
  reference: (label) => [
    step('search_knowledge_base', { query: `${label}: relevant internal context` }, `${label}: gather internal context`),
    step('compare_benchmarks', { industry: 'general', metric_type: 'maturity' }, `${label}: benchmark against peers`),
    step(
      'generate_report_section',
      { section_title: label, section_type: 'analysis' },
      `${label}: synthesize into an analysis section`
...
export function buildPlanFromManifest(
  ...
  const manifest: DiscoveryAgentManifestSummary | null =
    typeof manifestOrId === 'string' ? getDiscoveryAgentManifest(manifestOrId) : manifestOrId || null;
