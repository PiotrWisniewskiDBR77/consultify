...
suite(
  'GOLDEN CASE H — REQUEST_CHANGES, a revised proposal, a FAILED->retry restart, a PARTIAL result',
  () => {
    let control: Pool;
    ...
    it('walks request-changes -> revised proposal -> failed -> retry -> executed, with a PARTIAL result', async () => {
      ...
      try {
        ...
        const changesRequested = await asSponsor(
          'post',
          `${BASE}/proposals/${proposal1Id}/decision`
        )
        ...
        // mapping to []).
        const decisionAfterChanges = await asSponsor(
          'post',
          `${BASE}/proposals/${proposal1Id}/decision`
        )
        ...
        const failed = await asConsultant(
          'post',
          `${BASE}/proposals/${proposal2Id}/transition-to-failed`
        ).send({
        ...
        expect(caseRow.rows[0]).toMatchObject({
          case_status: 'CLOSED',
          closure_type: 'COMPLETED_PARTIAL',
        });
        ...
        expect(proposalsRow.rows[0]).toMatchObject({
          action_proposal_id: proposal1Id,
          status: 'REQUESTED_CHANGES',
        });
        expect(proposalsRow.rows[1]).toMatchObject({
          action_proposal_id: proposal2Id,
          status: 'AUDITED',
        });
        ...
        await control.query(
          `DELETE FROM case_workspace_node_result_acceptances WHERE case_id = $1`,
          [caseId]
        );
        await control.query(
