...
warnSkipped(
  'Golden Case E (TRANSFORMATION multi-module + wait satisfied by human input)',
  REACHABLE
);
...
suite(
  'GOLDEN CASE E — TRANSFORMATION: multiple modules feed one Case; a HUMAN wait is genuinely answered',
  () => {
    let control: Pool;
    ...
    it('links three distinct module artifacts to one Case and survives a real human-answered wait', async () => {
      ...
      try {
        ...
        expect(new Set(artifactTypes)).toEqual(
        ...
        await control.query(`DELETE FROM case_workspace_artifact_links WHERE case_id = $1`, [
          caseId,
        ]);
