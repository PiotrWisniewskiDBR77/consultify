...
suite(
  'GOLDEN CASE I — cross-tenant and revoked-membership access both fail closed, indistinguishably from 404',
  () => {
    let control: Pool;
    ...
    it('refuses a cross-tenant actor, a revoked member, and a non-member — all fail closed, no side effect', async () => {
      ...
      try {
        ...
        await asConsultantA('post', `${BASE}/cases/${caseId}/status`).send({
          targetStatus: 'ACTIVE',
        });
        ...
        const crossTenantMutation = await asOutsiderB(
          'post',
        ...
        const crossTenantPlanRead = await asOutsiderB(
          'get',
          `${BASE}/plan-versions/${draft.body.data.casePlanVersionId}/validate`
        );
        ...
        const crossTenantByProject = await asOutsiderB(
          'get',
          `${BASE}/cases/by-project/${projectA}`
        );
