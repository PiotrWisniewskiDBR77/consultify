...
suite(
  'eventOutboxService — transactional event outbox against a real PostgreSQL (doc 06 §6/§8)',
  () => {
    let control: Pool;
    ...
    it('dispatchPendingEvents marks a delivered row, leaves a failed row pending with a growing attempt count, and delivers it on a later pass (doc 06 §8 retry)', async () => {
      ...
      try {
        ...
        const scheduledRetryMs = new Date(
          afterFirstFailure!.next_retry_at as unknown as string
        ).getTime();
        ...
        expect(scheduledRetryMs).toBeGreaterThanOrEqual(
          beforeFailingPassMs + eventOutboxService.computeRetryBackoffMs(1)
        );
          ...
          withPgTransaction((client) =>
            eventOutboxService.publishEvent(
              ...
              envelope(orgId, caseId, {
                ...
                redactedSummary: {
                  document: 'x'.repeat(eventOutboxService.MAX_REDACTED_SUMMARY_BYTES + 1),
                },
              })
            )
          )
    ...
    it('countDeadLetterEvents agrees with listDeadLetterEvents, and both are empty before the threshold is crossed', async () => {
      ...
      try {
        ...
        for (
          let attempt = 0;
          attempt < eventOutboxService.DEAD_LETTER_ATTEMPT_THRESHOLD - 1;
          attempt += 1
        ) {
