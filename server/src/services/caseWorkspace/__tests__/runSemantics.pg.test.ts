...
suite(
  'runLifecycleService — Run semantics closure: join policies, SKIPPED audit, PARTIAL, restart, idempotency (D2)',
  () => {
    let control: Pool;
    ...
    function splitJoinGraph(joinNode: {
      joinPolicy?: string;
      joinRequiredCount?: number;
    }): CanonicalGraph {
      return {
        entryNodeIds: ['n1'],
        terminalNodeIds: ['end'],
    ...
    it('a PARALLEL_JOIN with joinPolicy=ANY resolves once the first of three branches succeeds', async () => {
      ...
      try {
        ...
        const evaluation = await executionGraphService.getGatewayEvaluation(
    ...
    it(
      ...
      async () => {
        ...
        try {
          ...
          expect(
            (
              await executionGraphService.listNodeResultAcceptancesForRun(
                created.runId,
                undefined,
                fx.actorId
              )
    ...
    it('a Run mid N_OF_M join survives a restart into a brand-new process and resumes to COMPLETED correctly', async () => {
      const fx = await seedPublishedCase(
        'restart-join',
        splitJoinGraph({ joinPolicy: 'N_OF_M', joinRequiredCount: 2 })
      );
