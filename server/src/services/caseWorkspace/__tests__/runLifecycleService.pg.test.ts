...
suite(
  'runLifecycleService — canonical Run lifecycle against a real PostgreSQL (doc 04 §3.4/§4.4/§5)',
  () => {
    let control: Pool;
    ...
    async function readRun(runId: string): Promise<RunDbRow | null> {
      const result = await control.query<RunDbRow>(
        `SELECT * FROM case_workspace_runs WHERE run_id = $1`,
        [runId]
      );
    ...
    it('createRun is idempotent on (caseId, idempotencyKey): a replay returns the SAME row and mints nothing new', async () => {
      ...
      try {
        ...
        const bindings = await control.query(
          `SELECT count(*)::int AS n FROM case_workspace_run_bindings WHERE case_id = $1`,
          [fx.caseId]
        );
        expect(Number(bindings.rows[0].n)).toBe(1);
        const v8Runs = await control.query(
          `SELECT count(*)::int AS n FROM v8_execution_runs WHERE run_id = $1`,
          [first.runId]
        );
    ...
    it('pauseRun/resumeRun are OCC-guarded: a stale expectedVersion is refused and leaves the row untouched', async () => {
      ...
      try {
        ...
        // silently re-applied.
        await expect(
          runLifecycleService.pauseRun(created.runId, fx.actorId, staleVersion)
        ).rejects.toThrow('run_lifecycle_version_conflict');
        ...
        const resumed = await runLifecycleService.resumeRun(
          created.runId,
          fx.actorId,
          paused.version
        );
    ...
    it('retryNode refuses a live node, and mints a fresh NodeRun once the node is FAILED_TERMINAL, resuming a BLOCKED Run', async () => {
      ...
      try {
        ...
        await expect(
          runLifecycleService.retryNode(created.runId, 'n1', fx.actorId)
        ).rejects.toThrow('run_lifecycle_node_not_retryable');
        ...
        const latest = await nodeRunService.getLatestNodeRunForNode(
          created.runId,
          'n1',
          fx.actorId
        );
    ...
    it('compensateRun marks REQUIRED nodes IN_PROGRESS, and recordNodeCompensationOutcome resolves the Run to COMPENSATED', async () => {
      ...
      try {
        ...
        const compensating = await runLifecycleService.compensateRun(
          created.runId,
          fx.actorId,
          started.run.version
        );
