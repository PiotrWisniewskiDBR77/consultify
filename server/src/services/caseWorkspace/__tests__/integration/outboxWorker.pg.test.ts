...
suite(
  'outboxWorker — the production outbox worker against a real PostgreSQL (CW-T-E, Problem 2)',
  () => {
    let control: Pool;
    ...
    it('a permanently-failing consumer drives a row to dead-letter after DEAD_LETTER_ATTEMPT_THRESHOLD attempts, and the worker stops claiming it', async () => {
      ...
      try {
        ...
        for (
          let attempt = 1;
          attempt <= eventOutboxService.DEAD_LETTER_ATTEMPT_THRESHOLD;
          attempt += 1
        ) {
    ...
    it('a worker that is fully stopped (simulating a crash/restart) is recovered by a freshly-started one, with no redelivery of what already went out', async () => {
      ...
      try {
        ...
        const firstEventId = await publish(orgId, caseId, {
          eventType: 'case.step_before_restart',
        });
        ...
        const secondEventId = await publish(orgId, caseId, {
          eventType: 'case.step_during_downtime',
        });
        ...
        try {
          ...
          await new Promise<void>((resolve, reject) => {
            const timer = setTimeout(
              () =>
                reject(
                  new Error(`process A never signalled ready. stderr: ${processAStderr.join('')}`)
                ),
              15_000
            );
            ...
            processA.on('exit', (code) => {
              ...
              reject(
                new Error(
                  `process A exited early (code ${code}). stderr: ${processAStderr.join('')}`
                )
              );
            });
          });
          ...
          try {
            await new Promise<void>((resolve, reject) => {
              const timer = setTimeout(
                () =>
                  reject(
                    new Error(`process B never signalled ready. stderr: ${processBStderr.join('')}`)
                  ),
                15_000
              );
              ...
              processB.on('exit', (code) => {
                ...
                reject(
                  new Error(
                    `process B exited early (code ${code}). stderr: ${processBStderr.join('')}`
                  )
                );
              });
            });
            ...
            const fullyDrained = await waitUntil(
              async () => (await readDeliveredCount()) === EVENT_COUNT,
              30_000
            );
    ...
    it(
      ...
      async () => {
        ...
        try {
          ...
          const pidResult = await stuckClient.query<{ pid: number }>(
    ...
    it('oldestPendingAgeSeconds is null when idle, positive while something ages, and null again once fully drained', async () => {
      ...
      try {
        ...
        expect(
          outboxWorker.getOutboxWorkerMetrics().lastTickResult?.oldestPendingAgeSeconds
        ).toBeNull();
      } finally {
        await teardown(orgId);
      }
