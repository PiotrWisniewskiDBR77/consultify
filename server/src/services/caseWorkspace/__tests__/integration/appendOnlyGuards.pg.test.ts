...
suite(
  'case_workspace append-only DB guards (R3-P1) — real trigger enforcement against real PostgreSQL',
  () => {
    let control: Pool;
    ...
    describe('case_workspace_event_outbox', () => {
      it('rejects UPDATE of a content column (event_type)', async () => {
        ...
        try {
          ...
          const err = await expectRejected(() =>
            control.query(
              `UPDATE case_workspace_event_outbox SET event_type = 'case.tampered' WHERE event_id = $1`,
              [eventId]
            )
          );
          ...
          const read = await control.query(
            `SELECT event_type FROM case_workspace_event_outbox WHERE event_id = $1`,
            [eventId]
          );
      ...
      it('rejects DELETE unconditionally', async () => {
        ...
        const read = await control.query(
          `SELECT event_id FROM case_workspace_event_outbox WHERE event_id = $1`,
          [eventId]
        );
    ...
    describe('case_workspace_event_inbox', () => {
      it('rejects UPDATE of a claim-identity column (event_id)', async () => {
        ...
        const err = await expectRejected(() =>
          control.query(
            `UPDATE case_workspace_event_inbox SET event_id = 'tampered' WHERE inbox_record_id = $1`,
            [inboxRecordId]
          )
        );
        expect(err.message).toMatch(/append-only/);
        ...
        const read = await control.query(
          `SELECT event_id FROM case_workspace_event_inbox WHERE inbox_record_id = $1`,
          [inboxRecordId]
        );
      ...
      it('allows UPDATE of ONLY processing-state columns (the exact shape reject()/apply()/recordInboxProcessingFailure issue)', async () => {
        ...
        await control.query(
          `UPDATE case_workspace_event_inbox SET status = 'RECEIVED', rejection_code = NULL WHERE inbox_record_id = $1`,
          [inboxRecordId]
        );
      ...
      it('rejects DELETE unconditionally', async () => {
        ...
        const err = await expectRejected(() =>
          control.query(`DELETE FROM case_workspace_event_inbox WHERE inbox_record_id = $1`, [
            inboxRecordId,
          ])
        );
        expect(err.message).toMatch(/append-only: DELETE is not permitted/);
        ...
        const read = await control.query(
          `SELECT inbox_record_id FROM case_workspace_event_inbox WHERE inbox_record_id = $1`,
          [inboxRecordId]
        );
        expect(read.rows).toHaveLength(1);
      });
    ...
    describe('case_workspace_history_events', () => {
      ...
      it('rejects ANY UPDATE', async () => {
        ...
        const err = await expectRejected(() =>
          control.query(
            `UPDATE case_workspace_history_events SET summary = 'tampered' WHERE event_id = $1`,
            [eventId]
          )
        );
        ...
        const read = await control.query(
          `SELECT summary FROM case_workspace_history_events WHERE event_id = $1`,
          [eventId]
        );
      ...
      it('rejects DELETE unconditionally', async () => {
        ...
        const read = await control.query(
          `SELECT event_id FROM case_workspace_history_events WHERE event_id = $1`,
          [eventId]
        );
        expect(read.rows).toHaveLength(1);
      });
    ...
    it('all three append-only triggers are present in information_schema', async () => {
      ...
      expect(result.rows).toEqual([
        {
          event_object_table: 'case_workspace_event_inbox',
          trigger_name: 'trg_case_workspace_event_inbox_guard',
        },
        {
          event_object_table: 'case_workspace_event_outbox',
          trigger_name: 'trg_case_workspace_event_outbox_guard',
        },
        {
          event_object_table: 'case_workspace_history_events',
          trigger_name: 'trg_case_workspace_history_events_block_mutation',
        },
      ]);
    });
  }
