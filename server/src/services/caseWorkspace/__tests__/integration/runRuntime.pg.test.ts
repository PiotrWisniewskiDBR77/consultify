...
suite(
  'runRuntime — canonical Run/NodeRun end to end against a real PostgreSQL (doc 04 §3.4/§4.4/§4.5/§7)',
  () => {
    let control: Pool;
    ...
    async function succeedAttempt(nodeRunId: string): Promise<void> {
      ...
      if (claim.outcome !== 'claimed')
        throw new Error(`unreachable — claim outcome was ${claim.outcome}`);
    ...
    // =========================================================================
    it('runs the full canonical pipeline to COMPLETED, resuming correctly after a readback from a BRAND NEW process', async () => {
    ...
    it('createRun refuses a SUPERSEDED plan version — a stale target mutates nothing', async () => {
      ...
      try {
        ...
        const v1After = await casePlanVersionService.getPlanVersion(
          fx.casePlanVersionId,
          fx.actorId
        );
