...
suite(
  'CW-T-B — authenticated event inbox ingress, over real HTTP (routes/caseWorkspace/eventInbox.routes.ts)',
  () => {
    let control: Pool;
    let app: Express;
    ...
    function buildBody(params: {
      ...
      return {
        ...
        signature:
          ...
          eventInboxService.computeInboxSignature(CHANNEL_SECRET, payload),
      };
    }
    ...
    it('wrong Case: correlationKey belongs to a DIFFERENT Case in the SAME org -> 422 TENANT_MISMATCH', async () => {
      ...
      try {
        ...
        if (otherCase) {
          await control
            .query(`DELETE FROM case_workspace_waits WHERE run_id = $1`, [otherCase.runId])
            .catch(() => undefined);
          await control
            .query(`DELETE FROM case_workspace_run_bindings WHERE run_id = $1`, [otherCase.runId])
            .catch(() => undefined);
          await control
            .query(`DELETE FROM v8_execution_runs WHERE run_id = $1`, [otherCase.runId])
            .catch(() => undefined);
          await control
            .query(`DELETE FROM case_core WHERE project_id = $1`, [otherCase.projectId])
            .catch(() => undefined);
          await control
            .query(`DELETE FROM projects WHERE id = $1`, [otherCase.projectId])
            .catch(() => undefined);
        }
        await teardown({ ...fixture, source });
      }
    ...
    it('ambiguous correlation key (two waits, same org, no caseId to disambiguate): 422 CORRELATION_AMBIGUOUS', async () => {
      ...
      try {
        ...
        const waitB = await seedWaitWithKey(
          { ...fixture, caseId: second.caseId, runId: second.runId },
          sharedKey
        );
        registerChannel(source);
    ...
    it('replay: an identical redelivery is 200 duplicate and produces NO second effect (sequential)', async () => {
      ...
      try {
        ...
        expect(replay.body).toMatchObject({
          received: true,
          outcome: 'duplicate',
          inboxRecordId: first.body.inboxRecordId,
        });
    ...
    it('malformed request body: 400 VALIDATION_ERROR, never 500', async () => {
      const res = await request(app)
        .post(`${BASE}/some-source/deliveries`)
        .send({ nonsense: true });
