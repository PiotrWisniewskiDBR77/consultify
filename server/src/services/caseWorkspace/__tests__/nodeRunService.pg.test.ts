...
suite(
  'nodeRunService — canonical NodeRun against a real PostgreSQL (doc 04 §3.4/§4.5, doc 06 §8)',
  () => {
    let control: Pool;
    ...
    it('rejects a write carrying a stale version and leaves the row byte-identical', async () => {
      ...
      try {
        ...
        const moved = await nodeRunService.transitionNodeRun(
          created.nodeRunId,
          'READY',
          staleVersion
        );
    ...
    it('refuses an attempt start and an attempt completion from a worker whose lease was superseded', async () => {
      ...
      try {
        ...
        const zombieClaim = await nodeRunService.claimNodeRun(created.nodeRunId, {
          leaseMs: 60_000,
        });
    ...
    it('consults the reconciliation check before reclaiming an expired lease, and closes the node from the readback instead of retrying when the effect already landed', async () => {
      ...
      try {
        ...
        let consulted = 0;
        const outcome = await nodeRunService.reclaimExpiredNodeRunLease(created.nodeRunId, () => {
          consulted += 1;
    ...
    it('getLatestNodeRunForNode returns null for a node that never ran, and the NEWEST row once a node has several (retry) NodeRuns', async () => {
      ...
      try {
        const neverRan = await nodeRunService.getLatestNodeRunForNode(
          fixture.runId,
          'n1',
          fixture.actorId
        );
        ...
        const otherNode = await nodeRunService.getLatestNodeRunForNode(
          fixture.runId,
          'n2',
          fixture.actorId
        );
