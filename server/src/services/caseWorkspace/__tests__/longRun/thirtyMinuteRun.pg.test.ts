...
const NODE_RUN_SERVICE_ABS = path.resolve(
  fileURLToPath(new URL('../../nodeRunService.ts', import.meta.url))
);
...
async function spawnDriver(
  ...
  const pid = await new Promise<number>((resolve, reject) => {
    ...
    child.on('exit', (code) => {
      ...
      reject(
        new Error(
          `driver exited early (code ${code}). stdout=${stdout.join('')} stderr=${stderr.join('')}`
        )
      );
    });
  });
...
suite(
  'E4 — a genuine >=30 minute Case Workspace Run: crash/restart, retry, waits, outbox resilience',
  () => {
    let control: Pool;
    ...
    async function teardown(params: { orgId: string; projectId: string }): Promise<void> {
      const q = (sql: string, values: unknown[]) =>
        control.query(sql, values).catch(() => undefined);
      await q(
    ...
    it(
      ...
      async () => {
        ...
        try {
          ...
          const run = await runLifecycleService.createRun(
            {
              ...
              idempotencyKey: `e4-run-${randomUUID()}`,
            },
            fx.actorId
          );
          ...
          logLine(
            logFile,
            `n1 SUCCEEDED, advanceRun -> ${afterN1.run.status}, created ${afterN1.createdNodeRunIds}`
          );
          ...
          expect(n2First.maxAttempts).toBe(1);
          {
            ...
            if (claim.outcome !== 'claimed')
              throw new Error('unreachable: n2 (first) not claimable');
            const attempt = await nodeRunService.startNodeRunAttempt(
              n2First.nodeRunId,
              { leaseOwner: claim.leaseOwner, fencingToken: claim.fencingToken, timeoutMs: 60_000 },
              claim.nodeRun.version
          ...
          logLine(
            logFile,
            `retryNode -> new NodeRun n2RetriedId=${n2RetriedId} maxAttempts=3, Run -> RUNNING`
          );
          ...
          let n2Version: number;
          {
            ...
            if (claim.outcome !== 'claimed')
              throw new Error('unreachable: n2 (retried) not claimable');
            const attempt = await nodeRunService.startNodeRunAttempt(
              n2RetriedId,
              { leaseOwner: claim.leaseOwner, fencingToken: claim.fencingToken, timeoutMs: 60_000 },
              claim.nodeRun.version
          ...
          logLine(
            logFile,
            `n2 (retried) attempt#1 FAILED_RETRYABLE -> RETRY_SCHEDULED, real backoff ${RETRY_DELAY_MS}ms`
          );
          ...
          logLine(
            logFile,
            `Process A (pid=${processA.pid}) SIGKILLed at ${new Date(killWallClock).toISOString()}`
          );
          ...
          logLine(
            logFile,
            `advanceRun after recovery -> ${afterN2Recovered.run.status}, created n3`
          );
          ...
          while (
            Date.now() < probeLogDeadline &&
            !(backoffProbeDelivered && deadLetterProbeDeadLettered)
          ) {
            ...
            for (const row of rows.rows) {
              ...
              if (row.event_type === 'longrun.backoff.probe' && row.delivered_at !== null)
                backoffProbeDelivered = true;
              if (
          ...
          // both probes settled).
          await pollUntil(
            async () => Date.now() >= Date.parse(wait.dueAt as string),
            WAIT_DUE_MS + 60_000,
            2_000
          );

          const waitClaim = await waitSubscriptionService.claimTimerWait(wait.waitId, {
            leaseMs: 30_000,
          });
          ...
          const resolved = await waitSubscriptionService.resolveWait(
            wait.waitId,
            {
              ...
              timerClaim: {
                ownerToken: waitClaim.ownerToken,
                fencingToken: waitClaim.fencingToken,
              },
            },
          ...
          const runDurationMs =
            Date.parse(finalRun.completedAt as string) - Date.parse(finalRun.startedAt as string);
          logLine(
            logFile,
          ...
          const deadLetterProbeFinal =
            await eventOutboxService.getOutboxEvent(deadLetterProbeEventId);
          expect(deadLetterProbeFinal?.deliveredAt).toBeNull();
          expect(deadLetterProbeFinal?.deliveryAttemptCount).toBeGreaterThanOrEqual(
            DEAD_LETTER_ATTEMPT_THRESHOLD
          );

          const deadLetterList = await eventOutboxService.listDeadLetterEvents({
            organizationId: fx.orgId,
          });
          expect(deadLetterList.map((e) => e.eventId)).toContain(deadLetterProbeEventId);

          const reconciliation = await outboxWorker.runOutboxReconciliationSweep({
            organizationId: fx.orgId,
          });
