...
suite(
  'decisionAdapter — Decision capability, dispatched end-to-end through executeCapability',
  () => {
    let control: Pool;
    ...
    beforeAll(async () => {
      ...
      await capabilityAdapterService.registerCapabilityWithAdapter(
        {
          ...decisionCreateRegistrationInput(registrarActorId),
    ...
    async function decisionCountForOrg(orgId: string): Promise<number> {
      const result = await control.query(
        `SELECT count(*)::int AS n FROM decisions WHERE organization_id = $1`,
        [orgId]
      );
      ...
      try {
        ...
        const second = await capabilityAdapterService.executeCapability(
          buildEnvelope({
            ...
            idempotencyKey: key3,
            payload: validPayload(fixture.caseId, fixture.actorId, {
              title: 'A second, distinct decision',
            }),
          })
        );
