...
describeDb(
  'FULL LIFECYCLE — criterion -> evidence -> finding -> action -> candidate -> closure -> effectiveness (real Postgres)',
  () => {
    ...
    it('runs the full chain and leaves a SELECT-able row at every stage', async () => {
      ...
      const reviewedFinding = await findingService.reviewFinding(
        orgId,
        actorFor(orgId, reviewer),
        finding.id,
        {
          decision: 'confirm',
        }
      ...
      // ------------------------------------------------------- 4. corrective action
      const action = await correctiveActionService.proposeAction(
        orgId,
        actorFor(orgId, auditee),
        finding.id,
        {
      ...
      const approvedAction = await correctiveActionService.approveAction(
        orgId,
        actorFor(orgId, lead),
        action.id
      );
      expect(approvedAction.status).toBe('approved');
      await correctiveActionService.reportImplementation(
        orgId,
        actorFor(orgId, auditee),
        action.id,
        {
      ...
      const verification = await verificationService.performVerification(
        orgId,
        actorFor(orgId, lead),
        planned.id,
        {
      ...
      const closedFinding = await findingService.closeFinding(
        orgId,
        actorFor(orgId, reviewer),
        finding.id,
        {
      ...
      await programService.transitionLifecycle(
        orgId,
        actorFor(orgId, lead),
        programId,
        'evidence_review'
      ...
      const closedProgram = await programService.transitionLifecycle(
        orgId,
        actorFor(orgId, lead),
        programId,
        'closure'
      ...
      const fullyClosed = await programService.transitionLifecycle(
        orgId,
        actorFor(orgId, lead),
        programId,
        'closed'
      ...
      const [
        ...
        pool.query(
          `SELECT id, conformity_status FROM audit_program_criteria WHERE id=$1 AND organization_id=$2`,
          [criterionId, orgId]
        ),
        pool.query(`SELECT id, accepted FROM audit_evidence WHERE id=$1 AND organization_id=$2`, [
          evidenceId,
          orgId,
        ]),
        pool.query(
          `SELECT id, status FROM audit_program_findings WHERE id=$1 AND organization_id=$2`,
          [finding.id, orgId]
        ),
        pool.query(
          `SELECT id, status FROM audit_corrective_actions WHERE id=$1 AND organization_id=$2`,
          [action.id, orgId]
        ),
        pool.query(
          `SELECT id, result, verification_kind FROM audit_verifications WHERE id=$1 AND organization_id=$2`,
          [verification.id, orgId]
        ),
        pool.query(
          `SELECT id, status, registered_initiative_id FROM audit_initiative_proposals WHERE id=$1 AND organization_id=$2`,
          [proposal.id, orgId]
        ),
        pool.query(
          `SELECT id, source_type, source_id FROM initiatives WHERE id=$1 AND organization_id=$2`,
          [ids.initiativeId, orgId]
        ),
        pool.query(
          `SELECT id, lifecycle_state FROM audit_programs WHERE id=$1 AND organization_id=$2`,
          [programId, orgId]
        ),
      ]);
    ...
    it('COLD READBACK — after a fresh pool and fresh service imports, every lifecycle row is still present with the same ids', async () => {
      ...
      try {
        ...
        const byStage = Object.fromEntries(
          rows.rows.map((r: { stage: string; id: string }) => [r.stage, r.id])
        );
