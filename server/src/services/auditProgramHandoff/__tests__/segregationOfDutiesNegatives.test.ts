...
describeDb('segregation of duties — independent negative controls (real Postgres)', () => {
  ...
  it('1. the SAME actor cannot conclude a criterion they answered as auditee', async () => {
    ...
    await criterionService.recordTest(
      organizationId,
    ...
    await expect(
      criterionService.concludeCriterion(
        organizationId,
        actorFor(organizationId, userId),
        criterionId,
        {
  ...
  it('2. the SAME actor (owner of the finding) cannot close their own finding', async () => {
    ...
    const finding = await findingService.createFinding(
      organizationId,
      actorFor(organizationId, authorId),
      {
        programId,
        criterionId,
    ...
    await findingService.reviewFinding(
      organizationId,
      actorFor(organizationId, thirdPartyId),
      finding.id,
      {
        decision: 'confirm',
      }
    ...
    const closed = await findingService.closeFinding(
      organizationId,
      actorFor(organizationId, thirdPartyId),
      finding.id,
      {
  ...
  it('3. the SAME actor (author of the finding) cannot review their own finding', async () => {
    ...
    const reviewed = await findingService.reviewFinding(
      organizationId,
      actorFor(organizationId, reviewerId),
      finding.id,
      {
        decision: 'confirm',
      }
  ...
  it('4. the SAME actor (owner/implementer of a corrective action) cannot verify its own effectiveness', async () => {
    ...
    const finding = await findingService.createFinding(
      organizationId,
      actorFor(organizationId, leadId),
      {
        programId,
        criterionId,
    ...
    const action = await correctiveActionService.proposeAction(
      organizationId,
      actorFor(organizationId, ownerId),
      finding.id,
      {
    ...
    await correctiveActionService.approveAction(
      organizationId,
      actorFor(organizationId, leadId),
      action.id
    );
    ...
    const planned = await verificationService.planVerification(
      organizationId,
      actorFor(organizationId, leadId),
      {
    ...
    await expect(
      verificationService.performVerification(
        organizationId,
        actorFor(organizationId, ownerId),
        planned.id,
        {
    ...
    const verified = await verificationService.performVerification(
      organizationId,
      actorFor(organizationId, leadId),
      planned.id,
      {
