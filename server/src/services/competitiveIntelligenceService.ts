...
const CompetitiveIntelligenceService = {
  ...
  initialize: async (): Promise<void> => {
    ...
    await DbPromise.run(
      'CREATE INDEX IF NOT EXISTS idx_mt_industry ON market_trends(industry)',
      [],
      {
        fallback: false,
      }
    ...
    await DbPromise.run(
      'CREATE INDEX IF NOT EXISTS idx_mt_category ON market_trends(category)',
      [],
      {
        fallback: false,
      }
