...
describe.skipIf(!REAL_PG)(
  'INT-BVP-001 — interview candidate exactly-once (real PostgreSQL)',
  () => {
    ...
    afterAll(async () => {
      ...
      try {
        ...
        await c.query(
          `DELETE FROM interview_candidate_handoffs WHERE organization_id IN ($1, $2)`,
          [ORG_A, ORG_B]
        );
    ...
    it('cold readback: a fresh connection still sees the same candidate and receipt ids', async () => {
      ...
      const c = await db();
      try {
        const candidateRow = await c.query(
          `SELECT id, status FROM initiative_candidates WHERE id = $1`,
...
describe.skipIf(!REAL_PG)(
  'INT-BVP-001 — migration applies safely on pre-existing duplicates',
  () => {
