...
class BillingCron {
  ...
  private async ensureDeps(): Promise<Dependencies> {
    if (!this.deps.budgetManagementService) {
      this.deps.budgetManagementService =
        await import('../../services/budgetManagementService.js').then((m) => m.default || m);
    }
    ...
    if (!this.deps.seatManagementService) {
      this.deps.seatManagementService =
        await import('../../services/seatManagementService.js').then((m) => m.default || m);
    }
      ...
      for (const org of orgs) {
        try {
          ...
          if (result.triggeredCount > 0) {
            ...
            logger.info(
              `[BillingCron] Triggered ${result.triggeredCount} alerts for org ${org.id}`
            );
          }
        } catch (err: any) {
    ...
    try {
      ...
      for (const org of orgs) {
        try {
          ...
          logger.error(
            `[BillingCron] Error generating invoice for org ${org.organization_id}:`,
            err
          );
        }
      }
      ...
      logger.info(
        `[BillingCron] Invoice generation completed. Generated ${invoicesGenerated} invoices`
      );
  ...
  async updateSeatCounts(): Promise<number> {
    ...
    try {
      ...
      const orgs = await deps.db.all<Organization>(
        'SELECT id FROM organizations WHERE status = ?',
        ['active']
      );
