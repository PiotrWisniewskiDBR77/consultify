...
export const info = (
  event: string,
  params: Omit<AuditLogParams, 'event' | 'level'> = {}
): AuditLogEntry => log({ ...params, level: LOG_LEVELS.INFO, event });

export const warn = (
  event: string,
  params: Omit<AuditLogParams, 'event' | 'level'> = {}
): AuditLogEntry => log({ ...params, level: LOG_LEVELS.WARN, event });

export const error = (
  event: string,
  params: Omit<AuditLogParams, 'event' | 'level'> = {}
): AuditLogEntry => log({ ...params, level: LOG_LEVELS.ERROR, event });

export const debug = (
  event: string,
  params: Omit<AuditLogParams, 'event' | 'level'> = {}
): AuditLogEntry => log({ ...params, level: LOG_LEVELS.DEBUG, event });
