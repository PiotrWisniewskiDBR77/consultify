...
export class DecisionController {
     ...
     */
    static createDecision = asyncHandler(async (req: AuthenticatedRequest<CreateDecisionRequest>, res: Response): Promise<void> => {
        ...
        const {
            projectId, title, description, pmoDomain,
            decisionOwnerId, relatedObjectType, relatedObjectId,
            dueDate, priority
        } = req.body;
        ...
        await queryHelpers.queryRun(sql, [
            id, projectId, title, description || null, pmoDomain,
            decisionOwnerId || userId, relatedObjectType || null, relatedObjectId || null,
            dueDate || null, priority || 'medium', 'pending', auditTrail
        ]);
    ...
    static decide = asyncHandler(async (req: AuthenticatedRequest<DecideRequest>, res: Response): Promise<void> => {
        ...
        if (currentDecision.decision_owner_id !== userId && req.user?.role !== 'ADMIN' && req.user?.role !== 'SUPERADMIN') {
     ...
     * Escalate decision
     */
    static escalateDecision = asyncHandler(async (req: AuthenticatedRequest<EscalateDecisionRequest>, res: Response): Promise<void> => {
        const { id } = req.params;
