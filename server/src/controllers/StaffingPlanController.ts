...
export class StaffingPlanController {
  ...
  static getPlan = asyncHandler(
    async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      ...
      if (!result) { res.status(404).json({ error: 'Staffing plan not found' }); return; }
  ...
  static updatePlan = asyncHandler(
    async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      ...
      const { name, status, plannedStart, plannedEnd, notes } = req.body;
      await staffingPlanService.updatePlan(planId, orgId, { name, status, plannedStart, plannedEnd, notes });
  ...
  static addRole = asyncHandler(
    async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      const orgId = req.user?.organizationId;
      const { planId } = req.params as any;
      if (!orgId) { res.status(401).json({ error: 'Unauthorized' }); return; }

      const { roleName, requiredSkills, fteRequired, assignedUserId, startDate, endDate, priority } = req.body;
      ...
      const role = await staffingPlanService.addRole(planId, {
        roleName: roleName.trim(), requiredSkills, fteRequired, assignedUserId, startDate, endDate, priority,
      });
  ...
  static updateRole = asyncHandler(
    async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      ...
      const { roleName, requiredSkills, fteRequired, fteAllocated, assignedUserId, startDate, endDate, priority, status } = req.body;
      await staffingPlanService.updateRole(roleId, planId, {
        roleName, requiredSkills, fteRequired, fteAllocated, assignedUserId, startDate, endDate, priority, status,
      });
      res.json({ success: true });
    }
  );
