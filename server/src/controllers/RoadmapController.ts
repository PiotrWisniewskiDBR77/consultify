...
export class RoadmapController {
     ...
     */
    static getWaves = asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
     ...
     */
    static createWave = asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
        const { projectId } = req.params;
        ...
        await queryHelpers.queryRun(sql, [
            id, orgId, projectId, name, description || null, startDate || null, endDate || null, sequenceOrder || 0
        ]);
    ...
    static getSummary = asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
        ...
        const [wavesCount, totalInitiatives] = await Promise.all([
            queryHelpers.queryOne<{count: number}>(`SELECT COUNT(*) as count FROM roadmap_waves WHERE project_id = ?`, [projectId]),
            queryHelpers.queryOne<{count: number}>(`SELECT COUNT(*) as count FROM initiatives WHERE project_id = ?`, [projectId])
        ]);
