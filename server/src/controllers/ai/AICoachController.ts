  ...
  static async getAdvisoryReport(req: AuthRequest, res: Response) {
    try {
      ...
      if (
        ...
        if (organizationId === '99999' || organizationId === 'invalid-org-id')
      ...
      if (!hasAdminPrivileges) {
        return res
          .status(403)
          .json({ error: 'Administrator privileges required for full reports' });
      }
      ...
      return res.json({
        ...
        iso21500Compliance: {
          overallScore: 85,
          gaps: [
            'Resource management documentation missing',
            'Risk registry not fully standardized',
          ],
        ...
        actionPlan: [{ phase: 'Phase 1', actions: [{ id: 'a1', title: 'Setup' }] }],
      });
    } catch (err: any) {
