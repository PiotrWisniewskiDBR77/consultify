...
export class LLMController {
    ...
    static async createProvider(req: Request, res: Response) {
        try {
            const { 
                name, provider, model_id, api_key, endpoint, 
                tier = 'standard', visibility = 'admin', 
                is_active = true, is_default = false,
                cost_per_1k = 0, context_window = 4096
            } = req.body;
            ...
            `, [id, name, provider, model_id, api_key, endpoint, tier, visibility, is_active ? 1 : 0, is_default ? 1 : 0, cost_per_1k, context_window]);
    ...
    static async updateProvider(req: Request, res: Response) {
        try {
            ...
            const allowedFields = ['name', 'provider', 'model_id', 'api_key', 'endpoint', 'tier', 'visibility', 'is_active', 'is_default', 'cost_per_1k', 'context_window'];
    ...
    static async getHealthStatus(req: Request, res: Response) {
        try {
            const providers = await dbAll('SELECT id, name, provider, model_id, is_active, visibility, tier FROM llm_providers WHERE is_active = 1', []);
    ...
    static async getDetailedHealth(req: Request, res: Response) {
        try {
            ...
            const providerHealthResults = await Promise.all(allProviders.map(async (provider: any) => {
    ...
    static async testProviderHealth(req: Request, res: Response) {
        try {
            ...
            const provider = await dbGet('SELECT * FROM llm_providers WHERE id = ?', [providerId]) as any;
            if (!provider) {
    ...
    static async testCapability(req: Request, res: Response) {
        try {
            ...
            // Get active provider
            const provider = await dbGet('SELECT * FROM llm_providers WHERE is_active = 1 ORDER BY is_default DESC LIMIT 1', []) as any;
            ...
            switch (capabilityId) {
                ...
                case 'eyes':
                    ...
                    testResult.details = { skipped: true, reason: 'Visual context requires multimodal model' };
                    break;
            ...
            `, [uuidv4(), (req as any).userId || 'system', (req as any).organizationId || 'system', provider.provider, provider.model_id, `health_test_${capabilityId}`, testResult.success ? 'success' : 'error', Date.now() - startTime]);
    ...
    static async getLogs(req: Request, res: Response) {
        try {
            ...
            const countRow = await dbGet('SELECT COUNT(*) as total FROM ai_usage_logs', []) as { total: number };
    ...
    static async getProvidersHealth(req: Request, res: Response) {
        try {
            const providers = await dbAll('SELECT * FROM llm_providers WHERE is_active = 1', []) as any[];
            
            const healthResults = await Promise.all(providers.map(async (provider: any) => {
                try {
                    const result = await llmService.testConnection({
                        provider: provider.provider,
                        apiKey: provider.api_key,
    ...
    static async getCosts(req: Request, res: Response) {
        try {
            const providers = await dbAll('SELECT id, name, provider, cost_per_1k FROM llm_providers', []) as any[];
    ...
    static async updateProviderTier(req: Request, res: Response) {
        try {
            ...
            await dbRun('UPDATE llm_providers SET tier = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [tier.toLowerCase(), id]);
