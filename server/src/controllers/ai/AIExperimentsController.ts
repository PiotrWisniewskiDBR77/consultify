...
export class AIExperimentsController {
    ...
    static async getExperiments(req: AuthRequest, res: Response) {
        try {
            ...
            const experimentsWithVariants = await Promise.all(experiments.map(async (exp: any) => {
                const variants = await all('SELECT * FROM ai_experiment_variants WHERE experiment_id = ?', [exp.id]);
                return {
                    ...exp,
        ...
        try {
            ...
            await run(
                `INSERT INTO ai_experiments (id, organization_id, name, description, type, target_capability, status, created_by)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [experimentId, req.organizationId, name, description, type || 'prompt_variation', targetCapability, 'draft', req.userId]
            );
            ...
            for (const variant of variants) {
                await run(
                    `INSERT INTO ai_experiment_variants (id, experiment_id, name, weight, config)
                     VALUES (?, ?, ?, ?, ?)`,
                    [uuidv4(), experimentId, variant.name, variant.weight, JSON.stringify(variant.config || {})]
                );
            }
    ...
    static async getExperimentById(req: AuthRequest, res: Response) {
        try {
            const experiment = await get('SELECT * FROM ai_experiments WHERE id = ? AND organization_id = ?', [req.params.id, req.organizationId]);
            
            if (!experiment) {
                return res.status(404).json({ error: 'Experiment not found' });
            }

            const variants = await all('SELECT * FROM ai_experiment_variants WHERE experiment_id = ?', [experiment.id]);
