...
export class ConnectionPool extends EventEmitter {
  ...
  private async createConnectionWithRetry(): Promise<IDatabase> {
    ...
    for (let attempt = 0; attempt < this.config.maxRetries; attempt++) {
      try {
        ...
        if (attempt < this.config.maxRetries - 1) {
          ...
          logger.warn(
            `[ConnectionPool] Connection attempt ${attempt + 1} failed, retrying in ${delay}ms...`
          );
    ...
    throw new Error(
      `Failed to create connection after ${this.config.maxRetries} attempts: ${lastError}`
    );
  }
  ...
  private async performHealthChecks(): Promise<void> {
    ...
    const checks = Array.from(this.connections.values()).map((conn) =>
      this.checkConnectionHealth(conn)
    );
