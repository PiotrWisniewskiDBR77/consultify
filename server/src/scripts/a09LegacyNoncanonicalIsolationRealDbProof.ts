...
async function main() {
  ...
  await pool.query(
    `CREATE TABLE v8_agent_run_identities (canonical_run_id TEXT NOT NULL, organization_id TEXT NOT NULL, PRIMARY KEY (canonical_run_id, organization_id));`
  );
  await pool.query(`INSERT INTO v8_agent_run_identities VALUES ('run-t01','org-t01')`);
  const { assertLegacyNoncanonicalExecution: guard } =
    await import('../ai/legacyNoncanonicalExecution.js');
  ...
  await assert.rejects(
    () =>
      guard({
        entrypoint: 'ai_playbook_executor',
        organizationId: 'org-t01',
        entityId: 'run-t01',
        payloads: [],
      }),
    /canonical_identity_forbidden/
  );
  await assert.rejects(
    () =>
      guard({
        ...
        organizationId: 'org-t01',
        entityId: 'decision',
        payloads: [{ context: { canonical_run_id: 'run-t01' } }],
      }),
    /canonical_identity_forbidden/
  );
  await assert.rejects(
    () =>
      guard({
        ...
        organizationId: 'org-t01',
        payloads: [{ branches: [{ Canonical_Run_Ref: 'run-t01' }] }],
      }),
    /canonical_identity_forbidden/
  );
  await assert.rejects(
    () =>
      guard({
        entrypoint: 'ai_playbook_executor',
        organizationId: 'org-t01',
        payloads: [{ Run_ID: 'run-t01' }],
      }),
    /canonical_identity_forbidden/
  );
  const foreignTenant = await guard({
    entrypoint: 'async_job_processor',
  ...
  console.log(
    JSON.stringify({
      ...
      alternateCaseAliasBlocked: true,
      tenantScoped: true,
    })
  );
}
