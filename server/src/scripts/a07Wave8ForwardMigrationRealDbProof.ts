...
const historical = fs.readFileSync(
  new URL('../../migrations/20260425_wave8_agent_runtime.sql', import.meta.url),
  'utf8'
);
const forward = fs.readFileSync(
  new URL('../../migrations/20260809_v8_wave8_agent_runtime_forward.sql', import.meta.url),
  'utf8'
);
...
async function inspect(schema: string) {
  const columns = await pool.query(
    `SELECT column_name,column_default,is_nullable FROM information_schema.columns
      WHERE table_schema=$1 AND table_name='wave8_agent_schedules'`,
    [schema]
  );
  ...
  const indexes = await pool.query(
    `SELECT indexname FROM pg_indexes WHERE schemaname=$1 AND indexname IN
      ('idx_wave8_agent_schedules_due','idx_wave8_tool_governance_run') ORDER BY indexname`,
    [schema]
  );
  ...
  const constraints = await pool.query(
    `SELECT COUNT(*)::int count FROM pg_constraint c JOIN pg_class t ON t.oid=c.conrelid
     ...
     WHERE n.nspname=$1 AND t.relname='wave8_agent_tool_governance_events'
       AND c.conname='wave8_agent_tool_governance_events_decision_check'`,
    [schema]
  );
...
async function main() {
  ...
  assert.deepEqual(fresh.rows[0], {
    scheduler_mode: 'durable_cron_worker',
    goal: '',
    timezone: 'UTC',
    attempt_count: 0,
    mandate_version: 1,
...
main().finally(async () => {
  ...
  await pool
    .query(
      'DROP SCHEMA IF EXISTS a07_forward_existing CASCADE; DROP SCHEMA IF EXISTS a07_forward_fresh CASCADE'
    )
    .catch(() => undefined);
  await pool.end();
});
