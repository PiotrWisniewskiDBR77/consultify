...
async function main(): Promise<void> {
  ...
  await pool.query(
    ...
    VALUES ('case-a11','org-a11',$1,'owner-a11','Diagnose transformation run','plan_proposed','mandate','lineage-a11','case-a11-idem',1)`,
    [runId]
  );
  await pool.query(
    `INSERT INTO v8_agent_run_identities(canonical_run_id,organization_id,transformation_case_id,lineage_id) VALUES ($1,'org-a11','case-a11','lineage-a11')`,
    [runId]
  );
  await pool.query(
  ...
  const expireInput = {
    ...
    targetId: staleRunId,
    action: 'expire_stale_review' as const,
    reason: 'Review deadline elapsed without a governance decision.',
    idempotencyKey: 'a11-expire-stale-review',
    now: '2026-08-07T10:00:00.000Z',
  };
  const [expired, expiredConcurrentReplay] = await Promise.all([
    operator.recoverAgentRunTarget(expireInput),
    operator.recoverAgentRunTarget(expireInput),
  ]);
  assert.equal(expired.recoveryId, expiredConcurrentReplay.recoveryId);
  assert.equal(
    [expired, expiredConcurrentReplay].filter((item) => item.idempotentReplay).length,
    1
  );
  ...
  const staleState = (
    ...
    (SELECT status FROM v8_action_proposals WHERE proposal_id='proposal-stale-a11') proposal_status`,
      [staleRunId]
    )
  ).rows[0];
  ...
  const replay = await operator.recoverAgentRunTarget(expireInput);
  assert.equal(replay.recoveryId, expired.recoveryId);
  assert.equal(replay.idempotentReplay, true);
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({ ...expireInput, reason: 'A materially different reason.' }),
    /recovery_idempotency_payload_conflict/
  );
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...expireInput,
        organizationId: 'org-foreign',
        idempotencyKey: 'a11-expire-foreign-tenant',
      }),
  ...
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...expireInput,
        executionRunId: '00000000-0000-4000-8000-000000009999',
        targetId: '00000000-0000-4000-8000-000000009999',
        idempotencyKey: 'a11-expire-foreign-run',
      }),
    /operator_target_not_found/
  );
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...expireInput,
        executionRunId: futureRunId,
        targetId: futureRunId,
        idempotencyKey: 'a11-expire-future',
      }),
    /stale_review_expiry_not_allowed/
  );
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...expireInput,
        executionRunId: wrongStateRunId,
        targetId: wrongStateRunId,
        idempotencyKey: 'a11-expire-wrong-state',
      }),
    /stale_review_expiry_not_allowed/
  );
  await pool.query(
  ...
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...expireInput,
        executionRunId: receiptRollbackRunId,
        targetId: receiptRollbackRunId,
        idempotencyKey: 'a11-expire-receipt-failure',
      }),
    /forced_expiry_receipt_failure/
  );
  assert.equal(
    (
      await pool.query(`SELECT state FROM v8_execution_runs WHERE run_id=$1`, [
        receiptRollbackRunId,
      ])
    ).rows[0].state,
    'waiting_for_review'
  );
  await pool.query(
    `DROP TRIGGER fail_a11_expiry_receipt_trigger ON v8_agent_operator_recovery_events; DROP FUNCTION fail_a11_expiry_receipt()`
  );
  await pool.query(
  ...
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...expireInput,
        executionRunId: transitionRollbackRunId,
        targetId: transitionRollbackRunId,
        idempotencyKey: 'a11-expire-transition-failure',
      }),
    /forced_expiry_transition_failure/
  );
  assert.equal(
    (
      await pool.query(`SELECT state FROM v8_execution_runs WHERE run_id=$1`, [
        transitionRollbackRunId,
      ])
    ).rows[0].state,
    'waiting_for_review'
  );
  await pool.query(
    `DROP TRIGGER fail_a11_expiry_transition_trigger ON v8_run_state_transitions; DROP FUNCTION fail_a11_expiry_transition()`
  );
  ...
  const retried = await operator.recoverAgentRunTarget({
    ...
    reason: 'Retry bounded transient failure.',
  ...
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...
        executionRunId: runId,
        actorUserId: 'operator-a11',
        targetId: 'nonexpired-a11',
        action: 'recover_expired_lease',
        reason: 'Must remain live.',
        idempotencyKey: 'a11-nonexpired-key',
        now: '2026-08-07T10:00:00.000Z',
      }),
    /branch_lease_not_expired/
  );
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...
        targetId: 'maxed-a11',
        action: 'retry_failed_branch',
        reason: 'Must respect max attempts.',
        idempotencyKey: 'a11-maxed-key',
      }),
  ...
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...recoveryInput,
        organizationId: 'org-foreign',
        idempotencyKey: 'a11-foreign-tenant',
      }),
    /operator_target_not_found/
  );
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...recoveryInput,
        executionRunId: 'foreign-run',
        idempotencyKey: 'a11-foreign-run',
      }),
    /operator_target_not_found/
  );
  await pool.query(
  ...
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...
        executionRunId: runId,
        actorUserId: 'operator-a11',
        targetId: 'rollback-a11',
        action: 'retry_failed_branch',
        reason: 'Receipt must be atomic.',
        idempotencyKey: 'a11-force-receipt-failure',
      }),
    /forced_receipt_failure/
  );
  assert.equal(
    (await pool.query(`SELECT status FROM v8_agent_branch_tasks WHERE task_id='rollback-a11'`))
      .rows[0].status,
    'failed'
  );
  await pool.query(
    `DROP TRIGGER fail_a11_receipt_trigger ON v8_agent_operator_recovery_events; DROP FUNCTION fail_a11_receipt()`
  );
  await pool.query(
    `INSERT INTO v8_agent_work_graphs(graph_id,execution_run_id,organization_id,lead_agent_id,mode,status,budget_json,created_by) VALUES ('graph-rollback-a11',$1,'org-a11','lead','sequential','planned','{}','owner-a11')`,
    [runId]
  );
  await pool.query(
    `INSERT INTO v8_agent_branch_tasks(task_id,graph_id,organization_id,specialist_agent_id,title,objective,expected_output_schema_json,dependencies_json,tool_scope_json,budget_json,status) VALUES ('graph-rollback-task','graph-rollback-a11','org-a11','ops','Pending','Rollback','{}','[]','[]','{}','pending')`
  );
  await pool.query(
  ...
  await assert.rejects(
    () =>
      operator.recoverAgentRunTarget({
        ...
        executionRunId: runId,
        actorUserId: 'operator-a11',
        targetId: 'graph-rollback-a11',
        action: 'cancel_graph',
        reason: 'Prove outbox rollback.',
        idempotencyKey: 'a11-cancel-outbox-failure',
      }),
  ...
  assert.deepEqual(
    (
      await pool.query(
        `SELECT (SELECT status FROM v8_agent_work_graphs WHERE graph_id='graph-rollback-a11') graph_status,(SELECT status FROM v8_agent_branch_tasks WHERE task_id='graph-rollback-task') task_status`
      )
    ).rows[0],
    { graph_status: 'planned', task_status: 'pending' }
  );
  await pool.query(
    `DROP TRIGGER fail_a11_outbox_trigger ON v8_agent_canonical_projection_outbox; DROP FUNCTION fail_a11_outbox()`
  );
  const cancelInput = {
    organizationId: 'org-a11',
    executionRunId: runId,
    actorUserId: 'operator-a11',
    targetId: 'graph-a11',
    action: 'cancel_graph' as const,
    reason: 'Stop graph cooperatively.',
    idempotencyKey: 'a11-cancel-graph-key',
  };
  const [cancelled, cancelReplay] = await Promise.all([
    operator.recoverAgentRunTarget(cancelInput),
    operator.recoverAgentRunTarget(cancelInput),
  ]);
  assert.equal(cancelled.recoveryId, cancelReplay.recoveryId);
  assert.equal([cancelled, cancelReplay].filter((x) => x.idempotentReplay).length, 1);
  const cancellationState = (
    await pool.query(
      `SELECT (SELECT status FROM v8_agent_work_graphs WHERE graph_id='graph-a11') graph_status,(SELECT status FROM v8_agent_branch_tasks WHERE task_id='nonexpired-a11') running_status,(SELECT status FROM v8_agent_branch_tasks WHERE task_id='rollback-a11') pending_status,(SELECT COUNT(*)::int FROM v8_agent_canonical_projection_outbox WHERE external_id='graph-a11') outbox_count`
    )
  ).rows[0];
  assert.deepEqual(cancellationState, {
    graph_status: 'cancellation_requested',
    running_status: 'cancellation_requested',
    pending_status: 'cancelled',
    outbox_count: 1,
  });
  const projected = await operator.processCanonicalProjectionOutbox({
    workerId: 'projection-worker-a11',
  });
  assert.equal(projected.length, 1);
  assert.equal(projected[0].status, 'applied');
  const restartReplay = await operator.processCanonicalProjectionOutbox({
    workerId: 'projection-worker-restart',
  });
  assert.deepEqual(restartReplay, []);
  assert.equal(
    (
      await pool.query(
        `SELECT COUNT(*)::int count FROM v8_agent_run_aliases WHERE canonical_run_id=$1 AND alias_type='work_graph' AND external_id='graph-a11'`,
        [runId]
      )
    ).rows[0].count,
    1
  ...
  await assert.rejects(
    () =>
      operator.acknowledgeWorkGraphTaskCancellation({
        organizationId: 'org-a11',
        executionRunId: runId,
        graphId: 'graph-a11',
        taskId: 'nonexpired-a11',
        workerId: 'wrong-worker',
      }),
    /cancellation_ack_not_allowed/
  );
  await operator.acknowledgeWorkGraphTaskCancellation({
    organizationId: 'org-a11',
    executionRunId: runId,
    graphId: 'graph-a11',
    taskId: 'nonexpired-a11',
  ...
  assert.equal(
    (await pool.query(`SELECT status FROM v8_agent_work_graphs WHERE graph_id='graph-a11'`)).rows[0]
      .status,
    'cancelled'
  );
