...
describe('demo acceptance fixtures safety contract',()=>{
  ...
  it('contains no destructive SQL and every row has tenant-scoped readback',()=>{
    for(const item of buildFixturePlan(ctx)){ expect(item.sql).not.toMatch(/\b(delete|truncate|drop|alter)\b/i); expect(item.sql).toMatch(/ON CONFLICT/i); expect(item.verifySql).toMatch(/organization_id = \$2/); }
  });
