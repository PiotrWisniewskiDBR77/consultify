...
import {
  buildFixturePlan,
  FINANCE_ACCEPTANCE_FLAG_KEYS,
  stableTextId,
  stableUuid,
} from '../../../scripts/acceptance-fixtures/fixturePlan';
import { buildGoldenChildPlan } from '../../../scripts/acceptance-fixtures/goldenChildPlan';
import {
  assertOrganizationAllowlisted,
  assertTarget,
} from '../../../scripts/acceptance-fixtures/run';
...
describe('demo acceptance fixtures safety contract', () => {
  it('uses deterministic IDs and covers every requested domain', () => {
    ...
    expect(new Set(buildFixturePlan(ctx).map((x) => x.domain))).toEqual(
      new Set([
        'case',
        'shared',
        'kpi',
        ...
        'artifact',
        'ideas',
        ...FINANCE_ACCEPTANCE_FLAG_KEYS.map((key) => `finance-flag:${key}`),
      ])
    );
  });
  it('contains no destructive SQL and every row has tenant-scoped readback', () => {
    for (const item of [...buildFixturePlan(ctx), ...buildGoldenChildPlan(ctx)]) {
      ...
      expect(item.verifySql).toMatch(/organization_id\s*=\s*\$2/);
    }
  });
  it('covers required golden child surfaces', () => {
    expect(buildGoldenChildPlan(ctx).map((x) => x.domain)).toEqual(
  ...
  it('fails closed outside demo and requires a distinct write token', () => {
    expect(() =>
      assertTarget({ DATABASE_URL: 'postgres://x', RAILWAY_ENVIRONMENT_NAME: 'production' }, false)
    ).toThrow(/Production/);
    expect(() =>
      assertTarget({ DATABASE_URL: 'postgres://x', RAILWAY_ENVIRONMENT_NAME: 'demo' }, true)
  ...
  it('requires the exact organization ID without changing organization type', () => {
    expect(() =>
      assertOrganizationAllowlisted({ ACCEPTANCE_ORG_ALLOWLIST: 'org-a, org-b' }, 'org-b')
    ).not.toThrow();
    expect(() =>
      assertOrganizationAllowlisted({ ACCEPTANCE_ORG_ALLOWLIST: 'org-a, org-b' }, 'org')
    ).toThrow(/allowlist/i);
    expect(
      buildFixturePlan(ctx)
        .map((item) => item.sql)
        .join('\n')
  ...
  it('documents the dedicated acceptance owner and Piotr readback without exposing a password', () => {
    const source = readFileSync(
      resolve(process.cwd(), 'server/scripts/acceptance-fixtures/run.ts'),
      'utf8'
    );
