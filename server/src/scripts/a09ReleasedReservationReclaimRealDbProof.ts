...
const proofDb = {
  ...
  get(text: string, params: unknown[], cb: (error: Error | null, row: unknown) => void) {
    void pool.query(adaptQuery(text), params).then(
...
async function main() {
  ...
  await pool.query(
    `CREATE TABLE v8_agent_adapter_invocations (invocation_id TEXT PRIMARY KEY, canonical_run_id TEXT NOT NULL, organization_id TEXT NOT NULL, adapter_key TEXT NOT NULL, idempotency_key TEXT NOT NULL, input_digest TEXT NOT NULL, status TEXT NOT NULL);`
  );
  await pool.query(
    `INSERT INTO v8_agent_resource_policies(policy_id,organization_id,project_id,max_concurrent_executions,max_estimated_cost_usd_per_run,lease_seconds) VALUES ('policy','org','project',1,1,300)`
  );
  await pool.query(
    `INSERT INTO v8_agent_adapter_invocations VALUES ('invocation','run','org','interviews','materialize','digest','failed')`
  );
  await pool.query(
  ...
  const retry = () =>
    reserveAgentResource({
      ...
      agentId: 'agent',
      toolName: 'tool',
      idempotencyKey: 'a06:run:interviews:materialize',
      estimatedCostUsd: 0.6,
      releasedRetry: {
        adapterKey: 'interviews',
        invocationIdempotencyKey: 'materialize',
        inputDigest: 'digest',
      },
    });
  const concurrent = await Promise.all([retry(), retry()]);
  assert.equal(
    concurrent.filter((item) => !item.idempotentReplay && item.status === 'reserved').length,
    1
  );
  assert.equal(
    concurrent.filter((item) => item.idempotentReplay && item.status === 'reserved').length,
  ...
  const readback = (
    await pool.query(
      `SELECT COUNT(*)::int rows, SUM(estimated_cost_usd)::float cost, MIN(status) status FROM v8_agent_resource_reservations`
    )
  ).rows[0];
  assert.deepEqual(readback, { rows: 1, cost: 0.6, status: 'reserved' });
  await pool.query(
    `UPDATE v8_agent_resource_reservations SET status='released' WHERE reservation_id='reservation'`
  );
  await assert.rejects(
    ...
    /resource_reclaim_payload_conflict/
  );
  await pool.query(
    `UPDATE v8_agent_resource_reservations SET status='settled' WHERE reservation_id='reservation'`
  );
