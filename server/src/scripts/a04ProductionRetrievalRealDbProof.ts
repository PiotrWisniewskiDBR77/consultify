...
async function main() {
  ...
  `);
  const { retrieveAndRevalidateTransformationContext } =
    await import('../services/v8/agentContextProductionRetrievalAdapter.js');
  const client = {
    query: (sql: string, params: unknown[] = []) => pool.query(adaptQuery(sql), params),
  } as any;
  const policy = {
  ...
  const first = await retrieveAndRevalidateTransformationContext({
  ...
  if (!('admitted' in first))
    throw new Error('Expected allowed retrieval with admitted candidates');
  assert.deepEqual(
    first.admitted.map((item: any) => item.artifactId),
    ['doc-allowed']
  );
  const second = await retrieveAndRevalidateTransformationContext({
    transformationCaseId: 'case-a',
    organizationId: 'org-a',
    actorUserId: 'user-a',
    policy,
  ...
  const foreign = await retrieveAndRevalidateTransformationContext({
  ...
  const blocked = await retrieveAndRevalidateTransformationContext({
    transformationCaseId: 'case-no-project',
    organizationId: 'org-a',
    actorUserId: 'user-a',
  ...
  const binding = await pool.query(
    `SELECT COUNT(*)::int count, MIN(memory_entry_id) memory_entry_id FROM v8_agent_working_memory_bindings`
  );
  const lineage = await pool.query(
    `SELECT COUNT(*)::int count FROM organization_context_lineage_events`
  );
  ...
  console.log(
    JSON.stringify({
      ...
      retryBindings: binding.rows[0].count,
      retryLineage: lineage.rows[0].count,
    })
  );
}
