...
const db = {
  all: (s: string, p: unknown[], c: any) =>
    void pool.query(adaptQuery(s), p).then((r) => c(null, r.rows), c),
  get: (s: string, p: unknown[], c: any) =>
    void pool.query(adaptQuery(s), p).then((r) => c(null, r.rows[0] ?? null), c),
  run: (s: string, p: unknown[], c: any) =>
    void pool.query(adaptQuery(s), p).then(
      (r) => c.call({ changes: r.rowCount ?? 0 }, null),
      (e) => c.call({ changes: 0 }, e)
    ),
  serialize: (c: () => void) => c(),
};
...
async function main() {
 ...
 `);
  ...
  const baseline = (
    await pool.query(
      `SELECT (SELECT COUNT(*)::int FROM transformation_cases) cases,(SELECT COUNT(*)::int FROM transformation_plans) plans,(SELECT COUNT(*)::int FROM v8_execution_runs) runs`
    )
  ).rows[0];
  const intake = await svc.startPlanningIntake({
    organizationId: 'org-t01-i03',
    actorUserId: 'user-t01-actor',
    idempotencyKey: 'a03-clarification-proof-1',
    mandate: 'Prepare a transformation plan',
    conversationId: 'conversation-a03',
  });
  assert.equal(intake.status, 'needs_clarification');
  assert.deepEqual(intake.missingKeys, ['measurable_outcomes', 'sponsor', 'scope', 'horizon']);
  const afterStart = (
    await pool.query(
      `SELECT (SELECT COUNT(*)::int FROM transformation_cases) cases,(SELECT COUNT(*)::int FROM transformation_plans) plans,(SELECT COUNT(*)::int FROM v8_execution_runs) runs`
    )
  ).rows[0];
  assert.deepEqual(afterStart, baseline);
  const partial = await svc.answerPlanningIntake({
    intakeId: intake.intakeId,
    organizationId: 'org-t01-i03',
    actorUserId: 'user-t01-actor',
    sponsor: 'COO',
  });
  assert.equal(partial.status, 'needs_clarification');
  await assert.rejects(
    () =>
      svc.convertPlanningIntake({
        intakeId: intake.intakeId,
        organizationId: 'org-t01-i03',
        actorUserId: 'user-t01-actor',
      }),
  ...
  const ready = await svc.answerPlanningIntake({
    intakeId: intake.intakeId,
    organizationId: 'org-t01-i03',
    actorUserId: 'user-t01-actor',
    measurableOutcomes: ['Reduce approval lead time to 2 days'],
    scope: 'Order approval workflow',
    horizon: 'Q4 2026',
  });
  assert.equal(ready.status, 'ready');
  assert.deepEqual(ready.missingKeys, []);
  ...
  assert.equal(left.transformationCaseId, right.transformationCaseId);
  const replay = await svc.convertPlanningIntake({
    intakeId: intake.intakeId,
    organizationId: 'org-t01-i03',
    actorUserId: 'user-t01-actor',
  });
  assert.equal(replay.transformationCaseId, left.transformationCaseId);
  assert.equal(replay.intake.idempotentReplay, true);
  await assert.rejects(
    () =>
      svc.answerPlanningIntake({
        intakeId: intake.intakeId,
