...
const RecommendationEngine = {
  ...
  generateRecommendations: async (signalsOrContext) => {
    ...
    if (!Array.isArray(signalsOrContext)) {
      ...
      if (
        !signalsOrContext ||
        Object.keys(signalsOrContext).length === 0 ||
        (!signalsOrContext.projectId && !signalsOrContext.data)
      ) {
    ...
    if (!Array.isArray(signalsOrContext)) {
      ...
      const promise = (async () => {
        try {
          ...
          if (
            RecommendationEngine.deps.aiPipeline &&
            RecommendationEngine.deps.aiPipeline.generateInitiatives
          ) {
            ...
            const recommendations = await RecommendationEngine.deps.aiPipeline.generateInitiatives(
              diagnosisReport,
              userId
            );
        ...
        return [
          {
            ...
            action:
              'Conduct a 15-minute sync with the user to identify friction points in their current task list.',
          ...
          },
          {
            ...
            action:
              "Trigger the 'first_value_checklist' playbook for this user to guide them through their first completion.",
            reasoning:
              "Help adoption ratio is low. Guiding the user through a guided flow can break the 'blank page' paralysis.",
            category: 'AI',
            priority: 2,
            impact: 'medium',
            confidence: 0.8,
        ...
        return [
          {
            ...
            action:
              "Identify the owner of the blocking task and move it to the 'Priority 1' slot for the next 24 hours.",
        ...
        return [
          {
            ...
            action:
              'Ask this member to record a short Loom or snippet on how they manage their workload.',
            reasoning:
              "High performers often have implicit processes that can be institutionalized to lift the whole team's average.",
            category: 'TEAM',
            priority: 4,
            impact: 'medium',
            confidence: 0.6,
    ...
    if (RecommendationEngine.deps.db && RecommendationEngine.deps.db.run) {
      ...
      await new Promise((resolve, reject) => {
        ...
        const runNext = (index) => {
          ...
          RecommendationEngine.deps.db.run(
  ...
  getRecommendationHistory: async (id, filter) => {
    if (RecommendationEngine.deps.db && RecommendationEngine.deps.db.all) {
      return new Promise((resolve, reject) => {
        RecommendationEngine.deps.db.all(
