...
const ActionExecutionAdapter = {
  ...
  executeDecision: async (decisionId, executedBy = 'SYSTEM', options = {}) => {
    ...
    const existingExecution = await new Promise((resolve, reject) => {
      db.get(
        `SELECT * FROM action_executions WHERE decision_id = ? AND status = 'SUCCESS'`,
        [decisionId],
        (err, row) => {
          if (err) reject(err);
          else resolve(row);
    ...
    if (dry_run) {
      const dryRunResult = await ActionExecutionAdapter._dryRunExecutor(
        executor,
        actionType,
        payload,
        metadata
