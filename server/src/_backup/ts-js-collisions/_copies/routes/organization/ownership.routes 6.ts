...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      await dbRun(`UPDATE ownership_transfer_requests SET status = 'completed' WHERE id = ?`, [
        transfer.id,
      ]);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      await dbRun(`UPDATE ownership_transfer_requests SET status = 'cancelled' WHERE id = ?`, [
        transfer.id,
      ]);
