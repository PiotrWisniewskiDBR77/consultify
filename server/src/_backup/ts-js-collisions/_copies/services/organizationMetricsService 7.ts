...
class OrganizationMetricsService {
    ...
    async getOverview(organizationId: string): Promise<OrganizationMetricsOverview> {
        try {
            ...
            const activeUsersResult = await db.query<{ count: number }>(
                `
            ...
            const totalUsersResult = await db.query<{ count: number }>(
                `
            ...
            const orgStatus = org?.status === 'trial' ? 'trial' : 
                            org?.plan === 'enterprise' ? 'enterprise' : 'paid';
            ...
            return {
                ...
                seatConfiguration: totalSeats > 0 ? {
                    seatsUsed,
                    totalSeats,
                    seatsRemaining,
                    utilizationPercent,
    ...
    async getTeamMetrics(organizationId: string): Promise<OrganizationTeamMetrics> {
        try {
            ...
            const membersResult = await db.query<{ count: number }>(
                `
                SELECT COUNT(*) as count
            ...
            const activeResult = await db.query<{ count: number }>(
                `
                SELECT COUNT(DISTINCT us.user_id) as count
                FROM user_sessions us
                INNER JOIN users u ON us.user_id = u.id
    ...
    async getAIAnalytics(organizationId: string): Promise<{
        ...
        try {
            ...
            const statsResult = await db.query<{
                ...
                `,
                [organizationId]
            ).catch(() => ({ rows: [{ total_calls: 0, success_calls: 0, total_tokens: 0, avg_latency: 0 }] }));
            ...
            const byProvider = providerResult.rows.map((row) => ({
                ...
                tokens: Number(row.tokens) || 0,
                cost: (Number(row.tokens) || 0) / 1000 * 0.002,
                successRate: Number(row.total_count) > 0 
                    ? Number(row.success_count) / Number(row.total_count) 
                    : 0,
            }));
     ...
     */
    async getMetricEvents(organizationId: string, limit: number = 20): Promise<{
        events: Array<{
            id: string;
            event_type: string;
            created_at: string;
