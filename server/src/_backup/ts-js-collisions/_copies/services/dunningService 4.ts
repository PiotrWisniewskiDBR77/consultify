...
export class DunningService {
  ...
  public async getDunningStatus(orgId: string): Promise<DunningStatus> {
    ...
    const daysSinceStart = org.dunning_started_at
      ? Math.floor(
          (Date.now() - new Date(org.dunning_started_at).getTime()) / (24 * 60 * 60 * 1000)
        )
      : 0;
  ...
  private async processOrganizationDunning(
    org: OrganizationRecord & { days_since_start?: number }
  ): Promise<void> {
    ...
    if (
      daysSinceStart >= DUNNING_SCHEDULE.FINAL_NOTICE &&
      currentStage < DUNNING_STAGES.FINAL_NOTICE
    ) {
  ...
  private getStageName(stage: number): DunningStageKey | undefined {
    return (Object.keys(DUNNING_STAGES) as Array<DunningStageKey>).find(
      (key) => DUNNING_STAGES[key] === stage
    );
  }
