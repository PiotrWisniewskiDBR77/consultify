...
class FeedbackAIServiceClass {
     ...
     */
    async analyzeFeedback(feedbackId: string, content: string, type: string): Promise<FeedbackAnalysis> {
        const id = uuidv4();
        ...
        try {
            ...
            // Calculate priority
            const priorityResult = this.calculatePriority(analysis.sentiment, analysis.sentimentScore, type, content);
...
Return ONLY valid JSON, no markdown.`;
     ...
     */
    private analyzeWithRules(content: string, type: string): {
        sentiment: 'positive' | 'negative' | 'neutral' | 'mixed';
        sentimentScore: number;
        categories: string[];
        keywords: string[];
        ...
        // Sentiment analysis
        const positiveWords = ['great', 'love', 'excellent', 'amazing', 'helpful', 'good', 'nice', 'perfect', 'thanks'];
        const negativeWords = ['bug', 'broken', 'error', 'crash', 'slow', 'terrible', 'awful', 'bad', 'hate', 'frustrating'];
    ...
    private async findSimilarFeedback(content: string, excludeId: string): Promise<string[]> {
        try {
            ...
            for (const fb of recentFeedback) {
                const fbWords = new Set(fb.message.toLowerCase().split(/\s+/).filter(w => w.length > 3));
     ...
     */
    private async getUserFeedbackStatus(userId: string): Promise<{ pendingCount: number; resolvedCount: number }> {
