...
function checkThresholds(config: WatchdogConfig): void {
  ...
  if (fiveXxInWindow >= config.fiveXxThreshold && shouldAlert('5xx_spike', config)) {
    logger.error(`[AlertWatchdog] 5xx SPIKE: ${fiveXxInWindow} errors in ${config.windowMs / 1000}s window`);
    notifyAlert('api_5xx_spike_detected', `5xx spike: ${fiveXxInWindow} errors in ${config.windowMs / 60000}min`);
  }
  ...
  if (p95 > config.latencyThresholdMs && shouldAlert('high_latency', config)) {
    logger.warn(`[AlertWatchdog] HIGH LATENCY: p95=${p95.toFixed(0)}ms (threshold: ${config.latencyThresholdMs}ms)`);
...
async function notifyAlert(type: string, message: string): Promise<void> {
  try {
    ...
    if (alertEmail && emailService?.sendEmail) {
      await emailService.sendEmail(alertEmail, `[Consultivity Alert] ${type}`, `<p>${message}</p><p>Time: ${new Date().toISOString()}</p>`);
    }
  } catch {
...
export function getWatchdogStats(): { totalRequests: number; totalFiveXx: number; windowRequests: number; windowFiveXx: number; p95Ms: number } {
  const now = Date.now();
...
const alertWatchdog = (req: Request, res: Response, next: NextFunction): void => {
  ...
  (res as any).end = function (...args: any[]) {
    ...
    if (records.length % 10 === 0) {
      try { checkThresholds(DEFAULT_CONFIG); } catch { /* fail-open */ }
    }
