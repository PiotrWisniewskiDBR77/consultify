 ...
 */
export function performanceMetricsMiddleware(req: RequestWithMetrics, res: Response, next: NextFunction): void {
...
export function getMetricsSummary(windowMinutes: number = 60): MetricsSummary {
    ...
    const recentRequests = metricsStore.requests.filter((m) => new Date(m.timestamp).getTime() > cutoff);
