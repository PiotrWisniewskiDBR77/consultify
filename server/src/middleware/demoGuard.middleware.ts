...
export const demoContextMiddleware = async (
    ...
    try {
        ...
        logger.debug(`[DemoMode] Middleware called: isDemoMode=${isDemoMode}, hasUser=${!!req.user}, userOrgId=${req.user?.organizationId}`);
        ...
        if (isDemoMode && req.user) {
            ...
            logger.info(`[DemoMode] User ${req.user.id} switched to demo mode (original org: ${originalOrgId} -> ${DEMO_ORG_ID})`);
        } else if (isDemoMode) {
...
export const getDemoStats = async () => {
    try {
        const [projects, initiatives, tasks, assessments] = await Promise.all([
            dbGet<{ count: number }>('SELECT COUNT(*) as count FROM projects WHERE organization_id = ?', [DEMO_ORG_ID]),
            dbGet<{ count: number }>('SELECT COUNT(*) as count FROM initiatives WHERE organization_id = ?', [DEMO_ORG_ID]),
            dbGet<{ count: number }>('SELECT COUNT(*) as count FROM tasks WHERE organization_id = ?', [DEMO_ORG_ID]),
            dbGet<{ count: number }>('SELECT COUNT(*) as count FROM assessments WHERE organization_id = ?', [DEMO_ORG_ID]),
        ]);
