...
const MAX_CONFIRMATION_STRING_LENGTH = 64;
...
export function requireConfirmation(actionType: string, riskLevel: RiskLevel = 'high') {
  ...
  return async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (
        typeof rawConfirmation === 'string' &&
        rawConfirmation.length > MAX_CONFIRMATION_STRING_LENGTH
      ) {
        respondJson(res, 428, {
          error: `Confirmation string must be at most ${MAX_CONFIRMATION_STRING_LENGTH} characters`,
          code: 'CONFIRMATION_STRING_TOO_LONG',
          actionType: normalizedActionType,
          riskLevel,
        });
        return;
      ...
      logger.error('[ConfirmAction] unexpected handler error', {
        err,
        actionType: normalizedActionType,
      });
      if (!safeRead(() => res.headersSent, false)) {
        respondJson(res, 500, {
          error: 'Unable to process confirmation gate',
          code: 'CONFIRMATION_GATE_UNEXPECTED',
          actionType: normalizedActionType,
        });
      }
      return;
