...
export function requireFrameworkAccess(frameworkId: string) {
  return async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!result.allowed) {
      ...
      res.status(403).json({ error: 'FRAMEWORK_ACCESS_DENIED', framework: frameworkId, accessLevel: result.accessLevel, reason: result.reason, upgradeCTA: result.upgradeCTA });
      return;
    }
    req.frameworkAccess = { allowed: true, accessLevel: result.accessLevel, requiresLegalNotice: result.requiresLegalNotice };
    next();
  };
}
...
export function requireDynamicFrameworkAccess(paramName = 'frameworkId') {
  return async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!orgId) { res.status(401).json({ error: 'UNAUTHORIZED' }); return; }
    if (!fwId) { res.status(400).json({ error: 'BAD_REQUEST', message: 'Framework ID required' }); return; }
    const result = await FrameworkEntitlementService.checkAccess(orgId, fwId.toUpperCase());
    if (!result.allowed) {
      res.status(403).json({ error: 'FRAMEWORK_ACCESS_DENIED', framework: fwId, accessLevel: result.accessLevel, reason: result.reason, upgradeCTA: result.upgradeCTA });
      return;
    }
    req.frameworkAccess = { allowed: true, accessLevel: result.accessLevel, requiresLegalNotice: result.requiresLegalNotice };
    next();
