...
export class EntityTranslationService {
   ...
   */
  async getTranslations(entityType: string, entityId: string): Promise<TranslationResult> {
    if (this.locale === 'en') {
  ...
  async translateEntities<T extends Record<string, any>>(
    ...
    return Promise.all(entities.map((entity) => this.translateEntity(entity, entityType, fields)));
  }
