...
const updateOrganizationMetadata = catchAsync(async (req, res, next) => {
    ...
    await deps.OrganizationMetadataService.setMetadata(id, key, value, valueType, category, isSensitive);
...
const updateUserProfileExtended = catchAsync(async (req, res, next) => {
    ...
    deps.db.run(
        ...
        [
            profileId, id,
            profileData.jobTitle, profileData.department, profileData.phone,
            profileData.timezone || 'UTC', profileData.locale || 'en',
            profileData.avatarUrl, profileData.bio,
...
const getUserActivity = catchAsync(async (req, res, next) => {
    ...
    deps.db.get('SELECT organization_id FROM users WHERE id = ?', [id], async (err, user) => {
        ...
        const activity = await deps.UserActivityService.calculateActivitySummary(
            id, user.organization_id, periodStart, periodEnd.toISOString().split('T')[0]
        );
        res.json(activity);
    });
});
...
const updateUserOnboardingProgress = catchAsync(async (req, res, next) => {
    ...
    deps.db.get('SELECT organization_id FROM users WHERE id = ?', [id], (err, user) => {
        ...
        deps.db.run(
            ...
            [
                progressId, id, user.organization_id, stepKey, stepName,
                completed ? 1 : 0, completed ? new Date().toISOString() : null,
                skipped ? 1 : 0, skipped ? new Date().toISOString() : null
            ],
            function (err) {
                if (err) return next(new AppError(err.message, 500));
...
const assignUserLicense = catchAsync(async (req, res, next) => {
    ...
    deps.db.get('SELECT organization_id FROM users WHERE id = ?', [id], async (err, user) => {
        ...
        const license = await deps.UserLicenseService.assignLicense(
            id, user.organization_id, licenseType, features, limits, expiresAt, req.user.id, notes
        );
        res.json(license);
    });
});
...
const addIPWhitelist = catchAsync(async (req, res, next) => {
    ...
    const result = await deps.IPWhitelistService.addIP(id, ipAddress, ipRange, description, req.user.id);
    res.json(result);
});
...
const getSecurityEvents = catchAsync(async (req, res, next) => {
    const { organizationId, userId, eventType, severity, resolved } = req.query;
    const events = await deps.SecurityEventService.getEvents({
        organizationId, userId, eventType, severity,
...
const addTicketComment = catchAsync(async (req, res, next) => {
    ...
    const comment = await deps.SupportTicketService.addComment(id, req.user.id, commentText, isInternal);
    res.json(comment);
});
...
const addFeedbackComment = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const { commentText, isInternal } = req.body;
    const comment = await deps.FeedbackService.addFeedbackComment(id, req.user.id, commentText, isInternal);
    res.json(comment);
});
...
const getUserAdoptionMetrics = catchAsync(async (req, res, next) => {
    ...
    deps.db.get('SELECT organization_id FROM users WHERE id = ?', [id], async (err, user) => {
        if (err || !user) return next(new AppError('User not found', 404));
        const metrics = await deps.UserAdoptionService.getMetrics(id, user.organization_id, startDate, endDate);
        res.json(metrics);
    });
});
    ...
    deps.db.get('SELECT organization_id FROM users WHERE id = ?', [id], async (err, user) => {
        ...
        if (status === 'granted') {
            await deps.ConsentManagementService.grantConsent(
                id, user.organization_id, consentType, consentVersion,
    ...
    for (const pref of preferences) {
        ...
        deps.db.run(
            `INSERT INTO notification_preferences 
             ...
             updated_at = datetime('now')`,
            [
                prefId, id, pref.organizationId, pref.notificationType,
                pref.channel, pref.isEnabled ? 1 : 0, pref.frequency || 'immediate'
            ]
        );
    }
...
const exportAuditLogs = catchAsync(async (req, res, next) => {
    ...
    if (format === 'csv') {
        const headers = ['ID', 'Admin Email', 'Action Type', 'Resource Type', 'Resource ID',
            'Description', 'IP Address', 'Risk Score', 'Status', 'Created At', 'Resolved At'];
        const rows = logs.map(l => [
            l.id, l.admin_email || '', l.action_type, l.resource_type || '', l.resource_id || '',
            l.description || '', l.ip_address || '', l.risk_score, l.status, l.created_at, l.resolved_at || ''
        ]);
...
const updateAdminPermission = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        'UPDATE permissions SET description = ?, category = ? WHERE key = ?',
...
const createApprovalWorkflow = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        `INSERT INTO admin_approval_workflows (id, name, description, resource_type, trigger_conditions_json, approvers_json, created_by, is_active)
         VALUES (?, ?, ?, ?, ?, ?, ?, 1)`,
        [id, name, description, resourceType, JSON.stringify(triggerConditions || {}), JSON.stringify(approvers || []), req.user.id]
    );
...
const updateApprovalWorkflow = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        ...
        [name, description, JSON.stringify(triggerConditions || {}), JSON.stringify(approvers || []), isActive ? 1 : 0, id]
    );
...
const createAnalyticsDashboard = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        `INSERT INTO admin_dashboards (id, name, description, layout_json, widgets_json, created_by)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [id, name, description, JSON.stringify(layout || {}), JSON.stringify(widgets || []), req.user.id]
    );
...
const getAnalyticsDashboardData = catchAsync(async (req, res, next) => {
    ...
    for (const widget of widgets) {
        ...
        switch (widget.dataSource) {
            ...
            case 'users':
                ...
                break;
            case 'revenue':
                widgetData[widget.id] = await deps.db.all('SELECT SUM(amount) as total FROM token_transactions WHERE type = "purchase"');
                break;
            default:
    ...
    res.json({ dashboard: { ...dashboard, layout: JSON.parse(dashboard.layout_json || '{}'), widgets }, widgetData });
});
...
const shareAnalyticsDashboard = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run('UPDATE admin_dashboards SET is_shared = ?, updated_at = datetime("now") WHERE id = ?', [isShared ? 1 : 0, id]);
...
const createAnalyticsReport = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        `INSERT INTO admin_saved_reports (id, name, description, report_type, filters_json, columns_json, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [id, name, description, reportType, JSON.stringify(filters || {}), JSON.stringify(columns || []), req.user.id]
    );
...
const executeAnalyticsReport = catchAsync(async (req, res, next) => {
    ...
    switch (report.report_type) {
        ...
        case 'users':
            ...
            break;
        case 'revenue':
            result = await deps.db.all('SELECT * FROM token_transactions WHERE type = "purchase" LIMIT 1000');
            break;
        default:
            result = [];
    }
...
const calculateBusinessMetric = catchAsync(async (req, res, next) => {
    ...
    switch (metric.metric_type) {
        ...
        case 'org_count':
            ...
            break;
        case 'revenue':
            const revenue = await deps.db.get('SELECT SUM(amount) as total FROM token_transactions WHERE type = "purchase"');
            value = revenue?.total || 0;
            break;
        case 'active_users':
            const activeUsers = await deps.db.get('SELECT COUNT(*) as count FROM users WHERE last_login > datetime("now", "-7 days")');
...
const createPredictiveModel = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        `INSERT INTO predictive_models (id, name, description, model_type, training_data_json, model_config_json)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [id, name, description, modelType, JSON.stringify(trainingData || {}), JSON.stringify(modelConfig || {})]
    );
...
const makePrediction = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        `INSERT INTO model_predictions (id, model_id, prediction_type, predicted_value, confidence_score, input_data_json)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [predictionId, id, model.model_type, predictedValue.toString(), confidenceScore, JSON.stringify(inputData || {})]
    );
...
const getLifecycleStages = catchAsync(async (req, res, next) => {
    const stages = await deps.db.all('SELECT * FROM customer_lifecycle_stages ORDER BY order_index ASC');
...
const getLifecycleStats = catchAsync(async (req, res, next) => {
    ...
    const totalTransitions = await deps.db.get('SELECT COUNT(*) as count FROM customer_lifecycle_transitions');
...
const createSuccessPlaybook = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        `INSERT INTO customer_success_playbooks (id, name, description, trigger_conditions_json, actions_json, created_by)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [id, name, description, JSON.stringify(triggerConditions || {}), JSON.stringify(actions || []), req.user.id]
    );
...
const updateSuccessPlaybook = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        ...
        [name, description, JSON.stringify(triggerConditions || {}), JSON.stringify(actions || []), isActive ? 1 : 0, id]
    );
...
const createCustomerContract = catchAsync(async (req, res, next) => {
    ...
    await deps.db.run(
        `INSERT INTO customer_contracts (id, organization_id, contract_type, start_date, end_date, renewal_date, value, currency, terms_json, document_url)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, organizationId, contractType, startDate, endDate, renewalDate, value || 0, currency || 'USD', JSON.stringify(terms || {}), documentUrl]
    );
...
const getPricingPlans = catchAsync(async (req, res, next) => {
    ...
    const plans = [
        ...
        { id: 'pro', name: 'Pro', price: 99, features: ['All features', '10 projects', 'Priority support'] },
        { id: 'enterprise', name: 'Enterprise', price: 499, features: ['Unlimited features', 'Unlimited projects', 'Dedicated support', 'SLA'] }
    ];
    res.json(plans);
});
...
const createSubscriptionChange = catchAsync(async (req, res, next) => {
    const { organizationId, fromPlanId, toPlanId, changeType, effectiveDate, prorationAmount } = req.body;
    const id = deps.uuid.v4();
...
const getRevenueRecognitions = catchAsync(async (req, res, next) => {
    ...
    res.json(recognitions.map(r => ({ ...r, recognitionSchedule: JSON.parse(r.recognition_schedule_json || '[]') })));
});

const createRevenueRecognition = catchAsync(async (req, res, next) => {
    const { organizationId, contractId, revenueAmount, currency, recognitionMethod, recognitionSchedule } = req.body;
    const id = deps.uuid.v4();
    ...
    await deps.db.run(
        `INSERT INTO revenue_recognition (id, organization_id, contract_id, revenue_amount, currency, recognition_method, recognition_schedule_json, remaining_amount)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, organizationId, contractId, revenueAmount, currency || 'USD', recognitionMethod || 'straight_line', JSON.stringify(recognitionSchedule || []), revenueAmount]
    );
...
const createRevenueForecast = catchAsync(async (req, res, next) => {
    const { forecastType, periodStart, periodEnd, forecastedAmount, currency, confidenceLevel, method, inputData } = req.body;
    const id = deps.uuid.v4();
    ...
    await deps.db.run(
        `INSERT INTO revenue_forecasts (id, forecast_type, period_start, period_end, forecasted_amount, currency, confidence_level, method, input_data_json)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, forecastType, periodStart, periodEnd, forecastedAmount, currency || 'USD', confidenceLevel || 0.8, method || 'linear', JSON.stringify(inputData || {})]
    );
...
const addPaymentMethod = catchAsync(async (req, res, next) => {
    ...
    if (isDefault) {
        ...
        await deps.db.run('UPDATE payment_methods SET is_default = 0 WHERE organization_id = ?', [organizationId]);
    }
...
const updatePaymentMethod = catchAsync(async (req, res, next) => {
    ...
    if (isDefault) {
        await deps.db.run('UPDATE payment_methods SET is_default = 0 WHERE organization_id = ?', [method.organization_id]);
    }
...
const getThreats = catchAsync(async (req, res, next) => {
    const { threatType, threatLevel, isBlocked, ipAddress, domain, limit = 100, offset = 0 } = req.query;
...
const addThreat = catchAsync(async (req, res, next) => {
    const { threatType, source, ipAddress, domain, reputationScore, threatLevel, description } = req.body;
