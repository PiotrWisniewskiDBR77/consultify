...
class ManagementReportRepository {
    ...
    async getCommentById(commentId) {
        return new Promise((resolve, reject) => {
            this.db.get('SELECT * FROM management_report_comments WHERE id = ?', [commentId], (err, row) => {
                if (err) reject(err);
                else resolve(row);
            });
        });
    ...
    async getUser(userId) {
        return new Promise((resolve, reject) => {
            this.db.get('SELECT id, first_name, last_name, email, role FROM users WHERE id = ?', [userId], (err, row) => {
                if (err) reject(err);
                else resolve(row);
            });
        });
    ...
    async getCompletedTasks(projectId, periodStart, periodEnd) {
        return new Promise((resolve, reject) => {
            this.db.all(`
                ...
                LIMIT 30
            `, [projectId, periodStart, periodEnd], (err, rows) => {
                if (err) reject(err);
                else resolve(rows || []);
            });
        });
    ...
    async getUpcomingTasks(projectId, dueDateLimit) {
        return new Promise((resolve, reject) => {
            this.db.all(`
                ...
                ORDER BY t.due_date ASC
                LIMIT 20
            `, [projectId, dueDateLimit], (err, rows) => {
                if (err) reject(err);
                else resolve(rows || []);
            });
        });
