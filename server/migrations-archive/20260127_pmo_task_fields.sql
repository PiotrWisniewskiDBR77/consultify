...
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS owner_id TEXT REFERENCES users(id) ON DELETE SET NULL;

-- Acceptance gate (approval)
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS requires_acceptance BOOLEAN DEFAULT FALSE;
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS acceptance_type TEXT; -- manual | automatic
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS acceptor_id TEXT REFERENCES users(id) ON DELETE SET NULL;

-- Backup assignee (substitute)
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS backup_assignee_id TEXT REFERENCES users(id) ON DELETE SET NULL;

-- Weight for initiative progress contribution
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS weight REAL DEFAULT 1.0;
