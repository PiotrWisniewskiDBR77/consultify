...
files.forEach((file) => {
  ...
  if (
    content.includes('router.use(defaultRateLimiter)') &&
    !content.includes('defaultRateLimiter } from')
  ) {
  ...
  if (
    content.includes('router.use(authRateLimiter)') &&
    !content.includes('authRateLimiter } from')
  ) {
