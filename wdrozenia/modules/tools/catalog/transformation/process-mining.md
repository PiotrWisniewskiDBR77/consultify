...
- **Tool name**: Process Mining (Discovery & Conformance)
- **Slug**: `process-mining`
- **Category**: Transformation
- **Level**: Advanced
- **Typical duration**: 60–180 minutes
- **Best for**: Discovering real process flows from event logs and identifying deviations/bottlenecks.
- **Not for**: Processes without event data; replacing stakeholder workshops.
- **Primary outputs**: As-is model, conformance gaps, bottlenecks, improvement/automation candidates.
...
- Event logs (caseId/activity/timestamp).
...
Order-to-cash has long lead times; stakeholders disagree about root causes.
...
- **VO (PL)**: "Process Mining (Discovery & Conformance) pomaga zaplanować transformację."
- **VO (EN)**: "Process Mining (Discovery & Conformance) helps plan transformation."
- **On-screen text (PL)**: "Process Mining (Discovery & Conformance) = Plan transformacji"
- **On-screen text (EN)**: "Process Mining (Discovery & Conformance) = Transformation plan"
...
- **VO (PL)**: "Eksportuj raport i generuj inicjatywy. Rozpocznij Process Mining (Discovery & Conformance) już dziś."
- **VO (EN)**: "Export the report and generate initiatives. Start Process Mining (Discovery & Conformance) today."
...
1. **What is Process Mining (Discovery & Conformance) used for?**
   A: It's used to discover actual process flows from event logs, compare them to intended models, identify bottlenecks and deviations, and generate improvement opportunities.

2. **What inputs are required?**
   A: Required inputs include event logs (case ID, activity, timestamp), process scope definition, and optional inputs like resource information, costs, and intended process models.

3. **How do we validate outputs with evidence?**
   A: Each discovered process path and conformance gap should be backed by event log evidence. Use conformance checking algorithms to quantify deviations and validate findings with process owners.

4. **How do we handle missing data?**
   A: Document data quality issues, use filtering to focus on complete cases, and prioritize data quality improvements as part of the improvement backlog.

5. **How do we convert outputs to initiatives?**
   A: Convert identified bottlenecks, deviations, and improvement opportunities into initiatives with clear rationale, expected impact, owners, and timelines. Link initiatives to specific process paths or conformance gaps.

6. **Who owns the decisions?**
   A: Process owners own decisions about process changes, while IT/data teams own data quality and system improvements. Establish clear ownership for each type of improvement.

7. **How often should this be updated?**
   A: Update quarterly to track process evolution and measure improvement impact. Re-run discovery after significant process changes or system updates.

8. **What are common anti-patterns?**
   A: Common anti-patterns include treating it as a one-time exercise, focusing only on discovery without conformance checking, and not linking findings to actionable improvements.
...
- [ ] Poor data quality → Fix: Clean and validate event logs before analysis
- [ ] No conformance checking → Fix: Always compare discovered models to intended processes

### Glossary (short)

| Term                 | Definition                                            | Example                                             |
| -------------------- | ----------------------------------------------------- | --------------------------------------------------- |
| Process Mining       | Automated discovery of process models from event logs | Discovering order fulfillment process from ERP logs |
| Event Log            | Record of process execution events                    | CSV with case ID, activity, timestamp               |
| Conformance Checking | Comparing discovered process to intended model        | Finding deviations from standard procedure          |
| Bottleneck           | Process step causing delays                           | Long wait time at approval step                     |
| As-is Model          | Discovered process model showing actual execution     | Process map extracted from logs                     |
| To-be Model          | Intended or improved process model                    | Target process design                               |

---

## 13. Additional Resources & Learning Links

- - See references: Process Mining book: https://link.springer.com/book/10.1007/978-3-662-49851-4

---

## 14. References (Authoritative Sources)

- Process Mining book: https://link.springer.com/book/10.1007/978-3-662-49851-4
- TF-PM: https://www.tf-pm.org/
- Celonis overview: https://www.celonis.com/process-mining/
