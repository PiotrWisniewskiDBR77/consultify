...
- Backend: `server/src/constants/initiativeStatuses.ts` (enum + transitions + gates + module mapping)
- Frontend: `src/services/initiativeLifecycle.ts` (UI meta + widoczność per moduł + CTA)

> **Uwaga 1 (moduły źródłowe):** workflow zawiera **DRAFT** oraz **PENDING_REVIEW** w modułach źródłowych (Tools/Assessment), zanim inicjatywa trafi do modułu Initiatives.
>
> **Uwaga 2 (terminal):** status **ARCHIVED** jest terminalny i służy do „zamknięcia historycznego” (po Benefits/Cancelled).
...
| **Zakończenie**  | Decyzja o zamknięciu trackingu lub upływ okresu → przejście do **ARCHIVED** |
| **Kto decyduje** | **Business Owner**, PMO                                                     |
| **Moduł UI**     | Benefits                                                                    |

---

## Statusy terminalne (CANCELLED / ARCHIVED)
...
| Aspekt            | Opis                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------- |
| **Co to znaczy**  | Inicjatywa zamknięta historycznie (read-only) i nie bierze udziału w operacyjnym workflow                   |
| **Kiedy używamy** | Po zakończeniu trackingu korzyści (TRACKING) **albo** po anulowaniu (CANCELLED), gdy chcemy zamrozić rekord |
| **Zasada**        | Brak edycji merytoryki; zostaje pełny audit trail                                                           |
| **Kto decyduje**  | **PMO** (lub rola delegowana w małych projektach)                                                           |
| **Moduł UI**      | Reporting / Initiatives (historyczne)                                                                       |
...
| **Initiatives** | REVIEW, PROMOTED, PLANNING, APPROVED, SCHEDULED, CANCELLED, ARCHIVED (historyczne) | PMO, Project Sponsor, Steering |
...
| `ARCHIVE`        | TRACKING/CANCELLED → ARCHIVED | PMO                        | Wykonawcza   | Reporting / Initiatives |

---

## Kluczowa zasada systemu

> **Consultant nie może wykonywać gate decisions biznesowych** (ACCEPT/APPROVE/SCHEDULE/UNBLOCK itd.).
> Jedyny wyjątek techniczny w fazie źródłowej to **`SUBMIT_FOR_REVIEW`** (wysłanie własnej pracy do przeglądu).
>
...
| 13  | ARCHIVED       | Archived       | Zarchiwizowana           | slate   | Archive       | Terminal         |
