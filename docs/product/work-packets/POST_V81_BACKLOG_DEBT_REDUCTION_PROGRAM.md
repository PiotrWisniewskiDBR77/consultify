...
| broader `Multiplayer / collaboration` breadth | active | accepted lane closed presence + lock indicators only; the broader lane is now promoted around visible degraded-state and runtime continuity on active collaboration surfaces, with honest table/legacy availability state plus overlay and table reconnect continuity, without claiming a full transport rewrite | `12` | remaining collaboration breadth is closed or explicitly retired |
