...
| broader `Results / KPI / ROI` parity | active | accepted lane closed bounded read/runtime truth seams, and the first fourteen broader write seams have now landed, but smaller deviation-case mutations still remain | `8` | remaining results writes and operator surfaces are closed or explicitly retired |
