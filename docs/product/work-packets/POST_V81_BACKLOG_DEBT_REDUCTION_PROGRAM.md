...
| broader `Partner Program` parity | active | accepted lane closed visible payout / campaign / profile seams; payout-history, statement-history, referred-customer list continuity, referral-tools body reads, referred-customer lifecycle readback, and `partner-home` onboarding status readback are now governed by V8-first reads, while deeper onboarding, client access, statement-source, and payout-settings breadth still remain | `10` | remaining partner breadth is closed or explicitly retired |
