...
| broader `Results / KPI / ROI` parity | active | accepted lane closed bounded read/runtime truth seams, and the first six broader write seams have now landed, but smaller KPI operator drawer mutations still remain | `8` | remaining results writes and operator surfaces are closed or explicitly retired |
