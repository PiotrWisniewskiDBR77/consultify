...
| `MyWork roof` | `yes` | `MYWORK_HOME_V1_SSOT.md`, `MY_WORK_INBOX_AND_SLA.md` | MyWork/UI closure packets | governed `/api/v8/my-work` route cluster now exposes roof summary plus direct deep-flow paths for canonical object state, surface projections, inbox materialization stats, and calendar phasing; summary still reports `homeViewUsesAggregatedContract=false` | staging no longer falls into `Too many requests`; live browser now settles with real Radar/My Work content and polling endpoints at `200`, but the explicit canonical Home block labels still do not appear in the live DOM | targeted route + component tests pass; live staging proof now spans `evidence/18-v81-mywork-roof-summary-proof.json`, `evidence/19-v81-mywork-roof-deep-flow-proof.json`, `evidence/20-v81-home-radar-staging-refresh.json`, `evidence/21-v81-home-radar-rate-limit-blocker.json`, `evidence/22-v81-home-radar-rate-limit-retest-after-shared-keying-fix.json`, `evidence/23-v81-home-radar-global-rate-limit-root-cause.json`, and `evidence/24-v81-home-radar-post-cutover-surface-proof.json` | `yellow` | `C` | `C-02f live contract-label visibility mismatch` | the staging runtime blocker is cleared, but closure proof remains partial because the live surface still lacks explicit canonical labels; `homeViewUsesAggregatedContract=false` and stringified inbox numerics still remain |
| `Radar` | `yes` | `MYWORK_RADAR_*` | MyWork/Radar packet chain | prior staging refresh repaired the `ideas`-table runtime defect, and the global limiter mitigation now lets authenticated `/api/my-work/radar` settle at `200` again on staging | fresh authenticated Radar runtime is recovered, but closure-grade surface proof remains partial because the explicit canonical Home/Radar labels still do not appear in the live DOM after cutover | refreshed evidence now spans the repaired runtime in `evidence/20-v81-home-radar-staging-refresh.json`, the limiter root cause in `evidence/23-v81-home-radar-global-rate-limit-root-cause.json`, and the post-cutover surface state in `evidence/24-v81-home-radar-post-cutover-surface-proof.json` | `yellow` | `C` | `C-02f live contract-label visibility mismatch` | staging runtime is no longer the blocker, but the live surface still does not prove the canonical label contract end-to-end |
...
2. `MyWork roof` now has direct routed + staging-proven deep-flow exposure, the live staging bundle contains the explicit Home/Radar canonical labels, and the staging global limiter blocker is now removed (`roof/summary`, `radar`, `context-summary`, notifications, tasks, and provider health all recovered to `200` after deployment `c8ff8bd1-4fc3-4f7e-afbc-0fe5b37bcc37`). Fresh closure proof is still partial because the explicit canonical Home/Radar labels are not visible in the settled live DOM, the proven truth still reports `mixed_truth`, `homeViewUsesAggregatedContract=false`, and staging still stringifies inbox aggregate numerics.
...
- `C-02f` - resolve the live canonical-label visibility mismatch after the staging runtime blocker was removed
