...
2. `MyWork roof` now has direct routed + staging-proven deep-flow exposure, but the proven truth still reports `mixed_truth`, keeps `homeViewUsesAggregatedContract=false`, and staging currently stringifies inbox aggregate numerics.
...
- `C-02c` - reconcile `Home/Radar` surface with exposed roof contract and frozen 8-block doctrine
