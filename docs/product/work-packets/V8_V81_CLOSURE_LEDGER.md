...
| `MyWork roof` | `yes` | `MYWORK_HOME_V1_SSOT.md`, `MY_WORK_INBOX_AND_SLA.md` | MyWork/UI closure packets | governed `/api/v8/my-work/roof/summary` now exposes closure-grade roof truth: explicit Home block maturity, inbox materialization snapshot, calendar phasing, and contract flags including `homeViewUsesAggregatedContract=false` | `HomeView` now displays a roof-truth strip above the Radar surface, explicitly stating `Radar overlay + outputs bridge` rather than silently pretending the frozen 8-block Home is already live | targeted route + component tests pass; live staging proof captured in `evidence/18-v81-mywork-roof-summary-proof.json` (`GET /api/v8/my-work/roof/summary` 200 with `overallStatus=mixed_truth`, block counts `2 real / 2 partial / 4 non-canonical`) | `yellow` | `C` | `C-02b follow-through: inbox/calendar/object-state proof` | truth is now exposed, but the exposed truth still confirms mixed roof semantics (`4` placeholder blocks, inbox not proven, external-sync calendar blocked) |
...
2. `MyWork roof` now has direct routed + staging-proven truth exposure, but that proof still reports `mixed_truth` and leaves inbox/object-state/calendar closure unresolved.
...
- `C-02b+` - convert exposed `MyWork roof` truth into deep-flow closure (`inbox`, `calendar`, canonical object state`)
