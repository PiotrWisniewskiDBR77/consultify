...
| `MyWork roof` | `yes` | `MYWORK_HOME_V1_SSOT.md`, `MY_WORK_INBOX_AND_SLA.md` | MyWork/UI closure packets | governed `/api/v8/my-work` route cluster now exposes roof summary plus direct deep-flow paths for canonical object state, surface projections, inbox materialization stats, and calendar phasing; summary still reports `homeViewUsesAggregatedContract=false` | local `HomeView` and the current live staging bundle both carry the explicit canonical-block mapping (`AI Pulse Core`, `Industry Lens`, `Execution Current`, `Command Dock`), but a fresh authenticated `My Work` load is currently blocked by staging-wide `429` responses before the settled surface can be proven | targeted route + component tests pass; live staging proof captured in `evidence/18-v81-mywork-roof-summary-proof.json`, `evidence/19-v81-mywork-roof-deep-flow-proof.json`, `evidence/20-v81-home-radar-staging-refresh.json`, and `evidence/21-v81-home-radar-rate-limit-blocker.json` (bundle contains canonical labels, but fresh browser load returns `429` on roof/radar/context polling endpoints and the latest mitigation deploy `3d731c33-be8e-41b9-861a-e2250506dd4e` was still `BUILDING` at capture time) | `yellow` | `C` | `C-02d staging rate-limit unblock` | deep-flow and prior Radar runtime proof remain valid, but fresh closure proof is currently blocked by staging `429` responses, `homeViewUsesAggregatedContract=false`, and stringified inbox numerics |
| `Radar` | `yes` | `MYWORK_RADAR_*` | MyWork/Radar packet chain | prior staging refresh repaired the `ideas`-table runtime defect, and the live staging `HomeView` bundle now proves the canonical label strings are shipped | fresh authenticated Radar proof is currently blocked by `429` responses on `roof/summary`, `radar`, and adjacent My Work polling endpoints before the surface can settle | refreshed evidence now spans the repaired runtime in `evidence/20-v81-home-radar-staging-refresh.json` and the current staging blocker in `evidence/21-v81-home-radar-rate-limit-blocker.json` (bundle label strings present, but fresh browser load returns `429`) | `yellow` | `C` | `C-02d staging rate-limit unblock` | the label mismatch is no longer explained by stale frontend code, but closure-grade surface proof is blocked until staging stops rate-limiting the authenticated My Work load |
...
- `C-02d` - unblock staging `My Work` rate limiting so refreshed `Home/Radar` surface proof can settle
