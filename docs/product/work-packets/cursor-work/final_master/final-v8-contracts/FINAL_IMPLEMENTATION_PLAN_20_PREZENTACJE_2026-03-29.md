...
| P20-B | delivered | this commit on `ws/c-artifact-evidence` | **Split-screen**: KimiWorkspaceShell extended with `prezentacje` lane (chat 420px left ↔ deck preview right); PrezentacjeView wraps shell with lane="prezentacje". **Pipeline**: useKimiArtifactPipeline extended for `presentation` artifact family + `presentation` output type; 8-step pipeline (snapshot→plan→preflight→accept→review→approve→materialize→generate). **Auto-trigger**: chat first user+AI exchange auto-starts pipeline. **Preview**: deck slide cards fetched from `/api/presentations/decks/{id}` with unified_json parsing (slide intent, title, bullet points); KPI cards (slide count, format, status). **Download**: PPTX via `/api/presentations/decks/{id}/download`. **DeckBuilder link**: "Open in Deck Builder" button links to `/presentations/builder/{id}`. **Progress**: 8-step TaskProgressBar. **Replay/Remix**: server-side re-run / reset to goal input. **Sidebar**: AppView.PREZENTACJE_GEN in menuConfig with Presentation icon. **Reopen**: artifactLinks presentation→`/prezentacje?artifactId=`. **Routes**: `/prezentacje` in routeConfig + AppRoutes. | Gamma-style split-screen generation workspace functional; pipeline wired to V8 artifact run lifecycle; deck preview shows slide cards with intent/title/bullets; DeckBuilder integration for full editing; Outputs Library integration via P19. | Bounded: deck preview is read-only slide card summary (not full WYSIWYG editor — use DeckBuilder for that); template selection not yet in KIMI flow (uses default); continuation depth (per-slide AI edits) available only in DeckBuilder; share/analytics posture bounded per §2.9; no cancel-mid-generation. Provenance via P18/P19. |
| P20-C | verified(evidence) | `2c4965e88e` + this commit on `ws/c-artifact-evidence` | **Integration**: split-screen chat↔deck → generate via V8 pipeline → deck slide card preview (intent/title/bullets from unified_json) → KPI summary (slide count, format, status) → download PPTX → open in DeckBuilder → Replay (server re-run) → Remix (new goal). **Regression**: export failure → toast error + retry; no ghost outputs (cleanupGhostOutputsByOrigin on failure). **Contract**: deck artifact payload includes type + lifecycle + export via v8_artifact_runs + v8_output_artifacts + v8_artifact_origin_links (P18); presentation registered via artifact registry (P19). **Rollback**: disable Prezentacje sidebar entry + route; preserve Outputs Library listing + DeckBuilder access + PPTX/PDF export via existing `/api/presentations/` endpoints. | Staging proof: (1) Open /prezentacje → split-screen renders; (2) Type goal in right pane textarea → click Generate → pipeline starts (8 steps visible); (3) Chat auto-trigger: send message → AI responds → pipeline auto-starts; (4) Progress bar shows snapshot→plan→…→generate content; (5) Deck preview loads with slide cards (intent, title, bullet points) + KPI cards; (6) "Open in Deck Builder" button links to /presentations/builder/{id}; (7) Download button triggers PPTX download; (8) Replay re-runs pipeline; Remix resets to goal input; (9) Sidebar entry "Prezentacje" navigates to /prezentacje; (10) Outputs Library shows deck with reopen path to /prezentacje?artifactId=. | Known limits: deck preview is read-only slide card summary (not full WYSIWYG — use DeckBuilder for that); template selection not yet in KIMI flow (uses default/empty templateId); continuation depth (per-slide AI edits, version history, revert) available only in DeckBuilder; share/analytics posture bounded per §2.9; no cancel-mid-generation; review grammar (draft/reviewed badges) not yet surfaced in KIMI flow (available in DeckBuilder/Outputs). |
