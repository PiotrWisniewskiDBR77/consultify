...
| P35-C | verified(evidence) | (see commits) | **Integration**: 24 tests — lifecycle CRUD, server-side search (cursor, filters), list cursor pagination, deep-link states, archive vs delete, bulk ops, attachment CRUD (create/list), session management (list/create), write conflict detection (409), team permissions, chat projects, regression guards. **Store**: 18 tests — grouping, lifecycle actions, serverSearch (success + degraded), local search, archive/unarchive/delete with API verification, write conflict (expectedVersion + 409 refresh), deep-link state management, folder/project separation. | 10-step staging proof: create/rename/pin/move/archive/unarchive/delete/search/deep-link/team-denial. | **Full canonical model**: sessions (runtime + CRUD), attachments (CRUD + degraded), metadata, message fields. **P34 consumed**: read permission before list/search. **Degraded posture**: E2 partial results, E4 permission denial UX, E5 deep-link states, E6 attachment degraded, E8 rate limit, E9 write conflict (409 + auto-refresh). **State banners**: archived/deleted/permission_denied/not_found in UnifiedChatPanel. **Known limits**: (1) lexical only, (2) ILIKE not FTS, (3) manual purge, (4) E7 partial retrieval needs P34 per-item decisions, (5) E10 export not implemented, (6) no delete-my-data UI, (7) attachment UI (picker/preview) not built. |
