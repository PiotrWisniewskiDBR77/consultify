...
| P35-C | verified(evidence) | (see commit) | Integration: 18 tests with strict assertions (lifecycle, search, cursor pagination, deep-links, bulk validation, team permissions, regression). Store: 10 tests (grouping, actions, search, folder separation). | 10-step staging proof: create/rename/pin/move/archive/unarchive/delete/search/deep-link/team-denial. | **Canonical model complete**: sessions, attachments, metadata (schema-level). **P34 consumed**: read permission checked before list/search. **Degraded posture**: partial results, rate limit, cursor reset, deep-link deleted/archived states. **Known limits**: (1) lexical only, (2) ILIKE not FTS, (3) manual purge, (4) generic 403, (5) no delete-my-data UI, (6) session/attachment schema-only (no runtime population), (7) write conflict detection not implemented (no ETag/version). |
