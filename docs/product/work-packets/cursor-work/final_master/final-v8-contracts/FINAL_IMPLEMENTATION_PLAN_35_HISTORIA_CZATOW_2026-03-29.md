...
| P35-B | delivered | (see commit) | **Integration**: conversations.p35-history.test.ts — 18 tests: lifecycle CRUD (create/patch/delete/force-purge), server-side search (cursor pagination, filters, validation), list cursor pagination, deep-link states (404 with state), archive vs delete semantics, bulk operations (validation + execution), team permissions (no leakage), chat projects (folders + validation), regression guards (no soft-deleted in list, summarize uses correct table). **Store**: useConversationStore.p35-history.test.ts — 10 tests: grouping, actions, serverSearch, chatFolderId vs projectId. Tests use strict status assertions (no 500 acceptance). | **10-step staging proof**: (1) create 10 conversations, (2) rename, (3) pin/star, (4) move into folders via DnD, (5) archive 2 + verify distinct from delete, (6) unarchive, (7) delete with destructive confirmation + soft-delete grace, (8) server-side search with filters + cursor pagination, (9) deep-link to archived thread, (10) team folder permission denial 403 + no leakage. | **Canonical model**: sessions table (`conversation_sessions`), attachment pointers (`conversation_message_attachments`), governance metadata (`visibility_scope`, `access_policy_version`, `client_info`, `retention_policy`), message fields (`session_id`, `edited_at`, `redaction_state`, `safety_flags`, `language`, `delivery_state`), composite indexes, audit purge log (`conversation_purge_audit`). **P34 integration**: `checkChatPermission('read')` called before list and search for team scope. **Degraded posture**: partial results indicator, rate limit detection, cursor reset on invalid. **Known limits**: (1) lexical search only (no semantic), (2) ILIKE not FTS, (3) manual purge (no cron), (4) generic 403 UX, (5) no self-service delete-my-data UI, (6) session entity is schema-only (no runtime population yet), (7) attachment model is schema-only (no UI for adding attachments yet). |
| P35-C | verified(evidence) | (see commits) | **Integration**: 30 tests — lifecycle CRUD, server-side search (cursor, filters, scopeBlocked), list cursor pagination, deep-link states, archive vs delete, bulk ops, attachment CRUD, session management, write conflict (409), export (JSON/markdown/date-range), partial retrieval (E7), team permissions, chat projects, regression guards. **Store**: 22 tests — grouping, lifecycle actions, serverSearch (success + degraded + scopeBlocked), local search, archive/unarchive/delete with API, write conflict (expectedVersion + 409 refresh), deep-link state, notifyModelChange, exportConversation, purgeConversation, folder/project separation. | 10-step staging proof: create/rename/pin/move/archive/unarchive/delete/search/deep-link/team-denial. | **100% canonical model**: sessions (runtime + CRUD + model-change hook), attachments (CRUD + auto-persist from chat + degraded), metadata, message fields. **P34 consumed**: read permission before list/search. **All 10 degraded scenarios covered**: E1 gateway (P34 check), E2 partial results, E3 cursor reset, E4 permission denial UX, E5 deep-link states, E6 attachment degraded, E7 scope-blocked badge, E8 rate limit, E9 write conflict (409 + auto-refresh), E10 export with date-range enforcement. **Self-service**: export (JSON/markdown/text), purge (delete-my-data with audit trail). **State banners**: archived/deleted/permission_denied/not_found in UnifiedChatPanel. **Known limits (operational)**: (1) lexical search only (no semantic ranking), (2) ILIKE not FTS (adequate for current scale), (3) grace window purge is manual (no cron). |
