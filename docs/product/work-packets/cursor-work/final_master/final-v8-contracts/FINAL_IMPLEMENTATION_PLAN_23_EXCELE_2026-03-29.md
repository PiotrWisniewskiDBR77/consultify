...
| P23-D (ext) | verified(evidence) | this commit on `ws/c-artifact-evidence` | **Intelligent Workbook Builder**: universal prompt→multi-sheet Excel engine. **WorkbookSchema** (Zod-validated): sheets[], columns[], rows[], formulas, styles, merges, freeze panes. **WorkbookBuilder** (ExcelJS): multi-sheet .xlsx with real Excel formulas (=SUM, cross-sheet refs like ='Assumptions'!B3*1.05), professional formatting (bold headers, number formats, alternating rows, borders), merged cells, freeze panes, tab colors, cell comments. **WorkbookGeneratorService**: LLM generates WorkbookSchema JSON → validate → build → retry loop (2 attempts + schema repair). **API**: POST /api/workbook/generate, GET /api/workbook/:id/download, POST /api/workbook/generate-and-download, GET /api/workbook/list. **KIMI pipeline**: ExceleView tries workbook generation first, falls back to table export; download handler prefers workbook. **Tests**: 14/14 pass (schema validation, multi-sheet builder, formulas, merges, validation errors, API endpoints). | Staging proof: (1) User types prompt in Excele chat; (2) System calls LLM to generate WorkbookSchema; (3) ExcelJS builds multi-sheet .xlsx with formulas; (4) Preview shows sheet names + KPIs; (5) Download delivers real .xlsx; (6) Works for any domain: budgets, project plans, risk matrices, competitive analyses, recruitment plans. | Domain-agnostic: same engine for finance, HR, ops, strategy. Known limits: no charts (ExcelJS chart API limited); no iterative validation loop with LLM (single generate + retry); no web research auto-integration (researchContext must be passed explicitly). |
