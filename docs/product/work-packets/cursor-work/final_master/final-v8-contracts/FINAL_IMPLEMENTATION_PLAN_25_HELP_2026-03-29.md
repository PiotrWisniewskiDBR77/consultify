...
| P25-D | delivered | pending | Vitest: `help.routes.test.ts` (5 tests: happy-path, empty KB, PL locale normalization, EN fallback, bilingual rationale); migration: `20260409_p25d_help_seed_and_lifecycle.sql` | N/A | Teresa integration: kickoff on full-screen chat, help metadata preserved in workspaceContext, helpModuleId passed to KB retrieval; UX fixes: related articles navigate, share button copies deep link, search shows EN-only badges, no-results shows alternatives+categories; content: help-overview + help-search-guide seed articles (EN+PL), enriched primers, lifecycle columns (last_reviewed_at, content_owner, redirect_to_slug); overclaim fix: MODULE_KNOWLEDGE replaced with MODULE_CONTEXT_HINTS. |
