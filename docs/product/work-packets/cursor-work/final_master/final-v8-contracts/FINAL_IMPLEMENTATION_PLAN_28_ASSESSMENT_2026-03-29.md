...
| P28-C | verified(evidence) | `PLACEHOLDER_P28C_COMMIT` | P28-B + `assessmentWorkbench.p28c-regression.test.ts` (completed read-only, promotion guard); smoke `smoke-p28-workbench-c.ts` | `final_master/evidence/P28_C_VERIFICATION_ROLLOUT_2026-03-31.md`; `npx tsx server/scripts/smoke-p28-workbench-c.ts` | Rollback: disable client calls to workbench mutations; `p28_workbench_v1` read-only for completed runs; promoted trace != automatic P19/P10 artifact |
