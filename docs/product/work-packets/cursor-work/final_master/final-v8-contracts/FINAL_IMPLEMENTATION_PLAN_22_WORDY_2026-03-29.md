...
| P22-B | delivered | `75d85e36ce`, `fe6a7a8db4`, `5773732f5d` on `ws/c-artifact-evidence` | **Split-screen**: KimiWorkspaceShell (chat 420px left ↔ artifact preview right); WordyView wraps shell with lane="wordy". **Pipeline**: useKimiArtifactPipeline wired to V8 artifact run lifecycle (snapshot→plan→preflight→accept→review→approve→materialize→generate). **Auto-trigger**: chat first user+AI exchange auto-starts pipeline. **Preview**: PDF iframe via `/api/report-builder/{id}/export/pdf` after `Api.post('/report-builder/{id}/generate')`. **Download**: PDF export endpoint. **Progress**: 8-step TaskProgressBar mapped from ArtifactRunRecord states. **Replay**: re-runs startGeneration(lastGoal) server-side. **Remix**: resets to goal input. **Sidebar**: AppView.WORDY in menuConfig. **Reopen**: artifactLinks report→`/wordy?artifactId=`. **Routes**: `/wordy` in routeConfig + AppRoutes. | KIMI evidence screenshots mapped (split-screen anatomy, task progress, completion state, preview posture, download posture, chat summary). Runtime shell functional end-to-end with real V8 pipeline. | Bounded: PDF preview depends on report-builder generate completing; track changes/comments/editor model blocked by MISSING INPUT in evidence mapping; optional save-to-Outputs creates registry entry but no explicit "Save" button UX. No silent apply; provenance via P18/P19. |
| P22-B (gap closure, 2026-04-11) | delivered | Gap closure commit on `main` | **Reopen fix (G4)**: WordyView now reads `?artifactId=` via useSearchParams, fetches report via `Api.get('/report-builder/{id}')`, and shows PDF preview — Outputs Library → Wordy loop restored. **PDF URL fix (G5)**: Removed spurious `/reports/` segment from all PDF/DOCX export URLs in WordyView and useKimiArtifactPipeline (`/api/report-builder/{id}/export/pdf` now matches backend route). **All-files link fix (G6)**: Changed dead `/results` link to `/presentations` (Outputs Library). **Double-chat fix (U1)**: Added `AppView.WORDY`, `AppView.EXCELE`, `AppView.PREZENTACJE_GEN` to `VIEWS_WITHOUT_CHAT_PANEL` in MainLayout to prevent duplicate chat panels. **Module header (U3)**: Added breadcrumb strip (icon + lane label + "/ Workspace") to KimiWorkspaceShell right pane. **i18n (G7)**: Added `sidebar.wordy`, `sidebar.excele`, `sidebar.prezentacje` + full `kimi.*` namespace (24 keys) in EN and PL. | New tests added (see P22-C gap closure row). | Remaining known limits unchanged: track changes/comments/editor model still blocked by MISSING INPUT. |
| P22-C | verified(evidence) | `5773732f5d` + this commit on `ws/c-artifact-evidence` | **Integration**: split-screen chat↔doc → generate via pipeline → PDF preview in iframe → download PDF → Replay (server re-run) → Remix (new goal). **Regression**: export failure → toast error + retry; no ghost outputs (cleanupGhostOutputsByOrigin on failure). **Contract**: artifact payload includes provenance/run info via v8_artifact_runs + v8_output_artifacts + v8_artifact_origin_links (P18). **Rollback**: disable Wordy sidebar entry + route; preserve read-only Outputs library access + PDF export. | Staging proof: (1) Open /wordy → split-screen renders; (2) Type goal in right pane textarea → click Generate → pipeline starts (8 steps visible); (3) Chat auto-trigger: send message → AI responds → pipeline auto-starts; (4) Progress bar shows snapshot→plan→…→generate content; (5) PDF preview loads in iframe after generate; (6) Download button opens PDF in new tab; (7) Replay re-runs pipeline; Remix resets to goal input; (8) Sidebar entry "Wordy" navigates to /wordy. | Known limits: track changes/comments/editor model not implemented (MISSING INPUT in evidence mapping §4.4); optional save-to-Outputs has no explicit "Save" button (registry entry created at materialize); PDF preview requires report-builder generate to complete (may show fallback card if slow); no cancel-mid-generation observed in KIMI reference. |
| P22-C (gap closure, 2026-04-11) | verified(evidence) | Gap closure commit on `main` | **Dedicated integration test (G1)**: `tests/integration/routes/wordy-p22.pipeline.test.ts` — document artifact pipeline (create → preflight → accept → materialize), ghost cleanup on failure, retry with history tracking (5 test cases, sqlite integration). **E2E smoke test (G2)**: `tests/e2e/smoke/deploy-gate-wordy.spec.ts` — report-builder health, 404 handling, /wordy route loads without console errors (4 test cases, Playwright). | All gap closure tests green; UI/UX compliance improved per Canon V3. | Resolved gaps: G1 (dedicated tests), G2 (E2E smoke), G4 (reopen), G5 (PDF URL), G6 (all-files link), G7 (i18n), U1 (double-chat), U3 (module header). G3 (staging recording) not addressed — requires manual session. |
| P22-D (deep integration, 2026-04-11) | delivered | Deep integration commit on `main` | **W1 Workspace identity**: KimiWorkspaceShell maps lanes to correct AppView (WORDY/EXCELE/PREZENTACJE_GEN) instead of AI_CHAT; getDefaultWorkspaceType returns `document`/`artifact`. **W2 Org context**: WordyView reads `currentOrganization` and appends org name to system prompt. **W3 Snapshot enrichment**: sourceContextRefs populated with org profile + project refs; artifactRefs pre-populated with recent user artifacts (W7). **W4 Sidebar handoff**: V8ArtifactRunControl "Continue in Wordy" button after completed document run. **W5 Expand routing**: MainLayout expand routes to /wordy when workspace type is `document`. **W6 Report provenance**: artifactRegistryService backfills contextSnapshotId + executionRunId on report artifact (matching presentation branch). **W8 Execution Hub**: "Generate in Wordy" button on report preview footer. **W9 My Work**: Pipeline completion posts best-effort notification to `/api/mywork/items`. **W10–W14 UI/UX Canon**: All rounded-* replaced with rounded-hig-* tokens; nested buttons fixed (a11y); aria-expanded/aria-controls/aria-label/role=tablist added; responsive breakpoints confirmed (flex-col lg:flex-row); border opacity aligned with Canon (border-slate-200/60, dark:border-white/5); unused conversationId prop removed. | All 15 integration gaps addressed. UI/UX Canon V3 compliance strengthened. | Known limits unchanged: track changes/comments/editor model blocked by MISSING INPUT; full assessment context injection remains Reports Builder domain. |
