...
| 29 | Program partnerski | `final-v8-contracts/FINAL_IMPLEMENTATION_PLAN_29_PROGRAM_PARTNERSKI_2026-03-29.md` | P29-A / P29-B / P29-C | verified(evidence) — P29-B rollout: `getProgramStatusDetail` + `whatNext`/`hold` na GET partner + superadmin status; błędy lifecycle z opcjonalnym `whatNext`; `buildPartnerWhatNextGuidance` + testy w `p28-p29.program-assessment.contract.test.ts`; smoke `server/scripts/smoke-p29-partner-program-b.ts`; evidence `final_master/evidence/P29_B_ROLLOUT_2026-03-31.md`; reszta jak wcześniej (ledger, runtime, trasy) |
