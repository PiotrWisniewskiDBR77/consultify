...
| 04 | KPI | `final-v8-contracts/FINAL_IMPLEMENTATION_PLAN_04_KPI_2026-03-29.md` | P04-A / P04-B / P04-C | verified(evidence) — frozen vocabulary canon (`KpiSignal`/`KpiTarget`/`KpiTrend`/`KpiReport`/`KpiReconciliation`/`KpiNextAction`); 6 workflow endpoints (signals/inspect/next-action/health/org-health/contract); degraded posture (missing_data/stale_data/discrepancy_unresolved/linkage_unavailable/permission_denied); permissions matrix (kpi_owner/finance_owner/viewer/commenter); linkage patterns (interpretation/driver/review/realization); anti-duplicate gates; closed-loop workflow states+transitions; 12/12 acceptance checklist; tests: 30 new + 113 existing = 143 total; evidence `final_master/evidence/P04_VERIFIED_CLOSEOUT_2026-03-31.md` |
| 05 | Finanse | `final-v8-contracts/FINAL_IMPLEMENTATION_PLAN_05_FINANSE_2026-03-29.md` | P05-A / P05-B / P05-C | verified(evidence) — Finance Lane E2E closure: import→analysis→mutation→readback (4-step bounded lane); 6 import outcomes + 4 mutation outcomes (error taxonomy §2.3.4); mutation audit (who/what/when); versioning current vs actual + explicit switchover; KPI↔Finance coherence check (coherent/stale/unavailable); 10 degraded scenarios §2.3.6; 11 new endpoints `/api/v8/finance/lane/*` + `/versions/*`; `financeCanon.ts` (P05 acceptance checklist 12/12, ownership boundary, anti-duplicate rules); tests: 26 integration + 36 existing = 62 total + contract + smoke; evidence `final_master/evidence/P05_BC_VERIFICATION_2026-03-31.md` |
| 06 | Radar | `final-v8-contracts/FINAL_IMPLEMENTATION_PLAN_06_RADAR_2026-03-29.md` | P06-A / P06-B / P06-C | verified(evidence) — Radar Triage Cockpit: 5 stable categories (no Misc); deterministic P0/P1/P2 ranking grammar (score formula + 4 hard-gate rules + tie-breakers); why-now payload (rationale+evidence+uncertainty); handoff to Inicjatywy/Wdrożenia/Notatki with full `radar_handoff_context`; 5 triage states (ready/degraded_missing_data/degraded_conflict/degraded_stale/blocked_permission); anti-duplicate gate; 4 endpoints `/api/v8/radar-triage/*`; tests: contract + smoke; evidence `final_master/evidence/P06_BC_VERIFICATION_2026-03-31.md` |
