...
| 2026-03-06 | **FINAL BATCH**: IDEA-06 + ORG-02,03,04 + AI-04,08 (6 tasków) | **V4-IDEA-06**: `idea_exports` table; export mindmap (PDF/outline/markdown) + whiteboard (PDF/PNG) z watermarking, async status tracking (pending→completed/failed). **V4-ORG-02**: `benchmark_cohort_policies` + `benchmark_query_audit` tables; cohort privacy z min N enforcement, suppression, noise/rounding, per-query audit logging. **V4-ORG-03**: `benchmark_framework_mappings` table; SIRI/ADMA/DRD/ISO framework dimensions z percentiles (P25/P50/P75/P90), "what good looks like" descriptions. **V4-ORG-04**: `benchmark_gap_analyses` + `benchmark_gap_initiative_links` tables; pipeline benchmark→gap identification→auto initiative creation, gap-to-initiative linkage. **V4-AI-04**: `ai_typed_actions` table; propose→accept→execute workflow z preview/diff, RBAC required role, idempotency keys, execution result tracking. **V4-AI-08**: `ai_domain_playbooks` + `ai_playbook_executions` tables; Strategy/Decisions/RAID/Stakeholders/Results domain playbooks z system prompts, example queries, required context, output schemas, versioning, execution logging z citations + confidence. 1 service, 1 route file, 30+ REST endpoints, 30+ frontend API methods. Ideas: 9/9, Organization: 9/9, AI Advisor: 8/8. **Dashboard: 120/120 impl — ALL V4 TASKS IMPLEMENTED.** |
| 2026-03-06 | R1 batch 8: INIT-04,06,07 + TOOL-03,06,07 + ENT-05,08 (8 tasków) | **V4-INIT-04**: `goals` + `goal_initiative_links` tables; Goals/OKR spine z hierarchy (parent_goal_id), progress tracking, rollup do initiatives z contribution_weight. **V4-INIT-06**: `initiative_ai_blueprints` table; AI blueprint generator z WBS/milestones/deps/resources jako proposal + citations, propose→apply/reject workflow. **V4-INIT-07**: `initiative_governance_gates` + `initiative_decision_links` tables; phase gates z required decisions + RAID status, evaluateGate (checks decision workflow_status), decision-initiative linkage. **V4-TOOL-03**: `tool_template_library` table; SWOT/PESTLE/Porter/Journey/BCG/OKR templates, org-curated + system templates, versioning, category filtering. **V4-TOOL-06**: `tool_knowledge_bank` + `tool_rag_queries` tables; scoped knowledge entries (session/organization), text search, RAG query logging z citations. **V4-TOOL-07**: `tool_entitlements` + `tool_usage_log` tables; licensed packs per org, checkEntitlement (monthly limit enforcement), usage stats. **V4-ENT-05**: `integration_connectors` + `integration_queue` + `integration_secrets` tables; connector registry z health checks, message queue z retry/backoff, secrets vaulting z rotation tracking, domain allowlists. **V4-ENT-08**: `observability_metrics` + `observability_slos` + `observability_traces` + `observability_dr_drills` tables; metric recording, SLO tracking z budget, distributed tracing (trace/span), DR drill management. 3 serwisy, 3 route files, 60+ REST endpoints, 50+ frontend API methods. Tools: 7/7, Initiatives: 7/7, Enterprise: 8/8. Dashboard: 99→107/120 impl. |
...
| V4-IDEA-06 | Export mindmap: PDF + outline/markdown; whiteboard: PDF/PNG z watermarkingiem | draft | impl | not_tested | — | P1 |
...
| V4-TOOL-03 | Biblioteka templates: SWOT/PESTLE/Porter/Journey/BCG/OKR; org-curated, versioning | draft | impl | not_tested | — | P1 |
...
| V4-TOOL-06 | Tool knowledge bank + scoped RAG: citations do interviews/notes/evidence | draft | impl | not_tested | — | P0 |
| V4-TOOL-07 | Entitlement model: licensed packs per org, policy enforcement | draft | impl | not_tested | V4-ENT-04 | P0 |
...
| V4-INIT-04 | Goals/OKR spine: obiekty Goal z rollup do initiatives; alignment UI | draft | impl | not_tested | — | P0 |
| V4-INIT-05 | Staffing plan: roles, allocations, skills; integracja z capacity model | draft | impl | not_tested | V4-TASK-06 | P0 |
| V4-INIT-06 | AI initiative blueprint generator: WBS/milestones/deps/resources jako proposal + citations | draft | impl | not_tested | — | P0 |
| V4-INIT-07 | Decision governance + RAID gates z initiative readiness | draft | impl | not_tested | — | P0 |
...
| V4-ENT-05 | Integration hub: connector registry, queue/retry, secrets vaulting, allowlists | draft | impl | not_tested | — | P0 |
...
| V4-ENT-08 | Observability: metrics, traces (OpenTelemetry), SLOs, DR drills, security hardening | draft | impl | not_tested | — | P0 |
...
| V4-ORG-02 | Cohort privacy: min N, suppression, noise/rounding, audit dla benchmark queries | draft | impl | not_tested | V4-ORG-01 | P0 |
| V4-ORG-03 | Framework mappings: SIRI/ADMA/DRD/ISO — percentiles, "what good looks like" | draft | impl | not_tested | V4-ORG-01 | P0 |
| V4-ORG-04 | Pipeline benchmark→gap→initiatives: automatyczne programy naprawcze | draft | impl | not_tested | V4-ORG-01 | P0 |
...
| V4-AI-04 | Typed actions framework: propose z preview/diff → accept → execution (RBAC, idempotency) | draft | impl | not_tested | V4-AI-01, V4-ENT-03 | P0 |
...
| V4-AI-08 | Domain playbooks: Strategy, Decisions, RAID, Stakeholders, Results | draft | impl | not_tested | V4-AI-01 | P0 |
