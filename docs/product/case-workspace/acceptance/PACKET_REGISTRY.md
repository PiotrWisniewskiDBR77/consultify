...
| CW-P10 | E9 | E4 | — (v8_agent_work_graphs/branch_tasks stay untouched, read-only-adjacent) | `case_workspace_gateway_evaluations` + `case_workspace_node_result_acceptances`, `executionGraphService.ts`, 8 methods — **IMPLEMENTED, migration+idempotent-rerun+esbuild+v8-untouched verified**, commit `1920cdf1ff`. Renumbered from CW-P09. Scope: deterministic gateway/acceptance FACT recording only, not an execution engine; timeout/compensation and the UI-facing rows of E9 (CW-02-*/CW-03-*, ~20 of 26 E9 rows) explicitly out of scope, blocked at W2-V0. | subset of 119 |
