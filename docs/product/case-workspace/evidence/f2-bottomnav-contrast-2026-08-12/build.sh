...
# recompiles against the CURRENT tailwind.config.js + src/index.css of the repo
# this evidence directory lives in, so results never go stale/lie.
#
...
echo "==> Compiling real Tailwind CSS (project tailwind.config.js + src/index.css)..."
# tailwind.config.js's `content` globs are relative (./src/**/*, ...), so the CLI must
