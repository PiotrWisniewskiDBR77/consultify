...
const rows = docs.map((d) => {
  const find = (id: string) => {
    for (const t of d.targets)
      for (const l of t.topMappedLines) if (l.canonicalId === id) return l.value;
    return null;
  };
  ...
  return {
    ...
    perStatement: d.targets.map((t) => ({
      type: t.statementType,
      period: t.selectedPeriodLabel,
      comparison: t.comparisonPeriodLabel,
      extracted: t.extractedLineCount,
      mapped: t.mappedLineCount,
      coveragePct: t.coveragePct,
      readiness: t.readinessStatus,
...
function misparseStats(d: Doc) {
  ...
  const examples: Array<{
    ...
    canonicalId: string;
    value: number;
    valueIfCommaWasThousandsSeparator: number;
  }> = [];
  for (const t of d.targets) {
      ...
      if (frac > 1e-9) {
        ...
        if (examples.length < 6)
          examples.push({
            ...
            value: l.value,
            valueIfCommaWasThousandsSeparator:
              Number(String(Math.abs(l.value)).replace('.', '')) * Math.sign(l.value),
          });
      }
    }
  }
  return {
    ...
    fractionalPct: total === 0 ? 0 : Math.round((fractional / total) * 1000) / 10,
    examples,
  };
}
...
const availability = NAMED_IN_SECTION_13.map((name) => ({
  ...
  matchingDocuments: docs
    .filter((d) => d.label.toLowerCase().includes(name.toLowerCase()))
    .map((d) => d.label),
}));
...
const out = {
  ...
  companies: rows,
  currencyDetectionDefects: currencyDefects.map((r) => ({
    label: r.label,
    declared: r.declaredCurrency,
    expected: r.expectedCurrency,
  })),
  thousandsSeparatorMisparse: rows
    .filter((r) => r.separatorMisparse.fractionalPct >= 30)
    .map((r) => ({
      label: r.label,
      declaredScaling: r.declaredScaling,
      fractionalPct: r.separatorMisparse.fractionalPct,
      totalLines: r.separatorMisparse.totalLines,
      examples: r.separatorMisparse.examples,
    })),
};
...
for (const a of availability)
  console.log(
    `  ${a.companyNamedInHandoffSection13.padEnd(12)} present=${a.presentInRepo} ${a.matchingDocuments.join(', ')}`
  );
console.log('\n=== per-company declared scale / currency ===');
for (const r of rows) {
  console.log(`  ${r.label}`);
  console.log(
    `      currency declared=${r.declaredCurrency} expected=${r.expectedCurrency ?? '?'} plausible=${r.currencyPlausible} | scaling=${r.declaredScaling} (x${r.unitMultiplier})`
  );
  console.log(
    `      revenue raw=${r.revenueRaw ?? 'n/a'} -> full ${r.revenueFullUnits === null ? 'n/a' : r.revenueFullUnits.toLocaleString('en-US')} | totalAssets raw=${r.totalAssetsRaw ?? 'n/a'} -> full ${r.totalAssetsFullUnits === null ? 'n/a' : r.totalAssetsFullUnits.toLocaleString('en-US')}`
  );
}
console.log(
...
for (const r of rows)
  console.log(
    `  ${r.label.padEnd(40)} ${String(r.separatorMisparse.fractionalPct).padStart(5)}% of ${r.separatorMisparse.totalLines} lines (declared scaling: ${r.declaredScaling})`
  );
console.log(`\n=== currency-detection defects: ${currencyDefects.length} ===`);
for (const d of currencyDefects)
  console.log(
    `  ${d.label}: declared ${d.declaredCurrency}, issuer reports in ${d.expectedCurrency}`
  );
