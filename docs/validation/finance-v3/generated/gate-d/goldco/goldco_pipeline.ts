...
if (
  ...
  throw new Error(
    'goldco_pipeline.ts requires RUN_DB_TESTS=1 MOCK_DB=false DATABASE_URL=postgresql://... against an ephemeral cluster — refusing to run against an ambiguous/default target.'
  );
}
...
async function main() {
  ...
  const monthPeriods: string[] = [];
  {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      const pid = await makeMonthPeriod(
        2025,
        m,
        start,
        end,
        `2025-M${String(m).padStart(2, '0')}`,
        prev
      );
  ...
  record(
    `[periods] FY2023=${periodFY2023} FY2024=${periodFY2024} FY2025=${periodFY2025} + 12 month periods for FY2025`
  );
  ...
  await makeCustomLine(
    `fsl-goldco-cta-oci-${randomUUID()}`,
    'BS',
    'CTA_OCI',
    'Cumulative Translation Adjustment (OCI)'
  );
  await makeCustomLine(
    `fsl-goldco-icloan-${randomUUID()}`,
    'BS',
    'INTERCOMPANY_LOAN',
  ...
  async function mapReconcileApprove(
    ...
    rules: any[],
    label: string,
    opts: { consolidationScope?: string } = {}
  ) {
    const scopedRules = opts.consolidationScope
      ? rules.map((r) => ({ ...r, consolidationScope: opts.consolidationScope }))
      : rules;
    ...
    const recon = await statementReconciliationService.runReconciliation({
      ...
      artifactId,
      businessVersionId,
      sourceSystem: 'goldco:vertical_slice',
  ...
  const entParentFY2023 = await makeEntity(
  ...
  const entParentFY2024 = await makeEntity(
  ...
  if (resParentFY2024Orig.approved?.ok) {
    ...
    const reopened = await artifactVersionService.reopenVersion({
      ...
      actorId: approverId,
      role: 'approver',
      expectedVersion: origApprovedVersion,
    ...
    if (!reopened.ok) {
      ...
      record(
        `  [PARENT FY2024 RESTATED] reopened vN+1=${newBvId} (parent=${origBvId}), status=${reopened.businessVersion.status}, version_kind=${reopened.businessVersion.version_kind}`
      );
      ...
      if (
        ...
        throw new Error(
          `BUG-GOLDCO-01 REGRESSION: reopenVersion() did not persist version_kind/restatement_reason/restatement_class as requested — got ${JSON.stringify({ version_kind: reopened.businessVersion.version_kind, restatement_reason: reopened.businessVersion.restatement_reason, restatement_class: reopened.businessVersion.restatement_class })}`
        );
      }
      record(
      ...
      const confirmKind = await withPinnedPostgresTransaction((tx: Tx) =>
        tx.queryOne(
          `SELECT version_kind, restatement_reason, restatement_class FROM finance_business_versions WHERE business_version_id = ?`,
          [newBvId]
        )
      );
      record(
        `  [PARENT FY2024 RESTATED] version_kind read back from DB: ${JSON.stringify(confirmKind)}`
      );
      if ((confirmKind as any)?.version_kind !== 'RESTATED') {
        throw new Error(
          `BUG-GOLDCO-01 REGRESSION: DB row does not show version_kind='RESTATED': ${JSON.stringify(confirmKind)}`
        );
      }
      ...
      if (!resRestated.approved?.ok) {
        throw new Error(
          `BUG-GOLDCO-03 REGRESSION: approveVersion() on the RESTATED version did not reach APPROVED: ${JSON.stringify(resRestated.approved)}`
        );
      }
      record(
        `  [PARENT FY2024 RESTATED] BUG-GOLDCO-03 FIX CONFIRMED: approveVersion() succeeded, status=${resRestated.approved.businessVersion.status}`
      );
      ...
      const origAfter = await withPinnedPostgresTransaction((tx: Tx) =>
        tx.queryOne(
          `SELECT status, superseded_by_version_id FROM finance_business_versions WHERE business_version_id = ?`,
          [origBvId]
        )
      );
      record(`  [PARENT FY2024 ORIGINAL] post-restatement status: ${JSON.stringify(origAfter)}`);
      if (
        (origAfter as any)?.status !== 'SUPERSEDED' ||
        (origAfter as any)?.superseded_by_version_id !== newBvId
      ) {
        throw new Error(
          `BUG-GOLDCO-03 REGRESSION: original vN did not become SUPERSEDED pointing at the restated child: ${JSON.stringify(origAfter)}`
        );
      }
      record(
      ...
      const origLinesStillThere = await withPinnedPostgresTransaction((tx: Tx) =>
        tx.queryOne(
          `SELECT count(*)::int AS n FROM finance_stmt_lines WHERE business_version_id = ?`,
          [origBvId]
        )
      );
      record(
        `  [PARENT FY2024 ORIGINAL] finance_stmt_lines row count (must be unchanged, ${rowsParentFY2024Orig.rawLines.length} expected): ${origLinesStillThere.n}`
      );
    }
  } else {
  ...
  const entParentFY2025 = await makeEntity(
  ...
  const entParentMonthly = await makeEntity(
    packParentMonthly.businessVersion.business_version_id,
    'PARENT',
    { functionalCurrency: 'PLN' }
  );
  ...
  const resParentMonthly = await mapReconcileApprove(
    packParentMonthly.businessVersion.business_version_id,
    packParentMonthly.artifact.artifact_id,
    packParentMonthly.businessVersion.version,
    monthlyRawLines,
    monthlyRules,
    'PARENT FY2025 MONTHLY'
  );
  ...
  record(
    `  [PROBE STANDALONE, ${UNBALANCED_DELTA} PLN off-balance] result: ${JSON.stringify(probeStandalone)}`
  );
  record(
    `  [PROBE CONSOLIDATED, ${UNBALANCED_DELTA} PLN off-balance] result: ${JSON.stringify(probeConsolidated)}`
  );
  ...
  if (probeStandalone.rejected && probeConsolidated.rejected) {
    record(
      `  BUG-GOLDCO-02 FIX CONFIRMED: the PLN ${UNBALANCED_DELTA} imbalance is now rejected at BOTH STANDALONE and CONSOLIDATED scope (previously STANDALONE was silently accepted).`
    );
  } else {
    throw new Error(
      `BUG-GOLDCO-02 REGRESSION: expected both STANDALONE and CONSOLIDATED to reject the unbalanced probe — got standalone.rejected=${probeStandalone.rejected} consolidated.rejected=${probeConsolidated.rejected}`
    );
  }
  ...
  const entGroupParent = await makeEntity(groupBvId, 'PARENT', {
    role: 'GROUP_PARENT',
  ...
  async function readBundleBack(
    ...
    out.retainedEarnings = await readCell(
      businessVersionId,
      entityCode,
      'RETAINED_EARNINGS',
      periodId,
    ...
    out.dividendsDeclared = await readCell(
      businessVersionId,
      entityCode,
      'DIVIDENDS_DECLARED',
      periodId,
  ...
  const pipelineResults: any = {
    ...
    bugs,
    parent: {
      FY2023: await readBundleBack(
        packParentFY2023.businessVersion.business_version_id,
        'PARENT',
        periodFY2023
      ),
      FY2024_original: await readBundleBack(
        packParentFY2024.businessVersion.business_version_id,
        'PARENT',
        periodFY2024
      ),
      FY2024_restated: packParentFY2024Restated
        ? await readBundleBack(packParentFY2024Restated.businessVersionId, 'PARENT', periodFY2024)
        : null,
      FY2025: await readBundleBack(
        packParentFY2025.businessVersion.business_version_id,
        'PARENT',
        periodFY2025
      ),
    ...
    parentMonthly: await Promise.all(
      oracle.parent.FY2025_monthly.map(async (m: any) => {
        ...
        out.netChangeCash = await readCell(
          packParentMonthly.businessVersion.business_version_id,
          'PARENT',
          'NET_CHANGE_CASH',
          periodId,
    ...
    sub: {
      FY2023: await readBundleBack(
        subFY2023.pack.businessVersion.business_version_id,
        'SUB',
        periodFY2023
      ),
      FY2024: await readBundleBack(
        subFY2024.pack.businessVersion.business_version_id,
        'SUB',
        periodFY2024
      ),
      FY2025: await readBundleBack(
        subFY2025.pack.businessVersion.business_version_id,
        'SUB',
        periodFY2025
      ),
    },
    group: {
      pl: {
        ...
        netIncomeConsolidated: await readCell(
          groupBvId,
          'PARENT',
          'NET_INCOME',
          periodFY2025,
      ...
      bs: {
        ...
        totalLiabilities: await readCell(
          groupBvId,
          'PARENT',
          'TOTAL_LIABILITIES',
          periodFY2025,
        ...
        totalLiabilitiesEquity: await readCell(
          groupBvId,
          'PARENT',
          'TOTAL_LIABILITIES_EQUITY',
          periodFY2025,
    ...
    restatement: packParentFY2024Restated
      ? {
          ...
          restatedStatusAfter: await withPinnedPostgresTransaction((tx: Tx) =>
            tx.queryOne(
              `SELECT status, version_kind, restatement_reason, restatement_class, parent_version_id FROM finance_business_versions WHERE business_version_id = ?`,
              [packParentFY2024Restated.businessVersionId]
            )
          ),
        }
      : null,
  ...
  record(
    `[goldco_pipeline] bugs found: ${bugs.length} (${bugs.map((b) => b.id).join(', ') || 'none'})`
  );
}
