...
if (
  ...
  throw new Error(
    'goldco_full_dag.ts requires RUN_DB_TESTS=1 MOCK_DB=false DATABASE_URL=postgresql://... against an ephemeral cluster — refusing to run against an ambiguous/default target.'
  );
}
...
async function main() {
  ...
  const baselineComputeService =
    await import('../../../../../../server/src/services/finance/canonical/baselineComputeService.js');
  const predictionPreflightService =
    await import('../../../../../../server/src/services/finance/canonical/predictionPreflightService.js');
  const predictionComputeService =
    await import('../../../../../../server/src/services/finance/canonical/predictionComputeService.js');
  const valuationComputeService =
    await import('../../../../../../server/src/services/finance/canonical/valuationComputeService.js');
  const valuationWaccService =
    await import('../../../../../../server/src/services/finance/canonical/valuationWaccService.js');
  const valuationTerminalService =
    await import('../../../../../../server/src/services/finance/canonical/valuationTerminalService.js');
  const valuationSensitivityService =
    await import('../../../../../../server/src/services/finance/canonical/valuationSensitivityService.js');
  const valuationBridgeService =
  ...
  await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryRun(`INSERT INTO organizations (id, name) VALUES (?, ?)`, [
      orgId,
      'GoldCo Manufacturing Group (Full DAG)',
    ])
  );
  ...
  if (!engineManifest)
    throw new Error(
      'finance_engine_manifests LEGACY_UNKNOWN sentinel row not found — migration b01 not applied?'
    );
  ...
  async function makeMonthPeriod(
    fiscalYear: number,
    month: number,
    previousPeriodId: string | null
  ) {
  ...
  record(
    `[periods] FY2023=${periodFY2023} FY2024=${periodFY2024} FY2025=${periodFY2025} FY2026=${periodFY2026} FY2027=${periodFY2027} FY2028=${periodFY2028}`
  );
  ...
  await makeCustomLine(
    `fsl-goldco-fd-cta-oci-${randomUUID()}`,
    'BS',
    'CTA_OCI',
    'Cumulative Translation Adjustment (OCI)'
  );
  await makeCustomLine(
    `fsl-goldco-fd-icloan-${randomUUID()}`,
    'BS',
    'INTERCOMPANY_LOAN',
    'Intercompany Loan (Parent <-> Sub)'
  );
  ...
  function buildFullPackRows(
    entityCode: string,
    periodId: string,
    currency: string,
    bundle: Bundle,
    opts: { includeCF?: boolean } = {}
  ) {
    const rawLines: any[] = [];
    const rules: any[] = [];
    const push = (
    ...
    const row = await withPinnedPostgresTransaction((tx: Tx) =>
      tx.queryOne(
        ...
        [
          ...
          businessVersionId,
          entityCode,
          `${entityCode} legal name`,
          opts.role ?? 'GROUP_PARENT',
          opts.consolidationMethod ?? 'NOT_CONSOLIDATED',
          opts.ownershipPct ?? null,
          opts.functionalCurrency,
          preparerId,
        ]
      )
    );
  ...
  async function mapReconcileApprove(
    ...
    const badBuckets = mapped.filter(
      (m: any) => m.bucket === 'UNMAPPED' || m.bucket === 'DUPLICATE'
    );
    if (badBuckets.length > 0) {
      record(
        `  [WARN][${label}] ${badBuckets.length} row(s) UNMAPPED/DUPLICATE: ${JSON.stringify(badBuckets.map((b: any) => ({ item: b.raw.lineItem, bucket: b.bucket, reason: b.reasonCode })))}`
      );
    }
    ...
    record(
      `  [${label}] reconciliation status=${recon.run.status} residual=${recon.run.residual} ready=${recon.readiness.ready} transitionOk=${recon.readiness.transitionResult?.ok}`
    );
    ...
    if (!started.ok) {
      record(`  [${label}] start_review FAILED: ${JSON.stringify(started)}`);
      return { mapped, recon, approved: null as any };
    }
    ...
    record(
      `  [${label}] approve ok=${approved.ok}${approved.ok ? '' : ' code=' + (approved as any).code}`
    );
  ...
  await makeEntity(packParentFY2023.businessVersion.business_version_id, 'PARENT', {
    functionalCurrency: 'PLN',
  });
  const rowsParentFY2023 = buildFullPackRows('PARENT', periodFY2023, 'PLN', oracle.parent.FY2023, {
    includeCF: true,
  });
  await mapReconcileApprove(
    packParentFY2023.businessVersion.business_version_id,
    packParentFY2023.artifact.artifact_id,
    packParentFY2023.businessVersion.version,
    rowsParentFY2023.rawLines,
    rowsParentFY2023.rules,
    'PARENT FY2023'
  );

  const packParentFY2024 = await makeStatementPack();
  await makeEntity(packParentFY2024.businessVersion.business_version_id, 'PARENT', {
    functionalCurrency: 'PLN',
  });
  const rowsParentFY2024Orig = buildFullPackRows(
    'PARENT',
    periodFY2024,
    'PLN',
    oracle.parent.FY2024_original,
    { includeCF: true }
  );
  const resParentFY2024Orig = await mapReconcileApprove(
    packParentFY2024.businessVersion.business_version_id,
    packParentFY2024.artifact.artifact_id,
    packParentFY2024.businessVersion.version,
    rowsParentFY2024Orig.rawLines,
    rowsParentFY2024Orig.rules,
    'PARENT FY2024 ORIGINAL'
  );
  ...
  if (resParentFY2024Orig.approved?.ok) {
    ...
    const reopened = await artifactVersionService.reopenVersion({
      organizationId: orgId,
      businessVersionId: origBvId,
      actorId: approverId,
      role: 'approver',
      expectedVersion: resParentFY2024Orig.approved.businessVersion.version,
    ...
    if (reopened.ok) {
      ...
      const rowsRestated = buildFullPackRows(
        'PARENT',
        periodFY2024,
        'PLN',
        oracle.parent.FY2024_restated,
        { includeCF: true }
      );
      const resRestated = await mapReconcileApprove(
        restatedBvId,
        reopened.businessVersion.artifact_id,
        reopened.businessVersion.version,
        rowsRestated.rawLines,
        rowsRestated.rules,
        'PARENT FY2024 RESTATED'
      );
      if (!resRestated.approved?.ok) {
        flag(
          'IF-01',
          `PARENT FY2024 RESTATED did not reach APPROVED: ${JSON.stringify(resRestated.approved)} — BUG-GOLDCO-03 regression?`
        );
      } else {
      ...
      flag(
        'IF-02',
        `reopenVersion() for PARENT FY2024 restatement FAILED: ${JSON.stringify(reopened)}`
      );
    }
  }

  const packParentFY2025 = await makeStatementPack();
  const entParentFY2025Annual = await makeEntity(
    packParentFY2025.businessVersion.business_version_id,
    'PARENT',
    { functionalCurrency: 'PLN' }
  );
  const rowsParentFY2025 = buildFullPackRows('PARENT', periodFY2025, 'PLN', oracle.parent.FY2025, {
    includeCF: true,
  });
  const resParentFY2025 = await mapReconcileApprove(
    packParentFY2025.businessVersion.business_version_id,
    packParentFY2025.artifact.artifact_id,
    packParentFY2025.businessVersion.version,
    rowsParentFY2025.rawLines,
    rowsParentFY2025.rules,
    'PARENT FY2025 ANNUAL'
  );
  ...
  async function subPack(label: string, periodId: string, bundle: Bundle) {
    ...
    await makeEntity(pack.businessVersion.business_version_id, 'SUB', {
      functionalCurrency: 'EUR',
    });
    const rows = buildFullPackRows('SUB', periodId, 'EUR', bundle);
    const res = await mapReconcileApprove(
      pack.businessVersion.business_version_id,
      pack.artifact.artifact_id,
      pack.businessVersion.version,
      rows.rawLines,
      rows.rules,
      `SUB ${label}`
    );
  ...
  await makeEntity(groupBvId, 'PARENT', {
    role: 'GROUP_PARENT',
    consolidationMethod: 'NOT_CONSOLIDATED',
    functionalCurrency: 'PLN',
  });
  const entGroupSub = await makeEntity(groupBvId, 'SUB', {
    role: 'SUBSIDIARY',
    consolidationMethod: 'FULL',
    ownershipPct: 80,
    functionalCurrency: 'EUR',
  });
  await makeEntity(groupBvId, 'ELIM', {
    role: 'ELIMINATION_BUCKET',
    consolidationMethod: 'NOT_CONSOLIDATED',
    functionalCurrency: 'PLN',
  });
  void entGroupSub;
  ...
  const pushGroup = (
    ...
    groupRawLines.push({
      lineItem: label,
      periodId: periodFY2025,
      entityCode,
      currency: 'PLN',
      value,
      sourceRef: { source: 'goldco_oracle.json:groupFY2025' },
    });
  ...
  pushGroup(
    ...
    'P&L',
    'NET_INCOME',
    g.pl.netIncomeConsolidated,
    'CONSOLIDATED'
  );
  pushGroup(
    'PARENT',
    'GROUP:BS:TOTAL_ASSETS',
    'BS',
    'TOTAL_ASSETS',
    g.bs.totalAssets,
    'CONSOLIDATED'
  ...
  pushGroup(
    'PARENT',
    'GROUP:BS:TOTAL_LIABILITIES',
    'BS',
    'TOTAL_LIABILITIES',
    g.bs.totalLiabilities,
    'CONSOLIDATED'
  );
  pushGroup('PARENT', 'GROUP:BS:EQUITY', 'BS', 'EQUITY', g.bs.totalEquity, 'CONSOLIDATED');
  pushGroup(
    'PARENT',
    'GROUP:BS:TOTAL_LIABILITIES_EQUITY',
    'BS',
    'TOTAL_LIABILITIES_EQUITY',
    g.bs.totalLiabilitiesEquity,
    'CONSOLIDATED'
  );
  ...
  pushGroup(
    'SUB',
    'SUBTRANS:BS:TOTAL_ASSETS',
    'BS',
    'TOTAL_ASSETS',
    st.bsPreCTA.totalAssets,
    'CONSOLIDATED'
  );
  pushGroup(
    'SUB',
    'SUBTRANS:BS:TOTAL_LIABILITIES',
    'BS',
    'TOTAL_LIABILITIES',
    st.bsPreCTA.totalLiabilities,
    'CONSOLIDATED'
  );
  pushGroup('SUB', 'SUBTRANS:BS:EQUITY', 'BS', 'EQUITY', st.equityPostCTA, 'CONSOLIDATED');
  pushGroup(
    'SUB',
    'SUBTRANS:BS:TOTAL_LIABILITIES_EQUITY',
    'BS',
    'TOTAL_LIABILITIES_EQUITY',
    st.totalLiabilitiesEquity,
    'CONSOLIDATED'
  );
  pushGroup('SUB', 'SUBTRANS:BS:CTA_OCI', 'BS', 'CTA_OCI', st.cta, 'CONSOLIDATED');
  groupRawLines.push({
    lineItem: 'ELIM:PARENT:INTERCOMPANY_LOAN',
    periodId: periodFY2025,
    entityCode: 'PARENT',
    currency: 'PLN',
    value: -oracle.intercompany.loanPLN,
    sourceRef: { leg: 'parent_receivable' },
  });
  groupRules.push({
    sourceLabel: 'ELIM:PARENT:INTERCOMPANY_LOAN',
    statementType: 'BS',
    lineCode: 'INTERCOMPANY_LOAN',
    consolidationScope: 'ELIMINATION',
    signConvention: 'NATURAL',
    eliminationCounterpartyEntityCode: 'SUB',
  });
  groupRawLines.push({
    lineItem: 'ELIM:SUB:INTERCOMPANY_LOAN',
    periodId: periodFY2025,
    entityCode: 'SUB',
    currency: 'PLN',
    value: -oracle.intercompany.loanPLN,
    sourceRef: { leg: 'sub_payable' },
  });
  groupRules.push({
    sourceLabel: 'ELIM:SUB:INTERCOMPANY_LOAN',
    statementType: 'BS',
    lineCode: 'INTERCOMPANY_LOAN',
    consolidationScope: 'ELIMINATION',
    signConvention: 'CONTRA',
    eliminationCounterpartyEntityCode: 'PARENT',
  });
  const resGroup = await mapReconcileApprove(
    groupBvId,
    packGroup.artifact.artifact_id,
    packGroup.businessVersion.version,
    groupRawLines,
    groupRules,
  ...
  const entBaselineSource = await makeEntity(baselineSourceBvId, 'PARENT', {
    functionalCurrency: 'PLN',
  });
  ...
  for (const m of oracle.parent.FY2025_monthly) {
    ...
    if (m.month === 12) {
      ...
      bsRawLines.push({
        lineItem: 'PARENT:DEC2025:retainedEarnings',
        periodId: decPeriodId,
        entityCode: 'PARENT',
        currency: 'PLN',
        value: oracle.parent.FY2025.closingRE,
        sourceRef: {},
      });
      bsRules.push({
        sourceLabel: 'PARENT:DEC2025:retainedEarnings',
        statementType: 'BS',
        lineCode: 'RETAINED_EARNINGS',
      });
      bsRawLines.push({
        lineItem: 'PARENT:DEC2025:dividendsDeclared',
        periodId: decPeriodId,
        entityCode: 'PARENT',
        currency: 'PLN',
        value: oracle.parent.FY2025.dividendsDeclared,
        sourceRef: {},
      });
      bsRules.push({
        sourceLabel: 'PARENT:DEC2025:dividendsDeclared',
        statementType: 'BS',
        lineCode: 'DIVIDENDS_DECLARED',
      });
      bsRawLines.push({
        lineItem: 'PARENT:DEC2025:cfo',
        periodId: decPeriodId,
        entityCode: 'PARENT',
        currency: 'PLN',
        value: oracle.parent.FY2025.cfo,
        sourceRef: {},
      });
      bsRules.push({ sourceLabel: 'PARENT:DEC2025:cfo', statementType: 'CF', lineCode: 'CFO' });
      bsRawLines.push({
        lineItem: 'PARENT:DEC2025:cfi',
        periodId: decPeriodId,
        entityCode: 'PARENT',
        currency: 'PLN',
        value: oracle.parent.FY2025.cfi,
        sourceRef: {},
      });
      bsRules.push({ sourceLabel: 'PARENT:DEC2025:cfi', statementType: 'CF', lineCode: 'CFI' });
      bsRawLines.push({
        lineItem: 'PARENT:DEC2025:cff',
        periodId: decPeriodId,
        entityCode: 'PARENT',
        currency: 'PLN',
        value: oracle.parent.FY2025.cff,
        sourceRef: {},
      });
  ...
  const resBaselineSource = await mapReconcileApprove(
    baselineSourceBvId,
    packBaselineSource.artifact.artifact_id,
    packBaselineSource.businessVersion.version,
    bsRawLines,
    bsRules,
    'PARENT FY2025 BASELINE-SOURCE (monthly + Dec closing BS)'
  );
  if (!resBaselineSource.approved?.ok) {
    throw new Error(
      `Baseline-source Statement Pack Version did not reach APPROVED — cannot proceed: ${JSON.stringify(resBaselineSource.approved)}`
    );
  }
  ...
  record(
    `[STATEMENT_TO_ANALYSIS] ok=${edgeStmtToAnalysis.ok} source=GROUP FY2025 consolidated (${groupBvId}) -> analysis (${analysisBvId})`
  );

  const catalogRows = await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryAll<{ id: string; kpi_code: string; category: string }>(
      `SELECT id, kpi_code, category FROM finance_analysis_kpi_catalog WHERE status = 'ACTIVE'`
    )
  );
  record(
    `[Analysis catalog] ${catalogRows.length} ACTIVE P0 KPI rows found (18 expected, all UNIVERSAL tier — no separate MANUFACTURING tier exists in the P0 catalog, documented boundary, WP-D03 ADR section 91/375)`
  );
  if (catalogRows.length !== 18)
    flag('IF-03', `Expected 18 P0 KPI catalog rows, found ${catalogRows.length}`);
  ...
  const groupEntityRow = await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryOne<{ id: string }>(
      `SELECT id FROM finance_stmt_entities WHERE business_version_id = ? AND entity_code = 'PARENT'`,
      [groupBvId]
    )
  );
  ...
  if (!computedKpis.ok)
    throw new Error(
      `kpiComputeService.computeAnalysisKpis failed: ${JSON.stringify(computedKpis)}`
    );
  record(
    `[Analysis] computed ${computedKpis.results.length} KPI rows for GROUP FY2025 consolidated:`
  );
  for (const r of computedKpis.results) {
    record(
      `  ${r.kpiCode}: status=${r.status} value=${r.value ?? 'null'} qualityFlag=${r.qualityFlag ?? 'null'}`
    );
  }
  const manufacturingRelevant = [
    'GROSS_MARGIN_PCT',
    'EBITDA_MARGIN_PCT',
    'DIO',
    'DSO',
  ...
  record(
    `[Analysis] manufacturing-relevant subset (inventory/margin cycle): ${manufacturingRelevant.join(', ')}`
  );
  ...
  record(
    `[Analysis] normalized-EBITDA comparability marker recorded via is_adjustment=true row on EBITDA_MARGIN_PCT (see adjustment_reason)`
  );
  ...
  const edgeStmtToModel = await lineageService.insertEdge({
    organizationId: orgId,
    sourceVersionId: baselineSourceBvId,
    sourceArtifactType: 'STATEMENT_PACK',
  ...
  record(
    `[STATEMENT_TO_MODEL] ok=${edgeStmtToModel.ok} source=baseline-source pack (${baselineSourceBvId}) -> baseline (${baselineBvId})`
  );
  ...
  const edgeAnalysisToModel = await lineageService.insertEdge({
    ...
    transformationKind: 'MANUAL_LINK',
    authorId: preparerId,
    assumptionSnapshotHash: assumptionSnapshotHash1,
  });
  record(
    `[ANALYSIS_TO_MODEL] ok=${edgeAnalysisToModel.ok} source=Analysis GROUP FY2025 (${analysisBvId}) -> baseline (${baselineBvId})`
  );
  if (!edgeStmtToModel.ok || !edgeAnalysisToModel.ok) {
    throw new Error(
      `Baseline lineage edges failed: STATEMENT_TO_MODEL=${JSON.stringify(edgeStmtToModel)} ANALYSIS_TO_MODEL=${JSON.stringify(edgeAnalysisToModel)}`
    );
  }
  ...
  record(
    `[Baseline FY2026] ${baselineResult.periodsComputed} periods computed. December: cash=${decResult.cash} netIncome=${decResult.netIncome} qualityFlag=${decResult.qualityFlag}`
  );
  if (decResult.qualityFlag !== 'FUNDING_GAP') {
    flag(
      'IF-05',
      `Expected December 2026 FUNDING_GAP quality flag (negative cash), got qualityFlag=${decResult.qualityFlag} cash=${decResult.cash}`
    );
  } else {
    record(
      `[Baseline FY2026] FUNDING_GAP confirmed on December — negative cash surfaced, never plugged, never blocked (DEC-FIN-002).`
    );
  }
  ...
  record(
    `[Baseline FY2026 annual roll-up] EBIT=${fy2026.EBIT.toFixed(2)} DEPRECIATION=${fy2026.DEPRECIATION.toFixed(2)} CAPEX=${fy2026.CAPEX.toFixed(2)} WORKING_CAPITAL(closing)=${fy2026.WORKING_CAPITAL.toFixed(2)}`
  );

  async function insertContinuationYear(
    periodId: string,
    fiscalYear: number,
    growth: number,
    base: typeof fy2026
    ...
    const grown = {
      EBIT: base.EBIT * (1 + growth),
      DEPRECIATION: base.DEPRECIATION * (1 + growth),
      CAPEX: base.CAPEX * (1 + growth),
      REVENUE: base.REVENUE * (1 + growth),
      NET_INCOME: base.NET_INCOME * (1 + growth),
      WORKING_CAPITAL: base.WORKING_CAPITAL * (1 + growth),
    };
    ...
    record(
      `[Baseline FY${fiscalYear} continuation, +${(growth * 100).toFixed(0)}%/yr off FY2026] EBIT=${grown.EBIT.toFixed(2)} DEPRECIATION=${grown.DEPRECIATION.toFixed(2)} CAPEX=${grown.CAPEX.toFixed(2)} WORKING_CAPITAL=${grown.WORKING_CAPITAL.toFixed(2)}`
    );
    return grown;
  }
  ...
  async function makeScenario(mode: string, name: string) {
    const art = await artifactVersionService.createArtifact({
      organizationId: orgId,
    ...
    const edge = await lineageService.insertEdge({
      ...
      transformationKind: 'MANUAL_LINK',
      authorId: preparerId,
      assumptionSnapshotHash: `sha256:goldco-fulldag-model-to-scenario-${mode}`,
    });
    if (!edge.ok)
      throw new Error(`MODEL_TO_SCENARIO edge failed for ${name}: ${JSON.stringify(edge)}`);
    return { art, bvId };
  }

  // --- STANDARD_BASE ---
  const base = await makeScenario('STANDARD_BASE', 'Base (= Baseline passthrough)');
  const basePreflight = await predictionPreflightService.runPreflight({
    organizationId: orgId,
    businessVersionId: base.bvId,
    runBy: preparerId,
    entityId: entBaselineSource,
    openingBalanceSheetPeriodId: openingBsPeriodId,
  });
  record(
    `[Prediction BASE] preflight ok=${basePreflight.ok} findingsCount=${(basePreflight as any).findingsCount ?? 'n/a'}`
  );
  ...
  record(
    `[Prediction BASE] compute ok=${baseCompute.ok} mode=${(baseCompute as any).mode ?? (baseCompute as any).code}`
  );
  if (!baseCompute.ok)
    flag('IF-06', `STANDARD_BASE Prediction compute failed: ${JSON.stringify(baseCompute)}`);

  // --- FUNDAMENTAL_INITIATIVE (efficiency programme) + direct COGS driver override -> CONFLICT ---
  const eff = await makeScenario(
    'FUNDAMENTAL_INITIATIVE',
    'Efficiency initiative + direct cost override (conflict test)'
  );
  const cogsLine = await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryOne<{ id: string }>(
      `SELECT id FROM financial_statement_lines WHERE line_code = 'COGS' AND organization_id IS NULL`
    )
  );
  ...
  await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryRun(
      ...
      [
        ...
        eff.bvId,
        initiativeRow.id,
        '4% COGS reduction from production efficiency programme',
        cogsLine.id,
        entBaselineSource,
        monthPeriods2026[0],
        preparerId,
  ...
  await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryRun(
      ...
      [
        ...
        entBaselineSource,
        conflictPeriod2026,
        (cogs2025 / revenue2025) * 0.97,
        preparerId,
      ]
    )
  );
  const effPreflight = await predictionPreflightService.runPreflight({
    organizationId: orgId,
    businessVersionId: eff.bvId,
    runBy: preparerId,
    entityId: entBaselineSource,
    openingBalanceSheetPeriodId: openingBsPeriodId,
  });
  record(
    `[Prediction EFFICIENCY] preflight ok=${effPreflight.ok} findingsCount=${(effPreflight as any).findingsCount ?? 'n/a'} requiredResolutionsCount=${(effPreflight as any).requiredResolutionsCount ?? 'n/a'}`
  );
  if (effPreflight.ok && effPreflight.findings.length > 0) {
    for (const f of effPreflight.findings) {
      record(
        `  finding: kind=${f.findingKind} period=${f.periodId} sourceCount=${f.sourceCount} layer1=${f.layer1CombinedImpactDecimal} layer2=${f.layer2CombinedImpactDecimal} requiresResolution=${f.requiresResolution}`
      );
    }
  } else {
    flag(
      'IF-07',
      `Expected preflight to detect the direct-cost-override vs initiative overlap on COGS/Jan-2026, got ${JSON.stringify(effPreflight)}`
    );
  }
  ...
  record(
    `[Prediction EFFICIENCY] compute BEFORE resolution: ok=${effComputeBeforeResolution.ok} code=${(effComputeBeforeResolution as any).code ?? 'n/a'}`
  );
  if (
    effComputeBeforeResolution.ok ||
    (effComputeBeforeResolution as any).code !== 'READINESS_GATE_FAILED'
  ) {
    flag(
      'IF-08',
      `Expected compute to be BLOCKED (READINESS_GATE_FAILED) before conflict resolution, got: ${JSON.stringify(effComputeBeforeResolution)}`
    );
  } else {
      ...
      await withPinnedPostgresTransaction((tx: Tx) =>
        tx.queryRun(
          ...
          [
            ...
            eff.bvId,
            f.findingId,
            'Accepted combined COGS impact — the May-2026 steel-pricing override and the efficiency initiative are both real, independently-sourced effects on the same cell; combined impact reviewed and accepted by the FP&A preparer.',
            preparerId,
          ]
        )
      );
  ...
  record(
    `[Prediction EFFICIENCY] compute AFTER resolution: ok=${effComputeAfterResolution.ok} mode=${(effComputeAfterResolution as any).mode ?? (effComputeAfterResolution as any).code}`
  );
  if (!effComputeAfterResolution.ok) {
    flag(
      'IF-09',
      `Expected compute to SUCCEED after conflict resolution, got: ${JSON.stringify(effComputeAfterResolution)}`
    );
  } else {
    const conflictResult = (effComputeAfterResolution as any).periods?.find(
      (p: any) => p.periodId === conflictPeriod2026
    );
    record(
      `  Jan-2026 COGS after resolution: ${conflictResult?.values?.COGS ?? 'n/a'} (combined effect of override + initiative, not double-counted)`
    );
  }

  // --- STANDARD_DOWNSIDE ---
  const downside = await makeScenario(
    'STANDARD_DOWNSIDE',
    'Downside (revenue growth 2% vs base 5%)'
  );
  ...
  const downsidePreflight = await predictionPreflightService.runPreflight({
    organizationId: orgId,
    businessVersionId: downside.bvId,
    runBy: preparerId,
    entityId: entBaselineSource,
    openingBalanceSheetPeriodId: openingBsPeriodId,
  });
  record(
    `[Prediction DOWNSIDE] preflight ok=${downsidePreflight.ok} findingsCount=${(downsidePreflight as any).findingsCount ?? 'n/a'}`
  );
  ...
  record(
    `[Prediction DOWNSIDE] compute ok=${downsideCompute.ok} mode=${(downsideCompute as any).mode ?? (downsideCompute as any).code}`
  );
  if (!downsideCompute.ok)
    throw new Error(`Downside Prediction compute failed: ${JSON.stringify(downsideCompute)}`);
  ...
  const downsideFy2026 = {
    EBIT: await downsideFy2026Sum('EBIT'),
    DEPRECIATION: await downsideFy2026Sum('DEPRECIATION'),
    CAPEX: await downsideFy2026Sum('CAPEX'),
    REVENUE: await downsideFy2026Sum('REVENUE'),
    NET_INCOME: await downsideFy2026Sum('NET_INCOME'),
    WORKING_CAPITAL: await downsideFy2026Closing('WORKING_CAPITAL'),
  };
  async function insertPredictionContinuationYear(
    bvId: string,
    periodId: string,
    fiscalYear: number,
    growth: number,
    base: typeof downsideFy2026
  ) {
    const grown = {
      EBIT: base.EBIT * (1 + growth),
      DEPRECIATION: base.DEPRECIATION * (1 + growth),
    ...
    record(
      `[Prediction DOWNSIDE FY${fiscalYear} continuation, +${(growth * 100).toFixed(0)}%/yr] EBIT=${grown.EBIT.toFixed(2)} WORKING_CAPITAL=${grown.WORKING_CAPITAL.toFixed(2)}`
    );
    return grown;
  }
  const downsideFy2027 = await insertPredictionContinuationYear(
    downside.bvId,
    periodFY2027,
    2027,
    0.02,
  ...
  await insertPredictionContinuationYear(
    downside.bvId,
    periodFY2028,
    2028,
    0.02,
    downsideFy2027 as any
  );
  ...
  const valCase = await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryOne<{ case_id: string }>(
      ...
      [
        orgId,
        'GoldCo Manufacturing Group — FY2026-2028 Enterprise Valuation',
        'Gold vertical slice full-DAG valuation, baseline + downside variants.',
        preparerId,
      ]
    )
  );
    ...
    await withPinnedPostgresTransaction((tx: Tx) =>
      tx.queryRun(
        ...
        [
          orgId,
          bvId,
          WACC_INPUTS.riskFreeRatePct,
          WACC_INPUTS.erpPct,
          WACC_INPUTS.betaUnlevered,
          WACC_INPUTS.targetDebtPct,
          WACC_INPUTS.targetEquityPct,
          WACC_INPUTS.targetDebtPct,
          WACC_INPUTS.targetEquityPct,
          WACC_INPUTS.costOfDebtPretaxPct,
          WACC_INPUTS.cashTaxRatePct,
          preparerId,
        ]
      )
    );
  ...
  // --- BASELINE variant ---
  const baselineVariant = await makeValuationVariant(
    'Baseline case',
    'FCFF DCF sourced from the Baseline Model (STANDARD case).'
  );
  const edgeModelToValuation = await lineageService.insertEdge({
    organizationId: orgId,
    sourceVersionId: baselineBvId,
    sourceArtifactType: 'BASELINE_MODEL',
    targetVersionId: baselineVariant.bvId,
    targetArtifactType: 'VALUATION_CASE',
    edgeType: 'MODEL_TO_VALUATION',
    transformationKind: 'MANUAL_LINK',
    authorId: preparerId,
    assumptionSnapshotHash: 'sha256:goldco-fulldag-model-to-valuation-baseline',
  });
  if (!edgeModelToValuation.ok)
    throw new Error(`MODEL_TO_VALUATION edge failed: ${JSON.stringify(edgeModelToValuation)}`);
  ...
  const dcfBaseline = await valuationComputeService.runDcfFcffValuation({
    organizationId: orgId,
    valuationBusinessVersionId: baselineVariant.bvId,
    entityId: entBaselineSource,
    requestedByUserId: preparerId,
    engineManifestId,
    projectionYears: projectionYearsBaseline,
    openingWorkingCapital: openingWc2025,
    terminal: { gPct: 2.5 },
  });
  if (!dcfBaseline.ok)
    throw new Error(`Baseline DCF FCFF valuation FAILED: ${JSON.stringify(dcfBaseline)}`);
  record(
    `[Valuation BASELINE] WACC=${dcfBaseline.wacc.waccPct.toFixed(4)}% terminalValue=${dcfBaseline.terminalValue.toFixed(2)} EV=${dcfBaseline.enterpriseValue.toFixed(2)} PLN`
  );

  // --- DOWNSIDE variant ---
  const downsideVariant = await makeValuationVariant(
    'Downside case',
    'FCFF DCF sourced from the Prediction downside scenario.'
  );
  const edgeScenarioToValuation = await lineageService.insertEdge({
    organizationId: orgId,
    sourceVersionId: downside.bvId,
    sourceArtifactType: 'PREDICTION_SCENARIO',
    targetVersionId: downsideVariant.bvId,
    targetArtifactType: 'VALUATION_CASE',
    edgeType: 'SCENARIO_TO_VALUATION',
    transformationKind: 'MANUAL_LINK',
    authorId: preparerId,
    assumptionSnapshotHash: 'sha256:goldco-fulldag-scenario-to-valuation-downside',
  });
  if (!edgeScenarioToValuation.ok)
    throw new Error(
      `SCENARIO_TO_VALUATION edge failed: ${JSON.stringify(edgeScenarioToValuation)}`
    );
  await writeWaccInputs(downsideVariant.bvId);
  const dcfDownside = await valuationComputeService.runDcfFcffValuation({
    organizationId: orgId,
    valuationBusinessVersionId: downsideVariant.bvId,
    entityId: entBaselineSource,
    requestedByUserId: preparerId,
    engineManifestId,
    projectionYears: projectionYearsBaseline,
  ...
  if (!dcfDownside.ok)
    throw new Error(`Downside DCF FCFF valuation FAILED: ${JSON.stringify(dcfDownside)}`);
  record(
    `[Valuation DOWNSIDE] WACC=${dcfDownside.wacc.waccPct.toFixed(4)}% terminalValue=${dcfDownside.terminalValue.toFixed(2)} EV=${dcfDownside.enterpriseValue.toFixed(2)} PLN`
  );
  if (dcfDownside.enterpriseValue >= dcfBaseline.enterpriseValue) {
    flag(
      'IF-10',
      `Expected downside EV (${dcfDownside.enterpriseValue}) < baseline EV (${dcfBaseline.enterpriseValue}) — got the opposite or equal.`
    );
  } else {
    record(
      `[Valuation] Downside EV < Baseline EV, as expected (${dcfDownside.enterpriseValue.toFixed(2)} < ${dcfBaseline.enterpriseValue.toFixed(2)}).`
    );
  }

  // --- Trading comps (synthetic peer set) on the BASELINE variant ---
  const compsMethod = await valuationComputeService.findOrCreateMethod({
    organizationId: orgId,
    ...
    await withPinnedPostgresTransaction((tx: Tx) =>
      tx.queryRun(
        ...
        [
          ...
          peer.name,
          peer.metric,
          JSON.stringify({ source: 'synthetic_peer_set_goldco_fulldag' }),
          preparerId,
        ]
      )
    );
  ...
  await valuationComputeService.setMethodResult({
    methodId: compsMethod.id,
    readiness: compsReadiness,
    resultValueStatus: 'PRESENT_NONZERO',
    resultEvDecimal: compsEv,
  });
  record(
    `[Valuation BASELINE comps] avgEV/EBITDA multiple=${avgMultiple.toFixed(2)}x on FY2026 EBITDA=${ebitda2026.toFixed(2)} -> comps EV=${compsEv.toFixed(2)} PLN`
  );

  // --- Exit multiple as a cross-check on the DCF method (NOT in the recommendation basket) ---
  const dcfMethodBaseline = await valuationComputeService.findOrCreateMethod({
    organizationId: orgId,
    businessVersionId: baselineVariant.bvId,
    methodType: 'DCF_FCFF',
    createdBy: preparerId,
  ...
  await valuationTerminalService.writeTerminalRow({
    organizationId: orgId,
    methodId: dcfMethodBaseline.id,
    convention: 'EXIT_MULTIPLE',
    exitMultipleValue: exitMultiple,
    terminalValueDecimal: exitTerminalValue,
    isPrimary: false,
    rationale:
      'Exit multiple cross-check (8.0x FY2028 EBITDA), not used in the recommendation basket — Gordon Growth remains the primary terminal convention.',
    createdBy: preparerId,
  });
  record(
    `[Valuation BASELINE exit-multiple cross-check] ${exitMultiple}x FY2028 EBITDA (${terminalYearEbitda.toFixed(2)}) = ${exitTerminalValue.toFixed(2)} PLN (cross-check only, terminal Gordon Growth remains primary)`
  );
  ...
  const basketMethods = await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryAll(`SELECT * FROM finance_valuation_methods WHERE business_version_id = ?`, [
      baselineVariant.bvId,
    ])
  );
  const recommendation = valuationComputeService.computeWeightedRecommendation(
    basketMethods as any
  );
  ...
  const waccAxis: [number, number, number, number, number] = [
    baseWacc - 2,
    baseWacc - 1,
    baseWacc,
    baseWacc + 1,
  ...
  const fcffYearsForGrid = dcfBaseline.fcffYears.map((y) => ({
    fiscalYear: y.fiscalYear,
    fcff: y.fcff!,
  }));
  const grid = valuationSensitivityService.buildWaccByTerminalGGrid({
    axes: { wacc: waccAxis, terminalG: gAxis },
    years: fcffYearsForGrid,
  ...
  record(
    `[Valuation BASELINE sensitivity] 25 cells built, monotonicity violation: ${violation ?? 'none'}`
  );
  ...
  const bridge = await valuationBridgeService.writeBridge({
    ...
    businessVersionId: baselineVariant.bvId,
    asOfDate,
    enterpriseValueDecimal: dcfBaseline.enterpriseValue,
    ...
    components: [
      ...
      {
        sequenceOrder: 3,
        componentKind: 'MINORITIES',
        sign: 'SUBTRACT_FROM_EV',
        amountDecimal: st.equityPostCTA * 0.2,
        asOfDate,
        rationale: '20% NCI in GoldCo Deutschland GmbH',
      },
    ],
    createdBy: preparerId,
  });
  if (!bridge.ok) {
    ...
    if (eq.ok) {
      await withPinnedPostgresTransaction((tx: Tx) =>
        tx.queryRun(
          `UPDATE finance_valuation_ev_equity_bridge SET equity_value_decimal = ? WHERE business_version_id = ?`,
          [eq.equityValueDecimal, baselineVariant.bvId]
        )
      );
      record(
        `[Valuation BASELINE EV->Equity bridge] EV=${dcfBaseline.enterpriseValue.toFixed(2)} -> Equity=${eq.equityValueDecimal.toFixed(2)} PLN`
      );
    }
  }
  ...
  if (!preSnap.ok) {
    flag(
      'IF-19a',
      `artifactVersionService.createComputeSnapshot() failed unexpectedly for the Valuation variant's business version: ${JSON.stringify(preSnap)}`
    );
  } else {
    preApprovalSnapshotId = preSnap.computeSnapshotId;
    record(
      `[Advisor] pre-approval finance_compute_snapshots row created via artifactVersionService.createComputeSnapshot() (IF-19 fix): ${preApprovalSnapshotId} (reused=${preSnap.reused})`
    );
  }
  ...
  const advisorOutputsDraft = [
    {
      ...
      narrative: `Baseline case WACC computed at ${dcfBaseline.wacc.waccPct.toFixed(2)}%, Enterprise Value PLN ${dcfBaseline.enterpriseValue.toFixed(0)}.`,
      driverRef: 'DCF_FCFF',
      impact: dcfBaseline.enterpriseValue,
      confidence: 'HIGH',
    },
    {
      kind: 'RISK',
      ...
      narrative:
        'The Baseline Model shows a negative December 2026 cash position (funding gap) driven by Q4 seasonality and the mandatory debt-sweep clause — financing needs to be arranged before then.',
      driverRef: 'CASH',
      impact: decResult.cash,
      confidence: 'HIGH',
    },
    {
      kind: 'HYPOTHESIS',
      title: 'Efficiency programme upside not yet in the baseline valuation',
      narrative:
        'The COGS efficiency initiative modeled in Prediction is not reflected in this Baseline-sourced valuation variant; a scenario-sourced variant using the efficiency scenario would likely show a materially higher EV.',
      driverRef: 'COGS',
      impact: null,
      confidence: 'MEDIUM',
    },
    {
      kind: 'QUESTION',
      title: 'Comps peer set representativeness',
      narrative:
        "The synthetic peer set (4 EU/CE industrial manufacturers) has not been validated against GoldCo's actual product mix — confirm comparability before relying on the comps cross-check.",
      driverRef: null,
      impact: null,
      confidence: 'LOW',
    },
  ...
  if (preApprovalSnapshotId) {
      ...
      await withPinnedPostgresTransaction((tx: Tx) =>
        tx.queryRun(
          ...
          [
            orgId,
            baselineVariant.bvId,
            preApprovalSnapshotId,
            o.kind,
            o.title,
            o.narrative,
            JSON.stringify({
              source: 'goldco_full_dag.ts',
              method: 'DCF_FCFF',
              businessVersionId: baselineVariant.bvId,
            }),
            o.driverRef,
            o.impact,
            o.confidence,
            `sha256:goldco-fulldag-advisor-${o.kind.toLowerCase()}`,
            preparerId,
          ]
        )
      );
    ...
    record(
      `[Advisor] ${advisorOutputsDraft.length} outputs written pre-approval (FACT/RISK/HYPOTHESIS/QUESTION), compute_snapshot_id=${preApprovalSnapshotId}`
    );
    const preApprovalFrozenCheck = await withPinnedPostgresTransaction((tx: Tx) =>
      tx.queryAll<{ is_frozen: boolean }>(
        `SELECT is_frozen FROM finance_valuation_advisor_outputs WHERE business_version_id = ?`,
        [baselineVariant.bvId]
      )
    );
    record(
      `[Advisor] pre-approval frozen state: ${preApprovalFrozenCheck.filter((r) => r.is_frozen).length}/${preApprovalFrozenCheck.length} frozen (expected 0/${preApprovalFrozenCheck.length} — not yet approved)`
    );
  }
  ...
  const reviewStarted = await artifactVersionService.transition({
    organizationId: orgId,
    businessVersionId: baselineVariant.bvId,
    action: 'start_review',
    actorId: reviewerId,
  ...
  record(
    `[Maker-checker] SELF-approval attempt by preparer: ok=${selfApprovalAttempt.ok} code=${(selfApprovalAttempt as any).code ?? 'n/a'}`
  );
  if (selfApprovalAttempt.ok || (selfApprovalAttempt as any).code !== 'SELF_APPROVAL_FORBIDDEN') {
    flag(
      'IF-13',
      `Expected self-approval to be REJECTED with SELF_APPROVAL_FORBIDDEN, got: ${JSON.stringify(selfApprovalAttempt)}`
    );
  } else {
  ...
  record(
    `[Maker-checker] reviewer-as-approver attempt: ok=${reviewerApprovalAttempt.ok} code=${(reviewerApprovalAttempt as any).code ?? 'n/a'}`
  );
  if (
    reviewerApprovalAttempt.ok ||
    (reviewerApprovalAttempt as any).code !== 'SELF_APPROVAL_FORBIDDEN'
  ) {
    flag(
      'IF-14',
      `Expected reviewer-as-approver to ALSO be rejected (HIGH_RISK forbids approver===reviewer too), got: ${JSON.stringify(reviewerApprovalAttempt)}`
    );
  } else {
  ...
  record(
    `[Maker-checker] distinct approver (financeAdmin, != preparer, != reviewer): ok=${distinctApproval.ok} status=${(distinctApproval as any).businessVersion?.status ?? 'n/a'}`
  );
  if (!distinctApproval.ok) {
    throw new Error(
      `Distinct-approver approval FAILED (should have succeeded): ${JSON.stringify(distinctApproval)}`
    );
  }
  ...
  if (preApprovalSnapshotId) {
    const frozenCheck = await withPinnedPostgresTransaction((tx: Tx) =>
      tx.queryAll<{ output_kind: string; is_frozen: boolean; frozen_at: string | null }>(
        `SELECT output_kind, is_frozen, frozen_at FROM finance_valuation_advisor_outputs WHERE business_version_id = ?`,
        [baselineVariant.bvId]
      )
    );
    const frozenCount = frozenCheck.filter((r) => r.is_frozen).length;
    record(
      `[Advisor] freeze check AFTER APPROVED business version: ${frozenCount}/${frozenCheck.length} rows have is_frozen=true`
    );
    if (frozenCount !== frozenCheck.length) {
      flag(
        'IF-16',
        `Expected ALL Advisor outputs to be frozen (is_frozen=true) once the parent business_version_id is APPROVED — got ${frozenCount}/${frozenCheck.length}. The freeze-on-approval trigger may not be firing correctly.`
      );
    } else {
      record(
        `[Advisor] freeze-on-approval trigger CONFIRMED working with real data: all ${frozenCount} rows frozen, frozen_at set.`
      );
    }
      ...
      await withPinnedPostgresTransaction((tx: Tx) =>
        tx.queryRun(
          ...
          [
            ...
            preApprovalSnapshotId,
            JSON.stringify({}),
            'sha256:goldco-fulldag-advisor-post-approval-attempt',
            preparerId,
          ]
        )
      );
    } catch (err: any) {
      postApprovalWriteRejected = /is APPROVED; new Advisor findings not permitted/.test(
        String(err?.message || err)
      );
    }
    record(`[Advisor] post-approval new-write rejected as expected: ${postApprovalWriteRejected}`);
    if (!postApprovalWriteRejected)
      flag(
        'IF-20',
        `A new finance_valuation_advisor_outputs row was NOT rejected after the parent business_version reached APPROVED — expected finance_valuation_advisor_outputs_no_new_after_approval() to fire.`
      );
  }
  ...
  const exportManifest = await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryOne<{ export_manifest_id: string }>(
      ...
      [
        orgId,
        distinctApproval.businessVersion.artifact_id,
        baselineVariant.bvId,
        `sha256:goldco-fulldag-export-${baselineVariant.bvId}`,
        preparerId,
      ]
    )
  );
  await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryRun(
      `INSERT INTO finance_export_manifest_sources (export_manifest_id, business_version_id, role) VALUES (?, ?, 'PRIMARY')`,
      [exportManifest.export_manifest_id, baselineVariant.bvId]
    )
  );
  await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryRun(
      ...
      [
        `sha256:goldco-fulldag-export-file-${exportManifest.export_manifest_id}`,
        `exports/goldco-fulldag/${exportManifest.export_manifest_id}.pdf`,
        exportManifest.export_manifest_id,
      ]
    )
  );
  const exportRow = await withPinnedPostgresTransaction((tx: Tx) =>
    tx.queryOne(`SELECT * FROM finance_export_manifests WHERE export_manifest_id = ?`, [
      exportManifest.export_manifest_id,
    ])
  );
  record(
    `[Export manifest] id=${exportManifest.export_manifest_id} status=${(exportRow as any)?.status} primary_business_version_id=${baselineVariant.bvId} (APPROVED source required and enforced live)`
  );
  ...
  record(
    `[Lineage] getAncestors(baselineVariant=${baselineVariant.bvId}) returned ${ancestors.length} edges:`
  );
  const edgeSummaries = ancestors.map(
    (e: any) =>
      `${e.source_artifact_type}(${e.source_version_id.slice(0, 8)}) -[${e.edge_type}]-> ${e.target_artifact_type}(${e.target_version_id.slice(0, 8)})`
  );
  for (const s of edgeSummaries) record(`  ${s}`);

  const reachesStatement = ancestors.some(
    (e: any) =>
      e.source_artifact_type === 'STATEMENT_PACK' && e.source_version_id === baselineSourceBvId
  );
  const reachesAnalysis = ancestors.some(
    (e: any) =>
      e.source_artifact_type === 'HISTORICAL_ANALYSIS' && e.source_version_id === analysisBvId
  );
  const reachesBaseline = ancestors.some(
    (e: any) => e.source_artifact_type === 'BASELINE_MODEL' && e.source_version_id === baselineBvId
  );
  record(
    `[Lineage] chain reaches original Statement Pack Version (baseline-source): ${reachesStatement}`
  );
  ...
  if (!reachesStatement || !reachesAnalysis || !reachesBaseline) {
    flag(
      'IF-17',
      `Lineage ancestors query did NOT reach the full chain: reachesStatement=${reachesStatement} reachesAnalysis=${reachesAnalysis} reachesBaseline=${reachesBaseline}`
    );
  } else {
    record(
      `[Lineage] CONFIRMED: the full DAG (Valuation -> Prediction/Baseline -> Analysis + Statement) is ONE queryable structure via finance_lineage_edges, not five separate islands.`
    );
  }

  const downsideAncestors = await lineageService.getAncestors(orgId, downsideVariant.bvId, 20);
  const downsideReachesPrediction = downsideAncestors.some(
    (e: any) =>
      e.source_artifact_type === 'PREDICTION_SCENARIO' && e.source_version_id === downside.bvId
  );
  const downsideReachesBaseline = downsideAncestors.some(
    (e: any) => e.source_artifact_type === 'BASELINE_MODEL' && e.source_version_id === baselineBvId
  );
  const downsideReachesStatement = downsideAncestors.some(
    (e: any) =>
      e.source_artifact_type === 'STATEMENT_PACK' && e.source_version_id === baselineSourceBvId
  );
  record(
    `[Lineage] downside variant ancestors: reachesPrediction=${downsideReachesPrediction} reachesBaseline=${downsideReachesBaseline} reachesStatement=${downsideReachesStatement} (${downsideAncestors.length} edges total)`
  );
  if (!downsideReachesPrediction || !downsideReachesBaseline || !downsideReachesStatement) {
    flag(
      'IF-18',
      `Downside variant's lineage ancestors did not reach the full chain (Prediction -> Baseline -> Statement).`
    );
  }
  ...
  record(
    `  TOTAL end-to-end (Statement -> Valuation, this script's own wall-clock): ${(totalMs / 1000).toFixed(1)}s`
  );
  ...
  const results = {
    ...
    businessVersionIds: {
      statement: {
        parentFY2023: packParentFY2023.businessVersion.business_version_id,
        parentFY2024Original: packParentFY2024.businessVersion.business_version_id,
        parentFY2024Restated: restatedBvId,
        parentFY2025Annual: packParentFY2025.businessVersion.business_version_id,
        groupFY2025Consolidated: groupBvId,
        baselineSource: baselineSourceBvId,
      },
      ...
      valuation: {
        case: caseId,
        baselineVariant: baselineVariant.bvId,
        downsideVariant: downsideVariant.bvId,
      },
    },
    keyResults: {
      ...
      predictionConflictBlockedThenResolved:
        (effComputeBeforeResolution as any).code === 'READINESS_GATE_FAILED' &&
        effComputeAfterResolution.ok,
      ...
      makerCheckerSelfApprovalRejected:
        !selfApprovalAttempt.ok && (selfApprovalAttempt as any).code === 'SELF_APPROVAL_FORBIDDEN',
  ...
  record(
    `[goldco_full_dag] integration findings: ${findings.length} (${findings.map((f) => f.id).join(', ') || 'none'})`
  );
}
