...
sequenceDiagram
    ...
    alt wiersz nie znaleziony (version mismatch)
        ...
        alt walidacja nie przechodzi
            ...
            API->>DB: COMMIT
            API->>DB: (poza tą transakcją, best-effort) UPDATE parent SET status='SUPERSEDED' jeśli T9 dotyczy
...
3. **(c) Status transition** — `UPDATE ... WHERE id=? AND version=expectedVersion` (ten sam wzorzec CAS co dziś w `approveModel`), ale teraz zapisuje też `compute_snapshot_id` z kroku (b) w tym samym stwierdzeniu transakcyjnym — snapshot i status nie mogą się rozjechać.
4. **(d) Audit log** — `INSERT` do `artifact_lifecycle_events` (append-only, nigdy `UPDATE`/`DELETE`) w tej samej transakcji. Audit log, który mógłby się nie zapisać przy sukcesie statusu (np. osobna transakcja/`fire-and-forget`), jest bezwartościowy do celów audytowych — stąd wymóg jednej transakcji.
...
| Po `SELECT FOR UPDATE`, w trakcie kroku (a)/(b)/(c)/(d), przed `COMMIT` | Cała transakcja **rollback automatyczny** (crash procesu = zerwane połączenie DB = serwer robi ROLLBACK; timeout = jawny ROLLBACK). Wersja zostaje w `IN_REVIEW`, **żaden** artefakt (snapshot, status, audit log) nie powstaje częściowo. Idempotency lock zwolniony (albo wygasa przez TTL), klient może bezpiecznie retry. |
| `COMMIT` wykonany, ale odpowiedź nie dotarła do klienta (sieć padła po stronie serwer→klient) | Stan w bazie jest kompletny i poprawny (wszystkie 4 kroki zapisane atomowo). Klient retry z tym samym `Idempotency-Key` → serwer rozpoznaje istniejący `idempotent_resources` wpis, **nie powtarza** transakcji, zwraca `200 idempotentReplay:true` z już zapisanym stanem. |
...
| Efekt uboczny T9 (`UPDATE parent SET status='SUPERSEDED'`) zawiedzie po commicie głównej transakcji (rzadki przypadek reopen→approve) | To jest osobna, mała transakcja (§6.4) — jej porażka NIE cofa zatwierdzenia `vN+1` (które już jest faktem, nieodwracalnym). System musi mieć proces naprawczy/reconciliation job (WP-B07 observability) wykrywający `APPROVED` wiersze, które mają zatwierdzonego potomka, a same nie są `SUPERSEDED`, i domykający to asynchronicznie z alertem — **nie** wolno przez to zablokować głównego zatwierdzenia. |
...
Gdy `vN+1` sama przechodzi `IN_REVIEW → APPROVED` (§5), **ten sam** krok (c) tamtej transakcji (albo bezpośrednio po commicie, w małej dodatkowej transakcji — patrz tabela awarii §5.2) wykonuje:
