...
### Sesja 2026-06-18 (Kontekst H2 — M13 close-out)
- ✅ **VIS-009 M13 Team-workload row-tint VERIFIED CLOSED** — Przegląd kodu potwierdził: brak rose-/primary row-tinting w Team Workload tabeli (`ResourcesAnalysis.tsx`). Wiersz `<tr>` używa neutral `hover:bg-slate-50` (bez tintowania statusem). Awatary/badge'y statusu używają `danger-500/20` (overallocated), `slate-200` (underutilized), `emerald-500/20` (ok) — semantycznie poprawne, NIE crimson/rose. Pierwotny rose-leak: `bg-rose-500/20` awatar + `bg-rose-500` pasek progresu naprawiony w `83fdc31998` (rose→danger token). Primary tint w panelu AI proposals naprawiony w `081f38396a`. Grep: 0 `rose-` w całym `src/components/Initiatives/`. Finding zamknięty.
- ✅ **SYS-8 M13 board/kanban preview „Otwórz" VERIFIED CLOSED** — `InitiativesHub.tsx:1473` wireduje `onOpenFull={() => handleOpenInitiativeDocument(item)}` do `InitiativePreviewV3Footer`. Footer (linii 533–542) renderuje przycisk „Otwórz"/„Open" (ExternalLink) gdy `onOpenFull` podany. Komentarz UWAGA #15 przy liniach 1468–1470 potwierdza celowy fix. `InitiativePreview.tsx` (legacy) też ma Open w footer. Finding zamknięty jako already-fixed.

### Sesja 2026-06-18 (Kontekst E — c.d. SYS-4)
- ✅ **SYS-4 CZĘŚCIOWE** — usunięcie fioletu/indigo spoza palety: `NotebookRightRail.tsx` (tab toggle active `bg-indigo-600` → neutral `bg-slate-800 dark:bg-white/[0.12]`), `SurveyShell.tsx` (progress bar fill `bg-indigo-500` → `var(--c-info)`). Inwentaryzacja: Finance create-CTA = `bg-blue-600` (NIE fiolet — już ok lub błędna identyfikacja audytu); Calendar event-bars violet-600 = nie znaleziono (już naprawione); `HomeView` category-map violet/indigo = LEGALNE (subtle tint + dot per kategoria, §4.0a OK — NIE solid fill, NIE CTA). Pozostałe `bg-indigo-*` = tła ikon/avatary/dekoracyjne kropki (nie progress, nie VIS-005). tsc: 0 błędów.
...
- ✅ **SYS-8 CZĘŚCIOWE** — audyt parytetu stopki preview (21 użyć `PreviewActionBar`): pokrycie szerokie (My Work, Initiatives, DiscoveryTools V3, Results ×6, Interview, Economics). Ad-hoc stopki znalezione i naprawione: `Execution/Manager/ProblemPreview.tsx` (custom `ActionButton` h-8 + `bg-blue-600` primary → kanoniczny `actionPillClass()` h-9+border, scheme primary/red/neutral), `MyWork/Executive/DecisionQueuePreview.tsx` (przycisk „Approve" crimson `text-primary-600` → semantycznie poprawny `text-emerald-600` success). `ToolSessionPreview.tsx` V1 (crimson footer) = MARTWY (tylko helper `getToolCategoryLabel` importowany; hub renderuje V3 z PreviewActionBar) → pominięty. `RowDetailPanel` convert-strip = neutralny, akceptowalny (kompaktowy 3-up, brak złych kolorów). `InterviewInsightPreview` = `ArtifactActionPanel` (legalny). tsc: 0 błędów. Pozostałe entity-previews (assessment, ReportsAndPresentations) — niższy priorytet, do deep-scan z auth.
- ✅ **Weryfikacja wizualna Settings CTA DOMKNIĘTA** — sesja zalogowana (Piotr OWNER) przez Chrome-MCP na localhost:3000. Zmierzone computed-style CTA „Save Changes" (Profile) + „Add Webhook" (Webhooks): **light mode** `bg-navy-900` = `rgb(15,23,42)` dark-navy + white text; **dark mode** `dark:bg-[#F4F7FB]` = `rgb(244,247,251)` light-surface + `rgb(10,15,30)` navy text. Skan `crimsonLeak` (r~133/g<60/b~47) = **0 trafień** w obu trybach. Probe `bg-navy-900 dark:bg-[#F4F7FB]` bez `.dark` = navy (dark: NIE wycieka do light). SYS-1 sidebar „Theme" active = neutral slate (potwierdzone wizualnie). **Mechanizm motywu:** Tailwind `darkMode:'class'` (.dark na <html>).
- ⚠️→✅ **„Systemowe HTTP 500 w Settings" = FAŁSZYWY ALARM (wycofane po weryfikacji logów)** — panele Theme/Webhooks/Language/AI-behavior pokazywały „HTTP 500 Internal Server Error", ale **backend log `/private/tmp/bk.log` = 0× status 500**: endpointy zwracają **200** (zalogowany) / **401** (token wygasł). Curl proxy `:3000` i backend `:3001` (bez tokena) = oba **401** „No token provided" (zdrowe). Przyczyna 500-ek w UI: **transient** — niestabilny Vite dev server (restartowany kilka razy w sesji) + wygasły token w trakcie długiego walkthrough; Api client renderuje błąd proxy/połączenia jako „HTTP 500…" (`api.ts` fallback `HTTP ${status} ${statusText}`). Webhooks „No presentations yet" = bleed-through w stanie błędu (kod EmptyState ma poprawne „No webhooks configured"). **Brak realnego buga backendowego.** (Wzorzec `finding_gap_reports_overstate` — log uratował przed niepotrzebną migracją na PROD DB.) Uboczne, znane: regional/org-context bardzo wolne vs Railway (54 zapytania/6.6s, 50/24s) = istniejący `finding_staging_db_perf` (N+1 latency), nie nowy bug.