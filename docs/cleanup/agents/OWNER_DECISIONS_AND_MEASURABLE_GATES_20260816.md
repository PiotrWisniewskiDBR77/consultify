...
| `AMD-MVP-FEATURE-ENABLEMENT-003` | `21A` | An MVP feature may become default-on after its exact candidate passes the complete signed journey, focused regressions and a tested feature-flag rollback. | All MVP feature packets and `REL-001-T01` | This is a speed decision, not blanket enablement. Owner/policy default-OFF features remain off until their explicit approval exists. Invalidate on feature behavior, flag contract, mounted route, candidate SHA or rollback mechanism change. |
| `AMD-MVP-AVAILABILITY-DEFERRED-003` | `22C` | MVP repository completion has no formal availability SLA. Availability targets and sustained production measurement move to the post-MVP hardening phase. | `NFR-PERF-001`, `OPS-OBS-001`, `REL-001-T01` | Internal functional, performance and failure-recovery gates still apply. Do not claim production availability or reliability. A release may be authorized only with the external/deployed facts explicitly available at that time. |
| `AMD-MEDIUM-SECURITY-REMEDIATION-003` | `23C` | A reachable Medium security finding may remain for at most 60 days only with a named owner, due date and tested compensating control. Critical/High, authentication bypass, tenant escape and secret disclosure remain zero-tolerance MVP blockers. | `SEC-PRIV-001`, all authenticated MVP surfaces, `REL-001-T01` | This specializes decision `15A` for delivery speed without weakening its zero-tolerance classes. Missing owner/date/control blocks technical `DONE`. Invalidate on severity, reachability, exploit path or compensating control change. |
| `AMD-MVP-DATA-MIGRATION-GATE-003` | `24A` | Every MVP data migration or backfill requires dry-run, exact affected-row count, checksum/integrity control, rollback plan and explicit execution authorization. | All migration/backfill tasks and `REL-001-T01` | Schema-only additive migrations retain their existing strict fresh/repeat/dry and late-shape gates. No production data mutation is authorized by this decision. Invalidate on source data, transform, schema, candidate SHA or rollback plan change. |
| `AMD-MVP-CANDIDATE-FREEZE-003` | `25B` | The final MVP exact SHA is frozen for 24 hours while full integrated gates run; zero P0/P1 defects may remain at acceptance. | `UI-CANON-ALL-001`, `REL-001-T01` | Only blocker fixes may alter the candidate during freeze; any code, dependency, migration, configuration or governed fixture change creates a new SHA and restarts the 24-hour gate. This is a technical freeze, not push/deploy/release authorization. |
| `AMD-EXE-SPINE-AUTHORITY-004` | `26A` | Runtime-v1 is the sole canonical writer of execution work; Case Workspace owns governance and lifecycle state; `execution_case_links` owns cross-model identity. Legacy PMO and V8 execution surfaces become compatibility adapters/read-only. One canonical execution-health projection replaces the four independent formulas, using governed progress/risk/decision/task facts and EVM SPI/CPI where authoritative data exists. | `EXE-MVP-SPINE-001`, `EXE-MVP-ACTIONS-001`, `FLOW-TRANSFORM-MVP-001`, downstream Results/Finance consumers | Piotr accepted option A on 2026-08-19. Implement hostile-safe additive identity/backfill/quarantine, exactly-one mapping for every legacy ID, Runtime-v1 write authority, Case Workspace governance adapters, one versioned health contract consumed by all four current readers, legacy write denial, rollback and exact-SHA migration/RealPG/signed journey evidence. Production backfill, push, deploy and release remain separately unauthorized. Invalidate on canonical writer, governance owner, identity mapping, health inputs/thresholds, compatibility window or rollback contract change. |
