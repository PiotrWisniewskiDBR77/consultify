...
| `RaidCanvas`             | `RaidCanvas.tsx`             | Pełny RAID log (Risk/Assumption/Issue/Dependency) z counter cards, filter tabs, P×I scoring (Risk), impact-only (A/I/D), proposed action, owner, due date, source, AI enhance | Initiative     |
...
## 5c) RaidCanvas — specyfikacja

### Plik

`src/components/shared/NModeSections/RaidCanvas.tsx`

### Przeznaczenie

Pełny moduł RAID Log (Risk, Assumption, Issue, Dependency) dla widoków N-mode.
Bazuje na sprawdzonym wzorcu `RiskCanvas`, rozszerzony o:

- 4 typy RAID z warunkowym renderowaniem per typ
- Counter cards + filter tabs
- Status, owner, due date, source per element
- "Proposed Action" textarea dla wszystkich typów
- Risk-specific: pełny scoring P×I + contingency + mitigation
- A/I/D: impact-only + type-specific action textarea
- AI field enhancement per pole

### Props (`RaidCanvasProps`)

| Prop              | Typ                                                                  | Wymagany | Opis                                                      |
| ----------------- | -------------------------------------------------------------------- | -------- | --------------------------------------------------------- |
| `items`           | `RaidItem[]`                                                         | ✅       | Lista elementów RAID (nieposortowana — komponent sortuje) |
| `onAddItem`       | `(type: RaidType) => void`                                           | ✅       | Dodaj nowy element z danym typem                          |
| `onUpdateItem`    | `(id: string, updates: Partial<RaidItem>) => void`                   | ✅       | Aktualizuj pola elementu                                  |
| `onRemoveItem`    | `(id: string) => void`                                               | ✅       | Usuń element po id                                        |
| `onAIGenerate`    | `() => void`                                                         | ❌       | AI generate handler (pomiń = ukryj przycisk AI)           |
| `isGeneratingAI`  | `boolean`                                                            | ❌       | Czy generowanie AI trwa                                   |
| `locked`          | `boolean`                                                            | ❌       | Tryb read-only                                            |
| `artifactContext` | `{ title: string; status: string; priority: string; type: string; }` | ✅       | Kontekst dla AIFieldEnhancer                              |
| `fieldKeyPrefix`  | `string`                                                             | ✅       | Unikalny prefix dla kluczy AI (np. `'init'`)              |
| `users`           | `Array<{ id: string; name: string }>`                                | ❌       | Lista użytkowników do dropdown owner                      |

### Typy

```typescript
type RaidType = 'risk' | 'assumption' | 'issue' | 'dependency';
type RaidLevel = 'low' | 'medium' | 'high' | 'critical';
type RaidStatus = 'open' | 'mitigated' | 'accepted' | 'closed' | 'resolved' | 'transferred';

interface RaidItem {
  id: string;
  type: RaidType;
  title: string;
  probability?: RaidLevel; // Risk only
  impact: RaidLevel; // All types
  category?: string;
  mitigation?: string; // Risk only
  contingency?: string; // Risk only
  proposedAction?: string; // All types
  status: RaidStatus;
  owner?: string;
  dueDate?: string;
  source?: string;
...
### Warunkowe renderowanie per typ RAID

| Pole            | Risk | Assumption | Issue | Dependency |
| --------------- | ---- | ---------- | ----- | ---------- |
| Probability     | ✅   | ❌         | ❌    | ❌         |
| Impact          | ✅   | ✅         | ✅    | ✅         |
| Score (P×I)     | ✅   | ❌         | ❌    | ❌         |
...
### Integracja w Initiative

D-mode: `RaidSection.tsx` → `CollapsibleSection` wrapper → `RaidCanvas`
N-mode: `InitiativeDocumentView.tsx` → case `'risk-raid'` → `RaidCanvas` bezpośrednio

### Różnica vs RiskCanvas

`RiskCanvas` = szczegółowa analiza ryzyka (P×I, mitigation, contingency) — używany w Task, Decision.
`RaidCanvas` = pełny RAID log z 4 typami + wszystkimi polami zarządczymi — używany w Initiative.
