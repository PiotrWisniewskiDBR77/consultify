...
| `REL-001-T01` | `PARTIAL / EXACT_PRODUCT_BUNDLE_NO_GO` | Exact product SHA `bcfb01483a` has four fresh PASS receipts and a deterministic `NOT_AUTHORIZED` bundle. A tested verifier precision fix removed 77 provable lexical false positives without a waiver path. Independent verification still correctly returns `NOT_VERIFIED`, `releaseGo=false`: 81 findings across 36 migration files (`13` ALTER/DROP, `13` rewrite candidates, `15` DROP, `40` unclassified). | Migration-by-migration classification/remediation, all preceding gates, explicit Piotr release authorization; production remains out of scope. | Audit all 36 remaining migrations without blanket suppression, regenerate and independently verify; no push/deploy/release before exact approval. |
