...
| `REL-001-T01` | `PARTIAL / EXACT_PRODUCT_BUNDLE_NO_GO` | Exact product SHA `bcfb01483a` has four fresh PASS receipts and a deterministic `NOT_AUTHORIZED` bundle. The independent verifier correctly returns `NOT_VERIFIED`, `releaseGo=false`: 158 migration-policy findings across 37 new migration files (`13` ALTER/DROP, `5` CASCADE, `49` data rewrite, `15` DROP, `76` unclassified). | Migration-by-migration classification/remediation, all preceding gates, explicit Piotr release authorization; production remains out of scope. | Audit all 37 migrations without waivers or suppression, regenerate and independently verify; no push/deploy/release before exact approval. |
