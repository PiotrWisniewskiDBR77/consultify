...
| `INT-INIT-AI-OBS-001` | 02 Interview | P1 | Functional observation / AI-assisted form fill | OBSERVED / OWNER_VERBAL_DECISION_NOT_YET_CAPTURED / REQUEST_RESPONSE_… | NIEWERYFIKOWALNE STATYCZNIE | Tylko dowód zrzutu ekranu ("Failed to fill the section with AI"); wymaga uruchomienia AI fill z realnym providerem, by ustalić przyczynę (provider vs walidacja vs serwer) — nie do rozstrzygnięcia statycznym gitem/grepem | trzeba uruchomić: AI Initiative Wizard → Fill with AI z żywym providerem i przechwycić od… |
| `ASM-OWN-001` | 04 Assessment | P0 | Backend contract / licensed Assessment | CAPTURED_UNRECONCILED / BLOCKS_OWNER_REVIEW | NIEWERYFIKOWALNE STATYCZNIE | Dokument sam przyznaje: kod ma trasę (`/api/method` zamontowana Gateway.ts:968, potwierdzone grep), ale zdalny backend za proxy zwracał 404 — to defekt wdrożenia/proxy, nie brak kodu; wymaga żywego serwera by ustalić dzisiejszy stan | trzeba: odpalić lokalny backend + sprawdzić GET /api/method/outputs przez realny proxy Vi… |
| `ASM-OWN-002` | 04 Assessment | P0 | Runtime truth / fail-closed workspace | CAPTURED_UNRECONCILED / BLOCKS_OWNER_REVIEW | NIEWERYFIKOWALNE STATYCZNIE | Ta sama rodzina co ASM-OWN-001 — RECOVERY_DRAFT fail-closed jest zależny od tego samego backendu; brak dowodu zmiany kodu, wymaga żywego backendu | trzeba: powtórzyć otwarcie sesji Assessment na żywym backendzie |
| `ASM-OWN-003` | 04 Assessment | P0 | Downstream read / Outputs | CAPTURED_UNRECONCILED / BLOCKS_OWNER_REVIEW | NIEWERYFIKOWALNE STATYCZNIE | Ta sama rodzina 404 na `/api/method/outputs` — trasa istnieje w kodzie (server/src/routes/method-core.routes.ts, zamontowana), ale downstream Outputs/Reports/Initiatives zależy od żywego backendu | trzeba: powtórzyć na żywym backendzie |
| `INI-OWN-001` | 05 Initiatives | P0 | Review data / methodology | OWNER_AUDIT_BLOCKED_BY_INCOMPLETE_REVIEW_DATA / NOT_ACCEPTED | NIEWERYFIKOWALNE STATYCZNIE | Wymaga przeglądu danych demo (11 inicjatyw, kompletność pól lifecycle) na żywej bazie/przeglądarce — nie do ustalenia statycznie | trzeba: otworzyć /initiatives?sampleData=initiatives i sprawdzić wypełnienie pól |
...
| `EXE-OWN-003` | 06 Execution | P0 | Review data / owner workflow | FIXED_LOCAL / OWNER_RETEST_REQUIRED | NIEWERYFIKOWALNE STATYCZNIE | Fix="uncommitted local review worktree" — dotyczy WYŁĄCZNIE danych demo do lokalnego przeglądu (nie produkcyjnego kodu); bez dostępu do tego workspace nie potwierdzę, ale produkcja i tak pozostaje fail-closed niezależnie od tego ustalenia | trzeba: sprawdzić /execution?tab=list&view=table na lokalnym seedzie z danymi Initiatives… |
...
| `RES-OWN-003` | 09 Results | P0 | Reviewability / data | LOCAL_FIXTURE_VISIBLE / DB_READBACK_OPEN / OWNER_RETEST_REQUIRED | NIEWERYFIKOWALNE STATYCZNIE | LOCAL_FIXTURE_VISIBLE/DB_READBACK_OPEN — wymaga żywej bazy by potwierdzić trwały zapis i odczyt na zimno | trzeba: przelot zapis→restart→odczyt na żywej Postgres |
...
- **NIEWERYFIKOWALNE STATYCZNIE: 8** (7%) — wszystkie ze źródła A+B
