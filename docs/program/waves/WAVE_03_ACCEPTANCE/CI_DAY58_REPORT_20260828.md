...
| `src/components/Initiatives/__tests__/initiativeRegisterProjection.scope.test.ts` | 18,21,27,29 TS2345/TS2339 | 4 | NAPRAWIAM (SKORYGOWANE FIX-4) | pierwotnie rozluźniono produkcyjną sygnaturę `canonicalInitiativeMatchesRegisterFilters`/`filterCanonicalInitiativeRegisterScope` w `initiativeRegisterProjection.ts` z `Pick<PortfolioInitiative,'projectId'\|'priority'>` na luźny `{ projectId?: string\|null; priority?: string\|null }`, żeby przepuścić fiksturę — to osłabiało kontrakt produkcyjny. Odbiór adwersaryjny to wykrył; FIX-4 przywrócił ścisłą sygnaturę `Pick<...>` i zamiast tego dopasował fikstury testowe (jawny typ `FixtureRow` + udokumentowana asercja tylko na jednym polu symulującym brakujący `priority` z realnych, niewalidowanych danych) |
| `src/components/Interview/InterviewHub.tsx` | 7523 TS2322 | 1 | NAPRAWIAM | jawny fallback statusu `UNKNOWN` |
| `src/components/MyWork/IdeaMapWorkspace.tsx` | 4512 TS2322 | 1 | NAPRAWIAM (SKORYGOWANE FIX-4) | BŁĘDNY OPIS: to NIE jest zawężenie, tylko rzutowanie `patch.state as NodeStatus` bez sprawdzenia przynależności. Odbiór adwersaryjny to wykrył. Realne zawężenie sprawdzono i odrzucono: `nativeStates` faktycznie oferowane w UI (kilka linijek wyżej) obejmują wartości spoza `NodeStatus` — `'validated'\|'ready_to_convert'\|'converted'` dla mapy myśli i `'todo'\|'in_progress'\|'done'\|'blocked'` dla tabeli — więc realny check przynależności do `NodeStatus` po cichu odrzucałby te legalne, wybierane przez użytkownika zmiany stanu (regresja zachowania), zamiast je zapisywać. Naprawa tego bez ryzyka wymaga poszerzenia samego typu `NodeStatus` (zmiana kontraktu poza zakresem tego dyżuru). Zostawiono asercję jako świadomy, opisany dług — kod ma teraz komentarz wyjaśniający dlaczego (linia ~4518) |
...
| `src/components/ResultsVNext/resultsSearchApi.ts` | 31 TS2558 | 1 | NAPRAWIAM (POTWIERDZONE FIX-4) | generyk sprawdzono ponownie na tipie — `Api.get<ResultsSearchResponse>` nadal nie kompiluje się (TS2558, `Api.get` w `services/api.ts` to zwykła niegeneryczna funkcja zwracająca `any`), więc pozostała niesprawdzana asercja `as ResultsSearchResponse`; dopisano komentarz w kodzie tłumaczący dlaczego i wpisano jako świadomy, śledzony dług (właściwa naprawa wymaga otypowania `Api.get` albo migracji na typowany moduł `api/*.api.ts`) |
