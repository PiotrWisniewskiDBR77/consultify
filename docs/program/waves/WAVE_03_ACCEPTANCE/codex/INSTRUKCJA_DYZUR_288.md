...
Twierdzę: `finance.routes.ts` ma 34 `router.get` i 51 tras zapisowych; cała powierzchnia
finansowa `/api/v8` to 270 tras (91 GET + 179 zapisowych) w 7 prefiksach z `v8/index.ts:120-131`,
wszystkie bez `createModuleGate`; 64 zapisy nie mają ani bramki, ani odczytu członkostwa;
`financeStatementMountedSurface.ts:78-98` montuje Sprawozdania przed `v8FeatureGate`;
`betaGate`/`createBetaGate` to atrapy. Pomiar statyczny z liczbami per router:
`docs/program/waves/WAVE_03_ACCEPTANCE/G20_BLOKERY_P0P1_20260903.md` (T1). Komendy z §0.3 to sprawdzają.
...
JEDEN wspólny middleware na wzorcu ścieżki obejmującym wszystkie 7 prefiksów finansów w
`v8/index.ts` PRZED pierwszym mountem finansów (linia ok. 120): `gatewayVerifyToken` (albo
równoważny, którego używają sąsiedzi w `Gateway.ts`) PRZED `createModuleGate('MODULE_ECONOMICS')`;
plus ta sama bramka na powierzchni `financeStatementMountedSurface.ts` (ok. linii 88). Wzorzec:
mount `/api/finance-statements` w `Gateway.ts`. Test jednostkowy wzorca: łapie 7 prefiksów,
nie łapie `/retrieval`, `/execution`, `/case-workspace`. Jeśli mount jest w `v8/index.ts`, sprawdź, czy
