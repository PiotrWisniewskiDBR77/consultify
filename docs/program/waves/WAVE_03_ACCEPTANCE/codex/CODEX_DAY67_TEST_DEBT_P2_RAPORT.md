# DYŻUR 67 — TEST DEBT P2 — RAPORT PO DRUGIEJ SAMOPRAWIE

## Status

`PARTIAL` — wszystkie zidentyfikowane i reprodukowalne przyczyny naprawialne wyłącznie w licencjonowanych plikach P2 zostały zamknięte. Końcowy izolowany regres: **39/49 zielonych plików, 10/49 czerwonych plików**. Pozostałe czerwienie są nazwanymi kontraktami wymagającymi produktu, schematu, konfiguracji release albo zabronionego globalnego seamu testowego. Nie deklaruję `FIXED` dla B9 ani żadnego z tych kontraktów.
...
- wynik: **39 PASS / 10 FAIL**. Logi: `/private/tmp/consultify-day67-test-debt-p2-artifacts/final-isolated-fourth/`.
...
Baseline drugiej samopoprawy miał 41 czerwonych plików. Poniżej każdy ma pierwszą przyczynę, kategorię, właściciela i werdykt: 31 zamknięto w P2, 10 pozostaje jako kontrakty poza licencją.
...
| `middleware/orgContext.middleware.test.ts` | role fixture odrzucane przez realny CHECK; brak dozwolonego seamu | dane/produkt: middleware lub zabroniony helper/mock | poza P2, czerwony |
...
| `services/artifactRegistryService.test.ts` | nieistniejący `resolvePresentationSlideCount` | produkt: artifact registry | poza P2, czerwony |
...
| `subscriptionAnalyticsService.test.ts` | import nieistniejącego serwisu | produkt: brakujący serwis | poza P2, czerwony |
...
## Pozostałe 10 czerwonych plików — pełne nazwy
...
4. `middleware/orgContext.middleware.test.ts`: `orgContext.middleware (L1) > sanitizes whitespace-padded membership role before attaching org context`; `orgContext.middleware (L1) > falls back to MEMBER role when membership role contains control characters`. Dane/produkt; CHECK PG odrzuca fixture; poprawny seam jest poza licencją.
5. `organizationService.test.js`: `OrganizationService > createOrganization > should create a new organization`; `... > should store organization in database`; `... > should add creator as OWNER member`; `OrganizationService > getOrganization > should return organization by ID`; `OrganizationService > addMember > should add a member to organization`; `... > should reject invalid roles`; `OrganizationService > getMembers > should return organization members`; `OrganizationService > getUserOrganizations > should return user organizations`; `OrganizationService > updateMemberRole > should update member role`; `OrganizationService > removeMember > should remove member from organization`; `OrganizationService > AI Settings > should get default AI settings`; `... > should update AI settings`. Produkt/schema; brak kolumny `created_by_user_id`; poza P2.
6. `services/artifactRegistryService.test.ts`: `artifactRegistryService > uses canonical deck_json cards over a stale materialized slide_count`. Produkt; nieistniejący eksport runtime; poza P2.
7. `services/partnerCertificatePdf.test.ts`: `generatePartnerCertificatePdf > integration contract: application font renders Polish glyph input into a valid PDF`. Produkt/środowisko; `pdfFonts.ts:143`, `Not a supported font format or standard PDF font`; poza P2. Helvetica dowodzi tylko unit-flow; realny font i polskie glify `NOT_PROVEN`.
8. `slackRouter.test.ts`: `slackRouter.routeToSlack > dedupe SURVIVES a simulated process restart (durable, DB-backed)`. Produkt/schema; timestamp bez strefy; poza P2.
9. `subscriptionAnalyticsService.test.ts`: kolekcja pliku nie startuje, bo `server/src/services/subscriptionAnalyticsService.js` nie istnieje. Produkt: martwy kontrakt albo brak produktu; poza P2.
10. `wave8AgentRuntimeService.test.ts`: `Wave 8 agent runtime > exposes a complete specialized agent catalog with scoped tools and schemas`; `... > stores admin-editable AgentDefinition overrides in the Wave 8 database catalog`; `... > blocks tool misuse and swarm without approval and budget gate`; `... > launches role-specific schema-valid output and creates owner-audited schedules`; `... > enforces approvalPolicy and processes due schedules`; `... > records eval run hooks as audit-only hooks without executing an evaluator`; `... > enforces tool scope before executing agent tools`. Produkt/schema; `wave8_schema_write_failed`; poza P2.
...
- summary final-isolated: `922c637bc86f41a150b38d6cf32f159b67bf483172493e934a3ed0cd28944617`;
- manifest logów: `45eb7dabdb5809986cac79adfaf2e0d4f7a50ded53008381f71c6160039982cf`;
- server build log: `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`;
- frontend build log: `67fa22fe4d2f2662e68104a8a96d019a67a3d2fee97d23c6778ff9c7c0c50b6f`;
- migration pass 1: `439ab8a5dd4d31e641dcbfe696bcc1a96c1ea71535ec00e083a53459fd7d7bc4`;
- migration pass 2: `445dd2d62f061bc07dcfe41382b2c78832bad274e0db3a04f4153ced42aa2cd1`.

## Commity i karta

Commity drugiej samopoprawy: `4634fe69eb`, `f616691cdb`, `a7e4085014`, `d041d2a4b3`, `77e7b4cf28`, `a9b88317f0`, `cdf75300c3`, `3a5180fef3`, `b32f1f2da4`, `37f796a33e`, `8e855adf03`. Każdy wypchnięto natychmiast na `github-backup`.
...
- finalnie: 39 PASS / 10 FAIL; B9 `stub`, `NOT_AUTHORIZED`;
- status: `PARTIAL`.
