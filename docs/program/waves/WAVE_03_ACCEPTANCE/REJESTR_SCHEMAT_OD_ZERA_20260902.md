...
## E. Dyżur agent/schemat-datetime-20260903 — pomiar i naprawa reszty `DATETIME`

Pełny raport: `docs/program/waves/WAVE_03_ACCEPTANCE/codex/SCHEMAT_DATETIME_RESZTA_20260903.md`.

Zmierzone samodzielnie (nie zaufano powyższej liście "pięciu plików" — była niekompletna/nieaktualna):

- **`integrationHubService.ts`** — jedyne wystąpienie `DATETIME` to komentarz w usuniętym 2026-08-28 martwym kodzie (`initialize()` bez importera). Nie DDL. Status: NIE DOTYCZY (fałszywy trop rejestru).
- **`notificationOutboxService.ts`** (`notification_outbox`), **`demoTrialTelemetryService.ts`** (`conversion_events`), **`ai/llmConfigService.ts`** (`llm_logs`, `llm_health_events`, `llm_providers`, `organization_llm_settings`, `llm_tier_assignments`, `ai_model_overrides`, `organization_provider_settings`, `tier_round_robin_state`) — WSZYSTKIE mają realną migrację (`20260719_baseline_gap.sql` i/lub dedykowane pliki), zweryfikowane grepem `create table if not exists` (wielkość liter nieistotna dla Postgresa). Rejestr powyżej ich nie znalazł, bo `llmConfigService`'s wiersze 594/621/649 oznaczył `NIE`/`NIEZWERYFIKOWANA` mimo że migracja istnieje z małej litery. Status: JUŻ NAPRAWIONE (fałszywy alarm rejestru), bez zmian w tym dyżurze.
- **`aiSettingsService.ts`** (`ai_user_tiers`) — POTWIERDZONA luka, brak migracji. NAPRAWIONE: `server/migrations/20260903_ai_user_tiers.sql` + runtime DDL na `TIMESTAMPTZ`.
- **`help.routes.ts`** (nie było w oryginalnym rejestrze — dopisane po własnym grepie) — z czterech tabel (`help_categories`, `help_articles`, `help_playbooks`, `help_events`) tylko `help_categories` nie miała migracji. NAPRAWIONE: `server/migrations/20260903_help_categories.sql` + `ensureHelpSchema()` przestał połykać błąd DDL (ten sam wzorzec co `emailVerificationService` przed 5b5c0e3849 — `ensured=true` ustawiane przed próbą, `catch` tylko logował `warn` i kontynuował).
- **Odkryty przy okazji, POZA zakresem tego dyżuru**: `help_articles` i `help_events` MAJĄ migrację, ale w INNYM kształcie niż zakłada runtime DDL/zapytania w `help.routes.ts` (migrowany `help_articles` ma `category`/`content`, kod czyta `category_id`/`body`/`status`; migrowany `help_events` nie ma `organization_id`/`article_id`/`metadata`, które kod wstawia). Zmierzone empirycznie na żywym Postgresie 2026-09-03: `GET /api/help/categories` faktycznie loguje `column "category_id" does not exist` i cicho degraduje do pustej listy (złapane przez `.catch(() => [])`); `POST /api/help/events` analogicznie zwróci `stored:false`. To NIE jest błąd typu DATETIME/42704 — to rozjazd kształtu kolumn między producentami tej samej nazwy tabeli. Zgłoszone jako osobne zadanie (spawn_task), nie naprawiane tu.
- **`server/src/jobs/aiWatchdog.ts`** (`watchdog_alerts`, `watchdog_runs`) — brak migracji, ALE zmierzone: zero importerów modułu TS w `server/src` poza samym plikiem (martwy kod, nieosiągalny w runtime). Status: NIEZWERYFIKOWANE JAKO ŻYWE RYZYKO — nie naprawiane (brak wołacza = brak P0).
- **`SuperAdminController.ts` `api_keys` DATETIME (linie 2039/2040/2081/2082)** — kolumny `created_at`/`revoked_at` już istnieją w migrowanym `api_keys` (000_initdb_core_tables.sql, 20260719_baseline_gap.sql), więc runtime `maybeAdd` ALTER jest no-opem chronionym przez `PRAGMA table_info` (osobne, niezależne ryzyko: PRAGMA to składnia SQLite, zachowanie na Postgresie niezweryfikowane w tym dyżurze). Status: NIE DOTYCZY luki DATETIME.
- **`server/src/database/DatabaseInitializer.ts:3645`** (`knowledge_docs.deleted_at DATETIME`) — kolumna już dodana migracjami `940_mw010_vault_document_versions.sql` i `20260719_baseline_gap.sql`. Status: NIE DOTYCZY.

Mechanizm dodatkowo potwierdzony pomiarem: `DbPromise`/`PostgresDatabase.run()`/`.get()`/`.all()`/`.prepare()` WSZYSTKIE wołają `adaptQuery()`, które tłumaczy `DATETIME` (typ) → `TIMESTAMP` w locie — dlatego żaden z powyższych `.run()`-owych plików faktycznie nie rzuca 42704 dziś. Jedyna metoda, która NIE woła `adaptQuery()`, to `.exec()` — to właśnie tamtędy przeszedł oryginalny bug w `emailVerificationService` (`DbPromise.exec(...)`). Żaden z pozostałych plików z `DATETIME` nie używa `.exec()` (zweryfikowane grepem), więc P0 dla nich to nie "42704 wywala rejestrację", tylko "schemat nieodtwarzalny z samego łańcucha migracji" — inny, cichszy wariant tego samego ryzyka.
