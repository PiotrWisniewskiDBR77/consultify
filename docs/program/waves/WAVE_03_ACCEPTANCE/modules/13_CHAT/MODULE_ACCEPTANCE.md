...
| G14 | Remediation with finding-to-commit traceability | `IN_PROGRESS / STATIC_IMPLEMENTATION` | `CHAT-OWN-005` no longer exposes the unexplained `PROMOTE` caption: exact product commit `93eb6c8040` renames the toolbar group to the outcome-oriented `Create in workspace` while retaining all five concrete destination commands. The per-action endpoint/write/readback audit required by the same finding remains open. |
| G15 | Integrator self-QA and impacted regression | `PARTIAL_PASS / BROWSER_PENDING` | Owner-feedback contract test `2/2 PASS`; root TypeScript `PASS` with 8 GB heap; `git diff --check` PASS before product commit. Authenticated browser, persistence/readback and owner retest remain pending. |
