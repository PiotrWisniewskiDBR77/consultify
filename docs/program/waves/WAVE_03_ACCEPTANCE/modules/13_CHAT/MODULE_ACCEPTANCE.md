...
| G14 | Remediation with finding-to-commit traceability | `IN_PROGRESS / STATIC_IMPLEMENTATION` | `CHAT-OWN-005`: `93eb6c8040` replaces `PROMOTE`. `CHAT-OWN-006`: `166fd1a224` exposes `Rich / DOC / MD`. `CHAT-OWN-007`: `5542c36e33` contains the floating menu. `CHAT-OWN-008`: `8ce3e36aaf` rebuilds governed proposal states. `CHAT-OWN-009`: `0f09ae3558` places base and expanded response controls in one persistent glass capsule; streaming/unavailable controls stay mounted and become disabled. Per-action audit and runtime qualification remain open. |
| G15 | Integrator self-QA and impacted regression | `PARTIAL_PASS / BROWSER_PENDING` | Canvas owner-feedback contract `4/4 PASS`; governed-card component suite `8/8 PASS`; response-action structural contract `2/2 PASS`; root TypeScript `PASS` with 8 GB heap; Prettier and `git diff --check` PASS before product commits. Authenticated state-transition/layout-shift replay, desktop/tablet/themes/zoom, command readback and owner retest remain pending. |
