...
| G14 | Remediation with finding-to-commit traceability | `IN_PROGRESS / STATIC_IMPLEMENTATION` | `CHAT-OWN-005`: commit `93eb6c8040` replaces `PROMOTE` with `Create in workspace`. `CHAT-OWN-006`: commit `166fd1a224` puts `Rich / DOC / MD` directly in the Canvas bar and removes duplicate menu paths. `CHAT-OWN-007`: commit `5542c36e33` gives the floating menu an isolated elevated layer, explicit opaque surfaces, viewport-bounded geometry, owned scrolling and trigger-focus restoration. Per-action audit and runtime qualification remain open. |
| G15 | Integrator self-QA and impacted regression | `PARTIAL_PASS / BROWSER_PENDING` | Owner-feedback contract test `4/4 PASS`; root TypeScript `PASS` with 8 GB heap; Prettier and `git diff --check` PASS before product commits. Authenticated desktop/tablet/theme/zoom replay, content-preservation readback and owner retest remain pending. |
