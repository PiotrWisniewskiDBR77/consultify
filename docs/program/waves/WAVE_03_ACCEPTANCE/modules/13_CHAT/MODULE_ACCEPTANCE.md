...
| G01 | Exact baseline and client/server/runtime/DB/migrations | `PASS_FOR_SOURCE_PREFLIGHT` | Exact source HEAD `086afa7f5a6cb2400ce19e6b42fc3258c2179228` on branch `codex/wave3-16-module-acceptance-20260821`; only the separately owned untracked `owner_feedback/` tree was present. A fresh guarded local PostgreSQL database applied `817/817` migrations and was removed after qualification with catalog absence `0`. Backend typecheck PASS. Exact mounted browser/client identity remains pending. |
...
| G04 | Reproducible realistic and boundary fixtures | `READY_PRESEED` | Fixture checkpoint `d32e651f90`. Guarded `scripts/dev/seed-wave3-chat-owner-review.mjs` provisions only exact local `consultify_w3_chat_owner_*` databases with explicit `YES`; seed/readback/reset/drop were proven on fresh `817/817` migrations. Stable owner/member/revoked/foreign identities, sourced conversation and two server-extracted citations produced one pending proposal through the canonical DB-backed service with no LLM/provider call. Every seed requires a new `CHAT_OWNER_FIXTURE_MANIFEST`; canonical post-readback validation precedes an exclusive `wx`/`0600`, secret-free manifest write and overwrite is refused. Repeated seed with a second manifest preserved IDs/hashes/state; reset left `0` fixture rows without deleting manifests; reseed reproduced hashes; drop proved catalog absence `0`. Manifest keeps `deepLinkVerified:false` and emits no password/database credential. A later provisioning diagnostic exposed unrelated migration-order failures and is not counted as fresh-migration evidence. |
| G05 | Functional preflight and cold readback | `IN_PROGRESS` | Exact-current focused replay: `8/8` files, `139/139 PASS`. Fresh disposable-PostgreSQL serial lane: handoff service `14/14`, mounted handoff `8/8`, target mapping `8/8`, partial recovery `7/7`, mounted disconnect/recovery `1/1`, deterministic provider-free mounted stream `1/1`; aggregate RealPG/mounted `6/6` files and `39/39 PASS`, zero skips. Cold independent SQL readback found zero rows in every claimed Chat fixture table; only baseline migration rows `system` remained. Database drop and catalog absence `0` verified. Aggregate focused plus RealPG is `178/178 PASS`. Backend typecheck and `git diff --check` PASS. React `act(...)` warnings remain non-gating harness debt. Stable owner fixture, cold browser/UI readback, live-provider reliability and owner gates remain open. |
...
|---|---|---|---|---|---|---|
| `CHAT-P-OWNER` | allowed | Full sourced-conversation, decision and owner-delivery journey | guarded fixture script; disposable DB reset/drop | conversation/proposal/decision/receipt | active same-tenant OWNER | `READY_PRESEED` |
