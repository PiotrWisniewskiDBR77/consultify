...
| G14 | Remediation with finding-to-commit traceability | `IN_PROGRESS / STATIC_IMPLEMENTATION` | `CHAT-OWN-005`: commit `93eb6c8040` replaces the unexplained `PROMOTE` caption with `Create in workspace` while retaining all five concrete destination commands. `CHAT-OWN-006`: commit `166fd1a224` puts the distinct `Rich / DOC / MD` modes directly in the Canvas bar and removes three duplicate menu paths. Per-action endpoint/write/readback audit and runtime qualification remain open. |
| G15 | Integrator self-QA and impacted regression | `PARTIAL_PASS / BROWSER_PENDING` | Owner-feedback contract test `3/3 PASS`; root TypeScript `PASS` with 8 GB heap; Prettier and `git diff --check` PASS before product commit. Authenticated browser, responsive/focus behavior, content-preservation readback and owner retest remain pending. |
