...
| G14 | Remediation with finding-to-commit traceability | `IN_PROGRESS / STATIC_IMPLEMENTATION` | `CHAT-OWN-005`: `93eb6c8040` replaces `PROMOTE`. `CHAT-OWN-006`: `166fd1a224` exposes `Rich / DOC / MD`. `CHAT-OWN-007`: `5542c36e33` contains the floating menu and restores focus. `CHAT-OWN-008`: `8ce3e36aaf` rebuilds the governed proposal as a rounded layered glass surface with semantic pending/materializable/working/materialized/rejected/failed states and removes the fixed shield accent. Per-action audit and runtime qualification remain open. |
| G15 | Integrator self-QA and impacted regression | `PARTIAL_PASS / BROWSER_PENDING` | Canvas owner-feedback contract `4/4 PASS`; governed-card component suite `8/8 PASS`; root TypeScript `PASS` with 8 GB heap; Prettier and `git diff --check` PASS before product commits. Authenticated desktop/tablet/theme/zoom replay, content-preservation/readback and owner retest remain pending. |
