...
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `PASS_WITH_FINDING` | Desktop verified in both in-app browser and independent Chrome. Independent cold readback passed in EN and PL plus light/dark themes; semantic controls remained operable and Chrome console returned zero warnings/errors. Server `/api/ready` exact SHA passed. `ORG-PF-005` records mixed PL/EN copy. Mobile remains explicitly non-gating. |
...
| `ORG-PF-005` | With account language set to Polish, the Organization shell and section headings translate, but Organization Profile internals and `Context governance` still mix English into the Polish journey. | In-app browser replay on `/organization`, product `ad0766ac4c1000c6c94934a1af1d53c0b4eed19c` | `OPEN_PRE_OWNER_VISIBLE` |
