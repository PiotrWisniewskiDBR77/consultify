...
| G07  | Piotr review card                                              | `READY_FOR_GUIDED_REPLAY_WITH_FINDINGS` | Day 85 exact-marker packet used the shared operator card at `../../GUIDED_OWNER_REPLAY.md`, row 13. Real OWNER login, exact local runtime and five named Organization surfaces are evidenced. Piotr has not performed the review; technical evidence is not owner acceptance. Report: `../../codex/CODEX_DAY85_ORGANIZATION_OWNER_REPORT.md`. |
| G08  | First-impression review                                        | `PACKET_READY_WITH_FINDINGS / OWNER_NOT_REVIEWED` | Day 85 captured and inspected `20 z 20` PNG files in light/dark themes. Mixed PL/EN, clipping, raw identifiers and non-critical crimson remain open. This is an integrator preflight packet; no owner first-impression decision exists. |
| G09  | Guided CX journey review                                       | `PARTIAL_TECHNICAL / OWNER_NOT_REVIEWED` | Real login and navigation to `Identity & Operating Model`, `Goals & Metrics`, `Challenges & Evidence`, `Risks & Opportunities` and `Sources & Claims` passed. The fixture supplies a true full state for only `2 z 5` surfaces; Goals, Challenges and Risks remain empty, so the guided happy path is incomplete. |
| G10  | Alternate-state owner review                                   | `PARTIAL_EVIDENCE_14_OF_20 / OWNER_NOT_REVIEWED` | True empty state was produced only in the owned ephemeral DB after a recoverable full dump, then the dump was restored and readback returned exit `0`. There are `20 z 20` inspected files, but only `14 z 20` required surface/theme/state cells are truthful; six nominal `full` images are empty and are not relabeled. |
