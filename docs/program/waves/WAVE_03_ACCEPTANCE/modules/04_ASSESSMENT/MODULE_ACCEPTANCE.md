...
| G14 | Remediation with finding-to-commit traceability | `BOUNDED_WIP_PARTIAL` | Pure Library and full-height Processes Preview are implemented in preserved WIP. The DRD workspace now exposes Interview/Matrix/Report plus separate Settings; Interview uses one active question with compact sequence steps; the permanent Teresa rail and technical footer are absent; document truth is retained in Settings. The owner-approved label `Insights` is now displayed while the internal `outputs` route/API identifier is deliberately preserved. Current browser evidence is under `../../../../evidence/current-browser-replay/2026-08-23/assessment/`. No commit or acceptance is claimed. |
