...
| `REC-INT-005` | P1 | Unify object-specific row actions | One permission/state-aware action registry per object; right-click and kebab expose the same applicable actions and truthful disabled reasons. | `INT-MENU-OWN-001`, `INT-MENU-EVD-001..008`; escalation wiring `5189ac05d6` | `PARTIAL_TECHNICAL_PASS`: Assignment table/grid share one registry and Escalate now reaches the permission-protected V8 handler (`58/58` focused tests; both typechecks pass). Reassign correctness, the remaining object action matrix, post-refresh persistence replay and owner retest remain open. |
