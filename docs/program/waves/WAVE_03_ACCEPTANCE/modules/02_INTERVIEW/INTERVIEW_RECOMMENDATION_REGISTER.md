...
| `REC-INT-003` | P0 | Restore the dedicated wide question workspace | Remove N-type embedding for single-question work and restore the accepted broad question rail/canvas/navigation while preserving current persistence and workflow corrections. | `INT-QCARD-OWN-001`; implementation `d560464f3f`; baseline before `809e3abe31` | `TECHNICAL_PASS`: single-question presentation is isolated from `NModeShell`; full Interview UI suite passes (`18` files, `92/92`), root typecheck passes and production build passes with the established 8 GB Node heap. Before/after visual parity, deployed journey replay and owner retest remain open. |
...
| `REC-INT-005` | P1 | Unify object-specific row actions | One permission/state-aware action registry per object; right-click and kebab expose the same applicable actions and truthful disabled reasons. | `INT-MENU-OWN-001`, `INT-MENU-EVD-001..008`; escalation `5189ac05d6`; safe reassignment `d13b676fc0` | `ASSIGNMENT_ACTIONS_TECHNICAL_PASS`: Assignment table/grid share one registry; Escalate reaches the permission-protected V8 handler; Reassign updates the existing ID through the tenant membership guard and is disabled with a reason after start (`92/92` focused tests; both typechecks pass). Remaining object action matrices, mounted persistence replay and owner retest remain open. |
