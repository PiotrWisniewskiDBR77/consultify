...
Status: `INTAKE_CLOSED / RECOMMENDATIONS_READY_FOR_PLANNING / NOT_IMPLEMENTED`
...
| `REC-INT-002` | P0 | Restore assignable-template discoverability | Every genuinely assignable published template appears. Ineligible templates show exact reason and recovery. Exact-version pinning remains correct for system, organization and private scopes. | `INT-ASSIGN-OWN-001`; live API record and Railway snapshot still required | API/UI eligibility parity, create + exact-version persistence + cold readback; no blank unexplained selector. |
...
| `REC-INT-004` | P0 | Make approval a visible durable lifecycle | `assigned/in progress → submitted → approved OR sent back with reason → corrected resubmission`; only frozen approved versions enter Insight. Preserve reviewer, reason, versions and history. | `INT-APPROVAL-OWN-001`; existing partial approve/send-back code | Persona/permission matrix, version freeze, downstream gate, persistence and cold readback pass. |
