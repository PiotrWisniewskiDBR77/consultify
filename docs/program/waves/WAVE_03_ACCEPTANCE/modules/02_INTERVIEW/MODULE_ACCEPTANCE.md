...
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `PARTIAL_OWNER_PACKET_20_OF_20_WITH_FINDINGS / DAY120_FIXTURE_INSIGHT_MEASURABLE_6_OF_6_OWNER_ACCEPTANCE_PENDING` | Day 93 added `20 z 20` real-product desktop PNG across five menu surfaces, light/dark and empty/full; report: `../../codex/CODEX_DAY93_INTERVIEW_OWNER_REPORT.md`. Day 107B proved real signed login and ApiGateway with auth bypass OFF, but the fixture had `0` Insights and full-card evidence remained missing; report: `../../codex/CODEX_DAY107_KARTA_INSIGHT_REPORT.md`. Day 120 fixed only the local owner-review seeder. On marker `86eeb60fb3`, disposable local Postgres `6003` and canonical runtime `4906/4907`, login and `GET /api/v8/interview/insights` returned HTTP `200`; the list contained `1 z 1` Insight. The full Preview card measured `6 z 6` requested values: Official Answers `3`, Issues/Risks `2`, Signals/Opportunities `2`, Findings `3`, Confidence `Insufficient`, Actions `0`. Mutation proof was red (`EXIT 1`, `FINAL readback mismatch`, `0` Insights) and green (`EXIT 0`, `1` Insight, `3` official answers), with empty mutation diff after `cp` restore. Empty/full × light/dark screenshots are `4 z 4`; report and SHA-256: `../../codex/CODEX_DAY120_FIXTURE_INSIGHT_REPORT.md`. This verifies fixture measurability, not owner acceptance. Existing UI findings, tablet/mobile, full a11y, the sixth Initiatives surface and owner acceptance remain open. |
...
| G17 | Owner retest decisions for every finding | `PARTIAL` | Otwarte po przeglądzie 2026-09-02: 3 kart zamkniętych jako „ok”, ale z merytoryczną uwagą właściciela, do której nie ma decyzji retestowej: `interview-creator-shell`, `interview-preview-canon`, `karta-insight`. Ekranów modułu w rejestrze grafiki: 7. Mianownik odbioru: 6 ekranów o ocenie A/B — wszystkie 6 mają decyzję właściciela. Poza odbiorem 1 ekranów o ocenie C („nie pokazujemy”) lub D („odłożone”): `insight-artifact`. Decyzje w bazie dla ekranów tego modułu: ok 6, nie 0, poprawka 0. Źródło: `../../../../grafika/ODBIOR_DECYZJE.json` (eksport 2026-09-02T14:15:19, 265 decyzji); mapowanie ekran→moduł: `../../MAPA_GRAFIKA_MODULY_20260902.md`. Linia grafiki: SHA `316bce9dd9aeff1bde71e368968b851467e93411`. Domyka to: naprawa, nowy zrzut i decyzja właściciela osobno przy każdej uwadze. Poprzedni stan bramki: `NOT_STARTED`. |
| G18 | Module accepted on exact SHA and checkpointed | `NOT_STARTED` | Brak podstawy do checkpointu — G17 tego modułu jest otwarte (3 kart zamkniętych jako „ok”, ale z merytoryczną uwagą właściciela, do której nie ma decyzji retestowej: `interview-creator-shell`, `interview-preview-canon`, `karta-insight`). Tagu nie założono. Poprzedni stan bramki: `NOT_STARTED`. |
