...
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `PARTIAL_OWNER_PACKET_20_OF_20_WITH_FINDINGS / DAY107_BLOCKED_BY_FIXTURE_LOCK` | Day 93 added a real-product desktop packet: 5 named menu surfaces × light/dark × empty/full = `20 z 20` PNG and `20 z 20` semantically matched states on exact marker `d80dd85cc7784095eed6f711b42366e5d9b7f74e`, local Postgres, real login and auth bypass OFF. Report: `../../codex/CODEX_DAY93_INTERVIEW_OWNER_REPORT.md`; artifacts and SHA-256 list remain outside repo under `/private/tmp/cx-day93-interview-artefakty`. Day 107 audited the Insight content card on `74a1d733e9`: the canonical Interview owner seeder only selects its owner and does not create `users`/`organization_members`; fresh RealPG therefore stopped before fixture creation (`0 z 4` screenshots, `0 z 6` end-to-end field verdicts). It also found that Findings failures collapse to UI `0`, Confidence correctly renders unknown as `—`, and Actions forces badge `0` in preview rather than counting business actions. Report: `../../codex/CODEX_DAY107_KARTA_INSIGHT_REPORT.md`. No product defect was changed and no `PASS/FIXED/VERIFIED` is claimed. Existing findings remain open: truncated names/header, mixed PL/EN/raw category keys, instruction-vs-product date-format mismatch, template counters inconsistent with a filtered empty state, Assigned empty state without explanation, non-clean organization-context console, and crimson used beyond critical semantics. Tablet/mobile, full a11y, the sixth Initiatives surface and owner acceptance remain unverified. |
