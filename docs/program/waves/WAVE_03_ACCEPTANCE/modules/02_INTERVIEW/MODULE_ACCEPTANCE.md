...
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `NOT_STARTED` | Dyżur 285 rozstrzygnął przyrząd-czy-produkt dla dyżuru 284 (6 ekranów, 48 kadrów; 32/48 błąd, 32/48 nadal zwinięte, 48/48 „z naruszeniami axe"). **K1:** świeża reprodukcja na tym samym markerze (`org-claims-sources` jako wzorzec współdzielony z modułem 01) odtworzyła dyżur 284 co do liczby — przyrząd sprawny. **K2 (axe):** ta sama reguła co w 01_ORGANIZATION — `landmark-one-main/page-has-heading-one/region/heading-order/landmark-no-duplicate-banner/landmark-unique` to opakowanie harnessu (dev-render montuje `InterviewWorkspace`/karty bez `MainLayout`, jedynego źródła `<main>`/`<h1>` w produkcji — `src/layouts/MainLayout.tsx:318,589,654`). Po odjęciu szumu: **5 z 6 ekranów ma REALNE naruszenia axe** (`color-contrast, heading-order`[realny na treści bez opakowania też], `select-name`, `aria-progressbar-name`, `aria-required-children/parent`, `listitem`): `drd-http-workspace, interview-creator-shell, interview-preview-canon, interview-sessions-status, karta-interview`. Tylko `karta-insight` miał wyłącznie szum opakowania. **K3 (32/48 „nadal zwinięte"):** zweryfikowane bezpośrednią interakcją na `karta-interview` (zrzut referencyjny dyżuru 284) — sekcje prawego panelu Powiązania/Źródła i założenia/Komentarze/Historia (`ArtifactRightPanel.tsx:516-524`) otworzone i sprawdzone RĘCZNIE: `aria-expanded` przechodzi false→true po pojedynczym kliknięciu i ZOSTAJE true nawet po otwarciu kolejnej sekcji (multi-open `Set`, brak wzajemnego wykluczania) — pętla `grafika-zrzuty.mjs` (klik sekwencyjny po WSZYSTKICH `[aria-expanded=false]`, w tym rozwijanym menu „Akcje" nad panelem) po prostu ich nie trafia; na `drd-http-workspace` odwrotnie — kategorie macierzy („Procesy Cyfrowe" itd.) są z zamysłu WYŁĄCZNE (otwarcie drugiej zamyka pierwszą, zweryfikowane bezpośrednio), więc harness nigdy nie zobaczy więcej niż jednej otwartej naraz. Zero realnie zepsutych toggle'i. **Console/HTTP:** 404 na `/api/v8/interview/insights`, `/api/v8/interview/assignments/*` — udokumentowana cecha harnessu bez backendu. **WERDYKT:** 5/6 ekranów ma realne, niezależne naruszenia axe — bramka pozostaje czerwona uczciwie. Poprzedni stan bramki: `NOT_STARTED` (dyżur 284, pełny R2 zmierzony). |
