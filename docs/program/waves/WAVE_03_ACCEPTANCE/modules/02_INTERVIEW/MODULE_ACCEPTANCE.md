...
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `PARTIAL_OWNER_PACKET_20_OF_20_WITH_FINDINGS / DAY107B_EMPTY_INSIGHT_LIST_VERIFIED_FULL_CARD_EVIDENCE_MISSING` | Day 93 added a real-product desktop packet: 5 named menu surfaces × light/dark × empty/full = `20 z 20` PNG and `20 z 20` semantically matched states on exact marker `d80dd85cc7784095eed6f711b42366e5d9b7f74e`, local Postgres, real login and auth bypass OFF. Report: `../../codex/CODEX_DAY93_INTERVIEW_OWNER_REPORT.md`; artifacts and SHA-256 list remain outside repo under `/private/tmp/cx-day93-interview-artefakty`. Day 107 first stopped on a missing fixture principal. DAY107B resumed on `c44c1efe18`: the corrected seeder created the synthetic owner, organization and ACTIVE membership; real signed login and ApiGateway worked with auth bypass OFF. The fixture nevertheless contains `0` Insightów although its review session has `3 z 3` answered questions. `GET /api/v8/interview/insights` returned HTTP `200` with an empty list, and the logged-in UI showed the canonical empty state in light/dark (`2 z 4` requested screenshots). Full-card fields remain `0 z 6` E2E because obtaining an Insight would require forbidden AI generation or changing the fixture. Findings fail-soft, Confidence `—` and Actions `0` therefore remain valuable static findings but are neither confirmed nor overturned on a runtime card. Report: `../../codex/CODEX_DAY107_KARTA_INSIGHT_REPORT.md`. No product defect was changed and no `PASS/FIXED/VERIFIED` is claimed. Existing findings remain open: truncated names/header, mixed PL/EN/raw category keys, instruction-vs-product date-format mismatch, template counters inconsistent with a filtered empty state, Assigned empty state without explanation, non-clean organization-context console, and crimson used beyond critical semantics. Tablet/mobile, full a11y, the sixth Initiatives surface and owner acceptance remain unverified. |
