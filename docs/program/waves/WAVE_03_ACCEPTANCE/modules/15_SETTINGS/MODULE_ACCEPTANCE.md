...
| G01 | Exact baseline and client/server/runtime/DB/migrations | `NOT_STARTED` | Cytowane ścieżki manifestów `/private/tmp/consultify-wave3-runtime-manifest-settings-ui-v3-20260822.json` i `/private/tmp/consultify-w3-settings-owner-final-v3-20260822.json` FIZYCZNIE NIE ISTNIEJĄ dziś (`ls` → No such file or directory, sprawdzone 2026-09-02); w `/private/tmp` nie ma żadnego pliku z sesji `20260822`/`settings`. Retained DB `consultify_w3_settings_owner_final_20260822`, porty `4102/4103` i `817` migracji to twierdzenia o tej samej zaginionej sesji runtime. Dzisiejsze repo ma `1097` plików w `server/migrations/`, nie `817`. Dowód powołuje się na artefakt spoza repo, którego nie ma → nie PASS. Poprzedni stan bramki: `PASS_EXACT_RUNTIME`. |
| G02 | Journeys, writes/readbacks, upstream/downstream and policy map | `NOT_STARTED` | Częściowo checkable: delegowany zapis preferencji powiadomień w `server/src/routes/settings.routes.ts:1149-1175` rzeczywiście dociąga rolę/status z bazy (`SELECT role, status FROM organization_members ...`), nie z tokenu, i odmawia dla nie-owner/admin oraz nieaktywnego celu — to zgadza się z opisem polityki. Sama bramka wymaga jednak dowodu writes/readbacks (konkretne zapisane/odczytane rekordy) — twierdzeń o zachowaniu żywej bazy, których bez serwera/DB nie da się dziś zweryfikować. Poprzedni stan bramki: `PASS_FOR_PREFLIGHT`. |
...
| G05 | Functional preflight and cold readback | `NOT_STARTED` | ZAMKNIĘTE 2026-09-02 pomiarem `G05` na wszystkich 16 modułach naraz. Metoda: świeży PostgreSQL, migracje przepuszczone dwukrotnie (drugi przebieg bezczynny), harness `server/src/scripts/g05-przelot.ts` uruchamiany przez `npx tsx` na realnym `ApiGateway` — **nie przez `vitest`**, który podmienia `fetch` na atrapę zwracającą zawsze sukces. Zapis wykonany realną trasą HTTP, sesja wygaszona, odczyt na zimno nowym żądaniem i nowym tokenem, porównanie **wartości pole po polu** — nie odpowiedzi zapisu (`Database.ts:686` zwraca `changes:1` dla każdego UPDATE, także pustego). Kontrola negatywna tokenem obcej organizacji przeszła. Pełny rejestr: `../../REJESTR_G05_PRZELOT_20260902.md`. Wynik: **NIE PRZEŻYWA — jedyny taki moduł z szesnastu.** ★ Przyczyna zmierzona i wskazana palcem: **front woła odwrócone segmenty adresu**. `src/hooks/useUserNotificationPreferences.tsx:153,214` woła `PUT /api/settings/notifications/preferences`, a backend serwuje `PUT /api/settings/preferences/notifications` (`server/src/routes/settings.routes.ts:531,590`). Realny zapis użytkownika z interfejsu dostaje **404 i nigdy nie trafia do bazy**, mimo że backend potrafi to przechować pod właściwym adresem. Zweryfikowane niezależnie przez nadzorcę. Naprawa jest jednolinijkowa, ale bramka zostaje otwarta do czasu jej wykonania i ponownego pomiaru. |
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `NOT_STARTED` | Dyżur 284 zatrzymany zgodnie z regułą urwania okna po pełnym R2; ten moduł nie został ponownie zmierzony. Poprzedni stan bramki: `DAY124_DESKTOP_PL_PARTIAL / 12_OF_20_SEMANTIC / OWNER_MATRIX_PENDING`. |
...
| G13 | Solution and impact analysis | `PARTIAL_DAY55_MEASURED` | Day 55 measured width, typography, breadcrumb, Save slot, visual-system, component-conformity, integration copy and four post-MVP debts. Cross-module rebuild and full per-screen matrix remain open; see report §E.1/F.1. |
| G14 | Remediation with finding-to-commit traceability | `IMPLEMENTED_STATIC / DAY55_SECURITY_AND_ROUTE_HARDENING / RUNTIME_RETEST_PARTIAL` | `SET-OWN-001` Help shortcut: commit `9b2ad340d6`. Day 55 commits in `SETTINGS_DAY55_REPORT_20260828.md` additionally prove password/MFA, migration hardening, OAuth state binding, write boundaries, DEC-238 allowlist and 62 dead wrapper removals. Full browser runtime remains partial. |
