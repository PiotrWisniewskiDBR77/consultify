...
|---|---|---|---|
| G00 | Scope, routes, dependencies, 82-task links and exclusions | `NOT_STARTED` | `SET-MVP-DELETE-001` approved-out boundary |
...
|---|---|---|---|---|---|
| _prepare before G07_ | `/settings` | _pending_ | Change preference → security/privacy state → refresh → deletion boundary | Automated destructive deletion | Settings findability, security confidence, transparent limits, confirmation clarity |
...
Accepted SHA: —
Date: —
Accepted-out/deferred: Destructive deletion remains OFF pending later policy/release authorization.
Evidence manifest: —
