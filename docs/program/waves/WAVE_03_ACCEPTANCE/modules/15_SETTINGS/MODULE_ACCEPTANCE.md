...
| G14 | Remediation with finding-to-commit traceability | `IMPLEMENTED_STATIC / RUNTIME_RETEST_PENDING` | `SET-OWN-001` Help shortcut: commit `9b2ad340d6` removes every Settings `InfoButton` import/render and deletes the CSS hiding workaround. This is implementation evidence, not owner acceptance. |
| G15 | Integrator self-QA and impacted regression | `PASS_STATIC_AND_COMPONENT / BROWSER_PENDING` | Full Settings source-tree absence guard plus Profile/Data Controls smoke and Organization navigation regression passed `7/7`; root TypeScript passed. Exact-SHA browser/keyboard replay remains required. |
