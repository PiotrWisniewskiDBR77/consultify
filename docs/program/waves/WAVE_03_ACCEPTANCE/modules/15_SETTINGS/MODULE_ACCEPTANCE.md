...
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `DAY124_DESKTOP_PL_PARTIAL / 12_OF_20_SEMANTIC / OWNER_MATRIX_PENDING` | Day 124 on exact `a1265154b73f` and local RealPG `:6007` produced 20/20 PNG files, but only 12/20 represent the declared surface: MEMBER is silently redirected to Profile on 8/10 alternate-state captures. OWNER surfaces are PL, but raw `USER`, missing denominators/permission explanations and non-blocking crimson remain. System light/dark emulation does not change the account-bound product theme, so two real themes are not proven. Console warnings/errors: 0. Tablet, EN, a11y and owner replay remain open. See `../../codex/CODEX_DAY124_USTAWIENIA_OWNER_REPORT.md`. |
