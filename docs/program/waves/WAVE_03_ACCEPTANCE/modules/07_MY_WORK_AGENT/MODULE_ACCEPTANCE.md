...
| G04 | Reproducible realistic and boundary fixtures | `IN_PROGRESS` | Real-PG source fixture is deterministic, idempotent and leaves zero rows. Mounted materialization, random-isolated canonical writer, Redis and lease fixtures cover the boundary matrix; database residue checks found no orphan proposal/approval/receipt/outbox rows and no run-created organizations. Deterministic local owner fixture script `scripts/dev/seed-wave3-my-work-owner-review.mjs` is ready and fail-closed; syntax, SQL readback and reset guard passed. Seed/mounted UI readback waits until the Organization owner screen is released. |
