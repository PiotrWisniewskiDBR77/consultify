...
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `NOT_STARTED` | Dyżur 285 rozstrzygnął przyrząd-czy-produkt dla dyżuru 284 (7 ekranów, 56 kadrów; 40/56 błąd, 40/56 nadal zwinięte, 56/56 „z naruszeniami axe"). **Odkrycie osobne od K1-K4:** ten moduł ma NAJSŁABSZY pomiar z czterech — dowód JSON (`a11yNaruszenia`/`sekcjeNadalZwiniete`) istnieje realnie tylko dla **2 z 7 ekranów** (`karta-tool`, `tools-sesja-wyjscie`); pozostałe 5 (`tools-outputs-insights-tab, tools-swot-initiative-proposal, tools-swot-library-detail, tools-swot-report, tools-swot-session-workspace`) mają PNG na dysku (`/private/tmp/cx-day284-g06-artefakty/R2/`), ale ZERO wpisu w wynikowym JSON — dokładnie 5×8=40 kadrów, więc te 40 „błąd"/„nadal zwinięte" w tabeli dyżuru 284 to te same 5 ekranów bez zmierzonych danych, nie 40 niezależnych defektów. To NIE jest artefakt tego dyżuru do rozstrzygnięcia — to luka pomiarowa dyżuru 284 (skrypt urwał zapis wyniku dla tych 5 przed dopisaniem do JSON), którą dziedziczymy bez naprawiania (poza zakresem K1-K4). **K1:** przyrząd sprawny — potwierdzone na wzorcu współdzielonym z 01_ORGANIZATION (identyczna reprodukcja). **K2 (axe, tylko 2 zmierzone ekrany):** `karta-tool` ma wyłącznie opakowanie harnessu (`landmark-one-main`, `region` — brak `MainLayout` w dev-render, jak w pozostałych 3 modułach); `tools-sesja-wyjscie` ma REALNE naruszenie (`color-contrast`) niezależne od opakowania. **K3:** te same 2 ekrany zweryfikowane tą samą metodą co 01/02 — sekcje prawego panelu (Powiązania/Źródła i założenia/Rezultaty/Komentarze/Historia) to `ArtifactRightPanel` multi-open, pętla harnessu ich nie trafia, zero zepsutych toggle'i. **WERDYKT:** bramka pozostaje czerwona z DWÓCH niezależnych powodów — (a) realny defekt na `tools-sesja-wyjscie`, (b) 5/7 ekranów w ogóle nie ma kompletu danych axe/rozwinięcia i wymaga ponownego pomiaru, nie tylko reinterpretacji. Poprzedni stan bramki: `NOT_STARTED` (dyżur 284, R2 częściowo niekompletny dla tego modułu). |
