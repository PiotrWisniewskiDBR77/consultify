...
| `GEN-2` | **Dokument Word** | działa; pętla szablonowa domknięta end-to-end, znacznik w wygenerowanym DOCX | `DEC-299`, `DEC-327`, D-8; dyżury 77, 90, 185, 190 | `PARTIAL` — dyżur 185 zachował i oznaczył niepoparte liczby; dyżur 190 usunął destrukcyjne kasowanie przez `obviousEnglish` i udowodnił realny DOCX przez `HTTP -> ApiGateway -> verifyToken -> PostgreSQL -> LLM -> DOCX`: `1` wywołanie OpenRouter, `generate 200`, readback `1/1`, eksport `200`, `269` słów, `0` fraz kasujących, widoczny znacznik assumption, mutacja tej samej odpowiedzi `0 -> 5` kasowników. Realna ścieżka jest `PROVEN`, ale bramka pozostaje `PARTIAL`: K5 jakości treści `FAIL` (surowy Markdown i zwarty akapit), a osobny próg graficzny D-8 `15/18` nie został zmierzony. |
