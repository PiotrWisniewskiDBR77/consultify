...
Current gate: `TECHNICAL_BROWSER_PASS / OWNER_REVIEW_BLOCKED_PRESENTATION_EMPTY / RESTRICTED_POLICY_CONFIRMATION_REQUIRED`
...
| G07  | Piotr review card                                              | `READY_WITH_BLOCKER`                      | Day 76 odtworzył na markerze `8d254e6…` dokładną macierz pięciu powierzchni: `20 z 20`, plus dwa zrzuty kart. Raport: `../../codex/CODEX_DAY76_MATERIALS_OWNER_REPORT.md`. Wszystkie `8 z 8` znanych defektów nadal występują. |
| G08  | First-impression review                                        | `EVIDENCE_COMPLETE_BLOCKED`               | Lokalna persona OWNER zalogowała się przez prawdziwy Gateway bez auth bypass. `22 z 22` obrazów obejrzano wizualnie. Pełna karta prezentacji nadal renderuje niemal pusty ekran mimo czterech slajdów w DOM; pakiet nie relabeluje jej jako sprawnej. |
| G09  | Guided CX journey review                                       | `PASS_TECHNICAL_REPLAY / VISUAL_BLOCKER`  | Gateway na 4700 i realna PG 5948 potwierdziły login, dane oraz końcowy readback `10 z 10`. Guided review nie może zostać uznany za wizualnie gotowy, dopóki pełna karta prezentacji pozostaje pusta. |
| G10  | Alternate-state owner review                                   | `EVIDENCE_PACKAGE_READY_WITH_FINDINGS`    | Pięć powierzchni pokazano w pełnym i kontrolowanym pustym stanie, jasno i ciemno. Po pustym przebiegu pełny dump odtworzono, a readback wrócił do `10 z 10`. Puste stany mają co najmniej cztery różne kompozycje, a Arkusze mówią językiem implementacji. |
...
| `MAT-D76-004` | Pełna karta prezentacji pokazuje `Karta 1 z 4`, ale jej powierzchnia jest niemal pusta. HTTP `200` dostarcza `deck_json.cards[4]`; `DeckBuilder` przekazuje je do `CardCanvas`, lecz dolny pasek `h-full ... flex-shrink-0` zajmuje całe `672px`, redukując główny canvas do `0px`, który następnie przycina `overflow-hidden`. Przyczyna: `DeckBuilderBottomBar.tsx:25` + montaż poza Artifact Studio w `DeckBuilderMelsView.tsx:404-407`; ścieżka canvasu `ExecutiveModuleShell/index.tsx:463,485,587-595`. | `/private/tmp/cx-day76-artefakty/day76-defect-04-presentation-full-dark.png`, SHA-256 `bec41f9c626993d6f8a67709a0c8eff0e3cae67ef62ba28b5cf534b712529878`; runtime DOM: canvas `487×0`, pasek `1184×672`. | `DIAGNOSED_OPEN_BLOCKER` |
