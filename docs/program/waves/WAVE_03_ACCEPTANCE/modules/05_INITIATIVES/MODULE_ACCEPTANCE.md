...
| `INI-OWN-007` | 2026-08-23 | „Nowy plan (...) What If (...) Analyze AI (...) dobrego ganta” | Plan analysis | Plan | Existing broad table/form does not express the intended scenario workspace. The current working candidate removes raw period JSON, creates a bounded weekly horizon from readable business inputs and supports period add/edit/remove. | Versioned plan-analysis registry; a new analysis selects initiatives/statuses, includes/excludes each item, AI proposes dependency-aware ordering, and the user edits a weekly Gantt before saving name/version/author. | Enables comparable planning scenarios without mutating source initiatives. | Owner review transcript | — | `P1` | `PARTIAL_PRODUCT_IMPLEMENTATION / OWNER_RETEST_PENDING` | working candidate after `9c4eabf8c1` | Focused Plan domain/surface regression `2 files / 7 tests PASS`; visible Gantt, include/status selection, AI proposal, comparison, browser replay and cold readback remain open. | `PENDING` |
