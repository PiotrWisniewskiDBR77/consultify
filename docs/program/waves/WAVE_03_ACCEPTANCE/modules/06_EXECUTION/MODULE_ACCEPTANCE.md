...
| G06 | Desktop/tablet, PL/EN, themes, states, a11y, console/HTTP | `PARTIAL_DESKTOP_PL_CURRENT_REPLAY` | Current local PL replay on `127.0.0.1:4390` captured all five surfaces with deterministic review data plus a Work item opened as a dynamic Menu 3 document. Evidence manifest: `../../../../evidence/current-browser-replay/2026-08-23/execution/MANIFEST.md`. Sterowanie still exposes the historical closure block above its register. Tablet, EN, theme, keyboard/a11y, clean console/HTTP capture and owner visual judgment remain open. |
