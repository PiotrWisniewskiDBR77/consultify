...
| `INI-C06`                  | `PARTIAL_PRODUCT_IMPLEMENTATION`                     | Versioned Plan domain and API foundation exist. Commits `1dc1761cfd` and `02aca5ee25` remove raw period JSON, generate and edit a bounded weekly horizon, expose lifecycle/status filtering and include/exclude selection, and render a clickable weekly timeline with one-period movement. Focused Plan regression: `2 files / 7 tests PASS`; component lint has zero findings. | Governed AI proposal, richer Gantt interaction, comparison, browser qualification and cold reopen remain open.                           |
