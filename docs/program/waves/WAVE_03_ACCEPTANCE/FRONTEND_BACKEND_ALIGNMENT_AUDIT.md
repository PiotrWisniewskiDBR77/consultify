...
|  13 | Chat            | `OWNER_VISUAL_NAVIGATION_REVIEW_COMPLETE / REMEDIATION_AND_INTEGRATION_AUDIT_REQUIRED` | Governed handoff UI remains canonical. On exact owner-review SHA `75bed3bb6485`, Piotr completed `Approve -> Create document -> Document created`; 25 screenshots and 17 correction/audit tasks are registered. `/task` and `/decision` continue to fail closed and the retired direct endpoint remains `410`. | Canvas persistence and every visible action API, branches, Signals, private/organization folder context, voice and live-provider behavior remain unqualified. Remediate, run integrator evidence, then obtain Piotr's formal exact-SHA retest verdict. |
