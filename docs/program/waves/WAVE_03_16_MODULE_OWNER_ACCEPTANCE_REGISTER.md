...
| 8 | `RES` | Results — `/results` | Inspect KPI/ROI/OKR, add an allowed observation and follow lineage to source. | Legacy writer absence/retention, foreign tenant, immutable history, retry. | `READY_FOR_REPLAY` | Exact-current mounted replay plus disposition of five canonical current writers. |
