...
|---:|---|---|---|---|---|---|
| 1 | `CHAT` | Chat — `/chat` | Continue a sourced conversation, approve a governed proposal and cold-reopen it. | Provider failure, stale/replayed approval, foreign tenant, no false citation/success. | `OWNER_VISUAL_NAVIGATION_REVIEW_COMPLETE / REMEDIATION_REQUIRED` | Seventeen tasks from the exact-SHA owner round require remediation/integrator qualification; formal owner verdict and final retest remain open. |
