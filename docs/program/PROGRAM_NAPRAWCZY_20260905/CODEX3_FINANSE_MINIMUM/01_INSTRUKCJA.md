...
> obowiązuje ta ramka.
> **Stan dokumentu: WYDANY — 2026-09-11 01:20, CTO (Fable), marker `19440011e9` = tip `origin/integracja/20260911`**
>
...
| `src/services/api/financeV2.types.ts` | **★ WĄSKA LICENCJA — wyłącznie DODANIE DTO** dla klienta odczytu zbiorczego rodowodu (`E3`, np. `BulkVersionLineageDto`). Klient z `financeV2.api.ts` jest nienapisywalny bez typu — ten wiersz zamyka lukę wykrytą w odbiorze W1/W2 (`97_ODBIOR_W1_W2.md` §1/FIX-2). **Zakaz zmiany istniejących pól/typów** | Czerwony kontrakt + brief |
...
| `tests/**` (NOWE pliki), `server/src/**/__tests__/**` (NOWE pliki), `src/components/**/__tests__/**` (NOWE pliki) | **★ PEŁNA LICENCJA**, z zastrzeżeniem `Z18`, `Z31`, `Z43`. **Nowe pliki w `tests/` wymagają `git add -f`**. Front (`src/components/**/__tests__/**`) dopisany w odbiorze W1/W2 (`97_ODBIOR_W1_W2.md` §1/FIX-2) — ta sama luka co wiersz `financeV2.types.ts` powyżej | — |
