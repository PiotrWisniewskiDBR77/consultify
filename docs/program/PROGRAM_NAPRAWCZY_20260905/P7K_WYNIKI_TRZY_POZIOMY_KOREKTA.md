...
## 11. Wklejka dla Codexa (aktualna — zastępuje wcześniejszą)
```
ZADANIE P7K — Wyniki: KPI · OKR · ROI dokładnie wg źródła prawdy właściciela. Praca do celu, w trzech częściach, prototyp przed kodem.
...
ŹRÓDŁO PRAWDY (czytaj w całości, w tej kolejności): docs/modules/07_rezultaty/SSOT_WYNIKI_KPI_OKR_ROI.md → docs/program/grafika/WYNIKI_ZALOZENIA_GRAFICZNE_20260905.md → docs/program/grafika/ROI_METODYKA_WLASCICIELA_20260905.md → załączniki docs/modules/07_rezultaty/zalaczniki/ (arkusz raportu KPI: 138 mierników, kolumny elementów, CEL/Rezultat per okres; karta wskaźnika) → ta paczka (docs/program/PROGRAM_NAPRAWCZY_20260905/P7K_WYNIKI_TRZY_POZIOMY_KOREKTA.md). W razie sprzeczności wygrywa SSOT. Nie pytaj właściciela — rozstrzyga dokument; niejasność → STOP z cytatem.

CEL: Menu 2 Wyników = KPI · OKR · ROI; każda funkcja otwiera TABELĘ RAPORTÓW. KPI (proces): raporty okresowe → raport = tabela mierników grupowana po obszarze z elementami z arkusza i parą CEL/Rezultat per okres + YTD, podsumowanie, „Dodaj miernik” → karta N miernika (Wyniki · Kontrakt · Pomiary · Odchylenia · Działania · Raporty · Historia). OKR (człowiek): raporty (zakres × cykl) → tabela rezultatów grupowana temat → cel, właściciel jako kolumna i filtr domyślny, START/CEL/BIEŻĄCA, postęp, pewność, termin, check-in → karta celu z rezultatami JAKO SEKCJĄ (bez osobnych stron KR). ROI (inwestycja): tabela analiz ze wskaźnikami i rekomendacją → karta N w trzech częściach: Założenia → Wyliczenia → Realizacja (Expected vs Actual, prawdziwość założeń, ROI po realizacji). Mechanika odchylenia (KPI i OKR): rezultat poza limitem → kolor → zgłoszenie do odpowiedzialnego (powiadomienie + Skrzynka) → automatyczna karta działania z polami z arkusza; w normie nic się nie dzieje.

KOLEJNOŚĆ PRACY: KROK 0 — mapowanie elementów miernika/rezultatu/analizy na istniejący schemat i DTO (rvn_kpi_*, kpiTool, okrApi, roiCaseDetailApi) → raport „jest / brak / dodać (migracja addytywna)”; KROK 1 — PROTOTYP w dev-render (dane DBR77 z arkusza: 3 ekrany KPI, 3 OKR, 2 ROI, 1440 jasny+ciemny) → zrzuty dla nadzorcy → STOP do akceptu; KROK 2 — część A (poziomy KPI/OKR: usunąć KpiCardSetPage i trasę /zestawienie, pigułkę „Wszystkie wskaźniki”, OkrKeyResultSetPage/CardPage; przepiąć istniejące ResultsKpiScorecardDetailPage jako L2; kolumny okresów) → KROK 3 — część B (wyzwalacz odchylenia + karta działania na istniejących przypadkach odchyleń) → KROK 4 — część C (ROI trzy części). Każda część kończy się §10 i raportem, dopiero potem następna.
KANON: StandardModuleBar/StandardTable/StandardPreview/N-card/ArtifactRightPanel, tokeny c-*, zero primary-*, czerwień tylko „poza limitem/otwarta karta działania”, kebab pionowy, i18n pl+en, nazwiska nie UUID, brak danych „—” nigdy 0. Moduł Wyniki niezamrożony — bez markerów. Serwer: tsc --build musi przejść; migracje addytywne; testy przeliczeń NPV/IRR/PP na przykładach z metodyki.
CEL OSIĄGNIĘTY = §10 + §16 tej paczki (rg usuniętych stron = 0; testy poziomów, odchylenia i przeliczeń z dowodem mutacyjnym; zrzuty L1/L2/L3 dla KPI, OKR, ROI z .json: aside ≤ 1, 0 błędów konsoli, 0 ≥400 poza udokumentowanymi, kolumny CEL/Rezultat ≥ 2 okresy + YTD, trasa KR nie istnieje, przepływ Playwright „rezultat poza limitem → czerwony wiersz → wpis w Skrzynce odpowiedzialnego → otwarta karta działania”). Raport per część. Zakazy: --no-verify, git stash, nowe komponenty tabel/kafelków, flagi, pytania do właściciela.
```
...
## 16. Źródło prawdy i część C (ROI) — 05.09 wieczór
Od teraz ŹRÓDŁEM PRAWDY dla tej paczki jest `docs/modules/07_rezultaty/SSOT_WYNIKI_KPI_OKR_ROI.md` (z załącznikami w `docs/modules/07_rezultaty/zalaczniki/`); w razie sprzeczności z §4–§15 wyżej wygrywa SSOT.
**Część C — ROI**: karta analizy rozłożona na trzy części (Założenia → Wyliczenia → Realizacja) wg SSOT §4 i `WYNIKI_ZALOZENIA_GRAFICZNE_20260905.md` §4; mapowanie na istniejące sekcje `RoiCaseCardSections.ts` i dane `roiCaseDetailApi.ts` (koszty, założenia, baseline, polityka kalkulacji, PIR); brakujące pola (scenariusze, wrażliwość, klasy korzyści, wariant bazowy, przegląd PIR z wariancją) = krok 0 mapowania + addytywne migracje; tabela analiz z kolumnami wskaźników i fazy. Kolejność: A (poziomy KPI/OKR) → B (odchylenie) → C (ROI). Każda część ma własny prototyp dev-render → zrzuty nadzorcy → akcept właściciela → budowa.
§10 dla C: zrzuty L1 (kolumny wskaźników, rekomendacja jako chip neutralny), L2 z trzema częściami w lewej nawigacji, sekcja Realizacja z tabelą Expected/Actual/Wariancja; zero „0” dla niepoliczonych; testy przeliczeń NPV/IRR/PP z przykładami z metodyki (1 mln / 400 tys. → PP 2,5 roku; NPV=0 przy IRR).
