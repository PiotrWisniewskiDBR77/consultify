...
## Fixture cleanup readback

Root read the actual cleanup log through COMMIT: ten categories report zero (aggregateState, relations, cards, candidate, receipts, audit, outbox, history, legacy, deliveryReceipts). The owned fixture removed20 aggregates,36 cards,1 candidate and62 receipts/audit/outbox entries each. Existing organization/project/accounts/policy are outside this cleanup scope. No retained forecast record remains for future tests; create separately scoped fixtures for subsequent work.
