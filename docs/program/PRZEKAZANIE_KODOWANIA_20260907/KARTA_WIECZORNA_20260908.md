...
Staging: `c36dc94359` (punkt cofnięcia `94b57ed9ad`). Dzień: 20 scaleń na gałęzi `mvp/inicjatywy-lancuch-20260907`, każde z zieloną bramką (tsc serwera 0, tsc frontu 192 = baza, build, bramka językowa).
...
- **Wersja angielska**: serwer wysyła kody błędów, front je tłumaczy; enumy przez słownik („Unknown” zamiast surowego UNKNOWN); moduł Inicjatywy w EN bez polskich słów w interfejsie; w Realizacji 209 polskich domyślnych już zamienionych, reszta (teksty na sztywno, daty, belka Zasobów) w toku (J7b).
