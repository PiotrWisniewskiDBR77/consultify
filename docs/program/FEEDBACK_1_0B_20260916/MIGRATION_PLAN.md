...
- zatrzymuje plan dla tabel bez PK, zmienianych kolumn będących częścią PK lub kolumną generowaną/identity, kolumn tekstowych mieszających parsowalne JSON i zwykły tekst oraz encji znalezionej w kluczu obiektu JSON;
...
- liczbę surowych delimiterów `<` i `>` przed i po jako kontrolę negatywną; dzięki temu bezpieczne `&amp;lt;` → `&lt;` nie daje fałszywego alarmu;
...
5. kontrolę, że nie przybył żaden surowy `<` / `>`; fixture `&amp;lt;script&amp;gt;` ma dać bezpieczny tekst `&lt;script&gt;`, nigdy element HTML.
