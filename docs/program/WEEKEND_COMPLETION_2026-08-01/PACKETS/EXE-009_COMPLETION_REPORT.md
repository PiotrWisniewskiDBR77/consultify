...
## Canonical integration acceptance — 2026-08-02

**Verdict: ACCEPTED after corrective integration work.** During verification
against the canonical real-Postgres schema, the Results writer was found to
copy `initiative_kpis.id` into `initiative_benefits.kpi_id`, whose FK actually
targets the separate legacy `kpi_definitions` registry. This made ordinary
KPI-backed delivery retry forever despite the source-branch tests.

The integrated implementation now leaves the legacy link null and preserves
the real lineage in `source_initiative_kpi_id` (migration 938, correct FK,
source-KPI uniqueness). Migration 938 also narrows migration 936's fallback
index so multiple planned KPIs can coexist. A regression test proves two
planned KPIs create exactly two durable benefit rows and retries create no
duplicates. Migration 933 was separately corrected to use PostgreSQL boolean
literals (`TRUE`) rather than integers.

Acceptance evidence on canonical integration HEAD:

- real PostgreSQL EXE-09 suite: **27/27 passed** (including concurrency,
  retries, restart reconciliation, tenant/RBAC boundaries, honest Finance
  `NEEDS_DECISION`, fallback and multi-KPI lineage);
- real PostgreSQL EXE-08 closure/evidence regression: **18/18 passed**;
- ClosureSection UI contract: **11/11 passed**;
- `npm run type-check`: passed;
- `npm run build`: passed;
- migration 938 replay: passed; additive statements/index recreation are
  idempotent;
- `git diff --check`: passed.

The Finance leg remains deliberately `NEEDS_DECISION`: EXE-09 does not invent
approval or book a planned/observed value as realized value automatically.
