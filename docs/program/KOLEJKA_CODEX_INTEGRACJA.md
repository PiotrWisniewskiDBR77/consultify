...
> **NOTA NADZORCY 2026-09-03 (wieczór):** numeracja pozycji 286–291 w tej kolejce ROZJECHAŁA SIĘ z wydanymi instrukcjami. Numer dyżuru nadaje PLIK `docs/program/waves/WAVE_03_ACCEPTANCE/codex/INSTRUKCJA_DYZUR_<nr>.md`, nie ta kolejka. Wydane: **286** = samokontrola testów G15 (16 modułów), **287** = pierścień fokusu `c-focus`, **288** = bramka modułu na 270 trasach finansów v8, **289** = pomoc (`help_*` schemat do kodu) + potwierdzenie martwego `NotificationSettingsV2`. Tematy zapisane tu pod 286–291 („rodzina podglądu”, „trzy dziury cross-org”, „surowe ID”, „77 uwag”, „W5 staging”, „finalny przebieg”) zachowują treść, ale dostaną kolejne wolne numery przy wydaniu (03.09 wieczór wydano też **290** dowody G19, **291** dowody runtime P0/P1, **292** menu akcji Wywiadu, **293** Biblioteka metodyk Oceny, **294** trzy defekty Czatu, **295** Moja Praca/Inicjatywy dowody — **296** wycieki błędów w 305 miejscach tras, **297** martwe komponenty od korzenia — **298** silnik raportu Oceny DRD wg zaakceptowanego prototypu — następny wolny: **299**).
>
> **NOTA NADZORCY 2026-09-03 (noc):** **299** kreator wywiadu po wlaczeniu (odbior lista czekowania, 4 czerwone testy a11y), **300** front — prezentacja bledow (rodzenstwo 296, warunek startu: 296 scalony), **301** licznik P0/P1 wedlug reguly `E1` (`DEC-2026-09-03-362`) jako skrypt + generowany rejestr blokujacych `G20`, **302** prawy panel Idei/Notatnika (B3), **303** preferencje Czatu (B6), **304** historia Czatu prywatna/organizacyjna (`CHAT-OWN-013`), **305** wspolny standard kart 7 typow (`XMOD-CARD-REC-001`), **306** dwa brakujace etapy Dynamic SWOT (`TLS-SWOT-OWN-001`) — **302-306 = FALA 2 (`DEC-2026-09-03-384`), kazdy z prototypem ZA FLAGA domyslnie OFF i zrzutami do akceptu PRZED budowa; scalenie i wlaczenie wylacznie po akcepcie wlasciciela na zrzutach**; **307** przelot bezpieczenstwa cross-org na realnej bazie, **308** jezyk polski (klucz istnieje ≠ przetlumaczony), **309** inwentarz testow PUSTYCH z dowodem mutacyjnym, **310** schemat poza migracjami (DDL w locie), **311** crimson poza semantyka — modul Czatu (**bez scalenia, do akceptu**), **312** domkniecia po odbiorach adwersaryjnych 03.09 (296 → 297 → 293 → 292 → 298 → 295; worktree `/private/tmp/cx-day{292,293,296,297}-*` JUZ ISTNIEJA i maja byc REUZYTE). 299-311 wydane 03.09 noc; 312 domkniecia; **313 domkniecia 2 (wycieki 35→0, AppError, 8 tras 500)**; **nastepny wolny: 314**.
