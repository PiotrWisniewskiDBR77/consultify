...
function inspect(file) {
  ...
  const common = {
    ...
    fonts: uniq([
      ...matchAll(xml, /(?:typeface|w:ascii|w:hAnsi)="([^"]+)"/g),
      ...matchAll(xml, /<name val="([^"]+)"/g),
    ]).filter((x) => !x.startsWith('+')),
    crimsonHits: (xml.match(/(?:B42318|DC143C|CRIMSON)/gi) || []).length,
    branding: {
...
function compare(id, appRel, mockRel, note) {
  ...
  const requirements = {
    aptosAndArialFallback: app.exists ? app.fonts.some((f) => /^Aptos/i.test(f)) && app.fonts.some((f) => /^Arial$/i.test(f)) : false,
    coBrandingConsultifyDbr77Client: app.exists ? app.branding.consultify && app.branding.dbr77 && app.branding.northwind : false,
    zeroCrimsonFilewide: app.exists ? app.crimsonHits === 0 : false,
  };
