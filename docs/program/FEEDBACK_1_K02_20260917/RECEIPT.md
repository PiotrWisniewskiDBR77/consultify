...
4. **P2 DIRTY po ręcznym zapisie:** zamknięte tokenem rewizji. Anulowanie debounce zwraca rewizję zastępowanego szkicu. Potwierdzony `recordAnswer` przechodzi do `SAVED` tylko wtedy, gdy w czasie żądania nie powstała nowsza edycja; nowsza edycja zachowuje `DIRTY` i własny timer autosave.
...
- Focused suite: **8 plików / 46 testów PASS**.
...
- Mutacja wyścigu `SAVED`: edycja B wykonana po rozpoczęciu zastępczego zapisu A pozostaje `DIRTY`, jej timer wykonuje autosave i dopiero wtedy przechodzi do `SAVED`.
- Niezależny review wykrył przed finalnym freeze wyścig fałszywego `SAVED`; poprawiono go tokenem rewizji i dodano dokładny test regresji. Finalny re-review: **ACCEPT, P0=0/P1=0**, własny przebieg recenzenta 16/16 PASS.

Focused suite obejmuje:

| Obszar | Wynik |
|---|---:|
| `useMethodWorkspaceSave.test.ts` | 10/10 |
