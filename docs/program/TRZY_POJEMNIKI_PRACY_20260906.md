...
| Inicjatywy | Analiza | A3 karty N + wycena + zatwierdzenie | Codex P4 | silnik zatwierdzeń | B | karta N z wyceną | 🔧 kandydat W88 zapisuje wycenę, podstawę, autora i czas w istniejącym review DEC-489; flaga pozostaje OFF do odbioru i wdrożenia CTO | `DefinitionCardContent.tsx`; `postgresMaterialCommandUnitOfWork.ts`; migracja addytywna `20262240_initiative_card_estimate.sql`; kolejka akceptów #12 | 15.09 |
...
**W88 / A3:** paczka karty N z wyceną dopisuje pozycję **12** bez zmiany istniejącej
numeracji. Pozycja ma dokładnie jeden obraz, docelowy URL stagingu oraz komplet flag potrzebnych
do żywego zrzutu; po jej dodaniu kolejka ma jedenaście otwartych pozycji.
...
| 12 | po 18.09 | **A3** karta N + wycena + zatwierdzenie | `evidence/a1-card-estimate/a3-initiative-card-estimate-en-light.png` — karta N z wyceną 40–60 h, autorem/czasem i widocznym przyciskiem zatwierdzenia | `https://staging.consultify.ai/initiatives` → podgląd inicjatywy → karta Definition | `VITE_INITIATIVES_PORTFOLIO_ANALYSIS` + `ENABLE_INITIATIVE_APPROVAL_V2` | kandydat musi zostać scalony i wdrożony przez CTO; potem włączyć obie flagi i użyć inicjatywy Northwind z uprawnionym recenzentem |
