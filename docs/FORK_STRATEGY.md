...
This document defines the technical strategy for splitting the **Consultify** platform into two distinct applications (`apps/consultify` and `apps/new-app`) while maintaining a shared core via a Monorepo architecture.
...
/
├── apps/
│   ├── consultify/       # The original Consulting Platform (Product A)
...
-   **Domain Logic**: Logic specific to _Consulting_ stays in `apps/consultify`. Logic unique to the _New App_ goes to `apps/new-app`. Logic common to both (e.g., Auth flow, Basic User Management) moves to `packages/shared` or a new `packages/core` library.
...
-   **schema separation**:
    ...
    -   App-specific tables: Prefixed or clearly owned by one domain (e.g., `projects` for Consultify).
...
### Build Commands
-   **Consultify**: `nx build consultify` -> outputs to `dist/apps/consultify`
...
-   **Consultify Deploy**: Triggers on push to `main` if `apps/consultify` is affected.
...
3.  **Refactor Consultify**: Update imports to use `@consultify/shared`.
4.  **Verify Separation**: Ensure `apps/new-app` does not import from `apps/consultify`.
