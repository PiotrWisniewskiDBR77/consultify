 ...
 */
export function getAssessmentButtonClasses(type: 'actual' | 'target', isActive: boolean): string {
    if (type === 'actual') {
        return isActive
 ...
 * @returns Tailwind CSS class string
 */
export function getLevelBubbleClasses(isActual: boolean, isTarget: boolean, isInGap: boolean): string {
    if (isActual) {
