...
export function useNotificationPreferences(
    ...
    const updateCategoryChannel = useCallback(
        async (category: NotificationCategory, channel: keyof ChannelSettings, enabled: boolean) => {
    ...
    const updateDigestSettings = useCallback(
        async (type: 'daily' | 'weekly', settings: { enabled?: boolean; time?: string; day?: string }) => {
            if (!preferences) return;
