...
let baseline: any = nullBaseline
  ? null
  : {
      baselineId: 'bl-model-1',
      caseId: CASE_ID,
      organizationId: ORG_ID,
      ...
      frozenBy: locked ? USER_ID : null,
      rowVersion: 3,
      createdBy: USER_ID,
      createdAt: '2026-06-01T09:05:00Z',
      updatedAt: '2026-07-15T11:00:00Z',
    };
let policy: any = nullPolicy
  ? null
  : {
      policyRowId: 'pol-model-1',
      caseId: CASE_ID,
      organizationId: ORG_ID,
      discountRatePct: 8,
...
const calcRuns: any[] = [
  {
    ...
    benefitCostRatio: 0.2,
    inputHash: 'hash-model-1',
...
if (!g.__RVN_ROI_MODEL_FETCH__) {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (seg[2] === 'baseline') {
      if (method === 'GET')
        return baseline
          ...
          : errorResponse('No baseline set for this case', 404, 'NOT_FOUND');
      if (method === 'PUT') {
        baseline = {
          ...(baseline ?? {}),
          ...body,
          baselineId: baseline?.baselineId ?? nextId('bl'),
          caseId: CASE_ID,
          organizationId: ORG_ID,
          rowVersion: (baseline?.rowVersion ?? 0) + 1,
          createdBy: USER_ID,
    ...
    if (seg[2] === 'calculation-policy') {
      if (method === 'GET')
        return policy
          ? jsonResponse({ calculationPolicy: policy })
          : errorResponse('No calculation policy set for this case', 404, 'NOT_FOUND');
      if (method === 'PUT') {
        policy = {
          ...(policy ?? {}),
          ...body,
          policyRowId: policy?.policyRowId ?? nextId('pol'),
          caseId: CASE_ID,
          organizationId: ORG_ID,
          rowVersion: (policy?.rowVersion ?? 0) + 1,
          createdBy: USER_ID,
    ...
    if (seg[2] === 'actuals' && seg.length === 3 && method === 'GET')
      return jsonResponse({ entries: actualEntries });
    if (seg[2] === 'actual-snapshots' && seg.length === 3 && method === 'GET')
      return jsonResponse({ actualSnapshots });
    if (seg[2] === 'variances' && seg.length === 3 && method === 'GET')
      return jsonResponse({ variances });
    ...
    if (seg[2] === 'post-investment-reviews' && seg.length === 3 && method === 'GET')
      return jsonResponse({ postInvestmentReviews: pirs });
    if (seg[2] === 'finance-links' && seg.length === 3 && method === 'GET')
      return jsonResponse({ financeLinks });
    if (seg[2] === 'finance-reconciliations' && seg.length === 3 && method === 'GET')
      return jsonResponse({ financeReconciliations });
    ...
    return errorResponse(
      `dev-render ROI model mock: unmatched ${method} ${path}`,
      404,
      'MOCK_UNMATCHED'
    );
  };
