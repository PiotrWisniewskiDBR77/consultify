...
const DEFAULT_AREA_ID = '1A';
const params = new URLSearchParams(window.location.search);
const requestedAreaId = params.get('area') || DEFAULT_AREA_ID;
const session = {
  id: SESSION_ID,
  name: 'Northwind digital maturity — September wave',
  ...
  state: 'in_progress',
  domainStage: 'interview',
  mode: 'guided_manual',
  ownerUserId: 'user-northwind-owner',
...
function answerEvent(level: number, state: string, text = '', justification?: string): MethodEvent {
  return {
    id: `event-${level}-${state}`,
    type: 'ANSWER_CONFIRMED',
    organizationId: 'org-northwind',
    sessionId: SESSION_ID,
    unitId: requestedAreaId,
    level,
    actorKind: 'human',
    actorUserId: 'user-northwind-owner',
    methodPackVersion: session.methodPackVersion,
    occurredAt: '2026-09-16T07:20:00.000Z',
    payload: { questionId: `${requestedAreaId}-L${level}-Q1`, answerState: state, text, justification },
  } as MethodEvent;
}
...
export default function U19DrdTrzyKolumnyScreen(): React.ReactElement {
  ...
  const axis =
    DRD_STRUCTURE.find((candidate) =>
      candidate.areas.some((item) => item.id === requestedAreaId)
    ) ?? DRD_STRUCTURE[0];
  const area = axis.areas.find((item) => item.id === requestedAreaId) ?? axis.areas[0];
  const areaId = area.id;
  const levels = pack.levels
    .filter((item) => item.unitId === areaId)
    .sort((a, b) => a.level - b.level);
  const questions = pack.questions.filter((item) => item.unitId === areaId);
  const [selectedLevel, setSelectedLevel] = useState(2);
  const [answer, setAnswer] = useState(
    'Weekly dashboard is generated automatically in the CRM every Monday 06:00; conversion and pipeline are live, average deal size is still tallied by hand in Excel.'
  );
  ...
  const progression = drdAdapter.resolveOpenLevels({
    unitId: areaId,
    confirmedLevels: confirmedLevelsFor(events, areaId),
    evidenceByLevel: {},
  });
  ...
  return (
    <div className="h-screen w-full bg-c-bg">
      <MethodWorkspaceShell
        session={session}
        methodName="DRD — Digital Readiness Diagnosis"
        packVersionLabel="2.0.0-methodpack.2"
        readiness={readiness}
        mode="guided_manual"
        onModeChange={() => {}}
        onExit={() => {}}
        ...
        onViewModeChange={() => {}}
        navigatorProps={{
          nodes: buildNavigatorNodes(events, 'en'),
          activeUnitId: areaId,
          onSelect: () => {},
        }}
        interviewProps={{} as never}
        teresaProps={{} as never}
        matrixProps={{} as never}
        reportContent={null}
        interviewContent={
          <DrdLevelInterviewWorkspace
            axis={axis}
            ...
            targetLevel={targetLevelFor(events, areaId)}
            answerText={answer}
            canWrite
            onAnswerChange={(_questionId, text) => setAnswer(text)}
        ...
        aiButton={
          <PracujZAI
            ...
            kontekstArtefaktu={{
              ...
              title: `DRD · ${area.name} · Level ${selectedLevel}`,
