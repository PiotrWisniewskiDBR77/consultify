...
Api.get = (async (url: string, ...rest: unknown[]) => {
  ...
  if (path === '/audits/criteria') {
    // MERGE NOTE (2026-08-26, audits-polish x audits-gaps): Metalpol
    // (`METALPOL_PROGRAM_ID`) needs its OWN two-row mock — Findings tab
    // resolves `criterionId` on each finding (`METALPOL_CRITERION_1/2`)
    // against this list, so a generic fallback would break that lookup and
    // show unresolved criteria on every Metalpol finding. Every other
    // programId gets the generic 64-row synthetic set (gap pack fix 2 —
