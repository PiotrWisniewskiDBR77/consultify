...
const RagEnhancedService = {
    ...
    hybridSearch: async (query, options = {}) => {
        ...
        try {
            ...
            const mergedResults = RagEnhancedService.mergeResults(vectorResults, keywordResults, graphResults, {
                vectorWeight,
                keywordWeight,
                graphWeight,
            });
    ...
    expandQuery: async (query) => {
        ...
        try {
            ...
            const expandedQuery = uniqueTerms.length > 0 ? `${query} ${uniqueTerms.slice(0, 5).join(' ')}` : query;
