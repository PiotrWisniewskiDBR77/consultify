 ...
 * Source: server/services/ragService.js
 * Target: server/src/services/ragService.ts
 * Status: wrapper
 *
 * TODO:
...
const deps = {
    ...
    set embeddingService(val) {
        this._embeddingService = val;
    },
    ...
    storeChunks: async (docId, chunks) => {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            await new Promise((resolve, reject) => {
                stmt.run(chunkId, docId, chunks[i], JSON.stringify(embedding || []), (err) => {
