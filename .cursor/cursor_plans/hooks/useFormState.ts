...
interface UseFormStateReturn<T> {
    ...
    arrayOps: {
        ...
        update: <K extends keyof T>(field: K, index: number, value: T[K] extends (infer U)[] ? U : never) => void;
    };
}

export function useFormState<T extends object>({ initialData, onSave }: UseFormStateOptions<T>): UseFormStateReturn<T> {
    ...
    const arrayOps = {
        add: useCallback(<K extends keyof T>(field: K, item: T[K] extends (infer U)[] ? U : never) => {
            setData((prev) => {
        ...
        update: useCallback(
            <K extends keyof T>(field: K, index: number, value: T[K] extends (infer U)[] ? U : never) => {
                setData((prev) => {
                    const arr = [...((prev[field] as unknown[]) || [])];
                    arr[index] = value;
                    return { ...prev, [field]: arr };
