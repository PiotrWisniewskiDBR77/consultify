...
> **Fala 3 korekta atrybucji (2026-06-12):** A: 22→23 (`EconomicsViewPlaceholder.tsx` usunięty, commit `b5de79ef03` — 1 MARTWY komponent zniknął; Fala 3 poz.2 częściowo DONE).

| Wymiar | Waga | Punkty | Uzasadnienie (1 zdanie) |
|---|---|---|---|
| A. Realność funkcji | 25 | 23 | Wszystkie 12 pozycji REALNE — **prawdziwa matematyka finansowa** (NPV/IRR/DCF/WACC, realny parsing Excel), billing honest (mock usunięty), bez fasady; `EconomicsViewPlaceholder` usunięty. |
...
- ~~**[MARTWY] `EconomicsViewPlaceholder.tsx`**~~ **USUNIĘTY** (`b5de79ef03`). (Brak martwego huba — FinanceHub żywy.)
...
| Modele | `/api/financial-modeling` (legacy) + v8 | financial_models/_outputs | DZIAŁA obliczeniowo; org-scope NAPRAWIONY (`e3945bc7fc`) |
...
**Findingi:**
- ~~**[P0] F-SEC-1 cross-org IDOR legacy `/api/financial-modeling/models/:id`**~~ **NAPRAWIONY** (`e3945bc7fc`) — `getModel` teraz bierze `orgId`, routes przekazują `req.user?.organizationId`; warunek `AND organization_id=?` aktywny. Historyczna luka: `SELECT * WHERE id=?` bez org na legacy by-id endpointach.
...
1. ~~**Org-scope na legacy financial-modeling**~~ **[DONE `e3945bc7fc`]** — `getModel` org-scoped; cross-org → 404.
...
2. **`EntityStatusChip`** zamiast własnych chipów + ~~wytnij `EconomicsViewPlaceholder.tsx`~~ **[DONE `b5de79ef03`]** — Weryfikacja: spójność, 0 referencji.
