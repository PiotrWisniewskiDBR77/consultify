...
> **Re-audit 2026-06-11 Fala 2:** P1 cross-org assignment injection NAPRAWIONY (commit `7df4b22d6d` — `auditProgramService.ts:388-401` waliduje assigneeIds przez `organization_members` WHERE); F: 6→8. Baner kreatora P3 też naprawiony (tekst już dokładny). Suma: 21+12+2+0+6+8+0=49.
> **Fala 5 (2026-06-12):** C: 2→8 — `audit-programs.test.ts` 17 testów PASS (commit `7df4b22d6d`, ten sam co P1 fix): CRUD org-scoping, generate-surveys fan-out, SEC-3 foreign-assignee filter, idempotency, rollup. Suma: 55.

| Wymiar | Waga | Punkty | Uzasadnienie (1 zdanie) |
|---|---|---|---|
| A. Realność funkcji | 25 | 21 | 8/8 REALNE — pełna pętla E2E (kreator→DB→fan-out do M10→rollup z DB) potwierdzona; baner kreatora naprawiony; minus: martwy FE edycji, kliencki filtr. |
...
| C. Testy automatyczne | 15 | 8 | 17 testów PASS (`audit-programs.test.ts`, commit `7df4b22d6d`): CRUD org-scope, fan-out, SEC-3, idempotency, rollup — solidne BE; brak FE/E2E. |
...
| F. Bezpieczeństwo/dostęp | 10 | 8 | Org-scope CZYSTY (7/7 handlerów); P1 cross-org injection NAPRAWIONY (commit `7df4b22d6d`); public showcase czysty. |
| G. Środowiska (Railway) | 10 | 0 | Faza 3 niewykonana. |
| **Hard cap zastosowany?** | — | — | Faza 4 niewykonana → max 70; BRAK cap cross-org. Suma 55 < 70. |

**Werdykt jednym akapitem (Fala 5 — 2026-06-12):** Moduł **funkcjonalnie kompletny end-to-end** — kreator 4-krokowy → POST → realna tabela `audit_programs`, fan-out to **realny idempotentny handoff do M10** (`interviewAssignmentService.create`, guard `surveysGenerated`), rollup z realnym SQL. Presety iso27001/new-company to uczciwe blueprinty. Org-scope czysty 7/7 handlerów. **P1 cross-org injection NAPRAWIONY** (`7df4b22d6d`): `interviewAssignmentService` waliduje assigneeIds przez `organization_members`. **17 BE testów PASS** (`audit-programs.test.ts`): CRUD cross-org 404, fan-out math, SEC-3 filter, idempotency, rollup — pętla E2E objęta. Brakuje: FE komponentów, E2E, `ModuleHub`, §27 lista, i18n bez inline `isPolish`, martwy FE edycji, kliencki search. Sufit: Fazy 3+4 niewykonane.
