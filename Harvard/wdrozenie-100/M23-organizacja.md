...
| L-06 | i18n `isPl` + nieprzetłumaczone opcje ról | W-01 | `CompetencyCatalog.tsx:54/297/361` (selektor pola); `OrganizationAdminPanel.tsx:179` | P2 | 4 | **ODROCZONA-Faza4 → Harvard 1** — `isPl` to legalny selektor pola DB (`namePl`/`labelPl` vs EN), NIE hardcoded-string debt; realny dług = ~96 hardkodów EN (manual; 0 `isPolish` → poza codemodem) + injekcja `translation.json` (H1) |
...
| L-08 | §27 (2 `<table>`) | W-01 | `OrganizationAdminPanel.tsx` (members), `CompetencyCatalog.tsx` (competencies) | P2 | 4 | **ZAMKNIĘTA 2026-06-17 e8c5b39c51** — oba `<table>`→FilterableTable (kolumny/badge/dane zachowane, 0 surowych `<table>`) |
| L-09 | SVG branding stored-XSS; martwy `analyze` 404; KG orgId fallback | W-01 | `branding.routes.ts` (multer svg) + `index.ts` (/uploads headers) | P2/P3 | 3 | **ZAMKNIĘTA 2026-06-17 e2ff308a54** — XSS 2 warstwy: DOMPurify upload-sanitize (fail-closed) + CSP sandbox+nosniff na /uploads svg/xml/html; test `tests/security/branding-svg-xss.test.ts` 6/6. `analyze`-404 = **N/D** (endpoint nie istnieje na branchu); KG orgId fallback = **N/D** (KnowledgeGraphExplorer bez orgId, scope serwerowy) |
