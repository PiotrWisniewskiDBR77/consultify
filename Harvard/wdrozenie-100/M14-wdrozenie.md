...
|----|------|---------|--------------------|-------|------|--------|
| L-01 | gating pilota tylko klient (serwer przepuszcza CRUD) | W-01 | Rollout CRUD `requireOrgRole('user')` (`rollout.routes.ts`, 17 verbs) | P1 | 2 | **ZAMKNIĘTA** — `requireRolloutWrite = requirePermission('MANAGE_ROLLOUT')` (`rollout.routes.ts:49`) na wszystkich verbach zapisu (PMO/manager tier, nie sam admin); 13/13 testów PASS. *(Korekta R3 2026-06-17: poprzednio błędnie opisane jako `requireOrgRole('admin')` — realnie `requirePermission('MANAGE_ROLLOUT')`.)* |
...
| L-04 | martwy kod (`ImplementationView`/`views/ExecutionView`/`ExecutionDetailPanel`) + 4 kandydaci | W-01 | importowany nieren./hardcode `projectId="default"` | P2 | 3 | **NAPRAWIONA** — `views/ExecutionView.tsx` + `views/ImplementationView.tsx` usunięte (`2d5bbc17dd`); 4 kandydaci (PeopleChange/RiskSignals/DelayDetection/ReportCompact) wytnięte (D-02→DP-7): grep `src/` = 0 referencji; pliki + testy usunięte; barrel `Execution/index.ts` oczyszczony | 2026-06-17 |
| L-05 | feed-forward M14→M15 martwy (dziura w kręgosłupie) | W-03,W-05 | `ExecutionHub.tsx:954` (`roi:{summary:null,items:[]}`); brak `?initiativeId=` export | P1 | 2 | **DP-6 PREVIEW-DONE / realny odbiór = D-01 (skoryg./doprecyz. 2026-06-19, read-only verify): feed-forward M14→M15 = DP-6 PREVIEW; realny odbiór czeka na OTWARCIE BETY M15 (D-01).** PREVIEW-DONE: brak fałszywego sygnału — `ExecutionHub.tsx:954` `roi:{summary:null,items:[]}` to uczciwe puste w lokalnym snapshocie (zero fabrykowanych liczb, zweryfikowane), `dataQuality:'partial'`; w Egzekucji **nie ma** przycisku sync do ukrycia (grep: tylko nav `/reports`,`/wordy`,`INITIATIVES`). **AWAITS D-01:** realny eksport sygnałów (`budget_health`,`roi_delta`) do tabeli czytanej przez Results + deep-link `?initiativeId=` Execution→Results (wzorzec istnieje `InitiativePreviewV3.tsx:435` — `?tab=results_reports&…&initiativeId=`, zweryfikowane; w Egzekucji brak — miejsce: sekcja ROI/summary zasilająca `:954`). Zależność blokująca: M15 beta-CLOSED → realny odbiór dopiero po otwarciu bety M15. |
| L-06 | 5 tabel Rollout poza §27 + bulk-bar Portfel bez akcji + Manager lanes grid | W-01,W-05 | `RolloutTab.tsx` RegisterTable (2 raw `<table>` pokrywa 5 rej.), `ExecutionHub.tsx:4855-4870` | P1 | 4 | **CZĘŚCIOWO ZAMKNIĘTA `5cdf90acdc` — 4 edytowalne rejestry Rollout (KPI/Risk/Change/Closure) → `FilterableTable` (filtry/sort/resize/density/persistKey, wiersze read-only) + kebab Edit→`RolloutRegisterEditModal` (nowy) + Delete→ConfirmDialog; persystencja przez te same patch/delete handlery (→`/api/rollout/*`). tsc 0 błędów; live-smoke Piotra = R6 Faza 4 (auth wall). Bulk-bar Portfela z akcjami = ZROBIONE `3b3f894565` (masowa zmiana statusu). Test regresji `788712291d` — `RolloutTab.smoke.test.tsx` 5/5 (read-only tekst, kebab→Edit→modal→PATCH minimalny diff, kebab→Delete→ConfirmDialog→DELETE) = behawioralny substytut live-smoke; złapał regresję 2 testów, którą wprowadziłem commitując bez testów. Manager lanes (`Manager/ProblemTable.tsx`) → §27 FilterableTable = ZAMKNIĘTE `82497d46fb` (search+liczniki severity przywrócone in-component po złapaniu regresji; test 7/7). **L-06 PEŁNA ✅.** Finding systemowy: `FilterableTable` (shared/ModuleHub) nie ma row-sort (`sortable` = martwy typ) ani free-text search — przy konwersji bogatych tabel trzeba je dorabiać in-component; rekom: zaimplementować `sortable` + search w shared component (poza strefą H4, oddać właścicielowi ModuleHub).** |
...
### 07 · Log — 2026-06-17 (Harvard 4 Fala 5): §27 i tokeny — `ReportDocumentView.tsx` DataTable + `RolloutTab.tsx` RegisterTable oznaczone §27-exempt: document-layout (JSX `React.ReactNode[][]` rows z ALink/IssueTag/SignalDot). Oba komentarze live w HEAD (`5cdf90acdc`). Hex tokens FinanceHub.tsx: `dark:bg-[#F4F7FB]`→`dark:bg-slate-50`, `dark:hover:bg-[#DDE5EF]`→`dark:hover:bg-slate-200` (primary CTA button) — commit `a448982af3`. L-06 §27 — PEŁNA ✅ (potwierdzone). rose→danger sweep: 139 żywych zmian w Execution/ (3 pliki usunięte dead-code pominięte; 6 heatmap DP-8 exempt) — commit `2acbba5c27` (merge `5b79023f27`).
### 07 · Log — 2026-06-17 (Harvard 4 r2/r3): **L-06 PEŁNA ✅** — Rollout 5 rejestrów → §27 FilterableTable + RolloutRegisterEditModal (`5cdf90acdc`, test `788712291d` 5/5); bulk-bar Portfela (`3b3f894565`); Manager `ProblemTable` → §27 + search/liczniki przywrócone in-component (`82497d46fb`, test 7/7). L-01 opis skorygowany (`requirePermission('MANAGE_ROLLOUT')`). L-05 udokumentowana preview-done vs D-01. Finding: shared `FilterableTable` bez row-sort/search → rekom impl (poza strefą H4). **DEAD-CODE USUNIĘTY `f3bbfffcb8` (2026-06-17):** `FullExecutionDashboardWorkspace.tsx` (workspaces/, 0 zewn. ref) + liście `Execution/BenefitsTracker.tsx`+`CorrectiveActions.tsx`+`KPIDashboard.tsx` + barrel `Execution/index.ts` oczyszczony. Klaster martwy od korzenia — usunięcie pełne po potwierdzeniu 0 importerów roota. Honest „Wkrótce (backend)" w `ExecutionHub.tsx:2536-2560,4716-4724` = brak endpointów archive/delete rollout/initiative (backend ticket). 2026-06-16: L-01+L-02+L-03+L-04(częściowo)+L-07+L-08(częściowo) zamknięte; martwy kod views wyciągnięty; bannery degradacji zweryfikowane grepem. 2026-06-13: brak uwag żywych (jawnie); teczka pogłębiona do M13-level (F Gherkin, C API enumerowane, DP-6/8/9 wpięte). Audyt 2026-06-11/12: ocena 52/100; `b9f2dee9d2`, `9974596da7`, `84757dc672`, `229cb35565`, `2fe1c81be3`. Re-ocena D/G po Fazie 3/4.
...
Weryfikacja kodu (read-only): top-3 zamknięte luki = PRAWDA. L-01 `requireRolloutWrite=requirePermission('MANAGE_ROLLOUT')` na 14 verbach zapisu (`rollout.routes.ts:49,108,139,189,258,288,329,377,405,447,495,523,563`). L-02 bannery degradacji renderowane: `healthSnapshotFailed`→`<Callout variant="warning">` (`ExecutionHub.tsx:2960`), `actionQueueFailed`→Callout (`:3718`), oba z PL/EN przez `t()`. L-04 martwy kod usunięty: `Execution/views/` katalog nie istnieje, `ImplementationView.tsx` nieobecny.

Hub: `ExecutionHub.tsx` (ModuleHub — Menu 1/2/3, taby, breadcrumbs).
| Ekran / widok | Cel | Plik |
|---|---|---|
| Executive Dashboard (Health Score) | Portfolio health, decyzje, blokery | `ExecutionHub.tsx` (sekcja PortfolioHealthScore) |
| Portfel — Tabela | Lista inicjatyw (FilterableTable+TableWithPreview, bulk status) | `ExecutionHub.tsx` |
| Portfel — Kanban | DnD egzekucji, status przez PATCH | `ExecutionInitiativesKanbanView.tsx` |
| Portfel — Timeline | Widok osi czasu inicjatyw | `ExecutionTimelineView.tsx` |
| Portfel — Workload | Obciążenie zasobów | `ExecutionWorkloadView.tsx` |
| Execution Management | Zarządzanie egzekucją | `ExecutionManagementView.tsx` |
| Action Queue / RAID | Decyzje/ryzyka/KPI deviations | `ExecutionHub.tsx` (+ `RiskSignalsPanel`, `MitigationPanel`, `CorrectiveActions`) |
| Rollout — Plan | Rejestr planu (read-only FilterableTable) | `RolloutTab.tsx` |
| Rollout — KPI | Rejestr KPI (FilterableTable + kebab Edit/Delete) | `RolloutTab.tsx` + `RolloutRegisterEditModal.tsx` |
| Rollout — Risks | Rejestr ryzyk | `RolloutTab.tsx` |
| Rollout — Change | Rejestr zmian | `RolloutTab.tsx` |
| Rollout — Closure | Rejestr zamknięcia | `RolloutTab.tsx` |
| Rollout Edit Modal | Edycja wiersza rejestru | `RolloutRegisterEditModal.tsx` |
| Manager — lanes/Problem | People-change/ProblemTable (V8, baner gdy OFF) | `ManagerModuleView.tsx`, `Manager/ProblemTable.tsx`, `Manager/ProblemPreview.tsx`, `Manager/AiRecommendationPanel.tsx`, `PeopleChangeWorkspace.tsx`, `WhyRedChain.tsx` |
| Budget Control | Panel budżetu | `BudgetControlPanel.tsx` |
| Delay Detection | Wykrywanie opóźnień | `DelayDetectionPanel.tsx` |
| Raport (dokument) | Widok raportu live-data | `ReportDocumentView.tsx`, `ReportCompactPanel.tsx` |
| Stany: degradacja PMO health / Action Queue | Callout warning (nie cicha pustka) | `ExecutionHub.tsx:2960,3718` |

Stany puste/ładowanie/błąd: per-hub. Modale: Rollout Edit, bulk status. Liczba ekranów: ~18.
