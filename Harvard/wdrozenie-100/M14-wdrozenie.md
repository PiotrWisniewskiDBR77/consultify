...
| L-05 | feed-forward M14→M15 martwy (dziura w kręgosłupie) | W-03,W-05 | `ExecutionHub.tsx:945`; brak `?initiativeId=` export | P1 | 2 | **DP-6 PREVIEW-DONE / realny odbiór = D-01 (precyzyjnie, read-only verify 2026-06-17):** PREVIEW-DONE: brak fałszywego sygnału — `ExecutionHub.tsx:954` `roi:{summary:null,items:[]}` to uczciwe puste w lokalnym snapshocie (zero fabrykowanych liczb), `dataQuality:'partial'`; w Egzekucji **nie ma** przycisku sync do ukrycia (grep: tylko nav `/reports`,`/wordy`,`INITIATIVES`). **AWAITS D-01:** realny eksport sygnałów (`budget_health`,`roi_delta`) do tabeli czytanej przez Results + deep-link `?initiativeId=` Execution→Results (wzorzec istnieje `InitiativePreviewV3.tsx:435`; w Egzekucji brak — miejsce: sekcja ROI/summary zasilająca `:954`). Zależność: M15 beta-CLOSED → realny odbiór po otwarciu bety. |
