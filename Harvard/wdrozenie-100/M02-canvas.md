...
| L-04 | cicha degradacja flaga OFF (legacy redirect bez banera) | W-01,W-06 | `UnifiedChatPanel.tsx:2393,2644,2849` | P3 | 3 | **ZAMKNIĘTA 2026-06-17** (R3 — NIE cicha: każda z 3 ścieżek redirect przy fladze OFF dodaje `addChatMessage` PRZED `navigateToRoute` — „Otwieram Tabele Studio/Dokumenty/Prezentacje — zaraz…" (`:2393`/`:2644`/`:2849`). Agent błędnie odczytał `:2393` jako cichy) | 2026-06-17 |
...
| L-07 | brak pickera sourceRefs | W-01 | `docGenerationRuntime.ts:386-420` (auto-scan) | P2 | 3 | **ODROCZONA (feature)** — BE akceptuje `sourceRefs` i auto-skanuje źródła org gdy puste (`:386-420`); brak FE-pickera = brakująca afordancja (nie bug/sec). Wymaga decyzji projektowej (gdzie picker, jakie źródła) + flaga deliverables-light ON (debloker Railway). Poza ścieżką krytyczną — do designu po włączeniu flagi | 2026-06-17 |
...
| L-11 | panel `WorkCanvasDocumentPanel` bez `useTranslation`, hardkody PL/EN | W-01 | grep: 0× `useTranslation` | P2 | 3/4 | **ZABLOKOWANA (Faza 4)** — wprowadzenie `t()` wymaga kluczy w `public/locales/*` (ZAKAZANE dla H1 — inny agent + merge na współdzielonym drzewie). Komponent w mojej strefie, ale bez kluczy locale nie domknę bez konfliktu. = M01/L-10 | 2026-06-13 |
