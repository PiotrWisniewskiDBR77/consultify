...
### 07 · Log + re-ocena — **2026-06-20 (manual schema → Playwright):**
> **Pełna manualna specyfikacja `Testy manualne/TESTY_M02_CANVAS.md` zautomatyzowana i uruchomiona w Playwright (headed, live app :3000/:3001→trolley, auth register-demo): 13/13 PASS** — `tests/e2e/smoke/m02-canvas-manual.spec.ts`.
> Pokryte: §1 (tytuł+autosave-blur, +New menu+szablony, „…"+Dock↔Markdown wspólne źródło, file-actions copy/share/save/close+data-save-state, history popover) · §1.3/1.4 capability-gating (output/promote/share disabled + reason, 0 żądań przy kliku — potwierdza `getCanvasActionAvailability`/`handleUnavailableAction`) · §2 toolbar (bold/italic/underline/strike/code/highlight + H1-3/bullet/ordered/task/blockquote/insert-table/undo/redo — asercje behawioralne na DOM ProseMirror) · §3 floating AI menu/selection-block na zaznaczeniu (triple-click) · §4 autosave debounce (`PUT /drafts/:id`) + reload-recovery (treść trwała w DB) · §7A deliverables route enabled (≠404 = flaga ON) · §7 dark+light render bez błędów konsoli.
> **Odkrycia z automatyzacji:** (1) standalone `/ai/work-canvas` **przeniesiony do chat-shell** — redirect → `/chat` + `WorkCanvasDocumentPanel` (deep-link otwierany przez `?workCanvas=1`); 6 starych speców `work-canvas-*.spec.ts` jest **STALE** (celują w usunięty `WorkCanvasShell` + label „Save Canvas document"→`availability.label`) = 10 FAIL — do modernizacji. (2) Pełna generacja czat→canvas (doc/deck/sheet) **wymaga canvas/deliverables capability** — register-demo user jej NIE ma (gating §1.3/1.4) → ścieżka E2E generacji zweryfikowana osobno na owner DBR77 (live Chrome). (3) Fresh demo user dostaje modal `FirstRunOnboarding` (gate `GET /api/preferences`→`onboarding_completed`) — suppress przez route-intercept. (4) `documentIntentDetector`: noun po dwukropku („raport: …") wypada z verb-adjacent `DOCUMENT_INTENT_PATTERNS` — generację odpala dopiero noun przyległy do czasownika („Napisz dokument…").
> **Poza zasięgiem automatu (manual/owner):** realna jakość treści LLM + tabele N-9/N-10 (provider DeepSeek bez balansu → scaffold), AI-diff Accept/Reject z realnym LLM, clipboard copy, share-link incognito preview, faktyczne pobranie plików eksportu, standalone `WorkCanvasShell` (§5 — usunięty).
>
> **2026-06-20 (odbiór) — Testy + interim →F live:**
> **Testy:** pełny zestaw M02 **173 PASS / 0 fail** (15 plików; `unbackedCanvasClaim` 36/36, `canvas/*`, `AIChat/*`, `WorkCanvasDocumentPanel` 33/33 + `handoffMount` 2/2, `work-canvas.routes` 40/40, `deliverablesGenerations.generate-format` 3/3, `canvasMaterializeCrossOrg` 3/3).
> **→F INTERIM zweryfikowany NA ŻYWO** (Claude przez Chrome, zalogowany OWNER DBR77, localhost:3000 + backend dev na trolley (NIE-PROD, dane jak demo), flagi: FE `.env.local` + BE `.env.staging.local` → OBIE ON lokalnie):
> - ✅ **handoff czat→canvas** — fraza intencji „Napisz dokument…" → prawy panel auto-montuje (#1 kręgosłup żywy po fixie `42bee38044`).
> - ✅ **doc** — `POST /api/deliverables/generations`→**200** (NIE 404 flag-OFF) → `/generate` → poll `GET /:id`; render rich PL prose grounded (3 źródła org), org-scoped (organizationId w logu).
> - ✅ **deck** — `CanvasPresentationView` 5 slajdów, tytuł + branding (DBR77 · czerwiec 2026 · internal), „Otwórz w Deck Builder", komunikat uczciwy „prezentacja jest po prawej".
> - ✅ **autosave-persyst po reload** — draft przeżywa reload (`work_canvas_drafts`), re-montaż przez **artifact switcher** (chip „Working document" + draft, reload-safe).
> - ✅ **komunikat uczciwy** (L-03) — „✅ Done — the document/presentation is on the right" NIE „dodałem" (potwierdza guard `unbackedCanvasClaim`).
> - **sheet** = ta sama ścieżka (`deliverablesGenerations` + generate-format 3/3) — nie odpalany ręcznie.
> - **i18n live:** shell PL („Porozmawiaj z Teresą", „WYNIK: Auto/Dokumenty/Tabele/Prezentacje") + EN; toolbar EN, treść PL.
> - **dark-mode** czysty (czat/edytor TipTap/slajdy).
> ⚠ **Env (NIE bug):** provider **DeepSeek bez balansu** (circuit OPEN, „Insufficient Balance") — niektóre body sekcji = scaffold anti-placeholder zamiast pełnej prozy; mechanizm triady (plan→generacja→montaż→persyst) działa w 100%. Odróżnić od buga.
> 🔑 **Operacyjna uwaga (intent regex):** PL deck-intent `(/(stwórz|przygotuj|wygeneruj|zbuduj|zrób)\s+(mi\s+)?(prezentacj|deck|slajd)/i)` — czasownik MUSI bezpośrednio poprzedzać „prezentacj"; „Zrób **krótką** prezentację" / „**Otwórz** w Canvasie deck" NIE matchują (potwierdza wąski regex z L-03). Format-buttony „Dokumenty/Tabele/Prezentacje" routują do standalone studiów (M18/19/20), in-canvas triada = tryb **Auto + intencja**.
> **DECYZJE PIOTRA 2026-06-20:** (1) DoD #4 paleta L-12 = **met + dług Visual Quality** → DoD **7/7**. (2) **Akceptacja interim local-live jako →F**; **Deploy demo + formalny →F + →UI audytor ODROCZONE** — wymagają flag Railway demo (akcja Piotra; prod=centerbeam osobna zgoda). **M02 NIE 8/8** (bramki 6/8 + formalny 7 poza moim zakresem).
>
> **2026-06-17 (Harvard 1) — R3 rekonsyliacja + 2 naprawy:**
