...
| L-08 | martwy `KnowledgePulse.tsx`, `notebook/InsertMenu.tsx` (0 importerów) | W-01 | grep importerów | MARTWY | 4 | **USUNIĘTE Z GITA 2026-06-17 `8c3285480d`, ALE WRÓCIŁY NA DYSK JAKO UNTRACKED (skoryg. 2026-06-19, git-race).** Stan dysku dziś: `src/components/MyWork/notebook/KnowledgePulse.tsx` (10.6 KB) + `notebook/InsertMenu.tsx` (7.3 KB) **istnieją, `git status`=`??` (untracked)**. **Importerów = 0** (zweryfikowane grepem 2026-06-19): `KnowledgePulse` ma tylko stały KOMENTARZ `MyWorkHub.tsx:1210` (nie import); `InsertMenu` — jedyne trafienia to `BlockInsertMenu` w `Reports/Premium/*` (inny komponent). ⚠ UWAGA build-integrity: pliki untracked = NIE na świeżym clone, a 0 importerów → nie łamią buildu DZIŚ; ale `KnowledgePulse.tsx` importuje żywy `./PulseItemPickerModal` (tracked) — gdyby ktoś przywrócił import KnowledgePulse, build by przeszedł lokalnie a padł na clone. **Akcja:** `rm` obu untracked sierot z dysku (poza zakresem tej teczki — tylko .md). NIE commitować ich z powrotem | 2026-06-17 |
