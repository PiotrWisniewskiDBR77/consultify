...
|----|------|---------|--------------------|-------|------|--------|---------|
| L-01 | handoff Radar/Inicjatywy PÓŁ-MARTWY (toast vs 0 INSERT) | W-01,W-07 | ~~`notebookHandoffService.ts:429`~~ → realnie `NotebookContent.tsx:1663` | P1 (WSPÓLNE M21) | 3 | **✅ ZAMKNIĘTA — KOREKTA R3 2026-06-17: poprzedni opis FAŁSZYWY. `buildRadarHandoff/buildInitiativeHandoff` (`notebookHandoffService.ts:431,470`) NADAL tylko `return` payload — ZERO INSERT (SHA `952f309eed` NIE tknął handoffu, tylko Menu3 L-04). Realny fix = migracja na CONVERT-PATH: FE woła `Api.convertNotebookPage(activePage.id,'initiative',…)` (`NotebookContent.tsx:1663`) → realna encja + toast „Inicjatywa utworzona" + `trackFunnelEvent('notebook_handoff')`. Kłamliwy toast zniknął, ale INNYM mechanizmem niż dokumentowano. TODO: zretire martwe build-only route `/handoff/radar|inicjatywy` (0 callerów FE — potw. M21).** | 2026-06-17 |
| L-02 | brak trwałego wielo-instancyjnego „trzeciego panelu" | W-02,W-05,W-06 | powłoka (`RightRail`/`SplitLayout`/`MainLayout`) | P1-design-program | 0 | **DECYZJA PIOTRA — zakres+rekom (read-only verify 2026-06-17):** Trwały wielo-instancyjny dok IDE-tabs NIE istnieje w kodzie. `RightRail` (ExecutiveModuleShell) jest per-moduł, JEDNO-panelowy (`RightRail.tsx:127` mutually-exclusive) i ZNIKA przy nawigacji; `useRailState.ts:11` trzyma tylko wymiary w localStorage; `SplitLayout` per-route. Jedyny panel przeżywający nawigację = globalny `UnifiedChatPanel` w `MainLayout.tsx:182-339` (driven by `useAppStore`). **REKOM:** potwierdzić DP-2=„globalny dok", ale ZBUDOWAĆ jako JEDEN reużywalny „workspace right-rail" w `MainLayout` + slice `useAppStore` (wzorzec=chat-panel), notatnik 1. najemcą — DOPIERO PO L-03. Blast radius = cała powłoka+wszystkie moduły. **OTWARTE PYTANIE:** globalny dok (przeżywa moduły, duży program) CZY kontekstowy per-notatka (wtedy L-02=L-03, bez zmian powłoki)? SPEC_07 §4c/otwarte#2. | 2026-06-17 |
| L-03 | ciężki Canonical Path + rozproszony prawy panel | W-03 | `NotebookCanonicalPathStrip.tsx:25-179` | P1-design | 3 | **DECYZJA PIOTRA — zakres+rekom (read-only verify 2026-06-17):** `NotebookCanonicalPathStrip.tsx:25-179` = czysta prezentacja; 4 akcje to callbacki celujące w ISTNIEJĄCE refy/handlery (`NotebookContent.tsx:2516-2536`: attachments-scroll, `setAiCommand('action')`, proposal-scroll, `handleConvertFromPanel`, `handleHandoffInitiatives`; gate `canConvertDeliverable`). **REKOM: ROZDZIELIĆ.** 4a (slim chip ①Sources②AI③Review④Convert w nagłówku, gate=tooltip) — ZROBIĆ TERAZ, refaktor 1 komponentu przy NIEZMIENIONYM interfejsie propsów, odzysk ~40% kanwy, ryzyko niskie. 4b (3 okna→1 rail 2 zakładki A „Praca"/B „Kontekst AI", panele `NotebookContent.tsx:2743-2825`) — ODŁOŻYĆ, dzieli „workspace right-rail" z L-02 + falowy refaktor 2954l z testami nieregresji mostów (convert×7/link-graph/outputs; SPEC §5 mapa). **OTWARTE PYTANIE:** forma chipu (A) slim-chip w nagłówku [rekom] CZY (B) 4 ikony w toolbarze; oraz 4a teraz CZY w pakiecie z 4b? | 2026-06-17 |
...
| L-08 | martwy `KnowledgePulse.tsx`, `notebook/InsertMenu.tsx` (0 importerów) | W-01 | grep importerów | MARTWY | 4 | **ZAMKNIĘTA 2026-06-17 `8c3285480d` (delecje wchłonięte przez równoległy `git add -A` agenta M06) — 0 importów potwierdzone grepem (jedyna wzmianka `KnowledgePulse` = komentarz `MyWorkHub.tsx:1214`, `InsertMenu` w Reports = inny komponent `BlockInsertMenu`). Usunięto `KnowledgePulse.tsx`+`InsertMenu.tsx` (tracked) + dupy `* 2.tsx` (untracked macOS cruft)** | 2026-06-17 |
| L-09 | TipTap/SlashMenu 0 testów; S5 fałszywa zieleń; 8 `it.todo` | W-01 | testy notebook | P0-test | — | **CZĘŚCIOWO 2026-06-17 — kontraktowe (`e92d779e7b`: V8 classify + validate 4/4) + `SlashMenu.behavior.test.tsx` 17/17 (`8e9ee5f435`: render, filtr label/keyword/id, nawigacja klawiaturą+wrap, routing AI→onAICommand, klik, pl/en, detectSlashTrigger 5 ścieżek). POZOSTAJE: edytor TipTap/autosave (NotebookContent 2900l, strefa H2 — wymaga wydzielenia testowalnych jednostek, osobna fala)** | 2026-06-17 |
...
| L-11 | `isPolish` inline ~186× | W-01 | `NotebookContent.tsx:218,276,296,613…` + `notebook/*` | P3 | 4 | **ZINWENTARYZOWANA 2026-06-17 — realnie ~200: 148 w `NotebookContent.tsx` + 52 w `notebook/*` (14 plików). To inline-ternary `isPolish?'PL':'EN'`, NIE brakujące klucze. Migracja do `t()` wymaga kluczy w `public/locales/` (STREFA ZAKAZANA wg AGENT_MAP) + ciężko dotyka NotebookContent.tsx 2900l = wysoka regresja → BLOKADA na decyzję Piotra + dostęp do locales; Faza 4 osobna fala** | 2026-06-17 |
