...
- **API — enumeracja (RBAC `requireOrgAccess`/`verifyToken` + org-scope):**
  ...
  - **Executive/Focus/Session:** `GET /executive-analytics` (L7973, **L-01 ZAMKNIĘTA — `requireRole` L7974**) *(skoryg. 2026-06-19: było L7907)*, `GET/POST /session-context` (GET L8689 / POST L8637 — **odczyt był martwy, usunięty L-02**) *(skoryg. 2026-06-19: było L8623/8571)*, `POST /focus/ai-plan` (L8464), `GET /morning-brief` (L6883), `POST /chat-actions` (L6964). `task-advisor.routes.ts` = 503 stub (L-03).
- **Reguły biznesowe:**
...
- **session-context odczyt martwy (L-02):** `GET /session-context` (L8689) *(skoryg. 2026-06-19: było L8623)* wykonany, ciało `if(...){}` to komentarz, wynik wyrzucany (`MyWorkHub.tsx:946-951`) → „ciągłość sesji L7" nie istnieje (D-01).
...
| 2 | Bezpieczeństwo | cross-org decyzje+inbox ✅ `b9f2dee9d2`/`45d74b0de1`; leak executive-analytics zamknięty role-guardem serwerowym (member→403; `requireRole` zweryfikowane wpięte na `my-work.routes.ts:7974`, commit `36deb2708c`) *(skoryg. 2026-06-19: było „zweryfikować na `:7907`" — zweryfikowane, realnie `:7974`)* |
...
|----|------|---------|--------------------|-------|------|--------|
| L-01 | leak executive-analytics (manager-gate tylko UI) | W-01 | `my-work.routes.ts:7973` (route) / `:7974` (requireRole) | P1 | 2 | **ZAMKNIĘTA `36deb2708c` (2026-06-17)** — `requireRole('ADMIN','MANAGER','OWNER','SUPERADMIN')` wpięte na route (`my-work.routes.ts:7974`, route def `:7973`, member→403 serwerowo, zweryfikowane w kodzie) *(skoryg. 2026-06-19: było `:7965`/`:7907`)*. Bonus: stale komentarz beta-gating `MyWorkHub.tsx:607` (mówił `BETA_ADMINS_EXEMPT=false`/„everyone blocked" — nieprawda, jest `true`+MYWORK_IDEAS=`open`) poprawiony; test `betaAccessGating.test.ts` (4/4, CI-gated) |
| L-02 | session-context odczyt = martwy stub | W-01 | `MyWorkHub.tsx:946-951`, `GET /session-context` `my-work.routes.ts:8689` *(skoryg. 2026-06-19: było `:8623`)* | P1 | 2 | **ZAMKNIĘTA `1c2d6ac279` (2026-06-17, D-01)** — feature był write-only (POST co 5s, ZERO konsumenta GET w FE → brak ciągłości); ciągłość już przez localStorage (`readStoredMyWorkDocuments`). Martwy zapis USUNIĘTY (nie półwpinano serwerowego odczytu) |
...
### 06 · Ryzyka i założenia — L-06 crash bez stack-trace → najpierw repro/telemetria (R3: nie kwalifikować przyczyny bez dowodu). L-01 leak: karta `d05382fb44` → zweryfikować że `requireRole` wpięte (member→403 test). #9 OAuth kalendarza = **BLOCKED-ON-ENV (deploy-time)**: wymaga `GOOGLE_CLIENT_ID`+`GOOGLE_CLIENT_SECRET`, `MICROSOFT_CLIENT_ID`+`MICROSOFT_CLIENT_SECRET` (+ callback URL) na Railway (`Config.ts:35-36,49-50`, wszystkie `.optional()` → puste w czystym deployu); prod=centerbeam → osobna zgoda Piotra. Kod CTA gotowy (L-07), backend OAuth czeka na env. Dev `.env` → Railway PROD DB.
### 07 · Log + re-ocena — **2026-06-20 (Odbiór do 100% — sesja żywa R6): 6/6 bramek realizacji ✅ → 🟢 DO ODBIORU.** Testy: **262 PASS / 34 pliki / 0 fail** (FE+BE+regresja); naprawione 3 zdryfowane testy (2× mock i18n `{defaultValue}`, 1× stale mock `decisions.remind`). 5 powierzchni zweryfikowane NA ŻYWO (localhost+staging DB, OWNER DBR77): Inbox 0-crash (256), §27 Tasks(200)/Decisions(5), honest kalendarz integ, dark+light czysty (0 danger-fill), console 0-err. 🔴 **NOWY P1 crash Manager ZNALEZIONY+NAPRAWIONY na żywo:** `AIOperatorOverviewCard.tsx:323` renderował legacy `nextMilestone:{name,targetDate}` (z DB `ai_operator_plans.plan_json`, stary kształt vs obecny string z `buildPlanGraph:1486`) surowo jako React-child → `Objects are not valid as a React child` → error-boundary dla ownera. Fix = defensywna koercja na warstwie prezentacji (obiekt→`name · data`) + test regresji `AIOperatorOverviewCard.nextMilestone.test.tsx` 3/3. Zweryf. żywo: pełny dashboard renderuje, „NEXT MILESTONE: Process Automation · 20/03/2026". ⚠ Working tree — czeka na commit+deploy demo. Zostają: Deploy (6), →F 39-scen (7), →UI 15-ekr (8); OAuth L-07 = env Railway (Piotr).
— **2026-06-18 (Harvard Final): Wizualne P0/P1 zamknięte.** VIS-013 crash Inbox zweryfikowany jako naprawiony `8bf85a6679` (2026-06-16 — `$$typeof` fix RelationChip) = 0 crashy. SYS-1 selekcja crimson→neutral/blue: `4155d717c3` (51 plików sweep, selection+CTA), `081f38396a` (nav+settings+exec), `c284b75e0e` (primitives Tabs/Dropdown/Selectors), `33dfeabced` (settings+initiatives). SYS-3 StatusPill danger-fill w light: `9f26e682ab` (centralnie ~34 callerów, dotyka Tasks/Decisions badge). SYS-7 EntityStatusChip admin views: `20e81567c1`. **Grafika M03: 🟢 (wszystkie P0/P1 ✅).** Pozostałe P2/P3: §27 sticky-thead overflow-hidden (odroczone), Calendar design D-03 (modułowe), i18n 2888× (Faza-4).
— 2026-06-13: 4 uwagi żywe zalogowane (#5/#9/#10/#11); teczka pogłębiona do M13-level. Audyt 2026-06-11: 54/100. 2026-06-17 (Harvard 2): L-01 ZAMKNIĘTA (`36deb2708c` — requireRole zweryf. + beta comment + test); L-02 ZAMKNIĘTA (`1c2d6ac279` — dead write removed); L-04 ZAMKNIĘTA (4/4 test CI-gated); L-05 ZAMKNIĘTA (`de3eccbdd0` — FocusCockpit orphan); L-10 ZAMKNIĘTA (`d03c0bc37f` — persistKey hook+test); L-06 już zamknięta (`$$typeof` 2026-06-16); L-03 INERT (0 user-impact); L-07 częściowa (OAuth=deploy-time env Railway); **L-08 ZAMKNIĘTA (initiative→in-context przez `handleOpenDocument`+`InitiativeFullView`; SSOT `openItemRouting.ts`; test 6/6 CI-gated)**; L-09 ODROCZONA D-03 (color-loss zamknięty migracją FullCalendar — zostaje tylko decyzja design Timeline vs cap); L-11 i18n FAZA-4 sweep (odroczona). **Wszystkie luki P0/P1 funkcjonalne+security ZAMKNIĘTE (w tym L-08 in-context open); pozostałe = decyzja design D-03 (Month cap/Timeline), sweep i18n FAZA-4, deploy-time OAuth.** Re-ocena po sesji żywej (R6).
...
> Inwentarz 5 powierzchni M03 (Inbox · Kalendarz · Zadania · Decyzje · Manager) + kluczowe modale/detale/stany. Ugruntowane w `src/components/MyWork/`.

| # | Ekran | Cel | Plik komponentu |
|---|-------|-----|-----------------|
| 1 | Hub My Work (Menu 1/2/3, dynamic tabs) | Powłoka przełączająca 5 powierzchni + IDE-tabs | `src/components/MyWork/MyWorkHub.tsx` · `src/views/MyWorkView.tsx` |
| 2 | Inbox (triage feed) | Triage z quick-actions, bulk, presety Overdue/Urgent/Inbox | `src/components/MyWork/InboxContent.tsx` |
| 3 | Kalendarz — grid Month/Week/Day/List | Unified feed task/initiative/decision + Google/Outlook | `src/components/MyWork/Calendar/CalendarView.tsx` + `CalendarGrid.tsx` |
| 4 | Kalendarz — sidebar źródeł | SOURCES toggle + „Podłącz w Integracjach" (deep-link) | `src/components/MyWork/Calendar/CalendarSidebar.tsx` |
| 5 | Modal tworzenia eventu kalendarza | Create event (task-backed + callbacki) | `src/components/MyWork/Calendar/CalendarCreateEventModal.tsx` |
| 6 | Zadania — tabela §27 + Kanban DnD | Lista zadań, filtry, sort, kebab, DnD | `src/components/MyWork/MyTasksListContent.tsx` |
| 7 | Detal zadania | Pełny widok zadania | `src/components/MyWork/table/` / `TaskDetailView` |
| 8 | Decyzje — tabela §27 + Kanban | Lifecycle approve/reject/escalate | `src/components/MyWork/DecisionsPanelContent.tsx` + `DecisionsKanbanBoard.tsx` |
| 9 | Detal decyzji | Pełny widok decyzji (history/escalation/opinions) | `src/components/MyWork/DecisionDetailView.tsx` (+`DecisionDetailModal.tsx`) |
| 10 | Manager / Executive Dashboard | Portfel zespołu, overload alerts (role-gated serwerowo) | `src/components/MyWork/Executive/` ExecutiveDashboard |
| 11 | Inicjatywa in-context (IDE-tab) | `initiative` otwiera `InitiativeFullView` w panelu (L-08) | render via `openItemRouting.ts` + `MyWorkHub.tsx` |
| 12 | Stan crash error-boundary (naprawiony — 0 crashy) | Granica błędu render-time | `RouteErrorBoundary`/`ViewErrorBoundary`; fix `PreviewRelations.tsx:42` |
| 13 | Stany puste/ładowanie/błąd tabel | Empty-state CTA, skeleton, error+retry | `MyTasksListContent`/`DecisionsPanelContent`/`InboxContent` |
| 14 | Detal notyfikacji | Podgląd pozycji Inbox | `src/components/MyWork/NotificationDetailView.tsx` |
| 15 | Convert-to (output/dialog/menu) | ConvertToOutput z encji | `ConvertToDialog.tsx` / `ConvertToOutputMenu.tsx` / `ConvertToMenu.tsx` |
