...
| L-07 | in-context open (nawiguje do modułu) | W-03 | `MyWorkHub.tsx` (`resolveOpenItemRoute`/`handleInitiativeClick`/`InitiativeFullView` render-case); `openItemRouting.ts` (SSOT) | P1-design | 0.4 | **ZAMKNIĘTA `42f11f6dce` (2026-06-17, Harvard 2 — koordynacja z H3)** — DP-2: klik inicjatywy otwiera się IN-CONTEXT w doku zakładek (`InitiativeFullView` self-fetch, ten sam widok co M13) zamiast hard-`navigate()`; SSOT `openItemRouting.ts` (`initiative`∈in-context; deck/doc→Canvas; ciężkie budget/valuation/report→pełny moduł); kalendarz `onInitiativeClick`→`handleInitiativeClick` in-context. Test `tests/unit/myWorkOpenItemRouting.test.ts` (6/6 PASS, CI-gated) jawnie pinuje `initiative→in-context` dla M03 L-08 **i** M13 L-07 (ta sama funkcja DP-2). Implementacja w strefie H2; żywy smoke = R6 (auth-gated). |
...
| L-11a | i18n inline ~1820× | W-01 | `Initiatives/` (`InitiativeDocumentView` 423+`sections/*`) | P1 | 4 | **ZAMKNIĘTA `9fdbbf82ce`+`f5f333c135` (2026-06-18, Harvard 2 — owner locales)** — wszystkie 26 `sections/*` + widoki top-level + helpery: `isPolish`→`t('initiatives.<section>.<key>')`, klucze PL+EN w `public/locales/{pl,en}`. Twarda bramka: 1297 użytych kluczy `initiatives.*`, 0 brakujących w pl I en; pozostałe `isPolish ?` (109 w sekcjach) = wyłącznie selektory danych dwujęzycznych + formatery dat (nie UI-stringi). Dług PL domknięty: +28 kluczy kanban/analysis/execution (EN-default→PL). PL render zweryfikowany runtime (dev-server pl.json: `kanban.noOwner`→„Nieprzypisana", `analysis.feasibility.allGood`→„Wszystkie inicjatywy mają zdrową wykonalność"). Screenshot nawigacyjny blocked-auth. |
...
| L-12 | governance router org-spoofable (`/api/initiatives-v4`) | W-01 | `Gateway.ts:906` | P1 | 1 | **NIEAKTUALNA (fałszywy alarm potwierdzony 2026-06-17)** — `requireUser()` w `server/src/routes/initiative-governance.routes.ts` czyta orgId WYŁĄCZNIE z JWT: `router.use(verifyToken)` (linia 14) → `const orgId = req.user?.organizationId \|\| req.organizationId` (linia 18). `req.user` = obiekt zdekodowany przez `verifyToken` z tokenu JWT — nie z req-params, nie z nagłówka klienta. Org-spoofing przez parametry URL/body niemożliwy. |
