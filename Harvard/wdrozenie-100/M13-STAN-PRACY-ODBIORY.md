...
| R4 | M13d Notyfikacje — build | R | **4/4** | ✅ | ✅ | 0/10 | N/A | ⬜ | ⬜ | 2 | 🟢 DEPLOYED demo — **epiki 4/4** (audyt 2026-06-23): E1 status-change kanoniczny (dubel Wariant A naprawiony, brak underscore) · E2 `notifyAssignment` WPIĘTY `TaskController:1895` (`40018dc782`) · E3 due-breach cron `job33` za flagą OFF (`3be1de017f`) · E4 blocker=CRITICAL. Testy notif 36/36 |
...
| V1 | Gant zadaniowy + drag-reschedule | V (widoki) | 3/3 | ✅ | ✅ | 0/10 | 🟡 | ⬜ | ⬜ | 2 | 🟢 GOTOWY code-side — bary tydzień+dziś-marker+drag-reschedule (W5 `5ab7c4f121`) + toggle Kalendarz/Gantt + **4 funkcje dobudowane `a8cfca1df4`**: zoom day/week/month · filtr statusu · linie zależności (SVG) · ścieżka krytyczna (`computeCriticalPath` DAG, ring-rose); TimelineSection liczy krawędzie+CP. Testy V 24/24; tsc clean |
...
**Epiki:** E1 status-change ✅ · E2 assignment ⬜ (niewpięty) · **E3 due-date breach ✅ DOMKNIĘTE code-side 2026-06-22** (serwis + `cron/InitiativeDueBreachCron.ts` deps[fetchCandidates/notify/markNotified] + lazy-ensure kolumny `due_breach_notified_for` + job33 Scheduler daily-6AM **za flagą `INITIATIVE_DUE_BREACH_CRON_ENABLED` OFF** + 15 testów; tsc clean; zostaje live-verify real-DB + ewentualne ON na demo-org) · E4 blocker ✅ (eskalacja statusu CRITICAL).
> R4 realizacja: **3/4 epiki ✅** (E1/E3/E4), E2 `notifyAssignment` = jedyna niewpięta (czeka decyzji o call-site).
