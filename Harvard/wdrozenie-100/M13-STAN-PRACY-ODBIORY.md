...
| V1 | Gant zadaniowy + drag-reschedule | V (widoki) | 2/3 | ✅ | ✅ | 0/10 | 🟡 | ⬜ | ⬜ | 2 | 🟢 GOTOWY code-side — `InitiativeGantt` (schedule-bary tydzień, dziś-marker, undated; wspólny `ScheduleItem` z Kalendarzem wg Q4) + toggle Kalendarz/Gantt w `TimelineSection`; tsc clean. Drag-reschedule = follow-up |

**Postęp programu:** 0 / 16 ZAMKNIĘTYCH (8/8) · **Seria G (AI bramki) 5/5 DEPLOYED demo** 🟢 (kod✅ testy✅ deploy✅ migracje✅ endpoint✅ flaga ON org Piotra✅ — zostaje →F klik Piotra + →UI screeny) · **Seria R (artefakty) 4/4 DEPLOYED demo** 🟢 (Kalendarz+banner decyzji+notyfikacje+DELETE live; →UI screeny; ogony: calendar drag-persist, R4 assignment/due/blocker) · **Seria C (tworzenie) 2/2 DEPLOYED demo** 🟢 (`4203292e`: C1 dedup `/similar-check`+wizard warning · C2 Teresa `generate_initiative` tool; e2e na demo = klik) · **Seria K (artefakt) 2/4 DEPLOYED** 🟢 (K1 §B3 validators+FE-lint · K3 korelacja trwała e2e; K2 CardContainer odłożony Q6; K4 AI-fill sekcji TODO) · **Seria V (widoki) 1/1 GOTOWA code-side** 🟢 (Gant zadaniowy + toggle Kalendarz/Gantt; drag-reschedule=follow-up) · Manual (Playwright/computer-use): 0/121 · Ekrany: 0/16.
