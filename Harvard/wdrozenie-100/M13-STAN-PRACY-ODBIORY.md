...
| R4 | M13d Notyfikacje — build | R | **4/4** | ✅ | ✅ | 1/10 live-verify | N/A | ⬜ | ⬜ | 2 | 🟢 DEPLOYED demo — **epiki 4/4** (audyt 2026-06-23): E1 status-change kanoniczny (dubel Wariant A naprawiony) · E2 `notifyAssignment` WPIĘTY `TaskController:1895` (`40018dc782`) · E3 due-breach cron `job33` za flagą OFF (`3be1de017f`) · E4 blocker=CRITICAL. Testy notif 36/36. **real-data 06-23: centrum in-app utknęło na „Loading" (artefakt headless M09) → live-verify w real browser** |
...
| V1 | Gant zadaniowy + drag-reschedule | V (widoki) | 3/3 | ✅ | ✅ | 3/10 real-data | 🟡 | ⬜ | ⬜ | 2 | 🟢 GOTOWY code-side — bary tydzień+dziś-marker+drag-reschedule (W5 `5ab7c4f121`) + toggle Kalendarz/Gantt + **4 funkcje `a8cfca1df4`**: zoom day/week/month · filtr statusu · linie zależności (SVG) · ścieżka krytyczna (`computeCriticalPath` DAG). **real-data 06-23: bary + zoom day/month PASS. Linie zależności NIE renderują — 2 bugi: (1) backend `task-dependencies` lowercase-alias NAPRAWIONY+DEPLOYED+zweryfikowany live (`planningPortfolioReadService` +3 testy); (2) FE rozjazd `ganttDependencies` (`sourceTaskId`+dedup) NAPRAWIONY. ALE pełny wizual blokuje 🟠 D1b: DWA Gantty/DWA źródła — domyślny `TimelinePlanner` rysuje z ręcznego `row.dependsOnId`, nie z task_dependencies. DECYZJA: wpiąć task_dependencies w TimelinePlanner (opcja A). Patrz `_ANALIZA_REALDATA_M13_2026-06-23.md §D1b`.** Testy V 24/24+3 |
...
- **Seria V (1):** D1 (backend) + FE-memo NAPRAWIONE+DEPLOYED, ale 🟠 **D1b** blokuje wizual linii — DWA Gantty/DWA źródła. **DECYZJA CTO: opcja A** (wpiąć task_dependencies w `TimelinePlanner.rows.dependsOnId`) = osobne, świadome zadanie. Reszta V1 (bary/zoom/drag/marker) PASS.

**Stan deployu (06-23):** ✅ fix D1 `task-dependencies` zdeployowany na demo i zweryfikowany live (endpoint zwraca realne ID). Seria G już DEPLOYED+flaga ON na org Piotra. **Następny krok:** (1) D1b opcja A — wpiąć task_dependencies w główny Gantt (TimelinePlanner) → wtedy V1 wizual domknięty; (2) Piotr robi →F/→UI na demo (dane bogate, hydratuje w realnej przeglądarce); (3) live-verify D2/D3.
