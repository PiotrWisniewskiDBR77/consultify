...
| V1 | Gant zadaniowy + drag-reschedule | V (widoki) | 3/3 | ✅ | ✅ | 3/10 real-data | 🟡 | ⬜ | ⬜ | 2 | 🟢 GOTOWY code-side — bary tydzień+dziś-marker+drag-reschedule (W5 `5ab7c4f121`) + toggle Kalendarz/Gantt + **4 funkcje `a8cfca1df4`**: zoom day/week/month · filtr statusu · linie zależności (SVG) · ścieżka krytyczna (`computeCriticalPath` DAG). **real-data 06-23: bary + zoom day/month PASS; linie zależności NIE renderowały — bug Postgres `task-dependencies` (lowercase-alias) NAPRAWIONY (`planningPortfolioReadService` + 3 testy); widoczne po re-deployu demo** (dane zależności zasiane). Testy V 24/24+3 |
...
- **Seria V (1):** **re-deploy demo z fixem `task-dependencies`** → Gantt pokaże linie zależności + ścieżkę krytyczną (dane zasiane) → wtedy V1 Manual domknięty wizualnie.

**Następny krok rekomendowany:** deploy na demo (fix D1 `task-dependencies` + Seria G migracje/flaga) → odblokowuje Gantt-deps + soft-block na żywo → Piotr robi →F/→UI na zasianych danych.
