...
|----|------|---------|--------------------|-------|------|-----------|---------|
| L-01 | trwałość 6/8 warstw in-memory → utrata po deployu | W-01,W-03,W-04 | mig. `780`+`781`; 6 DAO przepisane na Postgres (0 `new Map`, INSERT+UPSERT) | P1 | 1 | **ZAMKNIĘTA** — (1) kod (`953955bc2b`+`8d2b5d8cf4`), (2) cold-start CI proof (`approval-coldstart.contract.test.ts` 7/7), (3) **SCHEMA POTWIERDZONA NA ŻYWEJ BAZIE STAGING (trolley)**: wszystkie 7 tabel wave5 (`document_approvals/_audit`, `_content_blocks`, `_brand_voice_profiles`, `_audience_profiles`, `_source_packs`, `_share_links`) ISTNIEJĄ (read-only check 2026-06-17, 0 wierszy = brak ruchu, oczekiwane). DAO NIE spada do in-memory. Literalny restart round-trip = formalność (schema żyje) | 2026-06-17 |
...
| L-09 | i18n EN-only (`useTranslation` w 2 plikach) | W-01 | `DocumentStudio/*` (grep `useTranslation`=2) | P2 | 4 | **ZABLOKOWANA (Fala 4)** — `public/locales/*` ZAKAZANE w Fali 1; wymaga koordynacji z agentem i18n (inny commit); scope: sweep `t()` przez cały katalog `src/components/DocumentStudio/` |  |
