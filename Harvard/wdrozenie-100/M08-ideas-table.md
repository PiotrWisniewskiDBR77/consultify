...
| L-04 | Martwy kod (LegacyViewRouter, offline/, PublicFormView…); dual-stack ścieżka B (flaga OFF) | W-01 | `IdeaTableTool.tsx:2641-2682`, `table/views/*` | P2 | 3 | **CZĘŚCIOWO ZAMKNIĘTA (2026-06-17)** — martwy kod PublicFormView.tsx + offline/ (0 ref) USUNIĘTY (`f7fada6924` — zgarnięte do commita M02 przez wyścig współdzielonego indeksu; pliki GONE z HEAD). LegacyViewRouter = FALSE POSITIVE (żywy render `:2629`). **Cut mostu path-B (DP-7) = D-01 ODROCZONA**: refaktor ~40% kodu narzędzia, koordynacja z M20 (backend zostaje), nie correctness-bug → osobny pass po R6 |
| L-05 | **163 test-case'y poza CI** (14 plików, zweryf. grepem 2026-06-13) + brak S1-S5 + CI bez `Londyn` | W-01,W-03 | `.github/workflows/test-suite.yml:314,509` | P0-test | 4 | **GAP REPO-WIDE (zweryf. 2026-06-17)** — CI uruchamia jawnie `tests/unit`/`tests/integration`/`tests/components` (nadpisuje glob `src/**/__tests__/**` w vitest.config) → 15 plików testów tabeli pod `src/**/__tests__/` NIE w CI. To NIE M08-lokalne (cały `src/**/__tests__` repo) i dotyka współdzielonego `test-suite.yml` → wymaga ownera infra (nie unilateralnie przez H2 — wyścigi CI). Krytyczny test bezpieczeństwa M08 (L-03) JEST CI-gated (`tests/integration`). |
