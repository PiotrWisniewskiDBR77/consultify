...
| L-05 | brak `persistKey` (4 tabele) + i18n inline | W-01 | `src/components/Interview/` (**2090** i18n grep) | P1 | 4 | **persistKey = FALSE-POSITIVE (zweryf. 2026-06-17 sub-agentem): wszystkie tabele wywiadu (Sesje/Przydzielone/Szablony/Wnioski/Inbox/Inicjatywy) JUŻ persystują szerokości kolumn via `loadColumnWidths`/`saveColumnWidths`+`ColumnResizer` (`INTERVIEW_*_COL_WIDTHS_KEY`) + widoczność via `TABLE_VIEW_STORAGE_KEY` (`InterviewHub.tsx:8550-8586`). 0 zmian. • i18n (~2090 inline) = ZABLOKOWANA (locales poza strefą)** |
...
| D-05 | server STT na prod: który provider klucz? | OPENAI / GROQ / oba | Piotr | 2026-06-13 | **ROZSTRZYGNIĘTE → DP-1: OPENAI/Whisper** (egzekucja env wymaga zgody/dostępu prod). **ROZSZERZENIE 2026-06-18:** Gemini STT fallback (`9cdb40787d`) — `GEMINI_API_KEY` reużyty (Teresy) → zero nowego sekretu; działa bez OPENAI. |
...
- **2026-06-18 (Harvard Final): L-01 KOD ZAMKNIĘTY — Gemini STT fallback `9cdb40787d`:** `VoiceService.ts` dostał trzeci fallback `gemini-2.0-flash` speech-to-text po OPENAI→GROQ; `GEMINI_API_KEY` = klucz Teresy (już na Railway). Zero nowych sekretów. Testy 2/2. Deploy na Railway demo + live-verify = warunek R6 (w rękach Piotra/auto-deploy z GitHub).
- **2026-06-17 (Harvard 4 Fala 5): §27 audit — 7 surowych `<table>` w `src/components/Interview/` oznaczone jako §27-exempt** (commit `256248b784`): Sesje/Wnioski/Szablony/Przydziały/Inicjatywy w InterviewHub.tsx — moduł-lokalne resizable kolumny + FilterDropdown + checkbox selection tightly coupled; InsightViewer.tsx — pivot/cross-tab topic×session/stakeholder; QuestionsList.tsx — `TableWithPreviewLayout` z selection driving preview pane. L-06 ZAMKNIĘTA (§27-exempt komentarze w HEAD). Hex tokens: `dark:bg-[#F4F7FB]`→`dark:bg-slate-50`, `dark:hover:bg-[#DDE5EF]`→`dark:hover:bg-slate-200` (10× InterviewHub.tsx), `dark:via-[#0a0f1e]`→`dark:via-navy-900` (InterviewWorkspace.tsx), `dark:bg-[#151E32]`→`dark:bg-navy-800`, `dark:bg-[#0F172A]`→`dark:bg-navy-900`, `dark:hover:bg-[#151E32]`→`dark:hover:bg-navy-800` (TemplateBuilder.tsx) — commit `a448982af3`. InsightViewer.tsx `style` props print/PDF = §27-exempt. i18n (L-05): locales poza strefą — ZABLOKOWANA. Fala 5 rose→danger sweep: 75 klas `rose-*` → `danger-*` w 12 plikach `src/components/Interview/` — commit `7a17fcfe84` (merge `153f97b207`).
- **2026-06-17 (Harvard 3, runda 3): L-05 persistKey = FALSE-POSITIVE** (sub-agent: wszystkie tabele wywiadu już persystują kolumny via loadColumnWidths/ColumnResizer). i18n-część L-05 = ZABLOKOWANA (locales). Pozostają OTWARTE tylko: L-01 (PROD P0, czeka na Piotra) + i18n (locales). L-06 było false-positive (runda 2). M10 = praktycznie domknięte poza PROD P0 + i18n.
