...
| 9 | Statements → Model (grounding) | ⬜ | DOPRECYZOWANE probe'em 2026-07-01: kontrakt PRZYJMUJE `sourceStatementId` (mechanizm istnieje!) — realna praca = kompletny seed statementu (padło na „missing CASH lines") + wpięcie w UI tworzenia modelu + refresh-from-source. Mniejsze niż zakładano. |
...
| 5 | M15 OEE jednostki (+162252%) | ⬜ |
| 6 | M15 wykresy nie renderują | ⬜ |
| 7 | M15 panel KPI rozjechany + baseline „5 3" | ⬜ |
| 8 | M15 Lineage duplikaty + „unknown" | ⬜ |
| 9 | M16 kreator full-screen bez nawigacji | ⬜ |
| 10 | M16 tytuł kreatora pod logo | ⬜ |
...
| 15 | M06-UI3 command row nad panelem profilu (z-index) | ⬜ |
| 16 | M08 rail-undo | ✅ (T2.2, live) |
| 17 | M24 PATCH roli działa tylko z `user_id`; z membership `id` → 404 MEMBER_NOT_FOUND (znalezione probe'em 2026-07-01) | ⬜ |
...
| 8 | M12 Audyty (orkiestrator ankiet — fan-out przez szablony wywiadów) e2e mechanika + odbiór — **złapane macierzą pokrycia, wcześniej w ŻADNEJ liście** | ⬜ |
**Licznik H3: 0/8 ✅**
...
| # | Temat | Stan |
|---|---|---|
| 1 | M10 Wywiad: server-STT verify (P0 — utrata głosu) | ⬜ |
...
| 7 | Deploy pipeline + monitoring (D-J: probe'y → Panel Health) | ⬜ |
