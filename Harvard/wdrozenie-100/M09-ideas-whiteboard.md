...
|----|------|---------|--------------------|-------|------|--------|
| L-01 | Per-user dokument blokuje multiplayer (→ per-resource + membership/share) | W-01,W-04,W-05 | `my-work.routes.ts` GET /map | P0-struct | 1 | **NAPRAWIONA 2026-06-17 `5928262e0f`** — org-read fallback na GET `/map`: nie-właściciel z org czyta kanoniczną tablicę właściciela idei (200, nie 404); właściciel/single-player bez zmian (WRITE per-user) → zero regresu M05/M07/M08. Testy 4/4 (`my-work.map-orgread.contract.test.ts`: 200 non-owner, 404 cross-org, default-empty). **Pełny shared-WRITE persist = v1.1** (koord. M05; M06-wzór też tego nie ma). Żywe 2-sesje pending auth. |
| L-02 | Whiteboard nie nadaje/odbiera realtime `graph_patch` | W-01,W-05 | `IdeaWhiteboardTool.tsx` + `whiteboard/useWhiteboardCollab.ts` | P1 | 1 | **NAPRAWIONA 2026-06-17 `e23e36b856`** — whiteboard wpięty w org-scope WS (wzór M06): hook `useWhiteboardCollab` emit/odbiór `graph_patch` (add/move/remove node, add/remove edge) + guard własnego echa + remote-apply. `CollaborationOverlay` dostał `onRegisterSend`. Testy 6/6 (`useWhiteboardCollab.test.tsx`). Żywe 2-sesje pending auth. |
...
| L-04 | Stan sesji nieczytany (0 call-sites); role samonadawane; governance FE-only; voting niespójny | W-01 | `api.ts:18483`,`:1101`, `IdeaExportMenu.tsx:177`, `realtimePlatformService.ts:293` | P1/P2 | 3 | **OTWARTA (P1/P2, nie w tej fali)** — poza rdzeniem multiplayer (L-01/L-02). Wymaga: konsumpcji `getFacilitationSession` (timer/faza), enforcement ról serwerowo, governance BE, dot-voting spójny. Backlog po żywej weryfikacji multiplayer. |
| L-05 | Martwy kod; kształty zablokowane; NodeResizer; obrazy base64 | W-01 | `IdeaWhiteboardTool.tsx`, `WhiteboardToolbar.tsx:127` | P2 | 3 | **CZĘŚCIOWO 2026-06-17 `e23e36b856`** — base64 cap 10MB (paste+drop, toast). `useIdeasTeresaBridge` = **N/D** (0 referencji w repo, nie istnieje). Kształty circle/diamond/hexagon (toolbar dropdown) + NodeResizer = **OTWARTE** (P2 polish, defer). |
| L-06 | Brak testów WS/shared board + E2E | W-01 | `tests/unit/MyWork/`, `tests/integration/mywork/` | P0-test | 1+4 | **CZĘŚCIOWO 2026-06-17** — 3 suity w CI (auto-glob `tests/{unit,integration}`): `useWhiteboardCollab.test.tsx` 6/6 (graph_patch), `my-work.map-orgread.contract.test.ts` 4/4 (org-read). Ścieżki zgłoszone H2 (CI owner) — workflow nietknięty. **E2E 2-sesje na żywo pending auth.** |
