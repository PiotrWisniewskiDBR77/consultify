...
| 6 | Deploy demo | ✅ | **WDROŻONE 2026-06-20** — `915550f82b` na origin/demo (`scripts/deploy-demo.sh`, Railway demo env), build **SUCCESS**, żywe na **https://demo.consultify.ai** (health 200, `database:connected`, gitSha potwierdzony). Zawiera bugfix 42P18 (search crash+injection) + 95 testów komponentowych + E2E. ⚠ **Follow-up (NIE-blokujący):** `search_vector` DDL zaaplikowany na staging dev (trolley), NIE na demo DB — jeśli demo ma osobną bazę, zaaplikować ten sam idempotentny skrypt; jeśli demo dzieli centerbeam(prod), wymaga osobnej zgody. Bugfix 42P18 już na demo (search degraduje gracefully, nie crashuje, bez kolumny). |
