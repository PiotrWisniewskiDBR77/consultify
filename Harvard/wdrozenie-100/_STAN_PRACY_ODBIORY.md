...
| M07 | Ideas — Process Flow | 2/3 | 6/6 | 5/7 | 36✅ | 7✓/8; **1 real bug** | 🟡 | ⬜ | ⬜ | 12 | 🟡 BLISKO + **REALNY BUG PRODUKCYJNY** (2026-06-21): shell/dark/tool-switcher/add-node ✅; §3 persist-after-reload pada = **race autosave** — ProcessFlow odpala DWA równoległe `POST /map/sync` w tym samym ms → przegrany 409, payload (dodane węzły) ginie → 0 po reloadzie. **POTWIERDZONE NA BUILDZIE PRODUKCYJNYM** (:3001 dist, bez StrictMode, zdrowa DB) — NIE artefakt dev, dotyka demo. Fix in-flight guard NIE wystarczył (race cross-instance, cofnięty). **Do domknięcia: ŹRÓDŁO 2 syncy (prawdop. blob-sync + structural V8 CRUD `IdeaProcessFlowTool:630`) + serializacja** — fokusowe zadanie, NIE rush w żywym kodzie klientów |
