...
| M07 | Ideas — Process Flow | 2/3 | 6/6 | 5/7 | 36✅ | 6✓/8 +5skip (live 06-21) | 🟡 | ⬜ | ⬜ | 12 | 🟡 BLISKO + **REALNY BUG znaleziony** (live 2026-06-21): shell/dark/tool-switcher/add-node ✅; 2 faile (§3/§6 persist-after-reload) = **race autosave** — ProcessFlow odpala DWA równoległe `POST /map/sync` w tym samym ms → przegrany dostaje 409 (stary baseVersion) i jego payload (dodane węzły) ginie → 0 po reloadzie. **Diagnoza z logów backendu**; nie test, nie sesja. Fix (in-flight guard w `useIdeaMapSync`) NIE wystarczył — race jest między 2 instancjami/ścieżkami sync (cofnięty jako niezweryfikowany w żywym kodzie klientów). Do domknięcia: ustalić ŹRÓDŁO 2 syncy w ProcessFlow + serializacja cross-instance |
