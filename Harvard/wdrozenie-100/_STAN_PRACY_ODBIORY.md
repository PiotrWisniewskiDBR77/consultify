...
| M06 | Ideas — Mind Map | 1/3 | 7/7 | 6/7 | 230✅ | 🟢 **m06-live 4/4** (live 06-21) | 🟡 | ⬜ | ⬜ | 16 | 🟢 BLISKO ODBIORU — **2 root-cause'y test-infra naprawione** (2026-06-21): (1) mock-DB SELECT ignorował `LOWER(email)=LOWER(?)` → register-demo 400 na 2. emailu → kładło **127 testów** headless; (2) demo-session read-only (403 `DEMO_READ_ONLY` na `/map/sync`) → harness teraz mintuje non-demo token przez test-support bootstrap. **m06-live 4/4 zielone** (FIT + §2.1 sync=200 + §2.2 + §15.6 reload-persist). Commit `39380c9ded`. Pełny suite (~98 scenariuszy) w trakcie re-run. **5 luk produktowych** (BatchConvert/convert→Prezentacja/Timeline/3D/TimeHeatmap — niżej) + →F/→UI |
| M07 | Ideas — Process Flow | 2/3 | 6/6 | 5/7 | 36✅ | ✅ **3/3** interactions (live 06-21) | 🟡 | ⬜ | ⬜ | 12 | 🟢 DO ODBIORU — **REALNY BUG PRODUKCYJNY NAPRAWIONY+ZWERYFIKOWANY** (2026-06-21): §3 persist-after-reload był = **data-loss w `useIdeaMapSync`**. Root cause finalny: success-path czyścił `queuedPayloadRef=null` *bezwarunkowo* zanim `finally` go odczytał → deferred payload (P2 zakolejkowany przez 2. call w trakcie await) ginął cicho → 0 węzłów po reloadzie. Fix: czyść ref tylko gdy `=== payload` który właśnie zapisaliśmy (`b8626b01b1`). **Live-verify: §3 + §4.2 + §6.1 = 3/3 zielone** (write-access harness, prod-build path). Radiuje na M06/M08/M09 (wspólny hook). Czeka →F/→UI/deploy |
...
| M13 | Inicjatywy | 2 | 0/16 | — | 🔵 | ✅ **S1-S3 3/3** (live 06-21) | ⬜ | ⬜ | ⬜ | 16 | 🔵 PROGRAM M13 DEPTH — osobny SSOT: [`M13-STAN-PRACY-ODBIORY.md`](M13-STAN-PRACY-ODBIORY.md) (16 sub-modułów, serie G/R/C/K/V). **Postęp 2026-06-21:** Serie G (bramki AI 5/5) + R (artefakty W2 5/5) + **W5 Gantt drag-reschedule** (`5ab7c4f121`, pointer-events + PUT `/api/pmo/tasks/:id`) + **K4 AI-fill** (hypothesis/OKR/lessons-learned `80e5ef94ea`) code-side gotowe. **Headless acceptance S1 hub + S2 dokument + S3 Timeline/Gantt/Kalendarz = 3/3 zielone**. Done-by-the-way: DELETE 409+test 7/7, kebab Archive/Delete. Branch `feat/deliverables-w1`→demo |
