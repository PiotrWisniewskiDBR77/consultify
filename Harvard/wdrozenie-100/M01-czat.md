...
| L-10 | i18n inline 305× | W-01 | `src/components/AIChat/` | P1 | 4 | **ZABLOKOWANA-i18n (czeka na decyzję locales)** — re-grep 2026-06-17: **318** inline lang-conditionals (`i18n.language==='pl'`/`isPolish`) w `src/components/AIChat/` (było ~305). Ekstrakcja inline→`t()` wymaga kluczy w `public/locales/*` (ZAKAZANE dla H1 — inny agent + merge). Czysty `t()` z EN-default regresowałby PL (dominujący user). Czeka: zgoda Piotra na zniesienie zakazu + nadanie ownera i18n, albo koordynacja z agentem locale |
