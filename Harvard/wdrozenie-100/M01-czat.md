...
| L-03 | brak pokrycia S3/S4/S6 + smoke poza PR-gate | W-01 | `test-suite.yml` (push/PR Londyn) | P1-test | 2/4 | **CZĘŚCIOWO ZAMKNIĘTA 2026-06-17** (R3: pokrycie istnieje — `work-canvas-split` S4, `work-canvas-core-flow` S6, `chat-refresh-persistence` S1, `ai-chat-stream-e2e-mode`; **`test-suite.yml` JEST PR-gated** na push/PR Londyn → nowe unit-locki tej fali (detectMessageLanguage, shareMetadataWhitelist, aiMemoryGating) łapane w PR. Residual: dedykowany S3-ingest e2e + jawne wpięcie smoke do PR-gate = CI-infra, odroczone — współdzielony `.github/workflows`, nie ruszam unilateralnie) |
...
| L-05 | martwy `CodeInterpreter`(+`OrganizationMemoryPanel` jeśli ukryć) | W-01 | 0 zewn. referencji | P2 | 3 | **ZAMKNIĘTA 2026-06-17** (orphan `src/components/AIChat/CodeInterpreter/` usunięty — 0 importów potwierdzone grepem; `OrganizationMemoryPanel` → osobno L-01/DP-5) |
| L-06 | composer ramka-w-ramce | W-02 | `EnhancedChatInput.tsx:1076-1084` | P3 | 4 | **otwarta (Faza 4, live DOM-inspect)** — R3 analiza statyczna 2026-06-17: sam `EnhancedChatInput` ma DOKŁADNIE JEDNĄ ramkę (zewn. kontener `:1076-1084`, focus `border-c-focus-solid`); textarea `:1110-1179` i action-bar `:1189-1194` są bezramkowe (`bg-transparent`/`focus:outline-none`). Druga ramka (jeśli nadal jest) pochodzi z wrappera-rodzica w `UnifiedChatPanel`, nie z composera → wymaga `preview_inspect` żywego DOM by wskazać winowajcę. P3 polish, odroczone do smoke wizualnego |
...
| L-10 | i18n inline 305× | W-01 | `src/components/AIChat/` | P1 | 4 | **ZABLOKOWANA (Faza 4)** — ekstrakcja 305 inline-ternary wymaga dodania/utrzymania kluczy w `public/locales/*`, co jest ZAKAZANE dla H1 (inny agent + ryzyko merge na współdzielonym drzewie). Wymaga koordynacji z agentem i18n lub jawnej zgody Piotra. Komponenty AIChat są w mojej strefie, ale bez kluczy locale nie domknę bez konfliktu |
