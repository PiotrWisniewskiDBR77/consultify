...
| L-07 | Split-brain `versions` vs `snapshots` | W-01,W-04 | mig. 622 vs 20260611 | P2 | 3 | **ODROCZONA / OUT-OF-ZONE (2026-06-17, Harvard 2)** — `my_idea_map_versions` ma **0 czytelników runtime** (`grep` po `server/src/**` + `src/**` = tylko pliki migracji 622/baseline/archive + DB-inventory exports). Aktywny runtime używa wyłącznie `my_idea_map_snapshots`+`my_idea_activity` (`my-work.routes.ts:4634-4986`). To NIE żywy split-brain — to **martwa tabela** (orphan z mig.622). Rozstrzygnięcie = drop tabeli `versions`, czyli `server/migrations/**` (poza ZONE M05). W ZONE (`src/components/MyWork/**` + `my-work.routes.ts`) brak referencji → nic do zmiany. Zostaje D-02 (kanon) + cleanup migracji jako osobne zadanie poza-strefowe. |
