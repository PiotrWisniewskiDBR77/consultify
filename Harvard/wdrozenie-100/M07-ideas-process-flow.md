...
| L-06 | Brak testu kontraktu ID / call-site + E2E + CI bez `Londyn` | W-01 | `tests/*` (brak) | P0-test | 1+4 | **NIEAKTUALNA (DP-7 CUT) — ZAMKNIĘTA jako nieaktualna** (R3 2026-06-17): kontrakt ID serwerowy zniknął z kodem (0 server-side process-flow testów, `find server -path '*test*' -iname '*processflow*'` pusty); FE pokryte smoke `src/components/MyWork/processflow/__tests__/useProcessFlowCRUD.smoke.test.ts` — brak kontraktu do testowania po CUT |
