...
> **🔌 M25 = HUB SYNCHRONIZACJI ZEWNĘTRZNEJ (decyzja 2026-06-20).** Cała synchronizacja świata zewnętrznego konwerguje tu, przez jeden silnik `integrationOAuthEngine.ts` (providerzy/scopy `:43-131`) + `settings.routes.ts` (`/integrations/oauth/*`). Trzy osie, jeden mechanizm OAuth2:
> - **Kalendarze** — `google_calendar`/`outlook_calendar` (scope `calendar.events`). Konsument: M03 My Work/Kalendarz (panel SOURCES „Connect in Integrations →" wskazuje TU; M03 sam tylko renderuje feed wewnętrzny + ICS).
> - **Taski / PM-sync** — Jira i inne przez `v8/sync.routes.ts` + `pmSyncExternalAuthMaterializationService`. Konsumenci: M03 Tasks, M13/M14 (inicjatywy/wdrożenie).
> - **Przestrzenie chmurowe do czata** — cloud sources connectors `cloud.routes.ts` (`authType:'oauth2'`) → korpus wiedzy zasilający Teresę/Annę (M01 Czat).
>
> **Wniosek architektoniczny:** moduły-konsumenci (M03 kalendarz/taski, M01 czat-chmury) NIE implementują własnej synchronizacji — delegują do M25. Każdy nowy connector = wpis w `integrationOAuthEngine.ts` + sekrety env (patrz **L-11**). Aktywacja na żywo wymaga provider app-credentials (Piotr — Google Cloud Console / Azure Portal).
...
| **L-11** | **Synchronizacja zewnętrzna — provider app-credentials NIE ustawione (env Railway) → cała rodzina connectorów OAuth nieaktywna na demo/prod.** Silnik gotowy (`integrationOAuthEngine.ts`), brakuje sekretów rejestracji aplikacji u Google/Microsoft. Dotyczy 3 osi: **(a) kalendarze** `google_calendar`/`outlook_calendar` (scope `calendar.events`; UI „Connect in Integrations →" w M03 Kalendarz/SOURCES), **(b) taski / PM-sync** (Jira/external przez `v8/sync.routes.ts`, `pmSyncExternalAuthMaterializationService`), **(c) przestrzenie chmurowe do czata** (cloud sources connectors `cloud.routes.ts`, `authType:'oauth2'` → korpus wiedzy Teresy/Anny). | W-01, **M03 (deploy demo 2026-06-20)** | env: `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`GOOGLE_CALLBACK_URL`, `MICROSOFT_CLIENT_ID`/`MICROSOFT_CLIENT_SECRET`/`MICROSOFT_CALLBACK_URL` · redirect-URI: `{base}/api/settings/integrations/oauth/callback` (integracje) + `/api/auth/{google,microsoft}/callback` (login) · `integrationOAuthEngine.ts:43-131` (providerzy+scopy), `Config.ts:35-130` | P2 (rozszerzenie, NIE bloker odbioru) | 3 | **OTWARTA — po stronie Piotra (sekrety w Google Cloud Console + Azure Portal; ja zgłaszam, nie ustawiam).** Przeniesione z M03 (tam lokalnie „L-07 OAuth"). Bez tego: connectory pokazują „Coming soon"/nie startują OAuth; reszta app działa (kalendarz = wydarzenia wewnętrzne + ICS export). Realna praca synchronizacji (kalendarze+taski+chmury-do-czata) **konwerguje tu, w M25/Integrations** — nie w modułach-konsumentach. |
