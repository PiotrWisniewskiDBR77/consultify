...
### 07 · Log — 2026-06-17 (Harvard 4 runda 3) **AUDYT DEAD-CODE (gotowy do wykonania, read-only-verified):** klaster martwy w `src/components/Economics/` zakorzeniony w superseded `EconomicsHub.tsx` (0 ref — live entry = `EconomicsView.tsx`→`FinanceHub`) + martwy barrel `Economics/index.ts` (0 konsumentów). Do usunięcia ~18 plików: `EconomicsHub.tsx`, `index.ts`, `FinancialAnalysisPanel.tsx` + liście (`BenefitsTrackingDashboard`, `BusinessCaseGenerator`, `CashFlowChart`, `FinancialInputForm`, `FinancialMetricsPanel`, `InitiativeLinkingPanel`, `SensitivityChart`), `AIRecommendationsPanel`, `AnalysisCompareView`, `AnalysisCreateModal`, `AnalysisResultsPanel`, `DigitizationToolTab`, `PDFExportModal`, `AnalysisCatalog`, `Economics/EvidencePanel`, `Economics/VersionHistoryPanel`. Roots zweryfikowane 0-ref grepem (static+lazy+dynamic+string). NIE usuwać (LIVE): `ExcelImportWizard`, `InitiativeFinancialIntegration`, `modals/*`, `FinanceLane*`, `FinancePreviewPanel`, `FinanceVersionTimeline`, `FinanceModelDocumentView`. Wykonanie = osobny commit z bramką full-tsc (świadomie odłożone od rundy doc — bulk-rm 18 plików wymaga izolowanej weryfikacji). Dodatkowo: honest „Wkrótce (backend)" w `hooks/useFinanceRowActions.ts:321` = brak endpointu archive (backend ticket, nie cleanup). — 2026-06-17 (runda 2): i18n recount `Economics/` = **129** inline-ternar `isPl?:`/`isPolish?:` (FinanceHub.tsx 19) — to dług słownikowy z L-04 (wcześniej „~125"), NIE brakujące klucze locale; migracja wymaga `public/locales` (strefa zakazana wg AGENT_MAP) → report-only, blokada na decyzję Piotra + dostęp do locales (analog [[M04 L-11]]). Pozostałe luki M16 = 0 otwartych kodowo (L-01..L-07 zamknięte/N-D/DP). — 2026-06-13: brak uwag żywych (jawnie); teczka pogłębiona do M13-level (F Gherkin, DCF/WACC anchory + fałszywa zieleń zweryfikowana, DP-6/8 wpięte). **R3: `e3945bc7fc` zweryfikowany w git log** (legacy IDOR); **korekta i18n** (`isPolish` Economics = 0, WP zawyżał 19); `b5de79ef03` (placeholder usunięty). Re-ocena po Fazie 2/4.
