...
| L-05 | DeckBuilder 25× `isPolish` (grep całość 30×) | W-01 | `Presentations/*` (grep 2026-06-13=30) | P2 | 4 | **ZABLOKOWANA (Fala 4)** — `public/locales/*` ZAKAZANE w Fali 1; wymaga koordynacji z agentem i18n (inny commit); scope: sweep `t()` przez cały katalog `src/components/Presentations/` |  |
| L-06 | hardkody kolorów (127 hex grep — część legitna render) | W-01 | `Presentations/*` (grep 2026-06-17=139) | P3 | 4 | **ZAMKNIĘTA jako LEGALNE** — pomiar 139 hex (nie 127); 100% data-viz/deck-theme/brand-palette wg DP-8 (wizard/types.ts 117 motywy+chartPalette, useCollaboration 12 kursory, DeckThemeContext 5 + ThemeSwitcher 4 model DeckTheme, DeckBuilder 1 gradient cover); 0 chrome; 0 zamienione — przekraczają granicę app→eksport (PPTX/PDF nie rozwiąże `var(--)`), muszą być literalne | 2026-06-17 |
| L-07 | 15/21 testów p20 fałszywa zieleń + S4/S5 niezweryfikowane | W-01 | `evidence/f2_tests_report.md` | P0-test | 1 | **S4+S5 ZAMKNIĘTE** deterministycznie: S5 422-gate (`export-quality-gate.regression.test.ts`), **S4 round-trip** `deck-version-roundtrip.contract.test.ts` (6/6, mutation-verified: autosave→snapshot→restore na realnym SQL przez DbPromise+SQLite seam, realny router via supertest, bez live serwera). Pozostaje: 15 vacuous network-testów p20 (wymaga caboose, §06) | 2026-06-17 |
