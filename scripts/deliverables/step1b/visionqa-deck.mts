/**
 * STEP 1b — deck-visual VisionQA scorer (the `deck-visual` rubric the spec calls
 * for: "czytelność, hierarchia, balans, brak przepełnień, różnorodność").
 *
 * Scores each rendered slide PNG (BEFORE heuristic vs AFTER composition) with a
 * vision model and prints a before/after comparison + a deck-level variety score.
 * This is the additive `deck-visual` rubric — the existing presentationVisionQA
 * Service only scores AI IMAGES, not full slide layouts, so this is its sibling.
 *
 * Reuses the same Anthropic vision plumbing as presentationVisionQAService, but
 * reads LOCAL PNG files (base64) rather than URLs, since our slides are rendered
 * on disk.
 *
 ...
 *
 * Output JSON: docs/qa/deliverables/runs/2026-07-04-step1b/visionqa-results.json
 */
...
const runDir = path.resolve(__dirname, '../../../docs/qa/deliverables/runs/2026-07-04-step1b');
...
3. balance       — balans/kompozycja (rozkład treści na płótnie, wykorzystanie przestrzeni, brak dużych pustek)
...
(async () => {
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error('ERROR: ANTHROPIC_API_KEY not set. See README.md for the railway pull command.');
    process.exit(2);
  }
  ...
  console.log('\n=== deck-visual VisionQA — BEFORE (heuristic) vs AFTER (composition) ===');
  console.log(`  overall  BEFORE=${avgBefore.toFixed(3)}  AFTER=${avgAfter.toFixed(3)}  Δ=${(avgAfter - avgBefore >= 0 ? '+' : '') + (avgAfter - avgBefore).toFixed(3)}`);
  for (const d of dims) {
    const b = perDim(before, d);
    const a = perDim(after, d);
    console.log(`  ${d.padEnd(12)} BEFORE=${b.toFixed(3)}  AFTER=${a.toFixed(3)}  Δ=${(a - b >= 0 ? '+' : '') + (a - b).toFixed(3)}`);
  }

  const out = {
    generatedAt: new Date().toISOString(),
    model: 'claude-sonnet-4-6',
    topic: 'VTS Group — diagnoza transformacji cyfrowej',
    compositionSource: 'B1 planDeckLayout (claude-sonnet-4-6, PREMIUM) captured 2026-07-04',
    summary: {
  ...
  console.error('[visionqa] FAILED:', err);
  process.exit(1);
});
