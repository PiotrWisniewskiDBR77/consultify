...
if (!hasUrl && !hasParts) {
  console.error(
    `[dev] ${envFile} must contain either DATABASE_URL=... or DB_HOST/DB_NAME/DB_USER/DB_PASSWORD (and optionally DB_PORT/DB_SSL).`
  );
  process.exit(1);
}
...
if (databaseUrl) {
  if (raw.includes('USER:PASSWORD@HOST') || raw.includes('postgresql://USER:PASSWORD@HOST')) {
    console.error(
      `[dev] ${envFile} still contains placeholder DATABASE_URL. Please paste the real staging URL.`
    );
    process.exit(1);
  }
  if (!/^postgres(ql)?:\/\//i.test(databaseUrl)) {
    console.error(`[dev] ${envFile} DATABASE_URL doesn't look like Postgres.`);
    process.exit(1);
  }
  if (/\.railway\.internal(?::\d+)?\b/i.test(databaseUrl)) {
    console.error(
      `[dev] ${envFile} DATABASE_URL points to *.railway.internal (private Railway network).`
    );
    console.error(
