...
const reqs = [];
page.on('response', async (r) => { if (r.status() >= 400) reqs.push(r.status() + ' ' + r.url()); });
...
console.log('FAILED_REQS', JSON.stringify(reqs));
try {
  const assignRaw = await page.evaluate(async () => {
    const r = await fetch('/api/v8/interview/assignments/my', { credentials: 'include' });
    return { status: r.status, body: await r.text() };
  });
  console.log('ASSIGNMENTS_MY', JSON.stringify(assignRaw));
} catch (e) { console.log('ASSIGNMENTS_MY_ERR', String(e)); }
