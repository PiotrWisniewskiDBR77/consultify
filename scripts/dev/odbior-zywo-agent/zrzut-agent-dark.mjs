...
const ctx = await browser.newContext({ storageState: auth, viewport: { width: 1440, height: wysokosc }, colorScheme: 'dark', locale: 'pl-PL' });
...
await ctx.addInitScript(() => {
  try {
    ...
    obj.state = { ...(obj.state || {}), theme: 'dark' };
...
await page.goto('http://localhost:3042' + url, { waitUntil: 'domcontentloaded' });
