...
// KOPIA ROBOTNIKA (port 3097 — własny vite, żeby nie kolidować z innymi rękami).
// `storageState.origins[].localStorage` wraca w Playwright TYLKO dla origin,
// który zgadza się CO DO PORTU, więc przepisujemy origin na kopii w pamięci
// (plik sesji nietknięty). Ciasteczka nie mają portu w domenie.
const sesja = JSON.parse(fs.readFileSync(auth, 'utf8'));
sesja.origins = (sesja.origins || []).map((o) => ({ ...o, origin: String(o.origin).replace('http://localhost:3000', 'http://localhost:3097') }));
...
await page.goto('http://localhost:3097' + url, { waitUntil: 'domcontentloaded', timeout: 180000 });
