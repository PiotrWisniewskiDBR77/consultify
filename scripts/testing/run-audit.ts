...
function runTestLevel(level: (typeof testLevels)[0]): AuditResult {
  ...
  console.log(
    `${colors.bold}▶ ${level.level}: ${level.name} Tests${colors.reset} (${fileCount} files)`
  );
...
function generateHtmlReport(summary: AuditSummary): string {
  const levelRows = summary.levels
    .map(
      (l) => `
...
`);
  ...
  const levelsToRun = options.quick ? testLevels.filter((l) => l.quick) : testLevels;

  console.log(
    `${colors.bold}Mode:${colors.reset} ${options.quick ? 'Quick (L1-L2)' : 'Full (L1-L5)'}`
  );
...
`);
  ...
  for (const r of results) {
    const status =
      r.failed === 0 ? `${colors.green}✓${colors.reset}` : `${colors.red}✗${colors.reset}`;
    console.log(
      `  ${status} ${r.level} ${r.name.padEnd(15)} ${colors.green}${r.passed}${colors.reset}/${colors.red}${r.failed}${colors.reset} (${r.passRate})`
    );
  }
