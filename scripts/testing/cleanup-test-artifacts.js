 ...
 */
const cleanupConfig = {
  // Test database patterns (specific locations)
  testDatabases: [
    // Legacy/orphan DBs in project root
    {
      description: 'Project-root test DBs (test-*.db)',
      pattern: /^test-.*\.db$/,
      location: projectRoot,
    },
    // Orphan SQLite DBs created by some unit tests (seen to accumulate to >1GB)
    {
      description: 'Unit-test SQLite artifacts (*.test.db) in tests/unit/backend/services',
      pattern: /^.*\.test\.db$/,
      location: path.join(projectRoot, 'tests', 'unit', 'backend', 'services'),
    },
  ],
...
function cleanTestDatabases() {
  ...
  try {
    for (const rule of cleanupConfig.testDatabases) {
      if (!fs.existsSync(rule.location)) continue;

      const entries = fs.readdirSync(rule.location);
      for (const entry of entries) {
        if (!rule.pattern.test(entry)) continue;

        const filePath = path.join(rule.location, entry);
        try {
          const stats = fs.statSync(filePath);
          if (!stats.isFile()) continue;
          files.push({ path: filePath, size: stats.size, name: entry, rule: rule.description });
    ...
    console.log(
      `Found ${colors.yellow}${count}${colors.reset} test databases (${formatBytes(totalSize)})\n`
    );

    if (verbose) {
      for (const file of files.slice(0, 10)) {
        console.log(
          `  ${colors.yellow}•${colors.reset} ${file.rule}: ${file.name} (${formatBytes(file.size)})`
        );
      }
      if (files.length > 10) {
    ...
    if (!dryRun) {
      for (const file of files) {
        try {
          ...
          console.error(
            `${colors.red}✗ Failed to delete ${file.name}: ${err.message}${colors.reset}`
          );
        }
      }
      console.log(
        `${colors.green}✓ Deleted ${count} test databases (freed ${formatBytes(totalSize)})${colors.reset}`
      );
    } else {
      console.log(
        `${colors.yellow}[DRY RUN] Would delete ${count} files (${formatBytes(totalSize)})${colors.reset}`
      );
    }
...
function cleanTempDirs() {
  ...
  for (const relativePath of cleanupConfig.tempDirs) {
    ...
    try {
      ...
      console.error(
        `${colors.red}✗ Failed to remove ${relativePath}: ${err.message}${colors.reset}`
      );
    }
  }
...
`);
  ...
  if (dryRun) {
    console.log(
      `${colors.yellow}${colors.bold}DRY RUN MODE${colors.reset} - No files will be deleted\n`
    );
  }
