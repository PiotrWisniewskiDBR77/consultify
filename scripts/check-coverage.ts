...
function checkCoverage(): void {
  try {
    ...
    console.log(
      `Functions: ${coverageMetrics.functions.toFixed(2)}% (threshold: ${THRESHOLDS.functions}%)`
    );
    console.log(
      `Branches: ${coverageMetrics.branches.toFixed(2)}% (threshold: ${THRESHOLDS.branches}%)`
    );
    console.log(
      `Statements: ${coverageMetrics.statements.toFixed(2)}% (threshold: ${THRESHOLDS.statements}%)\n`
    );
